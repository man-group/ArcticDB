...
def escape_for_benchmark_filter(name: str) -> str:
    # Google Benchmark uses std::regex with ECMAScript syntax, where '-' is not
    # a metacharacter outside character classes. Python's re.escape still
    # escapes '-' to '\-', which std::regex rejects as "Unexpected escape
    # character" — the filter fails to compile, no benchmarks run, and the
    # binary exits 0 with empty stdout.
