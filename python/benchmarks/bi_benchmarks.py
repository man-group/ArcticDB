...
def get_query_groupby_city_count_isin_filter(
    ...
    return (
        q[q["Keyword"].isin(["kimbo", "tato", "maggot"])]
        .groupby("City")
        .agg({"Keyword": "count"})
    )
...
def get_query_groupby_city_count_filter_two_aggregations(
    ...
    return (
        q[q["Keyword"] == "maggot"]
        .groupby("City")
        .agg({"Keyword": "count", "Number of Records": "sum"})
    )
...
def assert_frame_equal(pandas_df: pd.DataFrame, arctic_df: pd.DataFrame):
    ...
    test.assert_frame_equal(
        pandas_df, arctic_df, check_column_type=False, check_dtype=False
    )
...
#         file = os.path.join(
#             Path(__file__).resolve().parent.parent, BIBenchmarks.CITY_BI_FILE2
#         )
...
#         print(
#             "If pandas query produces different dataframe than arctic one stop tests!"
#         )
...
#         _df = self.df.copy(deep=True)
#         arctic_df = self.time_query_groupby_city_count_isin_filter(
#             BIBenchmarks.params[0]
#         )
#         _df = get_query_groupby_city_count_isin_filter(_df)
#         assert_frame_equal(_df, arctic_df)

#         print(
#             "Pre-check correctness for query_groupby_city_count_filter_two_aggregations"
#         )
#         _df = self.df.copy(deep=True)
#         arctic_df = self.time_query_groupby_city_count_filter_two_aggregations(
#             BIBenchmarks.params[0]
#         )
...
#     def peakmem_query_groupby_city_count_isin_filter(
#         self, times_bigger
#     ) -> pd.DataFrame:
#         return self.query_groupby_city_count_isin_filter(times_bigger)

#     def query_groupby_city_count_filter_two_aggregations(
#         self, times_bigger
#     ) -> pd.DataFrame:
#         q = QueryBuilder()
...
#     def time_query_groupby_city_count_filter_two_aggregations(
#         self, times_bigger
#     ) -> pd.DataFrame:
