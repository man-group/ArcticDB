...
class MongoReadWrite(AsvBase):
    """
    This class is for general read write tests using MongoDB storage
    ...
    params = [100_000, 500_000]  # Smaller params for mongo to start with

    library_manager = TestLibraryManager(storage=Storage.MONGO, name_benchmark="READ_WRITE")

    def get_logger(self) -> Logger:
        return _get_logger(self)

    def get_library_manager(self) -> TestLibraryManager:
        return MongoReadWrite.library_manager

    def get_population_policy(self) -> LibraryPopulationPolicy:
        lpp = LibraryPopulationPolicy(self.get_logger(), AllColumnTypesGenerator()).set_parameters(MongoReadWrite.params)
        return lpp
...
class MongoWideDataFrameTests(MongoReadWrite):
    """
    This class is for general read write tests on MongoDB with wide dataframes
    ...
    library_manager = TestLibraryManager(
        ...
        name_benchmark="READ_WRITE_WIDE",
    ...
    #       of rows than what you defined in the test. To resolve this problem check with documentation:
    #           https://github.com/man-group/ArcticDB/wiki/ASV-Benchmarks:-Real-storage-tests
    params = [5000, 10000]  # Smaller params for mongo to start with

    number_rows = 1000  # Smaller for mongo

    def get_library_manager(self) -> TestLibraryManager:
        return MongoWideDataFrameTests.library_manager

    def get_population_policy(self) -> LibraryPopulationPolicy:
        lpp = LibraryPopulationPolicy(self.get_logger())
        lpp.set_parameters(MongoWideDataFrameTests.number_rows, MongoWideDataFrameTests.params)
        return lpp
...
class MongoReadWriteWithQueryStats(MongoReadWrite):
    """
    This class inherits from MongoReadWrite and always runs with query_stats enabled
    """
    ...
    timeout = 1200
    param_names = ["num_rows"]
    params = [100_000, 500_000]  # Smaller params for mongo

    library_manager = TestLibraryManager(storage=Storage.MONGO, name_benchmark="READ_WRITE_QUERY_STATS")

    def get_library_manager(self) -> TestLibraryManager:
        return MongoReadWriteWithQueryStats.library_manager

    def get_population_policy(self) -> LibraryPopulationPolicy:
        lpp = LibraryPopulationPolicy(self.get_logger(), AllColumnTypesGenerator()).set_parameters(
            MongoReadWriteWithQueryStats.params
        )
        return lpp
