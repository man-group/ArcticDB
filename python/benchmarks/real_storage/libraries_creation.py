...
class LibrariesBase(ABC):
    ...
    def get_arctic(self):
        if self.ac is None:
            if (self.type == Storage.AMAZON):
                self.ac = Arctic(AWS_URL_TEST)
            elif (self.type == Storage.LMDB):
                temp_dir = tempfile.gettempdir()
                #self.ac = Arctic(f"lmdb://{temp_dir}/benchmarks")
                self.ac = Arctic(f"lmdb://benchmarks")
    ...
    def get_library(self, num_symbols: int = 1) -> Library:
        ...
        if lib_opts is None:
            return ac.get_library(self.get_library_names(num_symbols)[0], create_if_missing=True)
        else:
            return ac.get_library(self.get_library_names(num_symbols)[0], create_if_missing=True, 
    ...
    def get_modifyable_library(self, num_symbols: int = 1) -> Library:
        ...
        lib_opts = self.get_library_options()
        if lib_opts is None:
            return ac.get_library(self.get_library_names(num_symbols)[1], create_if_missing=True)
        else:
            return ac.get_library(self.get_library_names(num_symbols)[1], create_if_missing=True, 
                                  library_options=lib_opts)
