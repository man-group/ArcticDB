...
def test_tail_negative_num_rows_greater_than_table_length(lmdb_version_store_tiny_segment, one_col_df, any_output_format):
    # one_col_df generates a dataframe with 10 rows
    lmdb_version_store_tiny_segment.set_output_format(any_output_format)
    generic_tail_test(
...
def test_tail_default_num_rows(lmdb_version_store_tiny_segment, one_col_df, any_output_format):
    ...
    expected = one_col_df().tail(num_rows)
    actual = lmdb_version_store_tiny_segment.tail(symbol).data
    assert_frame_equal_with_arrow(expected, actual)
...
def test_tail_with_column_filter(lmdb_version_store_tiny_segment, three_col_df, any_output_format):
    ...
    columns = ["x", "z"]
    expected = three_col_df().filter(items=columns).tail(num_rows)
    actual = lmdb_version_store_tiny_segment.tail(symbol, num_rows, columns=columns).data
    assert_frame_equal_with_arrow(expected, actual)
