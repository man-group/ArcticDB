...
from arcticdb.util.test import assert_frame_equal_with_arrow, convert_arrow_to_pandas_and_remove_categoricals
...
def test_lazy_read(lmdb_library):
    lib = lmdb_library
    lib._nvs.set_output_format(OutputFormat.EXPERIMENTAL_ARROW)
    sym = "test_lazy_read"
    df = pd.DataFrame(
...
def test_lazy_groupby(lmdb_library):
    ...
    received = convert_arrow_to_pandas_and_remove_categoricals(lazy_df.collect().data)
