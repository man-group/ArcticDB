...
def test_sparse_arrow_date_range(
    ...
    expected = table.slice(offset=offset, length=date_range_width + 1)
    if use_query_builder:
...
def test_sparse_column_selection(version_store_factory, column_group_size):
    ...
    table = pa.table(
        {
            ...
            "b": pa.array([None, 2.0, None], pa.float64()),
            "c": pa.array([True, None, False], pa.bool_()),
        }
    )
    lib.write(sym, table)
    received = lib.read(sym, columns=["a", "c"]).data
    expected = pa.table(
        {
            "a": pa.array([1, None, 3], pa.int64()),
            "c": pa.array([True, None, False], pa.bool_()),
    ...
    assert expected.equals(received)
    received_pandas = lib.read(sym, columns=["a", "c"], output_format="PANDAS").data
    assert_frame_equal_with_arrow_for_sparse(expected, received_pandas)


@pytest.mark.xfail(
    reason="columns parameter combined with row_range not supported for Arrow-written data (monday: 11432991054)"
)
@pytest.mark.parametrize("column_group_size", [1, 2, 3])
def test_sparse_column_selection_with_row_range(version_store_factory, column_group_size):
    lib = version_store_factory(column_group_size=column_group_size)
    lib.set_output_format("pyarrow")
    lib._set_allow_arrow_input()
    sym = "test_sparse_col_sel_row_range"
    table = pa.table(
        {
    ...
    lib.write(sym, table)
    received = lib.read(sym, columns=["b"], row_range=(1, 4)).data
    expected = pa.table({"b": pa.array([2.0, None, 4.0], pa.float64())})
    assert expected.equals(received)


# -- QueryBuilder processing --
...
class TestSparseArrowQueryBuilder:
    ...
    def test_filter_isin(self, lmdb_version_store_arrow):
        q = QueryBuilder()
        q = q[q["int_col"].isin([1, 5, 7])]
        received = lmdb_version_store_arrow.read(self.sym, query_builder=q).data
        mask = pc.is_in(self.table.column("int_col"), pa.array([1, 5, 7], pa.int64()))
        expected = self.table.filter(mask)
        assert expected.equals(received)
        received_pandas = lmdb_version_store_arrow.read(
            self.sym, query_builder=q, output_format="PANDAS"
    ...
    def test_project_add_val(self, lmdb_version_store_arrow):
        q = QueryBuilder()
        q = q.apply("new_col", q["int_col"] + 10)
        received = lmdb_version_store_arrow.read(self.sym, query_builder=q).data
        new_col = pc.add(self.table.column("int_col"), 10)
        expected = self.table.append_column("new_col", new_col)
        assert expected.equals(received)
        received_pandas = lmdb_version_store_arrow.read(
            self.sym, query_builder=q, output_format="PANDAS"
        ).data.reset_index()
...
def test_sparse_append_roundtrip(lmdb_version_store_arrow, write_sparse, append_sparse):
    ...
    # Use complementary null patterns across columns to test orthogonal sparsity
    if write_sparse:
        t1 = pa.table(
            {
...
# -- Dynamic schema with sparse data --
...
def test_sparse_dynamic_schema_combined(version_store_factory):
    ...
    expected = pa.table(
        {
            "int_to_float_col": pa.array([1.0, None, 3.0, None, 5.0, None], pa.float64()),
            "bool_col": pa.array([True, None, False, None, None, None], pa.bool_()),
            "str_col": pa.array([None, None, None, "d", None, "f"], pa.string()),
        }
    )
    assert expected.equals(received)
    received_pandas = lib.read(sym, output_format="PANDAS").data
