...
class TestMergeOracleBasic:
    ...
    def test_empty_source(self):
        target = pd.DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0]}, index=pd.date_range("2024-01-01", periods=3))
        source = pd.DataFrame({"a": pd.array([], dtype="int64"), "b": pd.array([], dtype="float64")}, index=pd.DatetimeIndex([]))
        result = merge(target, source, STRATEGY)
        assert_frame_equal(result, target)
...
class TestMergeOracleOnColumn:
    ...
    def test_on_multiple_columns(self):
        target = pd.DataFrame(
            {"a": [1, 2, 3, 4], "b": ["a", "b", "c", "d"], "c": ["A", "B", "A", "C"], "d": [10.1, 20.2, 30.3, 40.4], "e": [100, 200, 300, 400]},
            index=pd.date_range("2024-01-01", periods=4),
        )
        source = pd.DataFrame(
            {"a": [10, 20, 30, 40, 50], "b": ["a", "b", "c", "d", "e"], "c": ["A", "D", "A", "B", "C"], "d": [10.1, 50.5, 30.3, 40.4, 70.7], "e": [100, 500, 600, 400, 800]},
    ...
    def test_all_columns_in_on_returns_unchanged(self):
        target = pd.DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0]}, index=pd.date_range("2024-01-01", periods=3))
        source = pd.DataFrame(
            {"a": [1, 2, 3], "b": [99.0, 99.0, 99.0]}, index=pd.date_range("2024-01-01", periods=3)
        )
    ...
    def test_on_empty_list_same_as_none(self):
        target = pd.DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0]}, index=pd.date_range("2024-01-01", periods=3))
        source = pd.DataFrame(
            {"a": [10, 20], "b": [10.0, 20.0]}, index=pd.DatetimeIndex(["2024-01-01", "2024-01-05"])
        )
...
class TestMergeOracleWithRepeatedIndex:
    ...
    def test_repeated_index_with_on_columns(self):
        target = pd.DataFrame(
            ...
            index=pd.DatetimeIndex([pd.Timestamp(0), pd.Timestamp(1), pd.Timestamp(1), pd.Timestamp(1), pd.Timestamp(2)]),
        )
        source = pd.DataFrame(
