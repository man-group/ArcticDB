...
class TestResamplingOffset:
    ...
    def test_offset_smaller_than_freq(self, lmdb_version_store_v1, closed, offset):
        ...
        df = pd.DataFrame({"col": rng.integers(0, 100, len(idx))}, index=idx)
        lib.write(sym, df)
        generic_resample_test(lib, sym, "2min", all_aggregations_dict("col"), closed=closed, offset="30s")
    ...
    def test_offset_larger_than_freq(self, lmdb_version_store_v1, closed, offset):
        ...
        df = pd.DataFrame({"col": rng.integers(0, 100, len(idx))}, index=idx)
        lib.write(sym, df)
        generic_resample_test(lib, sym, "2min", all_aggregations_dict("col"), closed=closed, offset=offset)
    ...
    def test_with_date_range(self, lmdb_version_store_v1, closed, date_range, offset):
        ...
        lib.write(sym, df)
        generic_resample_test(
            lib, sym, "2min", all_aggregations_dict("col"), closed=closed, offset=offset, date_range=date_range
        )
...
class TestResamplingOrigin:
    ...
    def test_origin(self, lmdb_version_store_v1, closed, origin):
        ...
        df = pd.DataFrame({"col": rng.integers(0, 100, len(idx))}, index=idx)
        lib.write(sym, df)
        generic_resample_test(lib, sym, "2min", all_aggregations_dict("col"), closed=closed, origin=origin)
    ...
    def test_origin_is_multiple_of_freq(self, lmdb_version_store_v1, closed, origin, date_range):
        ...
        lib.write(sym, df)
        generic_resample_test(
            lib, sym, "2min", all_aggregations_dict("col"), closed=closed, origin=origin, drop_empty_buckets_for="col"
        )
