...
from arcticdb.exceptions import ErrorCode, SchemaException, UserInputException
...
class TestBasicReadIndex:
    def test_read_index_columns(self, in_memory_library_tiny_segment_static_dynamic, index):
    ...
    def test_read_index_column_and_row_slice(self, in_memory_library_tiny_segment_static_dynamic, index):
    ...
    def test_read_index_columns_head(self, in_memory_library_tiny_segment_static_dynamic, index, n):
        lib = in_memory_library_tiny_segment_static_dynamic
    ...
    @pytest.mark.parametrize("n", [3, -3])
    def test_read_index_columns_tail(self, in_memory_library_tiny_segment_static_dynamic, index, n):
        lib = in_memory_library_tiny_segment_static_dynamic
        lib.write("sym", pd.DataFrame({"col": range(0, len(index))}, index=index))
...
class TestReadEmptyIndex:
    def test_empty_range_index(self, in_memory_library_tiny_segment_static_dynamic):
        lib = in_memory_library_tiny_segment_static_dynamic
    ...
    def test_empty_datetime_index(self, in_memory_library_tiny_segment_static_dynamic):
        lib = in_memory_library_tiny_segment_static_dynamic
    ...
    def test_empty_multiindex(self, in_memory_library_tiny_segment_static_dynamic, input_index, expected_index):
        lib = in_memory_library_tiny_segment_static_dynamic
...
class TestReadIndexAsOf:
    ...
    def test_as_of_version(self, in_memory_library_tiny_segment_static_dynamic, indexes):
    ...
    def test_as_of_snapshot(self, in_memory_library_tiny_segment_static_dynamic, index):
...
class TestReadIndexRange:
    ...
    def test_date_range_throws(self, in_memory_library_tiny_segment_static_dynamic, non_datetime_index):
        lib = in_memory_library_tiny_segment_static_dynamic
...
class TestWithNormalizers:
    ...
    def test_recursive_filter_throws(
        ...
        q = q[q["a"] == 0]
        with pytest.raises(
            SchemaException, match=ErrorCode.E_OPERATION_NOT_SUPPORTED_WITH_RECURSIVE_NORMALIZED_DATA.name
        ):
    ...
    def test_numpy_array_head_throws(self, in_memory_library_tiny_segment_static_dynamic):
        ...
        with pytest.raises(SchemaException, match=ErrorCode.E_OPERATION_NOT_SUPPORTED_WITH_NUMPY_ARRAY.name):
            lib.head("sym_np", n=5)
...
class TestPickled:
    def test_throws(self, in_memory_library_tiny_segment_static_dynamic):
        lib = in_memory_library_tiny_segment_static_dynamic
