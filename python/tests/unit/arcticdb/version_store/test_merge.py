...
class MergeStrategy(NamedTuple):
    ...
    not_matched: MergeAction = MergeAction.DO_NOTHING
...
# lib.merge("sym", source, strategy=MergeStrategy(matched=MergeAction.DO_NOTHING, not_matched=naMergeAction.INSERT))
...
class TestMergeTimeseries:
    ...
    def test_static_schema_merge_throws_when_column_names_differ(self, lmdb_library, monkeypatch):
        lib = lmdb_library
        target = pd.DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0]}, index=pd.date_range("2024-01-01", periods=3))
        lib.write("sym", target)
    ...
    def test_merge_update_throws_when_target_row_is_matched_more_than_once(self, lmdb_library, monkeypatch):
        lib = lmdb_library
        target = pd.DataFrame({"a": [1, 2, 3], "b": [1.0, 2.0, 3.0]}, index=pd.date_range("2024-01-01", periods=3))
        lib.write("sym", target)
