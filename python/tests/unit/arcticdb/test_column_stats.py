...
def generate_symbol(lib, sym):
    ...
    expected_column_stats["MIN(col_1)"] = [df0["col_1"].min(), df1["col_1"].min()]
    expected_column_stats["MAX(col_1)"] = [df0["col_1"].max(), df1["col_1"].max()]
    expected_column_stats["MIN(col_2)"] = [df0["col_2"].min(), df1["col_2"].min()]
    expected_column_stats["MAX(col_2)"] = [df0["col_2"].max(), df1["col_2"].max()]
...
def test_column_stats_basic_flow(lmdb_version_store_tiny_segment, any_output_format):
    ...
    expected_column_stats.drop(
        expected_column_stats.columns.difference(["start_index", "end_index", "MIN(col_1)", "MAX(col_1)"]),
        axis=1,
        inplace=True,
    )
...
def test_column_stats_infinity(lmdb_version_store_tiny_segment, any_output_format):
    ...
    expected_column_stats["MIN(col_1)"] = [df0["col_1"].min(), df1["col_1"].min(), df2["col_1"].min()]
    expected_column_stats["MAX(col_1)"] = [df0["col_1"].max(), df1["col_1"].max(), df2["col_1"].max()]
    column_stats_dict = {"col_1": {"MINMAX"}}
...
def test_column_stats_dynamic_schema_missing_data(lmdb_version_store_tiny_segment_dynamic, any_output_format):
    ...
    expected_column_stats["MIN(col_1)"] = [
        df0["col_1"].min(),
        np.nan,
        df2["col_1"].min(),
        np.nan,
    ...
    expected_column_stats["MAX(col_1)"] = [
        df0["col_1"].max(),
        np.nan,
        df2["col_1"].max(),
        np.nan,
    ...
    expected_column_stats["MIN(col_2)"] = [
        df0["col_2"].min(),
        df1["col_2"].min(),
        np.nan,
        np.nan,
    ...
    expected_column_stats["MAX(col_2)"] = [
        df0["col_2"].max(),
        df1["col_2"].max(),
        np.nan,
        np.nan,
...
def test_column_stats_dynamic_schema_types_changing(lmdb_version_store_tiny_segment_dynamic, any_output_format):
    ...
    expected_column_stats = expected_column_stats.iloc[[0, 1]]
    expected_column_stats["MIN(int_widening)"] = [df0["int_widening"].min(), df1["int_widening"].min()]
    expected_column_stats["MAX(int_widening)"] = [df0["int_widening"].max(), df1["int_widening"].max()]

    expected_column_stats["MIN(int_narrowing)"] = [df0["int_narrowing"].min(), df1["int_narrowing"].min()]
    expected_column_stats["MAX(int_narrowing)"] = [df0["int_narrowing"].max(), df1["int_narrowing"].max()]

    expected_column_stats["MIN(unsigned_to_wider_signed_int)"] = [
        df0["unsigned_to_wider_signed_int"].min(),
        df1["unsigned_to_wider_signed_int"].min(),
    ]
    expected_column_stats["MAX(unsigned_to_wider_signed_int)"] = [
        df0["unsigned_to_wider_signed_int"].max(),
        df1["unsigned_to_wider_signed_int"].max(),
    ]

    expected_column_stats["MIN(wider_signed_to_unsigned_int)"] = [
        df0["wider_signed_to_unsigned_int"].min(),
        df1["wider_signed_to_unsigned_int"].min(),
    ]
    expected_column_stats["MAX(wider_signed_to_unsigned_int)"] = [
        df0["wider_signed_to_unsigned_int"].max(),
        df1["wider_signed_to_unsigned_int"].max(),
    ]

    expected_column_stats["MIN(unsigned_to_signed_int_same_width)"] = [
        df0["unsigned_to_signed_int_same_width"].min(),
        df1["unsigned_to_signed_int_same_width"].min(),
    ]
    expected_column_stats["MAX(unsigned_to_signed_int_same_width)"] = [
        df0["unsigned_to_signed_int_same_width"].max(),
        df1["unsigned_to_signed_int_same_width"].max(),
    ]

    expected_column_stats["MIN(int_to_float)"] = [df0["int_to_float"].min(), df1["int_to_float"].min()]
    expected_column_stats["MAX(int_to_float)"] = [df0["int_to_float"].max(), df1["int_to_float"].max()]

    expected_column_stats["MIN(float_to_int)"] = [df0["float_to_int"].min(), df1["float_to_int"].min()]
    expected_column_stats["MAX(float_to_int)"] = [df0["float_to_int"].max(), df1["float_to_int"].max()]
    column_stats_dict = {
        "int_widening": {"MINMAX"},
    ...
    column_stats = lib.read_column_stats(sym)
    assert_stats_equal(column_stats, expected_column_stats)
    assert column_stats.dtypes["MIN(int_widening)"] == np.uint16
    assert column_stats.dtypes["MAX(int_widening)"] == np.uint16

    assert column_stats.dtypes["MIN(int_narrowing)"] == np.int16
    assert column_stats.dtypes["MAX(int_narrowing)"] == np.int16

    assert column_stats.dtypes["MIN(unsigned_to_wider_signed_int)"] == np.int32
    assert column_stats.dtypes["MAX(unsigned_to_wider_signed_int)"] == np.int32

    assert column_stats.dtypes["MIN(wider_signed_to_unsigned_int)"] == np.int32
    assert column_stats.dtypes["MAX(wider_signed_to_unsigned_int)"] == np.int32

    assert column_stats.dtypes["MIN(unsigned_to_signed_int_same_width)"] == np.int32
    assert column_stats.dtypes["MAX(unsigned_to_signed_int_same_width)"] == np.int32

    assert column_stats.dtypes["MIN(int_to_float)"] == np.float64
    assert column_stats.dtypes["MAX(int_to_float)"] == np.float64

    assert column_stats.dtypes["MIN(float_to_int)"] == np.float64
    assert column_stats.dtypes["MAX(float_to_int)"] == np.float64
