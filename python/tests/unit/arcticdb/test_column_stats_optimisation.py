...
def test_column_stats_no_prune_ne_when_nat(in_memory_version_store, clear_query_stats, column_stats_filtering_enabled):
    """Time-type analogue of the in-band NaN case: min=max=ts, nan_count=2."""
    lib = in_memory_version_store

    ts = pd.Timestamp("2025-01-01")
    df = pd.DataFrame(
        {"a": [ts, pd.NaT, pd.NaT, ts]},
...
def test_column_stats_still_prunes_when_nan_does_not_save_row(
    ...
    """Negative control: NaN rows do NOT satisfy `< v`, so NONE_MATCH must stay NONE_MATCH
    and the segment must still be pruned even though nan_count > 0."""
    lib = in_memory_version_store
    ...
    full_df = pd.concat([df0, df1])
    expected = full_df[full_df["a"] < 1.0]
    assert_frame_equal(expected, result)
    assert table_data_reads == 0, "Both segments must still be pruned; NaN < 1.0 is false at row level"
