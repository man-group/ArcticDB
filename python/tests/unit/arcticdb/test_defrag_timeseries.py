...
def generic_defrag_test(lib, sym):
    ...
    col_slices = 1 if dynamic_schema else total_columns // cols_per_slice + (1 if total_columns % cols_per_slice != 0 else 0)
    _defrag_timeseries(lib, sym)
    assert_frame_equal(expected, lib.read(sym).data)
    num_segments = len(lib.read_index(sym))
    assert num_segments == col_slices * ((total_rows // rows_per_slice) + (1 if total_rows % rows_per_slice != 0 else 0))
...
def test_defrag_timeseries_col_sliced(version_store_factory):
    ...
    try:
        ...
        df_1 = pd.DataFrame({"col0": [6, 7], "col1": [8, 9], "col2": [10, 11]}, index=pd.date_range("2025-01-03", periods=2))
        lib.write(sym, df_0)
        lib.append(sym, df_1)
        generic_defrag_test(lib, sym)
    finally:
