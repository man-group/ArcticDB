...
def _enforce_s3_access_key(endpoint, access_key):
    # Make the moto server reject any request not signed with access_key. This state is server-global, so always
    # reset it on exit
...
def test_s3_default_auth_with_profile(s3_storage, lib_name, tmp_path, monkeypatch, route_env_to_extension):
    """aws_profile with the default credentials provider chain, credentials in the AWS *config* file (`[profile X]`),
    matching the boto3-style layout the feature targets."""
    ...
    # route_env_to_extension makes this AWS_CONFIG_FILE visible to the statically-linked AWS SDK on Windows
...
def test_s3_default_auth_with_profile_credentials_file(
    ...
    # route_env_to_extension makes this AWS_SHARED_CREDENTIALS_FILE visible to the statically-linked AWS SDK on Windows
...
@pytest.mark.storage
@pytest.mark.authentication
def test_s3_default_auth_profile_real(lib_name, real_s3_default_profile_storage):
    ac = Arctic(real_s3_default_profile_storage.arctic_uri)
    try:
        lib = ac.create_library(lib_name)
        df = pd.DataFrame({"a": [1, 2, 3]})
        lib.write("sym", df)
...
@pytest.mark.storage
@pytest.mark.authentication
def test_s3_default_auth_profile_credentials_real(lib_name, real_s3_default_profile_credentials_storage):
    # Same as test_s3_default_auth_profile_real but the profile credentials are in the AWS credentials file
    ac = Arctic(real_s3_default_profile_credentials_storage.arctic_uri)
    try:
        lib = ac.create_library(lib_name)
        df = pd.DataFrame({"a": [1, 2, 3]})
        lib.write("sym", df)
