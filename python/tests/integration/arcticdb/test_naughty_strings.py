...
def test_write_naughty_strings(lmdb_version_store):
    strings = read_strings()
    symbol = "naughty_strings_write"
...
def test_append_naughty_strings(lmdb_version_store):
    strings = read_strings()
    symbol = "naughty_strings_append"
    df = create_dataframe(strings)
    half_index = len(df) // 2
...
def test_update_naughty_strings(lmdb_version_store):
    strings = read_strings()
    symbol = "naughty_strings_append"
    df = create_dataframe(strings)
    total_length = len(df)
