...
from arcticdb.util.test import assert_frame_equal, assert_frame_equal_with_arrow, config_context
...
@pytest.mark.parametrize(
    "date_range",
    [
        (pd.Timestamp("2025-01-03 00:00:00.000000000"), pd.Timestamp("2025-01-03 00:00:00.000000000")),
        # (
...
def test_compat_arrow_range_old_updated_data(pandas_v1_venv, s3_ssl_disabled_storage, lib_name, date_range):
    ...
    with CompatLibrary(pandas_v1_venv, arctic_uri, lib_name) as compat:
        ...
        with compat.current_version() as curr:
            ...
            raise Exception(arrow_table, expected_df)
...
@pytest.mark.parametrize(
    ...
    [
        # (pd.Timestamp("2025-01-03 00:00:00.000000000"), pd.Timestamp("2025-01-03 00:00:00.000000000")),
        (pd.Timestamp("2025-01-04 00:00:00.000000000"), pd.Timestamp("2025-01-04 00:00:00.000000000")),
    ],
)
def test_compat_old_date_range(pandas_v1_venv, s3_ssl_disabled_storage, lib_name, date_range):
    # TODO: Add desc
    arctic_uri = s3_ssl_disabled_storage.arctic_uri
    with CompatLibrary(pandas_v1_venv, arctic_uri, lib_name) as compat:
        sym = "sym"
        ...
        with compat.current_version() as curr:
            ...
            with config_context("VersionMap.ReloadInterval", 0):
                # TODO: Fix ARROW
                # arrow_table = curr.lib.read(
                #     sym, date_range=date_range, output_format=OutputFormat.EXPERIMENTAL_ARROW
                # ).data
                expected_df = curr.lib.read(sym, date_range=date_range).data

                # assert_frame_equal_with_arrow(arrow_table, expected_df)
                assert expected_df.shape[0] == 1, f"Expected a single row but got {expected_df}"
