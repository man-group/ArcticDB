...
def generate_random_timestamp_array(size: int, start: str = "2020-01-01", end: str = "2030-01-01", seed: int = 432432):
...
def generate_random_float_array(size: int, dtype: np.floating = np.float32):
...
def generate_random_series(
    ...
    return pd.Series(generate_random_numpy_array(length, type, seed=None), index=index, name=name)


def verify_dynamically_added_columns(
    updated_df: pd.DataFrame, row_index: Union[int, pd.Timestamp], new_columns_to_verify: Set[str]
):
...
def generate_sparse_numpy_array_from_array(size: int, array: np.ndarray, seed: int = 3243253) -> np.ndarray:
...
def generate_random_sparse_numpy_array(
    size: int, dtype, density: float = 0.01, str_length: int = 10, seed: int = 8238
) -> np.ndarray:
    ...
    if pd.api.types.is_float_dtype(dtype):
        ...
        values = np.array(
            ListGenerators.generate_random_strings(
                str_size=str_length, length=num_values, include_unicode=True, seed=seed
            ),
            dtype=np.str_,
        )
    elif pd.api.types.is_datetime64_any_dtype(dtype):
...
def generate_random_series(
    type: ArcticTypes, length: int, name: str, start_time: pd.Timestamp = None, freq: str = "s", seed=3247
) -> pd.Series:
    """Generates random series of specified type with or without index"""
    set_seed(seed)
    index = None
    if start_time:
        index = pd.date_range(start_time, periods=length, freq=freq)
    return pd.Series(generate_random_numpy_array(length, type, seed), index=index, name=name)
...
class RandomStringPool:
    ...
    def __init__(self, str_length: int, pool_size: int, include_unicode: bool = False, seed=3):
        self.__pool = ListGenerators.generate_random_string_pool(
            str_length=str_length, pool_size=pool_size, include_unicode=include_unicode, seed=seed
        )
...
class ListGenerators:
    ...
    @classmethod
    def generate_random_string_pool(
        cls, str_length: int, pool_size: int, include_unicode: bool = False, seed=143
    ) -> List[str]:
        unique_values = set()
        while len(unique_values) < pool_size:
            unique_values.add(
                ListGenerators.random_string(length=str_length, include_unicode=include_unicode, seed=seed)
            )
        return list(unique_values)

    @classmethod
    def generate_random_strings(cls, str_size: int, length: int, include_unicode: bool = False, seed=1434) -> List[str]:
        if seed is not None:
            random.seed(seed)
        return [
            ListGenerators.random_string(length=str_size, include_unicode=include_unicode, seed=None)
            for _ in range(length)
        ]

    @classmethod
    def generate_random_ints(
        cls, dtype: ArcticIntType, size: int, min_value: int = None, max_value: int = None, seed=21321
    ) -> List[ArcticIntType]:
        if seed is not None:
            np.random.seed(seed)
...
class DFGenerator:
    ...
    def add_string_col(
        self, name: str, str_size: int, include_unicode: bool = False, num_unique_values: int = None
    ) -> "DFGenerator":
        """
        ...
        if num_unique_values is None:
            if self.__density:
                list = generate_random_sparse_numpy_array(
                    self.__size, str, self.__density, str_length=str_size, seed=None
                )
            else:
                list = ListGenerators.generate_random_strings(
                    str_size=str_size, length=self.__size, include_unicode=include_unicode, seed=None
                )
        else:
            pool = RandomStringPool(
                str_length=str_size, pool_size=num_unique_values, include_unicode=include_unicode, seed=None
            )
            if self.__density:
                list = generate_sparse_numpy_array_from_array(
                    self.__size, np.array(pool.get_list(int(self.__size * self.__density))), seed=None
                )
            else:
    ...
    def add_timestamp_index(
        ...
        start_time: Union[pd.Timestamp, TimestampNumber] = pd.Timestamp(0),
    ) -> "DFGenerator":
        if isinstance(start_time, TimestampNumber):
    ...
    def generate_random_dataframe(cls, rows: int, cols: int, indexed: bool = True, seed=123):
        ...
        """
        if indexed:
            return cls.generate_normal_dataframe(
                num_rows=rows, num_cols=cols, start_time=pd.Timestamp(0), freq="s", seed=seed
            )
        else:
            return cls.generate_normal_dataframe(num_rows=rows, num_cols=cols, seed=seed)

    @classmethod
    def generate_random_int_dataframe(
        cls,
        start_name_prefix: str,
        num_rows: int,
        num_cols: int,
        dtype: ArcticIntType = np.int64,
        min_value: int = None,
        max_value: int = None,
        seed: int = 3432,
    ) -> pd.DataFrame:
    ...
    @classmethod
    def generate_random_float_dataframe(
        cls,
        start_name_prefix: str,
        num_rows: int,
        num_cols: int,
    ...
    @classmethod
    def generate_random_strings_dataframe(
        cls, start_name_prefix: str, num_rows: int, num_cols: int, string_sizes: List[int] = None, seed: int = 4543
    ):
        """
        To be used to generate large number of same type columns, when generation time is
        critical
        ...
        if seed is not None:
            np.random.seed(seed=seed)
            random.seed(seed)
        data = [[random_string(string_sizes[col]) for col in range(num_cols)] for _ in range(num_rows)]
    ...
    def generate_wide_dataframe(
        ...
        cols, mod = divmod(
            num_cols - num_string_cols, len(supported_int_types_list + supported_float_types_list)
        )  # divide by number of unique frame types

        frames = []
        for dtype in supported_int_types_list:
            frame = cls.generate_random_int_dataframe(
                dtype.__name__, num_rows=num_rows, num_cols=cols, dtype=dtype, seed=seed
            )
            frames.append(frame)

        for dtype in supported_float_types_list:
            frame = cls.generate_random_float_dataframe(
                dtype.__name__, num_rows=num_rows, num_cols=cols, dtype=dtype, seed=seed
            )
            frames.append(frame)
