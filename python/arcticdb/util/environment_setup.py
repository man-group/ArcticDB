...
class StorageSetup:
    ...
    def get_arctic_uri(
        ...
        add_to_prefix: str = None,
        confirm_persistent_storage_need: bool = False,
    ) -> str:
...
class TestLibraryManager:
    ...
    def _get_arctic_client_persistent(self) -> Arctic:
        ...
        return self.__get_arctic_client_internal(storage_space, confirm_persistent_storage_need=True)

    def _get_arctic_client_modifiable(self) -> Arctic:
        return self.__get_arctic_client_internal(StorageSpace.MODIFIABLE, confirm_persistent_storage_need=False)

    def __get_arctic_client_internal(
        self, storage_space: StorageSpace, confirm_persistent_storage_need: bool = False
    ) -> Arctic:
    ...
    def get_library(self, library_type: LibraryType, lib_name_suffix: str = "") -> Library:
        ...
        if library_type == LibraryType.PERSISTENT:
            ...
            return self._get_arctic_client_modifiable().get_library(
                lib_name, create_if_missing=True, library_options=self.library_options
            )
        else:
    ...
    def __clear_all_libs(self, name_starts_with: str = None):
        ...
        if name_starts_with is not None:
            libs_to_delete = [lib_name for lib_name in libs_to_delete if lib_name.startswith(name_starts_with)]
    ...
    def __remove_all_test_libs(cls, ac: Arctic, uri_str_to_confirm: str):
        assert (
            uri_str_to_confirm in ac.get_uri()
        ), f"Expected string [{uri_str_to_confirm}] not found in uri : {ac.get_uri()}"
...
class VariableSizeDataframe(DataFrameGenerator):
    ...
    def get_dataframe(
        ...
        freq: Union[str, timedelta, pd.Timedelta, pd.DateOffset] = None,
        seed=888,
    ):
        ...
        if number_columns < self.wide_dataframe_generation_threshold:
            df = DFGenerator.generate_normal_dataframe(
                num_rows=number_rows, num_cols=number_columns, freq=freq, start_time=start_timestamp, seed=seed
            )
        else:
...
class LibraryPopulationPolicy:
    ...
    def set_parameters(
        self, number_rows: Union[int, List[int]], number_columns: Union[int, List[int]] = 10
    ...
    def _get_versions_list(self, number_symbols: int) -> List[np.int64]:
        if self.versions_max == 1:
            ...
            versions_list = ListGenerators.generate_random_list_with_mean(
                number_elements=number_symbols, specified_mean=self.mean, value_range=(1, self.versions_max), seed=365
            )
...
def populate_library_if_missing(
    manager: TestLibraryManager, policy: LibraryPopulationPolicy, lib_type: LibraryType, lib_name_suffix: str = ""
):
...
class SequentialDataframesGenerator:
    ...
    def generate_sequential_dataframes(
        self,
        ...
        for i in range(number_data_frames):
            df = self.df_generator.get_dataframe(
...
class TestsForTestLibraryManager:
    ...
    def test_modifiable_access(cls):
        ...
        assert (
            lib_name not in tlm._get_arctic_client_persistent().list_libraries()
        ), "Library name not in persistent space"
    ...
    def test_multiprocessing(cls):
        ...
        def worker_process():
            ...
            assert string_list_has_number(
                tlm._get_arctic_client_modifiable().list_libraries(), os.getpid()
            ), "There is a library with that pid"
            tlm.clear_all_modifiable_libs_from_this_process()
            assert not string_list_has_number(
                tlm._get_arctic_client_modifiable().list_libraries(), os.getpid()
            ), "There is NO library with that pid"
