...
class SetupConfig:
    ...
    @classmethod
    def get_aws_s3_arctic_uri(
        cls, prefix: str = MODIFIABLE_LIBS_PREFIX, confirm_persistent_storage_need: bool = False
    ) -> str:
        """
...
class EnvConfigurationBase(ABC):
    ...
    def __init__(
        self, storage: Storage, prefix: str = None, library_options: LibraryOptions = None, uris_cache: List[str] = None
    ):
        """
    ...
    def from_storage_info(cls, data: StorageInfo):
        ...
        """
        return cls(
            storage=data.storage, uris_cache=data.uris_cache, library_options=data.library_options, prefix=data.prefix
        )
    ...
    def get_storage_info(self) -> StorageInfo:
        ...
        return StorageInfo(
            ...
            uris_cache=list(self.ac_cache.keys()),
            library_options=self.__library_options,
            prefix=self.__storage_test_env_prefix,
        )
    ...
    def get_library_names(self, lib_suffix: Union[str, int] = None) -> List[str]:
        ...
        lib_names = [f"PERM_{lib_mid_part_name}_{lib_suffix}", f"MOD_{lib_mid_part_name}_{lib_suffix}"]
        return lib_names
    ...
    def get_library(self, library_suffix: Union[str, int] = None) -> Library:
        ...
        """
        return self._get_lib(self.get_arctic_client_persistent(), self.get_library_names(library_suffix)[0])
    ...
    def get_modifiable_library(self, library_suffix: Union[str, int] = None) -> Library:
        ...
        """
        return self._get_lib(self.get_arctic_client_modifiable(), self.get_library_names(library_suffix)[1])
    ...
    @classmethod
    def get_parameter_from_string(
        cls, string: str, param_indx: int, param_type: Union[int, str] = int
    ) -> Union[int, str]:
        """
...
class GeneralSetupLibraryWithSymbols(EnvConfigurationBase):
    ...
    def setup_all(self) -> "GeneralSetupLibraryWithSymbols":
        ...
        self.logger().info(
            f"TOTAL TIME (setup of one library with specified rows and column symbols): {time.time() - start} sec"
        )
        return self
...
class GeneralSetupSymbolsVersionsSnapshots(EnvConfigurationBase):
    ...
    def get_symbol_name(
        self, number_symbols: int, number_versions: int, with_metadata: bool, with_snapshot: bool
    ) -> str:
        """
        Maps rows and cols to proper symbol name
        This method should be used in ASV tests obtain symbol name based on parameters
...
class GeneralSetupLibraryWithSymbolsTests:
    ...
    def get_general_modifyanle_client(cls):
        setup = GeneralSetupLibraryWithSymbols(
            Storage.AMAZON, prefix=GeneralSetupLibraryWithSymbolsTests.PREFIX_FOR_TEST
        )
    ...
    def get_general_setup_test_mode(cls):
        setup = GeneralSetupLibraryWithSymbols(Storage.LMDB).set_test_mode()
    ...
    def test_setup_versions_and_snapshots(cls):
        setup = (
            GeneralSetupSymbolsVersionsSnapshots(storage=Storage.LMDB, prefix="LIST_SYMBOLS")
