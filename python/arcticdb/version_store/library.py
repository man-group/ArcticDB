...
class Library:
    ...
    def read_index_columns(
        ...
        -------
        VersionedItem object that contains a .data and .metadata element.
        The data element is of type`Union[pd.RangeIndex, pd.DatetimeIndex, pd.MultiIndex]`
        ...
        >>> df = pd.DataFrame({'column': [5,6,7]})
        >>> lib.write("symbol", df)
        >>> lib.read_index_columns("symbol").data
        RangeIndex(start=0, stop=3, step=1)

        >>> df = pd.DataFrame({'column': [5,6,7]}, index=pd.date_range(start="01/01/2024",end="01/3/2024"))
        >>> lib.write("symbol", df)
        >>> lib.read_index_columns("symbol").data
        DatetimeIndex(['2024-01-01', '2024-01-02', '2024-01-03'], dtype='datetime64[ns]', name='index', freq=None)
        """
        return self._nvs.read_index_columns(
            symbol=symbol,
            as_of=as_of,
            date_range=date_range,
            row_range=row_range
