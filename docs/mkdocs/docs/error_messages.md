...
| 7002       | An unsupported character was found in a symbol or library name.                           | We support only the ASCII characters between 32-126 inclusive and exclude `<`, `>` and `*` specifically. Change your name so it contains only valid characters. **If you want to bypass this check for symbol names, you can define an environment variable called - ARCTICDB_VersionStore_NoStrictSymbolCheck_int=1**. |
...
| Timestamp and numeric types cannot be mixed | A timestamp value/column (`pd.Timestamp`, `datetime.datetime`, `datetime.date`, or a `datetime64` column) was compared with, tested for membership against, returned from a `where(...)` alongside, or arithmetically combined with a plain numeric value/column. `timestamp ± integer` is exempt, since it is treated as a nanosecond offset. | Keep timestamp and numeric operands separate. `timestamp ± integer` is still permitted as an offset, and `timestamp - timestamp` (which yields `int64` nanoseconds) can be used instead of mixing in a plain number. |
| ...using the <operator\> operator. Only subtraction is supported between two timestamps. | `+`, `*`, `/`, or `**` was used with two timestamp operands (columns and/or values). ArcticDB has no duration type: `timestamp - timestamp` is the only arithmetic operation defined between two timestamps, yielding `int64` nanoseconds. `<operator\>` is rendered as the Python operator symbol (e.g. `+`), regardless of whether the check runs while the query's schema is computed or while the query is executed against data. | Use `-` between two timestamps for a nanosecond delta; `+`, `*`, `/`, and `**` are not defined between two timestamps. |
