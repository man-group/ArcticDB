...
LibraryTool::LibraryTool(std::shared_ptr<storage::Library> lib) {
  store_ = std::make_shared<async::AsyncStore<util::SysClock>>(
      lib, codec::default_lz4_codec(), encoding_version(lib->config()));
}
...
ReadResult LibraryTool::read(const VariantKey& key) {
  ...
  auto frame_and_descriptor =
      frame_and_descriptor_from_segment(std::move(segment_in_memory));
...
StreamDescriptor LibraryTool::read_descriptor(const VariantKey& key) {
  auto metadata_and_descriptor =
      store_->read_metadata_and_descriptor(key, storage::ReadKeyOpts{}).get();
...
std::vector<VariantKey> LibraryTool::find_keys(entity::KeyType kt) {
  ...
  store_->iterate_type(
      kt, [&](VariantKey&& found_key) { res.emplace_back(found_key); }, "");
  return res;
}
...
std::vector<VariantKey> LibraryTool::find_keys_for_id(entity::KeyType kt,
                                                      const StreamId& stream_id) {
  util::check(std::holds_alternative<StringId>(stream_id),
              "keys for id only implemented for string ids");

  std::vector<VariantKey> res;
  const auto& string_id = std::get<StringId>(stream_id);

  const IterateTypeVisitor& visitor = [&](VariantKey&& found_key) {
    // Only S3 handles the prefix in iterate_type, the others just return everything,
