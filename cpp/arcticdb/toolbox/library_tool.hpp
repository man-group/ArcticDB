...
public:
  ...
  [[nodiscard]] std::optional<google::protobuf::Any>
  read_metadata(const VariantKey& key);
  ...
  std::vector<VariantKey> find_keys_for_id(entity::KeyType kt,
...
private:
  ...
  // The only reason we use a shared_ptr for the store is to be able to pass it to
  // delete_all_keys_of_type. We can remove the shared_ptr when delete_all_keys_of_type
  // takes a const ref instead of a shared pointer.
