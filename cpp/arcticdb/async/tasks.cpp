...
// N.B. Not the same as the filtered descriptor commonly used in allocate_frame, as the
// segment may not contain all the columns in the filter
  ...
  return util::variant_match(index, [&desc, &filter_columns](const auto& idx) {
    if (filter_columns) {
      ...
      for (const auto& field : desc.fields()) {
        if (filter_columns->find(std::string{field.name()}) !=
            std::cend(*filter_columns)) {
...
DecodeSliceTask::decode_into_slice(storage::KeySegmentPair&& key_segment_pair) {
  ...
  ranges_and_key_.col_range_.second =
      ranges_and_key_.col_range_.first +
      (descriptor.field_count() - descriptor.index().field_count());
  ...
  return pipelines::SegmentAndSlice(std::move(ranges_and_key_),
                                    std::move(segment_in_memory));
}

pipelines::SliceAndKey DecodeSlicesTask::decode_into_slice(
    std::pair<Segment, pipelines::SliceAndKey>&& sk_pair) const {
