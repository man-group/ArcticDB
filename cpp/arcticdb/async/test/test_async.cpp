...
TEST(Async, StatsQueryDemo) {
    ...
    auto work = [&]() {
        std::vector<folly::Future<folly::Unit>> stuff;
        {
            ...
            */
            ...
            const auto& add_future_level = instance.root_level()->next_level_maps_[static_cast<size_t>(GroupName::storage_ops)]["AddFuture"];
        ...
        const auto& list_objects_level = instance.root_level()->next_level_maps_[static_cast<size_t>(GroupName::storage_ops)]["AddFuture"]
    ...
    t1.join();
    t2.join();
    auto root_levels = instance.root_levels();
    ASSERT_EQ(root_levels.size(), 2); // Each pseudo-python thread will create one root level
}
