...
TEST(Async, SinkBasic) {
    ...
    as::UserAuth au{"abc"};
    auto lib = library_index.get_library(library_path, as::OpenMode::WRITE, au, as::NativeVariantStorageMap());
...
std::shared_ptr<arcticdb::Store> create_store(const storage::LibraryPath &library_path,
    ...
    auto lib = library_index.get_library(library_path, as::OpenMode::WRITE, user_auth, as::NativeVariantStorageMap());
