...
std::pair<VariantKey, std::optional<Segment>>
lookup_match_in_dedup_map(const std::shared_ptr<DeDupMap>& de_dup_map,
                          storage::KeySegmentPair&& key_seg);

template <typename Callable>
auto read_and_continue(const VariantKey& key, std::shared_ptr<storage::Library> library,
                       const storage::ReadKeyOpts& opts, Callable&& c) {
  return async::submit_io_task(
             ReadCompressedTask{key, library, opts, std::forward<decltype(c)>(c)})
...
/*
 * AsyncStore is a wrapper around a Store that provides async methods for writing and
 * reading data. It is used by the VersionStore to write data to the Store
 * asynchronously. It also can be used to write data synchronously (using the `*_sync`
 * methods) and to write batch of data (using the `batch_*` methods). It can return both
 * uncompressed `SegmentInMemory` or compressed `Segment` depending on the method.
 */
template <class ClockType = util::SysClock> class AsyncStore : public Store {
public:
  AsyncStore(std::shared_ptr<storage::Library> library,
  ...
  folly::Future<entity::VariantKey> write(stream::KeyType key_type,
                                          ...
                                          IndexValue start_index, IndexValue end_index,
                                          SegmentInMemory&& segment) override {

    util::check(segment.descriptor().id() == stream_id,
                "Descriptor id mismatch in atom key {} != {}", stream_id,
                segment.descriptor().id());

    return async::submit_cpu_task(
               EncodeAtomTask{key_type, version_id, stream_id, start_index, end_index,
                              current_timestamp(), std::move(segment), codec_,
                              encoding_version_})
  ...
  write(stream::KeyType key_type, VersionId version_id, const StreamId& stream_id,
        timestamp creation_ts, IndexValue start_index, IndexValue end_index,
        SegmentInMemory&& segment) override {
    ...
    return async::submit_cpu_task(EncodeAtomTask{key_type, version_id, stream_id,
                                                 start_index, end_index, creation_ts,
                                                 std::move(segment), codec_,
                                                 encoding_version_})
        .via(&async::io_executor())
        .thenValue(WriteSegmentTask{library_});
  }
  ...
  folly::Future<entity::VariantKey> write(KeyType key_type, const StreamId& stream_id,
                                          SegmentInMemory&& segment) override {
    util::check(is_ref_key_class(key_type), "Expected ref key type got  {}", key_type);
    return async::submit_cpu_task(EncodeRefTask{key_type, stream_id, std::move(segment),
                                                codec_, encoding_version_})
        .via(&async::io_executor())
        .thenValue(WriteSegmentTask{library_});
  }

  entity::VariantKey write_sync(stream::KeyType key_type, VersionId version_id,
                                const StreamId& stream_id, IndexValue start_index,
    ...
    auto encoded = EncodeAtomTask{key_type,           version_id, stream_id,
                                  start_index,        end_index,  current_timestamp(),
  ...
  entity::VariantKey write_sync(PartialKey pk, SegmentInMemory&& segment) override {
    return write_sync(pk.key_type, pk.version_id, pk.stream_id, pk.start_index,
                      pk.end_index, std::move(segment));
  }

  entity::VariantKey write_sync(KeyType key_type, const StreamId& stream_id,
                                SegmentInMemory&& segment) override {
    util::check(is_ref_key_class(key_type), "Expected ref key type got  {}", key_type);
  ...
  folly::Future<entity::VariantKey> update(const entity::VariantKey& key,
    ...
    util::check(segment.descriptor().id() == stream_id,
                "Descriptor id mismatch in variant key {} != {}", stream_id,
                segment.descriptor().id());

    return async::submit_cpu_task(
               EncodeSegmentTask{key, std::move(segment), codec_, encoding_version_})
        .via(&async::io_executor())
        .thenValue(UpdateSegmentTask{library_, opts});
  }

  folly::Future<VariantKey> copy(KeyType key_type, const StreamId& stream_id,
                                 VersionId version_id,
                                 const VariantKey& source_key) override {
    return async::submit_io_task(CopyCompressedTask<ClockType>{
        source_key, key_type, stream_id, version_id, library_});
  }

  VariantKey copy_sync(KeyType key_type, const StreamId& stream_id,
                       VersionId version_id, const VariantKey& source_key) override {
    return CopyCompressedTask<ClockType>{source_key, key_type, stream_id, version_id,
                                         library_}();
  }

  timestamp current_timestamp() override { return ClockType::nanos_since_epoch(); }

  void iterate_type(KeyType type, const entity::IterateTypeVisitor& func,
                    const std::string& prefix) override {
    library_->iterate_type(type, func, prefix);
  }

  folly::Future<std::pair<entity::VariantKey, SegmentInMemory>>
  read(const entity::VariantKey& key, storage::ReadKeyOpts opts) override {
    return read_and_continue(key, library_, opts, DecodeSegmentTask{});
  }

  std::pair<entity::VariantKey, SegmentInMemory>
  read_sync(const entity::VariantKey& key, storage::ReadKeyOpts opts) override {
    return DecodeSegmentTask{}(read_dispatch(key, library_, opts));
  }

  folly::Future<storage::KeySegmentPair>
  read_compressed(const entity::VariantKey& key, storage::ReadKeyOpts opts) override {
    return read_and_continue(key, library_, opts, PassThroughTask{});
  }

  storage::KeySegmentPair read_compressed_sync(const entity::VariantKey& key,
  ...
  read_timeseries_descriptor_for_incompletes(
      const entity::VariantKey& key,
      storage::ReadKeyOpts opts = storage::ReadKeyOpts{}) override {
    return read_and_continue(key, library_, opts,
                             DecodeTimeseriesDescriptorForIncompletesTask{});
  }
  ...
  folly::Future<std::vector<RemoveKeyResultType>>
  remove_keys(std::vector<entity::VariantKey>&& keys,
              storage::RemoveOpts opts) override {
    return keys.empty() ? std::vector<RemoveKeyResultType>()
  ...
  remove_keys_sync(const std::vector<entity::VariantKey>& keys,
                   storage::RemoveOpts opts) override {
    return keys.empty() ? std::vector<RemoveKeyResultType>()
  ...
  std::vector<RemoveKeyResultType>
  remove_keys_sync(std::vector<entity::VariantKey>&& keys,
                   storage::RemoveOpts opts) override {
    return keys.empty() ? std::vector<RemoveKeyResultType>()
                        : RemoveBatchTask{std::move(keys), library_, opts}();
  }
  ...
  batch_read_compressed(std::vector<std::pair<entity::VariantKey, ReadContinuation>>&&
                            keys_and_continuations,
                        const BatchReadArgs& args) override {
    util::check(!keys_and_continuations.empty(),
                "Unexpected empty keys/continuation vector in batch_read_compressed");
    return folly::collect(folly::window(
                              std::move(keys_and_continuations),
                              [this](auto&& key_and_continuation) {
                                auto [key, continuation] =
                                    std::forward<decltype(key_and_continuation)>(
                                        key_and_continuation);
                                return read_and_continue(key, library_,
                                                         storage::ReadKeyOpts{},
    ...
    return folly::window(
        ...
        [this, columns_to_decode](auto&& ranges_and_key) {
          ...
          return read_and_continue(
              key, library_, storage::ReadKeyOpts{},
              DecodeSliceTask{std::move(ranges_and_key), columns_to_decode});
        },
        async::TaskScheduler::instance()->io_thread_count() * 2);
  }

  std::vector<folly::Future<bool>>
  batch_key_exists(const std::vector<entity::VariantKey>& keys) override {
  ...
  folly::Future<SliceAndKey> async_write(
      folly::Future<std::tuple<PartialKey, SegmentInMemory, pipelines::FrameSlice>>&&
          input_fut,
    ...
    return std::move(input_fut)
        .thenValue([this](auto&& input) {
          ...
          auto key_seg = EncodeAtomTask{std::move(key), ClockType::nanos_since_epoch(),
                                        std::move(seg), codec_, encoding_version_}();
          return std::pair<storage::KeySegmentPair, FrameSlice>(std::move(key_seg),
                                                                std::move(slice));
        })
        .thenValue(
            [de_dup_map](auto&& ks) -> std::pair<KeyOptSegment, pipelines::FrameSlice> {
              auto [key_seg, slice] = std::forward<decltype(ks)>(ks);
              return std::make_pair(
                  lookup_match_in_dedup_map(de_dup_map, std::move(key_seg)),
                  std::move(slice));
            })
        .via(&async::io_executor())
        .thenValue([lib = library_](auto&& item) {
          ...
          if (key_opt_segment.second)
            lib->write(Composite<storage::KeySegmentPair>(
                {VariantKey{key_opt_segment.first},
  ...
  void set_failure_sim(
      const arcticdb::proto::storage::VersionStoreConfig::StorageFailureSimulator& cfg)
      override {
