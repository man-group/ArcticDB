...
folly::Future<std::shared_ptr<storage::ObjectSizes>> get_object_sizes(KeyType type, const std::optional<StreamId>& stream_id_opt) override {
    ...
    if (library_->supports_object_size_calculation()) {
        ...
        auto res = std::make_shared<storage::ObjectSizes>(type);
        storage::ObjectSizesVisitor visitor = [res, match_stream_id](const VariantKey& key, storage::CompressedSize size) {
            if (!match_stream_id || match_stream_id.value()(key)) {
                res->count_ += 1;
                res->compressed_size_bytes_ += size;
            }
        };

        return async::submit_io_task(VisitObjectSizesTask{type, prefix, visitor, library_})
        .thenValueInline([res](auto&&) {
            return res;
        });
    }
    ...
    iterate_type(type, [&key_size_calculators, &counter, &bytes, &match_stream_id](const VariantKey&& k) {
        key_size_calculators.emplace_back(std::forward<const VariantKey>(k), [counter, bytes, match_stream_id] (auto&& key_seg) {
