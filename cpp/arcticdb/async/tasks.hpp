...
inline storage::KeySegmentPair read_dispatch(
    const entity::VariantKey& variant_key,
        ...
        return std::visit(
            [that = this](const auto& source_key) {
                ...
                auto target_key_seg = stream::make_target_key<ClockType>(
                    that->key_type_, that->stream_id_, that->version_id_, source_key, std::move(key_seg.segment())
                );
...
struct DecodeSlicesTask : BaseTask {
    ...
    explicit DecodeSlicesTask(const std::shared_ptr<std::unordered_set<std::string>>& filter_columns)
...
struct MemSegmentProcessingTask : BaseTask {
    ...
    explicit MemSegmentProcessingTask(std::vector<std::shared_ptr<Clause>> clauses, std::vector<EntityId>&& entity_ids)
...
struct MemSegmentFunctionTask : BaseTask {
    ...
    explicit MemSegmentFunctionTask(stream::StreamSource::DecodeContinuation&& func) : func_(std::move(func)) {}
...
struct DecodeMetadataTask : BaseTask {
    ...
    std::pair<std::optional<VariantKey>, std::optional<google::protobuf::Any>> operator()(storage::KeySegmentPair&& ks
    ) const {
...
struct DecodeMetadataAndDescriptorTask : BaseTask {
    ...
    std::tuple<VariantKey, std::optional<google::protobuf::Any>, StreamDescriptor> operator()(
        ...
        return std::make_tuple(std::move(key_seg.variant_key()), std::move(any), std::move(descriptor));
    }
};
