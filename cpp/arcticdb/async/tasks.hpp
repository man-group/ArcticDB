...
struct EncodeAtomTask : BaseTask {
  ...
  EncodeAtomTask(PartialKey&& pk, timestamp creation_ts, SegmentInMemory&& segment,
                 std::shared_ptr<arcticdb::proto::encoding::VariantCodec> codec_meta,
                 EncodingVersion encoding_version)
      : partial_key_(std::move(pk)), creation_ts_(creation_ts),
        segment_(std::move(segment)), codec_meta_(std::move(codec_meta)),
        encoding_version_(encoding_version) {}

  EncodeAtomTask(std::pair<PartialKey, SegmentInMemory>&& pk_seg, timestamp creation_ts,
                 std::shared_ptr<arcticdb::proto::encoding::VariantCodec> codec_meta,
                 EncodingVersion encoding_version)
      : partial_key_(std::move(pk_seg.first)), creation_ts_(creation_ts),
        segment_(std::move(pk_seg.second)), codec_meta_(std::move(codec_meta)),
        encoding_version_(encoding_version) {}

  EncodeAtomTask(
      KeyType key_type, GenerationId gen_id, StreamId stream_id, IndexValue start_index,
      IndexValue end_index, timestamp creation_ts, SegmentInMemory&& segment,
      const std::shared_ptr<arcticdb::proto::encoding::VariantCodec>& codec_meta,
      EncodingVersion encoding_version)
      : EncodeAtomTask(PartialKey{key_type, gen_id, std::move(stream_id),
                                  std::move(start_index), std::move(end_index)},
  ...
  storage::KeySegmentPair encode() {
    ...
    auto enc_seg = ::arcticdb::encode_dispatch(std::move(segment_), *codec_meta_,
                                               encoding_version_);
...
struct EncodeSegmentTask : BaseTask {
  ...
  EncodeSegmentTask(entity::VariantKey key, SegmentInMemory&& segment,
                    std::shared_ptr<arcticdb::proto::encoding::VariantCodec> codec_meta,
                    EncodingVersion encoding_version)
      : key_(std::move(key)), segment_(std::move(segment)),
        codec_meta_(std::move(codec_meta)), encoding_version_(encoding_version) {}
...
struct EncodeRefTask : BaseTask {
  ...
  EncodeRefTask(KeyType key_type, StreamId stream_id, SegmentInMemory&& segment,
                std::shared_ptr<arcticdb::proto::encoding::VariantCodec> codec_meta,
                EncodingVersion encoding_version)
      : key_type_(key_type), id_(std::move(stream_id)), segment_(std::move(segment)),
        codec_meta_(std::move(codec_meta)), encoding_version_(encoding_version) {}
...
struct UpdateSegmentTask : BaseTask {
  ...
  explicit UpdateSegmentTask(std::shared_ptr<storage::Library> lib,
                             storage::UpdateOpts opts)
      : lib_(std::move(lib)), opts_(opts) {}
...
template <typename Callable> struct KeySegmentContinuation {
  storage::KeySegmentPair key_seg_;
  Callable continuation_;
};

inline storage::KeySegmentPair
read_dispatch(const entity::VariantKey& variant_key,
              const std::shared_ptr<storage::Library>& lib,
              const storage::ReadKeyOpts& opts) {
  return util::variant_match(
      variant_key, [&lib, &opts](const auto& key) { return lib->read(key, opts); });
}

template <typename Callable> struct ReadCompressedTask : BaseTask {
  ...
  ReadCompressedTask(entity::VariantKey key, std::shared_ptr<storage::Library> lib,
                     storage::ReadKeyOpts opts, Callable&& continuation)
      : key_(std::move(key)), lib_(std::move(lib)), opts_(opts),
        continuation_(std::move(continuation)) {
    ARCTICDB_DEBUG(log::storage(), "Creating read compressed task for key {}: {}",
                   variant_key_type(key_), variant_key_view(key_));
  }
  ...
  KeySegmentContinuation<ContinuationType> operator()() {
    ...
    return KeySegmentContinuation<decltype(continuation_)>{
        read_dispatch(key_, lib_, opts_), std::move(continuation_)};
  }
};
...
template <typename ClockType> struct CopyCompressedTask : BaseTask {
  ...
  CopyCompressedTask(entity::VariantKey source_key, KeyType key_type,
                     StreamId stream_id, VersionId version_id,
                     std::shared_ptr<storage::Library> lib)
      : source_key_(std::move(source_key)), key_type_(key_type),
        stream_id_(std::move(stream_id)), version_id_(version_id),
        lib_(std::move(lib)) {
    ARCTICDB_DEBUG(log::storage(),
                   "Creating copy compressed task for key {} -> {} {} {}",
                   variant_key_view(source_key_), key_type_, stream_id_, version_id_);
  }
    ...
    return std::visit(
        [that = this](const auto& source_key) {
          ...
          auto target_key_seg = stream::make_target_key<ClockType>(
              that->key_type_, that->stream_id_, that->version_id_, source_key,
              std::move(key_seg.segment()));
          auto return_key = target_key_seg.variant_key();
          that->lib_->write(
              Composite<storage::KeySegmentPair>{std::move(target_key_seg)});
          return return_key;
        },
        source_key_);
  }
...
// Used in arcticdb-enterprise, do not remove without checking whether it is still used
// there
struct CopyCompressedInterStoreTask : async::BaseTask {
  ...
  CopyCompressedInterStoreTask(entity::VariantKey key_to_read,
                               ...
                               bool check_key_exists_on_targets, bool retry_on_failure,
      ...
      : key_to_read_(std::move(key_to_read)), key_to_write_(std::move(key_to_write)),
        check_key_exists_on_targets_(check_key_exists_on_targets),
        retry_on_failure_(retry_on_failure), source_store_(std::move(source_store)),
    ...
    ARCTICDB_DEBUG(
        ...
        variant_key_type(key_to_read_), variant_key_view(key_to_read_),
        key_to_write_.has_value() ? variant_key_type(key_to_write_.value())
                                  : variant_key_type(key_to_read_),
        key_to_write_.has_value() ? variant_key_view(key_to_write_.value())
...
private:
  ...
  // Returns an empty set if the copy succeeds, otherwise the set contains the names of
  // the target stores that failed
  std::unordered_set<std::string> copy() {
    ...
    if (check_key_exists_on_targets_) {
      target_stores_.erase(
          std::remove_if(target_stores_.begin(), target_stores_.end(),
    ...
    if (!target_stores_.empty()) {
      ...
      try {
        key_segment_pair =
            source_store_->read_compressed_sync(key_to_read_, storage::ReadKeyOpts{});
      } catch (const storage::KeyNotFoundException& e) {
        log::storage().debug("Key {} not found on the source: {}",
                             variant_key_view(key_to_read_), e.what());
      ...
      for (auto& target_store : target_stores_) {
        try {
          ...
          log::storage().debug("Key {} already exists on the target: {}",
                               variant_key_view(key_to_read_), e.what());
        } catch (const storage::KeyNotFoundException& e) {
          log::storage().debug("Key {} not found on the source: {}",
                               variant_key_view(key_to_read_), e.what());
          ...
          log::storage().error("Failed to write key {} to store {}: {}",
                               variant_key_view(key_to_read_), name, e.what());
...
struct DecodeSegmentTask : BaseTask {
  ...
  std::pair<VariantKey, SegmentInMemory>
  operator()(storage::KeySegmentPair&& ks) const {
...
private:
  ...
  decode_into_slice(storage::KeySegmentPair&& key_segment_pair);
};
...
struct DecodeSlicesTask : BaseTask {
  ...
  Composite<pipelines::SliceAndKey>
  operator()(Composite<std::pair<Segment, pipelines::SliceAndKey>>&& skp) const {
...
private:
  pipelines::SliceAndKey
  decode_into_slice(std::pair<Segment, pipelines::SliceAndKey>&& sk_pair) const;
};
...
struct MemSegmentProcessingTask : BaseTask {
  ...
  explicit MemSegmentProcessingTask(std::vector<std::shared_ptr<Clause>> clauses,
                                    std::vector<EntityId>&& entity_ids)
      : clauses_(std::move(clauses)), entity_ids_(std::move(entity_ids)) {}
...
struct DecodeTimeseriesDescriptorTask : BaseTask {
  ...
  operator()(storage::KeySegmentPair&& ks) const {
    ...
    util::check(static_cast<bool>(maybe_desc),
                "Failed to decode timeseries descriptor");
    return std::make_pair(std::move(key_seg.variant_key()), std::move(*maybe_desc));
  }
};
...
struct DecodeMetadataAndDescriptorTask : BaseTask {
  ...
  operator()(storage::KeySegmentPair&& ks) const {
    ...
    return std::make_tuple(std::move(key_seg.variant_key()), std::move(any),
                           std::move(descriptor));
  }
};
struct KeyExistsTask : BaseTask {
  ...
  KeyExistsTask(auto&& key, std::shared_ptr<storage::Library> lib)
      : key_(std::forward<decltype(key)>(key)), lib_(std::move(lib)) {
...
struct WriteCompressedTask : BaseTask {
  ...
  WriteCompressedTask(storage::KeySegmentPair&& kv,
                      std::shared_ptr<storage::Library> lib)
...
struct WriteCompressedBatchTask : BaseTask {
  ...
  WriteCompressedBatchTask(std::vector<storage::KeySegmentPair>&& kvs,
                           std::shared_ptr<storage::Library> lib)
      : kvs_(std::move(kvs)), lib_(std::move(lib)) {
    util::check(!kvs_.empty(), "WriteCompressedBatch task created with no data");

    ARCTICDB_DEBUG(log::storage(), "Creating read and decode task for {} keys",
                   kvs_.size());
  }
...
struct RemoveTask : BaseTask {
  ...
  RemoveTask(const VariantKey& key_, std::shared_ptr<storage::Library> lib_,
             storage::RemoveOpts opts)
      : key_(key_), lib_(std::move(lib_)), opts_(opts) {
    ARCTICDB_DEBUG(log::storage(), "Creating remove task for key {}: {}",
                   variant_key_type(key_), variant_key_view(key_));
  }
...
struct RemoveBatchTask : BaseTask {
  ...
  RemoveBatchTask(std::vector<VariantKey> key_, std::shared_ptr<storage::Library> lib_,
                  storage::RemoveOpts opts)
      : keys_(std::move(key_)), lib_(std::move(lib_)), opts_(opts) {
