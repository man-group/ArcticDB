...
void TaskScheduler::reattach_instance() {
    if (TaskScheduler::instance_) {
        ...
        ARCTICDB_DEBUG(
            log::schedule(), "Attached new task scheduler instance, now {}", uintptr_t(TaskScheduler::instance_->ptr_)
        );
    }
}
...
void TaskScheduler::init() { TaskScheduler::instance_ = std::make_shared<TaskSchedulerPtrWrapper>(new TaskScheduler); }
