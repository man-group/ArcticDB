...
void BitRateStats::add_stat(std::size_t bytes, double time_ms) {
  ...
  if (now - last_log_time_ns_ > log_frequency_ns &&
      stats_.compare_exchange_strong(current_stats, 0)) {
...
uint64_t BitRateStats::data_to_stat(std::size_t bytes, double time_ms) const {
  if (UNLIKELY(bytes > max_bytes || time_ms > max_time_ms)) {
    log::storage().warn(
        "Bit rate stats provided too large to represent, ignoring: {} in {}ms",
        format_bytes(bytes), time_ms);
    return 0;
  }
