...
StreamDescriptor idx_stream_desc(StreamId stream_id, IndexType index) {
  ...
  return StreamDescriptor{
      index_descriptor(stream_id, index,
...
template <class IndexType> struct IndexSliceDescriptor : StreamDescriptor {
  using DataTypeTag = typename IndexType::TypeDescTag::DataTypeTag;
...
using SliceTypeDescriptorTag =
    TypeDescriptorTag<DataTypeTag<DataType::UINT64>, DimensionTag<Dimension ::Dim0>>;
...
inline StreamId stream_id_from_segment(const SegmentInMemory& seg, ssize_t row) {
  if (const auto& fd = seg.descriptor()[int(FieldType::stream_id)];
      is_sequence_type(fd.type().data_type()))
...
auto read_key_row_into_builder(const SegmentInMemory& seg, ssize_t i) {
  return atom_key_builder()
      ...
      .start_index(
          pipelines::index::index_start_from_segment<SegmentInMemory, FieldType>(seg,
                                                                                 i))
      .end_index(
          pipelines::index::index_end_from_segment<SegmentInMemory, FieldType>(seg, i));
}
...
inline entity::AtomKey read_key_row_impl(const SegmentInMemory& seg, ssize_t i) {
  ...
  auto k = read_key_row_into_builder<FieldType>(seg, i).build(std::move(stream_id),
                                                              key_type);
...
inline entity::AtomKey read_key_row(const SegmentInMemory& seg, ssize_t i) {
  ...
  try {
    ...
    ARCTICDB_DEBUG(log::storage(), "Read legacy key from row '{}: {}'", k.type(),
                   k.view());
    return k;
  }
}
...
template <class KeyIt> class KeyRangeIterator : public IndexRangeFilter {
public:
...
inline auto generate_segments_from_keys(arcticdb::stream::StreamSource& read_store,
  ...
  return map([&read_store](auto&& key) {
           ARCTICDB_DEBUG(log::inmem(), "Getting segment for key {}: {}", key.type(),
                          key.view());
...
inline auto generate_keys_from_segments(
  ...
  return folly::gen::map([expected_key_type, expected_index_type,
                          &read_store](auto&& key_seg) {
           return folly::gen::detail::GeneratorBuilder<entity::AtomKey>() +
                  [&](auto&& yield) {
                    ...
                    while (!key_segs.empty()) {
                      ...
                      for (ssize_t i = 0; i < ssize_t(seg.row_count()); ++i) {
                        ...
                        if (read_key.type() != expected_key_type) {
                          util::check_arg(
                              expected_index_type &&
                                  read_key.type() == *expected_index_type,
                              "Found unsupported key type in index segment. Expected "
                              "{} or (index) {}, actual {}",
                              expected_key_type,
...
std::optional<KeyMemSegmentPair>
next_non_empty_segment(SegmentIteratorType& iterator_segments,
...
public:
  SegmentIterator(const IndexRange& index_range, KeyIt&& key_it,
                  std::shared_ptr<StreamSource> read_store)
      : IndexRangeFilter(index_range), key_it_(std::move(key_it)),
        read_store_(std::move(read_store)) {
    init_prefetch();
  }

  SegmentIterator(const TimestampRange& ts_rg, KeyIt&& key_it,
...
private:
  ...
  bool enqueue_next_key_read_request(
      folly::Duration timeout = util::timeout::get_default()) {
...
public:
  ...
  std::optional<entity::AtomKey> next(folly::Duration timeout) {
    while (true) {
      if (!key_seg_ &&
          !(row_id = 0, key_seg_ = next_non_empty_segment(seg_it_, timeout)))
        return std::nullopt;
...
inline std::set<StreamId> filter_by_regex(const std::set<StreamId>& results,
  ...
  std::copy_if(results.begin(), results.end(),
               ...
               [&regex](const StreamId& s_id) {
                 auto string_id = std::holds_alternative<StringId>(s_id)
                                      ? std::get<StringId>(s_id)
                                      : std::string();
...
get_index_columns_from_descriptor(const TimeseriesDescriptor& tsd) {
  const auto& norm_info = tsd.proto().normalization();
  const auto& stream_descriptor = tsd.as_stream_descriptor();
  // For explicit integer indexes, the index is actually present in the first column
  // even though the field_count is 0.
  ssize_t index_till;
  ...
  if (auto idx_type = common.index_type_case();
    ...
    index_till = common.index().is_physically_stored()
                     ? 1
                     : stream_descriptor.index().field_count();
  else
    index_till = 1 + common.multi_index()
                         .field_count(); // # The value of field_count is len(index) - 1
...
inline IndexRange get_range_from_segment(const Index& index,
                                         const SegmentInMemory& segment) {
  return util::variant_match(
      index, [](const EmptyIndex&) { return IndexRange{}; },
...
make_target_key(KeyType key_type, const StreamId& stream_id, VersionId version_id,
                const VariantKey& source_key, Segment&& segment) {
