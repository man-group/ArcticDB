...
class RowBuilder {
  public:
    ...
    void set_string_impl(std::size_t pos, const std::string& str) { aggregator_.set_string(pos, str); }
    ...
    [[nodiscard]] const arcticdb::entity::StreamDescriptor& descriptor() const { return aggregator_.descriptor(); }
  ...
  private:
    ...
    void set_scalar_impl(std::size_t pos, T val) {
        visit_field(descriptor().fields(pos), [&](auto&& tag) {
            ...
            if constexpr (std::is_same_v<
                                  typename std::decay_t<decltype(tag)>::DimensionTag,
    ...
    void check_pos(std::size_t pos) {
        ...
        util::check_arg(is_sequence_type(td.type().data_type()), "Set string called on non-string type column");
    }
