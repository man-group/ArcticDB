...
inline IndexType get_index_from_schema(const SchemaType& schema) {
  util::check(std::holds_alternative<IndexType>(schema.index()),
              "Schema and aggregator index type mismatch");
  return std::get<IndexType>(schema.index());
}

template <class Index, class Schema, class Aggregator> class RowBuilder {
public:
  ...
  explicit RowBuilder(Schema& schema, Aggregator& aggregator)
      : schema_(schema), index_(get_index_from_schema<IndexType, SchemaType>(schema_)),
        aggregator_(aggregator), nbytes_(0) {}

  RowBuilder(RowBuilder&) = delete;
  RowBuilder& operator=(RowBuilder&&) = delete;
  RowBuilder& operator=(RowBuilder&) = delete;

  template <class... Args> void start_row(const Args... args) {
    reset();
      ...
      index().set(
          [&](std::size_t pos, auto arg) {
            if constexpr (std::is_integral_v<decltype(arg)> ||
                          std::is_floating_point_v<decltype(arg)>)
  ...
  template <class RowBuilderSetter> void operator()(RowBuilderSetter&& setter) {
    SCOPE_FAIL { rollback_row(); };
    setter(*this);
    end_row();
  }

  void rollback_row() noexcept { reset(); }
  ...
  find_field(std::string_view field_name) const {
  ...
  template <class T, std::enable_if_t<std::is_same_v<std::decay_t<T>, std::string_view>,
                                      int> = 0>
  void set_scalar_by_name(std::string_view name, T val, DataType data_type) {
  ...
  void set_string_array(std::size_t pos, py::array& arr) {
    ...
    util::check_arg(info.strides.size() == 1,
                    "Assumed numpy string array has no strides");
    util::check_arg(info.shape.size() == 1, "Assumed numpy string array has no shapes");
    util::check_arg(info.itemsize == info.strides[0],
                    "Non-contiguous string arrays not currently supported");
    aggregator_.set_string_array(pos, info.itemsize, info.shape[0],
...
private:
  ...
  void set_block(std::size_t pos, T* val, size_t size) {
    descriptor().fields[pos].type_desc.visit_tag([&](auto&& tag) {
      ...
      if constexpr (std::is_same_v<typename DT::DimensionTag,
      ...
      if constexpr (std::is_same_v<typename std::decay_t<decltype(tag)>::DimensionTag,
        ...
        if constexpr ((std::is_integral_v<T> || std::is_floating_point_v<RawType>) &&
          ...
          throw ArcticCategorizedException<ErrorCategory::INTERNAL>(fmt::format(
              ...
              descriptor().fields(pos).type(), typeid(conv_val).name(), val,
              typeid(val).name()));
        }
      } else {
        throw ArcticCategorizedException<ErrorCategory::INTERNAL>(fmt::format(
