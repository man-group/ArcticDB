...
public:
  ...
  using TypeDescTag = TypeDescriptorTag<DataTypeTag<DataType::NANOSECONDS_UTC64>,
                                        DimensionTag<Dimension::Dim0>>;

  static constexpr size_t field_count() { return 1; }
  ...
  void set(RowCellSetter setter, const IndexValue& index_value) {
    if (std::holds_alternative<timestamp>(index_value)) {
      ...
      util::check_arg(ts >= ts_, "timestamp decreasing, current val={}, candidate={}",
                      ts_, ts);
      ts_ = ts;
      setter(0, ts);
    } else
...
public:
  ...
  using TypeDescTag = TypeDescriptorTag<DataTypeTag<DataType::ASCII_DYNAMIC64>,
                                        DimensionTag<Dimension::Dim0>>;
...
public:
  using TypeDescTag =
      TypeDescriptorTag<DataTypeTag<DataType::EMPTYVAL>, DimensionTag<Dimension::Dim0>>;
  static constexpr size_t field_count() { return 0; }

  static constexpr IndexDescriptor::Type type() { return IndexDescriptor::Type::EMPTY; }

  static constexpr const char* name() { return "empty"; }

  static constexpr EmptyIndex default_index() { return {}; }

  [[nodiscard]] static IndexValue
  start_value_for_segment(const SegmentInMemory& segment);
  [[nodiscard]] static IndexValue end_value_for_segment(const SegmentInMemory& segment);
  [[nodiscard]] static IndexValue
  start_value_for_keys_segment(const SegmentInMemory& segment);
  [[nodiscard]] static IndexValue
  end_value_for_keys_segment(const SegmentInMemory& segment);
};

using Index = std::variant<stream::TimeseriesIndex, stream::RowCountIndex,
                           stream::TableIndex, stream::EmptyIndex>;
