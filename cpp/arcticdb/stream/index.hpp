...
public:
    ...
    [[nodiscard]] FieldDescriptor field_proto(size_t) const
    {
        return FieldDescriptor{
            scalar_field_proto(typename Derived::TypeDescTag::DataTypeTag().data_type, std::string(derived()->name()))};
        ;
    }
};
...
public:
    ...
    using TypeDescTag = TypeDescriptorTag<DataTypeTag<DataType::MICROS_UTC64>, DimensionTag<Dimension::Dim0>>;
    ...
    void check(const StreamDescriptor::FieldsCollection& fields) const
    {
        util::check_arg(fields.size() >= int(field_count()),
            ...
            field_count(),
            fields.size());
        util::check_arg(type_desc_from_proto(fields[0].type_desc()) == this->field_proto(0).type_desc(),
            "expected field[0]={}, actual {}",
            this->field_proto(0),
            fields[0]);
    }
...
public:
    ...
    using TypeDescTag = TypeDescriptorTag<DataTypeTag<DataType::ASCII_DYNAMIC64>, DimensionTag<Dimension::Dim0>>;
...
inline Index index_type_from_descriptor(const StreamDescriptor::Proto& desc)
{
    ...
    default:
        util::raise_rte(
            "Data obtained from storage refers to an index type that this build of ArcticDB doesn't understandi ({}).",
            desc.index().kind());
    }
}
