...
folly::Future<arcticdb::entity::VariantKey> write_incomplete_frame(
    ...
    sorting::check<ErrorCode::E_UNSORTED_DATA>(
            !validate_index || sort_columns || index_is_not_timeseries_or_is_sorted_ascending(*frame),
    ...
    if(sort_columns && !sort_columns->empty()) {
        ...
        if(sort_columns->at(0) == mutable_seg.descriptor().field(0).name())
...
void write_parallel_impl(
    ...
    (void)write_incomplete_frame(store, stream_id, frame, validate_index, std::nullopt, sort_columns).get();
}
...
void append_incomplete(
    ...
    auto new_key = write_incomplete_frame(store, stream_id, frame, validate_index, std::move(next_key), std::nullopt).get();
