...
struct AppendMapEntry {
  ...
  entity::StreamDescriptor& descriptor() { return *slice_and_key_.slice_.desc(); }

  const pipelines::FrameSlice& slice() const { return slice_and_key_.slice_; }

  const entity::AtomKey& key() const { return slice_and_key_.key(); }
...
AppendMapEntry entry_from_key(const std::shared_ptr<stream::StreamSource>& store,
                              const entity::AtomKey& key, bool load_data);

std::vector<AppendMapEntry>
get_incomplete_append_slices_for_stream_id(const std::shared_ptr<Store>& store,
                                           const StreamId& stream_id,
                                           bool via_iteration, bool load_data);

inline std::vector<AppendMapEntry>
load_via_iteration(const std::shared_ptr<Store>& store, const StreamId& stream_id,
                   bool load_data) {
  ...
  store->iterate_type(KeyType::APPEND_DATA,
                      [&store, load_data, &output, &stream_id](const auto& vk) {
...
std::set<StreamId> get_incomplete_symbols(const std::shared_ptr<Store>& store) {
  ...
  store->iterate_type(KeyType::APPEND_DATA,
                      [&output](const auto& vk) { output.insert(variant_key_id(vk)); });
  return output;
}

std::set<StreamId> get_incomplete_refs(const std::shared_ptr<Store>& store) {
  std::set<StreamId> output;
  store->iterate_type(KeyType::APPEND_REF,
                      [&output](const auto& vk) { output.insert(variant_key_id(vk)); });
  return output;
}
...
std::set<StreamId> get_active_incomplete_refs(const std::shared_ptr<Store>& store) {
  ...
  store->iterate_type(KeyType::APPEND_REF,
                      [&ref_keys](const auto& vk) { ref_keys.insert(vk); });
...
void fix_slice_rowcounts(std::vector<AppendMapEntry>& entries,
                         size_t complete_rowcount) {
  for (auto& entry : entries) {
    complete_rowcount = entry.slice_and_key_.slice_.fix_row_count(
        static_cast<ssize_t>(complete_rowcount));
  }
}

TimeseriesDescriptor pack_timeseries_descriptor(
    const StreamDescriptor& descriptor, size_t total_rows,
    std::optional<AtomKey>&& next_key,
    arcticdb::proto::descriptors::NormalizationMetadata&& norm_meta) {
  auto tsd = make_timeseries_descriptor(total_rows, descriptor, std::move(norm_meta),
                                        std::nullopt, std::nullopt, std::move(next_key),
                                        false);
  return tsd;
}
...
SegmentInMemory incomplete_segment_from_frame(
  ...
  std::visit(
      [&](const auto& idx) {
        ...
        auto timeseries_desc =
            index_descriptor_from_frame(frame, existing_rows, std::move(prev_key));
        util::check(!timeseries_desc.fields().empty(),
                    "Expected fields not to be empty in incomplete segment");
        ...
        SingleSegmentAggregator agg{
            FixedSchema{descriptor, index}, [&](auto&& segment) {
              auto tsd = pack_timeseries_descriptor(
                  descriptor, existing_rows + num_rows, std::move(copy_prev_key),
                  std::move(norm_meta));
        ...
        if (has_index) {
          ...
          auto opt_error = aggregator_set_data(
              agg.descriptor().field(0).type(), index_tensor.value(), agg, 0, num_rows,
              offset_in_frame, slice_num_for_column, num_rows, allow_sparse);
        ...
        for (auto col = 0u; col < field_tensors.size(); ++col) {
          ...
          auto opt_error = aggregator_set_data(
              agg.descriptor().field(dest_col).type(), tensor, agg, dest_col, num_rows,
              offset_in_frame, slice_num_for_column, num_rows, allow_sparse);
          if (opt_error.has_value()) {
  ...
  ARCTICDB_DEBUG(
      ...
      output.row_count(), output.num_columns(), output.offset());
  return output;
}

folly::Future<arcticdb::entity::VariantKey>
write_incomplete_frame(const std::shared_ptr<Store>& store, const StreamId& stream_id,
                       const std::shared_ptr<InputTensorFrame>& frame,
                       bool validate_index, std::optional<AtomKey>&& next_key) {
  using namespace arcticdb::pipelines;
  ...
  return store->write(KeyType::APPEND_DATA, VersionId(0), stream_id, index_range.start_,
                      index_range.end_, std::move(segment));
}
...
void write_parallel(const std::shared_ptr<Store>& store, const StreamId& stream_id,
  ...
  (void)write_incomplete_frame(store, stream_id, frame, validate_index, std::nullopt)
      .get();
}
...
std::vector<SliceAndKey> get_incomplete(const std::shared_ptr<Store>& store,
                                        ...
                                        uint64_t last_row, bool via_iteration,
                                        bool load_data) {
  using namespace arcticdb::pipelines;

  std::unique_ptr<TimeseriesDescriptor> unused;
  auto entries = get_incomplete_append_slices_for_stream_id(store, stream_id,
                                                            via_iteration, load_data);
      ...
      [&entries](const IndexRange& index_range) {
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [&](const auto& entry) {
                                       return !intersects(
                                           index_range,
                                           entry.slice_and_key_.key().index_range());
                                     }),
                      std::end(entries));
      },
      [](const auto&) {
...
std::vector<AppendMapEntry> load_via_list(const std::shared_ptr<Store>& store,
                                          const StreamId& stream_id, bool load_data) {
  using namespace arcticdb::pipelines;

  ARCTICDB_DEBUG(log::version(), "Getting incomplete segments for stream {}",
                 stream_id);
...
std::pair<std::optional<AtomKey>, size_t>
read_head(const std::shared_ptr<StreamSource>& store, StreamId stream_id) {
  ...
  try {
    ...
    ARCTICDB_RUNTIME_DEBUG(log::version(),
                           "Failed to get head of append list for {}: {}", ref_key,
                           ex.what());
  }
...
std::pair<TimeseriesDescriptor, std::optional<SegmentInMemory>>
get_descriptor_and_data(const std::shared_ptr<StreamSource>& store, const AtomKey& k,
                        bool load_data,
                        storage::ReadKeyOpts opts = storage::ReadKeyOpts{}) {
  if (load_data) {
    auto [key, seg] = store->read_sync(k, opts);
    return std::make_pair(seg.index_descriptor(),
                          std::make_optional<SegmentInMemory>(seg));
  } else {
...
AppendMapEntry entry_from_key(const std::shared_ptr<StreamSource>& store,
                              const AtomKey& key, bool load_data) {
  ...
  auto frame_slice = FrameSlice{desc, ColRange{index_field_count, field_count},
...
void append_incomplete(const std::shared_ptr<Store>& store, const StreamId& stream_id,
  ...
  auto new_key = write_incomplete_frame(store, stream_id, frame, validate_index,
                                        std::move(next_key))
                     .get();

  ARCTICDB_DEBUG(log::version(),
                 "Wrote incomplete frame for stream {}, {} rows, total rows {}",
                 stream_id, num_rows, total_rows);
...
void append_incomplete_segment(const std::shared_ptr<Store>& store,
  ...
  auto new_key = store
                     ->write(arcticdb::stream::KeyType::APPEND_DATA, 0, stream_id,
                             start_index, end_index, std::move(seg))
                     .get();

  total_rows += seg_row_count;
  ARCTICDB_DEBUG(log::version(),
                 "Wrote incomplete frame for stream {}, {} rows, total rows {}",
                 stream_id, seg_row_count, total_rows);
...
get_incomplete_append_slices_for_stream_id(const std::shared_ptr<Store>& store,
  ...
  if (!entries.empty()) {
    ...
    if (index_desc.type() != IndexDescriptorImpl::Type::ROWCOUNT) {
      ...
      // Can't sensibly sort rowcount indexes, so you'd better have written them in the
      // right order
