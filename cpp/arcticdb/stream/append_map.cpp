...
struct AppendMapEntry {
    ...
    const entity::StreamDescriptor& descriptor() const { return *slice_and_key_.slice_.desc(); }
...
SegmentInMemory incomplete_segment_from_frame(
    ...
    auto output = std::visit(
        [&](const auto& idx) {
            ...
            if (has_index) {
                util::check(
                    static_cast<bool>(index_tensor), "Expected index tensor for index type {}", agg.descriptor().index()
                );
...
folly::Future<arcticdb::entity::VariantKey> write_incomplete_frame(
    ...
    return store->write(
        KeyType::APPEND_DATA, VersionId(0), stream_id, index_range.start_, index_range.end_, std::move(segment)
    );
}
...
void append_incomplete(
    ...
    ARCTICDB_DEBUG(
        log::version(), "Wrote incomplete frame for stream {}, {} rows, total rows {}", stream_id, num_rows, total_rows
    );
