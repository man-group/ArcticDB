...
StreamDescriptor BaseIndex<Derived>::create_stream_descriptor(
    StreamId stream_id, std::initializer_list<FieldRef> fields) const {
...
[[nodiscard]] IndexDescriptor::Type get_index_value_type(const AtomKey& key) {
  return std::holds_alternative<timestamp>(key.start_index())
...
template <typename Derived> FieldRef BaseIndex<Derived>::field(size_t) const {
  return {static_cast<TypeDescriptor>(typename Derived::TypeDescTag{}),
          std::string_view(derived()->name())};
}
...
void TimeseriesIndex::check(const FieldCollection& fields) const {
  ...
  const bool valid_type_promotion =
      has_valid_type_promotion(first_field_type, current_first_field_type).has_value();
  const bool trivial_type_compatibility =
      trivially_compatible_types(first_field_type, current_first_field_type);

  const bool compatible_types = valid_type_promotion || trivial_type_compatibility;

  util::check_arg(fields_size >= current_fields_size,
                  "expected at least {} fields, actual {}", current_fields_size,
                  fields_size);
  util::check_arg(compatible_types, "expected field[0]={}, actual {}", this->field(0),
                  fields[0]);
}
...
IndexValue
TimeseriesIndex::start_value_for_keys_segment(const SegmentInMemory& segment) {
  if (segment.row_count() == 0)
    return {NumericIndex{0}};
...
IndexValue TimeseriesIndex::end_value_for_keys_segment(const SegmentInMemory& segment) {
  ...
  auto last_ts =
      segment.template scalar_at<timestamp>(row_count - 1, end_index_id).value();
  return {last_ts};
}
...
void TableIndex::check(const FieldCollection& fields) const {
  util::check_arg(fields.size() >= int(field_count()),
                  "expected at least {} fields, actual {}", field_count(),
                  fields.size());

  util::check(fields.ref_at(0) == field(0), "Field descriptor mismatch {} != {}",
              fields.ref_at(0), field(0));
}
...
Index index_type_from_descriptor(const StreamDescriptor& desc) {
  ...
  default:
    util::raise_rte("Data obtained from storage refers to an index type that this "
                    "build of ArcticDB doesn't understand ({}).",
                    int(desc.index().type()));
  }
}
...
Index default_index_type_from_descriptor(const IndexDescriptorImpl& desc) {
  ...
  case IndexDescriptor::Type::ROWCOUNT:
    return RowCountIndex::default_index();
  default:
    util::raise_rte("Unknown index type {} trying to generate index type",
                    int(desc.type()));
  }
}
...
IndexDescriptor get_descriptor_from_index(const Index& index) {
  return util::variant_match(
      index, [](const auto& idx) { return static_cast<IndexDescriptorImpl>(idx); });
}
