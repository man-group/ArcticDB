...
/**
 * Set the minimum defaults into norm_meta. Originally created to synthesize norm_meta
...
void ensure_timeseries_norm_meta(
  ...
  if (norm_meta.input_type_case() ==
      arcticdb::proto::descriptors::NormalizationMetadata::INPUT_TYPE_NOT_SET) {
...
SegmentDescriptorImpl segment_descriptor_from_proto(
  ...
  output.index_ = IndexDescriptor(IndexDescriptor::Type(desc.index().kind()),
                                  desc.index().field_count());
  return output;
}

StreamId
stream_id_from_proto(const arcticdb::proto::descriptors::StreamDescriptor& desc) {
  return desc.id_case() == desc.kNumId ? StreamId(desc.num_id())
                                       : StreamId(desc.str_id());
}
