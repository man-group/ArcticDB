...
inline void merge_string_column(ChunkedBuffer& src_buffer,
                                ...
                                CursoredBuffer<ChunkedBuffer>& output, bool verify) {
  ...
  for (auto row = 0ULL; row < num_strings; ++row) {
    ...
    util::check(new_value >= 0, "Unexpected negative number {} in merge string column",
                new_value);
  ...
  if (verify) {
    ...
    util::check(num_strings == num_out, "Mismatch in input/output size {} != {}",
                num_strings, num_out);
...
inline void merge_string_columns(const SegmentInMemory& segment,
                                 const std::shared_ptr<StringPool>& merged_pool,
                                 bool verify) {
...
inline void merge_segments(std::vector<SegmentInMemory>& segments,
                           SegmentInMemory& merged, bool is_sparse) {
  ...
  for (auto& segment : segments) {
    ARCTICDB_DEBUG(log::version(), "Appending segment with {} rows",
                   segment.row_count());
    ...
    if (segment.row_count() &&
        segment.descriptor().index().type() == IndexDescriptorImpl::Type::TIMESTAMP) {
...
inline pipelines::FrameSlice merge_slices(std::vector<pipelines::FrameSlice>& slices,
