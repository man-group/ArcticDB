...
TEST(Append, Simple) {
  ...
  pipeline_context->slice_and_keys_ =
      arcticdb::get_incomplete(store, stream_id, range, 0, false, false);
...
TEST(Append, MergeDescriptorsPromote) {
  ...
  std::vector<FieldRef> fields{
      ...
      scalar_field(DataType::INT8, "int8"), scalar_field(DataType::INT16, "int16"),
      scalar_field(DataType::UINT8, "uint8"), scalar_field(DataType::UINT16, "uint16")};
  ...
  auto get_new_fields = []() {
    std::vector<std::vector<FieldRef>> new_fields{
        {scalar_field(DataType::NANOSECONDS_UTC64, "time"),
         scalar_field(DataType::INT16, "int8"), scalar_field(DataType::INT32, "int16"),
  ...
  new_desc_fields.emplace_back(
      std::make_shared<FieldCollection>(fields_from_range(get_new_fields()[0])));
  auto new_desc = merge_descriptors(original, std::move(new_desc_fields),
                                    std::vector<std::string>{});
  std::vector<std::shared_ptr<FieldCollection>> expected_desc_fields;
  expected_desc_fields.emplace_back(
      std::make_shared<FieldCollection>(fields_from_range(get_new_fields()[0])));

  auto result = std::equal(
      std::begin(new_desc.fields()), std::end(new_desc.fields()),
      std::begin(*expected_desc_fields[0]), std::end(*expected_desc_fields[0]),
...
TEST(Append, MergeDescriptorsNoPromote) {
  ...
  std::vector<std::shared_ptr<FieldCollection>> new_desc_fields;
  new_desc_fields.emplace_back(
      std::make_shared<FieldCollection>(fields_from_range(new_fields[0])));
  auto new_desc = merge_descriptors(original, std::move(new_desc_fields),
                                    std::vector<std::string>{});
  auto result =
      std::equal(std::begin(new_desc.fields()), std::end(new_desc.fields()),
