...
template<
    ...
    typename DTT,
    std::enable_if_t<
        std::is_floating_point_v<typename DTT::raw_type> || std::is_integral_v<typename DTT::raw_type>,
        int> = 0>
...
inline std::shared_ptr<storage::Library> test_library_from_config(
    const storage::LibraryPath& lib_path,
    ...
    return std::make_shared<storage::Library>(
        lib_path, storage::create_storages(lib_path, storage::OpenMode::DELETE, storage_cfg), std::move(vs_cfg)
    );
}
