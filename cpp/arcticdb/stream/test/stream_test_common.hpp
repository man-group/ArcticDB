...
template <typename T, typename U> void check_value(const T& t, const U& u) {
  if (t != u)
    std::cout << "Oops";
...
arcticdb::SegmentInMemory fill_test_data_segment(const StreamDescriptor& tsd,
                                                 SegmentFiller&& filler) {
  SegmentInMemory seg;
...
SegmentInMemory fill_test_index_segment(const StreamId& tsid,
                                        TsIndexKeyGen&& ts_key_gen) {
  SegmentInMemory seg;
...
template <class DataType> uint64_t digit_mask() {
...
template <typename DataType,
          std::enable_if_t<std::is_floating_point_v<DataType>, int> = 0>
DataType get_floating_point_value_for_offset(size_t start_val, size_t i) {
  return DataType(start_val) + i / (10 * digit_mask<DataType>());
}

template <typename DataType, class ContainerType,
          std::enable_if_t<std::is_integral_v<DataType>, int> = 0>
void fill_test_index_vector(ContainerType& container, DataType, size_t num_rows,
                            size_t start_val) {
...
template <typename DataType, class ContainerType,
          std::enable_if_t<std::is_integral_v<DataType>, int> = 0>
void fill_test_value_vector(ContainerType& container, DataType, size_t num_rows,
                            size_t start_val) {
  for (size_t i = 0; i < num_rows; ++i) {
...
struct DefaultStringGenerator {
  ...
  static void fill_string_vector(std::vector<uint8_t>& vec,
                                 size_t num_rows) ARCTICDB_UNUSED {
...
template <class ContainerType, typename DTT,
          std::enable_if_t<std::is_floating_point_v<typename DTT::raw_type> ||
                               std::is_integral_v<typename DTT::raw_type>,
                           int> = 0>
NativeTensor test_column(ContainerType& container, DTT, shape_t num_rows,
...
template <class ContainerType, typename DTT,
          class StringGenerator = DefaultStringGenerator>
...
void fill_test_column(arcticdb::pipelines::InputTensorFrame& frame,
                      ContainerType& container, DTT data_type_tag, size_t num_rows,
                      size_t start_val, bool is_index) {
  ...
  if (!is_index) {
    if constexpr (std::is_integral_v<RawType> || std::is_floating_point_v<RawType>)
      frame.field_tensors.emplace_back(
          test_column(container, data_type_tag, num_rows, start_val, is_index));
    else
      frame.field_tensors.emplace_back(
          test_string_column(container, data_type_tag, num_rows, start_val, is_index));
  } else {
...
inline void fill_test_frame(SegmentInMemory& segment,
                            ...
                            size_t num_rows, size_t start_val, size_t opt_row_offset) {
  util::check(!segment.descriptor().empty(),
              "Can't construct test frame with empty descriptor");
  ...
  for (; field != segment.descriptor().end(); ++field) {
    visit_field(*field, [&](auto type_desc_tag) {
      using DTT = typename decltype(type_desc_tag)::DataTypeTag;
      fill_test_column(
          frame, segment.column(std::distance(segment.descriptor().begin(), field)),
          DTT{}, num_rows, start_val + opt_row_offset, false);
    });
  }
...
StreamDescriptor get_test_descriptor(const StreamId& id,
                                     const std::vector<FieldRef>& fields) {
  return IndexType::default_index().create_stream_descriptor(
      id, folly::Range(fields.begin(), fields.end()));
}

template <typename IndexType>
TestTensorFrame get_test_frame(const StreamId& id, const std::vector<FieldRef>& fields,
                               size_t num_rows, size_t start_val,
...
inline auto get_test_empty_timeseries_segment(StreamId id, size_t num_rows) {
  return SegmentInMemory{
      get_test_descriptor<stream::TimeseriesIndex>(id, get_test_timeseries_fields()),
      num_rows};
}

inline auto get_test_timeseries_frame(const StreamId& id, size_t num_rows,
                                      size_t start_val) {
  return get_test_frame<stream::TimeseriesIndex>(id, get_test_timeseries_fields(),
                                                 num_rows, start_val);
}

inline auto get_test_simple_frame(const StreamId& id, size_t num_rows,
                                  size_t start_val) {
  return get_test_frame<stream::RowCountIndex>(id, get_test_simple_fields(), num_rows,
                                               start_val);
}

inline std::pair<storage::LibraryPath, arcticdb::proto::storage::LibraryConfig>
test_config(const std::string& lib_name) {
  auto unique_lib_name =
      fmt::format("{}_{}", lib_name, util::SysClock::nanos_since_epoch());
  ...
  config.mutable_storage_by_id()->insert(
      google::protobuf::MapPair<std::decay_t<decltype(storage_id)>,
                                std::decay_t<decltype(lmdb_config)>>(storage_id,
                                                                     lmdb_config));
  return std::make_pair(library_path, config);
}

inline std::shared_ptr<storage::Library>
test_library_from_config(const storage::LibraryPath& lib_path,
...
/**
 * Uses the argument to create a LibraryConfig for LMDB storage and instantiate a
 * Library with it.
 *
 * Note: the VariantStoreConfig will be monostate. If you need a version store with
 * special config, then inline this function and modify the config.
 */
...
/**
 * Creates a PythonVersionStore with test_library() (LMDB storage and monostate
 * VariantStoreConfig).
 *
