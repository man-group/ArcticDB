...
inline void write_key_to_segment(SegmentInMemory& segment, const entity::AtomKey& key) {
  ...
  std::visit(
      [&segment](auto&& val) {
        segment.set_scalar(int(pipelines::index::Fields::start_index), val);
      },
      key.start_index());
  std::visit(
      [&segment](auto&& val) {
        segment.set_scalar(int(pipelines::index::Fields::end_index), val);
      },
      key.end_index());
  segment.set_scalar(int(pipelines::index::Fields::version_id), key.version_id());
  std::visit(
      [&segment](auto&& val) {
        segment.set_scalar(int(pipelines::index::Fields::stream_id), val);
      },
      key.id());
  ...
  segment.set_scalar(int(pipelines::index::Fields::key_type),
                     static_cast<uint8_t>(key.type()));
  segment.end_row();
}

template <class DataIndexType> class FlatIndexingPolicy {
