  ...
  public:
    ...
    Aggregator(
        ...
        : schema_policy_(std::move(schema)),
          ...
          // TODO presizing with expected rowsize  (using a rowcount segmenting policy) means that buffer sizes are
          // always irregular, unless
          //  the expected rowsize matches the column size, or some more clever logic is done so that if the row-count
          //  subdivides the buffer size then you get default buffer sizes.
          segment_(
              desc.value_or(schema_policy_.default_descriptor()),
  ...
  private:
    ...
    void set_scalar_by_name(std::string_view name, T val, DataType data_type) {
        position_t pos = schema_policy_.get_column_idx_by_name(
            segment_, name, make_scalar_type(data_type), segmenting_policy_.expected_row_size(), segment_.row_count()
        );
        set_scalar(pos, val);
    }
