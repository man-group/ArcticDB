...
template <class Sysclock = util::SysClock> class TimeBasedSegmentPolicy {
public:
  ...
  bool operator()(AggregationStats& stats) const {
    ...
    ARCTICDB_DEBUG(log::inmem(),
                   "TimeBasedSegmentPolicy AggregationStats diff={}, max_diff={}", diff,
                   max_diff_);
...
using VariantPolicyType =
    std::variant<NeverSegmentPolicy, RowCountSegmentPolicy, TimeBasedSegmentPolicy<>>;

template <int N> class ListOfSegmentPolicies {
public:
  ...
  [[nodiscard]] size_t expected_row_size() const {
    size_t val = 0;
    // TODO: Do it differently later, currently just using 0 since that's what we need
    // for tickdata
    return val;
  }
...
public:
  ...
  template <class C>
  Aggregator(SchemaPolicy&& schema, C&& c,
      ...
      : schema_policy_(std::move(schema)), row_builder_(schema_policy_, self()),
        callback_(std::forward<Callback>(c)), stats_(),
        segmenting_policy_(std::move(segmenting_policy)),
        // TODO presizing with expected rowsize  (using a rowcount segmenting policy)
        // means that buffer sizes are always irregular, unless
        //  the expected rowsize matches the column size, or some more clever logic is
        //  done so that if the row-count subdivides the buffer size then you get
        //  default buffer sizes.
        segment_(desc.value_or(schema_policy_.default_descriptor()),
                 row_count.value_or(segmenting_policy_.expected_row_size()), false,
                 SparsePolicy::allow_sparse) {
    segment_.init_column_map();
    if constexpr (!(std::is_same_v<Index, EmptyIndex> ||
                    std::is_same_v<Index, RowCountIndex>)) {
  ...
  template <class... Args> RowBuilderType& start_row(Args... args) {
  ...
  void set_sparse_block(position_t idx, ChunkedBuffer&& buffer, Buffer&& shapes,
                        util::BitSet&& bitset) {
    segment_.set_sparse_block(idx, std::move(buffer), std::move(shapes),
                              std::move(bitset));
  }
  ...
  void set_string_array(position_t pos, size_t string_size, size_t num_strings,
                        char* data) {
...
private:
  ...
  void set_scalar_by_name(std::string_view name, T val, DataType data_type) {
    position_t pos = schema_policy_.get_column_idx_by_name(
        segment_, name, make_scalar_type(data_type),
        segmenting_policy_.expected_row_size(), segment_.row_count());
    set_scalar(pos, val);
  }
  ...
  void set_string_by_name(std::string_view name, std::string_view str, DataType desc) {
    position_t pos = schema_policy_.get_column_idx_by_name(
        segment_, name, make_scalar_type(desc), segmenting_policy_.expected_row_size(),
        segment_.row_count());
    set_string(pos, str);
  }
...
using DynamicTimestampAggregator =
    Aggregator<TimeseriesIndex, DynamicSchema, RowCountSegmentPolicy,
               SparseColumnPolicy>;
} // namespace arcticdb::stream
