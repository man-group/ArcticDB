...
template <class Index, class Schema, class SegmentingPolicy, class DensityPolicy>
inline void
Aggregator<Index, Schema, SegmentingPolicy, DensityPolicy>::commit_impl(bool final) {
  ...
  segment_ = SegmentInMemory(schema_policy_.default_descriptor(),
                             segmenting_policy_.expected_row_size(), false,
...
inline void Aggregator<Index, Schema, SegmentingPolicy, DensityPolicy>::commit() {
  if (ARCTICDB_LIKELY(segment_.row_count() > 0 || segment_.metadata()) ||
      segment_.has_index_descriptor()) {
    commit_impl(false);
  }
}
