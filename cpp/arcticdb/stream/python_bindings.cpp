...
std::vector<FieldWrapper> field_collection_to_ref_vector(const FieldCollection& fields) {
    ...
    std::transform(fields.begin(), fields.end(), std::back_inserter(result), [](const Field& field) {
        return FieldWrapper{field.type(), field.name()};
    });
    return result;
}
...
#undef DATA_TYPE
    ...
    python_util::add_repr(
            py::class_<StreamDescriptor>(m, "StreamDescriptor")
                    .def(py::init(
                            [](StreamId stream_id, IndexDescriptorImpl idx_desc, const std::vector<FieldRef>& fields) {
                                ...
                                return util::variant_match(index, [&stream_id, &fields](auto idx_type) {
                                    return StreamDescriptor{
                                            index_descriptor_from_range(stream_id, idx_type, fields_from_range(fields))
                                    };
                                });
                            }
                    ))
                    ...
                    .def("fields",
                         [](const StreamDescriptor& desc) { return field_collection_to_ref_vector(desc.fields()); })
                    .def("sorted", &StreamDescriptor::sorted)
                    .def_property_readonly("index", [](const StreamDescriptor& self) { return self.index(); })
    );

    py::class_<TimeseriesDescriptor>(m, "TimeseriesDescriptor")
            .def_property_readonly(
                    "fields",
                    [](const TimeseriesDescriptor& desc) { return field_collection_to_ref_vector(desc.fields()); }
            )
            .def_property_readonly(
                    "normalization",
                    [](const TimeseriesDescriptor& self) { return python_util::pb_to_python(self.normalization()); }
            )
            .def_property_readonly("sorted", [](const TimeseriesDescriptor& self) { return self.sorted(); })
            .def_property_readonly("index", [](const TimeseriesDescriptor& self) { return self.index(); })
            .def_property_readonly("total_rows", [](const TimeseriesDescriptor& self) { return self.total_rows(); })
            .def_property_readonly(
                    "next_key",
                    [](const TimeseriesDescriptor& self) -> std::optional<AtomKey> {
