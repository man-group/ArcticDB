...
std::vector<FieldWrapper>
field_collection_to_ref_vector(const FieldCollection& fields) {
  auto result = std::vector<FieldWrapper>{};
  result.reserve(fields.size());
  std::transform(
      fields.begin(), fields.end(), std::back_inserter(result),
      [](const Field& field) { return FieldWrapper{field.type(), field.name()}; });
  return result;
}
...
#define VALUE_TYPE(__VT__) .value(#__VT__, ValueType::__VT__)
      VALUE_TYPE(UINT) VALUE_TYPE(INT) VALUE_TYPE(FLOAT) VALUE_TYPE(BOOL)
          VALUE_TYPE(NANOSECONDS_UTC) VALUE_TYPE(ASCII_FIXED) VALUE_TYPE(UTF8_FIXED)
              VALUE_TYPE(BYTES) VALUE_TYPE(UTF_DYNAMIC) VALUE_TYPE(EMPTY)
...
#define DATA_TYPE(__DT__) .value(#__DT__, DataType::__DT__)
      DATA_TYPE(UINT8) DATA_TYPE(UINT16) DATA_TYPE(UINT32) DATA_TYPE(UINT64)
          DATA_TYPE(INT8) DATA_TYPE(INT16) DATA_TYPE(INT32) DATA_TYPE(INT64)
              DATA_TYPE(FLOAT32) DATA_TYPE(FLOAT64) DATA_TYPE(BOOL8)
                  DATA_TYPE(NANOSECONDS_UTC64) DATA_TYPE(ASCII_FIXED64)
...
#undef DATA_TYPE
  ...
  m.def("as_dim_checked", &as_dim_checked,
        "Turns a uint8_t into a Dimension enum object");

  python_util::add_repr(
      py::class_<TypeDescriptor>(m, "TypeDescriptor")
  ...
  python_util::add_repr(
      py::class_<StreamDescriptor>(m, "StreamDescriptor")
          .def(py::init([](StreamId stream_id, IndexDescriptorImpl idx_desc,
            ...
            return util::variant_match(index, [&stream_id, &fields](auto idx_type) {
              return StreamDescriptor{index_descriptor_from_range(
                  stream_id, idx_type, fields_from_range(fields))};
            });
          }))
  ...
  py::class_<TimeseriesDescriptor>(m, "TimeseriesDescriptor")
      ...
      .def_property_readonly("normalization",
                             [](const TimeseriesDescriptor& self) {
                               return python_util::pb_to_python(self.normalization());
                             })
      .def_property_readonly(
          "sorted", [](const TimeseriesDescriptor& self) { return self.sorted(); })
      .def_property_readonly(
          "index", [](const TimeseriesDescriptor& self) { return self.index(); })
  ...
  py::class_<PyTimestampRange>(m, "TimestampRange")
      ...
      .def("as_tuple",
           [](const PyTimestampRange& rg) { return static_cast<TimestampRange>(rg); })
  ...
  m.def("create_timestamp_index_stream_descriptor",
        [](StreamId tsid, const std::vector<FieldRef>& fields) {
...
void register_stream_bindings(py::module& m) {
  ...
  py::class_<SegmentInMemory>(m, "SegmentInMemory")
      ...
      .def("string_pool", &SegmentInMemory::string_pool,
  ...
  py::class_<Agg, std::shared_ptr<Agg>>(m, "FixedTimestampAggregator")
      .def(py::init(
          [](std::shared_ptr<SegmentHolder> holder, const StreamDescriptor& desc) {
            return std::make_shared<Agg>(
                Agg::SchemaPolicy{desc, TimeseriesIndex::default_index()},
                [hld = holder](SegmentInMemory&& segment) {
                  hld->segment = std::move(segment);
                });
          }))
      .def_property_readonly("row_builder", &Agg::row_builder,
                             py::return_value_policy::reference)
  ...
  py::class_<FixedTickRowBuilder>(m, "FixedTickRowBuilder")
      .def("start_row", [](FixedTickRowBuilder& b,
                           entity::timestamp timestamp) { b.start_row(timestamp); })
