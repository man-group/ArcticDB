...
#define VALUE_TYPE(__VT__) .value(#__VT__, ValueType::__VT__)
        VALUE_TYPE(UINT) VALUE_TYPE(INT) VALUE_TYPE(FLOAT) VALUE_TYPE(BOOL) VALUE_TYPE(NANOSECONDS_UTC)
            VALUE_TYPE(ASCII_FIXED) VALUE_TYPE(UTF8_FIXED) VALUE_TYPE(BYTES) VALUE_TYPE(UTF_DYNAMIC) VALUE_TYPE(EMPTY)
...
#define DATA_TYPE(__DT__) .value(#__DT__, DataType::__DT__)
        DATA_TYPE(UINT8) DATA_TYPE(UINT16) DATA_TYPE(UINT32) DATA_TYPE(UINT64) DATA_TYPE(INT8) DATA_TYPE(INT16)
            DATA_TYPE(INT32) DATA_TYPE(INT64) DATA_TYPE(FLOAT32) DATA_TYPE(FLOAT64) DATA_TYPE(BOOL8)
                DATA_TYPE(NANOSECONDS_UTC64) DATA_TYPE(ASCII_FIXED64) DATA_TYPE(ASCII_DYNAMIC64) DATA_TYPE(UTF_FIXED64)
...
#undef DATA_TYPE
    ...
    python_util::add_repr(
        py::class_<StreamDescriptor>(m, "StreamDescriptor")
            ...
            .def("fields", [](const StreamDescriptor& desc) { return field_collection_to_ref_vector(desc.fields()); })
    );

    py::class_<TimeseriesDescriptor>(m, "TimeseriesDescriptor")
        .def_property_readonly(
            "fields", [](const TimeseriesDescriptor& desc) { return field_collection_to_ref_vector(desc.fields()); }
        )
        .def_property_readonly(
            "normalization",
