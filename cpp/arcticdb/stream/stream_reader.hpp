...
public:
  RowsFromSegIterator(const IndexRange& index_range, SegmentIt&& seg_it)
      : IndexRangeFilter(index_range), seg_it_(std::move(seg_it)), seg_(std::nullopt),
        row_id(0) {}
  ...
  std::optional<RowType> next(folly::Duration timeout = util::timeout::get_default()) {
    ...
    while (true) {
      ...
      auto res =
          std::make_optional<RowType>(seg_->template make_row_ref<RowType>(row_id));

      // Not filtering rows where we have a rowcount index - the assumption is that it's
      // essentially an un-indexed blob that we need to segment somehow.
      auto accept =
          index_type == IndexDescriptorImpl::Type::ROWCOUNT ||
          accept_index(
              pipelines::index::index_start_from_row(res.value(), index_type).value());
...
public:
  using KeyRangeIteratorType =
      KeyRangeIterator<typename decltype(KeySupplierType()())::const_iterator>;
  using IdxSegmentIteratorType =
      SegmentIterator<KeyRangeIteratorType, IDX_PREFETCH_WINDOW>;
  using KeysFromSegIteratorType = KeysFromSegIterator<IdxSegmentIteratorType>;
  using DataSegmentIteratorType =
      SegmentIterator<KeysFromSegIteratorType, DATA_PREFETCH_WINDOW>;
  using RowsIteratorType = RowsFromSegIterator<DataSegmentIteratorType, RowType>;

  StreamReader(KeySupplierType&& gen, std::shared_ptr<StreamSource> store,
               const storage::ReadKeyOpts& opts = storage::ReadKeyOpts{},
               const IndexRange& index_range = unspecified_range())
      : key_gen_(std::move(gen)), index_range_(index_range), store_(store), opts_(opts),
  ...
  auto generate_rows() {
    return folly::gen::from(key_gen_()) |
           ...
           generate_keys_from_segments(*store_, entity::KeyType::TABLE_DATA,
                                       entity::KeyType::TABLE_INDEX) |
  ...
  template <class Visitor> void foreach_row(Visitor&& visitor) {
