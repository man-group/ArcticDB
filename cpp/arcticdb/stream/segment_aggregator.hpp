...
class SegmentAggregator
    : public Aggregator<Index, Schema, SegmentingPolicy, DensityPolicy> {
public:
  ...
  SegmentAggregator(SliceCallBack&& slice_callback, Schema&& schema,
  ...
  void add_segment(SegmentInMemory&& seg, const pipelines::FrameSlice& slice,
    ...
    ARCTICDB_DEBUG(log::version(), "Adding segment with descriptor {}",
                   segment.descriptor());
  ...
  void commit() override {
    ...
    util::check(segments_.size() == slices_.size(),
                "Segment and slice size mismatch, {} != {}", segments_.size(),
                slices_.size());
    if (segments_.size() == 1) {
      // One segment, and it could be huge, so don't duplicate it
      AggregatorType::segment() = segments_[0];
      if (!DensityPolicy::allow_sparse) { // static schema must have all columns as
                                          // column slicing is removed
        auto descriptor = AggregatorType::default_descriptor();
        for (const auto& field :
             AggregatorType::segment().fields()) { // segment's index is not set up here
          if (!descriptor.find_field(
                  field.name())) { // TODO: Bottleneck for wide segments
            descriptor.add_field(
                field); // dynamic schema's default descriptor has no data column
          }
        }
