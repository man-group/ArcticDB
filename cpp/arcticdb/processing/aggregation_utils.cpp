...
void add_data_type_impl(entity::DataType data_type,
                        std::optional<entity::DataType>& current_data_type) {
  if (current_data_type.has_value()) {
    auto common_type =
        has_valid_common_type(entity::TypeDescriptor(*current_data_type, 0),
