...
/*
 * Contains an expression tree as a map of string node names to nodes. Nodes contain
 * leaves which may be references to node names.
 *
 * ExpressionContext also contains the name of the root node as well as a map of node
 * names to constant values.
 *
 * This class effectively contains as AST - there is likely a frontend implementation in
 * a higher level language (for example Python).
 */
struct ExpressionContext {
  ...
  public:
    ...
    std::shared_ptr<T> get_value(std::string name) { return map_.at(name); }
  };

  void add_expression_node(const std::string& name,
