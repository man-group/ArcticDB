...
template<typename InputType>
struct OutputType<
        InputType, typename std::enable_if_t<
...
void SumAggregatorData::aggregate(
    ...
    // On the first call to this method, data_type_ will be a type capable of representing all the values in all the
    // input columns This may be too small to hold the result, as summing 2 values of the same type cannot necessarily
    // be represented by that type For safety, use the widest type available for the 3 numeric flavours (unsigned int,
    // signed int, float) to have the best chance of avoiding overflow
                            ...
                            if constexpr (!is_sequence_type(col_type_info::data_type)) {
                                Column::for_each_enumerated<typename col_type_info::TDT>(
                                        *input_column->column_,
                                        [&out_ptr, &groups](auto enumerating_it) {
...
inline void aggregate_impl(
        const std::optional<ColumnWithStrings>& input_column, const std::vector<size_t>& groups, size_t unique_values,
        std::vector<uint8_t>& aggregated_, std::optional<DataType>& data_type_
) {
                ...
                details::visit_type(
                        ...
                        [&input_column, &groups, &out_ptr](auto col_tag) {
                            ...
                            if constexpr (!is_sequence_type(col_type_info::data_type)) {
                                Column::for_each_enumerated<typename col_type_info::TDT>(
                                        *input_column->column_,
                                        [&groups, &out_ptr](auto enumerating_it) {
                                            ...
                                            if constexpr (std::is_floating_point_v<ColRawType>) {
                                                ...
                                                if constexpr (T == Extremum::MAX) {
                                                    val.value_ =
                                                            std::max(val.value_, GlobalRawType(enumerating_it.value()));
                                                } else {
                                                    val.value_ =
                                                            std::min(val.value_, GlobalRawType(enumerating_it.value()));
                                                }
                                                val.written_ = true;
                                            }
                                        }
...
inline SegmentInMemory finalize_impl(
    ...
    if (!aggregated_.empty()) {
        ...
        auto col = std::make_shared<Column>(
                make_scalar_type(dynamic_schema ? dynamic_schema_data_type : data_type_.value()),
                unique_values,
                AllocationType::PRESIZED,
                Sparsity::NOT_PERMITTED
        );
        ...
        col->set_row_data(unique_values - 1);
        res.add_column(
                scalar_field(dynamic_schema ? dynamic_schema_data_type : data_type_.value(), output_column_name.value),
                col
        );
        details::visit_type(*data_type_, [&aggregated_, &column_data, unique_values, dynamic_schema](auto col_tag) {
            ...
            if (dynamic_schema) {
                ...
                for (auto it = column_data.begin<DynamicSchemaTDT>(); it != column_data.end<DynamicSchemaTDT>();
                     ++it, ++in_ptr) {
                ...
                for (auto it = column_data.begin<typename col_type_info::TDT>();
                     it != column_data.end<typename col_type_info::TDT>();
                     ++it, ++in_ptr) {
                    if constexpr (is_floating_point_type(col_type_info::data_type)) {
                        *it = in_ptr->written_ ? in_ptr->value_
...
SegmentInMemory MeanAggregatorData::finalize(const ColumnName& output_column_name, bool, size_t unique_values) {
    ...
    if (!fractions_.empty()) {
        fractions_.resize(unique_values);
        auto col = std::make_shared<Column>(
                make_scalar_type(DataType::FLOAT64),
                fractions_.size(),
                AllocationType::PRESIZED,
                Sparsity::NOT_PERMITTED
        );
