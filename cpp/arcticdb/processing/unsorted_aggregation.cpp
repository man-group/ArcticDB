...
SegmentInMemory MinMaxAggregatorData::finalize(const std::vector<ColumnName>& output_column_names) const {
    ...
    if (min_.has_value()) {
        details::visit_type(min_->data_type(), [&output_column_names, &seg, &header, this](auto col_tag) {
            ...
            stats_metadata->set_data_col_offset(data_col_offset_);
            stats_metadata->set_stats_seg_offset(0);
            stats_metadata->set_type(arcticc::pb2::descriptors_pb2::COLUMN_STATS_MIN_V2);
            ...
            stats_metadata->set_type(arcticc::pb2::descriptors_pb2::COLUMN_STATS_MAX_V2);
