...
struct OutputType<
    ...
    typename std::enable_if_t<
        is_signed_type(InputType::DataTypeTag::data_type) && is_integer_type(InputType::DataTypeTag::data_type)>> {
...
inline SegmentInMemory finalize_impl(
    ...
    if (!aggregated_.empty()) {
        ...
        col->set_row_data(unique_values - 1);
        res.add_column(
            scalar_field(dynamic_schema ? dynamic_schema_data_type : data_type_.value(), output_column_name.value), col
        );
...
SegmentInMemory MeanAggregatorData::finalize(const ColumnName& output_column_name, bool, size_t unique_values) {
    ...
    if (!fractions_.empty()) {
        fractions_.resize(unique_values);
        auto col = std::make_shared<Column>(
            make_scalar_type(DataType::FLOAT64), fractions_.size(), AllocationType::PRESIZED, Sparsity::NOT_PERMITTED
        );
        auto column_data = col->data();
        std::transform(
            fractions_.cbegin(),
        ...
        details::visit_type(*data_type_, [&input_column, unique_values, &groups, this](auto global_tag) {
            ...
            details::visit_type(
                ...
                [this, &groups, &out_ptr, &col_data](auto col_tag) {
                    ...
                    auto groups_pos = 0;
                    while (auto block =
                               col_data.next<TypeDescriptorTag<ColumnTagType, DimensionTag<entity::Dimension::Dim0>>>()
                    ) {
