...
public:
  template <typename TDT> class Grouper {
  public:
    ...
    // The multiple return statements in here are ugly, but avoids creating temporaries
    // as far as possible
    std::optional<uint8_t> group(RawType key,
                                 const std::shared_ptr<StringPool>& sp) const {
