...
VariantData transform_to_placeholder(VariantData data) {
  ...
  // where a true result is very common, but is a premature change to make at this
  // point.
...
// This conversion could be done within visit_binary_boolean with an explosion of the
// number of visitations to handle To avoid that, this function:
                ...
                [&column_with_strings, &output_bitset](auto col_tag) {
                  ...
                  if constexpr (is_bool_type(type_info::data_type)) {
                    Column::transform<typename type_info::TDT>(
                        *column_with_strings.column_, output_bitset, false,
