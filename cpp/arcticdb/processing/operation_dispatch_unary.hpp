...
VariantData unary_operator(const Value& val, Func&& func) {
    ...
    details::visit_type(val.type().data_type(), [&](auto val_tag) {
        ...
        using TargetType =
            typename unary_arithmetic_promoted_type<typename type_info::RawType, std::remove_reference_t<Func>>::type;
...
VariantData unary_operator(const ColumnWithStrings& col, Func&& func) {
    ...
    details::visit_type(col.column_->type().data_type(), [&](auto col_tag) {
        ...
        if constexpr (is_numeric_type(type_info::data_type)) {
            using TargetType =
                typename unary_arithmetic_promoted_type<typename type_info::RawType, std::remove_reference_t<Func>>::
                    type;
...
VariantData visit_unary_operator(const VariantData& left, Func&& func) {
    return std::visit(
        util::overload{
            [&](const ColumnWithStrings& l) -> VariantData { return unary_operator(l, std::forward<Func>(func)); },
