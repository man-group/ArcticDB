...
inline std::string unary_operation_to_string(Func&& func,
                                             std::string_view operand_str) {
  return fmt::format("{}({})", func, operand_str);
}

template <typename Func> VariantData unary_operator(const Value& val, Func&& func) {
  ...
  details::visit_type(val.type().data_type(), [&](auto val_tag) {
    ...
    using TargetType =
        typename unary_arithmetic_promoted_type<typename type_info::RawType,
...
VariantData visit_unary_operator(const VariantData& left, Func&& func) {
  return std::visit(
      util::overload{[&](const ColumnWithStrings& l) -> VariantData {
...
VariantData unary_comparator(const ColumnWithStrings& col, Func&& func) {
  if (is_empty_type(col.column_->type().data_type()) ||
      is_integer_type(col.column_->type().data_type())) {
    if constexpr (std::is_same_v<std::remove_reference_t<Func>, IsNullOperator>) {
      return is_empty_type(col.column_->type().data_type())
      ...
      internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
          "Unexpected operator passed to unary_comparator");
    }
  }

  util::BitSet output_bitset;
  constexpr auto sparse_missing_value_output =
      std::is_same_v<std::remove_reference_t<Func>, IsNullOperator>;
  details::visit_type(col.column_->type().data_type(), [&](auto col_tag) {
    using type_info = ScalarTypeInfo<decltype(col_tag)>;
    // Non-explicit lambda capture due to a bug in LLVM:
    // https://github.com/llvm/llvm-project/issues/34798
    Column::transform<typename type_info::TDT>(
        *(col.column_), output_bitset, sparse_missing_value_output,
      ...
      util::overload{
          [&](const ColumnWithStrings& l) -> VariantData {
            return transform_to_placeholder(
                unary_comparator(l, std::forward<decltype(func)>(func)));
          },
          [](EmptyResult) -> VariantData {
            if constexpr (std::is_same_v<std::remove_reference_t<Func>,
                                         IsNullOperator>) {
              return FullResult{};
            } else if constexpr (std::is_same_v<std::remove_reference_t<Func>,
                                                NotNullOperator>) {
