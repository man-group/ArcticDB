...
Column SortedAggregator<aggregation_operator, closed_boundary>::aggregate(
    ...
    details::visit_type(
            res.type().data_type(),
            [this, &input_index_columns, &input_agg_columns, &bucket_boundaries, &string_pool, &res](
                ...
                // Need this here to only generate valid get_bucket_aggregator code, exception will have been thrown
                // earlier at runtime
                constexpr bool supported_aggregation_type_combo =
                        is_numeric_type(output_type_info::data_type) || is_bool_type(output_type_info::data_type) ||
                            ...
                            details::visit_type(
                                    ...
                                    [this,
                                     ...
                                     &reached_end_of_buckets](auto input_type_desc_tag) {
                                        ...
                                        // Again, only needed to generate valid code below, exception will have been
                                        // thrown earlier at runtime
                                        if constexpr ((is_numeric_type(input_type_info::data_type) &&
                                            ...
                                            for (auto index_it = index_data.template cbegin<IndexTDT>();
                                                ...
                                                if (ARCTICDB_LIKELY(current_bucket.contains(*index_it))) {
                                                    ...
                                                    // The above code will be more performant when the vast majority of
                                                    // buckets are empty See comment in
                                                    // ResampleClause::advance_boundary_past_value for mathematical and
                                                    // experimental bounds
                                                    ++bucket_start_it;
                                                    if (ARCTICDB_UNLIKELY(++bucket_end_it == bucket_boundaries_end)) {
                                                        ...
                                                        while (ARCTICDB_UNLIKELY(index_value_past_end_of_bucket(
                                                            ...
                                                            ++bucket_start_it;
                                                            if (ARCTICDB_UNLIKELY(
                                                                        ++bucket_end_it == bucket_boundaries_end
                                                                )) {
                                                                reached_end_of_buckets = true;
                                                                break;
                                                            }
...
DataType SortedAggregator<aggregation_operator, closed_boundary>::generate_output_data_type(
        DataType common_input_data_type
) const {
