...
Column SortedAggregator<aggregation_operator, closed_boundary>::aggregate(
  ...
  Column res(
      ...
      output_index_column.row_count(), true, false);
  details::visit_type(res.type().data_type(), [this, &input_index_columns,
                                               &input_agg_columns, &bucket_boundaries,
                                               &string_pool,
    ...
    // Need this here to only generate valid get_bucket_aggregator code, exception will
    // have been thrown earlier at runtime
    constexpr bool supported_aggregation_type_combo =
        is_numeric_type(output_type_info::data_type) ||
      ...
      for (auto [idx, input_agg_column] : folly::enumerate(input_agg_columns)) {
        ...
        if (input_agg_column.has_value()) {
          details::visit_type(
              ...
              [this, &output_it, &output_end_it, &bucket_aggregator,
               &agg_column = *input_agg_column,
               &input_index_column = input_index_columns.at(idx),
               &bucket_boundaries_end, &string_pool, &bucket_start_it, &bucket_end_it,
               &current_bucket, &reached_end_of_buckets](auto input_type_desc_tag) {
                using input_type_info = ScalarTypeInfo<decltype(input_type_desc_tag)>;
                // Again, only needed to generate valid code below, exception will have
                // been thrown earlier at runtime
                if constexpr ((is_numeric_type(input_type_info::data_type) &&
                               is_numeric_type(output_type_info::data_type)) ||
                              (is_sequence_type(input_type_info::data_type) &&
                               (is_sequence_type(output_type_info::data_type) ||
                  ...
                  schema::check<ErrorCode::E_UNSUPPORTED_COLUMN_TYPE>(
                      !agg_column.column_->is_sparse() &&
                          agg_column.column_->row_count() ==
                              input_index_column->row_count(),
                  ...
                  auto agg_it =
                      agg_data.template cbegin<typename input_type_info::TDT>();
                  ...
                  for (auto index_it = index_data.template cbegin<IndexTDT>();
                    ...
                    if (ARCTICDB_LIKELY(current_bucket.contains(*index_it))) {
                      ...
                      // reached_end_of_buckets = bucket_end_it ==
                      // bucket_boundaries_end; The above code will be more performant
                      // when the vast majority of buckets are empty See comment in
                      // ResampleClause::advance_boundary_past_value for mathematical
                      // and experimental bounds
                      ++bucket_start_it;
                      if (ARCTICDB_UNLIKELY(++bucket_end_it == bucket_boundaries_end)) {
                        reached_end_of_buckets = true;
                      } else {
                        while (ARCTICDB_UNLIKELY(index_value_past_end_of_bucket(
                            *index_it, *bucket_end_it))) {
                          ++bucket_start_it;
                          if (ARCTICDB_UNLIKELY(++bucket_end_it ==
...
template <AggregationOperator aggregation_operator, ResampleBoundary closed_boundary>
DataType
SortedAggregator<aggregation_operator, closed_boundary>::generate_common_input_type(
    const std::vector<std::optional<ColumnWithStrings>>& input_agg_columns) const {
  ...
  for (const auto& opt_input_agg_column : input_agg_columns) {
    if (opt_input_agg_column.has_value()) {
      ...
      // Column is missing from this row-slice due to dynamic schema, currently
      // unsupported
      schema::raise<ErrorCode::E_UNSUPPORTED_COLUMN_TYPE>(
          "Resample: Cannot aggregate column '{}' as it is missing from some row "
          "slices",
          get_input_column_name().value);
    }
  }
...
template <AggregationOperator aggregation_operator, ResampleBoundary closed_boundary>
void SortedAggregator<aggregation_operator, closed_boundary>::
    check_aggregator_supported_with_data_type(DataType data_type) const {
  schema::check<ErrorCode::E_UNSUPPORTED_COLUMN_TYPE>(
...
template <AggregationOperator aggregation_operator, ResampleBoundary closed_boundary>
DataType
SortedAggregator<aggregation_operator, closed_boundary>::generate_output_data_type(
    DataType common_input_data_type) const {
  ...
  if constexpr (aggregation_operator == AggregationOperator::SUM) {
    ...
    if (is_unsigned_type(common_input_data_type) ||
        is_bool_type(common_input_data_type)) {
...
template <AggregationOperator aggregation_operator, ResampleBoundary closed_boundary>
bool SortedAggregator<aggregation_operator, closed_boundary>::
    index_value_past_end_of_bucket(timestamp index_value, timestamp bucket_end) const {
  if constexpr (closed_boundary == ResampleBoundary::LEFT) {
