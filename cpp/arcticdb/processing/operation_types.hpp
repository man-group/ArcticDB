...
constexpr bool is_binary_operation(OperationType o) { return uint8_t(o) >= uint8_t(OperationType::ADD); }
...
struct unary_arithmetic_promoted_type {
    ...
    using type = typename
        /* Unsigned ints promote to themselves for the abs operator, and to a signed int of double the width with the
         * neg operator Floating point types promote to themselves with both operators Signed ints promote to a signed
         * int of double the width for both operators, as their range is not symmetric about zero */
        std::conditional_t<
            ...
            std::conditional_t<
                ...
                std::conditional_t<
                    ...
                    std::conditional_t<
                        ...
                        /* Plus, Minus, and Times operators can overflow if using max_width, so promote to a wider
                         * signed type e.g. -100*100 (both int8_t's) = -10000, requiring int16_t to hold the result */
                        typename arithmetic_promoted_type::details::signed_width_t<2 * max_width>,
                        // IsIn/IsNotIn operators, just use the type of the widest input
                    ...
                    std::conditional_t<
                        ...
                        std::conditional_t<
                            ...
                            // If the signed type is strictly larger than the unsigned type, then promote to the signed
                            // type
                            typename arithmetic_promoted_type::details::signed_width_t<max_width>,
                            // Otherwise, check if the unsigned one is the widest type we support
                            std::conditional_t<
                                std::is_same_v<LHS, uint64_t> || std::is_same_v<RHS, uint64_t>,
                                // Retains ValueSetBaseType in binary_membership(), which handles mixed int64/uint64
                                // operations gracefully
                                RHS,
                                // There should be a signed type wider than the unsigned type, so both can be exactly
                                // represented
                                typename arithmetic_promoted_type::details::signed_width_t<2 * max_width>>>>>>>>;
};
...
struct IsNullOperator {
    template<
        typename tag,
        std::enable_if_t<std::is_same_v<tag, TimeTypeTag> || std::is_same_v<tag, StringTypeTag>, bool> = true>
