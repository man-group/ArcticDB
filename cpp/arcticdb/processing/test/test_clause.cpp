...
template <typename T>
void segment_scalar_assert_all_values_equal(const arcticdb::ProcessingUnit& proc_unit,
...
void segment_string_assert_all_values_equal(const arcticdb::ProcessingUnit& proc_unit,
                                            const arcticdb::ColumnName& name,
...
TEST(Clause, PartitionEmptyColumn) {
  ...
  PartitionClause<arcticdb::grouping::HashingGroupers,
...
TEST(Clause, AggregationEmptyColumn) {
  ...
  auto proc_unit = ProcessingUnit{
      generate_groupby_testing_segment(num_rows, unique_grouping_values)};
  auto entity_ids = push_entities(component_manager, std::move(proc_unit));

  auto aggregated =
      gather_entities(component_manager, aggregation.process(std::move(entity_ids)));
  ...
  // Min, max, mean, count, first and last aggregations should not be present in the
  // output segment
...
void check_column(arcticdb::SegmentInMemory segment, std::string_view column_name,
                  std::size_t ugv, F&& f) {
...
TEST(Clause, AggregationColumn) {
  ...
  check_column<int64_t>(*segments[0], "sum_int", unique_grouping_values,
                        [](size_t idx) { return 450 + 10 * idx; });
  check_column<int64_t>(*segments[0], "min_int", unique_grouping_values,
                        [](size_t idx) { return idx; });
  check_column<int64_t>(*segments[0], "max_int", unique_grouping_values,
                        [](size_t idx) { return 90 + idx; });
  check_column<double>(*segments[0], "mean_int", unique_grouping_values,
                       [](size_t idx) { return double(45 + idx); });
  check_column<uint64_t>(*segments[0], "count_int", unique_grouping_values,
                         [](size_t) { return 10; });
}
...
TEST(Clause, AggregationSparseColumn) {
  ...
  size_t unique_grouping_values{10};
  auto proc_unit = ProcessingUnit{
      generate_groupby_testing_sparse_segment(num_rows, unique_grouping_values)};
  auto entity_ids = push_entities(component_manager, std::move(proc_unit));
...
TEST(Clause, AggregationSparseGroupby) {
  ...
  auto proc_unit = ProcessingUnit{
      generate_sparse_groupby_testing_segment(num_rows, unique_grouping_values)};
  auto entity_ids = push_entities(component_manager, std::move(proc_unit));
  ...
  using aggregation_test::check_column;
  check_column<int64_t>(*segments[0], "sum_int", unique_groups,
                        [unique_grouping_values](size_t idx) -> int64_t {
                          if (idx == 0) {
                            return 495;
                          } else {
                            return 450 -
                                   static_cast<int64_t>(idx % unique_grouping_values);
                          }
                        });
  check_column<int64_t>(*segments[0], "min_int", unique_groups,
                        [](size_t idx) -> int64_t {
                          if (idx == 0) {
                            return 0;
                          } else {
  ...
  check_column<int64_t>(*segments[0], "max_int", unique_groups,
                        [unique_grouping_values](size_t idx) -> int64_t {
                          if (idx == 0) {
                            return 99;
                          } else if (idx == 9) {
  ...
  check_column<double>(
      *segments[0], "mean_int", unique_groups,
      [unique_grouping_values](size_t idx) -> double {
        if (idx == 0) {
          return 49.5;
        } else {
  ...
  check_column<uint64_t>(*segments[0], "count_int", unique_groups,
                         [](size_t idx) -> uint64_t {
                           if (idx == 0) {
                             return 10;
                           } else {
...
TEST(Clause, Passthrough) {
  ...
  auto ret =
      gather_entities(component_manager, passthrough.process(std::move(entity_ids)));
...
TEST(Clause, Sort) {
  ...
  auto res =
      gather_entities(component_manager, sort_clause.process(std::move(entity_ids)));
...
TEST(Clause, Split) {
  ...
  auto res =
      gather_entities(component_manager, split_clause.process(std::move(entity_ids)));
  ASSERT_TRUE(res.segments_.has_value());
...
TEST(Clause, Merge) {
  ...
  MergeClause merge_clause{TimeseriesIndex{"time"}, SparseColumnPolicy{}, stream_id,
                           descriptor};
  ...
  for (auto x = 0u; x < num_segs; ++x) {
    ...
    segs.emplace_back(
        SegmentInMemory{seg.descriptor().clone(), num_rows / num_segs, false});
  }
