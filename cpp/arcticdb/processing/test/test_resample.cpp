...
auto generate_bucket_boundaries(std::vector<timestamp>&& bucket_boundaries) {
  return [bucket_boundaries = std::move(bucket_boundaries)](
             timestamp, timestamp, std::string_view, ResampleBoundary, timestamp) {
    return bucket_boundaries;
  };
}

TEST(Resample, StructureForProcessingBasic) {
  // Bucket boundaries such that the first processing unit does not need any rows from
  // the second row slice No column slicing
  StreamId sym{"sym"};
  ColRange col_range{1, 2};
  RowRange row_range_1{0, 100};
  RowRange row_range_2{100, 200};
  auto key_1 =
      AtomKeyBuilder().start_index(0).end_index(1000).build<KeyType::TABLE_DATA>(sym);
  auto key_2 =
      AtomKeyBuilder().start_index(2000).end_index(3000).build<KeyType::TABLE_DATA>(
          sym);
  RangesAndKey top(row_range_1, col_range, key_1);
  RangesAndKey bottom(row_range_2, col_range, key_2);
  // Insert into vector "out of order" to ensure structure_for_processing reorders
  // correctly
  std::vector<RangesAndKey> ranges_and_keys{bottom, top};

  ResampleClause<ResampleBoundary::LEFT> resample_clause{
      "dummy", ResampleBoundary::LEFT,
      generate_bucket_boundaries({1, 500, 1500, 2500, 2999}), 0};
...
TEST(Resample, StructureForProcessingColumnSlicing) {
  // Bucket boundaries such that the first processing unit does not need any rows from
  // the second row slice Two column slices
  StreamId sym{"sym"};
  ...
  // Insert into vector "out of order" to ensure structure_for_processing reorders
  // correctly
  std::vector<RangesAndKey> ranges_and_keys{top_right, bottom_left, bottom_right,
                                            top_left};
...
TEST(Resample, StructureForProcessingOverlap) {
  // Bucket boundaries such that the first processing unit needs rows from the second
  // row slice No column slicing
  StreamId sym{"sym"};
  ColRange col_range{1, 2};
  RowRange row_range_1{0, 100};
  RowRange row_range_2{100, 200};
  ...
  ResampleClause<ResampleBoundary::LEFT> resample_clause{
      "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({1, 500, 2500, 2999}),
      0};
  auto proc_unit_ids = resample_clause.structure_for_processing(ranges_and_keys, 0);
  ASSERT_EQ(ranges_and_keys.size(), 2);
  ASSERT_EQ(ranges_and_keys[0], top);
...
TEST(Resample, StructureForProcessingSubsumed) {
  // Bucket boundaries such that the first processing unit needs all of the rows from
  // the second and third row slices, such that there is only one element in the
  // returned vector No column slicing
  StreamId sym{"sym"};
  ColRange col_range{1, 2};
  RowRange row_range_1{0, 100};
  RowRange row_range_2{100, 200};
  ...
  auto key_2 =
      AtomKeyBuilder().start_index(2000).end_index(3000).build<KeyType::TABLE_DATA>(
          sym);
  auto key_3 =
      AtomKeyBuilder().start_index(3000).end_index(4000).build<KeyType::TABLE_DATA>(
          sym);
  RangesAndKey top(row_range_1, col_range, key_1);
  ...
  ResampleClause<ResampleBoundary::LEFT> resample_clause{
      "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({1, 500, 4500}), 0};
  auto proc_unit_ids = resample_clause.structure_for_processing(ranges_and_keys, 0);
...
TEST(Resample, StructureForProcessingExactBoundary) {
  // Bucket boundaries such that the first processing unit needs rows from the second
  // row slice when the right boundary is closed, but does not when the left boundary is
  // closed No column slicing
  StreamId sym{"sym"};
  ColRange col_range{1, 2};
  RowRange row_range_1{0, 100};
  RowRange row_range_2{100, 200};
  ...
  ResampleClause<ResampleBoundary::LEFT> resample_clause_left{
      "dummy", ResampleBoundary::LEFT,
      generate_bucket_boundaries({1, 500, 2000, 2500, 2999}), 0};
  auto proc_unit_ids =
      resample_clause_left.structure_for_processing(ranges_and_keys, 0);
  ASSERT_EQ(ranges_and_keys.size(), 2);
  ASSERT_EQ(ranges_and_keys[0], top);
  ASSERT_EQ(ranges_and_keys[1], bottom);
  ...
  ResampleClause<ResampleBoundary::RIGHT> resample_clause_right{
      "dummy", ResampleBoundary::LEFT,
      generate_bucket_boundaries({1, 500, 2000, 2500, 2999}), 0};
...
TEST(Resample, FindBuckets) {
  ...
  ResampleClause<ResampleBoundary::LEFT> resample_left(
      "left", ResampleBoundary::LEFT, generate_bucket_boundaries({0, 10, 20, 30, 40}),
      0);
  ResampleClause<ResampleBoundary::RIGHT> resample_right(
      "right", ResampleBoundary::RIGHT, generate_bucket_boundaries({0, 10, 20, 30, 40}),
      0);

  resample_left.bucket_boundaries_ = resample_left.generate_bucket_boundaries_(
      0, 0, "dummy", ResampleBoundary::LEFT, 0);
  resample_right.bucket_boundaries_ = resample_right.generate_bucket_boundaries_(
      0, 0, "dummy", ResampleBoundary::RIGHT, 0);
...
TEST(Resample, ProcessOneSegment) {
  ...
  ResampleClause<ResampleBoundary::LEFT> resample(
      "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({-1, 2, 5}), 0);
  resample.bucket_boundaries_ =
      resample.generate_bucket_boundaries_(0, 0, "dummy", ResampleBoundary::LEFT, 0);
  ...
  using index_TDT = TypeDescriptorTag<DataTypeTag<DataType::NANOSECONDS_UTC64>,
                                      DimensionTag<Dimension ::Dim0>>;
  auto index_column = std::make_shared<Column>(static_cast<TypeDescriptor>(index_TDT{}),
                                               0, false, true);
  using col_TDT =
      TypeDescriptorTag<DataTypeTag<DataType::INT64>, DimensionTag<Dimension ::Dim0>>;
  auto sum_column =
      std::make_shared<Column>(static_cast<TypeDescriptor>(col_TDT{}), 0, false, true);
  size_t num_rows{5};
  for (size_t idx = 0; idx < num_rows; ++idx) {
    index_column->set_scalar<int64_t>(static_cast<ssize_t>(idx),
                                      static_cast<int64_t>(idx));
    sum_column->set_scalar<int64_t>(static_cast<ssize_t>(idx),
                                    static_cast<int64_t>(idx));
  }
  SegmentInMemory seg;
  seg.add_column(scalar_field(index_column->type().data_type(), "index"), index_column);
  seg.add_column(scalar_field(sum_column->type().data_type(), "sum_column"),
                 sum_column);
  ...
  auto resampled =
      gather_entities(component_manager, resample.process(std::move(entity_ids)));
...
TEST(Resample, ProcessMultipleSegments) {
  ...
  ResampleClause<ResampleBoundary::LEFT> resample(
      "dummy", ResampleBoundary::LEFT,
      generate_bucket_boundaries({-15, -5, 5, 6, 25, 35, 45, 46, 55, 65}), 0);
  resample.bucket_boundaries_ =
      resample.generate_bucket_boundaries_(0, 0, "dummy", ResampleBoundary::LEFT, 0);
  ...
  //   6 - 25: Covers the last value from the first segment and the first value of the
  //   second segment
  ...
  seg_0->add_column(scalar_field(index_column->type().data_type(), "index"),
                    index_column);
  seg_0->add_column(scalar_field(sum_column->type().data_type(), "sum_column"),
                    sum_column);
  seg_0->set_row_id(1);
  auto row_range_0 = std::make_shared<RowRange>(0, 2);
  auto col_range_0 = std::make_shared<ColRange>(1, 2);

  index_column = std::make_shared<Column>(static_cast<TypeDescriptor>(index_TDT{}), 0,
                                          false, true);
  sum_column =
      std::make_shared<Column>(static_cast<TypeDescriptor>(col_TDT{}), 0, false, true);
  ...
  seg_1->add_column(scalar_field(index_column->type().data_type(), "index"),
                    index_column);
  seg_1->add_column(scalar_field(sum_column->type().data_type(), "sum_column"),
                    sum_column);
  seg_1->set_row_id(2);
  ...
  seg_2->add_column(scalar_field(index_column->type().data_type(), "index"),
                    index_column);
  seg_2->add_column(scalar_field(sum_column->type().data_type(), "sum_column"),
                    sum_column);
  seg_2->set_row_id(2);
  ...
  auto resampled_0 =
      gather_entities(component_manager, resample.process(std::move(ids_0)));
  ...
  auto resampled_1 =
      gather_entities(component_manager, resample.process(std::move(ids_1)));
  ...
  auto resampled_2 =
      gather_entities(component_manager, resample.process(std::move(ids_2)));
