...
auto generate_bucket_boundaries(std::vector<timestamp>&& bucket_boundaries) {
    return [bucket_boundaries = std::move(bucket_boundaries
            )](timestamp, timestamp, std::string_view, ResampleBoundary, timestamp) { return bucket_boundaries; };
}
...
TEST(Resample, StructureForProcessingBasic) {
    ...
    ResampleClause<ResampleBoundary::LEFT> resample_clause{
        "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({1, 500, 1500, 2500, 2999}), 0
    };
...
TEST(Resample, StructureForProcessingOverlap) {
    ...
    ResampleClause<ResampleBoundary::LEFT> resample_clause{
        "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({1, 500, 2500, 2999}), 0
    };
    auto proc_unit_ids = resample_clause.structure_for_processing(ranges_and_keys);
    ASSERT_EQ(ranges_and_keys.size(), 2);
    ASSERT_EQ(ranges_and_keys[0], top);
...
TEST(Resample, StructureForProcessingExactBoundary) {
    ...
    ResampleClause<ResampleBoundary::LEFT> resample_clause_left{
        "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({1, 500, 2000, 2500, 2999}), 0
    };
...
TEST(Resample, FindBuckets) {
    ...
    ResampleClause<ResampleBoundary::LEFT> resample_left(
        "left", ResampleBoundary::LEFT, generate_bucket_boundaries({0, 10, 20, 30, 40}), 0
    );
    ResampleClause<ResampleBoundary::RIGHT> resample_right(
        "right", ResampleBoundary::RIGHT, generate_bucket_boundaries({0, 10, 20, 30, 40}), 0
    );

    resample_left.bucket_boundaries_ =
        resample_left.generate_bucket_boundaries_(0, 0, "dummy", ResampleBoundary::LEFT, 0);
    resample_right.bucket_boundaries_ =
        resample_right.generate_bucket_boundaries_(0, 0, "dummy", ResampleBoundary::RIGHT, 0);
...
TEST(Resample, ProcessMultipleSegments) {
    ...
    ResampleClause<ResampleBoundary::LEFT> resample(
        "dummy", ResampleBoundary::LEFT, generate_bucket_boundaries({-15, -5, 5, 6, 25, 35, 45, 46, 55, 65}), 0
    );
    resample.bucket_boundaries_ = resample.generate_bucket_boundaries_(0, 0, "dummy", ResampleBoundary::LEFT, 0);
