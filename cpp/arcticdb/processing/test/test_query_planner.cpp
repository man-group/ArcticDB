...
std::shared_ptr<FilterClause> make_filter(
    ...
    ec.add_expression_node("root", std::make_shared<ExpressionNode>(ColumnName(column), OperationType::IDENTITY));
...
OperationType root_operation(const FilterClause& filter) {
    return filter.expression_context_->expression_nodes_.get_value(filter.root_node_name_.value)->operation_type_;
}
