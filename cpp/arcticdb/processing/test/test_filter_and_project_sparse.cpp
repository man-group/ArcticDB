    ...
    util::BitSet binary_filter(
        ...
        util::variant_match(
            ...
            [&](std::string_view right_column_name) {
                expression_node =
                    std::make_shared<ExpressionNode>(ColumnName(left_column_name), ColumnName(right_column_name), op);
            },
            [&](double value) {
                expression_node =
            ...
            [&](std::unordered_set<double> value_set) {
                expression_node =
                    std::make_shared<ExpressionNode>(ColumnName(left_column_name), ValueSetName(value_set_name), op);
                expression_context->add_value_set(
                    value_set_name, std::make_shared<ValueSet>(std::make_shared<std::unordered_set<double>>(value_set))
                );
            }
        );
        expression_context->add_expression_node(root_node_name, expression_node);
...
TEST_F(FilterProjectSparse, BinaryArithmeticSparseColLongerThanDenseCol) {
    auto projected_column =
        binary_projection("dense_floats_1", std::string_view{"sparse_floats_2"}, OperationType::MUL);
    ASSERT_TRUE(projected_column->opt_sparse_map().has_value());
