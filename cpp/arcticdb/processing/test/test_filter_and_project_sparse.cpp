...
protected:
  void SetUp() override {
    ...
    sparse_floats_1 =
        input_segment.column_ptr(input_segment.column_index("sparse_floats_1").value());
    sparse_floats_2 =
        input_segment.column_ptr(input_segment.column_index("sparse_floats_2").value());
    dense_floats_1 =
        input_segment.column_ptr(input_segment.column_index("dense_floats_1").value());
    dense_floats_2 =
        input_segment.column_ptr(input_segment.column_index("dense_floats_2").value());
    sparse_bools =
        input_segment.column_ptr(input_segment.column_index("sparse_bools").value());
    proc_unit = ProcessingUnit(std::move(input_segment));
    proc_unit.set_expression_context(expression_context);
  }

  std::shared_ptr<Column> unary_projection(std::string_view input_column_name,
                                           OperationType op) {
    ...
    auto expression_node =
        std::make_shared<ExpressionNode>(ColumnName(input_column_name), op);
  ...
  util::BitSet binary_filter(
    ...
    util::variant_match(
        ...
        [&](double value) {
          expression_node = std::make_shared<ExpressionNode>(
              ColumnName(left_column_name), ValueName(value_name), op);
          expression_context->add_value(
              value_name, std::make_shared<Value>(value, DataType::FLOAT64));
        },
        [&](std::unordered_set<double> value_set) {
          ...
          expression_context->add_value_set(
              value_set_name,
              std::make_shared<ValueSet>(
                  std::make_shared<std::unordered_set<double>>(value_set)));
        });
    expression_context->add_expression_node(root_node_name, expression_node);
  ...
  binary_projection(std::string_view left_column_name,
  ...
  std::shared_ptr<ExpressionContext> expression_context{
      std::make_shared<ExpressionContext>()};
...
TEST_F(FilterProjectSparse, UnaryProjection) {
  ...
  for (auto idx = 0; idx < sparse_floats_1->row_count(); idx++) {
    ASSERT_FLOAT_EQ(sparse_floats_1->reference_at<double>(idx),
...
TEST_F(FilterProjectSparse, BinaryComparisonIsIn) {
  auto bitset = binary_filter("sparse_floats_2", std::unordered_set<double>{5.0},
                              OperationType::ISIN);
  for (auto idx = 0; idx <= sparse_floats_2->last_row(); idx++) {
    auto opt_input_value = sparse_floats_2->scalar_at<double>(idx);
...
TEST_F(FilterProjectSparse, BinaryComparisonSparseColSparseCol) {
  auto bitset = binary_filter("sparse_floats_1", std::string_view{"sparse_floats_2"},
                              OperationType::LT);
  for (auto idx = 0; idx <= sparse_floats_2->last_row(); idx++) {
...
TEST_F(FilterProjectSparse, BinaryComparisonDenseColDenseCol) {
  auto bitset = binary_filter("dense_floats_1", std::string_view{"dense_floats_2"},
                              OperationType::EQ);
...
TEST_F(FilterProjectSparse, BinaryComparisonSparseColShorterThanDenseCol) {
  auto bitset = binary_filter("sparse_floats_1", std::string_view{"dense_floats_1"},
                              OperationType::GT);
...
TEST_F(FilterProjectSparse, BinaryComparisonSparseColLongerThanDenseCol) {
  auto bitset = binary_filter("dense_floats_1", std::string_view{"sparse_floats_1"},
                              OperationType::GT);
  for (auto idx = 0; idx <= sparse_floats_1->last_row(); idx++) {
    if (idx <= dense_floats_1->last_row()) {
      auto opt_left_value = dense_floats_1->scalar_at<double>(idx);
...
TEST_F(FilterProjectSparse, BinaryArithmeticColVal) {
  auto projected_column =
      binary_projection("sparse_floats_1", double{10.0}, OperationType::MUL);
  ASSERT_EQ(sparse_floats_1->last_row(), projected_column->last_row());
  ASSERT_EQ(sparse_floats_1->row_count(), projected_column->row_count());
  ASSERT_EQ(sparse_floats_1->opt_sparse_map(), projected_column->opt_sparse_map());
  for (auto idx = 0; idx < sparse_floats_1->row_count(); idx++) {
    ASSERT_FLOAT_EQ(10.0 * sparse_floats_1->reference_at<double>(idx),
                    projected_column->reference_at<double>(idx));
  }
}

TEST_F(FilterProjectSparse, BinaryArithmeticSparseColSparseCol) {
  auto projected_column = binary_projection(
      "sparse_floats_1", std::string_view{"sparse_floats_2"}, OperationType::MUL);
  ASSERT_TRUE(projected_column->opt_sparse_map().has_value());
  ASSERT_EQ(sparse_floats_1->sparse_map() & sparse_floats_2->sparse_map(),
            projected_column->sparse_map());
  ...
  for (auto idx = 0; idx <= projected_column->last_row(); idx++) {
    ...
    if (opt_left_value.has_value() && opt_right_value.has_value()) {
      ...
      if (std::isnan(*opt_left_value * *opt_right_value)) {
        ...
        ASSERT_FLOAT_EQ(*opt_left_value * *opt_right_value,
                        *projected_column->scalar_at<double>(idx));
      }
    } else {
...
TEST_F(FilterProjectSparse, BinaryArithmeticDenseColDenseCol) {
  auto projected_column = binary_projection(
      "dense_floats_1", std::string_view{"dense_floats_2"}, OperationType::MUL);
  ...
  for (auto idx = 0; idx < dense_floats_1->last_row(); idx++) {
    ASSERT_FLOAT_EQ(dense_floats_1->reference_at<double>(idx) *
                        dense_floats_2->reference_at<double>(idx),
                    projected_column->reference_at<double>(idx));
  }
}
...
TEST_F(FilterProjectSparse, BinaryArithmeticSparseColShorterThanDenseCol) {
  auto projected_column = binary_projection(
      "sparse_floats_1", std::string_view{"dense_floats_1"}, OperationType::MUL);
  ASSERT_TRUE(projected_column->opt_sparse_map().has_value());
  ASSERT_EQ(*sparse_floats_1->opt_sparse_map(), *projected_column->opt_sparse_map());
  ASSERT_EQ(projected_column->row_count(), projected_column->sparse_map().count());
  for (auto idx = 0; idx <= projected_column->last_row(); idx++) {
...
TEST_F(FilterProjectSparse, BinaryArithmeticSparseColLongerThanDenseCol) {
  auto projected_column = binary_projection(
      "dense_floats_1", std::string_view{"sparse_floats_2"}, OperationType::MUL);
  ASSERT_TRUE(projected_column->opt_sparse_map().has_value());
  for (auto idx = 0; idx < dense_floats_1->row_count(); idx++) {
    ASSERT_EQ(sparse_floats_2->sparse_map().get_bit(idx),
              projected_column->sparse_map().get_bit(idx));
  }
  ASSERT_EQ(projected_column->row_count(), projected_column->sparse_map().count());
  for (auto idx = 0; idx <= projected_column->last_row(); idx++) {
    auto opt_left_value = dense_floats_1->scalar_at<double>(idx);
