...
TEST(HasValidTypePromotion, DifferentDimensions) {
  ...
  TypeDescriptor source(ValueType::UNKNOWN_VALUE_TYPE, SizeBits::UNKNOWN_SIZE_BITS,
                        Dimension::Dim0);
  TypeDescriptor target(ValueType::UNKNOWN_VALUE_TYPE, SizeBits::UNKNOWN_SIZE_BITS,
                        Dimension::Dim1);
...
TEST(HasValidTypePromotion, NonNumericTypes) {
  using namespace arcticdb;
  using namespace arcticdb::entity;
  TypeDescriptor non_numeric_source(ValueType::ASCII_FIXED, SizeBits::UNKNOWN_SIZE_BITS,
                                    Dimension::Dim0);
  ...
  TypeDescriptor non_numeric_target(ValueType::ASCII_FIXED, SizeBits::UNKNOWN_SIZE_BITS,
                                    Dimension::Dim0);
...
TEST(HasValidTypePromotion, EmptyToEverything) {
  ...
  for (int value_type = int(ValueType::UNKNOWN_VALUE_TYPE);
       value_type < int(ValueType::COUNT); ++value_type) {
    for (int size_bits = int(SizeBits::UNKNOWN_SIZE_BITS);
         size_bits < int(SizeBits::COUNT); ++size_bits) {
      const TypeDescriptor target(ValueType(value_type), SizeBits(size_bits),
                                  Dimension::Dim0);
...
TEST(HasValidTypePromotion, EverythingToEmpty) {
  ...
  for (int value_type = int(ValueType::UNKNOWN_VALUE_TYPE);
    ...
    for (int size_bits = int(SizeBits::UNKNOWN_SIZE_BITS);
         size_bits < int(SizeBits::COUNT); ++size_bits) {
      const TypeDescriptor source(ValueType(value_type), SizeBits(size_bits),
                                  Dimension::Dim0);
...
TEST(HasValidCommonType, UintInt) {
  ...
  // has_valid_common_type should be symmetric, these assertions are as above with the
  // arguments reversed
