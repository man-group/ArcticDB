...
TEST(OperationDispatch, unary_operator) {
  ...
  size_t num_rows = 100;
  auto int_column = ColumnWithStrings(
      std::make_unique<Column>(generate_int_column(num_rows)), "int_col");
  auto empty_column =
      ColumnWithStrings(std::make_unique<Column>(generate_empty_column()), "empty_col");
...
TEST(OperationDispatch, binary_operator) {
  ...
  // int col + empty col
  EXPECT_THROW(visit_binary_operator(int_column, empty_column, PlusOperator{}),
               SchemaException);
  // empty col + int col
  EXPECT_THROW(visit_binary_operator(empty_column, int_column, PlusOperator{}),
               SchemaException);
  // empty col + empty col
  EXPECT_THROW(visit_binary_operator(empty_column, empty_column, PlusOperator{}),
               SchemaException);
  // empty col + val
  EXPECT_THROW(visit_binary_operator(empty_column, value, PlusOperator{}),
               SchemaException);
  // val + empty col
  EXPECT_THROW(visit_binary_operator(value, empty_column, PlusOperator{}),
               SchemaException);
}
...
TEST(OperationDispatch, binary_comparator) {
  ...
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(int_column, int_column, LessThanOperator{})));
  // int col < val
  ...
  // int col < empty col
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(int_column, empty_column, LessThanOperator{})));
  // empty col < int col
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(empty_column, int_column, LessThanOperator{})));
  // empty col < empty col
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(empty_column, empty_column, LessThanOperator{})));
  // empty col < val
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(empty_column, value, LessThanOperator{})));
  // val < empty col
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_comparator(value, empty_column, LessThanOperator{})));
}
...
TEST(OperationDispatch, binary_membership) {
  ...
  auto empty_column =
      ColumnWithStrings(std::make_unique<Column>(generate_empty_column()), "empty_col");
  std::unordered_set<int64_t> raw_set{0, 23, 82, static_cast<int64_t>(num_rows) - 1,
                                      1000000};
  auto value_set = std::make_shared<ValueSet>(
      std::make_shared<std::unordered_set<int64_t>>(raw_set));
  ...
  auto results_bitset_0 = std::get<util::BitSet>(variant_data_0);
  for (size_t idx = 0; idx < num_rows; idx++) {
    ASSERT_EQ(raw_set.count(static_cast<int64_t>(idx)) > 0,
              results_bitset_0.get_bit(idx));
  }
  // int col isnotin set
  auto variant_data_1 =
      visit_binary_membership(int_column, value_set, IsNotInOperator{});
  ASSERT_TRUE(std::holds_alternative<util::BitSet>(variant_data_0));
  auto results_bitset_1 = std::get<util::BitSet>(variant_data_1);
  for (size_t idx = 0; idx < num_rows; idx++) {
    ASSERT_EQ(raw_set.count(static_cast<int64_t>(idx)) == 0,
              results_bitset_1.get_bit(idx));
  }
  // empty col isin set
  ASSERT_TRUE(std::holds_alternative<EmptyResult>(
      visit_binary_membership(empty_column, value_set, IsInOperator{})));
  // empty col isnotin set
  ASSERT_TRUE(std::holds_alternative<FullResult>(
      visit_binary_membership(empty_column, value_set, IsNotInOperator{})));
}