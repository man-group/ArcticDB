...
TEST(ComponentManager, Simple) {
  ...
  auto key_0 = std::make_shared<AtomKey>(
      AtomKeyBuilder().version_id(1).build("symbol_0", KeyType::TABLE_DATA));
  ...
  auto key_1 = std::make_shared<AtomKey>(
      AtomKeyBuilder().version_id(2).build("symbol_1", KeyType::TABLE_DATA));
  ...
  ASSERT_EQ(component_manager.get<std::shared_ptr<SegmentInMemory>>({id_0})[0],
            segment_0);
  ASSERT_EQ(component_manager
                .get_initial_expected_get_calls<std::shared_ptr<SegmentInMemory>>(id_0),
            expected_get_calls_0);
  ...
  EXPECT_THROW(component_manager.get<std::shared_ptr<SegmentInMemory>>({id_0}),
               InternalException);

  ASSERT_EQ(component_manager.get<std::shared_ptr<SegmentInMemory>>({id_1})[0],
            segment_1);
  ASSERT_EQ(component_manager.get<std::shared_ptr<SegmentInMemory>>({id_1})[0],
            segment_1);
  ASSERT_EQ(component_manager
                .get_initial_expected_get_calls<std::shared_ptr<SegmentInMemory>>(id_1),
            expected_get_calls_1);
  ...
  EXPECT_THROW(component_manager.get<std::shared_ptr<SegmentInMemory>>({id_1}),
               InternalException);
}
