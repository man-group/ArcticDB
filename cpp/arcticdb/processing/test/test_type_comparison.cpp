...
TEST(Comparisons, Simple) {
  ...
  static_assert(
      std::is_same_v<right_uint64_t_comparable_unsigned::left_type, uint32_t>);
  static_assert(
      std::is_same_v<right_uint64_t_comparable_unsigned::right_type, uint64_t>);

  // Smaller types where is_signed(left) and is_signed(right) are the same are promoted
  // to the common type
