...
auto generate_bucket_boundaries(std::vector<timestamp>&& bucket_boundaries) {
  return [bucket_boundaries = std::move(bucket_boundaries)](
             timestamp, timestamp, std::string_view, ResampleBoundary,
             timestamp) mutable { return bucket_boundaries; };
}

RC_GTEST_PROP(Resample, StructureForProcessing, ()) {
  StreamId sym{"sym"};
  const auto unsorted_index_values =
      *rc::gen::unique<std::vector<timestamp>>(rc::gen::arbitrary<timestamp>());
  RC_PRE(unsorted_index_values.size() > 0);
  const auto unsorted_bucket_boundaries =
      *rc::gen::unique<std::vector<timestamp>>(rc::gen::arbitrary<timestamp>());
  ...
  // Use a single column slice as the interesting behaviour to test is around
  // row-slicing
  ColRange col_range{1, 2};

  // Only the start and end index values of each row-slice is used in
  // structure_for_processing, so use 2 rows per row-slice
  ...
  for (size_t idx = 0; idx < num_row_slices; idx++) {
    ...
    auto key = AtomKeyBuilder()
                   .start_index(start_idx_value)
                   .end_index(end_idx_value)
                   .build<KeyType::TABLE_DATA>(sym);
  ...
  for (size_t idx = 0; idx < bucket_boundaries.size() - 1; idx++) {
    if (left_boundary_closed) {
      bucket_boundary_pairs.emplace_back(bucket_boundaries[idx],
                                         bucket_boundaries[idx + 1] - 1);
    } else { // right_boundary_closed
      bucket_boundary_pairs.emplace_back(bucket_boundaries[idx] + 1,
                                         bucket_boundaries[idx + 1]);
    }
  }
  ...
  // Map from bucket_id to indexes in sorted_ranges_and_keys of row-slices needed for
  // this bucket
  std::vector<std::vector<size_t>> bucket_to_row_range_map(bucket_boundary_pairs.size(),
                                                           std::vector<size_t>());
  for (const auto& [bucket_id, bucket_boundary_pair] :
       folly::enumerate(bucket_boundary_pairs)) {
  ...
  for (const auto& row_range_ids : bucket_to_row_range_map) {
    if (!row_range_ids.empty()) {
      if (current_range_idx.has_value() &&
          row_range_ids.front() == *current_range_idx) {
  ...
  if (left_boundary_closed) {
    ResampleClause<ResampleBoundary::LEFT> resample_clause{
        ...
        generate_bucket_boundaries(std::move(bucket_boundaries)), 0};
