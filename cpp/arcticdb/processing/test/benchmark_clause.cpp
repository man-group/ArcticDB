...
SegmentInMemory get_segment_for_merge(const StreamId& id, size_t num_rows, size_t start,
                                      size_t step) {
  auto segment = SegmentInMemory{get_test_descriptor<stream::TimeseriesIndex>(
                                     id, {scalar_field(DataType::UINT8, "column")}),
                                 num_rows};
...
void time_merge_on_segments(const std::vector<SegmentInMemory>& segments,
  ...
  MergeClause merge_clause{TimeseriesIndex{"time"}, DenseColumnPolicy{}, stream_id,
                           descriptor};
...
void BM_hash_grouping_int(benchmark::State& state) {
  ...
  std::mt19937 gen(rd());
  // uniform_int_distribution (validly) undefined for int8_t in MSVC, hence the casting
...
void BM_hash_grouping_string(benchmark::State& state) {
  ...
  std::random_device rd;
  std::mt19937 gen(rd());
  const std::string character_set{"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                  "0123456789`¬!\"£$%^&*()_+-=[]{};:'@#~,<.>/? "};
  ...
  grouping::HashingGroupers::Grouper<
      ScalarTagType<DataTypeTag<DataType::UTF_DYNAMIC64>>>
      grouper;
  grouping::ModuloBucketizer bucketizer(num_buckets);
...
BENCHMARK(BM_hash_grouping_int<int16_t>)
    ->Args({100'000, 10, 2})
    ->Args({100'000, 10'000, 2});
BENCHMARK(BM_hash_grouping_int<int32_t>)
    ->Args({100'000, 10, 2})
    ->Args({100'000, 100'000, 2});
BENCHMARK(BM_hash_grouping_int<int64_t>)
    ->Args({100'000, 10, 2})
    ->Args({100'000, 100'000, 2});
