...
TEST_F(GenerateNormMetaTest, IdenticalNormMetas) {
    auto single = single_index({.name="ts", .tz="UTC"});
    ASSERT_TRUE(MessageDifferencer::Equals(generate_norm_meta({single, single}, {}), single.norm_metadata_));
    auto multi = multi_index({.name="ts", .tz="UTC"});
    ASSERT_TRUE(MessageDifferencer::Equals(generate_norm_meta({multi, multi}, {}), multi.norm_metadata_));
}
...
TEST_F(GenerateNormMetaTest, MultiIndexFakeFieldPos) {
    ...
    // Feels like there should be a simpler way to get the forrect FieldDescriptor* to pass to TreatAsSet to ignore the
