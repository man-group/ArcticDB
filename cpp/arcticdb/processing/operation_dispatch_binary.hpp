...
VariantData binary_membership(const ColumnWithStrings& column_with_strings, ValueSet& value_set, Func&& func) {
    ...
    log::version().debug(
        "Filtered column of size {} down to {} bits", column_with_strings.column_->last_row() + 1, output_bitset.count()
    );
...
VariantData binary_comparator(const ColumnWithStrings& left, const ColumnWithStrings& right, Func&& func) {
    ...
    details::visit_type(left.column_->type().data_type(), [&](auto left_tag) {
        ...
        details::visit_type(right.column_->type().data_type(), [&](auto right_tag) {
            ...
            if constexpr (is_sequence_type(left_type_info::data_type) && is_sequence_type(right_type_info::data_type)) {
                ...
                // If one or both columns are fixed width strings, we need to strip trailing null characters to get
                // intuitive results
                ...
                Column::transform<typename left_type_info::TDT, typename right_type_info::TDT>(
                    ...
                    *right.column_,
                    output_bitset,
                    sparse_missing_value_output,
                    [&func, &left, &right, strip_fixed_width_trailing_nulls](auto left_value, auto right_value)
                        -> bool {
                        return func(
                ...
                using comp =
                    typename arcticdb::Comparable<typename left_type_info::RawType, typename right_type_info::RawType>;
                Column::transform<typename left_type_info::TDT, typename right_type_info::TDT>(
                    *left.column_,
                    *right.column_,
                    output_bitset,
                ...
                user_input::raise<ErrorCode::E_INVALID_USER_ARGUMENT>(
                    ...
                    binary_operation_with_types_to_string(
                        left.column_name_, left.column_->type(), func, right.column_name_, right.column_->type()
                    )
                );
            }
        });
    ...
    ARCTICDB_DEBUG(
        log::version(),
        "Filtered column of size {} down to {} bits",
    ...
    details::visit_type(column_with_strings.column_->type().data_type(), [&](auto col_tag) {
        ...
        details::visit_type(val.type().data_type(), [&](auto val_tag) {
            ...
            if constexpr (is_sequence_type(col_type_info::data_type) && is_sequence_type(val_type_info::data_type)) {
                ...
                auto value_offset =
                    column_with_strings.string_pool_->get_offset_for_column(value_string, *column_with_strings.column_);
                Column::transform<typename col_type_info::TDT>(
                    *column_with_strings.column_,
                    output_bitset,
                    sparse_missing_value_output,
                ...
                auto value = static_cast<typename comp::right_type>(
    ...
    details::visit_type(left.column_->type().data_type(), [&](auto left_tag) {
        ...
        details::visit_type(right.column_->type().data_type(), [&](auto right_tag) {
            ...
            Column::transform<
                ...
                *(right.column_),
                *output_column,
                [&func](auto left_value, auto right_value) -> TargetType { return func.apply(left_value, right_value); }
            );
        });
    });
    return VariantData(ColumnWithStrings(
        std::move(output_column), binary_operation_column_name(left.column_name_, func, right.column_name_)
    ));
}
...
VariantData binary_operator(const ColumnWithStrings& col, const Value& val, Func&& func) {
    schema::check<ErrorCode::E_UNSUPPORTED_COLUMN_TYPE>(
        !is_empty_type(col.column_->type().data_type()), "Empty column provided to binary operator"
    );
    std::unique_ptr<Column> output_column;
        ...
        details::visit_type(val.type().data_type(), [&](auto val_tag) {
            ...
            if constexpr (arguments_reversed) {
                ...
                Column::transform<typename col_type_info::TDT, ScalarTagType<DataTypeTag<output_data_type>>>(
                    *(col.column_),
                    *output_column,
                    [&func, raw_value](auto input_value) -> TargetType { return func.apply(raw_value, input_value); }
                );
            } else {
                ...
                Column::transform<typename col_type_info::TDT, ScalarTagType<DataTypeTag<output_data_type>>>(
                    *(col.column_),
                    *output_column,
                    [&func, raw_value](auto input_value) -> TargetType { return func.apply(input_value, raw_value); }
                );
            }
        });
    });
