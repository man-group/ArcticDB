...
using NormMetaDescriptor =
    std::shared_ptr<arcticdb::proto::descriptors::NormalizationMetadata>;

// Contains constant data about the clause identifiable at construction time
struct ClauseInfo {
  // Whether processing segments need to be split into new processing segments after
  // this clause's process method has finished
  bool requires_repartition_{false};
  // Whether it makes sense to combine this clause with specifying which columns to view
  // in a call to read or similar
  bool can_combine_with_column_selection_{true};
  // The names of the columns that are needed for this clause to make sense
  // Could either be on disk, or columns created by earlier clauses in the processing
  // pipeline
  std::optional<std::unordered_set<std::string>> input_columns_{std::nullopt};
  // The name of the index after this clause if it has been modified, std::nullopt
  // otherwise
...
// Changes how the clause behaves based on information only available after it is
// constructed
...
struct IClause {
  template <class Base> struct Interface : Base {
    // Reorders ranges_and_keys into the order they should be queued up to be read from
    // storage. Returns a vector where each element is a vector of indexes into
    // ranges_and_keys representing the segments needed for one ProcessingUnit.
    // TODO #732: Factor out start_from as part of
    // https://github.com/man-group/ArcticDB/issues/732
    ...
    [[nodiscard]] std::optional<std::vector<std::vector<EntityId>>>
    repartition(std::vector<std::vector<EntityId>>&& entity_ids_vec) const {
  ...
  template <class T>
  using Members =
      folly::PolyMembers<&T::structure_for_processing, &T::process, &T::repartition,
                         &T::clause_info, &T::set_processing_config,
...
struct PassthroughClause {
  ...
  [[nodiscard]] std::optional<std::vector<std::vector<EntityId>>>
  repartition(ARCTICDB_UNUSED std::vector<std::vector<EntityId>>&&) const {
    return std::nullopt;
  }
...
struct FilterClause {
  ...
  explicit FilterClause(std::unordered_set<std::string> input_columns,
      ...
      : expression_context_(
            std::make_shared<ExpressionContext>(std::move(expression_context))),
...
template <typename GrouperType, typename BucketizerType> struct PartitionClause {
  ClauseInfo clause_info_;
  std::shared_ptr<ComponentManager> component_manager_;
  ...
  explicit PartitionClause(const std::string& grouping_column)
      : processing_config_(), grouping_column_(grouping_column) {
  ...
  process(std::vector<EntityId>&& entity_ids) const {
    ...
    std::vector<ProcessingUnit> partitioned_procs =
        partition_processing_segment<GrouperType, BucketizerType>(
    ...
    for (auto&& partitioned_proc : partitioned_procs) {
      std::vector<EntityId> proc_entity_ids =
          push_entities(component_manager_, std::move(partitioned_proc));
  ...
  repartition(std::vector<std::vector<EntityId>>&& entity_ids_vec) const {
    schema::check<ErrorCode::E_COLUMN_DOESNT_EXIST>(
        std::any_of(entity_ids_vec.cbegin(), entity_ids_vec.cend(),
    ...
    max_num_buckets = std::min(
        max_num_buckets, static_cast<int64_t>(std::numeric_limits<bucket_id>::max()));
    // Preallocate results with expected sizes, erase later if any are empty
    std::vector<std::vector<EntityId>> res(max_num_buckets);
    // With an even distribution, expect each element of res to have
    // entity_ids_vec.size() elements
    for (auto& res_element : res) {
      res_element.reserve(entity_ids_vec.size());
    }
    // Experimentation shows flattening the entities into a single vector and a single
    // call to component_manager_->get is faster than not flattening and making multiple
    // calls
    ...
    std::erase_if(res, [](const std::vector<EntityId>& entity_ids) {
      return entity_ids.empty();
    });
    return res;
  }
...
struct NamedAggregator {
  NamedAggregator(const std::string& aggregation_operator,
      ...
      : aggregation_operator_(aggregation_operator),
        input_column_name_(input_column_name), output_column_name_(output_column_name) {
...
struct AggregationClause {
  ...
  [[noreturn]] std::vector<std::vector<size_t>>
  structure_for_processing(ARCTICDB_UNUSED const std::vector<RangesAndKey>&,
...
template <ResampleBoundary closed_boundary> struct ResampleClause {
  ClauseInfo clause_info_;
  std::shared_ptr<ComponentManager> component_manager_;
  ProcessingConfig processing_config_;
  std::string rule_;
  ResampleBoundary label_boundary_;
  // This will contain the data range specified by the user (if any) intersected with
  // the range of timestamps for the symbol
  std::optional<TimestampRange> date_range_;
  // Inject this as a callback in the ctor to avoid language-specific dependencies this
  // low down in the codebase
  std::function<std::vector<timestamp>(timestamp, timestamp, std::string_view,
                                       ResampleBoundary, timestamp)>
      generate_bucket_boundaries_;
  ...
  ResampleClause(
      const std::string& rule, ResampleBoundary label_boundary,
      std::function<std::vector<timestamp>(timestamp, timestamp, std::string_view,
                                           ResampleBoundary, timestamp)>&&
          generate_bucket_boundaries,
      timestamp offset)
      : rule_(rule), label_boundary_(label_boundary),
  ...
  [[nodiscard]] std::string rule() const { return rule_; }
  ...
  bool index_range_outside_bucket_range(timestamp start_index,
                                        timestamp end_index) const;

  // Advances the bucket boundary iterator to the end of the last bucket that includes a
  // value from a row slice with the given last index value
  void advance_boundary_past_value(
  ...
  generate_bucket_boundaries(timestamp first_ts, timestamp last_ts,
...
struct RemoveColumnPartitioningClause {
  ...
  void
  set_processing_config(ARCTICDB_UNUSED const ProcessingConfig& processing_config) {}
...
struct SortClause {
  ...
  explicit SortClause(std::string column) : column_(std::move(column)) {}
...
struct MergeClause {
  ...
  MergeClause(stream::Index index, const stream::VariantColumnPolicy& density_policy,
              const StreamId& stream_id, const StreamDescriptor& stream_descriptor)
      : index_(std::move(index)), density_policy_(density_policy),
        stream_id_(stream_id), stream_descriptor_(stream_descriptor) {
    clause_info_.requires_repartition_ = true;
  }

  [[nodiscard]] std::vector<std::vector<size_t>>
  structure_for_processing(std::vector<RangesAndKey>& ranges_and_keys, size_t) {
...
struct RowRangeClause {
  ...
  explicit RowRangeClause(RowRangeType row_range_type, int64_t n)
      : row_range_type_(row_range_type), n_(n) {}

  explicit RowRangeClause(int64_t start, int64_t end)
      : row_range_type_(RowRangeType::RANGE), user_provided_start_(start),
