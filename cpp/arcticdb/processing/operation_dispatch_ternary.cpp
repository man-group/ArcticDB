...
// This handles the filter case where we select based on another filter if condition is true, or a fixed bool value otherwise e.g.
// lazy_df = lazy_df[where(lazy_df["col1"] < 0, lazy_df["col2"] < 0, True)]
// arguments_reversed refers to a case like
// lazy_df = lazy_df[where(lazy_df["col1"] < 0, True, lazy_df["col2"] < 0)]
                ...
                if constexpr(left_type_info::data_type == right_type_info::data_type && is_dynamic_string_type(left_type_info::data_type)) {
                    ...
                    if (left.string_pool_ == right.string_pool_) {
                        ...
                        ternary_transform<typename left_type_info::TDT, typename right_type_info::TDT, typename left_type_info::TDT>(
                                ...
                                *(right.column_),
                                *output_column,
                                [](bool cond, entity::position_t left_val, entity::position_t right_val) -> typename entity::position_t {
                        ...
                        ternary_transform<typename left_type_info::TDT, typename right_type_info::TDT, typename left_type_info::TDT>(
                                ...
                                *(right.column_),
                                *output_column,
                                [&string_pool, &left, &right](bool cond, entity::position_t left_val,
                                                              entity::position_t right_val) -> typename entity::position_t {
                                    if (cond) {
                                        ...
                                        if (is_a_string(left_val)) {
                                            return string_pool->get(
                                                    left.string_pool_->get_const_view(left_val)).offset();
                                        } else {
                                            return left_val;
                                        }
                                    } else {
                                        if (is_a_string(right_val)) {
                                            return string_pool->get(
                                                    right.string_pool_->get_const_view(right_val)).offset();
                                        } else {
                                            return right_val;
                                        }
                                    }
...
// This handles the projection case where we produce a new column by selecting values from an input column where condition
// is true, or a fixed value otherwise e.g.
// lazy_df["new_col"] = where(lazy_df["col1"] < 0, lazy_df["col2"], 5)
// arguments_reversed refers to a case like
// lazy_df["new_col"] = where(lazy_df["col1"] < 0, 5, lazy_df["col2"])
template<bool arguments_reversed>
        ...
        details::visit_type(val.data_type(), [&](auto val_tag) {
            ...
            if constexpr(is_sequence_type(col_type_info::data_type) && is_sequence_type(val_type_info::data_type)) {
                if constexpr(is_dynamic_string_type(col_type_info::data_type)) {
                    output_column = std::make_unique<Column>(make_scalar_type(DataType::UTF_DYNAMIC64), Sparsity::PERMITTED);
                    // It would be nice if we could just reuse the input column's string pool, and insert the value into it
                    // In experiments this is x7-x40 times faster than the current approach, depending on the unique
                    ...
                    // As there is no efficient way to check if a string is in the pool, this optimisation cannot be applied
                    string_pool = std::make_shared<StringPool>();
                    ...
                    ternary_transform<typename col_type_info::TDT, typename col_type_info::TDT, arguments_reversed>(
                            ...
                            *(col.column_),
                            value_offset,
                            *output_column,
                            [&string_pool, &col](bool cond, entity::position_t left_val, entity::position_t right_val) -> entity::position_t {
                                if (cond) {
                                    // This is a bit faster than ColumnWithStrings::string_at_offset in this case as
                                    // none of the additional checks/code are useful
                                    if (is_a_string(left_val)) {
                ...
                ternary_transform<typename col_type_info::TDT, ScalarTagType<DataTypeTag<output_data_type>>, arguments_reversed>(
                        condition,
                        *(col.column_),
                        value,
                        *output_column,
...
VariantData ternary_operator(const util::BitSet& condition, const ColumnWithStrings& col, EmptyResult) {
    ...
    details::visit_type(col.column_->type().data_type(), [&](auto col_tag) {
        using col_type_info = ScalarTypeInfo<decltype(col_tag)>;
        if constexpr (is_dynamic_string_type(col_type_info::data_type) ||
                             is_numeric_type(col_type_info::data_type) ||
    ...
    return {ColumnWithStrings(std::move(output_column), string_pool, ternary_operation_column_name<arguments_reversed>(col.column_name_, ""))};
}
        ...
        details::visit_type(right.data_type(), [&](auto right_tag) {
            ...
            if constexpr(is_sequence_type(left_type_info::data_type) && is_sequence_type(right_type_info::data_type)) {
                if constexpr(left_type_info::data_type == right_type_info::data_type && is_dynamic_string_type(left_type_info::data_type)) {
                    ...
                    internal::raise<ErrorCode::E_ASSERTION_FAILURE>("Unexpected fixed-width string value in ternary operator");
                }
            } else if constexpr ((is_numeric_type(left_type_info::data_type) && is_numeric_type(right_type_info::data_type)) ||
                                 (is_bool_type(left_type_info::data_type) && is_bool_type(right_type_info::data_type))) {
                using TargetType = typename ternary_operation_promoted_type<typename left_type_info::RawType, typename right_type_info::RawType>::type;
...
// is true, or a fixed value otherwise e.g.
// lazy_df["new_col"] = where(lazy_df["col1"] < 0, lazy_df["col2"], 5)
// but we have dynamic schema enabled, and the column is missing from some row slices
// arguments_reversed refers to a case like
// lazy_df["new_col"] = where(lazy_df["col1"] < 0, 5, lazy_df["col2"])
template<bool arguments_reversed>
