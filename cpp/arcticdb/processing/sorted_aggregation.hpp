...
struct ISortedAggregator {
  template <class Base> struct Interface : Base {
    ...
    [[nodiscard]] Column
    aggregate(const std::vector<std::shared_ptr<Column>>& input_index_columns,
              const std::vector<std::optional<ColumnWithStrings>>& input_agg_columns,
              const std::vector<timestamp>& bucket_boundaries,
              const Column& output_index_column, StringPool& string_pool) const {
      return folly::poly_call<2>(*this, input_index_columns, input_agg_columns,
                                 bucket_boundaries, output_index_column, string_pool);
    }
  };

  template <class T>
  using Members = folly::PolyMembers<&T::get_input_column_name,
                                     &T::get_output_column_name, &T::aggregate>;
};

using SortedAggregatorInterface = folly::Poly<ISortedAggregator>;

template <ResampleBoundary closed_boundary> class Bucket {
public:
...
template <typename T> class SumAggregatorSorted {
public:
  void push(T value) {
...
template <typename T, bool TimeType = false> class MeanAggregatorSorted {
public:
  void push(T value) {
    if constexpr (std::is_floating_point_v<T>) {
      if (ARCTICDB_LIKELY(!std::isnan(value))) {
...
template <typename T, bool TimeType = false> class MinAggregatorSorted {
public:
  MinAggregatorSorted() {
...
template <typename T, bool TimeType = false> class MaxAggregatorSorted {
public:
  MaxAggregatorSorted() {
    if constexpr (!std::is_floating_point_v<T> && !TimeType) {
...
template <typename T, bool TimeType = false> class FirstAggregatorSorted {
public:
  void push(T value) {
...
template <typename T, bool TimeType = false> class LastAggregatorSorted {
public:
  void push(T value) {
    if constexpr (std::is_same_v<T, std::optional<std::string_view>>) {
...
public:
  template <typename T, bool TimeType = false> void push(T value) {
    if constexpr (std::is_same_v<T, std::optional<std::string_view>>) {
...
private:
  [[nodiscard]] DataType generate_common_input_type(
  ...
  [[nodiscard]] DataType
  generate_output_data_type(DataType common_input_data_type) const;
  [[nodiscard]] bool index_value_past_end_of_bucket(timestamp index_value,
                                                    timestamp bucket_end) const;

  template <DataType input_data_type, typename Aggregator, typename T>
  void push_to_aggregator(
      Aggregator& bucket_aggregator, T value,
      ARCTICDB_UNUSED const ColumnWithStrings& column_with_strings) const {
    if constexpr (is_time_type(input_data_type) &&
                  aggregation_operator == AggregationOperator::COUNT) {
      bucket_aggregator.template push<timestamp, true>(value);
    } else if constexpr (is_numeric_type(input_data_type) ||
  ...
  [[nodiscard]] auto
  finalize_aggregator(Aggregator& bucket_aggregator,
                      ARCTICDB_UNUSED StringPool& string_pool) const {
...
namespace fmt {
template <> struct formatter<arcticdb::AggregationOperator> {
