...
struct IGroupingAggregator {
  ...
  template <class T>
  using Members =
      folly::PolyMembers<&T::get_input_column_name, &T::get_output_column_name,
                         &T::get_aggregator_data>;
};
...
struct IColumnStatsAggregatorData {
  template <class Base> struct Interface : Base {
    void aggregate(const ColumnWithStrings& input_column) {
      folly::poly_call<0>(*this, input_column);
    }
  ...
  template <class T> using Members = folly::PolyMembers<&T::aggregate, &T::finalize>;
};
...
struct IColumnStatsAggregator {
  template <class Base> struct Interface : Base {
    ...
    [[nodiscard]] ColumnStatsAggregatorData get_aggregator_data() const {
      return folly::poly_call<2>(*this);
    }
  };
  ...
  template <class T>
  using Members =
      folly::PolyMembers<&T::get_input_column_name, &T::get_output_column_names,
                         &T::get_aggregator_data>;
};
