...
public:
  ...
  template <typename T>
  std::vector<EntityId>
  add(std::vector<T>&& components,
      const std::optional<std::vector<EntityId>>& ids = std::nullopt) {
    ...
    if constexpr (std::is_same_v<T, std::shared_ptr<SegmentInMemory>>) {
      ...
      static_assert(sizeof(T) == 0,
                    "Unsupported component type passed to ComponentManager::add");
    }
    return insertion_ids;
  }

  template <typename T>
  EntityId add(T component, std::optional<EntityId> id = std::nullopt,
               std::optional<uint64_t> expected_get_calls = std::nullopt) {
  ...
  template <typename T> std::vector<T> get(const std::vector<EntityId>& ids) {
    if constexpr (std::is_same_v<T, std::shared_ptr<SegmentInMemory>>) {
      ...
      // See https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p2593r0.html
      static_assert(sizeof(T) == 0,
                    "Unsupported component type passed to ComponentManager::get");
    }
  }

  template <typename T> uint64_t get_initial_expected_get_calls(EntityId id) {
    ...
    if constexpr (std::is_same_v<T, std::shared_ptr<SegmentInMemory>>) {
      ...
      // Hacky workaround for static_assert(false) not being allowed
      // See https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p2593r0.html
      static_assert(sizeof(T) == 0, "Unsupported component type passed to "
                                    "ComponentManager::get_initial_expected_get_calls");
    }
  }
...
private:
  ...
  public:
    explicit ComponentMap(std::string&& entity_type, bool track_expected_gets)
        : entity_type_(std::move(entity_type)),
          opt_expected_get_calls_map_(
              track_expected_gets
                  ? std::make_optional<
                        ankerl::unordered_dense::map<EntityId, uint64_t>>()
                  : std::nullopt),
          opt_expected_get_calls_initial_map_(
              track_expected_gets
                  ? std::make_optional<
                        ankerl::unordered_dense::map<EntityId, uint64_t>>()
                  : std::nullopt) {};
    ...
    void add(const std::vector<EntityId>& ids, std::vector<T>&& entities) {
      std::lock_guard<std::mutex> lock(mtx_);
      for (auto [idx, id] : folly::enumerate(ids)) {
        ARCTICDB_DEBUG(log::storage(), "Adding {} with id {}", entity_type_, id);
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            map_.try_emplace(id, std::move(entities[idx])).second,
            "Failed to insert {} with ID {}, already exists", entity_type_, id);
    ...
    void add(EntityId id, T&& entity,
             std::optional<uint64_t> expected_get_calls = std::nullopt) {
      std::lock_guard<std::mutex> lock(mtx_);
      ARCTICDB_DEBUG(log::storage(), "Adding {} with id {}", entity_type_, id);
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          map_.try_emplace(id, std::move(entity)).second,
          "Failed to insert {} with ID {}, already exists", entity_type_, id);
      if (opt_expected_get_calls_map_.has_value()) {
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            expected_get_calls.has_value() && *expected_get_calls > 0,
            "Failed to insert {} with ID {}, must provide expected gets", entity_type_,
            id);
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            opt_expected_get_calls_map_->try_emplace(id, *expected_get_calls).second,
            "Failed to insert {} with ID {}, already exists", entity_type_, id);
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            opt_expected_get_calls_initial_map_->try_emplace(id, *expected_get_calls)
                .second,
            "Failed to insert {} with ID {}, already exists", entity_type_, id);
      }
    }
    std::vector<T> get(const std::vector<EntityId>& ids) {
      ...
      for (auto id : ids) {
        ...
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            entity_it != map_.end(), "Requested non-existent {} with ID {}",
            entity_type_, id);
        ...
        if (opt_expected_get_calls_map_.has_value()) {
          ...
          internal::check<ErrorCode::E_ASSERTION_FAILURE>(
              ...
              "Requested non-existent {} with ID {}", entity_type_, id);
          if (--expected_get_calls_it->second == 0) {
            ARCTICDB_DEBUG(log::storage(),
                           "{} with id {} has been fetched the expected number of "
                           "times, erasing from component manager",
                           entity_type_, id);
    ...
    uint64_t get_initial_expected_get_calls(EntityId id) {
      std::lock_guard<std::mutex> lock(mtx_);
      ARCTICDB_DEBUG(log::storage(),
                     "Getting initial expected get calls of {} with id {}",
                     entity_type_, id);
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          opt_expected_get_calls_initial_map_.has_value(),
          "Cannot get initial expected get calls for {} as they are not being tracked",
          entity_type_);
      auto it = opt_expected_get_calls_initial_map_->find(id);
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          it != opt_expected_get_calls_initial_map_->end(),
          "Requested non-existent {} with ID {}", entity_type_, id);
      return it->second;
    }
  ...
  private:
    ...
    // If not nullopt, tracks the number of calls to get for each entity id, and erases
    // from maps when it has been called this many times
    std::optional<ankerl::unordered_dense::map<EntityId, uint64_t>>
        opt_expected_get_calls_map_;
    std::optional<ankerl::unordered_dense::map<EntityId, uint64_t>>
        opt_expected_get_calls_initial_map_;
    std::mutex mtx_;
