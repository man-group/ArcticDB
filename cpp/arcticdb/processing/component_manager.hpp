    ...
    void add_entity(EntityId id, Args... args) {
        ...
        (
            [&] {
                ...
                // never accessed externally, so make this a named component.
                if constexpr (std::is_same_v<Args, EntityFetchCount>) {
                    auto&& remaining_entity_fetch_count_registry =
                        registry_.storage<EntityFetchCount>(remaining_entity_fetch_count_id);
    ...
    void replace_entities(const std::vector<EntityId>& ids, T value) {
        for (auto id : ids) {
            ...
            if constexpr (std::is_same_v<T, EntityFetchCount>) {
                ...
                remaining_entity_fetch_count_registry.patch(id, [value](EntityFetchCount& entity_fetch_count) {
                    entity_fetch_count = value;
                });
            }
        }
    }
        ...
        for (auto [idx, id] : folly::enumerate(ids)) {
            ...
            if constexpr (std::is_same_v<T, EntityFetchCount>) {
                auto&& remaining_entity_fetch_count_registry =
                    registry_.storage<EntityFetchCount>(remaining_entity_fetch_count_id);
                // For some reason named storages don't have a replace API
                remaining_entity_fetch_count_registry.patch(id, [&values, idx](EntityFetchCount& entity_fetch_count) {
                    entity_fetch_count = values[idx];
                });
            }
        }
    }
