...
void ProcessingUnit::apply_filter(util::BitSet&& bitset,
                                  PipelineOptimisation optimisation) {
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      segments_.has_value() && row_ranges_.has_value() && col_ranges_.has_value(),
      "ProcessingUnit::apply_filter requires all of segments, row_ranges, and "
      "col_ranges to be present");
  auto filter_down_stringpool = optimisation == PipelineOptimisation::MEMORY;

  for (auto&& [idx, segment] : folly::enumerate(*segments_)) {
    auto seg = filter_segment(*segment, std::move(bitset), filter_down_stringpool);
    auto num_rows = seg.is_null() ? 0 : seg.row_count();
    row_ranges_->at(idx) = std::make_shared<pipelines::RowRange>(
        row_ranges_->at(idx)->first, row_ranges_->at(idx)->first + num_rows);
    auto num_cols = seg.is_null() ? 0
                                  : seg.descriptor().field_count() -
                                        seg.descriptor().index().field_count();
    col_ranges_->at(idx) = std::make_shared<pipelines::ColRange>(
        col_ranges_->at(idx)->first, col_ranges_->at(idx)->first + num_cols);
...
void ProcessingUnit::truncate(size_t start_row, size_t end_row) {
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      segments_.has_value() && row_ranges_.has_value() && col_ranges_.has_value(),
      "ProcessingUnit::truncate requires all of segments, row_ranges, and col_ranges "
      "to be present");
  ...
  return util::variant_match(
      ...
      [&](const ColumnName& column_name) {
        for (const auto& segment : *segments_) {
          ...
          if (auto opt_idx = segment->column_index(column_name.value)) {
            return VariantData(ColumnWithStrings(
                segment->column_ptr(position_t(position_t(opt_idx.value()))),
                segment->string_pool_ptr(), column_name.value));
          }
        }
        // Try multi-index column names
        std::string multi_index_column_name =
            fmt::format("__idx__{}", column_name.value);
        for (const auto& segment : *segments_) {
          if (auto opt_idx = segment->column_index(multi_index_column_name)) {
            return VariantData(
                ColumnWithStrings(segment->column_ptr(position_t(*opt_idx)),
                                  segment->string_pool_ptr(), column_name.value));
          }
        }
        ...
        if (expression_context_ && !expression_context_->dynamic_schema_) {
          ...
          log::version().debug("Column {} not found in {}", column_name,
      ...
      [&](const ValueSetName& value_set_name) {
        return VariantData(
            expression_context_->value_sets_.get_value(value_set_name.value));
      },
      [&](const ExpressionName& expression_name) {
        if (auto computed = computed_data_.find(expression_name.value);
          ...
          auto expr =
              expression_context_->expression_nodes_.get_value(expression_name.value);
...
std::vector<ProcessingUnit> split_by_row_slice(ProcessingUnit&& proc) {
  ...
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(input.segments_.has_value(),
                                                  "split_by_row_slice needs Segments");
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(input.row_ranges_.has_value(),
                                                  "split_by_row_slice needs RowRanges");
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(input.col_ranges_.has_value(),
                                                  "split_by_row_slice needs ColRanges");
  auto include_expected_get_calls =
      input.segment_initial_expected_get_calls_.has_value();

  std::vector<ProcessingUnit> output;
  // Some clauses (e.g. AggregationClause) are lossy about row-ranges. We can assume
  // that if all of the input column ranges start with zero, that every segment belongs
  // to a different logical row-slice
  if (std::all_of(input.col_ranges_->begin(), input.col_ranges_->end(),
                  [](const auto& col_range) { return col_range->start() == 0; })) {
    output.reserve(input.segments_->size());
    for (size_t idx = 0; idx < input.segments_->size(); ++idx) {
      ProcessingUnit proc_tmp(std::move(*input.segments_->at(idx)),
                              std::move(*input.row_ranges_->at(idx)),
                              std::move(*input.col_ranges_->at(idx)));
      if (include_expected_get_calls) {
        proc_tmp.set_segment_initial_expected_get_calls(
    ...
    for (auto [idx, row_range_ptr] : folly::enumerate(*input.row_ranges_)) {
      if (auto it = output_map.find(*row_range_ptr); it != output_map.end()) {
        ...
        if (include_expected_get_calls) {
          inserted_it->second.segment_initial_expected_get_calls_.emplace(
              1, input.segment_initial_expected_get_calls_->at(idx));
        }
      }
    }
  ...
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      !output.empty(), "Unexpected empty output in split_by_row_slice");
  if (include_expected_get_calls) {
    // The expected get counts for all segments in a row slice should be the same
    // This should always be 1 or 2 for the first/last row slice, and 1 for all of the
    // others
    for (auto row_slice = output.cbegin(); row_slice != output.cend(); ++row_slice) {
      ...
      uint64_t max_expected_get_calls =
          row_slice == output.cbegin() || row_slice == std::prev(output.cend()) ? 2 : 1;
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          0 < expected_get_calls && expected_get_calls <= max_expected_get_calls,
          "expected_get_calls in split_by_row_slice should be 1 or 2, got {}",
          expected_get_calls);
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          std::all_of(
              row_slice->segment_initial_expected_get_calls_->begin(),
              row_slice->segment_initial_expected_get_calls_->end(),
              [&expected_get_calls](uint64_t i) { return i == expected_get_calls; }),
          "All segments in same row slice should have same expected_get_calls in "
          "split_by_row_slice");
    }
  }
