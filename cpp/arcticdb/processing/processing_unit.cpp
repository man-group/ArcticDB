...
void ProcessingUnit::truncate(size_t start_row, size_t end_row) {
    ...
    for (auto&& [idx, segment] : folly::enumerate(*segments_)) {
        ...
        auto num_rows = seg.is_null() ? 0 : seg.row_count();
        row_ranges_->at(idx) =
            std::make_shared<pipelines::RowRange>(row_ranges_->at(idx)->first, row_ranges_->at(idx)->first + num_rows);
        auto num_cols = seg.is_null() ? 0 : seg.descriptor().field_count() - seg.descriptor().index().field_count();
        col_ranges_->at(idx) =
            std::make_shared<pipelines::ColRange>(col_ranges_->at(idx)->first, col_ranges_->at(idx)->first + num_cols);
        segments_->at(idx) = std::make_shared<SegmentInMemory>(std::move(seg));
    }
}
        ...
        [&](const ColumnName& column_name) {
            for (const auto& segment : *segments_) {
                ...
                if (auto opt_idx = segment->column_index(column_name.value)) {
                    return VariantData(ColumnWithStrings(
                        segment->column_ptr(position_t(position_t(*opt_idx))),
            ...
            for (const auto& segment : *segments_) {
                if (auto opt_idx = segment->column_index(multi_index_column_name)) {
                    return VariantData(ColumnWithStrings(
                        segment->column_ptr(position_t(*opt_idx)), segment->string_pool_ptr(), column_name.value
                    ));
                }
            }
...
std::vector<ProcessingUnit> split_by_row_slice(ProcessingUnit&& proc) {
    ...
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        input.row_ranges_.has_value(), "split_by_row_slice needs RowRanges"
    );
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
    ...
    if (std::all_of(input.col_ranges_->begin(), input.col_ranges_->end(), [](const auto& col_range) {
        ...
        for (size_t idx = 0; idx < input.segments_->size(); ++idx) {
            ProcessingUnit proc_tmp(
                std::move(*input.segments_->at(idx)),
    ...
    if (include_entity_fetch_count) {
        ...
        for (auto row_slice = output.cbegin(); row_slice != output.cend(); ++row_slice) {
            ...
            uint64_t max_entity_fetch_count =
                row_slice == output.cbegin() || row_slice == std::prev(output.cend()) ? 2 : 1;
            internal::check<ErrorCode::E_ASSERTION_FAILURE>(
                0 < entity_fetch_count && entity_fetch_count <= max_entity_fetch_count,
