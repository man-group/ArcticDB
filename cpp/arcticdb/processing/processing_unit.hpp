...
/*
 * A processing unit is designed to be used in conjunction with the clause processing
 * framework. At the start of each clause's process method ProcessingUnits are
 * constructed from the provided entity IDs. All clauses at time of writing need
 * segments, row ranges, and column ranges. Some also require atom keys and the
 * partitioning bucket. In this case the previous clause must have populated these
 * fields in the component manager for the specified entity IDs, otherwise an assertion
 * will fail. At the end of the clause process method, any of these optional fields that
 * are present will be pushed to the component manager. For the components stored in
 * vectors, the vectors must be the same length, and the segment, row range, column
 * range, and atom key that share an index in their respective vectors are associated.
 *
 * In addition, the expression context is a constant, representing the AST for computing
 * expressions in filter and projection clauses. computed_data_ holds a map from a
 * string representation of a [sub-]expression of the AST to a computed value of this
 * expression. This way, if an expression appears twice in the AST, we will only compute
 * it once.
 */
struct ProcessingUnit {
  ...
  ProcessingUnit(SegmentInMemory&& seg,
    ...
    auto cols = col_range.value_or(
        ColRange(0, segment_in_memory.is_null()
                        ? 0
                        : segment_in_memory.descriptor().field_count() -
                              segment_in_memory.descriptor().index().field_count()));
    segments_.emplace(
        {std::make_shared<SegmentInMemory>(std::move(segment_in_memory))});
  ...
  void set_segment_initial_expected_get_calls(
  ...
  void
  set_expression_context(const std::shared_ptr<ExpressionContext>& expression_context) {
    expression_context_ = expression_context;
  }

  // The name argument to this function is either a column/value name, or uniquely
  // identifies an ExpressionNode object. If this function has been called before with
  // the same ExpressionNode name, then we cache the result in the computed_data_ map to
...
inline std::vector<pipelines::SliceAndKey> collect_segments(ProcessingUnit&& proc) {
  ...
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      proc.segments_.has_value() && proc.row_ranges_.has_value() &&
          proc.col_ranges_.has_value(),
      "collect_segments requires all of segments, row_ranges, and col_ranges to be "
      "present");
  for (auto&& [idx, segment] : folly::enumerate(*proc.segments_)) {
    pipelines::FrameSlice frame_slice(*proc.col_ranges_->at(idx),
                                      *proc.row_ranges_->at(idx));
...
std::pair<std::vector<bucket_id>, std::vector<uint64_t>>
get_buckets(const ColumnWithStrings& col, const Grouper& grouper,
            const Bucketizer& bucketizer) {
  // Mapping from row to bucket
  // 255 reserved for Nones and NaNs in string/float columns
  // Faster to initialise to 255 and use a raw ptr for the output than to call
  // emplace_back repeatedly
  std::vector<bucket_id> row_to_bucket(col.column_->last_row() + 1,
...
partition_processing_segment(ProcessingUnit& input,
  ...
  if (std::holds_alternative<ColumnWithStrings>(get_result)) {
    ...
    partitioning_column.column_->type().visit_tag([&output, &input,
                                                   &partitioning_column](
                                                      auto type_desc_tag) {
      ...
      using ResolvedGrouperType =
          typename GrouperType::template Grouper<TypeDescriptorTag>;
      ...
      if constexpr (!is_empty_type(TagType::data_type)) {
        ...
        if (num_buckets > std::numeric_limits<bucket_id>::max()) {
          log::version().warn("GroupBy partitioning buckets capped at {} (received {})",
                              std::numeric_limits<bucket_id>::max(), num_buckets);
        ...
        auto [row_to_bucket, bucket_counts] =
            get_buckets(partitioning_column, grouper, bucketizer);
          ...
          for (auto&& [output_idx, new_seg] : folly::enumerate(new_segs)) {
            if (bucket_counts.at(output_idx) > 0) {
              if (!procs.at(output_idx).segments_.has_value()) {
                procs.at(output_idx).segments_ =
                    std::make_optional<std::vector<std::shared_ptr<SegmentInMemory>>>();
                procs.at(output_idx).row_ranges_ = std::make_optional<
                    std::vector<std::shared_ptr<pipelines::RowRange>>>();
                procs.at(output_idx).col_ranges_ = std::make_optional<
                    std::vector<std::shared_ptr<pipelines::ColRange>>>();
              }
              procs.at(output_idx)
                  .segments_->emplace_back(
                      std::make_shared<SegmentInMemory>(std::move(new_seg)));
              procs.at(output_idx)
                  .row_ranges_->emplace_back(input.row_ranges_->at(input_idx));
              procs.at(output_idx)
                  .col_ranges_->emplace_back(input.col_ranges_->at(input_idx));
            }
          }
        }
