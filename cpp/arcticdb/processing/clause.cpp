...
[[nodiscard]] std::string ProjectClause::to_string() const {
    return expression_context_
               ? fmt::format("PROJECT Column[\"{}\"] = {}", output_column_, expression_context_->root_node_name_.value)
               : "";
}
...
std::vector<std::vector<EntityId>> AggregationClause::structure_for_processing(
    ...
    auto max_num_buckets =
        ConfigsMap::instance()->get_int("Partition.NumBuckets", async::TaskScheduler::instance()->cpu_thread_count());
...
std::vector<EntityId> AggregationClause::process(std::vector<EntityId>&& entity_ids) const {
    ...
    std::sort(std::begin(row_slices), std::end(row_slices), [](const auto& left, const auto& right) {
    ...
    for (auto& row_slice : row_slices) {
        for (auto agg_data : folly::enumerate(aggregators_data)) {
            ...
            internal::check<ErrorCode::E_ASSERTION_FAILURE>(
                std::all_of(
                    row_slice.row_ranges_->begin(),
                    row_slice.row_ranges_->end(),
                    [&](const auto& row_range) { return row_range->start() == row_slice.row_ranges_->at(0)->start(); }
                ),
                    ...
                    Column::for_each_enumerated<typename col_type_info::TDT>(*col.column_, [&](auto enumerating_it) {
                        ...
                        if constexpr (is_sequence_type(col_type_info::data_type)) {
                            ...
                            if (auto it = offset_to_group.find(offset); it != offset_to_group.end()) {
                                ...
                                offset_to_group.insert(std::make_pair<typename col_type_info::RawType, size_t>(
                        ...
                        if (auto it = hash_to_group->find(val); it == hash_to_group->end()) {
                            *row_to_group_ptr++ = next_group_id;
                            ...
                            hash_to_group->insert(std::make_pair<typename col_type_info::RawType, size_t>(
...
std::vector<std::vector<size_t>> ResampleClause<closed_boundary>::structure_for_processing(
    ...
    TimestampRange index_range(
        std::min_element(
            ...
            ranges_and_keys.end(),
            [](const RangesAndKey& left, const RangesAndKey& right) { return left.start_time() < right.start_time(); }
        )->start_time(),
        std::max_element(
            ranges_and_keys.begin(),
            ranges_and_keys.end(),
            [](const RangesAndKey& left, const RangesAndKey& right) { return left.end_time() < right.end_time(); }
        )->end_time()
    );
    ...
    bucket_boundaries_ =
        generate_bucket_boundaries_(date_range_->first, date_range_->second, rule_, closed_boundary, offset_);
...
std::vector<std::vector<EntityId>> ResampleClause<closed_boundary>::structure_for_processing(
    ...
    auto [segments, row_ranges, col_ranges] =
        component_manager_
            ->get_entities<std::shared_ptr<SegmentInMemory>, std::shared_ptr<RowRange>, std::shared_ptr<ColRange>>(
                entity_ids, false
            );
    ...
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        std::all_of(
            expected_fetch_counts.begin(),
            expected_fetch_counts.end(),
...
std::vector<EntityId> ResampleClause<closed_boundary>::process(std::vector<EntityId>&& entity_ids) const {
    ...
    // We can use the last timestamp from the first row-slice's index column, as by construction
    // (structure_for_processing) the bucket covering this value will cover the remaining index values this call is
    // responsible for
    ...
    for (const auto& aggregator : aggregators_) {
        ...
        auto aggregated_column = std::make_shared<Column>(aggregator.aggregate(
            input_index_columns, input_agg_columns, bucket_boundaries, *output_index_column, string_pool
        ));
        seg.add_column(
            scalar_field(aggregated_column->type().data_type(), aggregator.get_output_column_name().value),
            aggregated_column
        );
    }
...
[[nodiscard]] std::vector<EntityId> RemoveColumnPartitioningClause::process(std::vector<EntityId>&& entity_ids) const {
    ...
    if (output_seg.has_value()) {
        output = push_entities(
            *component_manager_,
            ProcessingUnit(
                std::move(*output_seg), RowRange{min_start_row, max_end_row}, ColRange{min_start_col, max_end_col}
            )
        );
    }
    return output;
...
void merge_impl(
    ...
    auto func = [&component_manager, &ret, &col_range, start_row = row_range.first](auto&& segment) mutable {
        ...
        ret.emplace_back(push_entities(
            *component_manager,
            ProcessingUnit{std::forward<decltype(segment)>(segment), RowRange{start_row, end_row}, col_range}
        ));
        start_row = end_row;
    };
...
std::vector<std::vector<EntityId>> MergeClause::structure_for_processing(
    ...
    std::vector<std::vector<EntityId>> ret;
    std::visit(
        [this, &ret, &input_streams, &comp = compare, stream_id = stream_id_, &row_range, &col_range](
...
std::vector<std::vector<EntityId>> RowRangeClause::structure_for_processing(
    ...
    auto [segments, old_row_ranges, col_ranges] =
        component_manager_
            ->get_entities<std::shared_ptr<SegmentInMemory>, std::shared_ptr<RowRange>, std::shared_ptr<ColRange>>(
                entity_ids, false
            );
...
void RowRangeClause::calculate_start_and_end(size_t total_rows) {
    ...
    case RowRangeType::RANGE:
        ...
        start_ =
            (user_provided_start_ >= 0 ? std::min(user_provided_start_, signed_total_rows)
                                       : std::max(signed_total_rows + user_provided_start_, static_cast<int64_t>(0)));
        end_ =
            (user_provided_end_ >= 0 ? std::min(user_provided_end_, signed_total_rows)
