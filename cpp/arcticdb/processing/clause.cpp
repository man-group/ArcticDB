...
std::vector<EntityId> FilterClause::process(std::vector<EntityId>&& entity_ids) const {
    ...
    ARCTICDB_RUNTIME_DEBUG(
            log::memory(), "Doing filter {} for entity ids {}", expression_context_->root_node_name_, entity_ids
    );
...
[[nodiscard]] std::string ProjectClause::to_string() const {
    return expression_context_
                   ? fmt::format(
                             "PROJECT Column[\"{}\"] = {}", output_column_, expression_context_->root_node_name_.value
                     )
                   : "";
}
            ...
            details::visit_type(
                    ...
                    [&row_slice,
                     ...
                     this](auto data_type_tag) {
                        ...
                        // too low?
                        // Tested with 100,000,000 row dataframe with 100,000 unique values in the grouping column.
                        // Timings: 11.14 seconds without caching 11.01 seconds with caching Not worth worrying about
                        // right now
...
std::vector<std::vector<EntityId>> ResampleClause<closed_boundary>::structure_for_processing(
    ...
    auto [segments, row_ranges, col_ranges] = component_manager_->get_entities<
            ...
            std::shared_ptr<ColRange>>(entity_ids, false);
