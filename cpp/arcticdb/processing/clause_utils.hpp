...
struct RangesAndEntity {
    explicit RangesAndEntity(
        EntityId id,
        std::shared_ptr<RowRange> row_range,
        std::shared_ptr<ColRange> col_range,
    ...
    timestamp start_time() const { return timestamp_range_->first; }

    timestamp end_time() const { return timestamp_range_->second; }
