...
enum IncrementRefCount { ON, OFF };

inline void prefill_with_none(
        PyObject** ptr_dest, size_t num_rows, size_t sparse_count, SpinLock& spin_lock,
...
inline std::vector<NamedAggregator> named_aggregators_from_dict(
    ...
    for (const auto& [output_column_name, var_agg_named_agg] : aggregations) {
        util::variant_match(
                ...
                [&named_aggregators,
                 &output_column_name](const std::pair<std::string, std::string>& input_col_and_agg) {
                    named_aggregators.emplace_back(
                            input_col_and_agg.second, input_col_and_agg.first, output_column_name
                    );
                }
        );
    }
