...
class ARCTICDB_VISIBILITY_HIDDEN PyRowRef : public py::tuple {
  ...
  explicit PyRowRef(const RowRef& row_ref)
      : py::tuple(row_ref.col_count()), row_ref_(row_ref) {
    ...
    for (position_t col = 0; col < position_t(segment.num_columns()); ++col) {
      visit_field(segment.column_descriptor(col), [this, &segment, &col,
                                                   row_pos](auto impl) {
        ...
        if constexpr (T::DimensionTag::value == Dimension::Dim0) {
          if constexpr (T::DataTypeTag::data_type == DataType::ASCII_DYNAMIC64 ||
            ...
            set_col(
                col,
                segment.scalar_at<RawType>(row_pos, col).value()); // TODO handle sparse
          }
        } else {
          // TODO handle utf too
          if (T::DataTypeTag::data_type == DataType::ASCII_FIXED64) {
            ...
            auto string_refs =
                segment.tensor_at<entity::position_t>(row_pos, col).value();
...
private:
  ...
  template <class O> void set_col(std::size_t col, O&& o) const {
...
template <typename Msg> py::object pb_to_python(const Msg& out) {
...
template <typename Msg> void pb_from_python(const py::object& py_msg, Msg& out) {
 ...
 */
template <class PyClass> PyClass& add_repr(PyClass& py_class) {
  py_class.def("__repr__",
               [](const typename PyClass::type& a) { return fmt::format("{}", a); });
  return py_class;
}
...
public:
  PyTimestampRange(const py::object& start, const py::object& end)
    ...
    util::check_arg(start_ <= end_, "expected star <= end, actual {}, {}", start_,
                    end_);
  }
  ...
  for (auto& res : ret) {
    util::variant_match(
        ...
        [&lst](ReadResult& read_result) {
          ...
          lst.append(py::make_tuple(read_result.item, std::move(read_result.frame_data),
                                    pynorm, pyuser_meta, multi_key_meta,
...
// 1: key is the column name to aggregate, value is the aggregation operator. Output
// column name will be the same as input column name 2: key is the column name to
// output, value is a pair where the first element is the input column name, and the
// second element is the aggregation operator These 2 styles can be mixed and matched
inline std::vector<NamedAggregator> named_aggregators_from_dict(
    const std::unordered_map<
        std::string, std::variant<std::string, std::pair<std::string, std::string>>>
        aggregations) {
  ...
  for (const auto& [output_column_name, var_agg_named_agg] : aggregations) {
    util::variant_match(
        ...
        [&named_aggregators, &output_column_name](const std::string& agg_operator) {
          named_aggregators.emplace_back(agg_operator, output_column_name,
                                         output_column_name);
        },
        [&named_aggregators, &output_column_name](
            const std::pair<std::string, std::string>& input_col_and_agg) {
          named_aggregators.emplace_back(input_col_and_agg.second,
                                         input_col_and_agg.first, output_column_name);
        });
  }
...
inline auto pd_to_offset(std::string_view rule) {
  static py::object to_offset = py::module::import("pandas")
                                    .attr("tseries")
                                    .attr("frequencies")
                                    .attr("to_offset");
