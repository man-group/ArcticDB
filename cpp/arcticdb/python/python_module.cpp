...
void register_log(py::module&& log) {
  ...
  py::enum_<LoggerId>(log, "LoggerId")
      ...
      .export_values();
  auto choose_logger =
      [&](LoggerId log_id) -> decltype(arcticdb::log::storage()) /* logger ref */ {
    switch (log_id) {
  ...
  log.def("log", [&](LoggerId log_id, spdlog::level::level_enum level,
    ...
    case spdlog::level::level_enum::err:
      ...
      break;
    default:
      arcticdb::util::raise_rte("Unsupported log level",
...
#define EXPOSE_TYPE(LABEL, TYPE)                                                       \
  m.def("get_config_" #LABEL, [](const std::string& label) {                           \
    return ConfigsMap::instance()->get_##LABEL(label);                                 \
  });                                                                                  \
  m.def("set_config_" #LABEL, [](const std::string& label, TYPE value) {               \
    ConfigsMap::instance()->set_##LABEL(label, value);                                 \
  });                                                                                  \
  m.def("unset_config_" #LABEL, [](const std::string& label) {                         \
    ConfigsMap::instance()->unset_##LABEL(label);                                      \
  });
...
void register_error_code_ecosystem(
    py::module& m, py::exception<arcticdb::ArcticException>& base_exception) {
  using namespace arcticdb;
  ...
  static py::exception<InternalException> internal_exception(m, "InternalException",
                                                             compat_exception.ptr());
  static py::exception<StorageException> storage_exception(m, "StorageException",
                                                           compat_exception.ptr());
  static py::exception<LMDBMapFullException> lmdb_map_full_exception(
      m, "LmdbMapFullError", storage_exception.ptr());
  static py::exception<UserInputException> user_input_exception(m, "UserInputException",
                                                                compat_exception.ptr());
  ...
  py::register_exception_translator([](std::exception_ptr p) {
    try {
      ...
      std::string msg = fmt::format(
          "E5003: LMDB map is full. Close and reopen your LMDB backed Arctic instance "
          "with a "
          ...
          "group__mdb.html#gaa2506ec8dab3d969b0e609cd82e619e5). "
          "LMDB info: message=[{}]",
          e.what());
      ...
      // let stop iteration bubble up, since this is how python implements iteration
      // termination
      std::rethrow_exception(p);
    } catch (const std::exception& e) {
      std::string msg =
          fmt::format("{}({})", arcticdb::get_type_name(typeid(e)), e.what());
      internal_exception(msg.c_str());
    }
  });

  py::register_exception<storage::DuplicateKeyException>(m, "DuplicateKeyException",
                                                         storage_exception.ptr());
  py::register_exception<PermissionException>(m, "PermissionException",
                                              storage_exception.ptr());
  ...
  auto sorting_exception = py::register_exception<SortingException>(
      m, "SortingException", compat_exception.ptr());
  py::register_exception<UnsortedDataException>(m, "UnsortedDataException",
                                                sorting_exception.ptr());
...
void reinit_scheduler() {
  ARCTICDB_DEBUG(arcticdb::log::version(),
                 "Post-fork, reinitializing the task scheduler");
  arcticdb::async::TaskScheduler::reattach_instance();
}

void reinit_lmdb_warning() {
  ARCTICDB_DEBUG(arcticdb::log::version(),
                 "Post-fork in child, resetting LMDB warning counter");
...
void register_instrumentation(py::module&& m) {
  ...
  remotery.def("log",
               [](std::string s ARCTICDB_UNUSED) { ARCTICDB_SAMPLE_LOG(s.c_str()) });
#endif
}
...
void register_metrics(py::module&& m) {
  ...
  py::class_<arcticdb::PrometheusInstance,
             std::shared_ptr<arcticdb::PrometheusInstance>>(prometheus, "Instance");
...
/// Register handling of non-trivial types. For more information @see
/// arcticdb::TypeHandlerRegistry and
/// @see arcticdb::ITypeHandler
void register_type_handlers() {
  using namespace arcticdb;
  TypeHandlerRegistry::instance()->register_handler(
      TypeDescriptor{DataType::EMPTYVAL, Dimension::Dim0}, arcticdb::EmptyHandler());
  constexpr std::array<DataType, 5> allowed_array_types = {
      DataType::INT64, DataType::FLOAT64, DataType::EMPTYVAL, DataType::FLOAT32,
      DataType::INT32};
  for (const DataType& data_type : allowed_array_types) {
    TypeHandlerRegistry::instance()->register_handler(
        TypeDescriptor{data_type, Dimension::Dim1}, arcticdb::ArrayHandler());
  }
  TypeHandlerRegistry::instance()->register_handler(
      TypeDescriptor{DataType::BOOL_OBJECT8, Dimension::Dim0}, arcticdb::BoolHandler());
}
...
PYBIND11_MODULE(arcticdb_ext, m) {
...
#endif
  // Set up the global exception handlers first, so module-specific exception handler
  // can override it:
  ...
  auto storage_submodule =
      m.def_submodule("storage", "Segment storage implementation apis");
  auto no_data_found_exception =
      py::register_exception<arcticdb::storage::NoDataFoundException>(
  ...
  auto version_submodule =
      m.def_submodule("version_store", "Versioned storage implementation apis");
