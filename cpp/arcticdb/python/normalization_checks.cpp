...
auto get_pandas_common_via_reflection(
    ...
    -> decltype(inner_function(norm_meta, std::declval<FieldType>(),
                               std::declval<FieldType>())) {
  try {
    if (norm_meta.input_type_case() !=
        proto::descriptors::NormalizationMetadata::INPUT_TYPE_NOT_SET) {
      if (auto one_of = proto::descriptors::NormalizationMetadata::descriptor()->field(
              norm_meta.input_type_case());
          one_of) {
        log::storage().info(
            "Inefficient NormalizationMetadata.input_type.{} access via reflection",
            one_of->name());
  ...
  log::storage().warn(
      "New NormalizationMetadata.input_type access failure. Cannot check.");
  return std::nullopt;
}

std::optional<std::decay_t<std::reference_wrapper<
    const arcticdb::proto::descriptors::NormalizationMetadata_Pandas>>>
get_common_pandas(const proto::descriptors::NormalizationMetadata& norm_meta) {
  ...
  case proto::descriptors::NormalizationMetadata::kNp:
    return std::nullopt;
  default:
    return get_pandas_common_via_reflection(
        norm_meta, [](auto& norm_meta, auto one_of, auto common_field) {
          ...
          return std::make_optional(
              std::reference_wrapper<Pandas>(*reinterpret_cast<Pandas*>(
                  const_cast<::google::protobuf::Message*>(&common_msg))));
        });
  }
}

std::optional<std::decay_t<
    std::reference_wrapper<arcticdb::proto::descriptors::NormalizationMetadata_Pandas>>>
get_common_pandas(proto::descriptors::NormalizationMetadata& norm_meta) {
  ...
  case proto::descriptors::NormalizationMetadata::kDf:
    return std::make_optional(
        std::reference_wrapper<Pandas>(*norm_meta.mutable_df()->mutable_common()));
  case proto::descriptors::NormalizationMetadata::kSeries:
    return std::make_optional(
        std::reference_wrapper<Pandas>(*norm_meta.mutable_series()->mutable_common()));
  case proto::descriptors::NormalizationMetadata::kTs:
    return std::make_optional(
        std::reference_wrapper<Pandas>(*norm_meta.mutable_ts()->mutable_common()));
  case proto::descriptors::NormalizationMetadata::kMsgPackFrame:
  case proto::descriptors::NormalizationMetadata::kNp:
    return std::nullopt;
  default:
...
/// If the checks above pass update the new normalization index so that it spans the
/// whole index (old + new)
...
void update_rowcount_normalization_data(
    ...
    proto::descriptors::NormalizationMetadata& new_norm, size_t old_length) {
  const auto old_pandas = get_common_pandas(old_norm);
  const auto new_pandas = get_common_pandas(new_norm);
  const auto* old_index =
      old_pandas->get().has_index() ? &old_pandas->get().index() : nullptr;
  const auto* new_index =
      new_pandas->get().has_index() ? &new_pandas->get().index() : nullptr;
  if (old_index) {
    constexpr auto error_suffix = " the existing version. Please convert both to use "
                                  "Int64Index if you need this to work.";
    ...
    normalization::check<ErrorCode::E_INCOMPATIBLE_INDEX>(
        ...
        new_index->is_physically_stored() ? "non-range" : "range-style", error_suffix);
    ...
    if (!old_index->is_physically_stored()) {
      ...
      normalization::check<ErrorCode::E_INCOMPATIBLE_INDEX>(
          ...
          "The appending data has a RangeIndex.start={} that is not contiguous with "
          "the "
          "stop ({}) of {}",
          new_start, stop, error_suffix);
...
bool check_pandas_like(const proto::descriptors::NormalizationMetadata& old_norm,
  ...
  if (old_pandas || new_pandas) {
    normalization::check<ErrorCode::E_UPDATE_NOT_SUPPORTED>(
        ...
        "Currently only supports modifying existing Pandas data with "
        "Pandas.\nexisting={}\nargument={}",
        util::newlines_to_spaces(old_norm), util::newlines_to_spaces(new_norm));
    ...
    normalization::check<ErrorCode::E_INCOMPATIBLE_INDEX>(
        ...
        "The argument has an index type incompatible with the existing "
        "version:\nexisting={}\nargument={}",
        util::newlines_to_spaces(old_norm), util::newlines_to_spaces(new_norm));
    // FUTURE: check PandasMultiIndex and many other descriptor types. Might be more
    // efficiently implemented using some structural comparison lib or do it via Python
    return true;
  }
  return false;
}
...
bool check_ndarray_append(const NormalizationMetadata& old_norm,
  ...
  if (old_norm.has_np() || new_norm.has_np()) {
    ...
    normalization::check<ErrorCode::E_WRONG_SHAPE>(
        std::equal(old_shape.begin() + 1, old_shape.end(), new_shape->begin() + 1,
                   new_shape->end()),
        "The appending NDArray must have the same shape as the existing (excl. the "
        "first dimension)");
...
void fix_normalization_or_throw(
  ...
  if (check_pandas_like(old_norm, new_norm)) {
    ...
    if (old_index_type == new_index_type &&
        old_index_type == IndexDescriptor::Type::ROWCOUNT) {
      update_rowcount_normalization_data(old_norm, new_norm,
                                         existing_isr.tsd().total_rows());
    }
    return;
  }
  if (is_append) {
