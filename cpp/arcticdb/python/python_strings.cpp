...
// Moved verbatim from python_strings.hpp
class DynamicStringReducer {
  ...
  public:
    ...
    void reduce(
            const Column& source_column, TypeDescriptor source_type, TypeDescriptor target_type, size_t num_rows,
            const StringPool& string_pool, const std::optional<util::BitSet>& bitset
    );
  ...
  private:
    ...
    std::pair<size_t, size_t> write_strings_to_destination(
            size_t num_rows, const Column& source_column,
    ...
    void assign_strings_local(
            size_t num_rows, const Column& source_column, bool has_type_conversion, const StringPool& string_pool,
            const std::optional<util::BitSet>& sparse_map
    ) {
    ...
    assign_strings_shared(size_t num_rows, const Column& source_column, bool has_type_conversion, const StringPool& string_pool, const std::optional<util::BitSet>&) {
        ...
        auto [none_count, nan_count] =
                write_strings_to_destination(num_rows, source_column, allocated, source_column.opt_sparse_map());
        handler_data_.increment_none_refcount(none_count);
        handler_data_.increment_nan_refcount(nan_count);
    }
    ...
    inline void process_string_views(
            bool has_type_conversion, bool is_utf, size_t end, const Column& source_column,
            const StringPool& string_pool, const std::optional<util::BitSet>& bitset
    );
    ...
    void process_string_views_for_type(
            size_t num_rows, const Column& source_column, bool has_type_conversion, const StringPool& string_pool,
            const std::optional<util::BitSet>& bitset, bool optimize_for_memory
    ) {
...
inline void DynamicStringReducer::process_string_views(
        bool has_type_conversion, bool is_utf, size_t num_rows, const Column& source_column,
        const StringPool& string_pool, const std::optional<util::BitSet>& bitset
) {
    ...
    case PyStringConstructor::Unicode_FromUnicode:
        process_string_views_for_type<UnicodeFromUnicodeCreator>(
                num_rows, source_column, has_type_conversion, string_pool, bitset, shared_data_.optimize_for_memory()
        );
        break;
...
void DynamicStringReducer::reduce(
        const Column& source_column, TypeDescriptor source_type, TypeDescriptor target_type, size_t num_rows,
        const StringPool& string_pool, const std::optional<util::BitSet>& bitset
) {
