...
/// When numpy creates an empty array its type is float64. We want to mimic this
/// because: i) There is no equivalent to empty value ii) We want input dataframes to be
/// exact match of the output and that includes the type
[[nodiscard]] static inline py::dtype generate_python_dtype(const TypeDescriptor& td,
                                                            stride_t type_byte_size) {
  if (is_empty_type(td.data_type())) {
    return py::dtype{"f8"};
  }
  return py::dtype{
      fmt::format("{}{:d}", get_dtype_specifier(td.data_type()), type_byte_size)};
}

/// @important This calls pybind's initialize array function which is NOT thread safe.
/// Moreover, numpy arrays can be created only by the thread holding the GIL. In
/// practice we can get away with allocating arrays only from a single thread (even if
/// it's not the one holding the GIL). This, however, is not guaranteed to work.
/// @todo Allocate numpy arrays only from the thread holding the GIL
[[nodiscard]] static inline PyObject*
initialize_array(const pybind11::dtype& descr, const shape_t shapes,
                 const stride_t strides, const void* source_ptr,
                 std::shared_ptr<Column> owner, std::mutex& creation_mutex) {
  std::lock_guard creation_guard{creation_mutex};
  // TODO: Py capsule can take only void ptr as input. We need a better way to handle
  // destruction
  //  Allocating shared ptr on the heap is sad.
  auto* object = new std::shared_ptr<Column>(std::move(owner));
  auto arr = py::array(descr, {shapes}, {strides}, source_ptr,
                       py::capsule(object, [](void* obj) {
...
void EmptyHandler::handle_type(const uint8_t*& input, uint8_t* dest,
                               const EncodedFieldImpl& field,
                               const ColumnMapping& mapping, size_t dest_bytes,
  ...
  ARCTICDB_TRACE(log::version(),
                 "Empty type handler invoked for source type: {}, destination type: "
                 "{}, num rows: {}",
                 mapping.source_type_desc_, mapping.dest_type_desc_, mapping.num_rows_);
  ...
  if (field.encoding_case() == EncodedFieldType::NDARRAY) {
    ...
    util::check(num_blocks <= 1, "Unexpected number of empty type blocks: {}",
                num_blocks);
...
void BoolHandler::handle_type(const uint8_t*& data, uint8_t* dest,
                              const EncodedFieldImpl& field, const ColumnMapping& m,
                              size_t, const std::shared_ptr<BufferHolder>&,
                              EncodingVersion encoding_version) {
  ARCTICDB_SAMPLE(HandleBool, 0)
  util::check(dest != nullptr, "Got null destination pointer");
  util::check(field.has_ndarray(), "Bool handler expected array");
  ARCTICDB_DEBUG(log::version(), "Bool handler got encoded field: {}",
                 field.DebugString());
  ...
  data += decode_field(m.source_type_desc_, field, data, decoded_data_sink, sparse_map,
                       encoding_version);
  const auto num_bools = sparse_map.has_value() ? sparse_map->count() : m.num_rows_;
  auto ptr_src =
      decoded_data.template ptr_cast<uint8_t>(0, num_bools * sizeof(uint8_t));
  if (sparse_map.has_value()) {
    ...
    ARCTICDB_TRACE(log::codec(),
                   "Bool handler didn't find a sparse map. Assuming dense array.");
...
void ArrayHandler::handle_type(const uint8_t*& data, uint8_t* dest,
                               const EncodedFieldImpl& field, const ColumnMapping& m,
                               size_t, const std::shared_ptr<BufferHolder>& buffers,
                               EncodingVersion encoding_version) {
  ...
  const auto strides =
      static_cast<stride_t>(get_type_size(m.source_type_desc_.data_type()));
  ...
  m.source_type_desc_.visit_tag([&](auto tdt) {
    ...
    for (auto en = bv->first(); en < bv->end(); ++en) {
      ...
      last_row = offset;
      *ptr_dest++ = initialize_array(py_dtype, shape, stride, ptr_src + block_pos,
                                     column, initialize_array_mutex);
      block_pos += shape * stride;
      if (shapes) {
        ++shapes;
      }
      if (block_it != blocks.end() && block_pos == (*block_it)->bytes() &&
          ++block_it != blocks.end()) {
