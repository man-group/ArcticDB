...
void EmptyHandler::convert_type(
    ...
    if(is_object_type(dest_type_desc) || is_empty_type(dest_type_desc.data_type())) {
        default_initialize(dest_buffer, offset_bytes, num_rows * type_size(), shared_data);
    } else {
...
void StringHandler::convert_type(
    ...
    auto ptr_dest = reinterpret_cast<PyObject**>(dest_data);
    DynamicStringReducer string_reducer{shared_data, ptr_dest, num_rows};
...
void ArrayHandler::handle_type(
    ...
    convert_type(*column, dest_buffer, m.num_rows_, m.offset_bytes_, m.source_type_desc_, m.dest_type_desc_, shared_data, {});
}
