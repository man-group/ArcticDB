    ...
    auto decoded_data = [&m, &ndarray, bytes, &dest_column]() {
        if (ndarray.sparse_map_bytes() > 0) {
            ...
            column.buffer().add_external_block(
                    dest_column.bytes_at(m.offset_bytes_, m.num_rows_ * sizeof(PyObject*)), bytes, 0UL
            );
            return column;
        }
    }();
...
void PythonArrayHandler::convert_type(
    ...
    if (source_column.is_sparse()) {
        ...
        python_util::prefill_with_none(
                ptr_dest, mapping.num_rows_, source_column.sparse_map().count(), handler_data.spin_lock()
        );
