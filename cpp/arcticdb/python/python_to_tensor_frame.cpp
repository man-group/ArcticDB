...
constexpr const char none_char[8] = {'\300', '\000', '\000', '\000',
                                     '\000', '\000', '\000', '\000'};
...
std::variant<StringEncodingError, PyStringWrapper> pystring_to_buffer(PyObject* obj,
                                                                      bool is_owned) {
  if (is_unicode(obj)) {
    return StringEncodingError(fmt::format(
        "Unexpected unicode in Python object with type {}", obj->ob_type->tp_name));
  }
  char* buffer;
  ssize_t length;
  if (PYBIND11_BYTES_AS_STRING_AND_SIZE(obj, &buffer, &length)) {
    return StringEncodingError(
        fmt::format("Unable to extract string contents from Python object with type {}",
                    obj->ob_type->tp_name));
  }
...
[[nodiscard]] static std::tuple<ValueType, uint8_t, ssize_t>
determine_python_object_type(PyObject* obj) {
  if (is_py_boolean(obj)) {
    normalization::raise<ErrorCode::E_UNIMPLEMENTED_INPUT_TYPE>(
        "Nullable booleans are not supported at the moment");
...
/// In case column is composed of arrays all arrays must have the same element type.
/// This iterates until if finds the first non-empty array and returns its type.
/// @todo We will iterate over all arrays in a column in aggregator_set_data anyways, so
/// this is redundant, however
///     the type is determined at the point when obj_to_tensor is called. We need to
///     make it possible to change the the column type in aggregator_set_data in order
///     not to iterate all arrays twice.
[[nodiscard]] static std::tuple<ValueType, uint8_t, ssize_t>
determine_python_array_type(PyObject** begin, PyObject** end) {
  auto none = py::none{};
  while (begin != end) {
    if (none.ptr() == *begin) {
      ++begin;
...
std::variant<StringEncodingError, PyStringWrapper>
py_unicode_to_buffer(PyObject* obj, std::optional<ScopedGILLock>& scoped_gil_lock) {
  if (!is_unicode(obj)) {
    return StringEncodingError(fmt::format(
        "Unexpected non-unicode in Python object with type {}", obj->ob_type->tp_name));
  }
  if (PyUnicode_IS_COMPACT_ASCII(obj)) {
    return PyStringWrapper(reinterpret_cast<char*>(PyUnicode_DATA(obj)),
                           PyUnicode_GET_LENGTH(obj));
    // Later versions of cpython expose macros in unicodeobject.h to perform this check,
    // and to get the utf8_length, but for 3.6 we have to hand-roll it
  } else if (reinterpret_cast<PyCompactUnicodeObject*>(obj)->utf8) {
    return PyStringWrapper(reinterpret_cast<PyCompactUnicodeObject*>(obj)->utf8,
                           reinterpret_cast<PyCompactUnicodeObject*>(obj)->utf8_length);
  } else {
    if (PyUnicode_READY(obj) != 0) {
      return StringEncodingError(fmt::format(
          "PyUnicode_READY failed on Python object with type", obj->ob_type->tp_name));
    }
    ...
    if (!utf8_obj) {
      return StringEncodingError(fmt::format(
          "Unable to extract string contents from Python object with type {}",
          obj->ob_type->tp_name));
    }
...
NativeTensor obj_to_tensor(PyObject* ptr, bool empty_types) {
  ...
  auto ndim = arr->nd;
  const ssize_t size =
      ndim == 1 ? arr->dimensions[0] : arr->dimensions[0] * arr->dimensions[1];
  // In Pandas < 2, empty series dtype is `"float"`, but as of Pandas 2.0, empty series
  // dtype is `"object"` The Normalizer in Python cast empty `"float"` series to
  // `"object"` so `EMPTY` is used here. See:
  // https://github.com/man-group/ArcticDB/pull/1049
  auto val_type =
      size == 0 && empty_types ? ValueType::EMPTY : get_value_type(descr->kind);
  auto val_bytes = static_cast<uint8_t>(descr->elsize);
  const int64_t element_count =
      ndim == 1 ? int64_t(arr->dimensions[0])
                : int64_t(arr->dimensions[0]) * int64_t(arr->dimensions[1]);
  ...
  if (is_empty_type(val_type)) {
    ...
    if (!is_fixed_string_type(val_type) && element_count > 0) {
      ...
      // Arctic allows both None and NaN to represent a string with no value. We have 3
      // options:
      // * In case all values are None we can mark this column segment as EmptyType and
      // avoid allocating storage
      //      memory for it
      // * In case all values are NaN we can't sample the value to check if it's UTF or
      // ASCII (since NaN is not
      //      UTF nor ASCII). In that case we choose to save it as UTF
      // * In case there is at least one actual string we can sample it and decide the
      // type of the column segment
      //      based on it
      // Note: ValueType::ASCII_DYNAMIC was used when Python 2 was supported. It is no
      ...
      if (sample == none.ptr() || is_py_nan(sample)) {
        ...
        util::check(
            c_style,
            "Non contiguous columns with first element as None not supported yet.");
  ...
  // and we can't use `val_bytes` to get this information since some dtype have another
  // `elsize` than 8.
  const SizeBits size_bits =
      is_empty_type(val_type) ? SizeBits::S64 : get_size_bits(val_bytes);
  ...
  const std::array<stride_t, 2> strides = {arr->strides[0],
                                           arr->nd > 1 ? arr->strides[1] : 0};
  const std::array<shape_t, 2> shapes = {arr->dimensions[0],
                                         arr->nd > 1 ? arr->dimensions[1] : 0};
  return {nbytes, arr->nd,       strides.data(), shapes.data(),
          dt,     descr->elsize, data,           ndim};
}

std::shared_ptr<InputTensorFrame> py_ndf_to_frame(const StreamId& stream_name,
                                                  const py::tuple& item,
  ...
  util::check(idx_names.size() == idx_vals.size(),
              "Number idx names {} and values {} do not match", idx_names.size(),
              idx_vals.size());
  ...
  if (!idx_names.empty()) {
    ...
    util::check(index_tensor.shape() != nullptr,
                "Index tensor expected to contain shapes");
  ...
  for (auto i = 0u; i < col_vals.size(); ++i) {
    ...
    if (tensor.expanded_dim() == 1) {
      ...
      res->desc.add_field(
          FieldRef{TypeDescriptor{tensor.data_type(), Dimension::Dim1}, col_names[i]});
    }
    res->field_tensors.push_back(std::move(tensor));
  }

  // idx_names are passed by the python layer. They are empty in case row count index is
  // used see:
  ...
  // Currently the python layers assign RowRange index to both empty dataframes and
  // dataframes which do not specify index explicitly. Thus we handle this case after
  // all columns are read so that we know how many rows are there.
  if (idx_names.empty()) {
    res->index = stream::RowCountIndex();
    res->desc.set_index_type(IndexDescriptor::Type::ROWCOUNT);
  }
...
std::shared_ptr<InputTensorFrame> py_none_to_frame() {
  ...
  auto tensor =
      NativeTensor{8, ndim, &strides, &shapes, DataType::UINT64, 8, none_char, ndim};
  res->num_rows = std::max(res->num_rows, static_cast<size_t>(tensor.shape(0)));
