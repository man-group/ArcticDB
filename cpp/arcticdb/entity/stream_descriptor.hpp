...
struct SegmentDescriptorImpl : public SegmentDescriptor {
    ...
    [[nodiscard]] const IndexDescriptorImpl& index() const { return static_cast<const IndexDescriptorImpl&>(index_); }
...
inline bool operator==(const SegmentDescriptorImpl& l, const SegmentDescriptorImpl& r) {
    return l.sorted_ == r.sorted_ && l.index() == r.index() && l.compressed_bytes_ == r.compressed_bytes_ &&
           l.uncompressed_bytes_ == r.uncompressed_bytes_;
}

inline bool operator!=(const SegmentDescriptorImpl& l, const SegmentDescriptorImpl& r) { return !(l == r); }
...
struct StreamDescriptor {
    ...
    [[nodiscard]] uint64_t uncompressed_bytes() const { return segment_desc_->uncompressed_bytes_; }

    [[nodiscard]] uint64_t compressed_bytes() const { return segment_desc_->compressed_bytes_; }

    [[nodiscard]] SortedValue sorted() const { return segment_desc_->sorted_; }

    [[nodiscard]] IndexDescriptorImpl index() const { return static_cast<IndexDescriptorImpl&>(segment_desc_->index_); }
    ...
    IndexDescriptorImpl& index() { return static_cast<IndexDescriptorImpl&>(segment_desc_->index_); }
    ...
    StreamDescriptor(StreamDescriptor&& other) noexcept : StreamDescriptor() { swap(*this, other); }
    ...
    std::string_view add_field(const Field& field) { return fields_->add(FieldRef{field.type(), field.name()}); }
    ...
    [[nodiscard]] std::shared_ptr<SegmentDescriptorImpl> data_ptr() const { return segment_desc_; }
    ...
    [[nodiscard]] std::optional<std::size_t> find_field(std::string_view view) const {
        auto it = std::ranges::find_if(*this, [&](const auto& field) { return field.name() == view; });
    ...
    friend bool operator!=(const StreamDescriptor& left, const StreamDescriptor& right) { return !(left == right); }
