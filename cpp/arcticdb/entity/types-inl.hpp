...
#define DT_CASE(__T__)                                                                                                 \
    case DataType::__T__:                                                                                              \
        ...
        DT_CASE(FLOAT64)
        DT_CASE(BOOL8)
        DT_CASE(SECONDS_UTC64)
        DT_CASE(MILLISECONDS_UTC64)
        DT_CASE(MICROSECONDS_UTC64)
...
#undef DT_CASE
    default:
        util::raise_rte(
                "Invalid dtype {}:{} - '{}' in visit dim",
                int(slice_value_type(dt)),
                int(slice_bit_size(dt)),
                datatype_to_str(dt)
        );
    }
}
...
constexpr TypeDescriptor null_type_descriptor() { return {DataType(ValueType::UNKNOWN_VALUE_TYPE), Dimension::Dim0}; }
