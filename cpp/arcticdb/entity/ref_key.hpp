...
public:
  RefKey(StreamId id, KeyType key_type, bool old_type = false)
      : id_(std::move(id)), key_type_(key_type), old_type_(old_type) {
    ...
    util::check(old_type || is_ref_key_class(key_type),
                "Can't create ref key with non-ref key class keytype {}", key_type);
  }
...
namespace fmt {
template <> struct formatter<RefKey> {
...
namespace std {
template <> struct hash<arcticdb::entity::RefKey> {
  inline arcticdb::HashedValue
  operator()(const arcticdb::entity::RefKey& k) const noexcept {
