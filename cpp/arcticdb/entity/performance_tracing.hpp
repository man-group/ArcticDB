...
#define ARCTICDB_RUNTIME_SAMPLE(name, flags)                                                                           \
    ...
    ScopedTimer runtime_timer =                                                                                        \
            !_scoped_timer_active_ ? ScopedTimer() : ScopedTimer(#name, [](auto msg) { log::timings().debug(msg); });
...
#define ARCTICDB_RUNTIME_SUBSAMPLE(name, flags)                                                                        \
    ...
    ScopedTimer runtime_sub_timer_##name = !_scoped_subtimer_##name_active_                                            \
                                                   ? ScopedTimer()                                                     \
                                                   : ScopedTimer(#name, [](auto msg) { log::timings().debug(msg); });
