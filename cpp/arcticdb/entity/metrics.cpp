...
void PrometheusInstance::configure(const MetricsConfig& config,
  ...
  if (configured_ && !reconfigure) {
    arcticdb::log::version().warn(
        "Prometheus already configured; Existing setting will be used");
    if (config.host != cfg_.host) {
      arcticdb::log::version().warn(
          "New Prometheus host is different from the existing: {} vs {}", config.host,
          cfg_.host);
    }
    if (config.port != cfg_.port) {
      arcticdb::log::version().warn(
          "New Prometheus port is different from the existing: {} vs {}", config.port,
          cfg_.port);
    }
    if (config.job_name != cfg_.job_name) {
      arcticdb::log::version().warn(
          "New Prometheus job_name is different from the existing: {} vs {}",
          config.job_name, cfg_.job_name);
    }
    if (config.instance != cfg_.instance) {
      arcticdb::log::version().warn(
          "New Prometheus instance is different from the existing: {} vs {}",
          config.instance, cfg_.instance);
    }
    if (config.prometheus_env != cfg_.prometheus_env) {
      arcticdb::log::version().warn(
          "New Prometheus env is different from the existing: {} vs {}",
          config.prometheus_env, cfg_.prometheus_env);
    }
    if (config.model_ != cfg_.model_) {
      arcticdb::log::version().warn(
          "New Prometheus model is different from the existing: {} vs {}",
          static_cast<int>(config.model_), static_cast<int>(cfg_.model_));
    }
    return;
  }
  ...
  if (cfg_.model_ == MetricsConfig::Model::PUSH) {
    ...
    gateway_ = std::make_shared<prometheus::Gateway>(cfg_.host, cfg_.port,
                                                     cfg_.job_name, labels);
    ...
    arcticdb::log::version().info("Prometheus endpoint created on {}/metrics",
                                  endpoint);
  } else {
...
void PrometheusInstance::registerMetric(
    prometheus::MetricType type, const std::string& name, const std::string& help,
...
void PrometheusInstance::incrementCounter(
...
void PrometheusInstance::setGauge(const std::string& name, double value,
                                  const std::map<std::string, std::string>& labels) {
  if (registry_.use_count() == 0)
    return;
...
void PrometheusInstance::setGaugeCurrentTime(
...
void PrometheusInstance::observeHistogram(
    const std::string& name, double value,
    const std::map<std::string, std::string>& labels) {
  if (registry_.use_count() == 0)
    return;
...
void PrometheusInstance::DeleteHistogram(
    const std::string& name, const std::map<std::string, std::string>& labels) {
  if (registry_.use_count() == 0)
    return;

  if (auto it = map_histogram_.find(name); it != map_histogram_.end()) {
    it->second.histogram->Remove(
        &it->second.histogram->Add(labels, it->second.buckets_list));
  } else {
    arcticdb::log::version().warn("Unregistered Histogram metric {}", name);
  }
}
void PrometheusInstance::observeSummary(
    const std::string& name, double value,
    const std::map<std::string, std::string>& labels) {
  if (registry_.use_count() == 0)
    return;
