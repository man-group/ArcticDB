...
void PrometheusInstance::configure(const MetricsConfig& config, const bool reconfigure) {
    ...
    if (cfg_.model_ == MetricsConfig::Model::PUSH) {
        ...
        ARCTICDB_DEBUG(log::version(), "Prometheus Push created with settings {}", cfg_);
        ...
        ARCTICDB_DEBUG(log::version(), "Prometheus endpoint created on {}/metrics", endpoint);
    }
    else {
        ARCTICDB_DEBUG(log::version(), "Prometheus not configured {}", cfg_);
    }
