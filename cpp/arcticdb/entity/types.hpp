...
#ifdef _WIN32
// `ssize_t` is defined in `sys/types.h` but it is not ISO C (it simply is POSIX), hence
// its is not defined natively by MSVC. See:
...
// TODO: shape_t probably should be unsigned. Negative shapes don't make sense. This
// will involve a lot of changes in native_tensor.hpp
using shape_t = int64_t;
using stride_t = int64_t;
using position_t = int64_t;

/** The VariantId holds int64 (NumericId) but is also used to store sizes up to uint64,
 * so needs safe conversion */
...
#ifndef _WIN32
static_assert(UNICODE_WIDTH == 4,
              "Only support python platforms where unicode width is 4");
#endif
...
enum class ValueType : uint8_t {
  ...
  //    SYMBOL = 6, // categorical string of low cardinality suitable for dictionary
  //    encoding
  ASCII_FIXED =
      7, // fixed size string when dim > 1, inputs of type uint8_t, no encoding
  UTF8_FIXED =
      8,     // fixed size string when dim > 1, inputs of type uint8_t, utf8 encoding
  ...
  // For instance, Pandas empty series whose types has not been specified is mapping to
  // EMPTY. When data is appended, the column type is inferred from the data and the
  // column is promoted to the inferred type.
  EMPTY = 13,
  /// Nullable booleans
  BOOL_OBJECT = 14,
  COUNT // Not a real value type, should not be added to proto descriptor. Used to count
...
constexpr bool is_numeric_type(ValueType v) {
  return v == ValueType::NANOSECONDS_UTC || (uint8_t(v) >= uint8_t(ValueType::UINT) &&
...
constexpr bool is_integer_type(ValueType v) {
  return uint8_t(v) == uint8_t(ValueType::INT) ||
...
constexpr uint8_t combine_val_bits(ValueType v,
                                   SizeBits b = SizeBits::UNKNOWN_SIZE_BITS) {
...
enum class DataType : uint8_t {
  ...
  NANOSECONDS_UTC64 =
      detail::combine_val_bits(ValueType::NANOSECONDS_UTC, SizeBits::S64),
...
constexpr DataType combine_data_type(ValueType v,
                                     SizeBits b = SizeBits::UNKNOWN_SIZE_BITS) {
  return static_cast<DataType>(detail::combine_val_bits(v, b));
}

// Constructs the corresponding DataType from a given primitive arithmetic type
// (u/int8_t, float, or double)
template <typename T> constexpr DataType data_type_from_raw_type() {
...
constexpr ValueType get_value_type(char specifier) noexcept {
  ...
  case 'b':
    ...
    // NOTE: this is safe as of Pandas < 2.0 because `datetime64` _always_ has been
    // using nanosecond resolution, i.e. Pandas < 2.0 _always_ provides `datetime64[ns]`
    // and ignores any other resolution. Yet, this has changed in Pandas 2.0 and other
    // resolution can be used, i.e. Pandas >= 2.0 will also provides `datetime64[us]`,
    // `datetime64[ms]` and `datetime64[s]`. See:
    ...
    // TODO: for the support of Pandas>=2.0, convert any `datetime` to `datetime64[ns]`
    // before-hand and do not rely uniquely on the resolution-less 'M' specifier if it
    // this doable.
  case 'M':
    return ValueType::NANOSECONDS_UTC; //  datetime // numpy doesn't support the buffer
                                       //  protocol for datetime64
  case 'U':
...
template <DataType DT> struct DataTypeTag {};

#define DATA_TYPE_TAG(__DT__, __T__)                                                   \
  template <> struct DataTypeTag<DataType::__DT__> : public DataTypeTagBase {          \
...
template <Dimension dim> struct DimensionTag {};

#define DIMENSION(__D__)                                                               \
  template <> struct DimensionTag<Dimension::Dim##__D__> : public DimensionTagBase {   \
...
struct TypeDescriptor {
  ...
  TypeDescriptor(DataType dt, uint8_t dim)
      : data_type_(dt), dimension_(as_dim_checked(dim)) {}
  constexpr TypeDescriptor(DataType dt, Dimension dim)
      : data_type_(dt), dimension_(dim) {}
  ...
  template <typename Callable> constexpr auto visit_tag(Callable&& callable) const;

  bool operator==(const TypeDescriptor& o) const {
    return data_type_ == o.data_type_ && dimension_ == o.dimension_;
  }

  bool operator!=(const TypeDescriptor& o) const { return !(*this == o); }
...
/// Some types are allowed not to have any data, e.g. empty arrays or the empty type
/// (which by design denotes the lack of data).
/// @return true if the type must contain data, false it it's allowed for the type to
/// have 0 bytes of data
...
constexpr bool is_numpy_array(TypeDescriptor td) {
  return (is_numeric_type(td.data_type()) || is_bool_type(td.data_type()) ||
...
constexpr bool is_pyobject_type(TypeDescriptor td) {
  return is_dynamic_string_type(slice_value_type(td.data_type())) ||
         is_bool_object_type(td.data_type()) || is_numpy_array(td);
}
...
template <typename DT, typename D> struct TypeDescriptorTag {
...
template <typename T> struct ScalarTypeInfo {
...
struct IndexDescriptorImpl : public IndexDescriptor {
  ...
  friend bool operator==(const IndexDescriptorImpl& left,
                         const IndexDescriptorImpl& right) {
...
public:
  ...
  void set(TypeDescriptor type, std::string_view name) {
    type_ = type;
    size_ = name.size();
    // There are always two characters, so we need to set the second in case of a
    // one-letter column name
    name_[1] = 0;
...
template <typename Callable> auto visit_field(const Field& field, Callable&& c) {
...
template <> struct less<arcticdb::StreamId> {
  bool operator()(const arcticdb::StreamId& left,
                  const arcticdb::StreamId& right) const {
...
template <> struct formatter<FieldRef> {
...
template <> struct formatter<FieldWrapper> {
