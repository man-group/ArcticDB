...
class FieldCollection {
    ...
    FieldCollection(CursoredBuffer<ChunkedBuffer>&& buffer, CursoredBuffer<Buffer>&& offsets, CursoredBuffer<Buffer> shapes) :
...
public:
    ...
    class FieldCollectionIterator :  public boost::iterator_facade<FieldCollectionIterator<ValueType>, ValueType, boost::forward_traversal_tag> {
    public:
        explicit FieldCollectionIterator(ChunkedBuffer* buffer) :
            buffer_(buffer) { }
    ...
    void erase_field(position_t pos) {
        util::check(static_cast<size_t>(pos) < size(), "Position {} out of range in drop field with {} fields", pos, size());
    ...
    inline void advance_data(std::size_t size) {
        buffer_.advance(position_t(size));
    }
    ...
    [[nodiscard]] size_t size() const {
        return (offsets_.bytes() / sizeof(shape_t));
    }
