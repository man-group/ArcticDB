...
class FieldCollection {
  ...
  static constexpr TypeDescriptor type_ =
      TypeDescriptor{DataType::UINT8, Dimension::Dim1};

  FieldCollection(CursoredBuffer<ChunkedBuffer>&& buffer,
                  CursoredBuffer<Buffer>&& offsets, CursoredBuffer<Buffer> shapes)
      : buffer_(std::move(buffer)), offsets_(std::move(offsets)),
...
public:
  ...
  class FieldCollectionIterator
      : public boost::iterator_facade<FieldCollectionIterator<ValueType>, ValueType,
                                      boost::forward_traversal_tag> {
  public:
    explicit FieldCollectionIterator(ChunkedBuffer* buffer) : buffer_(buffer) {}

    explicit FieldCollectionIterator(ChunkedBuffer* buffer, size_t pos)
    ...
    FieldCollectionIterator(const FieldCollectionIterator& other)
        : buffer_(other.buffer_), pos_(other.pos_) {}
    ...
    void increment() {
      pos_ += std::max(sizeof(ValueType), Field::calc_size(dereference().name()));
    }

    [[nodiscard]] ValueType& dereference() const {
      return *buffer_->ptr_cast<ValueType>(pos_, sizeof(ValueType));
    }
  ...
  [[nodiscard]] auto begin() {
    return FieldCollectionIterator<Field>(&buffer_.buffer());
  }

  [[nodiscard]] auto end() {
    return FieldCollectionIterator<Field>(&buffer_.buffer(), buffer_.bytes());
  }

  [[nodiscard]] auto begin() const {
    return FieldCollectionIterator<const Field>(
        const_cast<ChunkedBuffer*>(&buffer_.buffer()));
  }

  [[nodiscard]] auto end() const {
    return FieldCollectionIterator<const Field>(
        const_cast<ChunkedBuffer*>(&buffer_.buffer()), buffer_.bytes());
  }
  ...
  std::string_view add_field(const FieldRef& field) {
    return add_field(field.type_, field.name_);
  }

  std::string_view add_field(const TypeDescriptor& type, std::string_view name);

  // Note that this is expensive and is primarily intended for testing, where
  // drop_column is a good way of creating dynamic schema from a large dataframe
  void erase_field(position_t pos) {
    util::check(static_cast<size_t>(pos) < size(),
                "Position {} out of range in drop field with {} fields", pos, size());
  ...
  std::string_view add(const FieldRef& field) {
  ...
  [[nodiscard]] size_t get_offset(size_t pos) const {
    ...
    return *offsets_.buffer().ptr_cast<shape_t>((pos - 1) * sizeof(shape_t),
                                                sizeof(shape_t));
  }
  ...
  [[nodiscard]] FieldRef ref_at(size_t pos) const {
    const auto* field =
        buffer_.buffer().ptr_cast<Field>(get_offset(pos), sizeof(shape_t));
  ...
  [[nodiscard]] FieldCollection clone() const {
    return {buffer_.clone(), offsets_.clone(), shapes_.clone()};
  }
};
...
namespace fmt {
template <> struct formatter<arcticdb::FieldCollection> {
  ...
  auto format(const arcticdb::FieldCollection& fc, FormatContext& ctx) const {
    for (size_t i = 0; i < fc.size(); ++i) {
      if (i == fc.size() - 1) {
        fmt::format_to(ctx.out(), "FD<name={}, type={}, idx={}>", fc[i].name(),
                       fc[i].type(), i);
      } else {
        fmt::format_to(ctx.out(), "FD<name={}, type={}, idx={}>, ", fc[i].name(),
                       fc[i].type(), i);
