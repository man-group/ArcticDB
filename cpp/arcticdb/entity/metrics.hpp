...
public:
  ...
  MetricsConfig(const std::string& host, const std::string& port,
                const std::string& job_name, const std::string& instance,
                const std::string& prometheus_env, const Model model)
      : host(host), port(port), job_name(job_name), instance(instance),
        prometheus_env(prometheus_env), model_(model) {
...
public:
  ...
  void registerMetric(prometheus::MetricType type, const std::string& name,
                      const std::string& help,
                      const std::map<std::string, std::string>& staticLabels = {},
                      const std::vector<double>& buckets_list = {});
  // update pre-registered metrics with optional instance labels.  Each unique set of
  // labels generates a new metric instance
  ...
  void setGauge(const std::string& name, double value,
                const std::map<std::string, std::string>& labels = {});
  void setGaugeCurrentTime(const std::string& name,
                           const std::map<std::string, std::string>& labels = {});
...
inline void log_prometheus_gauge(const std::string& metric_name,
                                 const std::string& metric_desc, size_t val) {
  PrometheusInstance::instance()->registerMetric(prometheus::MetricType::Gauge,
                                                 metric_name, metric_desc, {});
  PrometheusInstance::instance()->setGauge(metric_name, static_cast<double>(val));
}

inline void log_prometheus_counter(const std::string& metric_name,
                                   const std::string& metric_desc, size_t val) {
  PrometheusInstance::instance()->registerMetric(prometheus::MetricType::Counter,
                                                 metric_name, metric_desc, {});
  PrometheusInstance::instance()->incrementCounter(metric_name,
                                                   static_cast<double>(val));
}

} // Namespace arcticdb

template <> struct fmt::formatter<arcticdb::MetricsConfig> {
  ...
  auto format(const arcticdb::MetricsConfig k, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(),
                          "MetricsConfig: host={}, port={}, job_name={}, instance={}, "
                          "prometheus_env={}, model={}",
                          k.host, k.port, k.job_name, k.instance, k.prometheus_env,
