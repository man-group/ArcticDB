...
struct DescriptorItem {
  DescriptorItem(entity::AtomKey&& key, std::optional<timestamp> start_index,
                 std::optional<timestamp> end_index,
                 std::optional<TimeseriesDescriptor> timeseries_descriptor)
      :

        key_(std::move(key)), start_index_(start_index), end_index_(end_index),
  ...
  std::optional<TimeseriesDescriptor> timeseries_descriptor() const {
