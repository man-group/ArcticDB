...
/** The IndexValue variant holds NumericIndex=timestamp=int64_t but is also used to
   store sizes up to uint64, so needs safe conversion. See also
   safe_convert_to_numeric_id. */
inline NumericIndex safe_convert_to_numeric_index(uint64_t input,
...
struct IndexRange {
  ...
  IndexRange(IndexValue start, IndexValue end)
      : start_(std::move(start)), end_(std::move(end)), specified_(true) {}

  explicit IndexRange(const TimestampRange& rg)
      : start_(rg.first), end_(rg.second), specified_(true) {}
  ...
  // N.B. Convenience function, variant construction will be too expensive for tight
  // loops
  ...
  friend bool intersects(const IndexRange& rg, const IndexValue& start,
                         const IndexValue& end) {
    if (!rg.specified_)
      return true;
...
inline IndexRange universal_range() {
  return IndexRange{std::numeric_limits<timestamp>::min(),
...
template <> struct formatter<arcticdb::entity::TimestampRange> {
...
template <> struct formatter<arcticdb::entity::IndexRange> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
...
inline bool IndexRange::accept(const IndexValue& index) {
  ...
  if (index >= start_ && index <= end_) {
    ARCTICDB_DEBUG(log::inmem(), "Returning index {} which is in range {}", index,
                   *this);
    return true;
  }

  ARCTICDB_DEBUG(log::inmem(), "Filtered index {} as it was not in range {}", index,
                 *this);
  return false;
}
