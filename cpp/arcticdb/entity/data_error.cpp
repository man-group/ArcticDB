...
DataError::DataError(
    ...
    const std::optional<pipelines::VersionQueryType>& version_query_type,
    std::optional<ErrorCode> error_code)
    : symbol_(std::move(symbol)), exception_string_(std::move(exception_string)),
...
std::optional<VersionRequestType> DataError::version_request_type() const {
  return version_request_type_;
}

std::optional<std::variant<std::monostate, int64_t, SnapshotId>>
DataError::version_request_data() const {
...
std::optional<ErrorCategory> DataError::error_category() const {
  return error_code_.has_value()
             ? std::make_optional<ErrorCategory>(get_error_category(*error_code_))
             : std::nullopt;
}
...
std::string DataError::to_string() const {
  ...
  if (version_request_type_.has_value()) {
    ...
    case VersionRequestType::SNAPSHOT:
      version_request_explanation =
          fmt::format("in snapshot '{}'", std::get<SnapshotId>(*version_request_data_));
      break;
    case VersionRequestType::TIMESTAMP:
      version_request_explanation =
          fmt::format("at time '{}'", std::get<int64_t>(*version_request_data_));
      break;
    case VersionRequestType::SPECIFIC:
      version_request_explanation = fmt::format(
          "with specified version '{}'", std::get<int64_t>(*version_request_data_));
      break;
  ...
  return fmt::format("Version {} of symbol '{}' unable to be retrieved: Error "
                     "Category: '{}' Exception String: '{}'",
                     version_request_explanation, symbol_, category_name,
                     exception_string_);
}
} // namespace arcticdb::entity