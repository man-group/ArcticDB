...
RemoteryInstance::RemoteryInstance() {
  ...
  if (rc) {
    ARCTICDB_DEBUG(arcticdb::log::version(),
                   ...
                   arcticdb::util::format(cfg), rc);
    rmt_ = nullptr;
  } else {
    ARCTICDB_DEBUG(arcticdb::log::version(), "Remotery created with settings {}",
...
struct ThreadNameCache {
  ...
  ThreadNameCache()
      : fqn_by_task_name_(),
        thread_name_(fmt::format("{}", arcticdb::get_thread_id())) {}

  std::string_view get_thread_name(const char* task_name) {
    if (auto fqn_it = fqn_by_task_name_.find(task_name);
        fqn_it != fqn_by_task_name_.end()) {
      return fqn_it->second;
    }
    auto [fqn_it, inserted] = fqn_by_task_name_.emplace(
        task_name, fmt::format("{}/{}", thread_name_, task_name));
    return fqn_it->second;
  }
};
} // namespace arcticdb::detail
