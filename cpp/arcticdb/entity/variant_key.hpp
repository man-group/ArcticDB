...
// Aliases to clarify usage and allow more detailed typing in the future, similar to
// aliases for AtomKey:
/** Should be a SNAPSHOT_REF key or the legacy SNAPSHOT AtomKey. */
using SnapshotVariantKey = VariantKey;

inline AtomKey& to_atom(VariantKey& vk) { return std::get<AtomKey>(vk); }

inline const AtomKey& to_atom(const VariantKey& vk) { return std::get<AtomKey>(vk); }

inline AtomKey to_atom(VariantKey&& vk) { return std::get<AtomKey>(std::move(vk)); }

inline RefKey& to_ref(VariantKey& vk) { return std::get<RefKey>(vk); }

inline const RefKey& to_ref(const VariantKey& vk) { return std::get<RefKey>(vk); }
...
inline bool variant_key_id_empty(const VariantKey& vk) {
  return std::visit(
      [](const auto& key) {
        return !std::holds_alternative<NumericId>(key.id()) &&
               std::get<StringId>(key.id()).empty();
      },
      vk);
}
...
template <> struct formatter<arcticdb::entity::VariantKey> {
