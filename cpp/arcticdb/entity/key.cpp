...
struct KeyData {
    ...
    KeyData(const char* long_name, char short_name, VariantType variant_type, KeyClass key_class,
...
#define STRING_REF(kt, name, c)                                                                                        \
    ...
    default:
        util::raise_rte("Could not get data for key_type {}", static_cast<int>(key_type));
    };
}

const char* key_type_long_name(KeyType key_type) { return get_key_data(key_type).long_name_; }

char key_type_short_name(KeyType key_type) { return get_key_data(key_type).short_name_; }

VariantType variant_type_from_key_type(KeyType key_type) { return get_key_data(key_type).variant_type_; }

KeyClass key_class_from_key_type(KeyType key_type) { return get_key_data(key_type).key_class_; }

const char* get_key_description(KeyType key_type) { return get_key_data(key_type).description_; }

bool is_string_key_type(KeyType key_type) { return variant_type_from_key_type(key_type) == VariantType::STRING_TYPE; }

bool is_ref_key_class(KeyType key_type) { return key_class_from_key_type(key_type) == KeyClass::REF_KEY; }
