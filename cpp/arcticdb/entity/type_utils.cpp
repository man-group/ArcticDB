...
std::optional<entity::TypeDescriptor> has_valid_common_type(
    ...
    if (!maybe_common_type && left.dimension() == right.dimension() && is_integer_type(left.data_type()) &&
        ...
        // To get here we must have one signed and one unsigned type, with the width of the signed type <= the width of
        // the unsigned type
        ...
        if (target_size < entity::SizeBits::COUNT) {
            maybe_common_type =
                    entity::TypeDescriptor{combine_data_type(entity::ValueType::INT, target_size), dimension};
        }
    }
