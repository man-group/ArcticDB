...
namespace arcticdb {
bool trivially_compatible_types(const entity::TypeDescriptor& left,
                                const entity::TypeDescriptor& right) {
  if (left == right)
    return true;

  // Multidimensional types are pointers
  if (left.dimension() >= entity::Dimension::Dim1 &&
      right.dimension() >= entity::Dimension::Dim1)
    return true;
  ...
  if (is_sequence_type(left.data_type()) && is_sequence_type(right.data_type())) {
    // TODO coercion of utf strings is not always safe, should allow safe conversion and
    // reinstate the stronger requirement for trivial conversion below.
    //        if(!is_utf_type(slice_value_type(left.data_type)) &&
    //        !is_utf_type(slice_value_type(right.data_type)))
    //            return true;

    return is_utf_type(slice_value_type(left.data_type())) ==
           is_utf_type(slice_value_type(right.data_type()));
  }
...
std::optional<entity::TypeDescriptor>
has_valid_type_promotion(const entity::TypeDescriptor& source,
                         const entity::TypeDescriptor& target) {

  if (source.dimension() != target.dimension()) {
    // Empty of dimension 0 means lack of any given type and can be promoted to anything
    // (even if the dimensions don't match), e.g. empty type can become int or array of
    // ints. Empty type of higher dimension is used to specify an empty array or an
    // empty matrix, thus it cannot become any other type unless the dimensionality
    // matches
    if (is_empty_type(source.data_type()) &&
        source.dimension() == entity::Dimension::Dim0)
      return target;
    return std::nullopt;
  }
  ...
  if (is_time_type(source_type)) {
    ...
    if (!is_utf_type(source_type) || !is_utf_type(target_type) ||
        !is_dynamic_string_type(target_type))
      return std::nullopt;
...
std::optional<entity::TypeDescriptor>
has_valid_common_type(const entity::TypeDescriptor& left,
                      const entity::TypeDescriptor& right) {
  ...
  // has_valid_type_promotion checks if the second type can represent all values of the
  // first type We also want to handle cases where there is a third type that can
  // represent both In practice, this is only the case when there is one signed and one
  // unsigned integer right now
  if (!maybe_common_type && left.dimension() == right.dimension() &&
      is_integer_type(left.data_type()) && is_integer_type(right.data_type())) {
    ...
    // To get here we must have one signed and one unsigned type, with the width of the
    // signed type <= the width of the unsigned type
