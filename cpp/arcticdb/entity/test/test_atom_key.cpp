...
TEST(Key, Basic) {
  ...
  AtomKey construct_numeric_key(numeric_id, version_id, creation_ts, content_hash,
                                timestamp_start, timestamp_end, numeric_data_key_type);
  ...
  auto numeric_key_from_bytes =
      atom_key_from_bytes(reinterpret_cast<uint8_t*>(numeric_key_string.data()),
                          numeric_key_string.size(), numeric_data_key_type);
  ...
  AtomKey construct_string_key(string_id, version_id, creation_ts, content_hash,
                               timestamp_start, timestamp_end, string_data_key_type);
  ...
  auto string_key_from_bytes =
      atom_key_from_bytes(reinterpret_cast<uint8_t*>(string_key_string.data()),
                          string_key_string.size(), string_data_key_type);
  ...
  auto string_index_from_bytes =
      atom_key_from_bytes(reinterpret_cast<uint8_t*>(string_index_key_string.data()),
                          string_index_key_string.size(), numeric_data_key_type);
...
TEST(Key, Formatting) {
  ...
  AtomKey k{arcticdb::StreamId{NumericId{999}},
            VersionId(123),
  ...
  auto k3 = atom_key_builder()
                ...
                .build<KeyType::TABLE_DATA>(NumericId{999});
...
TEST(AtomKey, ProtobufRoundtrip) {
  auto key = atom_key_builder()
                 ...
                 .start_index(3)
                 .end_index(4)
                 .build(StreamId{"Natbag"}, KeyType::TABLE_INDEX);
