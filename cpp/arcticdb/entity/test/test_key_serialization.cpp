...
std::string old_style_key(const AtomKey& key) {
  return fmt::format("{}|{}|{}|{}|{}|{}|{}", key.id(), key.version_id(),
                     key.creation_ts(), key.content_hash(),
                     int(stream::get_index_value_type(key)), key.start_index(),
                     key.end_index());
}
} // namespace arcticdb
...
TEST(KeySerialize, RefKeyTokenized) {
  ...
  auto out_key = from_tokenized_ref_key((const uint8_t*)str.data(), str.size(),
...
TEST(KeySerialize, RefKeySerialized) {
  ...
  auto out_key =
      from_serialized_ref_key((const uint8_t*)str.data(), KeyType::SNAPSHOT_REF);
  ASSERT_EQ(ref_key, out_key);
}
...
TEST(SerializeNumber, SignedNumbers) {
  ...
  auto new_key = from_tokenized_atom_key((const uint8_t*)tokenized.data(),
                                         tokenized.size(), key_type);
