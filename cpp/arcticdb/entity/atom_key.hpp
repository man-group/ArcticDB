...
public:
  ...
  AtomKeyImpl(StreamId id, VersionId version_id, timestamp creation_ts,
              ContentHash content_hash, IndexValueType start_index,
              IndexValueType end_index, KeyType key_type)
      : id_(std::move(id)), version_id_(version_id), creation_ts_(creation_ts),
        content_hash_(content_hash), key_type_(key_type),
        index_start_(std::move(start_index)), index_end_(std::move(end_index)) {}
  ...
  friend bool operator<(const AtomKeyImpl& l, const AtomKeyImpl& r) {
    auto lt =
        std::tie(l.id_, l.version_id_, l.index_start_, l.index_end_, l.creation_ts_);
    auto rt =
        std::tie(r.id_, r.version_id_, r.index_start_, r.index_end_, r.creation_ts_);
    return lt < rt;
  }
  ...
  size_t get_cached_hash() const {
    if (!hash_) {
      // arcticdb::commutative_hash_combine needs extra template specialisations for our
      // variant types, folly's built-in variant forwards to std::hash which should be
      // good enough for these simple types
      hash_ = folly::hash::hash_combine(id_, version_id_, creation_ts_, content_hash_,
                                        key_type_, index_start_, index_end_);
    }
    return *hash_;
  }
...
/**
 * Builder introduced since I feel having a ctor for the key with 4 fields with the same
 * type next to each other is going to result in inverted fields making it difficult at
 * call site to see what's happening. It might be avoided in perf critical situations.
...
public:
  ...
  auto& gen_id(VersionId v) {
    util::check_arg(version_id_ == 0,
                    "Should not set both version_id and version id on a key");
    version_id_ = v;
    return *this;
  }
  ...
  template <KeyType KT> AtomKeyImpl build(StreamId id) const {
    return {std::move(id), version_id_, creation_ts_, content_hash_, index_start_,
            index_end_,    KT};
  }

  AtomKeyImpl build(StreamId id, KeyType key_type) const {
    return {std::move(id), version_id_, creation_ts_, content_hash_,
            index_start_,  index_end_,  key_type};
  }
...
// Better would be to use intrusive pointers for strings into a local (e.g. per read
// call) deduped pool Using #pragma pack means the ankerl hashing can just treat the
// struct as an opaque buffer
#pragma pack(push)
#pragma pack(1)
struct AtomKeyPacked {

  AtomKeyPacked(VersionId version_id, timestamp creation_ts, ContentHash content_hash,
                KeyType key_type, timestamp index_start, timestamp index_end)
      : version_id_(version_id), creation_ts_(creation_ts), content_hash_(content_hash),
        key_type_(key_type), index_start_(index_start), index_end_(index_end) {}

  AtomKeyPacked(const AtomKey& atom_key)
      : version_id_(atom_key.version_id()), creation_ts_(atom_key.creation_ts()),
        key_type_(atom_key.type()), index_start_(atom_key.start_time()),
        index_end_(atom_key.end_time()) {}

  AtomKey to_atom_key(const StreamId& stream_id) const {
    return AtomKey(stream_id, version_id_, creation_ts_, content_hash_, index_start_,
                   index_end_, key_type_);
  }
  ...
  friend bool operator==(const AtomKeyPacked& l, const AtomKeyPacked& r) {
    return l.version_id_ == r.version_id_ && l.creation_ts_ == r.creation_ts_ &&
           l.content_hash_ == r.content_hash_ && l.key_type_ == r.key_type_ &&
           l.index_start_ == r.index_start_ && l.index_end_ == r.index_end_;
  }
};
...
// Could also do this for std::hash, but in cases where this struct is being used you
// should probably be using a more efficient hashing algorithm
template <> struct ankerl::unordered_dense::hash<arcticdb::entity::AtomKeyPacked> {
  using is_avalanching = void;

  [[nodiscard]] uint64_t
  operator()(const arcticdb::entity::AtomKeyPacked& key) const noexcept {
    return ankerl::unordered_dense::detail::wyhash::hash(
        &key, arcticdb::entity::AtomKeyPackedSize);
  }
};

// The formatting below deals with the display of keys in logs etc., i.e. in a
// human-readable format. Transformation of keys for persistence is handled elsewhere.
namespace fmt {

template <class FormatTag> struct formatter<FormattableRef<AtomKey, FormatTag>> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const FormattableRef<arcticdb::entity::AtomKey, FormatTag>& f,
              FormatContext& ctx) const {
    const auto& key = f.ref;
    return format_to(ctx.out(), FMT_STRING(FormatTag::format), key.type(), key.id(),
                     key.version_id(), key.content_hash(), key.creation_ts(),
                     tokenized_index(key.start_index()),
                     tokenized_index(key.end_index()));
  }
};

template <> struct formatter<AtomKey> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
...
namespace std {
template <> struct hash<arcticdb::entity::AtomKeyImpl> {
  inline arcticdb::HashedValue
  operator()(const arcticdb::entity::AtomKeyImpl& k) const noexcept {
