...
inline VariantId variant_id_from_token(std::string_view strv,
...
AtomKey atom_key_from_tokens(
    std::array<std::string_view, size_t(FieldIndex::num_key_fields)> arr,
    VariantType id_type, VariantType index_type, KeyType key_type) {
  auto start_index =
      variant_id_from_token(arr[int(FieldIndex::start_index)], index_type);
...
// This was the original way of doing it; we need to be able to read this kind but we
// don't want to write them any more
inline AtomKey key_from_old_style_bytes(const uint8_t* data, size_t size,
                                        KeyType key_type) {
  ...
  auto index_type =
      IndexDescriptorImpl::Type(util::num_from_strv(arr[int(OldKeyField::index_type)]));
  auto index_variant_type = variant_type_from_index_type(index_type);
  return atom_key_from_tokens<OldKeyField>(arr, id_variant_type, index_variant_type,
                                           key_type);
}
...
inline void serialize_string(const std::string& str, CursoredBuffer<T>& output) {
  util::check_arg(str.size() < max_string_size(),
                  "String too long for serialization type");
...
template <typename T> inline T unserialize_number(const uint8_t*& data) {
...
inline VariantType variant_type_from_id(const VariantId& id) {
  return std::holds_alternative<NumericId>(id) ? VariantType::NUMERIC_TYPE
                                               : VariantType::STRING_TYPE;
}
...
enum class FormatType : char {
  OPAQUE = 'o', // This is an efficient type (i.e. numbers as numbers), for things that
                // don't required string-like keys
  TOKENIZED = 't', // This is a readable type, with delimiters, for things that require
                   // keys to consist of printable characters
};

inline size_t max_id_size(const VariantId& id) {
  return util::variant_match(
      id, [](const StringId&) { return max_string_size(); },
      [](const auto&) { return sizeof(uint64_t); });
}
...
struct KeyDescriptor {
  explicit KeyDescriptor(const AtomKey& key, FormatType format_type)
      : identifier(SerializedKeyIdentifier), id_type(variant_type_from_id(key.id())),
        index_type(to_type_char(stream::get_index_value_type(key))),
        format_type(format_type) {}

  KeyDescriptor(const StringId& id, IndexDescriptorImpl::Type index_type,
                FormatType format_type)
      : identifier(SerializedKeyIdentifier), id_type(variant_type_from_id(id)),
        index_type(to_type_char(index_type)), format_type(format_type) {}
...
inline size_t max_key_size(const StreamId& id, const IndexDescriptor& idx) {
  return 3 * sizeof(uint64_t) + max_id_size(id) + (2 * max_index_size(idx)) +
         sizeof(KeyDescriptor);
}

} // namespace arcticdb::entity

namespace fmt {
template <> struct formatter<KeyDescriptor> {
  ...
  auto format(const KeyDescriptor& kd, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), FMT_COMPILE("{}{}{}{}"), kd.identifier,
                          char(kd.id_type), char(kd.index_type), char(kd.format_type));
  }
};
...
inline AtomKey from_serialized_atom_key(const uint8_t* data, KeyType key_type) {
  ...
  util::check(descr->identifier == SerializedKeyIdentifier,
              "Read invalid serialized key");
...
inline std::string to_tokenized_key(const AtomKey& key) {
  ...
  return fmt::format(FMT_COMPILE("{}*{}*{}*{}*{}*{}*{}"), kd, key.id(),
                     key.version_id(), key.creation_ts(), key.content_hash(),
...
inline AtomKey from_tokenized_atom_key(const uint8_t* data, size_t size,
                                       KeyType key_type) {
  const auto* descr = reinterpret_cast<const KeyDescriptor*>(data);
  util::check(descr->identifier == SerializedKeyIdentifier,
              "Read invalid serialized key");
  std::string_view cursor(reinterpret_cast<const char*>(data) + sizeof(KeyDescriptor),
                          size - sizeof(KeyDescriptor));
  auto tokens = std::count(std::begin(cursor), std::end(cursor), NewKeyDelimiter);
  auto index_variant_type =
      variant_type_from_index_type(from_type_char(descr->index_type));
  if (tokens == NumNewKeyFields) {
    auto arr = util::split_to_array<NumNewKeyFields>(cursor, NewKeyDelimiter);
    return atom_key_from_tokens<NewKeyField>(arr, descr->id_type, index_variant_type,
                                             key_type);
  } else {
    ...
    *replace_it = merged;
    ...
    return atom_key_from_tokens<NewKeyField>(fixup, descr->id_type, index_variant_type,
                                             key_type);
  }
}

inline RefKey from_tokenized_ref_key(const uint8_t* data, size_t size,
                                     KeyType key_type) {
  const auto* descr = reinterpret_cast<const KeyDescriptor*>(data);
  util::check(descr->identifier == SerializedKeyIdentifier,
              "Read invalid serialized key");
...
inline VariantKey from_tokenized_variant_key(const uint8_t* data, size_t size,
                                             KeyType key_type) {
...
inline VariantKey variant_key_from_bytes(const uint8_t* data, size_t size,
                                         KeyType key_type) {
  if (is_ref_key_class(key_type))
