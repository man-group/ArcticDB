...
inline arcticdb::proto::descriptors::SortedValue
sorted_value_to_proto(SortedValue sorted) {
  switch (sorted) {
...
// The type enum needs to be kept in sync with the protobuf one, which should not be
// changed
[[nodiscard]] arcticdb::proto::descriptors::IndexDescriptor
index_descriptor_to_proto(const IndexDescriptorImpl& index_descriptor) {
  arcticdb::proto::descriptors::IndexDescriptor proto;
  proto.set_kind(static_cast<arcticdb::proto::descriptors::IndexDescriptor_Type>(
      index_descriptor.type_));
...
arcticdb::proto::descriptors::AtomKey key_to_proto(const AtomKey& key) {
  ...
  util::variant_match(
      key.id(), [&](const StringId& id) { output.set_string_id(id); },
  ...
  util::check(std::holds_alternative<StringId>(key.start_index()) ||
                  !std::holds_alternative<StringId>(key.end_index()),
              "Start and end index mismatch");

  util::variant_match(
      key.start_index(), [&](const StringId& id) { output.set_string_start(id); },
      [&](const NumericId& id) { output.set_numeric_start(id); });

  util::variant_match(
      key.end_index(), [&](const StringId& id) { output.set_string_end(id); },
...
AtomKey key_from_proto(const arcticdb::proto::descriptors::AtomKey& input) {
  StreamId stream_id = input.id_case() == input.kNumericId
                           ? StreamId(input.numeric_id())
                           : StreamId(input.string_id());
  IndexValue index_start = input.index_start_case() == input.kNumericStart
                               ? IndexValue(input.numeric_start())
                               : IndexValue(input.string_start());
  IndexValue index_end = input.index_end_case() == input.kNumericEnd
                             ? IndexValue(input.numeric_end())
...
void copy_stream_descriptor_to_proto(
  ...
  *proto.mutable_index() = index_descriptor_to_proto(desc.index());
  util::variant_match(
      desc.id(), [&proto](const StringId& str) { proto.set_str_id(str); },
  ...
  for (const auto& field : desc.fields()) {
    ...
    new_field->mutable_type_desc()->set_dimension(
        static_cast<uint32_t>(field.type().dimension()));
    set_data_type(field.type().data_type(), *new_field->mutable_type_desc());
  }
}

arcticdb::proto::descriptors::TimeSeriesDescriptor
copy_time_series_descriptor_to_proto(const TimeseriesDescriptor& tsd) {
  ...
  copy_stream_descriptor_to_proto(index_stream_descriptor,
                                  *output.mutable_stream_descriptor());
  return output;
}
