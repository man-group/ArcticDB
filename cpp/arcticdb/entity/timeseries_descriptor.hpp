...
struct TimeseriesDescriptor {
  ...
  std::shared_ptr<FrameDescriptorImpl> frame_data_ =
      std::make_shared<FrameDescriptorImpl>();
  ...
  TimeseriesDescriptor(std::shared_ptr<FrameDescriptorImpl> frame_desc,
      ...
      : frame_data_(std::move(frame_desc)), segment_desc_(segment_desc),
        proto_(std::move(proto)), fields_(std::move(fields)), stream_id_(stream_id) {}
  ...
  void
  set_user_metadata(arcticdb::proto::descriptors::UserDefinedMetadata&& user_meta) {
    *proto_->mutable_user_meta() = std::move(user_meta);
  }

  void set_normalization_metadata(
      arcticdb::proto::descriptors::NormalizationMetadata&& norm_meta) {
    *proto_->mutable_normalization() = std::move(norm_meta);
  }

  void set_multi_key_metadata(
  ...
  [[nodiscard]] TimeseriesDescriptor clone() const {
    ...
    return {std::move(frame_desc), std::move(segment_desc), std::move(proto),
...
namespace fmt {
template <> struct formatter<arcticdb::TimeseriesDescriptor> {
  ...
  auto format(const arcticdb::TimeseriesDescriptor& tsd, FormatContext& ctx) const {
    if (!tsd.fields_ptr())
      return fmt::format_to(ctx.out(), "TimeseriesDescriptor<fields=empty, proto={}>",
                            tsd.proto());

    return fmt::format_to(ctx.out(), "TimeseriesDescriptor<fields={}, proto={}>",
                          tsd.fields(), tsd.proto());
  }
};

template <> struct formatter<arcticdb::TimeseriesDescriptor::Proto> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const arcticdb::TimeseriesDescriptor::Proto& tsd,
