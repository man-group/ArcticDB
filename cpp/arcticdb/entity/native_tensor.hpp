...
/*
 * A wrapper around a 1D or 2D tensor that provides a more convenient interface for
 * accessing the data in the tensor. This is used to pass data between the Python and
 * C++ layers.
 *
 * This typically stores the data of the numpy array backing a column of a pandas
 * DataFrame.
 */
struct NativeTensor {
  ...
  /// @param ndim The dimension of the tensor as reported by the API ArcticDB used to
  /// read it
  /// @param expanded_dim The dimension of the tensor as ArcticDB should perceive it.
  /// E.g. in case a column tensor holds pointers to an array it will be reported to be
  /// of dim1 (the column is an array of pointers) while we think of it as a dim2 tensor
  /// (a column is an array of arrays).
  NativeTensor(int64_t nbytes, int ndim, const stride_t* strides, const shape_t* shapes,
               DataType dt, stride_t elsize, const void* ptr, int expanded_dim)
      : nbytes_(nbytes), ndim_(ndim), dt_(dt), elsize_(elsize), ptr(ptr),
  ...
  NativeTensor(const NativeTensor& other)
      : nbytes_(other.nbytes_), ndim_(other.ndim_), dt_(other.dt_),
        elsize_(other.elsize_), ptr(other.ptr), expanded_dim_(other.expanded_dim_) {
  ...
  template <typename T> const T* ptr_cast(size_t pos) const {
    ...
    util::warn(elsize_condition,
               "Cannot safely ptr_cast when elsize_ is zero in NativeTensor");
    util::warn(strides_condition,
               "Cannot safely ptr_cast to type of size {} when strides ({}) is not a "
               "multiple of elsize ({}) in NativeTensor with dtype {}",
  ...
  stride_t elsize_;
  const void* ptr;
  /// @note: when iterating strides and shapes we should use the ndim as it is the
  /// dimension reported by the API providing the strides and shapes arrays,
  /// expanded_dim is what ArcticDB thinks of the tensor and using it can lead to out of
  /// bounds reads from strides and shapes.
  int expanded_dim_;
};
...
template <typename T> struct TypedTensor : public NativeTensor {
  static size_t itemsize() { return sizeof(T); }

  std::array<stride_t, 2> f_style_strides() {
    std::array<stride_t, 2> strides = {};
    if (std::any_of(std::begin(shapes_), std::end(shapes_),
                    [](auto x) { return x == 0; })) {
  ...
  template <typename... Ix> const T& at(Ix... index) const {
    return *(static_cast<const T*>(NativeTensor::data()) +
             byte_offset(ssize_t(index)...) / itemsize());
  }

  /// @param expanded_dim @see NativeTensor::NativeTensor for information about the
  /// difference between ndim and expanded_dim
  TypedTensor(const shape_t* shapes, ssize_t ndim, DataType dt, ssize_t elsize,
              const T* data, ssize_t expanded_dim)
      : NativeTensor(calc_elements(shapes, ndim) * itemsize(), ndim, nullptr, shapes,
                     dt, elsize, data, expanded_dim) {}

  explicit TypedTensor(const NativeTensor& tensor) : NativeTensor(tensor) {}

  TypedTensor(const NativeTensor& tensor, ssize_t slice_num, ssize_t regular_slice_size,
              ssize_t nvalues)
      : NativeTensor(nvalues * itemsize(), tensor.ndim(), tensor.strides(),
                     tensor.shape(), tensor.data_type(), tensor.elsize(), nullptr,
                     tensor.expanded_dim()) {

    ssize_t stride_offset;
    if (ndim() > 1) {
      // Check that we can evenly subdivide a matrix into n rows (otherwise we'd have to
      // have extra state to track how far along a row we were
    ...
    // The new column shape * the column stride tells us how far to move the data
    // pointer from the origin
...
template <typename T> py::array to_py_array(const TypedTensor<T>& tensor) {
  return py::array({tensor.shape(), tensor.shape() + tensor.ndim()},
                   reinterpret_cast<const T*>(tensor.data()));
}

template <typename T> using TensorType = TypedTensor<T>;
