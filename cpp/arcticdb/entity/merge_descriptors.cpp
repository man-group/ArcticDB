...
namespace arcticdb {
StreamDescriptor
merge_descriptors(const StreamDescriptor& original,
  ...
  if (original.index().uninitialized()) {
    if (default_index) {
      ...
      util::variant_match(
          temp_idx, [&merged_fields, &merged_fields_map](const auto& idx) {
            using IndexType = std::decay_t<decltype(idx)>;
            merged_fields.emplace_back(idx.name());
            merged_fields_map.try_emplace(
                idx.name(), TypeDescriptor{typename IndexType::TypeDescTag{}});
          });
    } else {
  ...
  // Merge all the fields for all slices, apart from the index which we already have
  // from the first descriptor. Note that we preserve the ordering as we see columns,
  // especially the index which needs to be column 0.
  for (const auto& fields : entries) {
    if (has_index) {
      util::variant_match(
          index, [](const EmptyIndex&) {}, [](const RowCountIndex&) {},
          [&fields](const auto& idx) { idx.check(*fields); });
    }

    for (size_t idx = has_index ? 1u : 0u; idx < static_cast<size_t>(fields->size());
         ++idx) {
      const auto& field = fields->at(idx);
      const auto& type_desc = field.type();
      if (filtered_set.empty() ||
          (filtered_set.find(field.name()) != filtered_set.end())) {
        if (auto existing = merged_fields_map.find(field.name());
            existing != merged_fields_map.end()) {
          ...
          if (existing_type_desc != type_desc) {
            ...
            if (new_descriptor) {
              ...
              util::raise_rte("No valid common type between {} and {} for column {}",
                              existing_type_desc, type_desc, field.name());
            }
          }
        } else {
  ...
  return arcticdb::entity::StreamDescriptor{
      original.id(), get_descriptor_from_index(index), std::move(new_fields)};
}
...
merge_descriptors(const StreamDescriptor& original,
  ...
  std::unordered_set<std::string_view> filtered_set =
      filtered_columns.has_value()
          ? std::unordered_set<std::string_view>(filtered_columns->begin(),
                                                 filtered_columns->end())
...
merge_descriptors(const StreamDescriptor& original,
  ...
  for (const auto& entry : entries) {
    fields.push_back(
        std::make_shared<FieldCollection>(entry.slice_.desc()->fields().clone()));
  }
  return merge_descriptors(original, fields, filtered_columns, default_index);
}

StreamDescriptor
merge_descriptors(const std::shared_ptr<Store>& store, const StreamDescriptor& original,
                  const std::vector<pipelines::SliceAndKey>& entries,
                  const std::unordered_set<std::string_view>& filtered_set,
                  const std::optional<IndexDescriptorImpl>& default_index) {
  std::vector<std::shared_ptr<FieldCollection>> fields;
  for (const auto& entry : entries) {
    fields.push_back(std::make_shared<FieldCollection>(
        entry.segment(store).descriptor().fields().clone()));
  }
