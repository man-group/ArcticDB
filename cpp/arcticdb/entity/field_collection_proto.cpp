...
FieldCollection
fields_from_proto(const arcticdb::proto::descriptors::StreamDescriptor& desc) {
...
void proto_from_fields(const FieldCollection& fields,
                       arcticdb::proto::descriptors::StreamDescriptor& desc) {
  for (const auto& field : fields) {
    auto new_field = desc.add_fields();
    new_field->MergeFrom(
        field_proto(field.type().data_type(), field.type().dimension(), field.name()));
  }
}
