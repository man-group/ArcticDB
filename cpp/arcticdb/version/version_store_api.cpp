...
VersionResultVector get_all_versions_for_symbols(
    ...
    for (auto& s_id : stream_ids) {
        ...
        for (const auto& entry : all_versions) {
            ...
            res.emplace_back(
                s_id, entry.version_id(), entry.creation_ts(), snapshots_for_symbol[{s_id, entry.version_id()}], false
            );
        }
...
void PythonVersionStore::snapshot(
    ...
    if (versions.empty()) {
        ...
        missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
            !filtered_symbols.empty(), "No valid symbols in the library, skipping creation for snapshot: {}", snap_name
        );
...
VersionedItem PythonVersionStore::write_versioned_dataframe(
    ...
    auto versioned_item =
        write_versioned_dataframe_internal(stream_id, frame, prune_previous_versions, sparsify_floats, validate_index);
...
ReadResult PythonVersionStore::read_column_stats_version(const StreamId& stream_id, const VersionQuery& version_query) {
        ...
        util::variant_match(
            ...
            [&res](ReadVersionOutput& read_version) {
                res.emplace_back(create_python_read_result(
                    read_version.versioned_item_, std::move(read_version.frame_and_descriptor_)
                ));
            },
...
ReadResult PythonVersionStore::read_dataframe_version(
    ...
    return create_python_read_result(
        opt_version_and_frame.versioned_item_, std::move(opt_version_and_frame.frame_and_descriptor_)
    );
}
...
ReadResult read_dataframe_from_file(const StreamId& stream_id, const std::string& path, ReadQuery& read_query) {
    auto opt_version_and_frame =
        read_dataframe_from_file_internal(stream_id, path, read_query, ReadOptions{}, codec::default_lz4_codec());
