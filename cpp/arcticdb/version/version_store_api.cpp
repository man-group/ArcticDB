...
VersionedItem PythonVersionStore::write_dataframe_specific_version(
    const StreamId& stream_id, const py::tuple& item, const py::object& norm,
  ...
  if (auto version_key = ::arcticdb::get_specific_version(store(), version_map(),
                                                          stream_id, version_id);
      version_key) {
    log::version().warn("Symbol stream_id: {} already exists with version_id: {}",
                        stream_id, version_id);
    return {std::move(*version_key)};
  }

  auto versioned_item = write_dataframe_impl(
      store(), VersionId(version_id),
...
std::vector<std::shared_ptr<InputTensorFrame>> create_input_tensor_frames(
  ...
  for (size_t idx = 0; idx < stream_ids.size(); idx++) {
    output.emplace_back(convert::py_ndf_to_frame(
        stream_ids[idx], items[idx], norms[idx], user_metas[idx], empty_types));
  }
  return output;
}
...
std::vector<std::variant<VersionedItem, DataError>> PythonVersionStore::batch_write(
  ...
  auto frames = create_input_tensor_frames(stream_ids, items, norms, user_metas,
                                           cfg().write_options().empty_types());
  return batch_write_versioned_dataframe_internal(stream_ids, std::move(frames),
                                                  prune_previous_versions,
                                                  validate_index, throw_on_error);
}
...
std::vector<std::variant<VersionedItem, DataError>> PythonVersionStore::batch_append(
  ...
  auto frames = create_input_tensor_frames(stream_ids, items, norms, user_metas,
                                           cfg().write_options().empty_types());
  return batch_append_internal(stream_ids, std::move(frames), prune_previous_versions,
                               validate_index, upsert, throw_on_error);
}
...
using VersionResult =
    std::tuple<StreamId, VersionId, timestamp, std::vector<SnapshotId>, bool>;
struct VersionComp {
...
using SymbolVersionToSnapshotMap =
    std::unordered_map<std::pair<StreamId, VersionId>, std::vector<SnapshotId>>;
using SymbolVersionTimestampMap =
    std::unordered_map<std::pair<StreamId, VersionId>, timestamp>;

using VersionResultVector = std::vector<VersionResult>;

VersionResultVector
list_versions_for_snapshot(const std::set<StreamId>& stream_ids,
  ...
  util::check(versions_for_snapshots.count(snap_name.value()) != 0,
              "Snapshot not found");
  ...
  for (auto& s_id : stream_ids) {
    ...
    res.emplace_back(s_id, version_key.version_id(), version_key.creation_ts(),
                     snapshots_for_symbol[{s_id, version_key.version_id()}], false);
  }
...
void get_snapshot_version_info(
  ...
  // The symbols might appear in more than 1 snapshot and "snapshots" needs to be
  // populated After SNAPSHOT_REF key introduction, this operation is no longer slow
  ...
  for (const auto& [snap_id, index_keys] : *versions_for_snapshots) {
    for (const auto& index_key : index_keys) {
      ...
      creation_ts_for_version_symbol[{index_key.id(), index_key.version_id()}] =
          index_key.creation_ts();
    }
  }
...
VersionResultVector
get_latest_versions_for_symbols(const std::shared_ptr<Store>& store,
                                const std::shared_ptr<VersionMap>& version_map,
                                const std::set<StreamId>& stream_ids,
                                SymbolVersionToSnapshotMap& snapshots_for_symbol) {
  VersionResultVector res;
  for (auto& s_id : stream_ids) {
    const auto& opt_version_key =
        get_latest_undeleted_version(store, version_map, s_id);
    if (opt_version_key) {
      res.emplace_back(
          s_id, opt_version_key->version_id(), opt_version_key->creation_ts(),
          snapshots_for_symbol[{s_id, opt_version_key->version_id()}], false);
    }
  }
  std::sort(res.begin(), res.end(), VersionComp());
  return res;
...
VersionResultVector get_all_versions_for_symbols(
  ...
  for (auto& s_id : stream_ids) {
    ...
    for (const auto& entry : all_versions) {
      ...
      res.emplace_back(s_id, entry.version_id(), entry.creation_ts(),
                       snapshots_for_symbol[{s_id, entry.version_id()}], false);
    }
    for (const auto& [sym_version, creation_ts] : creation_ts_for_version_symbol) {
      // For all symbol, version combinations in snapshots, check if they have been
      // pruned, and if so use the information from the snapshot indexes and set deleted
      // to true.
      if (sym_version.first == s_id &&
          unpruned_versions.find(sym_version) == std::end(unpruned_versions)) {
        res.emplace_back(sym_version.first, sym_version.second, creation_ts,
                         snapshots_for_symbol[sym_version], true);
      }
    }
  }
  std::sort(res.begin(), res.end(), VersionComp());
...
VersionResultVector
PythonVersionStore::list_versions(const std::optional<StreamId>& stream_id,
  ...
  std::optional<SnapshotMap> versions_for_snapshots;
  if (do_snapshots) {
    get_snapshot_version_info(store(), snapshots_for_symbol,
                              creation_ts_for_version_symbol, versions_for_snapshots);

    if (snap_name)
      return list_versions_for_snapshot(stream_ids, snap_name, *versions_for_snapshots,
                                        snapshots_for_symbol);
  }

  if (opt_false(latest_only))
    return get_latest_versions_for_symbols(store(), version_map(), stream_ids,
                                           snapshots_for_symbol);
  else
    return get_all_versions_for_symbols(store(), version_map(), stream_ids,
                                        snapshots_for_symbol,
...
py::object get_metadata_for_snapshot(const std::shared_ptr<Store>& store,
                                     const VariantKey& snap_key) {
  auto seg = store->read_sync(snap_key).second;
  return get_metadata_from_segment(seg);
}

std::pair<std::vector<AtomKey>, py::object>
get_versions_and_metadata_from_snapshot(const std::shared_ptr<Store>& store,
                                        const VariantKey& vk) {
...
std::vector<std::pair<SnapshotId, py::object>>
PythonVersionStore::list_snapshots(std::optional<bool> load_metadata) {
  ...
  iterate_snapshots(
      store(), [store = store(), &snap_ids, fetch_metadata](VariantKey& vk) {
        auto snapshot_meta_as_pyobject =
            fetch_metadata ? get_metadata_for_snapshot(store, vk) : py::none{};
...
void PythonVersionStore::add_to_snapshot(
  ...
  util::check(version_queries.empty() || stream_ids.size() == version_queries.size(),
              "List length mismatch in add_to_snapshot: {} != {}", stream_ids.size(),
  ...
  auto [snapshot_contents, user_meta] =
      get_versions_and_metadata_from_snapshot(store(), snap_key);
  auto [specific_versions_index_map, latest_versions_index_map] =
      get_stream_index_map(stream_ids, version_queries);
  ...
  auto missing = filter_keys_on_existence(
      utils::copy_of_values_as<VariantKey>(*specific_versions_index_map), store(),
      false);
  util::check(missing.empty(), "Cannot snapshot version(s) that have been deleted: {}",
              missing);
  ...
  if (is_delete_keys_immediately) {
    delete_trees_responsibly(store(), version_map(), deleted_keys,
                             get_master_snapshots_map(store()), snap_name)
        .get();
    if (version_map()->log_changes()) {
      log_delete_snapshot(store(), snap_name);
    }
  }
  write_snapshot_entry(store(), retained_keys, snap_name, user_meta,
                       version_map()->log_changes());
}
...
void PythonVersionStore::remove_from_snapshot(
  ...
  std::vector<AtomKey> retained_keys;
  for (auto&& key : snapshot_contents) {
    if (symbol_versions.find(SymbolVersion{key.id(), key.version_id()}) ==
        symbol_versions.end()) {
      retained_keys.emplace_back(std::move(key));
    } else {
      if (is_delete_keys_immediately) {
...
void PythonVersionStore::snapshot(const SnapshotId& snap_name,
  ...
  util::check_arg(skip_symbols.empty() || versions.empty(),
                  "Only one of skip_symbols and versions can be set");

  // Explicitly set logging to error prior to testing if snapshot already exists as
  // otherwise we're guaranteed to see a warning message for all new snapshots (as the
  // key doesn't exist!)
  ...
  if (versions.empty()) {
    // User did not provide explicit versions to snapshot, get latest of all filtered
    // symbols.
    auto skip_symbols_set =
        std::set<StreamId>(skip_symbols.begin(), skip_symbols.end());
    auto all_symbols = list_streams();

    std::vector<StreamId> filtered_symbols;
    std::set_difference(all_symbols.begin(), all_symbols.end(),
                        skip_symbols_set.begin(), skip_symbols_set.end(),
    ...
    auto sym_index_map =
        batch_get_latest_version(store(), version_map(), filtered_symbols, false);
    index_keys = utils::values(*sym_index_map);
  } else {
    auto sym_index_map = batch_get_specific_version(
        store(), version_map(), versions,
        BatchGetVersionOption::LIVE_AND_TOMBSTONED_VER_REF_IN_OTHER_SNAPSHOT);
    if (allow_partial_snapshot) {
      missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
          !sym_index_map->empty(),
          "None of the symbol-version pairs specified in versions exist, skipping "
          "creation for snapshot: {}",
          snap_name);
    } else {
      if (sym_index_map->size() != versions.size()) {
        std::string error_msg =
            fmt::format("Snapshot {} will not be created. Specified symbol-version "
                        "pairs do not exist in the library: ",
                        snap_name);
    ...
    util::check(missing.empty(),
                "Cannot snapshot version(s) that have been deleted: {}", missing);
  }

  ARCTICDB_DEBUG(log::version(), "Total Index keys in snapshot={}", index_keys.size());
  write_snapshot_entry(store(), index_keys, snap_name, user_meta,
                       version_map()->log_changes());
}

std::set<StreamId>
PythonVersionStore::list_streams(const std::optional<SnapshotId>& snap_name,
  ...
  return list_streams_internal(snap_name, regex, prefix, opt_use_symbol_list,
                               opt_all_symbols);
}
...
VersionedItem PythonVersionStore::write_partitioned_dataframe(
  ...
  //    TODO: We are not actually partitioning stuff atm, just assuming a single
  //    partition is passed for now.
  ...
  for (size_t idx = 0; idx < partitioned_dfs.size(); idx++) {
    ...
    auto versioned_item = write_dataframe_impl(
        store(), version_id,
        convert::py_ndf_to_frame(subkeyname, partitioned_dfs[idx], norm_meta,
                                 py::none(), cfg().write_options().empty_types()),
        write_options, de_dup_map, false);
      ...
      [&stream_id, version_id, &multi_key_fut, store = store()](auto&& segment) {
        multi_key_fut = store
                            ->write(KeyType::PARTITION,
                                    ...
                                    stream_id, NumericIndex{0}, // start_index
...
VersionedItem PythonVersionStore::write_versioned_composite_data(
    const StreamId& stream_id, const py::object& metastruct,
    const std::vector<StreamId>& sub_keys, const std::vector<py::tuple>& items,
    const std::vector<py::object>& norm_metas, const py::object& user_meta,
  ...
  ARCTICDB_DEBUG(log::version(),
                 ...
                 stream_id, version_id);
  // TODO: Assuming each sub key is always going to have the same version attached to
  // it.
  ...
  auto index_keys = folly::collect(batch_write_internal(std::move(version_ids),
                                                        sub_keys, std::move(frames),
                                                        std::move(de_dup_maps), false))
                        .get();
  release_gil.reset();
  auto multi_key = write_multi_index_entry(store(), index_keys, stream_id, metastruct,
                                           user_meta, version_id);
...
VersionedItem PythonVersionStore::write_versioned_dataframe(
    const StreamId& stream_id, const py::tuple& item, const py::object& norm,
    const py::object& user_meta, bool prune_previous_versions, bool sparsify_floats,
    bool validate_index) {
...
VersionedItem PythonVersionStore::append(const StreamId& stream_id,
                                         const py::tuple& item, const py::object& norm,
                                         const py::object& user_meta, bool upsert,
                                         bool prune_previous_versions,
                                         bool validate_index) {
  return append_internal(stream_id,
                         convert::py_ndf_to_frame(stream_id, item, norm, user_meta,
                                                  cfg().write_options().empty_types()),
                         upsert, prune_previous_versions, validate_index);
}
...
VersionedItem PythonVersionStore::update(const StreamId& stream_id,
  ...
  return update_internal(stream_id, query,
                         convert::py_ndf_to_frame(stream_id, item, norm, user_meta,
                                                  cfg().write_options().empty_types()),
                         upsert, dynamic_schema, prune_previous_versions);
}

VersionedItem PythonVersionStore::delete_range(const StreamId& stream_id,
                                               const UpdateQuery& query,
                                               bool dynamic_schema,
                                               bool prune_previous_versions) {
  return delete_range_internal(
      stream_id, query, DeleteRangeOptions{dynamic_schema, prune_previous_versions});
}
...
VersionedItem PythonVersionStore::write_metadata(const StreamId& stream_id,
                                                 const py::object& user_meta,
                                                 bool prune_previous_versions) {
  arcticdb::proto::descriptors::UserDefinedMetadata user_meta_proto;
  python_util::pb_from_python(user_meta, user_meta_proto);
  return write_versioned_metadata_internal(stream_id, prune_previous_versions,
...
void PythonVersionStore::create_column_stats_version(
  ...
  create_column_stats_version_internal(stream_id, column_stats, version_query,
                                       read_options);
}
...
ReadResult
PythonVersionStore::read_column_stats_version(const StreamId& stream_id,
                                              const VersionQuery& version_query) {
  ARCTICDB_SAMPLE(ReadColumnStats, 0)
  auto [versioned_item, frame_and_descriptor] =
      read_column_stats_version_internal(stream_id, version_query);
  return make_read_result_from_frame(frame_and_descriptor, versioned_item.key_);
}

ColumnStats
PythonVersionStore::get_column_stats_info_version(const StreamId& stream_id,
                                                  const VersionQuery& version_query) {
...
VersionedItem PythonVersionStore::compact_incomplete(
    const StreamId& stream_id, bool append, bool convert_int_to_float,
    bool via_iteration /*= true */, bool sparsify /*= false */,
  ...
  CompactIncompleteOptions options{.prune_previous_versions_ = prune_previous_versions,
                                   .append_ = append,
...
VersionedItem PythonVersionStore::sort_merge(const StreamId& stream_id,
                                             const py::object& user_meta, bool append,
                                             bool convert_int_to_float,
                                             bool via_iteration, bool sparsify,
                                             bool prune_previous_versions) {
  std::optional<arcticdb::proto::descriptors::UserDefinedMetadata> meta;
...
std::unordered_map<VersionId, bool>
PythonVersionStore::get_all_tombstoned_versions(const StreamId& stream_id) {
  return ::arcticdb::get_all_tombstoned_versions(store(), version_map(), stream_id);
}

FrameAndDescriptor create_frame(const StreamId& target_id, SegmentInMemory seg,
                                const ReadOptions& read_options) {
  auto context = std::make_shared<PipelineContext>(
      seg, in_memory_key(KeyType::TABLE_DATA, target_id, VersionId{0}));
  seg.unsparsify();
  reduce_and_fix_columns(context, seg, read_options);
  auto norm_meta = make_timeseries_norm_meta(target_id);
  const auto desc = make_timeseries_descriptor(
      seg.row_count(), seg.descriptor().clone(), std::move(norm_meta), std::nullopt,
      std::nullopt, std::nullopt, false);
  return FrameAndDescriptor{std::move(seg), desc, {}, {}};
}

ReadResult PythonVersionStore::read_dataframe_merged(
    const StreamId& target_id, const std::vector<StreamId>& stream_ids,
    const VersionQuery&, // TODO batch_get_specific_version
    const ReadQuery& query, const ReadOptions& read_options) {
  if (stream_ids.empty())
    util::raise_rte("No symbols given");

  auto stream_index_map =
      batch_get_latest_version(store(), version_map(), stream_ids, false);
  std::vector<AtomKey> index_keys;
  std::transform(stream_index_map->begin(), stream_index_map->end(),
  ...
  auto [key, metadata, descriptor] =
      store()->read_metadata_and_descriptor(stream_index_map->begin()->second).get();
  auto index = index_type_from_descriptor(descriptor);

  FrameAndDescriptor final_frame;
  VariantColumnPolicy density_policy = read_options.allow_sparse_
                                           ? VariantColumnPolicy{SparseColumnPolicy{}}
                                           : VariantColumnPolicy{DenseColumnPolicy{}};

  merge_frames_for_keys(
      target_id, std::move(index), NeverSegmentPolicy{}, density_policy, index_keys,
      query, store(), [&final_frame, &target_id, &read_options](auto&& seg) {
        final_frame =
            create_frame(target_id, std::forward<decltype(seg)>(seg), read_options);
      });
...
std::vector<std::variant<ReadResult, DataError>>
PythonVersionStore::batch_read(const std::vector<StreamId>& stream_ids,
                               const std::vector<VersionQuery>& version_queries,
                               std::vector<std::shared_ptr<ReadQuery>>& read_queries,
                               const ReadOptions& read_options) {

  auto read_versions_or_errors =
      batch_read_internal(stream_ids, version_queries, read_queries, read_options);
  std::vector<std::variant<ReadResult, DataError>> res;
  for (auto&& [idx, read_version_or_error] :
       folly::enumerate(read_versions_or_errors)) {
    util::variant_match(
        read_version_or_error,
        [&res](ReadVersionOutput& read_version) {
          res.emplace_back(
              create_python_read_result(read_version.versioned_item_,
...
ReadResult PythonVersionStore::read_dataframe_version(const StreamId& stream_id,
  ...
  auto opt_version_and_frame = read_dataframe_version_internal(
      stream_id, version_query, read_query, read_options);
...
void PythonVersionStore::delete_snapshot(const SnapshotId& snap_name) {
  ...
  if (variant_key_type(snap_key) == KeyType::SNAPSHOT_REF &&
      cfg().write_options().delayed_deletes()) {
    ARCTICDB_DEBUG(log::version(), "Delaying deletion of Snapshot {}", snap_name);
    tombstone_snapshot(store(), to_ref(snap_key), std::move(snap_segment),
                       version_map()->log_changes());
  } else {
...
void PythonVersionStore::delete_snapshot_sync(const SnapshotId& snap_name,
                                              const VariantKey& snap_key) {
  ...
  auto snap_map = get_master_snapshots_map(
      store(), std::tie(snap_key, index_keys_in_current_snapshot));

  ARCTICDB_DEBUG(log::version(), "Deleting Snapshot {}", snap_name);
  store()->remove_key(snap_key).get();

  try {
    delete_trees_responsibly(store(), version_map(), index_keys_in_current_snapshot,
                             snap_map, snap_name)
        .get();
    ARCTICDB_DEBUG(log::version(), "Deleted orphaned index keys in snapshot {}",
                   snap_name);
  } catch (const std::exception& ex) {
    log::version().warn(
        "Garbage collection of unreachable deleted index keys failed due to: {}",
        ex.what());
  }
}
...
std::vector<SnapshotVariantKey>
    ARCTICDB_UNUSED iterate_snapshot_tombstones(const std::string& limit_stream_id,
    ...
    store->iterate_type(
        ...
        [&store, &candidates, &snap_tomb_keys](VariantKey&& snap_tomb_key) {
          ...
          if (!indexes.empty()) {
            ...
            candidates.insert(std::move_iterator(indexes.begin()),
                              std::move_iterator(indexes.end()));
            indexes.clear();
          }

          ARCTICDB_DEBUG(log::version(),
                         "Processed {} keys from snapshot {}. {} are unique.",
                         snap_seg.row_count(), variant_key_id(snap_tomb_key),
...
void PythonVersionStore::delete_version(const StreamId& stream_id,
                                        VersionId version_id) {
  ARCTICDB_RUNTIME_DEBUG(log::version(), "Command: delete_version");
  auto result =
      ::arcticdb::tombstone_version(store(), version_map(), stream_id, version_id);
...
void PythonVersionStore::prune_previous_versions(const StreamId& stream_id) {
  ARCTICDB_RUNTIME_DEBUG(log::version(),
                         "Command: prune_previous_versions stream_id={}", stream_id);
  const std::shared_ptr<VersionMapEntry>& entry = version_map()->check_reload(
      store(), stream_id, LoadStrategy{LoadType::ALL, LoadObjective::UNDELETED_ONLY},
      __FUNCTION__);
  storage::check<ErrorCode::E_SYMBOL_NOT_FOUND>(!entry->empty(),
                                                "Symbol {} is not found", stream_id);
  ...
  if (!prev_id) {
    ARCTICDB_DEBUG(log::version(), "No previous versions to prune for stream_id={}",
                   stream_id);
    return;
  }

  auto previous =
      ::arcticdb::get_specific_version(store(), version_map(), stream_id, *prev_id);
  auto [_, pruned_indexes] =
      version_map()->tombstone_from_key_or_all(store(), stream_id, previous);
...
void PythonVersionStore::delete_all_versions(const StreamId& stream_id) {
  ...
  try {
    auto [version_id, all_index_keys] =
    ...
    log::version().error(
        "Got storage exception in delete - possible parallel deletion?: {}", ex.what());
  } catch (const CodecException& ex) {
    log::version().error(
        "Got codec exception in delete - possible parallel deletion?: {}", ex.what());
  }
}

std::vector<timestamp>
PythonVersionStore::get_update_times(const std::vector<StreamId>& stream_ids,
                                     const std::vector<VersionQuery>& version_queries) {
  return batch_get_update_times(stream_ids, version_queries);
}

timestamp PythonVersionStore::get_update_time(const StreamId& stream_id,
                                              const VersionQuery& version_query) {
  return get_update_time_internal(stream_id, version_query);
}

namespace {
py::object metadata_protobuf_to_pyobject(
    const std::optional<google::protobuf::Any>& metadata_proto) {
  py::object pyobj;
  if (metadata_proto) {
...
std::pair<VersionedItem, py::object>
PythonVersionStore::read_metadata(const StreamId& stream_id,
                                  const VersionQuery& version_query) {
  ...
  if (!metadata.first.has_value())
    throw NoDataFoundException(
        fmt::format("read_metadata: version not found for symbol", stream_id));
...
PythonVersionStore::batch_write_metadata(const std::vector<StreamId>& stream_ids,
...
PythonVersionStore::batch_restore_version(
    const std::vector<StreamId>& stream_ids,
    const std::vector<VersionQuery>& version_queries) {
  return batch_restore_version_internal(stream_ids, version_queries);
}

std::vector<std::variant<std::pair<VersionedItem, py::object>, DataError>>
PythonVersionStore::batch_read_metadata(
  ...
  auto metadatas_or_errors =
      batch_read_metadata_internal(stream_ids, version_queries, read_options);
  ...
  for (auto&& metadata_or_error : metadatas_or_errors) {
    if (std::holds_alternative<
            std::pair<VariantKey, std::optional<google::protobuf::Any>>>(
            metadata_or_error)) {
      auto& [key, meta_proto] =
          std::get<std::pair<VariantKey, std::optional<google::protobuf::Any>>>(
              metadata_or_error);
      VersionedItem version{std::move(to_atom(key))};
      if (meta_proto.has_value()) {
        auto res = std::make_pair(std::move(version),
                                  metadata_protobuf_to_pyobject(std::move(meta_proto)));
...
DescriptorItem PythonVersionStore::read_descriptor(const StreamId& stream_id,
                                                   const VersionQuery& version_query) {
...
ReadResult PythonVersionStore::read_index(const StreamId& stream_id,
                                          const VersionQuery& version_query) {
  ...
  if (!version)
    throw NoDataFoundException(
        fmt::format("read_index: version not found for symbol '{}'", stream_id));
...
std::vector<AtomKey>
PythonVersionStore::get_version_history(const StreamId& stream_id) {
...
PythonVersionStore::list_incompletes(const StreamId& stream_id) {
...
void PythonVersionStore::clear(const bool continue_on_error) {
  ...
  if (store()->fast_delete()) {
    // Most storage backends have a fast deletion method for a db/collection equivalent,
    // eg. drop() for mongo and lmdb and iterating each key is always going to be
    // suboptimal.
...
bool PythonVersionStore::empty() {
  ...
  try {
    foreach_key_type([store = store()](KeyType key_type) {
      // Skip symbol list keys, as these can be generated by list_symbols calls even if
      // the lib is empty
...
void write_dataframe_to_file(const StreamId& stream_id, const std::string& path,
                             const py::tuple& item, const py::object& norm,
                             const py::object& user_meta) {
  ARCTICDB_SAMPLE(WriteDataframeToFile, 0)
  auto frame = convert::py_ndf_to_frame(stream_id, item, norm, user_meta, false);
  write_dataframe_to_file_internal(stream_id, frame, path, WriteOptions{},
                                   codec::default_lz4_codec(), EncodingVersion::V2);
}

ReadResult read_dataframe_from_file(const StreamId& stream_id, const std::string& path,
                                    ReadQuery& read_query) {
  auto opt_version_and_frame = read_dataframe_from_file_internal(
      stream_id, path, read_query, ReadOptions{}, codec::default_lz4_codec());
