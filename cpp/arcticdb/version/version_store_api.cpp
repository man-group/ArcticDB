...
VersionResultVector list_versions_for_snapshot_with_snapshot_list(
        const R& stream_ids, std::optional<SnapshotId> snap_name, SnapshotMap&& versions_for_snapshots,
...
VersionResultVector PythonVersionStore::list_versions(
    ...
    std::vector<StreamId> ordered_stream_ids(std::make_move_iterator(stream_ids.begin()), std::make_move_iterator(stream_ids.end()));
