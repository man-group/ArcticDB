...
get_num_version_entries(const std::shared_ptr<Store>& store, size_t batch_size) {
  ...
  store->iterate_type(entity::KeyType::VERSION,
                      [&output, batch_size, max_blocks](const VariantKey& key) {
...
FrameAndDescriptor frame_and_descriptor_from_segment(SegmentInMemory&& seg) {
  ...
  if (seg_descriptor.index().type() == IndexDescriptor::Type::ROWCOUNT)
    ...
    ensure_timeseries_norm_meta(*tsd.mutable_proto().mutable_normalization(),
                                seg_descriptor.id(), false);
