...
public:
  OpLog() = delete;
  OpLog(AtomKey&& key);
  OpLog(StringId id, VersionId version_id, const std::string& action,
...
namespace fmt {
template <> struct formatter<arcticdb::OpLog> {
  ...
  auto format(const arcticdb::OpLog& op_log, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), "{} {} v{} at {}", op_log.action(), op_log.id(),
                          op_log.version_id(), op_log.creation_ts());
  }
};
} // namespace fmt
