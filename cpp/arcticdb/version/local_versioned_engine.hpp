...
/**
 * Implements the parent interface and provides additional methods not needed/suitable
 * by a RemoteVersionedEngine.
 *
...
public:
  ...
  VersionedItem update_internal(const StreamId& stream_id, const UpdateQuery& query,
                                const std::shared_ptr<InputTensorFrame>& frame,
                                bool upsert, bool dynamic_schema,
                                bool prune_previous_versions) override;

  VersionedItem append_internal(const StreamId& stream_id,
                                const std::shared_ptr<InputTensorFrame>& frame,
                                bool upsert, bool prune_previous_versions,
                                bool validate_index) override;

  VersionedItem delete_range_internal(const StreamId& stream_id,
                                      const UpdateQuery& query,
                                      const DeleteRangeOptions& option) override;

  void append_incomplete_segment(const StreamId& stream_id,
                                 SegmentInMemory&& seg) override;

  std::pair<VersionedItem, TimeseriesDescriptor>
  restore_version(const StreamId& id, const VersionQuery& version_query) override;

  void append_incomplete_frame(const StreamId& stream_id,
                               const std::shared_ptr<InputTensorFrame>& frame,
  ...
  std::optional<VersionedItem> get_specific_version(const StreamId& stream_id,
                                                    SignedVersionId signed_version_id);

  std::optional<VersionedItem> get_version_at_time(const StreamId& stream_id,
                                                   timestamp as_of);

  std::optional<VersionedItem> get_version_from_snapshot(const StreamId& stream_id,
                                                         const SnapshotId& snap_name);

  IndexRange get_index_range(const StreamId& stream_id,
                             const VersionQuery& version_query) override;

  std::optional<VersionedItem> get_version_to_read(const StreamId& stream_id,
                                                   const VersionQuery& version_query);

  FrameAndDescriptor
  read_dataframe_internal(const std::variant<VersionedItem, StreamId>& identifier,
                          ReadQuery& read_query,
                          const ReadOptions& read_options) override;

  ReadVersionOutput read_dataframe_version_internal(
      const StreamId& stream_id, const VersionQuery& version_query,
      ReadQuery& read_query, const ReadOptions& read_options) override;

  DescriptorItem read_descriptor_internal(const StreamId& stream_id,
                                          const VersionQuery& version_query);

  void write_parallel_frame(const StreamId& stream_id,
                            const std::shared_ptr<InputTensorFrame>& frame,
                            bool validate_index) const override;
  ...
  void delete_tree(const std::vector<IndexTypeKey>& idx_to_be_deleted,
    ...
    delete_trees_responsibly(store(), version_map(), idx_to_be_deleted, snapshot_map,
                             std::nullopt, checks)
        .get();
  };

  std::set<StreamId>
  list_streams_internal(std::optional<SnapshotId> snap_name,
  ...
  VersionedItem write_versioned_dataframe_internal(
      ...
      bool prune_previous_versions, bool allow_sparse, bool validate_index) override;
  ...
  get_metadata_async(folly::Future<std::optional<AtomKey>>&& version_fut,
                     const StreamId& stream_id, const VersionQuery& version_query);

  folly::Future<DescriptorItem> get_descriptor(AtomKey&& key);

  folly::Future<DescriptorItem>
  get_descriptor_async(folly::Future<std::optional<AtomKey>>&& version_fut,
                       const StreamId& stream_id, const VersionQuery& version_query);

  void create_column_stats_internal(const VersionedItem& versioned_item,
                                    ColumnStats& column_stats,
                                    const ReadOptions& read_options);

  void create_column_stats_version_internal(const StreamId& stream_id,
                                            ColumnStats& column_stats,
                                            const VersionQuery& version_query,
                                            const ReadOptions& read_options);

  void
  drop_column_stats_internal(const VersionedItem& versioned_item,
  ...
  ReadVersionOutput
  read_column_stats_version_internal(const StreamId& stream_id,
                                     const VersionQuery& version_query);

  ColumnStats get_column_stats_info_internal(const VersionedItem& versioned_item);

  ColumnStats get_column_stats_info_version_internal(const StreamId& stream_id,
                                                     const VersionQuery& version_query);

  VersionedItem write_individual_segment(const StreamId& stream_id,
  ...
  batch_write_versioned_metadata_internal(
  ...
  batch_read_internal(const std::vector<StreamId>& stream_ids,
                      const std::vector<VersionQuery>& version_queries,
                      std::vector<std::shared_ptr<ReadQuery>>& read_queries,
                      const ReadOptions& read_options);

  std::vector<std::variant<ReadVersionOutput, DataError>>
  temp_batch_read_internal_direct(const std::vector<StreamId>& stream_ids,
                                  const std::vector<VersionQuery>& version_queries,
                                  std::vector<std::shared_ptr<ReadQuery>>& read_queries,
                                  const ReadOptions& read_options);

  std::vector<std::variant<DescriptorItem, DataError>>
  batch_read_descriptor_internal(const std::vector<StreamId>& stream_ids,
                                 const std::vector<VersionQuery>& version_queries,
                                 const ReadOptions& read_options);
  ...
  timestamp get_update_time_internal(const StreamId& stream_id,
                                     const VersionQuery& version_query);

  std::vector<timestamp>
  batch_get_update_times(const std::vector<StreamId>& stream_ids,
                         const std::vector<VersionQuery>& version_queries);
  ...
  std::pair<std::optional<VariantKey>, std::optional<google::protobuf::Any>>
  read_metadata_internal(const StreamId& stream_id, const VersionQuery& version_query);

  bool is_symbol_fragmented(const StreamId& stream_id,
                            std::optional<size_t> segment_size) override;
  ...
  VersionedItem sort_index(const StreamId& stream_id, bool dynamic_schema,
                           bool prune_previous_versions) override;
  ...
  void
  write_version_and_prune_previous(bool prune_previous_versions,
  ...
  VersionIdAndDedupMapInfo
  create_version_id_and_dedup_map(const version_store::UpdateInfo&& update_info,
                                  const StreamId& stream_id,
                                  const WriteOptions& write_options);
  ...
  static LocalVersionedEngine _test_init_from_store(const std::shared_ptr<Store>& store,
...
protected:
  template <class ClockType = util::SysClock>
  explicit LocalVersionedEngine(const std::shared_ptr<Store>& store,
                                const ClockType& = ClockType{});
  ...
  /**
   * Take tombstoned indexes that have been pruned in the version map and perform the
   * actual deletion for indexes that are safe to delete (eg indexes contained in a
   * snapshot are skipped).
   *
   * @param pruned_indexes Must all share the same id() and should be tombstoned.
   */
  folly::Future<folly::Unit>
  delete_unreferenced_pruned_indexes(std::vector<AtomKey>&& pruned_indexes,
  ...
  /**
   * Get the queried, if specified, otherwise the latest, versions of index keys for
   * each specified stream.
   * @param version_queries Only explicit versions are supported at the moment. The
   * implementation currently accepts deleted versions (e.g. to support reading
   * snapshots) and it's the caller's responsibility to verify. A pair of std unordered
   * maps are returned. The first one contains all the Atom keys for those queries that
   * we have specified a version. The second one contains all the Atom keys of the last
   * undeleted version for those queries that we haven't specified any version.
   */
  SpecificAndLatestVersionKeys
  get_stream_index_map(const std::vector<StreamId>& stream_ids,
                       const std::vector<VersionQuery>& version_queries);

  std::variant<VersionedItem, StreamId>
  get_version_identifier(const StreamId& stream_id, const VersionQuery& version_query,
                         const ReadOptions& read_options,
