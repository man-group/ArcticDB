    ...
    for (auto& stream_id : stream_ids) {
        vector_fut.push_back(
            async::submit_io_task(
                CheckReloadTask{
                    store, version_map, stream_id, LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY}
                }
    ...
    submit_tasks_for_range(
        ...
        [store, version_map](auto& sym_version) {
            LoadStrategy load_strategy{
                LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY, static_cast<SignedVersionId>(sym_version.second)
            };
