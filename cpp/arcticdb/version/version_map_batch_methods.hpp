...
struct SymbolStatus {
  ...
  SymbolStatus(VersionId version_id, bool exists, timestamp ts)
      : version_id_(version_id), exists_(exists), timestamp_(ts) {}
};
...
inline std::optional<std::string> collect_futures_exceptions(auto&& futures) {
  ...
  for (auto&& collected_fut : futures) {
    if (!collected_fut.hasValue()) {
      all_exceptions = all_exceptions.value_or("")
                           .append(collected_fut.exception().what().toStdString())
                           .append("\n");
    }
  }
...
inline void submit_tasks_for_range(Inputs inputs, TaskSubmitter submitter,
  ...
  std::optional<std::string> all_exceptions =
      collect_futures_exceptions(std::move(collected_futs));
  internal::check<ErrorCode::E_RUNTIME_ERROR>(!all_exceptions.has_value(),
                                              all_exceptions.value_or(""));
}

inline std::shared_ptr<std::unordered_map<StreamId, SymbolStatus>>
batch_check_latest_id_and_status(
      ...
      *symbols,
      [store, version_map, &load_strategy](auto& symbol) {
        return async::submit_io_task(
            CheckReloadTask{store, version_map, symbol, load_strategy});
      },
      [output, mutex](const auto& id, const std::shared_ptr<VersionMapEntry>& entry) {
        ...
        if (index_key) {
          ...
          if (index_key) {
            ...
            output->insert(std::make_pair<StreamId, SymbolStatus>(
                StreamId{id},
                {index_key->version_id(), false, index_key->creation_ts()}));
          } else {
...
inline std::shared_ptr<std::unordered_map<StreamId, AtomKey>> batch_get_latest_version(
  ...
  ARCTICDB_SAMPLE(BatchGetLatestVersion, 0)
  const LoadStrategy load_strategy{LoadType::LATEST,
                                   include_deleted ? LoadObjective::INCLUDE_DELETED
  ...
  submit_tasks_for_range(
      ...
      [store, version_map, &load_strategy](auto& stream_id) {
        return async::submit_io_task(
            CheckReloadTask{store, version_map, stream_id, load_strategy});
      },
...
inline std::vector<
    folly::Future<std::pair<std::optional<AtomKey>, std::optional<AtomKey>>>>
batch_get_latest_undeleted_and_latest_versions_async(
  ...
  std::vector<folly::Future<std::pair<std::optional<AtomKey>, std::optional<AtomKey>>>>
      vector_fut;
  for (auto& stream_id : stream_ids) {
    vector_fut.push_back(
        async::submit_io_task(
            ...
            .thenValue([](const std::shared_ptr<VersionMapEntry>& entry) {
              return std::make_pair(entry->get_first_index(false).first,
                                    entry->get_first_index(true).first);
            }));
  }
  return vector_fut;
}
        ...
        async::submit_io_task(
            ...
            .thenValue([](auto entry) {
              ...
              VersionId next_version_id =
                  latest_version.has_value() ? latest_version->version_id() + 1 : 0;
              return version_store::UpdateInfo{latest_undeleted_version,
                                               next_version_id};
            }));
  }
  return vector_fut;
...
// This version assumes that there is only one version per symbol, so no need for the
// state machine below
inline std::shared_ptr<std::unordered_map<StreamId, AtomKey>>
batch_get_specific_version(
    const std::shared_ptr<Store>& store, const std::shared_ptr<VersionMap>& version_map,
  ...
  submit_tasks_for_range(
      ...
      [store, version_map](auto& sym_version) {
        LoadStrategy load_strategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                                   static_cast<SignedVersionId>(sym_version.second)};
        return async::submit_io_task(
            CheckReloadTask{store, version_map, sym_version.first, load_strategy});
      },
      [output, option, output_mutex, store, tombstoned_vers, tombstoned_vers_mutex](
        ...
        auto version_details = find_index_key_for_version_id_and_tombstone_status(
            sym_version.second, entry);
        ...
        } else if (option == BatchGetVersionOption::
                                 LIVE_AND_TOMBSTONED_VER_REF_IN_OTHER_SNAPSHOT &&
                   version_details.version_status_ == VersionStatus::TOMBSTONED) {
          // Need to allow tombstoned version but referenced in other snapshot(s) can be
          // "re-snapshot"
          log::version().warn("Version {} for symbol {} is tombstoned, need to check "
                              "snapshots (this can be slow)",
                              sym_version.second, sym_version.first);
          std::lock_guard lock{*tombstoned_vers_mutex};
          tombstoned_vers->emplace_back(sym_version.first,
                                        version_details.key_.value());
        }
      });
  ...
  if (!tombstoned_vers->empty()) {
    ...
    for (const auto& tombstoned_ver : *tombstoned_vers) {
      ...
      if (cit != snap_map.cend() &&
          std::any_of(
              cit->second.cbegin(), cit->second.cend(),
 ...
 */
inline std::shared_ptr<std::unordered_map<std::pair<StreamId, VersionId>, AtomKey>>
batch_get_specific_versions(const std::shared_ptr<Store>& store,
  ...
  submit_tasks_for_range(
      std::move(tasks_input),
      [store, version_map](auto sym_version) {
        auto first_version = *std::min_element(std::begin(sym_version.second),
                                               std::end(sym_version.second));
        LoadStrategy load_strategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                                   static_cast<SignedVersionId>(first_version)};
        return async::submit_io_task(
            CheckReloadTask{store, version_map, sym_version.first, load_strategy});
      },

      [output, &sym_versions, include_deleted,
       mutex](auto sym_version, const std::shared_ptr<VersionMapEntry>& entry) {
        auto sym_it = sym_versions.find(sym_version.first);
        util::check(sym_it != sym_versions.end(),
                    "Failed to find versions for symbol {}", sym_version.first);
        const auto& versions = sym_it->second;
        for (auto version : versions) {
          auto index_key =
              find_index_key_for_version_id(version, entry, include_deleted);
          if (index_key) {
            std::lock_guard lock{*mutex};
...
// [StreamVersionData] is used to combine different [VersionQuery]s for a stream_id into
// a list of needed snapshots and a single [LoadStrategy] which will query the union of
// all version queries. It only ever produces load parameters where
// to_load=UNDELETED_ONLY.
struct StreamVersionData {
  size_t count_ = 0;
  LoadStrategy load_strategy_ =
      LoadStrategy{LoadType::NOT_LOADED, LoadObjective::UNDELETED_ONLY};
...
std::vector<folly::Future<std::optional<AtomKey>>>
batch_get_versions_async(const std::shared_ptr<Store>& store,
                         const std::shared_ptr<VersionMap>& version_map,
