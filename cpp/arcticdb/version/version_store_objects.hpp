...
/**
 * Parameter object for VersionedEngine::delete_tree to control which checks to perform
 * before physically deleting data.
 *
 * Currently, there's no code path that doesn't require checking snapshots, so no field
 * for it.
 */
struct PreDeleteChecks {
  ...
  /**
   * Check if the key being deleted is still visible. Eager deletion after tombstoning
   * can disable this.
   */
  bool version_visible = true;

  /**
   * Check the neighbouring versions for data blocks shared with the key(s) being
   * deleted. (At the time of writing, data can only be shared between neighbouring
   * versions. Future dedup may break this?)
   */
   ...
   * For example, certain callers to delete_tree() already performed some of the checks
   * above, so can disable the corresponding flags and put the results here.
   */
...
/**
 * Output from tombstone_version() with additional fields on top of PreDeleteChecks.
 * Safe to be sliced to the latter.
 */
...
/**
 * Output from [batch_]get_latest_undeleted_version_and_next_version_id. Contains info
 * needed for writing operations that depend on a previous live version existing:
...
struct UpdateInfo {
  ...
  /**
   * The smallest version ID not in use (live or deleted). Zero iff no versions exist
