...
SymbolListData::SymbolListData(std::shared_ptr<VersionMap> version_map,
                               StreamId type_indicator, uint32_t seed)
    : type_holder_(std::move(type_indicator)), seed_(seed),
...
struct LoadResult {
  ...
  std::vector<AtomKey>&& detach_symbol_list_keys() {
    return std::move(symbol_list_keys_);
  }
};

auto warning_threshold() {
  return 2 * static_cast<size_t>(ConfigsMap::instance()
                                     ->get_int("SymbolList.MaxDelta")
                                     .value_or(ConfigsMap::instance()->get_int(
                                         "SymbolList.MaxCompactionThreshold", 700)));
}

bool is_new_style_key(const AtomKey& key) {
  return util::variant_match(
      key.end_index(), [](std::string_view str) { return str == version_string; },
      [](NumericIndex n) { return n == version_identifier; });
}

std::vector<SymbolListEntry>
load_previous_from_version_keys(const std::shared_ptr<Store>& store,
                                SymbolListData& data) {
  ...
  store->iterate_type(KeyType::VERSION_REF, [&data, &stream_ids](const auto& key) {
    ...
    if (stream_ids.size() == warning_threshold() && !data.warned_expected_slowdown_) {
      log::symbol().warn(
          ...
          "https://docs.arcticdb.io/technical/on_disk_storage/#symbol-list-caching\n\n"
          "To resolve, run `list_symbols` through to completion to compact the symbol "
          "list cache.\n"
  ...
  auto res = folly::collect(batch_get_latest_undeleted_and_latest_versions_async(
                                store, data.version_map_, stream_ids))
                 .get();
...
std::vector<AtomKey>
get_all_symbol_list_keys(const std::shared_ptr<StreamSource>& store,
                         SymbolListData& data) {
  std::vector<AtomKey> output;
  uint64_t uncompacted_keys_found = 0;
  store->iterate_type(KeyType::SYMBOL_LIST, [&data, &output,
                                             &uncompacted_keys_found](auto&& key) {
    auto atom_key = to_atom(std::forward<decltype(key)>(key));
    if (atom_key.id() != compaction_id) {
      uncompacted_keys_found++;
    }
    if (uncompacted_keys_found == warning_threshold() &&
        !data.warned_expected_slowdown_) {
      log::symbol().warn(
          "`list_symbols` may take longer than expected as there have been many "
          "modifications since `list_symbols` was last called. \n\n"
          "See here for more information: "
          "https://docs.arcticdb.io/technical/on_disk_storage/#symbol-list-caching\n\n"
  ...
  std::sort(output.begin(), output.end(),
            [](const AtomKey& left, const AtomKey& right) {
              // Some very old symbol list keys have a non-zero version number, but with
              // different semantics to the new style, so ignore it. See
              // arcticdb-man#116. Most old style symbol list keys have version ID 0
              // anyway.
...
MaybeCompaction last_compaction(const std::vector<AtomKey>& keys) {
  auto pos = std::find_if(keys.rbegin(), keys.rend(),
                          [](const auto& key) { return key.id() == compaction_id; });

  if (pos == keys.rend()) {
    return std::nullopt;
  } else {
    return {(pos + 1)
                .base()}; // reverse_iterator -> forward itr has an offset of 1 per docs
  }
}

// The below string_at and scalar_at functions should be used for symbol list cache
// segments instead of the ones provided in SegmentInMemory, because the symbol list
// structure is the only place where columns can have more entries than the segment has
// rows. Hence, we need to bypass the checks inside SegmentInMemory's function and
// directly call the Column's string_at and scalar_at.
std::string string_at(const SegmentInMemory& seg, position_t row, position_t col) {
  ...
  util::check(offset.has_value(),
              "Symbol list trying to call string_at for missing row {}, column {}", row,
              col);
...
T scalar_at(const SegmentInMemory& seg, position_t row, position_t col) {
  ...
  util::check(scalar.has_value(),
              "Symbol list trying to call scalar_at for missing row {}, column {}", row,
              col);
  return scalar.value();
}

StreamId stream_id_from_segment(DataType data_type, const SegmentInMemory& seg,
                                position_t row_id, position_t column) {
...
std::vector<SymbolListEntry>
read_old_style_list_from_storage(const SegmentInMemory& seg) {
  ...
  for (auto row : seg)
    output.emplace_back(stream_id_from_segment(data_type, seg, row.row_id_, 0), 0, 0,
                        ActionType::ADD);
...
std::vector<SymbolListEntry>
read_new_style_list_from_storage(const SegmentInMemory& seg) {
  std::vector<SymbolListEntry> output;
  if (seg.empty())
    return output;
  ...
  // Because we need to be backwards compatible with the old style symbol list, the
  // additions and deletions are in separate columns. The first three columns are the
  // symbol, version and timestamp for the additions, and the next three are the same
  // for the deletions. Old-style symbol lists will ignore everything but the first
  // column which will mean that they can't do any conflict resolution but will get the
  // correct data.
  util::check(seg.column(0).row_count() == seg.column(1).row_count() &&
  ...
  for (auto i = 0L; i < seg.column(0).row_count(); ++i) {
    ...
    ARCTICDB_RUNTIME_DEBUG(log::symbol(), "Reading added symbol {}: {}@{}", stream_id,
                           reference_id, reference_time);
    output.emplace_back(stream_id, reference_id, reference_time, ActionType::ADD);
  }

  if (seg.descriptor().field_count() == 6) {
    util::check(seg.column(3).row_count() == seg.column(4).row_count() &&
    ...
    for (auto i = 0L; i < seg.column(3).row_count(); ++i) {
      ...
      ARCTICDB_RUNTIME_DEBUG(log::symbol(), "Reading deleted symbol {}: {}@{}",
                             stream_id, reference_id, reference_time);
...
std::vector<SymbolListEntry>
read_from_storage(const std::shared_ptr<StreamSource>& store, const AtomKey& key) {
...
MapType load_journal_keys(const std::vector<AtomKey>& keys) {
  ...
  for (const auto& key : keys) {
    ...
    const auto version_id =
        is_new_style_key(key) ? key.version_id() : unknown_version_id;
    const auto timestamp = key.creation_ts();
    ActionType action = std::get<StringId>(action_id) == DeleteSymbol
                            ? ActionType::DELETE
                            : ActionType::ADD;
...
std::optional<SymbolEntryData>
timestamps_too_close(const std::vector<SymbolEntryData>::const_reverse_iterator& first,
...
bool contains_unknown_reference_ids(const std::vector<SymbolEntryData>& updated) {
  return std::any_of(std::begin(updated), std::end(updated),
                     [](const auto& data) { return has_unknown_reference_id(data); });
}
...
SymbolVectorResult vector_okay(bool all_same_version, bool all_same_action,
                               size_t latest_id_count) {
  return {ProblematicResult{false}, all_same_version, all_same_action, latest_id_count};
}

SymbolVectorResult is_problematic_vector(const std::vector<SymbolEntryData>& updated,
  ...
  if (auto timestamp_problem =
          timestamps_too_close(start, updated, min_allowed_interval, all_same_action);
      timestamp_problem)
...
ProblematicResult is_problematic(const std::vector<SymbolEntryData>& updated,
                                 timestamp min_allowed_interval) {
  return is_problematic_vector(updated, min_allowed_interval).problematic_result_;
}

ProblematicResult is_problematic(const SymbolListEntry& existing,
                                 const std::vector<SymbolEntryData>& updated,
                                 timestamp min_allowed_interval) {
  ARCTICDB_DEBUG(log::symbol(), "{} {} {}", existing.stream_id_,
                 static_cast<const SymbolEntryData&>(existing), updated);

  const auto& latest = *std::crbegin(updated);
  if (existing.reference_id_ > latest.reference_id_)
    return ProblematicResult{existing};

  auto [problematic_result, vector_all_same_version, vector_all_same_action,
        last_id_count] = is_problematic_vector(updated, min_allowed_interval);
  if (problematic_result)
    return problematic_result;

  if (problematic_result.contains_unknown_reference_ids_ ||
      has_unknown_reference_id(existing))
    return cannot_determine_validity();

  const bool all_same_action =
      vector_all_same_action && existing.action_ == latest.action_;

  if (latest.timestamp_ - existing.timestamp_ < min_allowed_interval &&
      !all_same_action)
    return ProblematicResult{latest.reference_id_ > existing.reference_id_ ? latest
                                                                           : existing};
...
CollectionType merge_existing_with_journal_keys(
  ...
  const auto min_allowed_interval =
      ConfigsMap::instance()->get_int("SymbolList.MinIntervalNs", 100'000'000LL);
  ...
  for (auto& previous_entry : existing_keys) {
    ...
    if (updated == std::end(update_map)) {
      if (previous_entry.action_ == ActionType::ADD)
        ...
        util::check(previous_entry.action_ == ActionType::DELETE,
                    "Unknown action type {} in symbol list",
                    static_cast<uint8_t>(previous_entry.action_));
    } else {
      util::check(!updated->second.empty(), "Unexpected empty entry for symbol {}",
                  updated->first);
      if (auto problematic_entry =
              is_problematic(previous_entry, updated->second, min_allowed_interval);
          problematic_entry) {
        ...
        symbols.emplace_back(updated->first, last_entry->reference_id_,
                             last_entry->timestamp_, last_entry->action_);
      }
  ...
  for (const auto& [symbol, entries] : update_map) {
    ...
    if (auto problematic_entry = is_problematic(entries, min_allowed_interval);
        problematic_entry) {
      problematic_symbols.try_emplace(symbol, problematic_entry.reference_id(),
                                      problematic_entry.time());
    } else {
      const auto& last_entry = entries.rbegin();
      symbols.emplace_back(symbol, last_entry->reference_id_, last_entry->timestamp_,
                           last_entry->action_);
    }
  }
  ...
  if (!problematic_symbols.empty()) {
    ...
    auto versions =
        batch_check_latest_id_and_status(store, version_map, symbol_versions);
    ...
    for (const auto& [symbol, reference_pair] : problematic_symbols) {
      ...
      if (auto version = versions->find(symbol); version != versions->end()) {
        ...
        if (symbol_state.exists_) {
          ARCTICDB_DEBUG(log::symbol(),
                         "Problematic symbol/version pair: {}@{}: exists at id {}",
                         symbol, reference_id, symbol_state.version_id_);
          symbols.emplace_back(symbol, symbol_state.version_id_,
                               symbol_state.timestamp_, ActionType::ADD);
        } else {
          symbols.emplace_back(symbol, symbol_state.version_id_,
                               symbol_state.timestamp_, ActionType::DELETE);
          ARCTICDB_DEBUG(log::symbol(),
        ...
        ARCTICDB_DEBUG(log::symbol(),
                       "Problematic symbol/version pair: {}@{}: cannot be found",
                       symbol, reference_id);
        symbols.emplace_back(symbol, reference_id, reference_pair.second,
                             ActionType::DELETE);
      }
    }
    std::sort(std::begin(symbols), std::end(symbols),
              [](const auto& l, const auto& r) { return l.stream_id_ < r.stream_id_; });
  }
...
CollectionType load_from_symbol_list_keys(
  ...
  return merge_existing_with_journal_keys(version_map, store, keys,
                                          std::move(previous_compaction));
}

CollectionType load_from_version_keys(const std::shared_ptr<VersionMap>& version_map,
  ...
  return merge_existing_with_journal_keys(version_map, store, keys,
                                          std::move(previous_entries));
}

LoadResult attempt_load(const std::shared_ptr<VersionMap>& version_map,
  ...
  load_result.maybe_previous_compaction =
      last_compaction(load_result.symbol_list_keys_);

  if (load_result.maybe_previous_compaction)
    load_result.symbols_ =
        load_from_symbol_list_keys(version_map, store, load_result.symbol_list_keys_,
                                   *load_result.maybe_previous_compaction);
  else {
    load_result.symbols_ =
        load_from_version_keys(version_map, store, load_result.symbol_list_keys_, data);
    ...
    for (const auto& key : load_result.symbol_list_keys_)
      util::check(keys_in_versions.find(StreamId{std::get<StringIndex>(
                      key.start_index())}) != keys_in_versions.end(),
...
inline StreamDescriptor journal_stream_descriptor(ActionType action,
                                                  const StreamId& id) {
  return util::variant_match(
      id,
      [action](const NumericId&) {
        return StreamDescriptor{
            stream_descriptor(action_id(action), RowCountIndex(),
                              {scalar_field(DataType::UINT64, "symbol")})};
      },
...
void write_journal(const std::shared_ptr<Store>& store, const StreamId& symbol,
                   ActionType action, VersionId reference_id) {
  ...
  seg.end_row();
  store->write_sync(KeyType::SYMBOL_LIST, reference_id, action_id(action),
                    IndexValue{symbol}, version_indicator, std::move(seg));
}
...
void SymbolList::add_symbol(const std::shared_ptr<Store>& store, const StreamId& symbol,
                            VersionId reference_id) {
  write_symbol(store, symbol, reference_id);
}

void SymbolList::remove_symbol(const std::shared_ptr<Store>& store,
                               const StreamId& symbol, VersionId reference_id) {
...
StreamDescriptor add_symbol_stream_descriptor(const StreamId& stream_id,
                                              const StreamId& type_holder) {
  auto data_type = std::holds_alternative<StringId>(type_holder)
                       ? DataType::ASCII_DYNAMIC64
                       : DataType::UINT64;
  return stream_descriptor(
      stream_id, RowCountIndex(),
...
StreamDescriptor delete_symbol_stream_descriptor(const StreamId& stream_id,
                                                 const StreamId& type_holder) {
  auto data_type = std::holds_alternative<StringId>(type_holder)
                       ? DataType::ASCII_DYNAMIC64
                       : DataType::UINT64;
...
bool SymbolList::needs_compaction(const LoadResult& load_result) const {
  if (!load_result.maybe_previous_compaction) {
    log::version().debug(
        "Symbol list: needs_compaction=[true] as no previous compaction");
    return true;
  }
  ...
  if (auto fixed = ConfigsMap::instance()->get_int("SymbolList.MaxDelta")) {
    ...
    log::version().debug("Symbol list: Fixed draw for compaction. "
                         "needs_compaction=[{}] n_keys=[{}], MaxDelta=[{}]",
                         result, n_keys, fixed.value());
    return result;
  }

  int64_t min =
      ConfigsMap::instance()->get_int("SymbolList.MinCompactionThreshold", 300);
  int64_t max =
      ConfigsMap::instance()->get_int("SymbolList.MaxCompactionThreshold", 700);
  util::check(max >= min,
              "Bad configuration, min compaction threshold=[{}] > max compaction "
              "threshold=[{}]",
              min, max);
  ...
  log::version().debug("Symbol list: Random draw for compaction. needs_compaction=[{}] "
                       "n_keys=[{}], draw=[{}]",
                       result, n_keys, draw);
  return result;
}

void write_symbol_at(const StreamId& type_holder, SegmentInMemory& list_segment,
                     const SymbolListEntry& entry, position_t column) {
  util::variant_match(
      type_holder,
      [&entry, &list_segment, column](const StringId&) {
        util::check(std::holds_alternative<StringId>(entry.stream_id_),
      ...
      [&entry, &list_segment, column](const NumericId&) {
        util::check(std::holds_alternative<NumericId>(entry.stream_id_),
...
void write_entry(const StreamId& type_holder, SegmentInMemory& segment,
...
SegmentInMemory write_entries_to_symbol_segment(const StreamId& stream_id,
                                                const StreamId& type_holder,
                                                const CollectionType& symbols) {
  SegmentInMemory added_segment{add_symbol_stream_descriptor(stream_id, type_holder)};
  SegmentInMemory deleted_segment{
      delete_symbol_stream_descriptor(stream_id, type_holder)};
  ...
  if (!deleted_segment.empty()) {
    for (auto col = 0UL; col < deleted_segment.descriptor().fields().size(); ++col) {
      ...
      added_segment.add_column(
          FieldRef{field.type(), field.name()},
          deleted_segment.column_ptr(static_cast<position_t>(col)));
    }
    util::check(added_segment.descriptor().field_count() == 6,
    ...
    merge_string_column(src, deleted_segment.string_pool_ptr(),
                        added_segment.string_pool_ptr(), cursor, false);
    std::swap(src, cursor.buffer());
  }

  util::check(added_segment.row_count() ==
                  static_cast<size_t>(added_segment.column(0).row_count()),
...
VariantKey write_symbols(const std::shared_ptr<Store>& store,
                         const CollectionType& symbols, const StreamId& stream_id,
                         const StreamId& type_holder) {
  ARCTICDB_RUNTIME_DEBUG(log::symbol(), "Writing {} symbols to symbol list cache",
                         symbols.size());
  ...
  ARCTICDB_RUNTIME_DEBUG(log::symbol(),
                         "Writing symbol segment with stream id {} and {} rows",
                         stream_id, segment.row_count());
  return store->write_sync(KeyType::SYMBOL_LIST, 0, stream_id, NumericIndex{0},
                           NumericIndex{0}, std::move(segment));
}

std::vector<Store::RemoveKeyResultType> delete_keys(const std::shared_ptr<Store>& store,
  ...
  for (auto& atom_key : to_remove) {
    // Corner case: if the newly written Compaction key (exclude) has the same timestamp
    // as an existing one (e.g. when a previous compaction round failed in the deletion
    // step), we don't want to delete the former
    if (atom_key != exclude)
...
bool has_recent_compaction(const std::shared_ptr<Store>& store,
                           const std::optional<std::vector<AtomKey>::const_iterator>&
                               maybe_previous_compaction) {
  ...
  if (maybe_previous_compaction.has_value()) {
    ...
    store->iterate_type(
        KeyType::SYMBOL_LIST,
        [&found_last, &has_newer,
         &last_compaction_key =
             *maybe_previous_compaction.value()](const VariantKey& key) {
    ...
    // Version keys source
    store->iterate_type(
        KeyType::SYMBOL_LIST, [&has_newer](const VariantKey&) { has_newer = true; },
        std::get<std::string>(compaction_id));
  }
...
std::set<StreamId> SymbolList::load(const std::shared_ptr<VersionMap>& version_map,
                                    const std::shared_ptr<Store>& store,
                                    bool no_compaction) {
  LoadResult load_result = ExponentialBackoff<StorageException>(100, 2000).go(
      [this, &version_map, &store]() {
  ...
  if (!no_compaction && needs_compaction(load_result)) {
    ...
    try {
      if (StorageLock lock{StringId{CompactionLockName}}; lock.try_lock(store)) {
        ...
        ARCTICDB_RUNTIME_DEBUG(log::symbol(),
                               "Checking whether we still need to compact under lock");
        compact_internal(store, load_result);
      } else {
        ARCTICDB_RUNTIME_DEBUG(log::symbol(),
                               "Not compacting the symbol list due to lock contention");
      }
      ...
      ARCTICDB_RUNTIME_DEBUG(log::symbol(),
                             "Not compacting the symbol list due to lack of permission",
                             ex.what());
    } catch (const std::exception& ex) {
      log::symbol().warn("Ignoring error while trying to compact the symbol list: {}",
                         ex.what());
    }
  }
...
void SymbolList::compact_internal(const std::shared_ptr<Store>& store,
                                  LoadResult& load_result) const {
  if (!has_recent_compaction(store, load_result.maybe_previous_compaction)) {
    auto written =
        write_symbols(store, load_result.symbols_, compaction_id, data_.type_holder_);
    delete_keys(store, load_result.detach_symbol_list_keys(),
                std::get<AtomKey>(written));
  }
}
