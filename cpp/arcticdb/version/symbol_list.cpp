...
auto warning_threshold() {
    return 2 *
           static_cast<size_t>(ConfigsMap::instance()
    ...
    return util::variant_match(
        ...
        [action](const NumericId&) {
            return StreamDescriptor{
                stream_descriptor(action_id(action), RowCountIndex(), {scalar_field(DataType::UINT64, "symbol")})
            };
        },
        [action](const StringId&) {
            return StreamDescriptor{
                stream_descriptor(action_id(action), RowCountIndex(), {scalar_field(DataType::UTF_DYNAMIC64, "symbol")})
            };
        }
    );
}
...
bool SymbolList::needs_compaction(const LoadResult& load_result) const {
    ...
    log::version().debug(
        "Symbol list: Random draw for compaction. needs_compaction=[{}] n_keys=[{}], draw=[{}]", result, n_keys, draw
    );
    return result;
}
