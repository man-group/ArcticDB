...
struct VersionMapEntry {
    ...
    bool has_individual_tombstone(VersionId version_id) const { return tombstones_.count(version_id) != 0; }
    ...
    void check_ordering() const {
        ...
        for (const auto& key : keys_) {
            ...
            if (is_index_key_type(key.type())) {
                util::check(
                    key.version_id() <= version_id, "Out of order version ids: {} > {}", key.version_id(), version_id
                );
