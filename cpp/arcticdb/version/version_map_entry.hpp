...
/*
 * version_map_entry contains the class VersionMapEntry which encapsulates the data
 * contained for each stream_id in the version_map. (See VersionMapEntry)
 */
#pragma once
...
// The LoadStrategy describes how to load versions from the version chain. It consists
// of: load_type: Describes up to which point in the chain we need to go.
...
struct LoadStrategy {
  ...
  LoadStrategy(LoadType load_type, LoadObjective load_objective,
               int64_t load_from_time_or_until)
      : load_type_(load_type), load_objective_(load_objective) {
    switch (load_type_) {
    ...
    default:
      internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
          "LoadStrategy constructor with load_from_time_or_until parameter {} provided "
          "invalid load_type {}",
  ...
  void validate() const {
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        ...
        "Invalid load parameter: load_type {} with load_util {}", int(load_type_),
        load_until_version_.value_or(VersionId{}));
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        (load_type_ == LoadType::FROM_TIME) == load_from_time_.has_value(),
        "Invalid load parameter: load_type {} with load_from_time_ {}", int(load_type_),
        load_from_time_.value_or(timestamp{}));
  }
};

inline bool is_undeleted_strategy_subset(const LoadStrategy& left,
                                         const LoadStrategy& right) {
  switch (left.load_type_) {
  case LoadType::NOT_LOADED:
    return true;
  case LoadType::LATEST:
    // LATEST is not a subset of DOWNTO because DOWNTO may not reach the latest
    // undeleted version.
    return right.load_type_ != LoadType::NOT_LOADED &&
           right.load_type_ != LoadType::DOWNTO;
  case LoadType::DOWNTO:
    if (right.load_type_ == LoadType::ALL) {
      return true;
    }
    if (right.load_type_ == LoadType::DOWNTO &&
        ((left.load_until_version_.value() >= 0) ==
         (right.load_until_version_.value() >= 0))) {
      // Left is subset of right only when the [load_until]s have same sign and left's
      // version is >= right's version
...
// Returns a strategy which is guaranteed to load all versions requested by left and
// right. Works only on strategies with include_deleted=false.
inline LoadStrategy union_of_undeleted_strategies(const LoadStrategy& left,
                                                  const LoadStrategy& right) {
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      !left.should_include_deleted(), "Trying to produce a union of undeleted "
                                      "strategies but left strategy includes deleted.");
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      !right.should_include_deleted(),
      "Trying to produce a union of undeleted strategies but right strategy includes "
      "deleted.");
  ...
  // If none is subset of the other, then we should load all versions. We can't be less
  // conservative because we can't know where to load to with strategies which have a
  // different load type. E.g. for FROM_TIME and DOWNTO we can't know where to read to
  // unless we know the version chain. A possible workaround for this is to restructure
  // loading the version chain to get a set of LoadStrategies and stop searching only
  // when all of them are satisfied.
  return LoadStrategy{LoadType::ALL, LoadObjective::UNDELETED_ONLY};
}

template <typename T> bool deque_is_unique(std::deque<T> vec) {
...
inline void check_is_index_or_tombstone(const AtomKey& key) {
  util::check(is_index_or_tombstone(key),
              "Expected index or tombstone key type but got {}", key);
}

inline AtomKey index_to_tombstone(const AtomKey& index_key, const StreamId& stream_id,
...
inline AtomKey index_to_tombstone(VersionId version_id, const StreamId& stream_id,
                                  timestamp creation_ts) {
  return atom_key_builder()
...
struct LoadProgress {
  ...
  VersionId oldest_loaded_undeleted_index_version_ =
      std::numeric_limits<VersionId>::max();
  timestamp earliest_loaded_timestamp_ = std::numeric_limits<timestamp>::max();
  timestamp earliest_loaded_undeleted_timestamp_ =
      std::numeric_limits<timestamp>::max();
};

struct VersionMapEntry {
  /*
    VersionMapEntry is all the data we have in-memory about each stream_id in the
    version map which in its essence is a map of StreamId: VersionMapEntry. It's created
    from the linked-list-like structure that we have in the storage, where the head_
    points to the latest version and keys_ are basically all the index/version keys
    loaded in memory in a deque - based on the load_strategy.

    load_strategy signifies the current state of the in memory structure vs the state on
    disk, where LATEST will just load the latest version, and ALL loads everything in
    memory by going through the linked list on disk.

    It also contains a map of version_ids and the tombstone key corresponding to it iff
    it has been pruned or explicitly deleted.
   */
  ...
  void sort() {
    ...
    // Sorting by creation_ts is safe from clock skew because we don't support parallel
    // writes to the same symbol.
  ...
  // Below four functions used to return optional<AtomKey> of the tombstone, but copying
  // keys is expensive and only one function was actually interested in the key, so they
  // now return bool. See get_tombstone().
  ...
  bool is_tombstoned(const AtomKey& key) const {
    return is_tombstoned_via_tombstone_all(key.version_id()) ||
           has_individual_tombstone(key.version_id());
  }

  bool is_tombstoned(VersionId version_id) const {
    return is_tombstoned_via_tombstone_all(version_id) ||
           has_individual_tombstone(version_id);
  }
  ...
  std::optional<AtomKey> get_tombstone(VersionId version_id) {
    ...
    return it != tombstones_.end() ? std::make_optional<AtomKey>(it->second)
                                   : std::nullopt;
  }
  ...
  std::string dump() const {
    ...
    for (const auto& tombstone : tombstones_)
      strm << fmt::format("    {} - {}", tombstone.first, tombstone.second)
           << std::endl;
  ...
  void check_ordering() const {
    ...
    auto first_index =
        std::find_if(std::begin(keys_), std::end(keys_),
    ...
    for (const auto& key : keys_) {
      if (key.type() == KeyType::VERSION) {
        if (!version_timestamp)
          ...
          util::check(key.creation_ts() <= *version_timestamp,
                      "out of order timestamp: {} > {}", key.creation_ts(),
                      version_timestamp.value());
        }
      }
      if (is_index_key_type(key.type())) {
  ...
  void check_stream_id() const {
    ...
    util::check_rte(id_to_version_id.size() == 1, "Multiple symbols in keys: {}",
                    fmt::format("{}", id_to_version_id));
  }

  void try_set_tombstone_all(const AtomKey& key) {
    util::check(key.type() == KeyType::TOMBSTONE_ALL,
                "Can't set tombstone all key with key {}", key);
...
inline bool is_live_index_type_key(const AtomKeyImpl& key,
                                   const std::shared_ptr<VersionMapEntry>& entry) {
  return is_index_key_type(key.type()) && !entry->is_tombstoned(key);
}

inline std::optional<VersionId>
get_prev_version_in_entry(const std::shared_ptr<VersionMapEntry>& entry,
                          VersionId version_id) {
  // sorted in decreasing order
  // entry->keys_ is not sorted in version_id anymore (due to tombstones), we only need
  // to fetch live index keys which will be sorted on version_id
  auto index_keys = entry->get_indexes(false);

  if (auto iterator_lt = std::upper_bound(
          std::begin(index_keys), std::end(index_keys), version_id,
...
inline std::optional<VersionId>
get_next_version_in_entry(const std::shared_ptr<VersionMapEntry>& entry,
                          VersionId version_id) {
  // sorted in decreasing order
  // entry->keys_ is not sorted in version_id any more (due to tombstones), we only need
  // to fetch live index keys which will be sorted on version_id
  auto index_keys = entry->get_indexes(false);
...
inline VersionDetails find_index_key_for_version_id_and_tombstone_status(
  ...
  auto key = std::find_if(
      std::begin(entry->keys_), std::end(entry->keys_), [version_id](const auto& key) {
  ...
  return VersionDetails{*key, entry->is_tombstoned(*key) ? VersionStatus::TOMBSTONED
                                                         : VersionStatus::LIVE};
}

inline std::optional<AtomKey>
find_index_key_for_version_id(VersionId version_id,
                              const std::shared_ptr<VersionMapEntry>& entry,
                              bool included_deleted = true) {
  auto version_details =
      find_index_key_for_version_id_and_tombstone_status(version_id, entry);
  if ((version_details.version_status_ == VersionStatus::TOMBSTONED &&
       included_deleted) ||
...
inline std::optional<std::pair<AtomKey, AtomKey>>
get_latest_key_pair(const std::shared_ptr<VersionMapEntry>& entry) {
...
namespace fmt {
template <> struct formatter<arcticdb::LoadType> {
...
template <> struct formatter<arcticdb::LoadObjective> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
