...
class PythonVersionStore : public LocalVersionedEngine {
  ...
  public:
    ...
    ReadResult read_column_stats_version(const StreamId& stream_id, const VersionQuery& version_query);
...
inline std::vector<std::variant<ReadResult, DataError>> frame_to_read_result(
    std::vector<ReadVersionOutput>&& keys_frame_and_descriptors
) {
    ...
    for (auto& read_version_output : keys_frame_and_descriptors) {
        ...
        read_results.emplace_back(ReadResult(
            ...
            PythonOutputFrame{
                read_version_output.frame_and_descriptor_.frame_, read_version_output.frame_and_descriptor_.buffers_
            },
