...
/**
 * PythonVersionStore contains all the Python cruft that isn't portable, as well as
 * non-essential features that are part of the backwards-compatibility with Arctic
 * Python but that we think are random/a bad idea and aren't part of the main product.
 */
...
public:
  ...
  VersionedItem write_versioned_composite_data(
      ...
      const std::vector<py::tuple>& items, const std::vector<py::object>& norm_metas,
      const py::object& user_meta, bool prune_previous_versions);

  VersionedItem
  write_partitioned_dataframe(const StreamId& stream_id, const py::tuple& item,
                              const py::object& norm_meta,
                              const std::vector<std::string>& partition_cols);

  ReadResult read_partitioned_dataframe(const StreamId& stream_id,
                                        const ReadQuery& query,
                                        const ReadOptions& read_options);

  VersionedItem append(const StreamId& stream_id, const py::tuple& item,
                       const py::object& norm, const py::object& user_meta, bool upsert,
                       bool prune_previous_versions, bool validate_index);

  VersionedItem update(const StreamId& stream_id, const UpdateQuery& query,
                       const py::tuple& item, const py::object& norm,
                       const py::object& user_meta, bool upsert, bool dynamic_schema,
                       bool prune_previous_versions);

  VersionedItem delete_range(const StreamId& stream_id, const UpdateQuery& query,
                             bool dynamic_schema, bool prune_previous_versions);

  void append_incomplete(const StreamId& stream_id, const py::tuple& item,
                         const py::object& norm, const py::object& user_meta,
                         bool validate_index) const;

  VersionedItem
  compact_incomplete(const StreamId& stream_id, bool append, bool convert_int_to_float,
  ...
  void write_parallel(const StreamId& stream_id, const py::tuple& item,
                      const py::object& norm, const py::object& user_meta,
                      bool validate_index) const;

  VersionedItem write_metadata(const StreamId& stream_id, const py::object& user_meta,
                               bool prune_previous_versions);

  void create_column_stats_version(const StreamId& stream_id, ColumnStats& column_stats,
                                   const VersionQuery& version_query);

  void drop_column_stats_version(const StreamId& stream_id,
                                 const std::optional<ColumnStats>& column_stats_to_drop,
                                 const VersionQuery& version_query);

  ReadResult read_column_stats_version(const StreamId& stream_id,
                                       const VersionQuery& version_query);

  ColumnStats get_column_stats_info_version(const StreamId& stream_id,
                                            const VersionQuery& version_query);
  ...
  VersionedItem sort_merge(const StreamId& stream_id, const py::object& user_meta,
                           bool append, bool convert_int_to_float, bool via_iteration,
                           bool sparsify, bool prune_previous_versions);
  ...
  std::pair<VersionedItem, py::object> read_metadata(const StreamId& stream_id,
                                                     const VersionQuery& version_query);

  std::vector<std::variant<DescriptorItem, DataError>>
  batch_read_descriptor(const std::vector<StreamId>& stream_ids,
                        const std::vector<VersionQuery>& version_queries,
                        const ReadOptions& read_options);

  DescriptorItem read_descriptor(const StreamId& stream_id,
                                 const VersionQuery& version_query);
  ...
  get_update_times(const std::vector<StreamId>& stream_ids,
                   const std::vector<VersionQuery>& version_queries);

  timestamp get_update_time(const StreamId& stream_id,
                            const VersionQuery& version_query);
  ...
  void snapshot(const SnapshotId& snap_name, const py::object& user_meta,
  ...
  list_snapshots(const std::optional<bool> load_metadata);
  ...
  std::vector<std::tuple<StreamId, VersionId, timestamp, std::vector<SnapshotId>, bool>>
  list_versions(const std::optional<StreamId>& stream_id,
  ...
  batch_write(const std::vector<StreamId>& stream_ids,
              const std::vector<py::tuple>& items, const std::vector<py::object>& norms,
              const std::vector<py::object>& user_metas, bool prune_previous_versions,
              bool validate_index, bool throw_on_error);

  std::vector<std::variant<VersionedItem, DataError>>
  batch_write_metadata(const std::vector<StreamId>& stream_ids,
                       const std::vector<py::object>& user_meta,
                       bool prune_previous_versions, bool throw_on_error);

  std::vector<std::variant<VersionedItem, DataError>>
  batch_append(const std::vector<StreamId>& stream_ids,
  ...
  batch_restore_version(const std::vector<StreamId>& id,
                        const std::vector<VersionQuery>& version_query);

  std::vector<std::variant<ReadResult, DataError>>
  batch_read(const std::vector<StreamId>& stream_ids,
             const std::vector<VersionQuery>& version_queries,
             std::vector<std::shared_ptr<ReadQuery>>& read_queries,
             const ReadOptions& read_options);

  std::vector<std::variant<std::pair<VersionedItem, py::object>, DataError>>
  batch_read_metadata(const std::vector<StreamId>& stream_ids,
                      const std::vector<VersionQuery>& version_queries,
                      const ReadOptions& read_options);

  std::set<StreamId>
  list_streams(const std::optional<SnapshotId>& snap_name = std::nullopt,
  ...
  get_all_tombstoned_versions(const StreamId& stream_id);
...
inline std::vector<std::variant<ReadResult, DataError>>
frame_to_read_result(std::vector<ReadVersionOutput>&& keys_frame_and_descriptors) {
  ...
  for (auto& read_version_output : keys_frame_and_descriptors) {
    ...
    read_results.emplace_back(ReadResult(
        ...
        PythonOutputFrame{read_version_output.frame_and_descriptor_.frame_,
                          read_version_output.frame_and_descriptor_.buffers_},
        desc_proto.normalization(), desc_proto.user_meta(), desc_proto.multi_key_meta(),
        std::vector<AtomKey>{}));
  }
  return read_results;
}
