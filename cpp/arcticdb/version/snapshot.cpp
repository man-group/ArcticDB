...
void write_snapshot_entry(std::shared_ptr<StreamSink> store, std::vector<AtomKey>& keys,
                          const SnapshotId& snapshot_id, const py::object& user_meta,
                          bool log_changes, KeyType key_type) {
  ...
  IndexAggregator<RowCountIndex> snapshot_agg(
      snapshot_id, [&store, key_type, &snapshot_id](SegmentInMemory&& segment) {
        store->write(key_type, snapshot_id, std::move(segment)).get();
      });

  ARCTICDB_DEBUG(log::snapshot(), "Constructing snapshot {}", snapshot_id);
  // Most of the searches in snapshot are for a given symbol, this helps us do a binary
  // search on the segment on read time.
  std::sort(keys.begin(), keys.end(),
            [](const AtomKey& l, const AtomKey& r) { return l.id() < r.id(); });
  ...
  if (!user_meta.is_none()) {
    ...
    python_util::pb_from_python(
        user_meta, *timeseries_descriptor.mutable_proto().mutable_user_meta());
...
void tombstone_snapshot(const std::shared_ptr<StreamSink>& store, const RefKey& key,
                        SegmentInMemory&& segment_in_memory, bool log_changes) {
  store->remove_key_sync(key); // Make the snapshot "disappear" to normal APIs
  if (log_changes) {
    log_delete_snapshot(store, key.id());
  }
  // Append a timestamp to the ID so that other snapshot(s) can reuse the same snapshot
  // name before the cleanup job:
  std::string new_key = fmt::format(
      "{}@{:x}", key, util::SysClock::coarse_nanos_since_epoch() / 1'000'000);
  store->write(KeyType::SNAPSHOT_TOMBSTONE, new_key, std::move(segment_in_memory))
      .get();
}

void tombstone_snapshot(const std::shared_ptr<StreamSink>& store,
                        storage::KeySegmentPair&& key_segment_pair, bool log_changes) {
  store->remove_key(key_segment_pair.ref_key())
      .get(); // Make the snapshot "disappear" to normal APIs
  if (log_changes) {
  ...
  // name before the cleanup job:
  std::string new_key =
      fmt::format("{}@{:x}", key_segment_pair.ref_key(),
                  util::SysClock::coarse_nanos_since_epoch() / 1'000'000);
...
void iterate_snapshots(const std::shared_ptr<Store>& store,
  ...
  store->iterate_type(
      KeyType::SNAPSHOT_REF, [&snap_variant_keys, &seen](VariantKey&& vk) {
  ...
  for (auto& vk : snap_variant_keys) {
    try {
      ...
      ARCTICDB_DEBUG(
          log::version(),
          "Ignored exception due to {} being deleted during iterate_snapshots().");
    }
  }
}

std::optional<size_t> row_id_for_stream_in_snapshot_segment(SegmentInMemory& seg,
                                                            bool using_ref_key,
                                                            const StreamId& stream_id) {
  if (using_ref_key) {
    // With ref keys we are sure the snapshot segment has the index atom keys sorted by
    // stream_id.
    auto lb = std::lower_bound(
        std::begin(seg), std::end(seg), stream_id, [&](auto& row, StreamId t) {
          auto row_stream_id =
              stream_id_from_segment<pipelines::index::Fields>(seg, row.row_id_);
          return row_stream_id < t;
        });

    if (lb == std::end(seg) || stream_id_from_segment<pipelines::index::Fields>(
                                   seg, lb->row_id_) != stream_id) {
      return std::nullopt;
    }
...
std::unordered_set<entity::AtomKey>
get_index_keys_in_snapshots(const std::shared_ptr<Store>& store,
                            const StreamId& stream_id) {
  ARCTICDB_SAMPLE(GetIndexKeysInSnapshot, 0)

  std::unordered_set<entity::AtomKey> index_keys_in_snapshots{};

  iterate_snapshots(store, [&index_keys_in_snapshots, &store,
                            &stream_id](const VariantKey& vk) {
    ...
    if (opt_idx_for_stream_id) {
      ARCTICDB_DEBUG(log::snapshot(), "Found index key for {} at {}", stream_id,
                     *opt_idx_for_stream_id);
      auto stream_idx = *opt_idx_for_stream_id;
      index_keys_in_snapshots.insert(
          read_key_row(snapshot_segment, static_cast<ssize_t>(stream_idx)));
    } else {
 ...
 */
std::pair<std::vector<AtomKey>, std::unordered_set<AtomKey>>
...
std::optional<VariantKey> get_snapshot_key(const std::shared_ptr<Store>& store,
                                           const SnapshotId& snap_name) {
  ARCTICDB_SAMPLE(getSnapshot, 0)

  if (auto maybe_ref_key = get_ref_key(snap_name);
      store->key_exists_sync(maybe_ref_key))
    return maybe_ref_key;

  // Fall back to iteration
  ARCTICDB_DEBUG(log::version(),
                 "Ref key not found for snapshot, falling back to slow path: {}",
                 snap_name);
...
std::unordered_map<SnapshotId, std::optional<VariantKey>>
all_ref_keys(const std::vector<SnapshotId>& snap_names,
...
std::unordered_map<SnapshotId, std::optional<VariantKey>>
get_snapshot_keys_via_iteration(const std::vector<bool>& ref_key_exists,
...
std::unordered_map<SnapshotId, std::optional<VariantKey>>
get_keys_for_snapshots(const std::shared_ptr<Store>& store,
                       const std::vector<SnapshotId>& snap_names) {
  std::vector<VariantKey> ref_keys;
  ref_keys.resize(snap_names.size());
  std::transform(std::begin(snap_names), std::end(snap_names), std::begin(ref_keys),
                 [](const auto& name) { return get_ref_key(name); });

  auto found_keys =
      folly::collect(store->batch_key_exists(ref_keys))
          .via(&async::io_executor())
          .thenValue([&snap_names, &ref_keys, store](std::vector<bool> ref_key_exists) {
            if (std::all_of(std::begin(ref_key_exists), std::end(ref_key_exists),
                            [](bool b) { return b; })) {
              return all_ref_keys(snap_names, ref_keys);
            } else {
              return get_snapshot_keys_via_iteration(ref_key_exists, snap_names,
                                                     ref_keys, store);
            }
          });
...
std::optional<std::pair<VariantKey, SegmentInMemory>>
get_snapshot(const std::shared_ptr<Store>& store, const SnapshotId& snap_name) {
  ARCTICDB_SAMPLE(getSnapshot, 0)
...
std::set<StreamId> list_streams_in_snapshot(const std::shared_ptr<Store>& store,
                                            const SnapshotId& snap_name) {
...
std::vector<AtomKey>
get_versions_from_segment(const SegmentInMemory& snapshot_segment) {
...
std::vector<AtomKey> get_versions_from_snapshot(const std::shared_ptr<Store>& store,
                                                const VariantKey& vk) {
...
MasterSnapshotMap get_master_snapshots_map(
    ...
    const std::optional<
        const std::tuple<const SnapshotVariantKey&, std::vector<IndexTypeKey>&>>&
