...
inline entity::VariantKey
write_multi_index_entry(std::shared_ptr<StreamSink> store, std::vector<AtomKey>& keys,
  ...
  IndexAggregator<RowCountIndex> multi_index_agg(
      stream_id, [&multi_key_fut, &store, version_id, stream_id](auto&& segment) {
        multi_key_fut = store
...
inline std::optional<AtomKey> read_segment_with_keys(const SegmentInMemory& seg,
  ...
  for (; row < ssize_t(seg.row_count()); ++row) {
    ...
    if (is_index_key_type(key.type())) {
      ...
      earliest_loaded_timestamp =
          std::min(earliest_loaded_timestamp, key.creation_ts());

      if (!entry.is_tombstoned(key)) {
        oldest_loaded_undeleted_index =
            std::min(oldest_loaded_undeleted_index, key.version_id());
        earliest_loaded_undeleted_timestamp =
            std::min(earliest_loaded_timestamp, key.creation_ts());
      }
  ...
  util::check(row == ssize_t(seg.row_count()),
              "Unexpected ordering in journal segment");
  ...
  load_progress.oldest_loaded_undeleted_index_version_ =
      std::min(load_progress.oldest_loaded_undeleted_index_version_,
               oldest_loaded_undeleted_index);
  ...
  load_progress.earliest_loaded_undeleted_timestamp_ =
      std::min(load_progress.earliest_loaded_undeleted_timestamp_,
               earliest_loaded_undeleted_timestamp);
  return next;
}

inline std::optional<AtomKey>
read_segment_with_keys(const SegmentInMemory& seg,
...
std::shared_ptr<VersionMapEntry> build_version_map_entry_with_predicate_iteration(
    const std::shared_ptr<StreamSource>& store, Predicate&& predicate,
    const StreamId& stream_id, const std::vector<KeyType>& key_types,
    bool perform_read_segment_with_keys = true) {

  auto prefix = std::holds_alternative<StringId>(stream_id)
  ...
  for (auto key_type : key_types) {
    store->iterate_type(
        key_type,
        [&predicate, &read_keys, &store, &output,
         &perform_read_segment_with_keys](VariantKey&& vk) {
  ...
  if (!perform_read_segment_with_keys) {
    output->keys_.insert(output->keys_.end(), std::move_iterator(read_keys.begin()),
    ...
    auto latest_key =
        std::max_element(std::begin(read_keys), std::end(read_keys),
...
inline void check_is_version(const AtomKey& key) {
  util::check(key.type() == KeyType::VERSION, "Expected version key type but got {}",
              key);
}

inline void read_symbol_ref(const std::shared_ptr<StreamSource>& store,
                            const StreamId& stream_id, VersionMapEntry& entry) {
  std::pair<entity::VariantKey, SegmentInMemory> key_seg_pair;
  // Trying to read a missing ref key is expected e.g. when writing a previously missing
  // symbol. If the ref key is missing we keep the entry empty and should not raise
  // warnings.
  ...
  try {
    ...
    try {
      key_seg_pair =
          store->read_sync(RefKey{stream_id, KeyType::VERSION, true}, read_opts);
    } catch (const storage::KeyNotFoundException&) {
      return;
    }
  }
...
inline void write_symbol_ref(std::shared_ptr<StreamSink> store,
  ...
  ARCTICDB_DEBUG(log::version(),
                 ...
                 latest_index, journal_key);

  IndexAggregator<RowCountIndex> ref_agg(
      latest_index.id(), [&store, &latest_index](auto&& s) {
...
// Given the latest version, and a negative index into the version map, returns the
// desired version ID or std::nullopt if it would be negative
inline std::optional<VersionId> get_version_id_negative_index(VersionId latest,
                                                              SignedVersionId index) {
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      index < 0, "get_version_id_negative_index expects a negative index, received {}",
      index);
  // +1 so that as_of=-1 returns the latest entry
  auto candidate_version_id = static_cast<SignedVersionId>(latest) + index + 1;
  return candidate_version_id >= 0 ? std::make_optional<VersionId>(
                                         static_cast<VersionId>(candidate_version_id))
                                   : std::nullopt;
}

std::unordered_map<StreamId, size_t>
get_num_version_entries(const std::shared_ptr<Store>& store, size_t batch_size);
...
inline bool
continue_when_loading_version(const LoadStrategy& load_strategy,
  ...
  if (is_positive_version_query(load_strategy)) {
    if (load_progress.oldest_loaded_index_version_ >
        static_cast<VersionId>(*load_strategy.load_until_version_)) {
      // Should continue when version was not reached
      return true;
    }
  } else {
    if (latest_version.has_value()) {
      if (auto opt_version_id = get_version_id_negative_index(
              *latest_version, *load_strategy.load_until_version_);
          opt_version_id &&
          load_progress.oldest_loaded_index_version_ > *opt_version_id) {
        // Should continue when version was not reached
        return true;
      }
    } else {
  ...
  ARCTICDB_DEBUG(log::version(),
                 "Exiting load downto because loaded to version {} for request {} with "
                 "{} total versions",
                 load_progress.oldest_loaded_index_version_,
                 *load_strategy.load_until_version_, latest_version.value());
  return false;
}

inline void set_latest_version(const std::shared_ptr<VersionMapEntry>& entry,
...
inline bool continue_when_loading_from_time(const LoadStrategy& load_strategy,
                                            const LoadProgress& load_progress) {
  if (!load_strategy.load_from_time_)
    return true;

  auto loaded_deleted_or_undeleted_timestamp =
  ...
  ARCTICDB_DEBUG(log::version(), "Exiting load from timestamp because request {} <= {}",
                 loaded_deleted_or_undeleted_timestamp, *load_strategy.load_from_time_);
  return false;
}

inline bool
continue_when_loading_latest(const LoadStrategy& load_strategy,
                             const std::shared_ptr<VersionMapEntry>& entry) {
...
inline bool
continue_when_loading_undeleted(const LoadStrategy& load_strategy,
                                const std::shared_ptr<VersionMapEntry>& entry,
                                const LoadProgress& load_progress) {
  ...
  if (entry->tombstone_all_) {
    // We need the check below because it is possible to have a tombstone_all which
    // doesn't cover all version keys after it. For example when we use
    // prune_previous_versions (without write) we write a tombstone_all key which
    // applies to keys before the previous one. So it's possible the version chain can
    // look like: v0 <- v1 <- v2 <- tombstone_all(version=1) In this case we need to
    // terminate at v1.
    const bool is_deleted_by_tombstone_all = entry->tombstone_all_->version_id() >=
                                             load_progress.oldest_loaded_index_version_;
    if (is_deleted_by_tombstone_all) {
      ARCTICDB_DEBUG(log::version(),
                     "Exiting because tombstone all key deletes all versions beyond: "
                     "{} and the oldest loaded index has version: {}",
...
inline bool
penultimate_key_contains_required_version_id(const AtomKey& key,
                                             const LoadStrategy& load_strategy) {
  if (is_positive_version_query(load_strategy)) {
    return key.version_id() <=
           static_cast<VersionId>(load_strategy.load_until_version_.value());
  } else {
    return *load_strategy.load_until_version_ == -1;
  }
}

inline bool key_exists_in_ref_entry(const LoadStrategy& load_strategy,
  ...
  if (cached_penultimate_key && is_partial_load_type(load_strategy.load_type_)) {
    ...
    if (load_strategy.load_type_ == LoadType::DOWNTO &&
        penultimate_key_contains_required_version_id(*cached_penultimate_key,
                                                     load_strategy)) {
      return true;
    }

    if (load_strategy.load_type_ == LoadType::FROM_TIME &&
        cached_penultimate_key->creation_ts() <=
            load_strategy.load_from_time_.value()) {
      return true;
    }
  }
