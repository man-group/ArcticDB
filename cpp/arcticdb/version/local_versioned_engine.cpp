...
LocalVersionedEngine::LocalVersionedEngine(
    const std::shared_ptr<storage::Library>& library, const ClockType&)
...
template <class ClockType>
LocalVersionedEngine::LocalVersionedEngine(const std::shared_ptr<Store>& store,
                                           const ClockType&)
    : store_(store), symbol_list_(std::make_shared<SymbolList>(version_map_)) {}

void LocalVersionedEngine::initialize(
  ...
  ARCTICDB_RUNTIME_DEBUG(
      ...
      uintptr_t(this), library->library_path(),
...
template LocalVersionedEngine::LocalVersionedEngine(
    const std::shared_ptr<storage::Library>& library, const util::SysClock&);
template LocalVersionedEngine::LocalVersionedEngine(
    const std::shared_ptr<Store>& library, const util::SysClock&);
template LocalVersionedEngine::LocalVersionedEngine(
    const std::shared_ptr<storage::Library>& library, const util::ManualClock&);
...
folly::Future<folly::Unit> LocalVersionedEngine::delete_unreferenced_pruned_indexes(
  ...
  try {
    if (!pruned_indexes.empty() && !cfg().write_options().delayed_deletes()) {
      // TODO: the following function will load all snapshots, which will be
      // horrifyingly inefficient when called multiple times from batch_*
      auto [not_in_snaps, in_snaps] =
          get_index_keys_partitioned_by_inclusion_in_snapshots(
      ...
      return delete_trees_responsibly(store(), version_map(), not_in_snaps, {}, {},
                                      checks)
          .thenError(folly::tag_t<std::exception>{}, [](auto const& ex) {
            log::version().warn(
                "Failed to clean up pruned previous versions due to: {}", ex.what());
          });
    }
  } catch (const std::exception& ex) {
    // Best-effort so deliberately swallow
    log::version().warn("Failed to clean up pruned previous versions due to: {}",
                        ex.what());
  }
  return folly::Unit();
}
...
void LocalVersionedEngine::create_column_stats_internal(
    const VersionedItem& versioned_item, ColumnStats& column_stats,
...
void LocalVersionedEngine::create_column_stats_version_internal(
    const StreamId& stream_id, ColumnStats& column_stats,
    const VersionQuery& version_query, const ReadOptions& read_options) {
...
FrameAndDescriptor
LocalVersionedEngine::read_column_stats_internal(const VersionedItem& versioned_item) {
...
ReadVersionOutput LocalVersionedEngine::read_column_stats_version_internal(
  ...
  return ReadVersionOutput{std::move(versioned_item.value()),
                           std::move(frame_and_descriptor)};
}
...
FrameAndDescriptor LocalVersionedEngine::read_dataframe_internal(
    const std::variant<VersionedItem, StreamId>& identifier, ReadQuery& read_query,
    const ReadOptions& read_options) {
...
std::set<StreamId>
LocalVersionedEngine::list_streams_internal(std::optional<SnapshotId> snap_name,
...
LocalVersionedEngine::get_latest_version(const StreamId& stream_id) {
  auto key = get_latest_undeleted_version(store(), version_map(), stream_id);
  if (!key) {
    ARCTICDB_DEBUG(log::version(),
                   "get_latest_version didn't find version for stream_id: {}",
                   stream_id);
    return std::nullopt;
  }
...
std::optional<VersionedItem>
LocalVersionedEngine::get_specific_version(const StreamId& stream_id,
                                           SignedVersionId signed_version_id) {
  ARCTICDB_RUNTIME_DEBUG(log::version(), "Command: get_specific_version");
  auto key = ::arcticdb::get_specific_version(store(), version_map(), stream_id,
                                              signed_version_id);
  if (!key) {
    ...
    if (signed_version_id >= 0) {
      ...
      auto [opt_latest_key, deleted] =
          ::arcticdb::get_latest_version(store(), version_map(), stream_id);
      if (opt_latest_key.has_value()) {
        auto opt_version_id = get_version_id_negative_index(
            opt_latest_key->version_id(), signed_version_id);
    ...
    auto index_key = std::find_if(
        index_keys.begin(), index_keys.end(),
        [version_id](const AtomKey& k) { return k.version_id() == version_id; });
    if (index_key != index_keys.end()) {
      ARCTICDB_DEBUG(log::version(),
                     "Found version {} for symbol {} in snapshot:", version_id,
                     stream_id);
      key = *index_key;
    } else {
      ARCTICDB_DEBUG(log::version(),
...
std::optional<VersionedItem>
LocalVersionedEngine::get_version_at_time(const StreamId& stream_id, timestamp as_of) {
  ...
  if (!index_key) {
    log::version().warn(
        "read_dataframe_timestamp: version id not found for stream {} timestamp {}",
        stream_id, as_of);
    return std::nullopt;
  }
...
std::optional<VersionedItem>
LocalVersionedEngine::get_version_from_snapshot(const StreamId& stream_id,
                                                const SnapshotId& snap_name) {
  auto opt_snapshot = get_snapshot(store(), snap_name);
  if (!opt_snapshot) {
    throw storage::NoDataFoundException(snap_name);
  }
  // A snapshot will normally be in a ref key, but for old libraries it still needs to
  // fall back to iteration of atom keys.
  ...
  ARCTICDB_DEBUG(log::version(), "read_snapshot: {} id not found for snapshot {}",
                 stream_id, snap_name);
  return std::nullopt;
}

std::optional<VersionedItem>
LocalVersionedEngine::get_version_to_read(const StreamId& stream_id,
                                          const VersionQuery& version_query) {
...
IndexRange LocalVersionedEngine::get_index_range(const StreamId& stream_id,
                                                 const VersionQuery& version_query) {
...
ReadVersionOutput LocalVersionedEngine::read_dataframe_version_internal(
    const StreamId& stream_id, const VersionQuery& version_query, ReadQuery& read_query,
    const ReadOptions& read_options) {
  auto version = get_version_to_read(stream_id, version_query);
  const std::variant<VersionedItem, StreamId> identifier =
      get_version_identifier(stream_id, version_query, read_options, version);
  auto frame_and_descriptor =
      read_dataframe_internal(identifier, read_query, read_options);
  return ReadVersionOutput{version.value_or(VersionedItem{}),
                           std::move(frame_and_descriptor)};
}
...
folly::Future<DescriptorItem> LocalVersionedEngine::get_descriptor(AtomKey&& k) {
  ...
  return store()->read(key).thenValue([](auto&& key_seg_pair) -> DescriptorItem {
    ...
    if (seg.row_count() > 0) {
      const auto& start_index_column =
          seg.column(position_t(index::Fields::start_index));
      details::visit_type(start_index_column.type().data_type(),
                          [&start_index_column, &start_index](auto column_desc_tag) {
                            ...
                            if constexpr (is_time_type(type_info::data_type)) {
                              start_index =
                                  start_index_column.template scalar_at<timestamp>(0);
                            }
                          });
      ...
      details::visit_type(
          ...
          [&end_index_column, &end_index,
           row_count = seg.row_count()](auto column_desc_tag) {
            using type_info = ScalarTypeInfo<decltype(column_desc_tag)>;
            if constexpr (is_time_type(type_info::data_type)) {
              // -1 as the end timestamp in the data keys is one nanosecond greater than
              // the last value in the index column
              end_index =
                  *end_index_column.template scalar_at<timestamp>(row_count - 1) - 1;
            }
          });
    }
    return DescriptorItem{std::move(key), start_index, end_index,
                          std::move(timeseries_descriptor)};
  });
}

folly::Future<DescriptorItem> LocalVersionedEngine::get_descriptor_async(
    folly::Future<std::optional<AtomKey>>&& version_fut, const StreamId& stream_id,
    const VersionQuery& version_query) {
  return std::move(version_fut)
      .thenValue([this, &stream_id, &version_query](std::optional<AtomKey>&& key) {
        missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
            key.has_value(),
            "Unable to retrieve descriptor data. {}@{}: version not found", stream_id,
            version_query);
...
DescriptorItem
LocalVersionedEngine::read_descriptor_internal(const StreamId& stream_id,
                                               const VersionQuery& version_query) {
...
std::vector<std::variant<DescriptorItem, DataError>>
LocalVersionedEngine::batch_read_descriptor_internal(
    const std::vector<StreamId>& stream_ids,
    const std::vector<VersionQuery>& version_queries, const ReadOptions& read_options) {

  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      read_options.batch_throw_on_error_.has_value(),
      "ReadOptions::batch_throw_on_error_ should always be set here");

  auto version_futures =
      batch_get_versions_async(store(), version_map(), stream_ids, version_queries);
  ...
  for (auto&& [idx, version_fut] : folly::enumerate(version_futures)) {
    descriptor_futures.emplace_back(get_descriptor_async(
        std::move(version_fut), stream_ids[idx], version_queries[idx]));
  }
...
std::shared_ptr<DeDupMap>
LocalVersionedEngine::get_de_dup_map(const StreamId& stream_id,
                                     const std::optional<AtomKey>& maybe_prev,
                                     const WriteOptions& write_options) {
  auto de_dup_map = std::make_shared<DeDupMap>();
  if (write_options.de_duplication) {
    auto maybe_undeleted_prev =
        get_latest_undeleted_version(store(), version_map(), stream_id);
    if (maybe_undeleted_prev) {
      ...
      // This means we don't have any live versions(all tombstoned), so will try to
      // dedup from snapshot versions
      auto snap_versions = get_index_keys_in_snapshots(store(), stream_id);
      auto max_iter =
          std::max_element(std::begin(snap_versions), std::end(snap_versions),
...
VersionedItem LocalVersionedEngine::sort_index(const StreamId& stream_id,
                                               bool dynamic_schema,
                                               bool prune_previous_versions) {
  auto maybe_prev = get_latest_undeleted_version(store(), version_map(), stream_id);
  util::check(maybe_prev.has_value(), "Cannot delete from non-existent symbol {}",
              stream_id);
  auto version_id = get_next_version_from_key(*maybe_prev);
  auto [index_segment_reader, slice_and_keys] =
      index::read_index_to_vector(store(), *maybe_prev);
  if (dynamic_schema) {
    ...
    std::sort(std::begin(slice_and_keys), std::end(slice_and_keys),
              [](const auto& left, const auto& right) {
                auto lt =
                    std::tie(left.slice_.col_range.first, left.key().start_index());
                auto rt =
                    std::tie(right.slice_.col_range.first, right.key().start_index());
                return lt < rt;
              });
  }

  auto total_rows =
      adjust_slice_rowcounts(slice_and_keys, std::make_optional<size_t>(0U));
  ...
  auto time_series = make_timeseries_descriptor(
      total_rows, StreamDescriptor{tsd.as_stream_descriptor()},
      std::move(*tsd.mutable_proto().mutable_normalization()),
      std::move(*tsd.mutable_proto().mutable_user_meta()), std::nullopt, std::nullopt,
      bucketize_dynamic);

  auto versioned_item = pipelines::index::index_and_version(index, store(), time_series,
                                                            std::move(slice_and_keys),
                                                            stream_id, version_id)
                            .get();
  write_version_and_prune_previous(prune_previous_versions, versioned_item.key_,
                                   maybe_prev);
  ARCTICDB_DEBUG(log::version(), "sorted index of stream_id: {} , version_id: {}",
                 stream_id, version_id);
  return versioned_item;
}

VersionedItem
LocalVersionedEngine::delete_range_internal(const StreamId& stream_id,
                                            const UpdateQuery& query,
                                            const DeleteRangeOptions& option) {
  auto maybe_prev = get_latest_undeleted_version(store(), version_map(), stream_id);
  util::check(maybe_prev.has_value(), "Cannot delete from non-existent symbol {}",
              stream_id);
  auto versioned_item = delete_range_impl(store(), *maybe_prev, query,
                                          get_write_options(), option.dynamic_schema_);
  write_version_and_prune_previous(option.prune_previous_versions_, versioned_item.key_,
                                   maybe_prev);
  return versioned_item;
}

VersionedItem LocalVersionedEngine::update_internal(
    const StreamId& stream_id, const UpdateQuery& query,
    const std::shared_ptr<InputTensorFrame>& frame, bool upsert, bool dynamic_schema,
    bool prune_previous_versions) {
  ...
  if (update_info.previous_index_key_.has_value()) {
    ...
    auto versioned_item =
        update_impl(store(), update_info, query, frame, get_write_options(),
                    dynamic_schema, cfg().write_options().empty_types());
    write_version_and_prune_previous(prune_previous_versions, versioned_item.key_,
    ...
    if (upsert) {
      ...
      auto versioned_item = write_dataframe_impl(
          store_, update_info.next_version_id_, frame, write_options,
          std::make_shared<DeDupMap>(), false, true);
...
VersionedItem LocalVersionedEngine::write_versioned_metadata_internal(
  ...
  auto update_info = get_latest_undeleted_version_and_next_version_id(
      store(), version_map(), stream_id);
  if (update_info.previous_index_key_.has_value()) {
    ARCTICDB_DEBUG(log::version(), "write_versioned_metadata for stream_id: {}",
                   stream_id);
    auto index_key = UpdateMetadataTask{store(), update_info, std::move(user_meta)}();
    write_version_and_prune_previous(prune_previous_versions, index_key,
                                     update_info.previous_index_key_);
    return VersionedItem{std::move(index_key)};
  } else {
    ...
    auto versioned_item = write_versioned_dataframe_internal(
        stream_id, frame, prune_previous_versions, false, false);
    if (cfg_.symbol_list())
      symbol_list().add_symbol(store_, stream_id, update_info.next_version_id_);
...
std::vector<std::variant<VersionedItem, DataError>>
LocalVersionedEngine::batch_write_versioned_metadata_internal(
    const std::vector<StreamId>& stream_ids, bool prune_previous_versions,
    bool throw_on_error,
  ...
  for (const auto&& [idx, stream_update_info_fut] :
       folly::enumerate(stream_update_info_futures)) {
    write_metadata_versions_futs.push_back(
        std::move(stream_update_info_fut)
            .thenValue([this, user_meta_proto = std::move(user_meta_protos[idx]),
                        &stream_id = stream_ids[idx]](auto&& update_info) mutable
                       -> folly::Future<IndexKeyAndUpdateInfo> {
              auto index_key_fut = folly::Future<AtomKey>::makeEmpty();
              if (update_info.previous_index_key_.has_value()) {
                index_key_fut = async::submit_io_task(UpdateMetadataTask{
                    store(), update_info, std::move(user_meta_proto)});
              } else {
                auto frame = convert::py_none_to_frame();
                frame->desc.set_id(stream_id);
                ...
                auto de_dup_map = std::make_shared<DeDupMap>();
                index_key_fut =
                    async_write_dataframe_impl(store(), version_id, frame,
                                               write_options, de_dup_map, false, false);
              }
              return std::move(index_key_fut)
                  .thenValue(
                      [update_info = std::forward<decltype(update_info)>(update_info)](
                        ...
                        return IndexKeyAndUpdateInfo{std::move(index_key),
                                                     std::move(update_info)};
                      });
            })
            .thenValue(
                [this, prune_previous_versions](auto&& index_key_and_update_info) {
                  auto&& [index_key, update_info] = index_key_and_update_info;
                  return write_index_key_to_version_map_async(
                      version_map(), std::move(index_key), std::move(update_info),
  ...
  for (auto&& [idx, write_metadata_version] :
       folly::enumerate(write_metadata_versions)) {
    if (write_metadata_version.hasValue()) {
      write_metadata_versions_or_errors.emplace_back(
          std::move(write_metadata_version.value()));
    } else {
      if (throw_on_error) {
...
VersionedItem LocalVersionedEngine::write_versioned_dataframe_internal(
    const StreamId& stream_id, const std::shared_ptr<InputTensorFrame>& frame,
  ...
  auto versioned_item = write_dataframe_impl(store(), version_id, frame, write_options,
                                             de_dup_map, allow_sparse, validate_index);

  if (cfg().symbol_list())
    symbol_list().add_symbol(store(), stream_id, versioned_item.key_.version_id());

  write_version_and_prune_previous(prune_previous_versions, versioned_item.key_,
                                   deleted ? std::nullopt : maybe_prev);
  return versioned_item;
}

std::pair<VersionedItem, TimeseriesDescriptor>
LocalVersionedEngine::restore_version(const StreamId& stream_id,
                                      const VersionQuery& version_query) {
  ...
  missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
      static_cast<bool>(version_to_restore),
      "Unable to restore {}@{}: version not found", stream_id, version_query);
  auto [maybe_prev, deleted] =
      ::arcticdb::get_latest_version(store(), version_map(), stream_id);
  ARCTICDB_DEBUG(log::version(), "restore for stream_id: {} , version_id = {}",
                 stream_id, version_to_restore->key_.version_id());
  return AsyncRestoreVersionTask{store(), version_map(), stream_id,
                                 version_to_restore->key_, maybe_prev}()
      .get();
}

VersionedItem
LocalVersionedEngine::write_individual_segment(const StreamId& stream_id,
  ...
  stream::StreamSink::PartialKey pk{KeyType::TABLE_DATA, version_id, stream_id,
                                    range.start_, range.end_};

  auto frame_slice = FrameSlice{segment};
  auto descriptor =
      make_timeseries_descriptor(segment.row_count(), segment.descriptor().clone(), {},
                                 std::nullopt, std::nullopt, std::nullopt, false);
  auto key = store_->write(pk, std::move(segment)).get();
  std::vector sk{SliceAndKey{frame_slice, to_atom(key)}};
  auto index_key_fut =
      index::write_index(index, std::move(descriptor), std::move(sk),
                         IndexPartialKey{stream_id, version_id}, store_);
...
std::unordered_map<StreamId, VersionId>
min_versions_for_each_stream(const std::vector<AtomKey>& keys) {
...
delete_trees_responsibly(std::shared_ptr<Store> store,
                         ...
                         const PreDeleteChecks& check, const bool dry_run) {
  ...
  // Check snapshot:
  if (check.snapshots) {
    keys_to_delete.remove_if([&snapshot_map, &snapshot_being_deleted,
                              &not_to_delete](const auto& target_key) {
      ...
      if (find_symbol != snapshot_map.end()) {
        ...
        if (snaps_itr != keys_map.end()) {
          ...
          if (snaps.size() > count_for_snapshot_being_deleted) {
            log::version().debug("Skipping the deletion of index {}:{} because it "
                                 "exists in another snapshot",
                                 target_key.id(), target_key.version_id());
            return true;
          }
        }
        copy_versions_nearest_to_target(keys_map, target_key,
                                        not_to_delete); // Check 2)
      }
      return false;
    });
  }
  ...
  if (load_type != LoadType::NOT_LOADED) {
        ...
        auto load_strategy =
            load_type == LoadType::DOWNTO
                ? LoadStrategy{load_type, LoadObjective::INCLUDE_DELETED,
                               static_cast<SignedVersionId>(min.second)}
                : LoadStrategy{load_type, LoadObjective::INCLUDE_DELETED};
        const auto entry =
            version_map->check_reload(store, min.first, load_strategy, __FUNCTION__);
    ...
    keys_to_delete.remove_if([&entry_map, &check, &not_to_delete](const auto& key) {
      ...
      get_matching_prev_and_next_versions(
          ...
          [v = key.version_id()](const AtomKeyImpl& key,
                                 const std::shared_ptr<VersionMapEntry>& entry) {
            // Can't use is_live_index_type_key() because the target version's index key
            // might have already been tombstoned, so will miss it and thus not able to
            // find the prev/next key.
            return is_index_key_type(key.type()) &&
                   (key.version_id() == v || !entry->is_tombstoned(key));
          });
      return false;
    });
  }
  ...
  // Resolve:
  // Check 2) implementations does not consider that the key they are adding to
  // not_to_delete might actually be in keys_to_delete, so excluding those:
  ...
  log::version().debug("Candidate: {} total of data keys",
                       data_keys_to_be_deleted.size());

  read_opts.dont_warn_about_missing_key = true;
  auto data_keys_not_to_be_deleted =
      recurse_index_keys(store, *not_to_delete, read_opts);
  not_to_delete.clear();
  log::version().debug("Forbidden: {} total of data keys",
                       data_keys_not_to_be_deleted.size());
  storage::RemoveOpts remove_opts;
  remove_opts.ignores_missing_key_ = true;

  std::vector<entity::VariantKey> vks_column_stats;
  std::transform(keys_to_delete->begin(), keys_to_delete->end(),
                 std::back_inserter(vks_column_stats),
                 [](const IndexTypeKey& index_key) {
                   return index_key_to_column_stats_key(index_key);
                 });
  log::version().debug("Number of Column Stats keys to be deleted: {}",
                       vks_column_stats.size());
  ...
  log::version().debug("Number of Data keys to be deleted: {}",
                       vks_data_to_delete.size());

  folly::Future<folly::Unit> remove_keys_fut;
  if (!dry_run) {
    ARCTICDB_TRACE(log::version(), fmt::format("Column Stats keys to be deleted: {}",
                                               fmt::join(vks_column_stats, ", ")));
    ARCTICDB_TRACE(log::version(), fmt::format("Index keys to be deleted: {}",
                                               fmt::join(vks_to_delete, ", ")));
    ARCTICDB_TRACE(log::version(), fmt::format("Data keys to be deleted: {}",
                                               fmt::join(vks_data_to_delete, ", ")));

    // Delete any associated column stats keys first
    remove_keys_fut =
        store->remove_keys(std::move(vks_column_stats), remove_opts)
            .thenValue([store = store, vks_to_delete = std::move(vks_to_delete),
...
void LocalVersionedEngine::append_incomplete_segment(const StreamId& stream_id,
                                                     SegmentInMemory&& seg) {
...
VersionedItem LocalVersionedEngine::compact_incomplete_dynamic(
  ...
  auto versioned_item = compact_incomplete_impl(
      store_, stream_id, user_meta, update_info, options, get_write_options());

  write_version_and_prune_previous(options.prune_previous_versions_,
                                   versioned_item.key_,
                                   update_info.previous_index_key_);
...
bool LocalVersionedEngine::is_symbol_fragmented(const StreamId& stream_id,
                                                std::optional<size_t> segment_size) {
  auto update_info = get_latest_undeleted_version_and_next_version_id(
      store(), version_map(), stream_id);
  auto options = get_write_options();
  auto pre_defragmentation_info =
      get_pre_defragmentation_info(store(), stream_id, update_info, options,
                                   segment_size.value_or(options.segment_row_size));
  return is_symbol_fragmented_impl(pre_defragmentation_info.segments_need_compaction);
}

VersionedItem
LocalVersionedEngine::defragment_symbol_data(const StreamId& stream_id,
                                             std::optional<size_t> segment_size,
                                             bool prune_previous_versions) {
  log::version().info("Defragmenting data for symbol {}", stream_id);

  // Currently defragmentation only for latest version - is there a use-case to allow
  // compaction for older data?
  auto update_info = get_latest_undeleted_version_and_next_version_id(
      store(), version_map(), stream_id);
...
folly::Future<ReadVersionOutput> async_read_direct(
    const std::shared_ptr<Store>& store, const VariantKey& index_key,
    SegmentInMemory&& index_segment, std::shared_ptr<ReadQuery> read_query,
    std::shared_ptr<BufferHolder> buffers, const ReadOptions& read_options) {
  auto index_segment_reader =
      std::make_shared<index::IndexSegmentReader>(std::move(index_segment));
  ...
  auto pipeline_context =
      std::make_shared<PipelineContext>(StreamDescriptor{tsd.as_stream_descriptor()});
  ...
  return fetch_data(frame, pipeline_context, store, dynamic_schema, buffers)
      .thenValue([pipeline_context, frame, read_options](auto&&) mutable {
        ScopedGILLock gil_lock;
        reduce_and_fix_columns(pipeline_context, frame, read_options);
      })
      .thenValue([index_segment_reader, frame, index_key, buffers, read_query,
                  pipeline_context](auto&&) mutable {
        ...
        return ReadVersionOutput{
            VersionedItem{to_atom(index_key)},
            FrameAndDescriptor{
                frame, std::move(index_segment_reader->mutable_tsd()), {}, buffers}};
      });
}

std::vector<ReadVersionOutput>
LocalVersionedEngine::batch_read_keys(const std::vector<AtomKey>& keys) {
  py::gil_scoped_release release_gil;
  std::vector<folly::Future<std::pair<entity::VariantKey, SegmentInMemory>>>
      index_futures;
  ...
  for (auto&& [index_key, index_segment] : indexes) {
    results_fut.emplace_back(async_read_direct(
        store(), keys[i], std::move(index_segment), std::make_shared<ReadQuery>(),
        std::make_shared<BufferHolder>(), ReadOptions{}));
    ++i;
  }
...
std::vector<std::variant<ReadVersionOutput, DataError>>
LocalVersionedEngine::temp_batch_read_internal_direct(
    const std::vector<StreamId>& stream_ids,
    ...
    read_versions_futs.emplace_back(
        std::move(version)
            .thenValue([store = store()](auto&& maybe_index_key) {
              missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
                  maybe_index_key.has_value(), "Version not found for symbol");
              return store->read(*maybe_index_key);
            })
            .thenValue([store = store(),
                        read_query = read_queries.empty()
                                         ? std::make_shared<ReadQuery>()
                                         : read_queries[idx],
                        read_options](auto&& key_segment_pair) {
              auto [index_key, index_segment] = std::move(key_segment_pair);
              return async_read_direct(store, index_key, std::move(index_segment),
                                       read_query, std::make_shared<BufferHolder>(),
                                       read_options);
            }));
  }
  // TODO: https://github.com/man-group/ArcticDB/issues/241
  // Move everything from here to the end of the function out into batch_read_internal
  // as part of #241
...
LocalVersionedEngine::batch_read_internal(
  ...
  if (std::none_of(
          std::begin(read_queries), std::end(read_queries),
          [](const auto& read_query) { return !read_query->clauses_.empty(); })) {
    return temp_batch_read_internal_direct(stream_ids, version_queries, read_queries,
                                           read_options);
  }
  ...
  for (size_t idx = 0; idx < stream_ids.size(); idx++) {
    auto version_query =
        version_queries.size() > idx ? version_queries[idx] : VersionQuery{};
    auto read_query = read_queries.size() > idx ? *read_queries[idx] : ReadQuery{};
    // TODO: https://github.com/man-group/ArcticDB/issues/241
    // Remove this try-catch in favour of the implementation in
    // temp_batch_read_internal_direct as part of #241
    try {
      auto read_version = read_dataframe_version_internal(
          stream_ids[idx], version_query, read_query, read_options);
      read_versions_or_errors.emplace_back(std::move(read_version));
    } catch (const NoSuchVersionException& e) {
      if (*read_options.batch_throw_on_error_) {
        throw;
      }
      read_versions_or_errors.emplace_back(DataError(stream_ids[idx], e.what(),
                                                     version_query.content_,
      ...
      read_versions_or_errors.emplace_back(
          DataError(stream_ids[idx], e.what(), version_query.content_));
    }
  }
  return read_versions_or_errors;
}
...
void LocalVersionedEngine::write_version_and_prune_previous(
    bool prune_previous_versions, const AtomKey& new_version,
    const std::optional<IndexTypeKey>& previous_key) {
  if (prune_previous_versions) {
    auto pruned_indexes =
        version_map()->write_and_prune_previous(store(), new_version, previous_key);
...
folly::Future<VersionedItem> LocalVersionedEngine::write_index_key_to_version_map_async(
    const std::shared_ptr<VersionMap>& version_map, AtomKey&& index_key,
    UpdateInfo&& stream_update_info, bool prune_previous_versions,
  ...
  if (prune_previous_versions) {
    write_version_fut =
        async::submit_io_task(WriteAndPrunePreviousTask{
                                  store(), version_map, index_key,
                                  std::move(stream_update_info.previous_index_key_)})
            .thenValue([this, index_key](auto&& atom_key_vec) {
              return delete_unreferenced_pruned_indexes(std::move(atom_key_vec),
                                                        index_key);
            });
  } else {
    write_version_fut = async::submit_io_task(WriteVersionTask{
        store(), version_map, index_key, stream_update_info.previous_index_key_});
  }

  if (add_new_symbol) {
    write_version_fut =
        std::move(write_version_fut)
            .then([this, index_key_id = index_key.id(),
                   reference_id = index_key.version_id()](auto&&) {
              return async::submit_io_task(WriteSymbolTask(store(), symbol_list_ptr(),
                                                           index_key_id, reference_id));
            });
  }
...
std::vector<folly::Future<AtomKey>> LocalVersionedEngine::batch_write_internal(
    ...
    std::vector<std::shared_ptr<DeDupMap>> de_dup_maps, bool validate_index) {
  ...
  for (size_t idx = 0; idx < stream_ids.size(); idx++) {
    results_fut.emplace_back(async_write_dataframe_impl(
        store(), version_ids[idx], std::move(frames[idx]), get_write_options(),
        de_dup_maps[idx], false, validate_index));
  }
  return results_fut;
}

VersionIdAndDedupMapInfo LocalVersionedEngine::create_version_id_and_dedup_map(
    const version_store::UpdateInfo&& update_info, const StreamId& stream_id,
    const WriteOptions& write_options) {
  if (cfg().write_options().de_duplication()) {
    ...
    return VersionIdAndDedupMapInfo{update_info.next_version_id_,
...
std::vector<std::variant<VersionedItem, DataError>>
LocalVersionedEngine::batch_write_versioned_dataframe_internal(
    const std::vector<StreamId>& stream_ids,
    std::vector<std::shared_ptr<pipelines::InputTensorFrame>>&& frames,
  ...
  auto write_options = get_write_options();
  auto update_info_futs = batch_get_latest_undeleted_version_and_next_version_id_async(
      store(), version_map(), stream_ids);
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      stream_ids.size() == update_info_futs.size(),
      "stream_ids and update_info_futs must be of the same size");
  ...
  for (auto&& update_info_fut : folly::enumerate(update_info_futs)) {
    ...
    version_futures.push_back(
        std::move(*update_info_fut)
            .thenValue([this, &stream_id = stream_ids[idx],
                        &write_options](auto&& update_info) {
              return create_version_id_and_dedup_map(std::move(update_info), stream_id,
                                                     write_options);
            })
            .via(&async::cpu_executor())
            .thenValue([this, &stream_id = stream_ids[idx], &write_options,
                        &validate_index,
                        &frame = frames[idx]](auto&& version_id_and_dedup_map) {
              auto& [version_id, de_dup_map, update_info] = version_id_and_dedup_map;
              ARCTICDB_SAMPLE(WriteDataFrame, 0)
              ARCTICDB_DEBUG(log::version(), "Writing dataframe for stream id {}",
                             stream_id);
              auto write_fut =
                  async_write_dataframe_impl(store(), version_id, frame, write_options,
                                             de_dup_map, false, validate_index);
              return std::move(write_fut).thenValue(
                  [update_info = std::move(update_info)](auto&& index_key) mutable {
                    return IndexKeyAndUpdateInfo{std::move(index_key),
                                                 std::move(update_info)};
                  });
            })
...
VersionedItem LocalVersionedEngine::append_internal(
  ...
  if (update_info.previous_index_key_.has_value()) {
    ...
    auto versioned_item =
        append_impl(store(), update_info, frame, get_write_options(), validate_index,
    ...
    if (upsert) {
      ...
      auto versioned_item = write_dataframe_impl(
          store_, update_info.next_version_id_, frame, write_options,
          std::make_shared<DeDupMap>(), false, validate_index);
...
LocalVersionedEngine::batch_append_internal(
    const std::vector<StreamId>& stream_ids,
    std::vector<std::shared_ptr<pipelines::InputTensorFrame>>&& frames,
    bool prune_previous_versions, bool validate_index, bool upsert,
    bool throw_on_error) {
  py::gil_scoped_release release_gil;
  ...
  for (const auto&& [idx, stream_update_info_fut] :
    ...
    append_versions_futs.push_back(
        std::move(stream_update_info_fut)
            .thenValue([this, frame = std::move(frames[idx]), validate_index,
                        stream_id = stream_ids[idx], upsert](auto&& update_info) mutable
                       -> folly::Future<IndexKeyAndUpdateInfo> {
              auto index_key_fut = folly::Future<AtomKey>::makeEmpty();
              auto write_options = get_write_options();
              if (update_info.previous_index_key_.has_value()) {
                index_key_fut = async_append_impl(store(), update_info, frame,
                                                  write_options, validate_index,
                                                  cfg().write_options().empty_types());
              } else {
                missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
                ...
                auto version_id = 0;
                auto de_dup_map = std::make_shared<DeDupMap>();
                index_key_fut = async_write_dataframe_impl(store(), version_id, frame,
                                                           write_options, de_dup_map,
                                                           false, validate_index);
              }
              return std::move(index_key_fut)
                  .thenValue([update_info = std::move(update_info)](
                                 auto&& index_key) mutable -> IndexKeyAndUpdateInfo {
                    return IndexKeyAndUpdateInfo{std::move(index_key),
                                                 std::move(update_info)};
                  });
...
struct WarnVersionTypeNotHandled {
  ...
  void warn(const StreamId& stream_id) {
    if (!warned) {
      log::version().warn("Only exact version numbers are supported when using "
                          "add_to_snapshot/batch_restore_version calls."
                          "The queries passed for '{}', etc. are ignored and the "
                          "latest versions will be used!",
                          stream_id);
      warned = true;
    }
  }
...
std::map<StreamId, VersionId>
get_sym_versions_from_query(const std::vector<StreamId>& stream_ids,
                            const std::vector<VersionQuery>& version_queries) {
...
std::map<StreamId, VersionVectorType>
get_multiple_sym_versions_from_query(const std::vector<StreamId>& stream_ids,
                                     const std::vector<VersionQuery>& version_queries) {
  ...
  for (const auto& stream_id : folly::enumerate(stream_ids)) {
    const auto& query = version_queries[stream_id.index].content_;
    if (std::holds_alternative<SpecificVersionQuery>(query))
      sym_versions[*stream_id].push_back(
          std::get<SpecificVersionQuery>(query).version_id_);
    else
      warner.warn(*stream_id);
  }
  return sym_versions;
...
std::vector<std::pair<VersionedItem, TimeseriesDescriptor>>
LocalVersionedEngine::batch_restore_version_internal(
    const std::vector<StreamId>& stream_ids,
    const std::vector<VersionQuery>& version_queries) {
  util::check(stream_ids.size() == version_queries.size(),
              "Symbol vs version query size mismatch: {} != {}", stream_ids.size(),
  ...
  auto versions_to_restore =
      batch_get_specific_version(store(), version_map(), sym_versions);
  std::vector<folly::Future<std::pair<VersionedItem, TimeseriesDescriptor>>> fut_vec;

  for (const auto& stream_id : folly::enumerate(stream_ids)) {
    auto prev = previous->find(*stream_id);
    auto maybe_prev = prev == std::end(*previous)
                          ? std::nullopt
                          : std::make_optional<AtomKey>(to_atom(prev->second));

    auto version = versions_to_restore->find(*stream_id);
    util::check(version != std::end(*versions_to_restore),
                "Did not find version for symbol {}", *stream_id);
    fut_vec.emplace_back(async::submit_io_task(AsyncRestoreVersionTask{
        store(), version_map(), *stream_id, to_atom(version->second), maybe_prev}));
  }
  auto output = folly::collect(fut_vec).get();

  std::vector<folly::Future<folly::Unit>> symbol_write_futs;
  for (const auto& item : output) {
    symbol_write_futs.emplace_back(async::submit_io_task(
        WriteSymbolTask(store(), symbol_list_ptr(), item.first.key_.id(),
                        item.first.key_.version_id())));
  }
...
timestamp
LocalVersionedEngine::get_update_time_internal(const StreamId& stream_id,
                                               const VersionQuery& version_query) {
  auto version = get_version_to_read(stream_id, version_query);
  if (!version)
    throw storage::NoDataFoundException(
        fmt::format("get_update_time: version not found for symbol", stream_id));
...
SpecificAndLatestVersionKeys LocalVersionedEngine::get_stream_index_map(
  ...
  if (!version_queries.empty()) {
    auto sym_versions =
        get_multiple_sym_versions_from_query(stream_ids, version_queries);
    specific_versions =
        batch_get_specific_versions(store(), version_map(), sym_versions);
    std::vector<StreamId> latest_ids;
    std::copy_if(std::begin(stream_ids), std::end(stream_ids),
                 std::back_inserter(latest_ids), [&sym_versions](const auto& key) {
                   return sym_versions.find(key) == std::end(sym_versions);
                 });
    latest_versions =
        batch_get_latest_version(store(), version_map(), latest_ids, false);
  } else {
    specific_versions =
        std::make_shared<std::unordered_map<std::pair<StreamId, VersionId>, AtomKey>>();
    latest_versions =
        batch_get_latest_version(store(), version_map(), stream_ids, false);
  }
...
LocalVersionedEngine::get_metadata(std::optional<AtomKey>&& key) {
  if (key.has_value()) {
...
LocalVersionedEngine::get_metadata_async(
    folly::Future<std::optional<AtomKey>>&& version_fut, const StreamId& stream_id,
    const VersionQuery& version_query) {
  return std::move(version_fut)
      .thenValue([this, &stream_id, &version_query](std::optional<AtomKey>&& key) {
        missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
            key.has_value(), "Unable to retrieve  metadata. {}@{}: version not found",
            stream_id, version_query);
...
std::vector<std::variant<std::pair<VariantKey, std::optional<google::protobuf::Any>>,
                         DataError>>
LocalVersionedEngine::batch_read_metadata_internal(
    const std::vector<StreamId>& stream_ids,
    const std::vector<VersionQuery>& version_queries, const ReadOptions& read_options) {
  ...
  for (auto&& [idx, version] : folly::enumerate(version_futures)) {
    metadata_futures.emplace_back(
        get_metadata_async(std::move(version), stream_ids[idx], version_queries[idx]));
  }
  ...
  for (auto&& [idx, metadata] : folly::enumerate(metadatas)) {
    if (metadata.hasValue()) {
      ...
      // For historical reasons, batch_read_metadata does not raise if the version does
      // not exist (unlike batch_read)
      if (*read_options.batch_throw_on_error_ &&
          !exception.is_compatible_with<NoSuchVersionException>()) {
...
std::pair<std::optional<VariantKey>, std::optional<google::protobuf::Any>>
LocalVersionedEngine::read_metadata_internal(const StreamId& stream_id,
                                             const VersionQuery& version_query) {
  auto version = get_version_to_read(stream_id, version_query);
  std::optional<AtomKey> key =
      version.has_value() ? std::make_optional<AtomKey>(version->key_) : std::nullopt;
...
VersionedItem LocalVersionedEngine::sort_merge_internal(
  ...
  auto update_info = get_latest_undeleted_version_and_next_version_id(
      store(), version_map(), stream_id);
  auto versioned_item =
      sort_merge_impl(store_, stream_id, user_meta, update_info, options);
  write_version_and_prune_previous(options.prune_previous_versions_,
                                   versioned_item.key_,
                                   update_info.previous_index_key_);
  return versioned_item;
...
void LocalVersionedEngine::configure(
    const storage::LibraryDescriptor::VariantStoreConfig& cfg) {
  util::variant_match(
      cfg, [](std::monostate) { /* Unknown config */ },
      [&cfg = cfg_, version_map = version_map(),
      ...
      [](const auto& conf) {
        util::raise_rte("Configuration of LocalVersionedEngine must use a "
                        "VersionStoreConfig, actual {}",
                        [&conf] { util::format(conf); });
      });
}

timestamp LocalVersionedEngine::latest_timestamp(const std::string& symbol) {
  if (auto latest_incomplete = latest_incomplete_timestamp(store(), symbol);
      latest_incomplete)
...
std::unordered_map<KeyType, KeySizesInfo> LocalVersionedEngine::scan_object_sizes() {
  ...
  foreach_key_type([&store = store(), &sizes = sizes](KeyType key_type) {
    std::vector<std::pair<VariantKey, stream::StreamSource::ReadContinuation>>
        key_size_calculators;
    store->iterate_type(
        key_type, [&key_size_calculators, &sizes, &key_type](const VariantKey&& k) {
          auto& sizes_info = sizes[key_type];
          ++sizes_info.count;
          key_size_calculators.emplace_back(
              std::forward<const VariantKey>(k), [&sizes_info](auto&& ks) {
    ...
    if (!key_size_calculators.empty()) {
      store->batch_read_compressed(std::move(key_size_calculators), BatchReadArgs{})
          .get();
    }
  });
...
std::unordered_map<StreamId, std::unordered_map<KeyType, KeySizesInfo>>
LocalVersionedEngine::scan_object_sizes_by_stream() {
  std::mutex mutex;
  ...
  foreach_key_type([&store = store(), &sizes, &mutex](KeyType key_type) {
    ...
    store->iterate_type(
        key_type, [&keys, &mutex, &sizes, key_type](const VariantKey&& k) {
          keys.emplace_back(std::forward<const VariantKey>(k),
                            [key_type, &sizes, &mutex](auto&& ks) {
                              auto key_seg = std::move(ks);
...
void LocalVersionedEngine::move_storage(KeyType key_type, timestamp horizon,
...
std::variant<VersionedItem, StreamId> LocalVersionedEngine::get_version_identifier(
    const StreamId& stream_id, const VersionQuery& version_query,
    const ReadOptions& read_options, const std::optional<VersionedItem>& version) {
  if (!version) {
    if (opt_false(read_options.incompletes_)) {
      log::version().warn(
          "No index: Key not found for {}, will attempt to use incomplete segments.",
          stream_id);
      return stream_id;
    } else {
