...
void LocalVersionedEngine::create_column_stats_version_internal(
    ...
    missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
        versioned_item.has_value(), "create_column_stats_version_internal: version not found for stream '{}'", stream_id
    );
...
void LocalVersionedEngine::drop_column_stats_version_internal(
    ...
    auto versioned_item = get_version_to_read(stream_id, version_query);
    missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
        versioned_item.has_value(), "drop_column_stats_version_internal: version not found for stream '{}'", stream_id
    );
...
ReadVersionOutput LocalVersionedEngine::read_column_stats_version_internal(
    ...
    auto versioned_item = get_version_to_read(stream_id, version_query);
    missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
        versioned_item.has_value(), "read_column_stats_version_internal: version not found for stream '{}'", stream_id
    );
        ...
        if (seg.row_count() > 0) {
            ...
            details::visit_type(
                ...
                [&end_index_column, &end_index, row_count = seg.row_count()](auto column_desc_tag) {
                    using type_info = ScalarTypeInfo<decltype(column_desc_tag)>;
                    if constexpr (is_time_type(type_info::data_type)) {
                        // -1 as the end timestamp in the data keys is one nanosecond greater than the last value in the
                        // index column
...
DescriptorItem LocalVersionedEngine::read_descriptor_internal(
    ...
    auto version = get_version_to_read(stream_id, version_query);
    missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
        version.has_value(), "Unable to retrieve descriptor data. {}@{}: version not found", stream_id, version_query
    );
...
std::vector<std::variant<DescriptorItem, DataError>> LocalVersionedEngine::batch_read_descriptor_internal(
    ...
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        read_options.batch_throw_on_error_.has_value(), "ReadOptions::batch_throw_on_error_ should always be set here"
    );
...
VersionedItem LocalVersionedEngine::update_internal(
    ...
    if (update_info.previous_index_key_.has_value()) {
        ...
        auto versioned_item = update_impl(
            store(), update_info, query, frame, get_write_options(), dynamic_schema, cfg().write_options().empty_types()
        );
        write_version_and_prune_previous(prune_previous_versions, versioned_item.key_, update_info.previous_index_key_);
        return versioned_item;
    } else {
            ...
            std::move(stream_update_info_fut)
                .thenValue(
                    [this,
                     ...
                     &stream_id = stream_ids[idx]](auto&& update_info) mutable -> folly::Future<IndexKeyAndUpdateInfo> {
                        ...
                        return std::move(index_key_fut)
                            .thenValue(
                                [update_info = std::forward<decltype(update_info)>(update_info)](auto&& index_key
                                ) mutable -> IndexKeyAndUpdateInfo {
...
std::pair<VersionedItem, TimeseriesDescriptor> LocalVersionedEngine::restore_version(
    ...
    missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
        static_cast<bool>(version_to_restore), "Unable to restore {}@{}: version not found", stream_id, version_query
    );
    auto [maybe_prev, deleted] = ::arcticdb::get_latest_version(store(), version_map(), stream_id);
    ARCTICDB_DEBUG(
        log::version(), "restore for stream_id: {} , version_id = {}", stream_id, version_to_restore->key_.version_id()
    );
...
VersionedItem LocalVersionedEngine::write_individual_segment(
    ...
    auto descriptor = make_timeseries_descriptor(
        segment.row_count(), segment.descriptor().clone(), {}, std::nullopt, std::nullopt, std::nullopt, false
    );
    ...
    if (load_type != LoadType::NOT_LOADED) {
        ...
        keys_to_delete.remove_if([&entry_map, &check, &not_to_delete](const auto& key) {
            ...
            if (check.version_visible && !entry->is_tombstoned(key)) { // Check 1)
                log::version().debug(
                    "Skipping the deletion of index {}:{} because it exists in version map", key.id(), key.version_id()
                );
                return true;
            }
    ...
    if (!dry_run) {
        ...
        remove_keys_fut =
            store->remove_keys(std::move(vks_column_stats), remove_opts)
                ...
                .thenValue(
                    [store = store, vks_data_to_delete = std::move(vks_data_to_delete), remove_opts](auto&&) mutable {
...
VersionedItem LocalVersionedEngine::compact_incomplete_dynamic(
    ...
    auto update_info = get_latest_undeleted_version_and_next_version_id(store(), version_map(), stream_id);
    auto versioned_item =
        compact_incomplete_impl(store_, stream_id, user_meta, update_info, options, get_write_options());

    write_version_and_prune_previous(
        options.prune_previous_versions_, versioned_item.key_, update_info.previous_index_key_
    );
...
VersionedItem LocalVersionedEngine::defragment_symbol_data(
    ...
    auto versioned_item = defragment_symbol_data_impl(
        store(), stream_id, update_info, options, segment_size.has_value() ? *segment_size : options.segment_row_size
    );
...
folly::Future<ReadVersionOutput> async_read_direct(
    ...
    return async_read_direct_impl(
        store, index_key, std::move(index_segment), read_query, shared_data, handler_data, read_options
    );
}
...
std::vector<std::variant<ReadVersionOutput, DataError>> LocalVersionedEngine::batch_read_internal(
    ...
    for (size_t i = 0; i < stream_ids.size(); ++i) {
        ...
        try {
            auto read_version =
                read_dataframe_version_internal(stream_ids[i], version_query, read_query, read_options, handler_data);
            ...
            read_versions_or_errors.emplace_back(
                DataError(stream_ids[i], e.what(), version_query.content_, ErrorCode::E_NO_SUCH_VERSION)
            );
            ...
            read_versions_or_errors.emplace_back(
                DataError(stream_ids[i], e.what(), version_query.content_, ErrorCode::E_KEY_NOT_FOUND)
            );
            ...
            read_versions_or_errors.emplace_back(DataError(stream_ids[i], e.what(), version_query.content_));
        }
    }
    return read_versions_or_errors;
}
        ...
        version_futures.push_back(
            std::move(*update_info_fut)
                ...
                .thenValue([this, &stream_id = stream_ids[idx], &write_options, &validate_index, &frame = frames[idx]](
                    ...
                    return std::move(write_fut).thenValue([update_info =
                                                               std::move(update_info)](auto&& index_key) mutable {
                        return IndexKeyAndUpdateInfo{std::move(index_key), std::move(update_info)};
                    });
                })
                .thenValue([this, prune_previous_versions](auto&& index_key_and_update_info) {
...
std::vector<std::variant<VersionedItem, DataError>> LocalVersionedEngine::batch_append_internal(
    ...
    for (const auto&& [idx, stream_update_info_fut] : folly::enumerate(stream_update_info_futures)) {
        append_versions_futs.push_back(
            std::move(stream_update_info_fut)
                .thenValue(
                    [this, frame = std::move(frames[idx]), validate_index, stream_id = stream_ids[idx], upsert](
                        auto&& update_info
                        ...
                        return std::move(index_key_fut)
                            .thenValue(
                                [update_info =
                                     std::move(update_info)](auto&& index_key) mutable -> IndexKeyAndUpdateInfo {
                                    return IndexKeyAndUpdateInfo{std::move(index_key), std::move(update_info)};
                                }
                            );
                    }
                ...
                .thenValue(
                    [this, prune_previous_versions](auto&& index_key_and_update_info) -> folly::Future<VersionedItem> {
                        auto&& [index_key, update_info] = index_key_and_update_info;
                        return write_index_key_to_version_map_async(
                            version_map(), std::move(index_key), std::move(update_info), prune_previous_versions
                        );
...
struct WarnVersionTypeNotHandled {
    ...
    void warn(const StreamId& stream_id) {
        if (!warned) {
            log::version().warn(
                "Only exact version numbers are supported when using add_to_snapshot/batch_restore_version calls."
...
std::vector<std::pair<VersionedItem, TimeseriesDescriptor>> LocalVersionedEngine::batch_restore_version_internal(
    ...
    util::check(
        sym_versions.size() == version_queries.size(), "Restore versions requires specific version to be supplied"
    );
    ...
    for (const auto& item : output) {
        symbol_write_futs.emplace_back(async::submit_io_task(
            WriteSymbolTask(store(), symbol_list_ptr(), item.first.key_.id(), item.first.key_.version_id())
        ));
    }
...
folly::Future<std::pair<VariantKey, std::optional<google::protobuf::Any>>> LocalVersionedEngine::get_metadata_async(
    ...
    return std::move(version_fut)
        .thenValue([this, &stream_id, &version_query](std::optional<AtomKey>&& key) {
            missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
                key.has_value(), "Unable to retrieve  metadata. {}@{}: version not found", stream_id, version_query
            );
...
std::vector<std::variant<std::pair<VariantKey, std::optional<google::protobuf::Any>>, DataError>> LocalVersionedEngine::
    ...
    for (auto&& [idx, metadata] : folly::enumerate(metadatas)) {
        if (metadata.hasValue()) {
            ...
            // For historical reasons, batch_read_metadata does not raise if the version does not exist (unlike
            // batch_read)
...
VersionedItem LocalVersionedEngine::sort_merge_internal(
    ...
    auto versioned_item = sort_merge_impl(store_, stream_id, user_meta, update_info, options, write_opts);
    write_version_and_prune_previous(
        options.prune_previous_versions_, versioned_item.key_, update_info.previous_index_key_
    );
    add_to_symbol_list_on_compaction(stream_id, options, update_info);
...
void LocalVersionedEngine::configure(const storage::LibraryDescriptor::VariantStoreConfig& cfg) {
    util::variant_match(
        ...
        [](const auto& conf) {
            util::raise_rte("Configuration of LocalVersionedEngine must use a VersionStoreConfig, actual {}", [&conf] {
                util::format(conf);
            });
        }
    );
