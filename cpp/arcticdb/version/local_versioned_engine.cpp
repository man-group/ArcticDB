...
std::vector<std::variant<ResultValueType, DataError>> transform_batch_items_or_throw(
        ...
        TransformBatchResultsFlags flags, std::span<const VersionQuery> versoin_queries = {}
) {
        ...
        if (version_or_exception.hasValue()) {
            ...
            if (flags.throw_on_error_ && (!is_missing_version_exception || throw_on_missing_symbol)) {
                ...
                DataError data_error =
                        versoin_queries.empty()
                                ? DataError(stream_ids[idx], exception.what().toStdString())
                                : DataError(
                                          stream_ids[idx], exception.what().toStdString(), versoin_queries[idx].content_
                                  );
...
VersionedItem LocalVersionedEngine::write_individual_segment(
    ...
    auto index_key_fut = index::write_index(
            index, std::move(descriptor), std::move(sk), IndexPartialKey{stream_id, version_id}, store_
    );
...
VersionedItem LocalVersionedEngine::compact_incomplete_dynamic(
    ...
    auto versioned_item = compact_incomplete_impl(
            store_, stream_id, user_meta, update_info, options, get_write_options(), pipeline_context
    );
                        ...
                        .thenValue([store = store(),
                            ...
                            if (opt_index_key.has_value()) {
                                ...
                                if (opt_false(read_options.incompletes())) {
                                    ...
                                    missing_data::raise<ErrorCode::E_NO_SUCH_VERSION>(
                                            "batch_read_internal: version matching query '{}' not found for symbol "
                                            "'{}'",
                                            version_queries[idx],
                                            stream_ids[idx]
                                    );
                ...
                std::move(stream_update_info_fut)
                        .thenValue(
                                [this,
                                 ...
                                 validate_index,
                                 stream_id = stream_ids[idx],
                                 upsert](auto&& update_info) mutable -> folly::Future<IndexKeyAndUpdateInfo> {
                                    auto index_key_fut = folly::Future<AtomKey>::makeEmpty();
                                    auto write_options = get_write_options();
                                    if (update_info.previous_index_key_.has_value()) {
                                    ...
                                    return std::move(index_key_fut)
                                            .thenValue(
                                                    [update_info = std::move(update_info)](auto&& index_key
                        ...
                        .thenValue([this,
                            ...
                            if (update_info.previous_index_key_.has_value()) {
                                ...
                                missing_data::check<ErrorCode::E_NO_SUCH_VERSION>(
                                        ...
                                        "Using \"upsert=True\" will create create the symbol instead of throwing this "
                                        "exception.",
                                        stream_id
                                );
                                index_key_fut = async_write_dataframe_impl(
                            ...
                            return std::move(index_key_fut)
                                    .thenValueInline([update_info = std::move(update_info)](auto&& index_key) mutable {
                                        return IndexKeyAndUpdateInfo{std::move(index_key), std::move(update_info)};
                                    });
                        })
                        .thenValue([this, prune_previous_versions](auto&& index_key_and_update_info) {
...
VersionedItem LocalVersionedEngine::sort_merge_internal(
    ...
    auto versioned_item =
            sort_merge_impl(store_, stream_id, user_meta, update_info, options, get_write_options(), pipeline_context);
