...
ReadVersionOutput LocalVersionedEngine::read_dataframe_version_internal(
    ...
    auto frame_and_descriptor = read_frame_for_version(store(), identifier, read_query, read_options);
...
folly::Future<ReadVersionOutput> async_read_direct(
        ...
        const ReadOptions& read_options) {
    return async_read_direct_impl(store, index_key, std::move(index_segment), read_query, shared_data, read_options);
}
...
std::vector<std::variant<ReadVersionOutput, DataError>> LocalVersionedEngine::batch_read_internal(
    ...
    for (size_t i=0; i < stream_ids.size(); ++i) {
        ...
        try {
            auto read_version = read_dataframe_version_internal(stream_ids[i], version_query, read_query, read_options);
