...
VersionedItem write_dataframe_impl(
        ...
        const std::shared_ptr<DeDupMap>& de_dup_map, bool sparsify_floats, bool validate_index
) {
...
static UpdateRanges compute_update_ranges(
        const FilterRange& row_filter, const InputTensorFrame& update_frame,
...
static void check_can_update(
        const InputTensorFrame& frame, const index::IndexSegmentReader& index_segment_reader,
        const UpdateInfo& update_info, bool dynamic_schema, bool empty_types
) {
    util::check(
    ...
    (void)check_and_mark_slices(
            index_segment_reader, dynamic_schema, false, std::nullopt, index_segment_reader.bucketize_dynamic()
    );
...
static std::shared_ptr<std::vector<SliceAndKey>> get_keys_affected_by_update(
        const index::IndexSegmentReader& index_segment_reader, const InputTensorFrame& frame, const UpdateQuery& query,
        bool dynamic_schema
) {
    std::vector<FilterQuery<index::IndexSegmentReader>> queries = build_update_query_filters<index::IndexSegmentReader>(
...
folly::Future<ReadVersionOutput> read_multi_key(
    ...
    return read_frame_for_version(store, versioned_item, std::make_shared<ReadQuery>(), ReadOptions{}, handler_data)
            .thenValue([multi_key_desc = std::move(multi_key_desc),
                        keys = std::move(keys)](ReadVersionOutput&& read_version_output) mutable {
...
bool read_incompletes_to_pipeline(
        ...
        const ReadQuery& read_query, const ReadOptions& read_options, bool convert_int_to_float, bool via_iteration,
        bool sparsify, bool dynamic_schema
) {
...
struct CopyToBufferTask : async::BaseTask {
    ...
    CopyToBufferTask(
            ...
            DecodePathData shared_data, std::any& handler_data, bool fetch_index, OutputFormat output_format
    ) :
...
folly::Future<SegmentInMemory> prepare_output_frame(
    ...
    const auto allocation_type =
            read_options.output_format() == OutputFormat::ARROW ? AllocationType::DETACHABLE : AllocationType::PRESIZED;
...
folly::Future<SegmentInMemory> do_direct_read_or_process(
    ...
    if (!read_query->clauses_.empty()) {
        ...
        mark_index_slices(
                pipeline_context, opt_false(read_options.dynamic_schema()), pipeline_context->bucketize_dynamic_
        );
        const auto allocation_type = read_options.output_format() == OutputFormat::ARROW ? AllocationType::DETACHABLE
...
VersionedItem sort_merge_impl(
        ...
        const UpdateInfo& update_info, const CompactIncompleteOptions& options, const WriteOptions& write_options,
...
VersionedItem compact_incomplete_impl(
    ...
    CompactionResult result = util::variant_match(
            std::move(policies),
            [&slices, pipeline_context = pipeline_context, &store, &options, &previous_sorted_value, &write_options](
                ...
                if constexpr (std::is_same_v<IndexType, TimeseriesIndex>) {
                    pipeline_context->desc_->set_sorted(deduce_sorted(
                            previous_sorted_value.value_or(SortedValue::ASCENDING), SortedValue::ASCENDING
                    ));
                }
...
folly::Future<ReadVersionOutput> read_frame_for_version(
    ...
    DecodePathData shared_data;
    return version_store::do_direct_read_or_process(
