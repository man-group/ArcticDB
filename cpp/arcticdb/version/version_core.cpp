...
static std::vector<RowRange> find_unique_row_ranges(const std::vector<SliceAndKey>& slices_and_keys) {
...
static void calc_on_disk_slices(
        const std::vector<RowRange>& new_row_ranges, std::vector<SliceAndKey>& prev_slice_and_keys,
        size_t& last_row_on_disk, std::vector<SliceAndKey>& slices_and_keys
) {
    ...
    return async::submit_io_task(SetupPipelineContextTask{store, std::move(resolved), read_query, {}})
            ...
            .thenValue([store,
                ...
                if (frame) {
                    ...
                    merged_tsd = std::make_shared<TimeseriesDescriptor>(
                            index::get_merged_tsd(frame->offset + frame->num_rows, dynamic_schema, existing_tsd, frame)
                    );
                }
                                ...
                                [pipeline_context = std::move(pipeline_context),
                                 ...
                                 frame](std::vector<SliceAndKey>&& slices_and_keys
                                    ...
                                    const std::vector<RowRange> new_row_ranges =
                                            find_unique_row_ranges(slices_and_keys);
                                                ...
                                                TimeseriesDescriptor tsd =
                                                        merged_tsd
                                                                ...
                                                                : make_timeseries_descriptor(
                                                                          ...
                                                                          pipeline_context->release_normalization(),
                                                                          pipeline_context
                                                                                  ->release_opt_user_defined_metadata(),
                                                                          std::nullopt,
                                                                          std::nullopt,
                                                                          write_options.bucketize_dynamic
                                                                  );
