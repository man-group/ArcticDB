...
FrameAndDescriptor read_multi_key(
    ...
    auto res = read_frame_for_version(store, versioned_item, read_query, ReadOptions{});
...
void copy_frame_data_to_buffer(
    ...
    if(auto handler = get_type_handler(src_column.type(), dst_column.type()); handler) {
        handler->convert_type(src_column, buffer, num_rows, offset, src_column.type(), dst_column.type(), shared_data, source.string_pool_ptr());
...
struct CopyToBufferTask : async::BaseTask {
    ...
    folly::Unit operator()() {
        ...
        for (auto idx = 0u; idx < index_field_count && fetch_index_; ++idx) {
            copy_frame_data_to_buffer(target_segment_, idx, source_segment_, idx, frame_slice_.row_range, shared_data_);
        }
        ...
        for (auto field_col = index_field_count; field_col < field_count; ++field_col) {
            ...
            copy_frame_data_to_buffer(target_segment_, *frame_loc_opt, source_segment_, field_col, frame_slice_.row_range, shared_data_);
        }
        return folly::Unit{};
    }
};
