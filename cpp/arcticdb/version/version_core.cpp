...
void modify_descriptor(
  ...
  if (opt_false(read_options.force_strings_to_object_) ||
      opt_false(read_options.force_strings_to_fixed_))
...
VersionedItem
write_dataframe_impl(const std::shared_ptr<Store>& store, VersionId version_id,
  ...
  ARCTICDB_DEBUG(log::version(),
                 ...
                 frame->desc.id(), version_id, frame->num_rows);
  auto atom_key_fut = async_write_dataframe_impl(
      store, version_id, frame, options, de_dup_map, sparsify_floats, validate_index);
  return VersionedItem(std::move(atom_key_fut).get());
}

folly::Future<entity::AtomKey>
async_write_dataframe_impl(const std::shared_ptr<Store>& store, VersionId version_id,
                           const std::shared_ptr<InputTensorFrame>& frame,
                           const WriteOptions& options,
                           const std::shared_ptr<DeDupMap>& de_dup_map,
                           bool sparsify_floats, bool validate_index) {
  ...
  return write_frame(std::move(partial_key), frame, slicing_arg, store, de_dup_map,
                     sparsify_floats);
}

namespace {
IndexDescriptorImpl check_index_match(const arcticdb::stream::Index& index,
                                      const IndexDescriptorImpl& desc) {
  if (std::holds_alternative<stream::TimeseriesIndex>(index))
    util::check(desc.type() == IndexDescriptor::Type::TIMESTAMP ||
                    desc.type() == IndexDescriptor::Type::EMPTY,
                "Index mismatch, cannot update a non-timeseries-indexed frame with a "
                "timeseries");
  else
    util::check(desc.type() == IndexDescriptorImpl::Type::ROWCOUNT,
                "Index mismatch, cannot update a timeseries with a "
                "non-timeseries-indexed frame");

  return desc;
}
} // namespace

void sorted_data_check_append(const InputTensorFrame& frame,
  ...
  sorting::check<ErrorCode::E_UNSORTED_DATA>(
      ...
      "When calling append with validate_index enabled, the existing data must be "
      "sorted");
}
...
folly::Future<AtomKey> async_append_impl(const std::shared_ptr<Store>& store,
  ...
  ARCTICDB_DEBUG(log::version(), "append stream_id: {} , version_id: {}", stream_id,
                 update_info.next_version_id_);
  auto index_segment_reader =
      index::get_index_reader(*(update_info.previous_index_key_), store);
  ...
  fix_descriptor_mismatch_or_throw(APPEND, options.dynamic_schema, index_segment_reader,
                                   *frame, empty_types);
  if (validate_index) {
  ...
  auto slicing_arg = get_slicing_policy(options, *frame);
  return append_frame(IndexPartialKey{stream_id, update_info.next_version_id_}, frame,
                      slicing_arg, index_segment_reader, store, options.dynamic_schema,
...
VersionedItem append_impl(const std::shared_ptr<Store>& store,
  ...
  ARCTICDB_SUBSAMPLE_DEFAULT(WaitForWriteCompletion)
  auto version_key_fut = async_append_impl(store, update_info, frame, options,
                                           validate_index, empty_types);
  auto version_key = std::move(version_key_fut).get();
  auto versioned_item = VersionedItem(to_atom(std::move(version_key)));
  ARCTICDB_DEBUG(log::version(), "write_dataframe_impl stream_id: {} , version_id: {}",
                 versioned_item.symbol(), update_info.next_version_id_);
...
void ensure_keys_line_up(const KeyContainer& slice_and_keys) {
  ...
  for (const auto& sk : slice_and_keys) {
    util::check(!start || sk.slice_.row_range.first == end.value(),
                "Can't update as there is a sorting mismatch at key {} relative to "
                "previous key {} - expected index {} got {}",
...
inline std::pair<std::vector<SliceAndKey>, std::vector<SliceAndKey>>
intersecting_segments(const std::vector<SliceAndKey>& affected_keys,
                      const IndexRange& front_range, const IndexRange& back_range,
                      VersionId version_id, const std::shared_ptr<Store>& store) {
  ...
  for (const auto& affected_slice_and_key : affected_keys) {
    ...
    if (intersects(affected_range, front_range) &&
        !overlaps(affected_range, front_range) &&
        is_before(affected_range, front_range)) {
      auto front_overlap_key =
          rewrite_partial_segment(affected_slice_and_key, front_range, version_id,
                                  AffectedSegmentPart::START, store);
      if (front_overlap_key)
        intersect_before.push_back(*front_overlap_key);
    }

    if (intersects(affected_range, back_range) &&
        !overlaps(affected_range, back_range) && is_after(affected_range, back_range)) {
      auto back_overlap_key =
          rewrite_partial_segment(affected_slice_and_key, back_range, version_id,
                                  AffectedSegmentPart::END, store);
      if (back_overlap_key)
...
VersionedItem delete_range_impl(const std::shared_ptr<Store>& store,
                                const AtomKey& prev, const UpdateQuery& query,
                                const WriteOptions&&, bool dynamic_schema) {

  const StreamId& stream_id = prev.id();
  auto version_id = get_next_version_from_key(prev);
  util::check(std::holds_alternative<IndexRange>(query.row_filter),
              "Delete range requires index range argument");
  ...
  util::check_rte(!index_segment_reader.is_pickled(),
                  "Cannot delete date range of pickled data");

  auto index =
      index_type_from_descriptor(index_segment_reader.tsd().as_stream_descriptor());
  ...
  std::vector<FilterQuery<index::IndexSegmentReader>> queries =
      build_update_query_filters<index::IndexSegmentReader>(
          query.row_filter, index, index_range, dynamic_schema,
  ...
  std::set_difference(std::begin(index_segment_reader), std::end(index_segment_reader),
                      std::begin(affected_keys), std::end(affected_keys),
  ...
  const std::array<std::vector<SliceAndKey>, 5> groups{
      strictly_before(orig_filter_range, unaffected_keys), std::move(intersect_before),
      std::move(intersect_after), strictly_after(orig_filter_range, unaffected_keys)};
  ...
  auto version_key_fut =
      util::variant_match(index, [&index_segment_reader, &flattened_slice_and_keys,
                                  &stream_id, &version_id, &store](auto idx) {
        using IndexType = decltype(idx);
        return pipelines::index::write_index<IndexType>(
            index_segment_reader.tsd(), std::move(flattened_slice_and_keys),
            IndexPartialKey{stream_id, version_id}, store);
      });
  auto version_key = std::move(version_key_fut).get();
  auto versioned_item = VersionedItem(to_atom(std::move(version_key)));
  ARCTICDB_DEBUG(log::version(), "updated stream_id: {} , version_id: {}", stream_id,
                 version_id);
  return versioned_item;
}

void sorted_data_check_update(InputTensorFrame& frame,
  ...
  // If changing this error message, the corresponding message in
  // _normalization.py::restrict_data_to_date_range_only should also be updated
  sorting::check<ErrorCode::E_UNSORTED_DATA>(
      input_data_is_sorted, "When calling update, the input data must be sorted.");
  bool existing_data_is_sorted =
...
VersionedItem update_impl(const std::shared_ptr<Store>& store,
                          const UpdateInfo& update_info, const UpdateQuery& query,
                          const std::shared_ptr<InputTensorFrame>& frame,
                          const WriteOptions&& options, bool dynamic_schema,
                          bool empty_types) {
  util::check(update_info.previous_index_key_.has_value(),
  ...
  ARCTICDB_DEBUG(log::version(),
                 ...
                 stream_id, update_info.previous_index_key_->version_id());
  auto index_segment_reader =
      index::get_index_reader(*(update_info.previous_index_key_), store);
  ...
  util::check(index_desc.type() == IndexDescriptor::Type::TIMESTAMP ||
                  index_desc.type() == IndexDescriptor::Type::EMPTY,
              "Update not supported for non-timeseries indexes");
  sorted_data_check_update(*frame, index_segment_reader);
  bool bucketize_dynamic = index_segment_reader.bucketize_dynamic();
  (void)check_and_mark_slices(index_segment_reader, dynamic_schema, false, std::nullopt,
                              bucketize_dynamic);
  ...
  std::vector<FilterQuery<index::IndexSegmentReader>> queries =
      build_update_query_filters<index::IndexSegmentReader>(
          query.row_filter, frame->index, frame->index_range, dynamic_schema,
          index_segment_reader.bucketize_dynamic());
  auto combined = combine_filter_functions(queries);
  auto affected_keys = filter_index(index_segment_reader, std::move(combined));
  std::vector<SliceAndKey> unaffected_keys;
  ...
  util::check(
      ...
      "Unaffected vs affected keys split was inconsistent {} + {} != {}",
      affected_keys.size(), unaffected_keys.size(), index_segment_reader.size());
  ...
  auto new_slice_and_keys =
      slice_and_write(frame, slicing_arg,
  ...
  auto [intersect_before, intersect_after] = util::variant_match(
      ...
      [&](std::monostate) {
        ...
        return intersecting_segments(affected_keys, front_range, back_range,
                                     update_info.next_version_id_, store);
      },
      [&](const IndexRange& idx_range) {
        orig_filter_range = idx_range;
        return intersecting_segments(affected_keys, idx_range, idx_range,
                                     update_info.next_version_id_, store);
      },
      [](const RowRange&)
          -> std::pair<std::vector<SliceAndKey>, std::vector<SliceAndKey>> {
  ...
  const std::array<std::vector<SliceAndKey>, 5> groups{
      strictly_before(orig_filter_range, unaffected_keys), std::move(intersect_before),
      std::move(new_slice_and_keys), std::move(intersect_after),
      strictly_after(orig_filter_range, unaffected_keys)};
  auto flattened_slice_and_keys = flatten_and_fix_rows(groups, row_count);

  util::check(unaffected_keys.size() + new_keys_size + (affected_keys.size() * 2) >=
                  flattened_slice_and_keys.size(),
              "Output size mismatch: {} + {} + (2 * {}) < {}", unaffected_keys.size(),
              new_keys_size, affected_keys.size(), flattened_slice_and_keys.size());

  std::sort(std::begin(flattened_slice_and_keys), std::end(flattened_slice_and_keys));
  auto tsd = index::get_merged_tsd(row_count, dynamic_schema,
                                   index_segment_reader.tsd(), frame);
  auto version_key_fut = index::write_index(
      stream::index_type_from_descriptor(tsd.as_stream_descriptor()), std::move(tsd),
...
FrameAndDescriptor read_multi_key(const std::shared_ptr<Store>& store,
...
std::vector<EntityId> process_clauses(
    ...
    std::vector<std::shared_ptr<Clause>>
        clauses) { // pass by copy deliberately as we don't want to modify read_query
  std::vector<folly::FutureSplitter<pipelines::SegmentAndSlice>>
      segment_and_slice_future_splitters;
  segment_and_slice_future_splitters.reserve(segment_and_slice_futures.size());
  for (auto&& future : segment_and_slice_futures) {
    segment_and_slice_future_splitters.emplace_back(
        folly::splitFuture(std::move(future)));
  }
  ...
  for (const auto& list : processing_unit_indexes) {
    for (auto idx : list) {
      internal::check<ErrorCode::E_ASSERTION_FAILURE>(
          ...
          "Index {} in processing_unit_indexes out of bounds >{}", idx,
          segment_proc_unit_counts.size() - 1);
      segment_proc_unit_counts[idx]++;
    }
  }
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      std::all_of(segment_proc_unit_counts.begin(), segment_proc_unit_counts.end(),
  ...
  // Reverse the order of clauses and iterate through them backwards so that the erase
  // is efficient
      ...
      if (first_clause) {
        ...
        futures.emplace_back(
            folly::collect(local_futs)
                ...
                .thenValue([component_manager, &segment_proc_unit_counts,
                            &entity_added_mtx, &entity_added, &clauses,
                            entity_ids = std::move(entity_ids)](
                               std::vector<pipelines::SegmentAndSlice>&&
                                   segment_and_slices) mutable {
                  for (auto&& [idx, segment_and_slice] :
                       folly::enumerate(segment_and_slices)) {
                    ...
                    if (!entity_added[entity_id]) {
                      component_manager->add(
                          std::make_shared<SegmentInMemory>(
                              std::move(segment_and_slice.segment_in_memory_)),
                          entity_id, segment_proc_unit_counts[entity_id]);
                      component_manager->add(
                          std::make_shared<RowRange>(
                              std::move(segment_and_slice.ranges_and_key_.row_range_)),
                          entity_id);
                      component_manager->add(
                          std::make_shared<ColRange>(
                              std::move(segment_and_slice.ranges_and_key_.col_range_)),
                          entity_id);
                      component_manager->add(
                          std::make_shared<AtomKey>(
                              std::move(segment_and_slice.ranges_and_key_.key_)),
                          entity_id);
                      entity_added[entity_id] = true;
                    }
                  }
                  return async::MemSegmentProcessingTask(clauses,
                                                         std::move(entity_ids))();
                }));
      } else {
        futures.emplace_back(async::submit_cpu_task(
            async::MemSegmentProcessingTask(clauses, std::move(entity_ids))));
      }
    }
    first_clause = false;
...
void set_output_descriptors(const ProcessingUnit& proc,
  ...
  if (new_stream_descriptor.has_value() && proc.segments_.has_value()) {
    ...
    new_stream_descriptor =
        merge_descriptors(*new_stream_descriptor, fields, std::vector<std::string>{});
  }
  if (new_stream_descriptor.has_value()) {
    // Finding and erasing fields from the FieldCollection contained in StreamDescriptor
    // is O(n) in number of fields So maintain map from field names to types in the
    // new_stream_descriptor to make these operations O(1) Cannot use set of FieldRef as
    // the name in the output might match the input, but with a different type after
    // processing
    ...
    // Erase field from new_fields as we add them to final_stream_descriptor, as all
    // fields left in new_fields after these operations were created by the processing
    // pipeline, and so should be appended Index columns should always appear first
    ...
    // Iterate through new_stream_descriptor->fields() rather than remaining new_fields
    // to preserve ordering e.g. if there were two projections then users will expect
    // the column produced by the first one to appear first in the output df
    for (const auto& field : new_stream_descriptor->fields()) {
...
columns_to_decode(const std::shared_ptr<PipelineContext>& pipeline_context) {
  std::shared_ptr<std::unordered_set<std::string>> res;
  ARCTICDB_DEBUG(log::version(), "Creating columns list with {} bits set",
                 pipeline_context->overall_column_bitset_
                     ? pipeline_context->overall_column_bitset_->count()
                     : -1);
  if (pipeline_context->overall_column_bitset_) {
    ...
    while (en < en_end) {
      ARCTICDB_DEBUG(log::version(), "Adding field {}",
                     pipeline_context->desc_->field(*en).name());
...
std::vector<RangesAndKey>
generate_ranges_and_keys(const std::vector<SliceAndKey>& slice_and_keys) {
  std::vector<RangesAndKey> res;
  res.reserve(slice_and_keys.size());
  for (auto& slice_and_key : slice_and_keys) {
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(slice_and_key.key_.has_value(),
                                                    "Missing key in pipeline context");
    // Take a copy here as things like defrag need the keys in
    // pipeline_context->slice_and_keys_ that aren't being modified at the end
 ...
 *
 * Slices are processed in an order defined by the first clause in the pipeline, with
 * slices corresponding to the same processing unit collected into a single
 * ProcessingUnit. Slices contained within a single ProcessingUnit are processed within
 * a single thread.
 *
 * The processing of a ProcessingUnit is scheduled via the Async Store. Within a single
 * thread, the segments will be retrieved from storage and decompressed before being
 * passed to a MemSegmentProcessingTask which will process all clauses up until a clause
 * that requires a repartition.
 */
std::vector<SliceAndKey>
read_and_process(const std::shared_ptr<Store>& store,
                 const std::shared_ptr<PipelineContext>& pipeline_context,
  ...
  // Each element of the vector corresponds to one processing unit containing the list
  // of indexes in ranges_and_keys required for that processing unit i.e. if the first
  // processing unit needs ranges_and_keys[0] and ranges_and_keys[1], and the second
  // needs ranges_and_keys[2] and ranges_and_keys[3] then the structure will be {{0, 1},
  // {2, 3}}
  std::vector<std::vector<size_t>> processing_unit_indexes =
      read_query.clauses_[0]->structure_for_processing(ranges_and_keys, start_from);
  component_manager->set_next_entity_id(ranges_and_keys.size());

  // Start reading as early as possible
  auto segment_and_slice_futures = store->batch_read_uncompressed(
      std::move(ranges_and_keys), columns_to_decode(pipeline_context));

  auto processed_entity_ids =
      process_clauses(component_manager, std::move(segment_and_slice_futures),
                      std::move(processing_unit_indexes), read_query.clauses_);
  auto proc = gather_entities(component_manager, std::move(processed_entity_ids));

  if (std::any_of(read_query.clauses_.begin(), read_query.clauses_.end(),
...
SegmentInMemory read_direct(const std::shared_ptr<Store>& store,
  ...
  fetch_data(frame, pipeline_context, store, opt_false(read_options.dynamic_schema_),
             buffers)
      .get();
  util::print_total_mem_usage(__FILE__, __LINE__, __FUNCTION__);
  return frame;
...
void add_index_columns_to_query(const ReadQuery& read_query,
  ...
  if (read_query.columns.has_value()) {
    ...
    for (const auto& index_column : index_columns) {
      if (std::find(std::begin(*read_query.columns), std::end(*read_query.columns),
                    index_column) == std::end(*read_query.columns))
...
FrameAndDescriptor read_segment_impl(const std::shared_ptr<Store>& store,
                                     const VariantKey& key) {
  auto [_, seg] = store->read_sync(key);
  return frame_and_descriptor_from_segment(std::move(seg));
}

FrameAndDescriptor read_index_impl(const std::shared_ptr<Store>& store,
                                   const VersionedItem& version) {
  return read_segment_impl(store, version.key_);
}

std::optional<pipelines::index::IndexSegmentReader>
get_index_segment_reader(const std::shared_ptr<Store>& store,
                         const std::shared_ptr<PipelineContext>& pipeline_context,
  ...
  try {
    ...
    ARCTICDB_DEBUG(log::version(), "Key not found from versioned item {}: {}",
                   version_info.key_, ex.what());
    throw storage::NoDataFoundException(fmt::format(
        "When trying to read version {} of symbol `{}`, failed to read key {}: {}",
        version_info.version(), version_info.symbol(), version_info.key_, ex.what()));
  }
  ...
  return std::make_optional<pipelines::index::IndexSegmentReader>(
      std::move(index_key_seg.second));
}

void read_indexed_keys_to_pipeline(
    const std::shared_ptr<Store>& store,
    const std::shared_ptr<PipelineContext>& pipeline_context,
    const VersionedItem& version_info, ReadQuery& read_query,
    const ReadOptions& read_options) {
  ...
  user_input::check<ErrorCode::E_INVALID_USER_ARGUMENT>(
      !(index_segment_reader.tsd().proto().normalization().has_custom() &&
        read_query.columns && read_query.columns->empty()),
      "Reading the index column is not supported when recursive or custom normalizers "
      "are used.");
  user_input::check<ErrorCode::E_INVALID_USER_ARGUMENT>(
      !(index_segment_reader.is_pickled() && read_query.columns &&
        read_query.columns->empty()),
      "Reading index columns is not supported with pickled data.");
  ARCTICDB_DEBUG(log::version(), "Read index segment with {} keys",
  ...
  pipeline_context->norm_meta_ =
      std::make_unique<arcticdb::proto::descriptors::NormalizationMetadata>(std::move(
          *index_segment_reader.mutable_tsd().mutable_proto().mutable_normalization()));
  pipeline_context->user_meta_ =
      std::make_unique<arcticdb::proto::descriptors::UserDefinedMetadata>(std::move(
          *index_segment_reader.mutable_tsd().mutable_proto().mutable_user_meta()));
...
void read_incompletes_to_pipeline(const std::shared_ptr<Store>& store,
                                  ...
                                  const ReadOptions& read_options,
                                  bool convert_int_to_float, bool via_iteration,
                                  bool sparsify) {

  auto incomplete_segments =
      get_incomplete(store, pipeline_context->stream_id_, read_query.row_filter,
                     pipeline_context->last_row(), via_iteration, false);

  if (incomplete_segments.empty())
    return;

  // Mark the start point of the incompletes, so we know that there is no column slicing
  // after this point
  ...
  if (pipeline_context->slice_and_keys_.empty()) {
    add_index_columns_to_query(
        read_query, incomplete_segments.begin()->segment(store).index_descriptor());
  }
  ...
  if (!pipeline_context->norm_meta_) {
    pipeline_context->norm_meta_ =
        std::make_unique<arcticdb::proto::descriptors::NormalizationMetadata>();
    auto segment_tsd = first_seg.index_descriptor();
    pipeline_context->norm_meta_->CopyFrom(segment_tsd.proto().normalization());
    ensure_timeseries_norm_meta(*pipeline_context->norm_meta_,
                                pipeline_context->stream_id_, sparsify);
  }

  pipeline_context->desc_ = merge_descriptors(pipeline_context->descriptor(),
                                              incomplete_segments, read_query.columns);
...
void check_incompletes_index_ranges_dont_overlap(
  ...
  /*
   ...
   Checks:
    - that the existing data is not known to be unsorted in the case of a parallel
   append
    - that the index ranges of incomplete segments do not overlap with one another
    - that the earliest timestamp in an incomplete segment is greater than the latest
   timestamp existing in the symbol in the case of a parallel append
   */
  if (pipeline_context->descriptor().index().type() ==
      IndexDescriptorImpl::Type::TIMESTAMP) {
    std::optional<timestamp> last_existing_index_value;
    // Beginning of incomplete segments == beginning of all segments implies all
    // segments are incompletes, so we are writing, not appending
    if (pipeline_context->incompletes_begin() != pipeline_context->begin()) {
      sorting::check<ErrorCode::E_UNSORTED_DATA>(
          ...
          "Cannot append staged segments to existing data as existing data is not "
          "sorted in ascending order");
      auto last_indexed_slice_and_key =
          std::prev(pipeline_context->incompletes_begin())->slice_and_key();
    ...
    for (auto it = pipeline_context->incompletes_begin(); it != pipeline_context->end();
         it++) {
      sorting::check<ErrorCode::E_UNSORTED_DATA>(
          !last_existing_index_value.has_value() ||
              it->slice_and_key().key().start_time() >= *last_existing_index_value,
          "Cannot append staged segments to existing data as incomplete segment "
          "contains index value < existing data (in UTC): {} <= {}",
          date_and_time(it->slice_and_key().key().start_time()),
          date_and_time(*last_existing_index_value));
      auto [_, inserted] =
          unique_timestamp_ranges.insert({it->slice_and_key().key().start_time(),
                                          it->slice_and_key().key().end_time()});
      // This is correct because incomplete segments aren't column sliced
      sorting::check<ErrorCode::E_UNSORTED_DATA>(
          inserted,
          "Cannot finalize staged data as 2 or more incomplete segments cover "
          "identical index values (in UTC): ({}, {})",
          date_and_time(it->slice_and_key().key().start_time()),
          date_and_time(it->slice_and_key().key().end_time()));
    };
    for (auto it = unique_timestamp_ranges.begin(); it != unique_timestamp_ranges.end();
         it++) {
      auto next_it = std::next(it);
      if (next_it != unique_timestamp_ranges.end()) {
        sorting::check<ErrorCode::E_UNSORTED_DATA>(
            next_it->first >= it->second,
            "Cannot finalize staged data as incomplete segment index values overlap "
            "one another (in UTC): ({}, {}) intersects ({}, {})",
            date_and_time(it->first), date_and_time(it->second - 1),
            date_and_time(next_it->first), date_and_time(next_it->second - 1));
      }
    }
  }
}

void copy_frame_data_to_buffer(const SegmentInMemory& destination, size_t target_index,
                               SegmentInMemory& source, size_t source_index,
  ...
  auto type_promotion_error_msg =
      fmt::format("Can't promote type {} to type {} in field {}", src_column.type(),
                  dst_column.type(), destination.field(target_index).name());
  if (is_empty_type(src_column.type().data_type())) {
    dst_column.type().visit_tag([&](auto dst_desc_tag) {
      util::default_initialize<decltype(dst_desc_tag)>(dst_ptr,
                                                       num_rows * dst_rawtype_size);
    });
    // Do not use src_column.is_sparse() here, as that misses columns that are dense,
    // but have fewer than num_rows values
  } else if (src_column.opt_sparse_map().has_value() &&
             has_valid_type_promotion(src_column.type(), dst_column.type())) {
    details::visit_type(dst_column.type().data_type(), [&](auto dst_tag) {
      using dst_type_info = ScalarTypeInfo<decltype(dst_tag)>;
      util::default_initialize<typename dst_type_info::TDT>(
          dst_ptr, num_rows * dst_rawtype_size);
      ...
      details::visit_type(src_column.type().data_type(), [&](auto src_tag) {
        ...
        Column::for_each_enumerated<typename src_type_info::TDT>(
            src_column, [typed_dst_ptr](auto enumerating_it) {
    ...
    details::visit_type(
        src_column.type().data_type(), [&src_data, &dst_ptr](auto src_desc_tag) {
    ...
    details::visit_type(dst_column.type().data_type(), [&src_data, &dst_ptr,
                                                        &src_column,
                                                        &type_promotion_error_msg](
                                                           auto dest_desc_tag) {
      ...
      details::visit_type(
          src_column.type().data_type(),
          [&src_data, &typed_dst_ptr, &type_promotion_error_msg](auto src_desc_tag) {
            ...
            if constexpr (std::is_arithmetic_v<typename source_type_info::RawType> &&
                          std::is_arithmetic_v<DestinationType>) {
              const auto src_cend = src_data.cend<typename source_type_info::TDT>();
              for (auto src_it = src_data.cbegin<typename source_type_info::TDT>();
                   src_it != src_cend; ++src_it) {
...
void copy_segments_to_frame(const std::shared_ptr<Store>& store,
  ...
  for (auto context_row : folly::enumerate(*pipeline_context)) {
    ...
    for (auto idx = 0u; idx < index_field_count && context_row->fetch_index(); ++idx) {
      copy_frame_data_to_buffer(frame, idx, segment, idx,
                                slice_and_key.slice_.row_range);
    }

    auto field_count = slice_and_key.slice_.col_range.diff() + index_field_count;
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        field_count == segment.descriptor().field_count(),
        "Column range does not match segment descriptor field count in "
        "copy_segments_to_frame: {} != {}",
        field_count, segment.descriptor().field_count());
    for (auto field_col = index_field_count; field_col < field_count; ++field_col) {
      ...
      copy_frame_data_to_buffer(frame, *frame_loc_opt, segment, field_col,
...
SegmentInMemory
prepare_output_frame(std::vector<SliceAndKey>&& items,
                     const std::shared_ptr<PipelineContext>& pipeline_context,
                     const std::shared_ptr<Store>& store,
                     const ReadOptions& read_options) {
  ...
  std::sort(std::begin(pipeline_context->slice_and_keys_),
            ...
            [](const auto& left, const auto& right) {
              return std::tie(left.slice_.row_range, left.slice_.col_range) <
                     std::tie(right.slice_.row_range, right.slice_.col_range);
            });
  adjust_slice_rowcounts(pipeline_context);
  const auto dynamic_schema = opt_false(read_options.dynamic_schema_);
  mark_index_slices(pipeline_context, dynamic_schema,
...
void create_column_stats_impl(const std::shared_ptr<Store>& store,
  ...
  try {
    ...
    // If all the column stats we are being asked to create already exist, there's no
    // work to do
  ...
  read_indexed_keys_to_pipeline(store, pipeline_context, versioned_item, read_query,
                                read_options);
  ...
  if (!old_segment.has_value()) {
    ...
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        new_segment.column(0) == old_segment->column(0) &&
            new_segment.column(1) == old_segment->column(1),
...
void drop_column_stats_impl(const std::shared_ptr<Store>& store,
                            const VersionedItem& versioned_item,
  ...
  if (!column_stats_to_drop.has_value()) {
    ...
    if (columns_to_keep.empty()) {
      ...
      for (const auto& field : old_fields) {
        ...
        if (!columns_to_keep.contains(std::string{column_name}) &&
            column_name != start_index_column_name &&
...
FrameAndDescriptor read_column_stats_impl(const std::shared_ptr<Store>& store,
                                          const VersionedItem& versioned_item) {
  auto column_stats_key = index_key_to_column_stats_key(versioned_item.key_);
  ...
  try {
    ...
    storage::raise<ErrorCode::E_KEY_NOT_FOUND>("Failed to read column stats key: {}",
                                               e.what());
  }
}

ColumnStats get_column_stats_info_impl(const std::shared_ptr<Store>& store,
                                       const VersionedItem& versioned_item) {
  auto column_stats_key = index_key_to_column_stats_key(versioned_item.key_);
  // Remove try-catch once AsyncStore methods raise the new error codes themselves
  try {
    auto stream_descriptor = std::get<StreamDescriptor>(
        store->read_metadata_and_descriptor(column_stats_key).get());
...
SegmentInMemory
do_direct_read_or_process(const std::shared_ptr<Store>& store, ReadQuery& read_query,
                          const ReadOptions& read_options,
                          const std::shared_ptr<PipelineContext>& pipeline_context,
  ...
  if (!read_query.clauses_.empty()) {
    ...
    frame =
        prepare_output_frame(std::move(segs), pipeline_context, store, read_options);
  } else {
    ARCTICDB_SAMPLE(MarkAndReadDirect, 0)
    util::check_rte(
        !(pipeline_context->is_pickled() &&
          std::holds_alternative<RowRange>(read_query.row_filter)),
        "Cannot use head/tail/row_range with pickled data, use plain read instead");
    mark_index_slices(pipeline_context, opt_false(read_options.dynamic_schema_),
                      pipeline_context->bucketize_dynamic_);
...
FrameAndDescriptor
read_dataframe_impl(const std::shared_ptr<Store>& store,
  ...
  if (std::holds_alternative<StreamId>(version_info)) {
    ...
    read_indexed_keys_to_pipeline(store, pipeline_context,
                                  std::get<VersionedItem>(version_info), read_query,
                                  read_options);
  }

  if (pipeline_context->multi_key_) {
    user_input::check<ErrorCode::E_INVALID_USER_ARGUMENT>(
        !read_query.columns || (!pipeline_context->only_index_columns_selected() &&
                                !read_query.columns->empty()),
        "Reading the index column is not supported when recursive or custom "
  ...
  if (opt_false(read_options.incompletes_)) {
    ...
    if (!existing_range.specified_ || query_range.end_ > existing_range.end_)
      read_incompletes_to_pipeline(store, pipeline_context, read_query, read_options,
                                   false, false, false);
  }

  if (std::holds_alternative<StreamId>(version_info) &&
      !pipeline_context->incompletes_after_) {
    missing_data::raise<ErrorCode::E_NO_SYMBOL_DATA>(
        "read_dataframe_impl: read returned no data for symbol {} (found no versions "
        "or append data)",
  ...
  auto frame = do_direct_read_or_process(store, read_query, read_options,
                                         pipeline_context, buffers);
...
VersionedItem collate_and_write(
    const std::shared_ptr<Store>& store,
    const std::shared_ptr<PipelineContext>& pipeline_context,
    const std::vector<FrameSlice>& slices, std::vector<VariantKey> keys,
    size_t append_after,
    const std::optional<arcticdb::proto::descriptors::UserDefinedMetadata>& user_meta) {
  util::check(keys.size() == slices.size(),
              "Mismatch between slices size and key size");
  TimeseriesDescriptor tsd;
  ...
  return util::variant_match(index, [&store, &pipeline_context, &slices, &keys,
                                     &append_after, &tsd](auto idx) {
    using IndexType = decltype(idx);
    ...
    ARCTICDB_DEBUG(log::version(), "Adding {} existing keys and {} new keys: ",
...
VersionedItem sort_merge_impl(
    ...
    const UpdateInfo& update_info, const CompactIncompleteOptions& options) {
  auto pipeline_context = std::make_shared<PipelineContext>();
  ...
  if (options.append_ && update_info.previous_index_key_.has_value()) {
    read_indexed_keys_to_pipeline(store, pipeline_context,
                                  *(update_info.previous_index_key_), read_query,
                                  ReadOptions{});
  ...
  read_incompletes_to_pipeline(store, pipeline_context, read_query, ReadOptions{},
                               options.convert_int_to_float_, options.via_iteration_,
                               options.sparsify_);

  std::vector<entity::VariantKey> delete_keys;
  for (auto sk = pipeline_context->incompletes_begin(); sk != pipeline_context->end();
       ++sk) {
    const auto& slice_and_key = sk->slice_and_key();
    util::check(slice_and_key.key().type() == KeyType::APPEND_DATA,
                "Deleting incorrect key type {}", slice_and_key.key().type());
  ...
  util::variant_match(
      ...
      [&](const stream::TimeseriesIndex& timeseries_index) {
        read_query.clauses_.emplace_back(
            std::make_shared<Clause>(SortClause{timeseries_index.name()}));
        read_query.clauses_.emplace_back(
            std::make_shared<Clause>(RemoveColumnPartitioningClause{}));
        // const auto split_size = ConfigsMap::instance()->get_int("Split.RowCount",
        // 10000);
        ...
        read_query.clauses_.emplace_back(std::make_shared<Clause>(
            MergeClause{timeseries_index, SparseColumnPolicy{}, stream_id,
                        pipeline_context->descriptor()}));
        auto segments =
            read_and_process(store, pipeline_context, read_query, ReadOptions{},
                             pipeline_context->incompletes_after());
        if (options.append_ && update_info.previous_index_key_.has_value() &&
            update_info.previous_index_key_->end_time() - 1 >
                std::get<timestamp>(TimeseriesIndex::start_value_for_segment(
                    segments[0].segment_.value()))) {
        ...
        pipeline_context->total_rows_ =
            num_versioned_rows + get_slice_rowcounts(segments);

        auto index = index_type_from_descriptor(pipeline_context->descriptor());
        stream::SegmentAggregator<TimeseriesIndex, DynamicSchema, RowCountSegmentPolicy,
                                  SparseColumnPolicy>
            aggregator{[&slices](FrameSlice&& slice) {
                         slices.emplace_back(std::move(slice));
                       },
                       DynamicSchema{pipeline_context->descriptor(), index},
                       [pipeline_context = pipeline_context, &fut_vec,
                        &store](SegmentInMemory&& segment) {
                         auto local_index_start =
                             TimeseriesIndex::start_value_for_segment(segment);
                         auto local_index_end =
                             TimeseriesIndex::end_value_for_segment(segment);
                         stream::StreamSink::PartialKey pk{
                             KeyType::TABLE_DATA, pipeline_context->version_id_,
                             pipeline_context->stream_id_, local_index_start,
                             local_index_end};
                         fut_vec.emplace_back(store->write(pk, std::move(segment)));
                       }};

        for (auto sk = segments.begin(); sk != segments.end(); ++sk) {
          aggregator.add_segment(std::move(sk->segment(store)), sk->slice(),
                                 options.convert_int_to_float_);

          sk->unset_segment();
        }
        aggregator.commit();
        pipeline_context->desc_->set_sorted(
            deduce_sorted(previous_sorted_value.value_or(SortedValue::ASCENDING),
                          SortedValue::ASCENDING));
      },
      [&](const auto&) {
        util::raise_rte("Sort merge only supports datetime indexed data. You data does "
                        "not have a datetime index.");
      });

  auto keys = folly::collect(fut_vec).get();
  auto vit = collate_and_write(store, pipeline_context, slices, keys,
                               pipeline_context->incompletes_after(), norm_meta);
...
VersionedItem compact_incomplete_impl(
  ...
  read_incompletes_to_pipeline(store, pipeline_context, ReadQuery{}, ReadOptions{},
                               options.convert_int_to_float_, options.via_iteration_,
                               options.sparsify_);
  ...
  for (auto sk = pipeline_context->incompletes_begin(); sk != pipeline_context->end();
    ...
    util::check(sk->slice_and_key().key().type() == KeyType::APPEND_DATA,
                "Deleting incorrect key type {}", sk->slice_and_key().key().type());
  ...
  auto policies = std::make_tuple(
      ...
      options.sparsify_ ? VariantColumnPolicy{SparseColumnPolicy{}}
                        : VariantColumnPolicy{DenseColumnPolicy{}});

  util::variant_match(
      std::move(policies), [&fut_vec, &slices, pipeline_context = pipeline_context,
                            &store, &options, &previous_sorted_value, &write_options](
                               auto&& idx, auto&& schema, auto&& column_policy) {
        ...
        do_compact<IndexType, SchemaType, RowCountSegmentPolicy, ColumnPolicyType>(
            pipeline_context->incompletes_begin(), pipeline_context->end(),
            pipeline_context, fut_vec, slices, store, options.convert_int_to_float_,
  ...
  auto keys = folly::collect(fut_vec).get();
  auto vit = collate_and_write(store, pipeline_context, slices, keys,
                               pipeline_context->incompletes_after(), user_meta);

  store->remove_keys(delete_keys).get();
  return vit;
}

PredefragmentationInfo get_pre_defragmentation_info(const std::shared_ptr<Store>& store,
                                                    const StreamId& stream_id,
                                                    const UpdateInfo& update_info,
                                                    const WriteOptions& options,
                                                    size_t segment_size) {
  util::check(update_info.previous_index_key_.has_value(),
              "No latest undeleted version found for data compaction");
  ...
  for (auto it = slice_and_keys.begin(); it != slice_and_keys.end(); ++it) {
    ...
    if (slice.col_range.start() == pipeline_context->descriptor()
                                       .index()
                                       .field_count()) { // where data column starts
    ...
    ++segment_idx;
    if (compaction_start_info &&
        slice.row_range.start() < compaction_start_info->first) {
      auto start_point = std::lower_bound(
          first_col_segment_idx.begin(), first_col_segment_idx.end(),
          slice.row_range.start(), [](auto lhs, auto rhs) { return lhs.first < rhs; });
      if (start_point != first_col_segment_idx.end())
        compaction_start_info = *start_point;
      else {
        log::version().warn("Missing segment containing column 0 for row {}; Resetting "
                            "compaction starting point to 0",
                            slice.row_range.start());
        compaction_start_info = {0u, 0u};
      }
    }
  }
  return {pipeline_context, read_query,
          first_col_segment_idx.size() - num_to_segments_after_compact,
          compaction_start_info
              ? std::make_optional<size_t>(compaction_start_info->second)
              : std::nullopt};
}
...
VersionedItem defragment_symbol_data_impl(const std::shared_ptr<Store>& store,
                                          const StreamId& stream_id,
                                          const UpdateInfo& update_info,
                                          const WriteOptions& options,
                                          size_t segment_size) {
  ...
  std::vector<folly::Future<VariantKey>> fut_vec;
  std::vector<FrameSlice> slices;
  const auto index = index_type_from_descriptor(
      pre_defragmentation_info.pipeline_context->descriptor());
  auto policies = std::make_tuple(
      index, options.dynamic_schema
                 ? VariantSchema{DynamicSchema::default_schema(index, stream_id)}
                 : VariantSchema{FixedSchema::default_schema(index, stream_id)});

  util::variant_match(std::move(policies), [&fut_vec, &slices, &store, &options,
                                            &pre_defragmentation_info,
                                            segment_size = segment_size](
                                               auto&& idx, auto&& schema) {
    pre_defragmentation_info.read_query.clauses_.emplace_back(
        std::make_shared<Clause>(RemoveColumnPartitioningClause{}));
    auto segments = read_and_process(store, pre_defragmentation_info.pipeline_context,
    ...
    do_compact<IndexType, SchemaType, RowCountSegmentPolicy, DenseColumnPolicy>(
        segments.begin(), segments.end(), pre_defragmentation_info.pipeline_context,
        fut_vec, slices, store, false, segment_size);
  });

  auto keys = folly::collect(fut_vec).get();
  auto vit =
      collate_and_write(store, pre_defragmentation_info.pipeline_context, slices, keys,
                        pre_defragmentation_info.append_after.value(), std::nullopt);
