...
void read_indexed_keys_to_pipeline(
    ...
    ARCTICDB_DEBUG(log::version(), "read_indexed_keys_to_pipeline: Symbol {} Found {} keys with {} total rows",
                   version_info.symbol(), pipeline_context->slice_and_keys_.size(), pipeline_context->total_rows_);
}
