...
get_latest_undeleted_version(const std::shared_ptr<Store>& store,
  ...
  const auto entry =
      version_map->check_reload(store, stream_id, load_strategy, __FUNCTION__);
  return entry->get_first_index(false).first;
}

inline std::pair<std::optional<AtomKey>, bool>
get_latest_version(const std::shared_ptr<Store>& store,
                   const std::shared_ptr<VersionMap>& version_map,
                   const StreamId& stream_id) {
...
// The next version ID returned will be 0 for brand new symbols, or one greater than the
// largest ever version created so far
...
get_all_versions(const std::shared_ptr<Store>& store,
                 const std::shared_ptr<VersionMap>& version_map,
                 const StreamId& stream_id) {
...
inline std::optional<AtomKey>
get_specific_version(const std::shared_ptr<Store>& store,
                     const std::shared_ptr<VersionMap>& version_map,
                     const StreamId& stream_id, SignedVersionId signed_version_id,
  ...
  if (signed_version_id >= 0) {
    ...
    if (opt_latest.has_value()) {
      auto opt_version_id =
          get_version_id_negative_index(opt_latest->version_id(), signed_version_id);
...
inline bool get_matching_prev_and_next_versions(
    ...
    MatchingAcceptor matching_acceptor, PrevAcceptor prev_acceptor,
    NextAcceptor next_acceptor, KeyFilter key_filter) {
...
inline void insert_if_undeleted(const std::shared_ptr<Store>& store,
                                const std::shared_ptr<VersionMap>& version_map,
                                const VariantKey& key, std::set<StreamId>& res) {
  auto id = variant_key_id(key);
  if (has_undeleted_version(store, version_map, id))
    res.insert(std::move(id));
}

inline std::unordered_map<VersionId, bool>
get_all_tombstoned_versions(const std::shared_ptr<Store>& store,
                            const std::shared_ptr<VersionMap>& version_map,
                            const StreamId& stream_id) {
...
inline version_store::TombstoneVersionResult
tombstone_version(const std::shared_ptr<Store>& store,
                  const std::shared_ptr<VersionMap>& version_map,
  ...
  ARCTICDB_DEBUG(log::version(), "Tombstoning version {} for stream {}", version_id,
                 stream_id);
  LoadStrategy load_strategy{LoadType::ALL, LoadObjective::UNDELETED_ONLY};
  auto entry = version_map->check_reload(store, stream_id, load_strategy, __FUNCTION__);
  // Might as well do the previous/next version check while we find the required
  // version_id. But if entry is empty, it's possible the load failed (since
  // iterate_on_failure=false above), so set the flag to defer the check to
  // delete_tree() (instead of reloading in case eager delete is disabled).
  ...
  get_matching_prev_and_next_versions(
      ...
      is_live_index_type_key // Entry could be cached with deleted keys even if
                             // LOAD_UNDELETED
  );
  ...
  if (res.keys_to_delete.empty()) {
    ...
    if (entry->is_tombstoned(version_id)) {
      ...
      if (!allow_tombstoning_beyond_latest_version) {
        ...
        if (!latest_key || latest_key->version_id() < version_id)
          util::raise_rte("Can't delete version {} for symbol {} - it's higher than "
                          "the latest version",
    ...
    version_map->write_tombstone(store, res.keys_to_delete[0], stream_id, entry,
                                 creation_ts);
  }
...
inline std::optional<AtomKey>
get_index_key_from_time(timestamp from_time, const std::vector<AtomKey>& keys) {
  auto at_or_after = std::lower_bound(
      std::begin(keys), std::end(keys), from_time,
      [](const AtomKey& v_key, timestamp cmp) { return v_key.creation_ts() > cmp; });
...
inline std::optional<AtomKey>
load_index_key_from_time(const std::shared_ptr<Store>& store,
                         const std::shared_ptr<VersionMap>& version_map,
                         const StreamId& stream_id, timestamp from_time) {
  LoadStrategy load_strategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                             from_time};
  auto entry = version_map->check_reload(store, stream_id, load_strategy, __FUNCTION__);
...
inline std::vector<AtomKey>
get_index_and_tombstone_keys(const std::shared_ptr<Store>& store,
                             const std::shared_ptr<VersionMap>& version_map,
                             const StreamId& stream_id) {
  LoadStrategy load_strategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED};
  const auto entry =
...
inline std::set<StreamId> list_streams(const std::shared_ptr<Store>& store,
  ...
  if (prefix && store->supports_prefix_matching()) {
    ...
    store->iterate_type(
        KeyType::VERSION_REF, [&store, &res, &version_map, all_symbols](auto&& vk) {
