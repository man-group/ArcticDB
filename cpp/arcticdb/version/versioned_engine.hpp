...
struct ReadVersionOutput {
  ...
  ReadVersionOutput(VersionedItem&& versioned_item,
 ...
 *
 * It is currently implemented by LocalVersionedEngine, but could also be implemented
 * with a RemoteVersionedEngine that sends Protobufs down a wire (to a server with a
 * LocalVersionedEngine talking directly to the storage).
 */
...
public:
  ...
  virtual VersionedItem delete_range_internal(const StreamId& stream_id,
                                              const UpdateQuery& query,
                                              const DeleteRangeOptions& option) = 0;

  virtual VersionedItem sort_index(const StreamId& stream_id, bool dynamic_schema,
                                   bool prune_previous_versions = false) = 0;

  virtual void append_incomplete_frame(const StreamId& stream_id,
                                       const std::shared_ptr<InputTensorFrame>& frame,
                                       bool validate_index) const = 0;

  virtual void append_incomplete_segment(const StreamId& stream_id,
                                         SegmentInMemory&& seg) = 0;

  virtual void remove_incomplete(const StreamId& stream_id) = 0;

  virtual void write_parallel_frame(const StreamId& stream_id,
                                    const std::shared_ptr<InputTensorFrame>& frame,
                                    bool validate_index) const = 0;

  /**
   * Delete the given index keys, and their associated data excluding those shared with
   * keys not in the argument.
   *
  ...
  virtual VersionedItem write_versioned_dataframe_internal(
      ...
      bool prune_previous_versions, bool allow_sparse, bool validate_index) = 0;

  /** Test-specific cut-down version of write_versioned_dataframe_internal */
  virtual VersionedItem write_individual_segment(const StreamId& stream_id,
                                                 SegmentInMemory&& segment,
                                                 bool prune_previous_versions) = 0;

  virtual std::set<StreamId>
  ...
  virtual IndexRange get_index_range(const StreamId& stream_id,
  ...
  virtual bool is_symbol_fragmented(const StreamId& stream_id,
                                    std::optional<size_t> segment_size) = 0;

  virtual VersionedItem defragment_symbol_data(const StreamId& stream_id,
                                               std::optional<size_t> segment_size,
                                               bool prune_previous_versions) = 0;

  virtual void move_storage(KeyType key_type, timestamp horizon,
  ...
  [[nodiscard]] virtual const arcticdb::proto::storage::VersionStoreConfig&
  cfg() const = 0;
