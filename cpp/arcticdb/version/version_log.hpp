...
inline StreamDescriptor log_stream_descriptor(const StreamId& event) {
    return stream_descriptor(event, stream::RowCountIndex(), {});
};

inline void log_event(
        const std::shared_ptr<stream::StreamSink>& store, const StreamId& id, std::string action,
...
inline void log_write(const std::shared_ptr<stream::StreamSink>& store, const StreamId& symbol, VersionId version_id) {
    log_event(store, symbol, WriteVersionId, version_id);
}

inline void log_tombstone(
        const std::shared_ptr<stream::StreamSink>& store, const StreamId& symbol, VersionId version_id
) {
    log_event(store, symbol, TombstoneVersionId, version_id);
}

inline void log_tombstone_all(
        const std::shared_ptr<stream::StreamSink>& store, const StreamId& symbol, VersionId version_id
) {
    log_event(store, symbol, TombstoneAllVersionId, version_id);
}

inline void log_create_snapshot(const std::shared_ptr<stream::StreamSink>& store, const SnapshotId& snapshot_id) {
    log_event(store, snapshot_id, CreateSnapshotId);
}

inline void log_delete_snapshot(const std::shared_ptr<stream::StreamSink>& store, const SnapshotId& snapshot_id) {
