...
OpLog::OpLog(AtomKey&& key)
    : id_(std::get<StringIndex>(key.start_index())), version_id_(key.version_id()),
      action_(std::get<StringId>(key.id())), creation_ts_(key.creation_ts()),
      content_hash_(key.content_hash()) {}

OpLog::OpLog(StringId id, VersionId version_id, const std::string& action,
             timestamp creation_ts)
    : id_(id), version_id_(version_id), action_(action), creation_ts_(creation_ts) {}
...
AtomKey OpLog::extract_key() {
  util::check(content_hash_.has_value(),
              "Cannot extract Atomkey from OpLog without content hash");
