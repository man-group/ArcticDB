...
void write_symbol_metadata(
    ...
    ARCTICDB_DEBUG(log::version(), "Writing metadata key for {} {} {} {} {}", symbol, start_index, end_index, total_rows, update_time);
    store->write_sync(KeyType::METRICS, VersionId{total_rows}, symbol, start_index, end_index, update_time, std::move(segment));
}
...
std::pair<SegmentInMemory, std::vector<AtomKey>> compact_metadata_keys(const std::shared_ptr<Store>& store) {
    ...
    }, NeverSegmentPolicy{}, std::nullopt, keys.size()};
...
void get_symbol_metadata_keys(
    ...
    log::version().info("Found {} metadata journal keys", journal_keys.size());
