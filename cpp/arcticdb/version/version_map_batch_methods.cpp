...
void StreamVersionData::do_react(std::monostate) {
    ++count_;
    load_strategy_ =
        union_of_undeleted_strategies(load_strategy_, LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY});
}
        ...
        if (num_snaps == 1) {
            ...
            if (fut == snapshot_futures.end()) {
                auto [splitter, _] = snapshot_futures.emplace(
                    ...
                    folly::FutureSplitter{store->read(*snapshot_key->second)
                                              .thenValue(
        ...
        if (maybe_fut == version_futures.end()) {
            auto [splitter, inserted] = version_futures.emplace(
                ...
                folly::FutureSplitter{
                    async::submit_io_task(CheckReloadTask{store, version_map, symbol, version_data.load_strategy_})
                        .thenValue([](std::shared_ptr<VersionMapEntry> version_map_entry) {
                            return VersionEntryOrSnapshot{std::move(version_map_entry)};
                        })
                }
...
std::vector<folly::Future<std::optional<AtomKey>>> batch_get_versions_async(
    ...
    for (const auto& symbol : folly::enumerate(symbols)) {
        ...
        output.push_back(
            std::move(version_entry_fut).thenValue([vq = version_query, sid = *symbol](auto version_or_snapshot) {
                return util::variant_match(
                    version_or_snapshot,
                    ...
                    [&sid](std::optional<SnapshotPair> snapshot) {
                        ...
                        return opt_id ? std::make_optional<AtomKey>(
                                            read_key_row(snap_segment, static_cast<ssize_t>(*opt_id))
                                        )
                                      : std::nullopt;
                    }
                );
