...
void StreamVersionData::do_react(
    const pipelines::SpecificVersionQuery& specific_version) {
  ++count_;
  load_strategy_ = union_of_undeleted_strategies(
      load_strategy_, LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                                   specific_version.version_id_});
}

void StreamVersionData::do_react(
    const pipelines::TimestampVersionQuery& timestamp_query) {
  ++count_;
  load_strategy_ = union_of_undeleted_strategies(
      load_strategy_, LoadStrategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                                   timestamp_query.timestamp_});
}
...
get_key_for_version_query(const std::shared_ptr<VersionMapEntry>& version_map_entry,
...
struct SnapshotCountMap {
  ...
  explicit SnapshotCountMap(
      const ankerl::unordered_dense::map<StreamId, StreamVersionData>& version_data) {
  ...
  size_t get_size(const SnapshotId& snapshot) {
    ...
    util::check(it != snapshot_counts_.end(), "Missing snapshot data for snapshot {}",
                snapshot);
    return it->second;
  }
};
...
using VersionEntryOrSnapshot =
    std::variant<std::shared_ptr<VersionMapEntry>, std::optional<SnapshotPair>>;
...
folly::Future<VersionEntryOrSnapshot> set_up_snapshot_future(
  ...
  util::check(snapshot_key != std::end(*snapshot_key_map),
              "Missing snapshot data for snapshot {}", snapshot_query.name_);
  if (!snapshot_key->second) {
    return folly::makeFuture(std::make_optional<SnapshotPair>());
  } else {
    if (num_snaps == 1) {
      return store->read(*snapshot_key->second)
          .thenValue([](SnapshotPair&& snapshot_pair) {
                    ...
                    .thenValue(
                        [snap_key = *snapshot_key->second](
                            std::pair<VariantKey, SegmentInMemory>
                                snapshot_output) mutable -> VersionEntryOrSnapshot {
                          return SnapshotPair{std::move(snap_key),
                                              std::move(snapshot_output.second)};
                        })});
  ...
  if (version_data.count_ == 1) {
    ...
    if (maybe_fut == version_futures.end()) {
      auto [splitter, inserted] = version_futures.emplace(
          symbol,
          folly::FutureSplitter{
              async::submit_io_task(CheckReloadTask{store, version_map, symbol,
                                                    version_data.load_strategy_})
                  .thenValue([](std::shared_ptr<VersionMapEntry> version_map_entry) {
                    return VersionEntryOrSnapshot{std::move(version_map_entry)};
                  })});
...
batch_get_versions_async(const std::shared_ptr<Store>& store,
  ...
  util::check(symbols.size() == version_queries.size(),
              "Symbol and version query list mismatch: {} != {}", symbols.size(),
  ...
  auto snapshot_key_map = std::make_shared<SnapshotKeyMap>(
      get_keys_for_snapshots(store, snapshot_count_map->snapshots()));
  ...
  for (const auto& symbol : folly::enumerate(symbols)) {
    ...
    util::variant_match(
        version_query.content_,
        [&version_entry_fut, &snapshot_count_map, &snapshot_key_map, &snapshot_futures,
         &store](const pipelines::SnapshotVersionQuery& snapshot_query) {
          version_entry_fut =
              set_up_snapshot_future(snapshot_futures, snapshot_count_map,
                                     snapshot_key_map, snapshot_query, store);
        },
        [&version_entry_fut, &version_data, &symbol, &version_futures, &store,
         &version_map](const auto&) {
          const auto it = version_data.find(*symbol);
          util::check(it != version_data.end(), "Missing version data for symbol {}",
                      *symbol);

          version_entry_fut = set_up_version_future(
              *symbol, it->second, version_futures, store, version_map);
        });

    output.push_back(
        std::move(version_entry_fut)
              ...
              return util::variant_match(
                  ...
                  [&sid](std::optional<SnapshotPair> snapshot) {
                    ...
                    return opt_id ? std::make_optional<AtomKey>(read_key_row(
                                        snap_segment,
                                        static_cast<ssize_t>(opt_id.value())))
                                  : std::nullopt;
                  });
            }));
  }
