...
void StreamVersionData::do_react(std::monostate) {
    ++count_;
    load_param_.load_strategy_ = union_of_undeleted_strategies(load_param_.load_strategy_, LoadStrategy{LoadType::LOAD_LATEST, false});
}

void StreamVersionData::do_react(const pipelines::SpecificVersionQuery &specific_version) {
    ++count_;
    load_param_.load_strategy_ = union_of_undeleted_strategies(load_param_.load_strategy_, LoadStrategy{LoadType::LOAD_DOWNTO, false, specific_version.version_id_});
}

void StreamVersionData::do_react(const pipelines::TimestampVersionQuery &timestamp_query) {
    ++count_;
    load_param_.load_strategy_ = union_of_undeleted_strategies(load_param_.load_strategy_, LoadStrategy{LoadType::LOAD_FROM_TIME, false, timestamp_query.timestamp_});
}
