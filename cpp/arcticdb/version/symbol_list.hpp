...
struct SymbolListData {
  ...
  explicit SymbolListData(std::shared_ptr<VersionMap> version_map,
                          StreamId type_indicator = StringId(), uint32_t seed = 0);
};
...
struct SymbolEntryData {
  ...
  SymbolEntryData(entity::VersionId reference_id, timestamp time, ActionType action)
      : reference_id_(reference_id), timestamp_(time), action_(action) {}
...
struct SymbolListEntry : public SymbolEntryData {
  ...
  SymbolListEntry(StreamId stream_id, entity::VersionId reference_id,
                  timestamp reference_time, ActionType action)
...
struct ProblematicResult {
  ...
  explicit ProblematicResult(bool old_style_refs)
      : contains_unknown_reference_ids_(old_style_refs) {}
...
public:
  explicit SymbolList(std::shared_ptr<VersionMap> version_map,
                      StreamId type_indicator = StringId(), uint32_t seed = 0)
      : data_(std::move(version_map), std::move(type_indicator), seed) {}

  std::set<StreamId> load(const std::shared_ptr<VersionMap>& version_map,
                          const std::shared_ptr<Store>& store, bool no_compaction);

  std::vector<StreamId> get_symbols(const std::shared_ptr<Store>& store,
                                    bool no_compaction = false) {
    auto symbols = load(data_.version_map_, store, no_compaction);
    return {std::make_move_iterator(symbols.begin()),
  ...
  static void add_symbol(const std::shared_ptr<Store>& store, const StreamId& symbol,
                         entity::VersionId reference_id);

  static void remove_symbol(const std::shared_ptr<Store>& store, const StreamId& symbol,
                            entity::VersionId reference_id);

  static void clear(const std::shared_ptr<Store>& store);

private:
  void compact_internal(const std::shared_ptr<Store>& store,
...
struct WriteSymbolTask : async::BaseTask {
  ...
  WriteSymbolTask(std::shared_ptr<Store> store, std::shared_ptr<SymbolList> symbol_list,
                  StreamId stream_id, entity::VersionId reference_id)
      : store_(std::move(store)), symbol_list_(std::move(symbol_list)),
        stream_id_(std::move(stream_id)), reference_id_(reference_id) {}
...
template <> struct formatter<arcticdb::ActionType> {
  ...
  auto format(arcticdb::ActionType a, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), "{}",
                          a == arcticdb::ActionType::ADD ? "ADD" : "DELETE");
  }
};

template <> struct formatter<arcticdb::SymbolEntryData> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
  ...
  auto format(const arcticdb::SymbolEntryData& s, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(), "[{},{}@{}]", s.reference_id_, s.action_,
                          s.timestamp_);
  }
};
