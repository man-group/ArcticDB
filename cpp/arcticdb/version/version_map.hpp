...
class VersionMapImpl {
  ...
  public:
    ...
    std::optional<AtomKey> do_write(
        ...
        storage::check<ErrorCode::E_NON_INCREASING_INDEX_VERSION>(
                key.type() != KeyType::TABLE_INDEX || !entry->head_.has_value() ||
                        key.version_id() > entry->head_->version_id(),
                "Trying to write TABLE_INDEX key with a non-increasing version. New version: {}, Last version: {}. "
                "This is most likely due to parallel writes to the same symbol, which is not supported.",
                key.version_id(),
    ...
    AtomKey write_tombstone(
            std::shared_ptr<Store> store, const std::variant<AtomKey, VersionId>& key, const StreamId& stream_id,
