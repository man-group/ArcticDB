...
public:
    ...
    bool has_cached_entry(const StreamId &stream_id, const LoadParameter& load_param) const {
        ...
        util::check(requested_load_type == LoadType::NOT_LOADED
                    || requested_load_type == LoadType::LOAD_LATEST
                    || requested_load_type == LoadType::LOAD_DOWNTO
                    || requested_load_type == LoadType::LOAD_LATEST_UNDELETED
                    || requested_load_type == LoadType::LOAD_UNDELETED
                    || requested_load_type == LoadType::LOAD_FROM_TIME
                    || requested_load_type == LoadType::LOAD_ALL, "Unexpected load type requested {}", requested_load_type);
        ...
        switch(cached_load_type) {
            ...
            case LoadType::LOAD_LATEST:
                if (requested_load_type == LoadType::NOT_LOADED || requested_load_type == LoadType::LOAD_LATEST) {
                    return true;
                }
