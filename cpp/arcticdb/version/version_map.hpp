  ...
  public:
    ...
    VariantKey journal_single_key(
        ...
        IndexAggregator<RowCountIndex> journal_agg(key.id(), [&store, &journal_key, &key](auto&& segment) {
            stream::StreamSink::PartialKey pk{
                KeyType::VERSION, key.version_id(), key.id(), IndexValue(NumericIndex{0}), IndexValue(NumericIndex{0})
            };
    ...
    std::optional<AtomKey> do_write(
        ...
        util::check(
            ...
            key.version_id(),
            entry->head_ ? entry->head_->version_id() : VariantId{""}
        );
    ...
    void remove_entry_version_keys(
        ...
        if (entry.head_) {
            util::check(
                entry.head_->id() == stream_id, "Id mismatch for entry {} vs symbol {}", entry.head_->id(), stream_id
            );
  ...
  private:
    std::shared_ptr<VersionMapEntry> compact_entry(
        ...
        new_entry->keys_.erase(
            std::remove_if(
                parent + 1, std::end(new_entry->keys_), [](const auto& k) { return k.type() == KeyType::VERSION; }
            ),
            std::end(new_entry->keys_)
        );
    ...
    AtomKey write_entry_to_storage(
        ...
        IndexAggregator<RowCountIndex> version_agg(
            ...
            [&store, &journal_key, &version_id, &stream_id](auto&& segment) {
                stream::StreamSink::PartialKey pk{
                    KeyType::VERSION, version_id, stream_id, IndexValue(NumericIndex{0}), IndexValue(NumericIndex{0})
                };
    ...
    std::shared_ptr<VersionMapEntry> storage_reload(
        ...
        entry->load_strategy_ = LoadStrategy{
            LoadType::NOT_LOADED, LoadObjective::INCLUDE_DELETED
        }; // FUTURE: to make more thread-safe with #368
  ...
  public:
    bool check_ref_key(std::shared_ptr<Store> store, const StreamId& stream_id) {
        ...
        try {
            ...
            log::version().warn(
                "Loading versions from storage via ref key failed with error: {} for stream {}", err.what(), stream_id
            );
            return false;
        }
        return true;
