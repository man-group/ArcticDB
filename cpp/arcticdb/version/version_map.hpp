...
/*
 * version_map.hpp contains VersionMap which at it's core is a map of {Stream_id:
 * VersionMapEntry} (see version_map_entry.hpp) (see VersionMapImpl for details)
 *
 */
#pragma once
...
template <class Clock = util::SysClock> class VersionMapImpl {
  /*
   * VersionMap at it's core is an in-memory map of {StreamId: VersionMapEntry}.
   *
   * It contains utility functions for working with retrieving and storing in-memory
   * version state from the structure in storage, which are used by PythonVersionStore
   * to abstract all the versioning related operations.
   *
   * The core functionality here is functions like:
   *
   * get_latest_version, get_all_versions, get_specific version, write_version and
   * delete_version.
   *
   * This allows us to move all the implementation details of how versions live in
   * memory and storage outside PythonVersionStore which is just an orchestrator.
   *
   ...
   *
   * When a version is tombstoned, e.g. lib.write('a', 1), lib.write('a', 2) with
   * pruning will tombstone the first version which creates a new version key on storage
   * in the same order of the timeline of operations and that key will have a tombstone
   * key type in its segment instead of an index key, and a version key pointing to the
   * next version key.
   *
   * Note that there is no TOMBSTONE key type in the storage, it's just an indicator
   * inside a version key that tells us that this version has been deleted or pruned
   * with tombstones enabled.
   *
   ...
   *     |
   *     |
   * Version Keys: ['sym',v1| [i1|v0]] (ts2) -- ['sym',v0| [v0|t0]] (ts1) --  ['sym',v2|
   * [i0]] (ts0)
   *
   * We also have methods to compact this linked list on storage if it becomes too large
   * on disk, which would lead us to do multiple reads from storage if not compacted.
   *
   * FALLBACK TO ITERATION
   * We also have an alternative method to fetch all version keys which is to fall back
   * to iterating the storage to basically just fetch all relevant key types, which is
   * useful in case we have consistency issues in the ref keys.
   *
   * This class also has utils to check and fix the ref key structure using the
   * iteration method from storage.
   *
   * Note that VERSION_JOURNAL is a key type which is only there for backwards
   * compatibility reasons and is never used in for new libraries.
   *
   * CACHING in VERSION MAP
   * when someone requests the latest version, we do have a grace period of
   * DEFAULT_RELOAD_INTERVAL where we will just use the data in the in memory map if it
   * exists rather than reading the ref key from the storage.
   *
   */

  /**
   * If we one day replace the String in StreamId with something cheap to copy again, we
   * can easily drop the & here.
   *
...
public:
  ...
  void follow_version_chain(const std::shared_ptr<Store>& store,
    ...
    if (ref_entry.keys_.size() == 3) {
      util::check(is_index_or_tombstone(ref_entry.keys_[1]),
  ...
  void load_via_ref_key(std::shared_ptr<Store> store, const StreamId& stream_id,
                        const LoadStrategy& load_strategy,
                        const std::shared_ptr<VersionMapEntry>& entry) {
    load_strategy.validate();
    static const auto max_trial_config =
        ConfigsMap::instance()->get_int("VersionMap.MaxReadRefTrials", 2);
    ...
    while (true) {
      try {
        ...
        // We retry to read via ref key because it could have been modified by someone
        // else (e.g. compaction)
        log::version().warn("Loading versions from storage via ref key failed with "
                            "error: {} for stream {}. Retrying",
                            err.what(), stream_id);
        entry->head_.reset();
        entry->keys_.clear();
        continue;
  ...
  void load_via_iteration(std::shared_ptr<Store> store, const StreamId& stream_id,
    ...
    auto match_stream_id = [&stream_id](const AtomKey& k) {
      return k.id() == stream_id;
    };
    entry = build_version_map_entry_with_predicate_iteration(
        store, match_stream_id, stream_id,
        use_index_keys_for_iteration
            ? std::vector<KeyType>{KeyType::TABLE_INDEX, KeyType::MULTI_KEY}
  ...
  /**
   * Tombstone all non-deleted versions of the given stream and do the related
   * housekeeping.
   * @param first_key_to_tombstone The first key in the version chain that should be
   * tombstoned. When empty then the first index key onwards is tombstoned, so the whole
   * chain is tombstoned.
   */
  std::pair<VersionId, std::vector<AtomKey>> tombstone_from_key_or_all(
    ...
    auto entry = check_reload(
        ...
        __FUNCTION__);
    auto output = tombstone_from_key_or_all_internal(store, stream_id,
                                                     first_key_to_tombstone, entry);

    if (validate_)
      entry->validate();

    if (entry->head_)
      write_symbol_ref(store, *entry->keys_.cbegin(), std::nullopt,
                       entry->head_.value());

    return output;
  }

  std::string dump_entry(const std::shared_ptr<Store>& store,
  ...
  std::vector<AtomKey>
  write_and_prune_previous(std::shared_ptr<Store> store, const AtomKey& key,
                           const std::optional<AtomKey>& previous_key) {
    ARCTICDB_DEBUG(log::version(),
                   "Version map pruning previous versions for stream {}", key.id());
    auto entry = check_reload(
        store, key.id(), LoadStrategy{LoadType::ALL, LoadObjective::UNDELETED_ONLY},
        __FUNCTION__);
    auto [_, result] =
        tombstone_from_key_or_all_internal(store, key.id(), previous_key, entry);

    auto previous_index = do_write(store, key, entry);
    write_symbol_ref(store, *entry->keys_.cbegin(), previous_index,
                     entry->head_.value());
  ...
  std::pair<VersionId, std::deque<AtomKey>>
  delete_all_versions(std::shared_ptr<Store> store, const StreamId& stream_id) {
  ...
  bool requires_compaction(const std::shared_ptr<VersionMapEntry>& entry) const {
    int64_t num_blocks = std::count_if(
        entry->keys_.cbegin(), entry->keys_.cend(),
        [](const AtomKey& key) { return key.type() == KeyType::VERSION; });

    static const auto max_blocks =
        ConfigsMap::instance()->get_int("VersionMap.MaxVersionBlocks", 5);
  ...
  void compact_and_remove_deleted_indexes(std::shared_ptr<Store> store,
                                          const StreamId& stream_id) {
    // This method has no API, and is not tested in the rapidcheck tests, but could
    // easily be enabled there. It compacts the version map but skips any keys which
    // have been deleted (to free up space).
    ...
    auto latest_version =
        std::find_if(std::begin(entry->keys_), std::end(entry->keys_),
    ...
    if (const auto first_is_tombstone = entry->get_tombstone(new_version_id);
        first_is_tombstone)
        ...
        if (tombstone) {
          if (!store->key_exists(key).get())
            ...
            if (tombstone.value().type() == KeyType::TOMBSTONE_ALL)
              ...
              new_entry->tombstones_.insert(
                  std::make_pair(key.version_id(), tombstone.value()));
    ...
    new_entry->head_ =
        write_entry_to_storage(store, stream_id, new_version_id, new_entry);
  ...
  VariantKey journal_single_key(std::shared_ptr<StreamSink> store, const AtomKey& key,
    ...
    IndexAggregator<RowCountIndex> journal_agg(
        key.id(), [&store, &journal_key, &key](auto&& segment) {
          stream::StreamSink::PartialKey pk{KeyType::VERSION, key.version_id(),
                                            key.id(), IndexValue(NumericIndex{0}),
  ...
  AtomKey update_version_key(std::shared_ptr<Store> store,
    ...
    IndexAggregator<RowCountIndex> version_agg(stream_id, [&journal_key_fut, &store,
                                                           &version_key](
                                                              auto&& segment) {
      journal_key_fut =
          store->update(version_key, std::forward<decltype(segment)>(segment)).wait();
    });
  ...
  void compact_if_necessary_stand_alone(const std::shared_ptr<Store>& store,
                                        size_t batch_size) {
    ...
    for (const auto& [symbol, size] : map) {
      ...
      if (num_sym_compacted % 50 == 0) {
        ARCTICDB_RUNTIME_DEBUG(log::version(), "Compacted {} symbols",
                               num_sym_compacted);
      }
    }
    ARCTICDB_RUNTIME_DEBUG(log::version(), "Compacted {} out of {} total symbols",
                           num_sym_compacted, total_symbols);
  }
  ...
  void overwrite_symbol_tree(std::shared_ptr<Store> store, const StreamId& stream_id,
                             const std::vector<AtomKey>& index_keys) {
    auto entry = check_reload(
        store, stream_id, LoadStrategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED},
        __FUNCTION__);
   ...
   * @param iterate_on_failure Use `iterate_type` (slow!) if the linked-list-based load
   * logic throws
   */
  std::shared_ptr<VersionMapEntry> check_reload(std::shared_ptr<Store> store,
    ...
    ARCTICDB_DEBUG(log::version(), "Check reload in function {} for id {}", function,
                   stream_id);
   ...
   */
  std::optional<AtomKey> do_write(std::shared_ptr<Store> store, const AtomKey& key,
                                  const std::shared_ptr<VersionMapEntry>& entry) {
    if (validate_)
      entry->validate();
  ...
  AtomKey write_tombstone(std::shared_ptr<Store> store,
    ...
    auto tombstone =
        write_tombstone_internal(store, key, stream_id, entry, creation_ts);
  ...
  void remove_entry_version_keys(const std::shared_ptr<Store>& store,
                                 ...
                                 const StreamId& stream_id) const {
    if (entry.head_) {
      util::check(entry.head_->id() == stream_id,
                  "Id mismatch for entry {} vs symbol {}", entry.head_->id(),
                  stream_id);
    ...
    for (const auto& key : entry.keys_) {
      util::check(key.id() == stream_id, "Id mismatch for entry {} vs symbol {}",
                  key.id(), stream_id);
   ...
   * @return whether we have a cached entry suitable for the load strategy, so do not
   * need to go to storage
   */
  bool has_cached_entry(const StreamId& stream_id,
                        const LoadStrategy& requested_load_strategy) const {
    LoadType requested_load_type = requested_load_strategy.load_type_;
    util::check(requested_load_type < LoadType::UNKNOWN,
                "Unexpected load type requested {}", requested_load_type);
    ...
    const timestamp reload_interval =
        reload_interval_.value_or(ConfigsMap::instance()->get_int(
            "VersionMap.ReloadInterval", DEFAULT_RELOAD_INTERVAL));

    const auto& entry = entry_it->second;
    if (const timestamp cache_timing = now() - entry->last_reload_time_;
        cache_timing > reload_interval) {
      ARCTICDB_DEBUG(log::version(),
                     "Latest read time {} too long ago for last acceptable cached "
                     "timing {} (cache period {}) for symbol {}",
                     entry->last_reload_time_, cache_timing, reload_interval,
                     stream_id);
    ...
    case LoadType::LATEST: {
      // If entry has at least one (maybe undeleted) index we have the latest value
      // cached

      // This check can be slow if we have thousands of deleted versions before the
      // first undeleted and we're looking for an undeleted version. If that is ever a
      // problem we can just store a boolean whether we have an undeleted version.
      auto opt_latest =
          entry->get_first_index(requested_load_strategy.should_include_deleted())
              .first;
    ...
    case LoadType::DOWNTO:
      ...
      return loaded_as_far_as_version_id(
          *entry, requested_load_strategy.load_until_version_.value());
    case LoadType::FROM_TIME: {
      // We check whether the cached (deleted or undeleted) timestamp is before or at
      // the requested one
      auto cached_timestamp =
    ...
    case LoadType::ALL:
      // We can use cache when it was populated by a ALL call, in which case it is only
      // unsafe to use when the cache is of undeleted versions and the request is for
      // all versions
      if (cached_load_type == LoadType::ALL) {
        return entry->load_strategy_.should_include_deleted() ||
               !requested_load_strategy.should_include_deleted();
      }
      return false;
    default:
...
private:
  ...
  compact_entry(std::shared_ptr<Store> store, const StreamId& stream_id,
                const std::shared_ptr<VersionMapEntry>& entry) {
    ...
    // The first version key is in head and the second version key is first in
    // entry.keys_
    if (validate_)
      entry->validate();
    util::check(entry->head_.value().type() == KeyType::VERSION,
                "Type of head must be version");
    auto new_entry = std::make_shared<VersionMapEntry>(*entry);

    auto parent =
        std::find_if(std::begin(new_entry->keys_), std::end(new_entry->keys_),
                     [](const auto& k) { return k.type() == KeyType ::VERSION; });

    // Copy version keys to be removed
    std::vector<VariantKey> version_keys_compacted;
    std::copy_if(parent + 1, std::end(new_entry->keys_),
    ...
    new_entry->keys_.erase(
        std::remove_if(parent + 1, std::end(new_entry->keys_),
                       [](const auto& k) { return k.type() == KeyType::VERSION; }),
  ...
  void write_to_entry(const std::shared_ptr<VersionMapEntry>& entry, const AtomKey& key,
   ...
   * Checks whether oldest_loaded_index_version_ in entry is earlier than that specified
   * in load_param.
   *
   * @param entry the version map state to check
   * @param load_param the load request to test for completeness
   * @return true if and only if entry already contains data at least as far back as
   * load_param requests
   */
  bool loaded_as_far_as_version_id(const VersionMapEntry& entry,
                                   SignedVersionId requested_version_id) const {
    if (requested_version_id >= 0) {
      if (entry.load_progress_.oldest_loaded_index_version_ <=
          static_cast<VersionId>(requested_version_id)) {
      ...
      if (opt_latest.has_value()) {
        ...
        if (opt_version_id.has_value() &&
          ...
          ARCTICDB_DEBUG(log::version(),
                         "Loaded as far as required value {}, have {} and there are {} "
                         "total versions",
                         requested_version_id,
                         entry.load_progress_.oldest_loaded_index_version_,
  ...
  std::shared_ptr<VersionMapEntry>& get_entry(const StreamId& stream_id) {
    ...
    return map_.try_emplace(stream_id, std::make_shared<VersionMapEntry>())
        .first->second;
  }

  AtomKey write_entry_to_storage(std::shared_ptr<Store> store,
                                 const StreamId& stream_id, VersionId version_id,
                                 const std::shared_ptr<VersionMapEntry>& entry) {
    AtomKey journal_key;
    entry->validate_types();

    IndexAggregator<RowCountIndex> version_agg(
        stream_id, [&store, &journal_key, &version_id, &stream_id](auto&& segment) {
          stream::StreamSink::PartialKey pk{KeyType::VERSION, version_id, stream_id,
                                            IndexValue(NumericIndex{0}),
                                            IndexValue(NumericIndex{0})};

          journal_key =
              to_atom(store->write_sync(pk, std::forward<decltype(segment)>(segment)));
        });
  ...
  std::shared_ptr<VersionMapEntry> storage_reload(std::shared_ptr<Store> store,
                                                  ...
                                                  const LoadStrategy& load_strategy) {
    /*
     * Goes to the storage for a given symbol, and recreates the VersionMapEntry from
     * preferably the ref key structure, and if that fails it then goes and builds that
     * from iterating all keys from storage which can be much slower, though always
     * consistent.
     */
    ...
    entry->load_strategy_ = LoadStrategy{
        LoadType::NOT_LOADED,
        LoadObjective::INCLUDE_DELETED}; // FUTURE: to make more thread-safe with #368
    ...
    util::check(entry->keys_.empty() || entry->head_,
                "Non-empty VersionMapEntry should set head");
    if (validate_)
      entry->validate();
  ...
  std::shared_ptr<VersionMapEntry>
  rewrite_entry(std::shared_ptr<Store> store, const StreamId& stream_id,
                const std::shared_ptr<VersionMapEntry>& entry) {
    auto new_entry = std::make_shared<VersionMapEntry>();
    std::copy_if(std::begin(entry->keys_), std::end(entry->keys_),
...
public:
  bool check_ref_key(std::shared_ptr<Store> store, const StreamId& stream_id) {
    ...
    if (maybe_latest_pair->first != ref_entry.keys_[0] ||
        maybe_latest_pair->second != *ref_entry.head_) {
      log::version().warn(
          "Ref entry is incorrect for stream {}, either {} != {} or {} != {}",
          stream_id, maybe_latest_pair->first, ref_entry.head_.value(),
          maybe_latest_pair->second, ref_entry.keys_[0]);
      return false;
    }

    try {
      auto entry_ref = std::make_shared<VersionMapEntry>();
      load_via_ref_key(store, stream_id,
  ...
  bool indexes_sorted(const std::shared_ptr<Store>& store, const StreamId& stream_id) {
    ...
    return std::is_sorted(std::cbegin(indexes), std::cend(indexes),
                          [](const auto& l, const auto& r) { return l > r; });
  }

  void scan_and_rewrite(const std::shared_ptr<Store>& store,
                        const StreamId& stream_id) {
    log::version().warn("Version map scanning and rewriting  versions for stream {}",
                        stream_id);
    auto entry = get_entry(stream_id);
    entry->clear();
  ...
  void remove_and_rewrite_version_keys(std::shared_ptr<Store> store,
                                       const StreamId& stream_id) {
        ...
        [&entry, &missing_versions](const auto& vk) {
          ...
          if (it == std::end(entry->keys_)) {
            ...
            if (!entry->head_ || std::tie(key.id(), key.version_id()) !=
                                     std::tie(entry->head_.value().id(),
                                              entry->head_.value().version_id()))
  ...
  std::vector<AtomKey> find_deleted_version_keys(std::shared_ptr<Store> store,
                                                 const StreamId& stream_id) {
  ...
  void recover_deleted(std::shared_ptr<Store> store, const StreamId& stream_id) {
    ...
    auto missing_versions =
        find_deleted_version_keys_for_entry(store, stream_id, entry);

    entry->keys_.insert(std::begin(entry->keys_), std::begin(missing_versions),
                        std::end(missing_versions));
    entry->sort();
    rewrite_entry(store, stream_id, entry);
  }
...
private:
  ...
  std::pair<VersionId, std::vector<AtomKey>> tombstone_from_key_or_all_internal(
    ...
    if (!entry) {
      entry = check_reload(store, stream_id,
    ...
    if (!output.empty()) {
      auto tombstone_key = write_tombstone_all_key_internal(
          store, first_key_to_tombstone.value(), entry);
      if (log_changes_) {
  ...
  // Invalidates the cached undeleted entry if it got tombstoned either by a tombstone
  // or by a tombstone_all
  void maybe_invalidate_cached_undeleted(VersionMapEntry& entry) {
    if (entry.is_tombstoned(
            entry.load_progress_.oldest_loaded_undeleted_index_version_)) {
      entry.load_progress_.oldest_loaded_undeleted_index_version_ =
  ...
  AtomKey
  write_tombstone_all_key_internal(const std::shared_ptr<Store>& store,
                                   const AtomKey& previous_key,
                                   const std::shared_ptr<VersionMapEntry>& entry) {
    auto tombstone_key =
        get_tombstone_all_key(previous_key, store->current_timestamp());
  ...
  AtomKey write_tombstone_internal(
      std::shared_ptr<Store> store, const std::variant<AtomKey, VersionId>& key,
      const StreamId& stream_id, const std::shared_ptr<VersionMapEntry>& entry,
      const std::optional<timestamp>& creation_ts = std::nullopt) {
    if (validate_)
      entry->validate();

    auto tombstone =
        util::variant_match(key, [&stream_id, store, &creation_ts](const auto& k) {
          return index_to_tombstone(k, stream_id,
                                    creation_ts.value_or(store->current_timestamp()));
        });
