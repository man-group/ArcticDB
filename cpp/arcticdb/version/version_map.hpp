    ...
    void compact_if_necessary_stand_alone(const std::shared_ptr<Store>& store, size_t batch_size) {
        ...
        for(const auto& [symbol, size] : map) {
            ...
            if (num_sym_compacted % 50 == 0) {
                ARCTICDB_DEBUG(log::version(), "Compacted {} symbols", num_sym_compacted);
            }
        }
        ARCTICDB_DEBUG(log::version(), "Compacted {} out of {} total symbols", num_sym_compacted, total_symbols);
    }
