...
template <typename Model>
void check_latest_undeleted_versions(const Model& s0, MapStorePair& sut,
                                     std::string symbol) {
  using namespace arcticdb;
...
template <typename Model> struct Compact : rc::state::Command<Model, MapStorePair> {
  std::string symbol_;
...
RC_GTEST_PROP(VersionMap, Rapidcheck, ()) {
  ...
  rc::state::check(
      ...
      rc::state::gen::execOneOfWithArgs<
          WriteVersion<VersionMapModel>, WriteAndPrunePreviousVersion<VersionMapModel>,
          GetLatestVersion<VersionMapModel>, GetAllVersions<VersionMapModel>,
