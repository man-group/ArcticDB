...
TEST(VersionMap, TestLoadsRefAndIteration) {
  ...
  version_map->load_via_ref_key(
      store, id, LoadStrategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED},
      entry_ref);
  ...
  for (size_t idx = 0; idx < entry_iteration->keys_.size(); idx++)
    if (entry_iteration->keys_[idx] != entry_ref->keys_[idx]) {
      util::raise_rte("Keys Mismatch on idx {}: {} != {}", idx,
                      entry_iteration->keys_[idx], entry_ref->keys_[idx]);
    }
...
void write_old_style_journal_entry(const AtomKey& key,
                                   std::shared_ptr<StreamSink> store) {
  IndexAggregator<RowCountIndex> journal_agg(key.id(), [&](auto&& segment) {
    store
        ->write(KeyType::VERSION_JOURNAL, key.version_id(), key.id(),
                IndexValue(NumericIndex{0}), IndexValue(NumericIndex{0}),
                std::move(segment))
        .wait();
  });
...
TEST(VersionMap, GetNextVersionInEntry) {
  ...
  entry->keys_.push_back(
      AtomKeyBuilder().version_id(5).build<KeyType::TABLE_INDEX>(""));
  entry->keys_.push_back(
      AtomKeyBuilder().version_id(3).build<KeyType::TABLE_INDEX>(""));
  entry->keys_.push_back(
      AtomKeyBuilder().version_id(1).build<KeyType::TABLE_INDEX>(""));
  entry->keys_.push_back(
      AtomKeyBuilder().version_id(0).build<KeyType::TABLE_INDEX>(""));
...
TEST(VersionMap, FixRefKeyTombstones) {
  ...
  auto entry = version_map->check_reload(
      store, id, LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED},
      __FUNCTION__);
...
TEST(VersionMap, StorageLogging) {
  ...
  store->iterate_type(
      KeyType::LOG, [&](VariantKey&& vk) { log_keys.emplace(std::get<AtomKey>(vk)); },
      "");
...
write_two_versions(std::shared_ptr<InMemoryStore> store,
                   std::shared_ptr<VersionMap> version_map, StreamId id) {
  auto entry = version_map->check_reload(
      store, id, LoadStrategy{LoadType::NOT_LOADED, LoadObjective::INCLUDE_DELETED},
      __FUNCTION__);
  ...
  // We override the symbol ref without a prev_key on purpose. This way we'll only load
  // the version=1 from the ref key
...
void write_alternating_deleted_undeleted(std::shared_ptr<InMemoryStore> store,
...
TEST(VersionMap, FollowingVersionChain) {
  ...
  auto check_strategy_loads_to = [&](LoadStrategy load_strategy,
                                     VersionId should_load_to) {
    ...
    EXPECT_EQ(follow_result->load_progress_.oldest_loaded_index_version_,
              VersionId{should_load_to});
  };

  check_strategy_loads_to(LoadStrategy{LoadType::DOWNTO, LoadObjective::INCLUDE_DELETED,
                                       static_cast<SignedVersionId>(0)},
                          0);
  check_strategy_loads_to(LoadStrategy{LoadType::DOWNTO, LoadObjective::INCLUDE_DELETED,
                                       static_cast<SignedVersionId>(-2)},
                          1);
  // DOWN_TO will not skip through tombstoned versions even when include_deleted=false
  check_strategy_loads_to(LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                                       static_cast<SignedVersionId>(-1)},
                          2);
  check_strategy_loads_to(LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                                       static_cast<SignedVersionId>(0)},
                          0);

  // FROM_TIME when include_deleted=false will skip through deleted versions to go to
  // the latest undeleted version before the timestamp.
  check_strategy_loads_to(LoadStrategy{LoadType::FROM_TIME,
  ...
  check_strategy_loads_to(
      LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED}, 2);
  check_strategy_loads_to(LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY},
                          1);

  check_strategy_loads_to(LoadStrategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED},
                          0);
  check_strategy_loads_to(LoadStrategy{LoadType::ALL, LoadObjective::UNDELETED_ONLY},
                          0);
}
...
TEST(VersionMap, FollowingVersionChainWithCaching) {
  ...
  auto check_loads_versions = [&](LoadStrategy load_strategy, uint32_t should_load_any,
                                  uint32_t should_load_undeleted) {
  ...
  check_loads_versions(LoadStrategy{LoadType::DOWNTO, LoadObjective::INCLUDE_DELETED,
                                    static_cast<SignedVersionId>(-1)},
                       1, 0);
  // FROM_TIME should not be cached by the DOWNTO and should reload from storage up to
  // the latest undeleted version, hence loading 2 versions, 1 of which is undeleted.
  check_loads_versions(LoadStrategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                                    static_cast<timestamp>(10)},
                       2, 1);
  // LATEST should be cached by the FROM_TIME, so we still have the same 2 loaded
  // versions
  check_loads_versions(LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED},
                       2, 1);
  // This FROM_TIME should still use the cached 2 versions
  check_loads_versions(LoadStrategy{LoadType::FROM_TIME, LoadObjective::INCLUDE_DELETED,
                                    static_cast<timestamp>(1)},
                       2, 1);
  ...
  // LATEST should still be cached, but the cached entry now needs to have no undeleted
  // keys
  check_loads_versions(LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED},
                       2, 0);
  // FROM_TIME UNDELETED_ONLY should no longer be cached even though we used the same
  // request before because the undeleted key it went to got deleted. So it will load
  ...
  // LATEST should still be cached, but the cached entry now needs to have one more
  // undeleted version
  check_loads_versions(LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED},
                       4, 1);
...
TEST(VersionMap, FollowingVersionChainEndEarlyOnTombstoneAll) {
  ...
  // Deleting should add a TOMBSTONE_ALL which should end searching for undeleted
  // versions early.
  version_map->delete_all_versions(store, id);
  ...
  for (auto load_strategy :
       {LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                     static_cast<SignedVersionId>(0)},
    ...
    follow_result->clear();
    version_map->follow_version_chain(store, ref_entry, follow_result, load_strategy);
    // When loading with any of the specified load strategies with include_deleted=false
    // we should end following the version chain early at version 1 because that's when
    // we encounter the TOMBSTONE_ALL.
    EXPECT_EQ(follow_result->load_progress_.oldest_loaded_index_version_, VersionId{1});
  }

  for (auto load_strategy :
       {LoadStrategy{LoadType::DOWNTO, LoadObjective::INCLUDE_DELETED,
                     static_cast<SignedVersionId>(0)},
        LoadStrategy{LoadType::FROM_TIME, LoadObjective::INCLUDE_DELETED,
                     static_cast<timestamp>(0)},
        LoadStrategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED}}) {
    follow_result->clear();
    version_map->follow_version_chain(store, ref_entry, follow_result, load_strategy);
    // When loading with any of the specified load strategies with include_deleted=true
    // we should continue to the beginning at version 0 even though it was deleted.
...
TEST(VersionMap, CacheInvalidation) {
  ...
  auto check_caching = [&](LoadStrategy to_load, LoadStrategy to_check_if_cached,
    ...
    EXPECT_EQ(clean_version_map->has_cached_entry(id, to_check_if_cached),
              expected_outcome);
  };
  ...
  std::vector<LoadStrategy> should_load_to_v[num_versions] = {
      ...
      std::vector<LoadStrategy>{
          ...
          LoadStrategy{
              ...
              static_cast<timestamp>(2)}, // when include_deleted=false FROM_TIME
                                          // searches for an undeleted version
          LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY},
      },
  ...
  for (auto i = 0u; i < num_versions; ++i) {
    for (auto j = 0u; j < num_versions; ++j) {
      // For every two versions we check that all load params for earlier versions cache
      // load paramse for later versions:
      check_all_caching(should_load_to_v[i], should_load_to_v[j], i <= j);
    }

    // ALL and LOAD_UNDELETED because they both load to v0 (UNDELETED_ONLY will load v0
    // because only there it will load
    check_all_caching({load_all_param, load_all_undeleted_param}, should_load_to_v[i],
                      true);
    check_all_caching(should_load_to_v[i], {load_all_param, load_all_undeleted_param},
                      false);
  }
}
...
TEST(VersionMap, CacheInvalidationWithTombstoneAfterLoad) {
  ...
  auto entry = version_map->check_reload(store, id,
                                         LoadStrategy{LoadType::DOWNTO,
                                                      LoadObjective::INCLUDE_DELETED,
                                                      static_cast<SignedVersionId>(1)},
                                         __FUNCTION__);

  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY}));
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                       static_cast<timestamp>(1)}));
  ASSERT_FALSE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                       static_cast<timestamp>(0)}));
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                       static_cast<SignedVersionId>(-1)}));
  ASSERT_FALSE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                       static_cast<SignedVersionId>(-2)}));
  ...
  // Now when the cached version is deleted, we should invalidate the cache for load
  // parameters which look for undeleted.
  ASSERT_FALSE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY}));
  ASSERT_FALSE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::FROM_TIME, LoadObjective::UNDELETED_ONLY,
                       static_cast<timestamp>(1)}));
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::LATEST, LoadObjective::INCLUDE_DELETED}));
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::FROM_TIME, LoadObjective::INCLUDE_DELETED,
                       static_cast<timestamp>(1)}));
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::DOWNTO, LoadObjective::UNDELETED_ONLY,
                       static_cast<SignedVersionId>(-1)}));
  ...
  const auto latest_undeleted_entry =
      version_map->check_reload(store, id, load_strategy, __FUNCTION__);
...
TEST(VersionMap, CacheInvalidationWithTombstoneAllAfterLoad) {
  ...
  // We should not invalidate the cache because the version we loaded to is still
  // undeleted
  ASSERT_TRUE(version_map->has_cached_entry(
      id, LoadStrategy{LoadType::LATEST, LoadObjective::UNDELETED_ONLY}));
  ASSERT_TRUE(version_map->has_cached_entry(
...
TEST(VersionMap, CompactionUpdateCache) {
  ...
  auto assert_keys_in_entry_and_store =
      [&store, &id](std::shared_ptr<VersionMapEntry> entry, int expected_version_keys,
                    int expected_index_keys, int expected_tombstone_keys) {
        int present_version_keys = 0, present_index_keys = 0,
            present_tombstone_keys = 0;
        ...
        store->iterate_type(KeyType::VERSION,
                            [&](VariantKey&&) { ++version_keys_in_store; });
  ...
  for (auto i = 10; i < 20; ++i) {
    ...
    version_map->write_version(store, key, std::nullopt);
    if (i % 3 == 0) {
      version_map->write_tombstone(store, VersionId{static_cast<uint64_t>(i)}, id,
                                   entry);
    }
  }
  ...
  // TODO: If we ever use compact_and_remove_deleted_indexes fix the below assertions
  // (method is currently unused with TODOs to fix):
  // version_map->compact_and_remove_deleted_indexes(store, id);
  // assert_keys_in_entry_and_store(entry, 2, 17, 3);

  // Flush and reload to see that what we have in storage also matches what we have in
  // the cache.
  version_map->flush();
