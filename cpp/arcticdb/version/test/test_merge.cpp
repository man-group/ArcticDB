...
protected:
  std::string get_name() override { return "test.merge_streams"; }
};
...
TEST_F(MergeReadsTestStore, SimpleStaticSchema) {
  ...
  using MergeAggregator =
      Aggregator<TimeseriesIndex, FixedSchema, stream::RowCountSegmentPolicy,
  ...
  MergeSinkWrapper wrapper1(stream_id1, {scalar_field(DataType::UINT64, "thing1"),
                                         scalar_field(DataType::UINT64, "thing2")});

  auto& aggregator1 = wrapper1.aggregator_;

  MergeSinkWrapper wrapper2(stream_id2, {scalar_field(DataType::UINT64, "thing1"),
                                         scalar_field(DataType::UINT64, "thing2")});
  ...
  test_store_->write_individual_segment(stream_id1, std::move(wrapper1.segment()),
                                        false);
  test_store_->write_individual_segment(stream_id2, std::move(wrapper2.segment()),
                                        false);

  const std::string target_id("some_merged_stuff");
  pipelines::ReadQuery read_query;
  auto read_result = test_store_->read_dataframe_merged(
      target_id, {stream_id1, stream_id2}, pipelines::VersionQuery{}, read_query,
      ReadOptions{});
