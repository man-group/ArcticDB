...
TEST(SortIndex, Basic) {
  ...
  for (auto i = 0; i < 20; ++i) {
    auto key = atom_key_builder()
                   ...
                   .end_index(i + 1)
                   .creation_ts(i)
                   .build<KeyType::TABLE_DATA>(stream_id);
  ...
  auto desc = stream_descriptor(stream_id, stream::RowCountIndex(),
                                {scalar_field(DataType::UTF_DYNAMIC64, "col_1")});
  TimeseriesDescriptor timeseries_desc;
  timeseries_desc.set_stream_descriptor(desc);
  index::IndexWriter<stream::RowCountIndex> index_writer(mock_store, partial_key,
                                                         std::move(timeseries_desc));
  ...
  auto engine = version_store::LocalVersionedEngine::_test_init_from_store(
      mock_store, util::SysClock{});
