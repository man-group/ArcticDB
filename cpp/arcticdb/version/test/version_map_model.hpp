...
AtomKey make_test_index_key(std::string id, VersionId version_id, KeyType key_type,
...
struct MapStorePair {
  ...
  MapStorePair(bool tombstones)
      : tombstones_(tombstones), store_(std::make_shared<InMemoryStore>()) {}
  ...
  void write_and_prune_previous(const std::string& id) {
    ARCTICDB_DEBUG(log::version(), "MapStorePair, write_and_prune_previous version {}",
                   id);
    auto prev = get_latest_version(store_, map_, id).first;
    auto version_id = prev ? prev->version_id() + 1 : 0;

    if (tombstones_)
      map_->write_and_prune_previous(
          store_, make_test_index_key(id, version_id, KeyType::TABLE_INDEX), prev);
    else
...
struct VersionMapTombstonesModel {
  ...
  std::optional<VersionId> get_latest_version(const std::string& id) const {
    ARCTICDB_DEBUG(log::version(), "VersionMapTombstonesModel, get_latest_version {}",
                   id);
    auto it = data_.find(id);
    return it == data_.end() || it->second.empty()
               ? std::nullopt
  ...
  std::optional<VersionId> get_latest_undeleted_version(const std::string& id) const {
    ARCTICDB_DEBUG(log::version(),
                   "VersionMapTombstonesModel, get_latest_undeleted_version {}", id);
    auto it = data_.find(id);
    if (it == data_.end())
      return std::nullopt;
    ...
    for (auto v : it->second) {
      if (tombstones == tombstones_.end() ||
          tombstones->second.find(v) == tombstones->second.end())
        return v;
    }
    return std::nullopt;
  }
  ...
  std::vector<VersionId> get_all_versions(const std::string& id) const {
    ...
    if (it != data_.end()) {
      auto tombstones = tombstones_.find(id);
      std::copy_if(std::begin(it->second), std::end(it->second),
                   std::back_inserter(output), [&](auto v) {
                     return tombstones == tombstones_.end() ||
                            tombstones->second.find(v) == tombstones->second.end();
                   });
    }
    return output;
  ...
  void write_and_prune_previous(const std::string& id) {
    ARCTICDB_DEBUG(log::version(),
                   "VersionMapTombstonesModel, write_and_prune_previous version {}",
                   id);
    auto prev = get_latest_version(id);
    VersionId version_id{0};
    if (prev) {
