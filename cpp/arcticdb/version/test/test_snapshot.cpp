...
TEST(SnapshotCreate, Basic) {
  ...
  auto library = create_library(library_path, OpenMode::WRITE,
                                arcticdb::storage::lmdb::pack_config(temp_path));
  ...
  FixedSchema schema{RowCountIndex::create_stream_descriptor(
      stream_id, {FieldDescriptor(TypeDescriptor(c1_dt, Dimension::Dim0)),
  ...
  auto t1 = [&] {
    return NativeTensor{c1_bytes, 1, &c1_strides, &c1_shapes, c1_dt, col1.data()};
  };
  ...
  auto t2 = [&] {
    return NativeTensor{c2_bytes, 1, &c2_strides, &c2_shapes, c2_dt, col2.data()};
  };
