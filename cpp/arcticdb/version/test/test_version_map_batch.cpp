...
void add_versions_for_stream(const std::shared_ptr<VersionMap>& version_map,
                             const std::shared_ptr<Store>& store,
                             const StreamId& stream_id, size_t num_versions,
                             size_t start = 0u) {
  ...
  for (uint64_t i = 0; i < num_streams; i++) {
    ...
    for (uint64_t j = 0; j < num_versions_per_stream; j++) {
      ...
      version_queries.emplace_back(
          VersionQuery{SpecificVersionQuery{static_cast<SignedVersionId>(j)}});
    }
  }

  // do batch versions read
  auto versions = folly::collect(batch_get_versions_async(store, version_map,
                                                          stream_ids, version_queries))
                      .get();
...
TEST_F(VersionMapBatchStore, SimpleTimestampQueries) {
  ...
  // First, for test purposes information, we retrieve the full information the stored
  // data in order to know the timestamps
  ...
  for (uint64_t i = 0; i < num_streams; i++) {
    for (uint64_t j = 0; j < num_versions_per_stream; j++) {
      uint64_t idx = i * num_versions_per_stream + j;
      version_queries.emplace_back(
          VersionQuery{TimestampVersionQuery{timestamp(versions[idx]->creation_ts())}});
    }
  }
  ...
  for (uint64_t i = 0; i < num_streams; i++) {
    ...
    for (uint64_t j = 0; j < num_versions_per_stream; j++) {
      ...
      ASSERT_EQ(versions_querying_with_timestamp[idx]->version_id(),
                versions[idx]->version_id());
      ASSERT_EQ(versions_querying_with_timestamp[idx]->creation_ts(),
                versions[idx]->creation_ts());
    }
  }
}
...
TEST_F(VersionMapBatchStore, MultipleVersionsSameSymbolVersionIdQueries) {
  ...
  for (uint64_t i = 0; i < num_versions; i++) {
    ...
    version_queries.emplace_back(
        VersionQuery{SpecificVersionQuery{static_cast<SignedVersionId>(i)}});
  }
...
TEST_F(VersionMapBatchStore, MultipleVersionsSameSymbolTimestampQueries) {
  ...
  for (uint64_t i = 0; i < num_versions; i++) {
    version_queries.emplace_back(
        VersionQuery{TimestampVersionQuery{timestamp(versions[i]->creation_ts())}});
  }
  ...
  for (uint64_t i = 0; i < num_versions; i++) {
    ...
    ASSERT_EQ(versions_querying_with_timestamp[i]->version_id(),
              versions[i]->version_id());
    ASSERT_EQ(versions_querying_with_timestamp[i]->creation_ts(),
              versions[i]->creation_ts());
  }
}
...
TEST_F(VersionMapBatchStore, CombinedQueries) {
  ...
  for (uint64_t i = 0; i < num_streams; i++) {
    ...
    for (uint64_t j = 0; j < num_versions_per_stream; j++) {
      ...
      ASSERT_EQ(versions_querying_with_mix_types[idx + 1]->version_id(),
                versions[idx_versions]->version_id());
      ASSERT_EQ(versions_querying_with_mix_types[idx + 1]->creation_ts(),
                versions[idx_versions]->creation_ts());
      ASSERT_EQ(versions_querying_with_mix_types[idx + 2]->version_id(),
                num_versions_per_stream - 1);
    }
  }
}
...
TEST_F(VersionMapBatchStore, SpecificVersionsShouldCopyInput) {
  ...
  for (uint64_t i = 0; i < 1000; ++i) {
    ...
    // We add to the sym_versions a missing symbol after the batch_get to mimic the
    // issue: https://github.com/man-group/ArcticDB/issues/1716
