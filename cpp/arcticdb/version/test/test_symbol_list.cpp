...
static const StorageFailureSimulator::ParamActionSequence RAISE_ON_2ND_CALL = {
    no_op, fault(), no_op};
 ...
 * 4. Step 3 (Get the compaction lock) failures (SymbolListWith*Failures where the
 * sequence affected locks)
...
struct SymbolListSuite : Test {
  ...
  // The max_delta_ can be set at construction time only, so to change requires a new
  // instance:
  ...
  [[nodiscard]] auto get_symbol_list_keys(const std::string& prefix = "") const {
    ...
    store->iterate_type(
        ...
        [&keys](const VariantKey& k) { keys.push_back(k); }, prefix);
    return keys;
  }
};
...
struct SymbolListWithReadFailures : SymbolListSuite,
                                    testing::WithParamInterface<FailSimParam> {
...
TEST_P(SymbolListWithReadFailures, VersionMapSource) {
  ...
  auto key1 = atom_key_builder()
                  ...
                  .start_index(4)
                  .end_index(5)
                  .build(symbol_3, KeyType::TABLE_INDEX);
  ...
  auto key2 = atom_key_builder()
                  ...
                  .start_index(5)
                  .end_index(6)
                  .build(symbol_1, KeyType::TABLE_INDEX);
  ...
  auto key3 = atom_key_builder()
                  ...
                  .start_index(6)
                  .end_index(7)
                  .build(symbol_2, KeyType::TABLE_INDEX);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(FailSimParam{}, // No failure
           ...
           FailSimParam{
               {FailureType::READ, {fault(), fault(), fault(), fault(), no_op}}},
           FailSimParam{
               {FailureType::READ, {fault(0.4), no_op}}}, // 40% chance of exception
           FailSimParam{{FailureType::ITERATE, RAISE_ONCE},
                        {FailureType::READ, {no_op, fault(), no_op}}}));
...
struct SymbolListWithWriteFailures : SymbolListSuite,
                                     testing::WithParamInterface<WriteFailuresParams> {
  ...
  void check_num_symbol_list_keys_match_expectation(
      std::map<CompactOutcome, size_t> size_by_outcome) const {
...
TEST_P(SymbolListWithWriteFailures, SubsequentCompaction) {
  ...
  // NOT_CLEANED_UP case checks that deletion happens after the compaction key is
  // written
  check_num_symbol_list_keys_match_expectation(
      {{{CompactOutcome::NOT_WRITTEN, 501},
        {CompactOutcome::WRITTEN, 1},
        {CompactOutcome::NOT_CLEANED_UP, 502}}});
...
TEST_P(SymbolListWithWriteFailures, InitialCompact) {
  ...
  for (int64_t i = 0; i < n + 1; ++i) {
    ...
    auto key1 = atom_key_builder()
                    ...
                    .start_index(4)
                    .end_index(5)
                    .build(symbol, KeyType::TABLE_INDEX);
    version_map->write_version(store, key1, std::nullopt);
    expected.emplace_back(symbol);
  }
  ...
  check_num_symbol_list_keys_match_expectation({{{CompactOutcome::NOT_WRITTEN, 0},
                                                 {CompactOutcome::WRITTEN, 1},
                                                 {CompactOutcome::NOT_CLEANED_UP, 1}}});
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(WriteFailuresParams{FailSimParam{}, CompactOutcome::WRITTEN}, // No failures
           WriteFailuresParams{{{FailureType::WRITE, RAISE_ONCE}},
                               CompactOutcome::NOT_WRITTEN}, // Interferes with locking
           WriteFailuresParams{{{FailureType::WRITE, RAISE_ON_2ND_CALL}},
                               CompactOutcome::NOT_WRITTEN},
           WriteFailuresParams{{{FailureType::DELETE, RAISE_ONCE}},
                               CompactOutcome::NOT_CLEANED_UP}));
...
std::optional<T> random_choice(const std::set<T>& set, U& gen) {
  ...
  std::uniform_int_distribution dis(
      0, static_cast<int>(std::min(size_t{0}, set.size() - 1)));
...
public:
  explicit SymbolListState(std::shared_ptr<Store> store,
                           std::shared_ptr<VersionMap> version_map)
      : store_(std::move(store)), version_map_(std::move(version_map)) {}
...
private:
  void do_add() {
    ...
    if (live_symbols_.count(symbol) == 0 && deleted_symbols_.count(symbol) == 0) {
      ...
      version_map_->write_version(
          store_, atom_key_builder().version_id(0).build(symbol, KeyType::TABLE_INDEX),
          std::nullopt);
  ...
  void do_readd() {
    ...
    if (symbol.has_value()) {
      ...
      version_map_->write_version(store_,
                                  atom_key_builder()
                                      .version_id(versions_[*symbol])
                                      .build(*symbol, KeyType::TABLE_INDEX),
                                  std::nullopt);
  ...
  void assert_invariants(const std::shared_ptr<SymbolList>& symbol_list) const {
    ...
    std::set<StreamId> symbol_list_symbols_set{symbol_list_symbols_vec.begin(),
                                               symbol_list_symbols_vec.end()};
...
bool result_equals(const ProblematicResult& got, const SymbolEntryData& expected) {
  return got.reference_id() == expected.reference_id_ &&
         got.action() == expected.action_ && got.time() == expected.timestamp_;
}
...
TEST(SymbolList, IsProblematic) {
  ...
  std::vector<SymbolEntryData> vec5{{0, 0, ActionType::ADD},
  ...
  std::vector<SymbolEntryData> vec6{{0, 1_s, ActionType::DELETE},
                                    {0, 2_s, ActionType::DELETE},
                                    {0, 3_s, ActionType::ADD}};
  ...
  std::vector<SymbolEntryData> vec8{{0, 0, ActionType::ADD},
                                    {0, 1_s, ActionType::DELETE},
                                    {0, 1_s + 100, ActionType::ADD}};
  ...
  std::vector<SymbolEntryData> vec9{{0, 0, ActionType::ADD},
                                    {unknown_version_id, 1_s, ActionType::DELETE},
...
TEST(SymbolList, IsProblematicWithStored) {
  ...
  std::vector<SymbolEntryData> vec2{{0, 1_s, ActionType::DELETE},
                                    {1, 2_s, ActionType::ADD},
  ...
  std::vector<SymbolEntryData> vec3{{0, 1_s, ActionType::ADD},
                                    {0, 2_s, ActionType::DELETE},
                                    {1, 3_s, ActionType::ADD},
                                    {2, 4_s, ActionType::ADD}};
  ...
  std::vector<SymbolEntryData> vec4{{0, 1_s, ActionType::ADD},
                                    {0, 2_s, ActionType::ADD}};
  ...
  std::vector<SymbolEntryData> vec5{{0, 1_s, ActionType::DELETE},
                                    {1, 2_s, ActionType::DELETE},
                                    {1, 3_s, ActionType::ADD},
  ...
  std::vector<SymbolEntryData> vec6{{unknown_version_id, 1_s, ActionType::ADD},
                                    {0, 2_s, ActionType::ADD},
                                    {0, 3_s, ActionType::DELETE}};
  ...
  std::vector<SymbolEntryData> vec7{{0, 2_s, ActionType::ADD},
                                    {0, 3_s, ActionType::DELETE}};
  ...
  std::vector<SymbolEntryData> vec8{{0, 1_s, ActionType::ADD},
                                    {0, 2_s, ActionType::ADD}};
...
TEST(Problematic, RealTimestamps2) {
  ...
  std::vector<SymbolEntryData> vec1{{0, 1696255639552055287, ActionType::ADD},
...
TEST(Problematic, RealTimestamps) {
  SymbolListEntry entry1{"test", 0, 1694701083539622714, ActionType::ADD};
  std::vector<SymbolEntryData> vec1{{0, 1694701083516771231, ActionType::ADD},
  ...
  std::vector<SymbolEntryData> vec2{{0, 1694779976040611297, ActionType::ADD},
  ...
  std::vector<SymbolEntryData> vec3{{0, 1696510154081738353, ActionType::ADD},
...
bool is_compacted(const std::shared_ptr<Store>& store) {
  ...
  return symbol_keys.size() == 1 &&
         symbol_keys[0].id() == StreamId{std::string{CompactionId}};
}

bool all_symbols_match(const std::shared_ptr<Store>& store, SymbolList& symbol_list,
                       std::vector<StreamId>& expected) {
...
TEST_F(SymbolListSuite,
       ExtremelyBackwardsCompatInterleavedWithSomewhatBackwardsCompat) {
  ConfigsMap::instance()->set_int("SymbolList.MaxDelta", 1);
  auto version_store = get_test_engine();
  const auto& store = version_store._test_get_store();
  auto version_map = std::make_shared<VersionMap>();
  ...
  // Very old arcticc clients wrote version numbers on the symbol list entries, which
  // needs special handling.
  extremely_backwards_compat_write_journal(store, "symbol", std::string{AddSymbol}, 0);
  extremely_backwards_compat_write_journal(store, "symbol", std::string{DeleteSymbol},
                                           1);
  backwards_compat_write_journal(store, "symbol", std::string{AddSymbol});
...
TEST_F(SymbolListSuite, BackwardsCompat) {
  ...
  for (auto i = 0U; i < 10; i += 2) {
    ...
    backwards_compat_write_journal(store, fmt::format("symbol_{}", i + 1),
...
struct TestSymbolListTask {
  ...
  TestSymbolListTask(const std::shared_ptr<SymbolListState>& state,
      ...
      : state_(state), store_(store), symbol_list_(symbol_list) {}
...
TEST_F(SymbolListSuite, CompactionThreshold) {
  ...
  std::vector<VariantKey> keys;
  store->iterate_type(entity::KeyType::SYMBOL_LIST,
                      [&keys](auto&& k) { keys.push_back(k); });
...
TEST_F(SymbolListSuite, CompactionThresholdRandomChoice) {
  // given
  // Compaction thresholds [1, 10] form random choice, seeded with value 1 => choice of
  // threshold is 5 (or 6 on Mac)
  ScopedConfig min("SymbolList.MinCompactionThreshold", 1);
...
struct WriteSymbolsTask {
  ...
  WriteSymbolsTask(const std::shared_ptr<Store>& store, size_t offset,
      ...
      : store_(store), symbol_list_(std::make_shared<SymbolList>(version_map_)),
        offset_(offset), versions_(versions) {}
...
struct SymbolListRace
    : SymbolListSuite,
      testing::WithParamInterface<std::tuple<char, bool, bool, bool>> {};
...
// For symbol list source (which is used for subsequent compactions), all flag
// combinations are valid:
INSTANTIATE_TEST_SUITE_P(SymbolListSource, SymbolListRace,
                         Combine(Values('S'), Bool(), Bool(), Bool()));
// For version keys source (initial compaction), there's no old compaction key to
// remove:
INSTANTIATE_TEST_SUITE_P(VersionKeysSource, SymbolListRace,
                         Combine(Values('V'), Values(false), Bool(), Bool()));
