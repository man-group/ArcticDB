...
/* === Tests overview ===
 * Test each below for both sources (refer to RFC 006 for terminology):
 * 1. load_internal_with_retry (SymbolListWithReadFailures suite)
 * 2. no_compaction (FUTURE)
 * 3. Step 2 condition (implicit in most tests)
 * 4. Step 3 lock fail (SymbolListWith*Failures where the sequence affected locks)
