...
TEST(Sparse, Simple) {
  ...
  using DynamicAggregator =
      Aggregator<TimeseriesIndex, DynamicSchema, stream::NeverSegmentPolicy,
  ...
  aggregator.start_row(timestamp{0})(
      [](auto& rb) { rb.set_scalar_by_name("first", uint32_t(5), DataType::UINT32); });

  aggregator.start_row(timestamp{1})(
      [](auto& rb) { rb.set_scalar_by_name("second", uint64_t(6), DataType::UINT64); });
...
TEST_F(SparseTestStore, SimpleRoundtrip) {
  ...
  auto read_result = test_store_->read_dataframe_version(
      stream_id, pipelines::VersionQuery{}, read_query, read_options);
...
// Just write a dummy StreamDescriptor to the TimeseriesDescriptor to avoid duplicating
// it as the same information is available in the segment header. This is how arcticc
// behaves so the test is important to check compat with existing data.
//
// This functions helps to test that readers look for the StreamDescriptor on the
// header, not in the TimeseriesDescriptor.
//
// Compare with `append_incomplete_segment`. Keep this function even if it duplicates
// `append_incomplete_segment` so that we have protection against
// `append_incomplete_segment` changing how it writes the descriptors in future.
void append_incomplete_segment_backwards_compat(
  ...
  auto tsd = arcticdb::make_timeseries_descriptor(
      seg_row_count, std::move(desc), NormalizationMetadata{}, std::nullopt,
      std::nullopt, std::move(next_key), false);
...
TEST_F(SparseTestStore, SimpleRoundtripBackwardsCompat) {
  ...
  auto segment = wrapper.segment();
  append_incomplete_segment_backwards_compat(test_store_->_test_get_store(), stream_id,
                                             std::move(segment));
...
TEST_F(SparseTestStore, DenseToSparse) {
  ...
  aggregator.start_row(timestamp{6})(
      [](auto& rb) { rb.set_scalar_by_name("first", uint32_t(7), DataType::UINT32); });
...
TEST_F(SparseTestStore, SimpleRoundtripStrings) {
  ...
  aggregator.start_row(timestamp{0})([](auto& rb) {
    rb.set_scalar_by_name(std::string_view{"first"}, std::string_view{"five"},
                          DataType::UTF_DYNAMIC64);
  });

  aggregator.start_row(timestamp{1})([](auto& rb) {
    rb.set_scalar_by_name(std::string_view{"second"}, std::string_view{"six"},
...
TEST_F(SparseTestStore, Multiblock) {
  ...
  for (size_t i = 0; i < num_rows; i += 2) {
    aggregator.start_row(timestamp(i))([&](auto& rb) {
      rb.set_scalar_by_name(std::string_view{"first"}, uint32_t(i + 1),
                            DataType::UINT32);
    });

    aggregator.start_row(timestamp(i + 1))([&](auto& rb) {
      rb.set_scalar_by_name(std::string_view{"second"}, uint64_t(i + 2),
                            DataType::UINT64);
    });
  }
...
TEST_F(SparseTestStore, CompactWithStrings) {
  ...
  for (size_t i = 0; i < num_rows; i += 2) {
    aggregator.start_row(timestamp(i))([&](auto& rb) {
      ...
      rb.set_scalar_by_name(std::string_view{"first"}, std::string_view{val},
                            DataType::UTF_DYNAMIC64);
    });

    aggregator.start_row(timestamp(i + 1))([&](auto& rb) {
      auto val = fmt::format("{}", i + 2);
      rb.set_scalar_by_name(std::string_view{"second"}, std::string_view{val},
                            DataType::UTF_FIXED64);
    });
  }
  ...
  for (size_t i = 0; i < num_rows; i += 2) {
    ...
    ASSERT_TRUE(std::holds_alternative<convert::PyStringWrapper>(str_wrapper));
    ASSERT_EQ(strcmp(std::get<convert::PyStringWrapper>(str_wrapper).buffer_,
                     expected.data()),
              0);
