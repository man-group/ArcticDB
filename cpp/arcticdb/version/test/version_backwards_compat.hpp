...
std::deque<AtomKey> backwards_compat_delete_all_versions(
    ...
    auto entry = version_map->check_reload(store, stream_id, LoadParameter{LoadType::LOAD_ALL, true}, __FUNCTION__);
...
std::vector<AtomKey> backwards_compat_write_and_prune_previous(std::shared_ptr<Store>& store, std::shared_ptr<VersionMap>& version_map, const AtomKey &key) {
    ...
    auto entry = version_map->check_reload(store, key.id(), LoadParameter{LoadType::LOAD_ALL, true},  __FUNCTION__);
