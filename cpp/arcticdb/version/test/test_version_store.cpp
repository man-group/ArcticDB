...
auto write_version_frame(
    const arcticdb::StreamId& stream_id, arcticdb::VersionId v_id,
    arcticdb::version_store::PythonVersionStore& pvs, size_t rows = 1000000,
    bool update_version_map = false, size_t start_val = 0,
    const std::optional<arcticdb::entity::AtomKey>& previous_key = std::nullopt,
    const std::shared_ptr<arcticdb::DeDupMap>& de_dup_map =
        std::make_shared<arcticdb::DeDupMap>()) {
...
TEST(PythonVersionStore, WriteWithPruneVersions) {
  ...
  version_store._test_get_version_map()->write_and_prune_previous(mock_store, key,
                                                                  std::nullopt);
...
TEST(PythonVersionStore, IterationVsRefWrite) {
  ...
  version_map->load_via_ref_key(
      mock_store, stream_id,
      LoadStrategy{LoadType::ALL, LoadObjective::INCLUDE_DELETED}, ref_entry);

  EXPECT_EQ(std::string(iter_entry->head_.value().view()),
            std::string(ref_entry->head_.value().view()));
  ASSERT_EQ(iter_entry->keys_.size(), ref_entry->keys_.size());
  for (size_t idx = 0; idx != iter_entry->keys_.size(); idx++) {
    EXPECT_EQ(std::string(iter_entry->keys_[idx].view()),
              std::string(ref_entry->keys_[idx].view()));
  }
...
TEST_F(VersionStoreTest, SortMerge) {
  ...
  for (auto i = 0; i < 10; ++i) {
    auto wrapper = SinkWrapper(symbol, {scalar_field(DataType::UINT64, "thing1"),
  ...
  CompactIncompleteOptions options{.prune_previous_versions_ = false,
                                   .append_ = true,
...
TEST_F(VersionStoreTest, CompactIncompleteDynamicSchema) {
  ...
  auto read_result = test_store_->read_dataframe_version(symbol, VersionQuery{},
...
TEST_F(VersionStoreTest, StressBatchWrite) {
  ...
  folly::collect(test_store_->batch_write_internal(
                     version_ids, symbols, std::move(frames), dedup_maps, false))
      .get();
}
...
TEST_F(VersionStoreTest, StressBatchReadUncompressed) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    for (int j = 0; j < 10; ++j) {
      ...
      test_store_->write_versioned_dataframe_internal(symbol, std::move(wrapper.frame_),
                                                      false, false, false);
    }
  ...
  auto latest_versions = test_store_->batch_read(symbols, std::vector<VersionQuery>(10),
                                                 read_queries, read_options);
  for (auto&& [idx, version] : folly::enumerate(latest_versions)) {
    auto expected =
        get_test_simple_frame(std::get<ReadResult>(version).item.symbol(), 10, idx);
...
TEST(VersionStore, UpdateWithin) {
  ...
  std::vector<FieldRef> fields{scalar_field(DataType::UINT8, "thing1"),
                               scalar_field(DataType::UINT8, "thing2"),
                               scalar_field(DataType::UINT16, "thing3"),
                               scalar_field(DataType::UINT16, "thing4")};

  auto test_frame =
      get_test_frame<stream::TimeseriesIndex>(symbol, fields, num_rows, start_val);
  version_store.write_versioned_dataframe_internal(symbol, std::move(test_frame.frame_),
                                                   false, false, false);
  ...
  version_store.update_internal(symbol, UpdateQuery{}, std::move(update_frame.frame_),
                                false, false, false);

  ReadQuery read_query;
  auto read_result = version_store.read_dataframe_version_internal(
      symbol, VersionQuery{}, read_query, ReadOptions{});
...
TEST(VersionStore, UpdateWithinSchemaChange) {
  ...
  std::vector<FieldRef> update_fields{scalar_field(DataType::UINT8, "thing1"),
                                      scalar_field(DataType::UINT8, "thing2"),
                                      scalar_field(DataType::UINT16, "thing3"),
  ...
  ReadQuery read_query;
  auto read_result = version_store.read_dataframe_version_internal(
      symbol, VersionQuery{}, read_query, read_options);
  const auto& seg = read_result.frame_and_descriptor_.frame_;
...
TEST(VersionStore, TestWriteAppendMapHead) {
  ...
  auto descriptor = StreamDescriptor{
      symbol, IndexDescriptorImpl{1u, IndexDescriptorImpl::Type::TIMESTAMP},
