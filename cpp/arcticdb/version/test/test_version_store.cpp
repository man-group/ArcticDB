...
TEST(PythonVersionStore, IterationVsRefWrite) {
    ...
    version_map->load_via_ref_key(
        mock_store, stream_id, LoadStrategy{LoadType::ALL, arcticdb::LoadObjective::INCLUDE_DELETED}, ref_entry_compact
    );
...
TEST(VersionStore, UpdateWithin) {
    ...
    auto handler_data = get_type_handler_data();
    auto read_result =
        version_store.read_dataframe_version_internal(symbol, VersionQuery{}, read_query, ReadOptions{}, handler_data);
...
TEST(VersionStore, UpdateWithinSchemaChange) {
    ...
    auto handler_data = get_type_handler_data();
    auto read_result =
        version_store.read_dataframe_version_internal(symbol, VersionQuery{}, read_query, read_options, handler_data);
    const auto& seg = read_result.frame_and_descriptor_.frame_;
...
TEST(VersionStore, TestWriteAppendMapHead) {
    ...
    auto descriptor = StreamDescriptor{
        symbol,
        IndexDescriptorImpl{1u, IndexDescriptorImpl::Type::TIMESTAMP},
