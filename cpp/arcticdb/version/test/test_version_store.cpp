...
TEST(PythonVersionStore, CompactIncomplete) {
    using namespace arcticdb;
    using namespace arcticdb::storage;
    using namespace arcticdb::stream;
    using namespace arcticdb::pipelines;
    ...
    for (size_t i = 0; i < 10; ++i) {
        for(size_t j = 0; j < 20; ++j ) {
            ...
            auto v1 = seg.scalar_at<uint64_t>(count, 1);
            ASSERT_EQ(v1.value(), j);
            auto v2 = seg.scalar_at<uint64_t>(count , 2);
            ASSERT_EQ(v2.value(), i + j);
            ++count;
        }
    }

    // Now do a normal append
    auto wrapper = SinkWrapper(symbol, {
        scalar_field_proto(DataType::UINT64, "thing1"),
        scalar_field_proto(DataType::UINT64, "thing2")
    });
    ...
    wrapper.aggregator_.commit();
    SegmentToInputFrameAdapter adapter{std::move(wrapper.segment())};
    version_store->append_internal(symbol, std::move(adapter.input_frame_), false, false);
    auto new_read_result = version_store->read_dataframe_version(symbol, VersionQuery{}, read_query, ReadOptions{});
    const auto& new_seg = new_read_result.frame_data.frame();

    count = 0;
    // Check original
    for (size_t i = 0; i < 10; ++i) {
        for(size_t j = 0; j < 20; ++j ) {
            auto idx = new_seg.scalar_at<uint64_t>(count, 0);
            ASSERT_EQ(idx.value(), count);
            auto v1 = new_seg.scalar_at<uint64_t>(count, 1);
            ASSERT_EQ(v1.value(), j);
            auto v2 = new_seg.scalar_at<uint64_t>(count , 2);
            ASSERT_EQ(v2.value(), i + j);
            ++count;
        }
    }
