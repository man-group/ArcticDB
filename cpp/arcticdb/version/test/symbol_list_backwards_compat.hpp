...
/*
 * This file represents the way that the symbol list used to work, so that we can test
 * that new- and old-style symbol list functionalirt can co-exist
 */
...
void backwards_compat_read_list_from_storage(const std::shared_ptr<StreamSource>& store,
                                             const AtomKey& key,
  ...
  missing_data::check<ErrorCode::E_UNREADABLE_SYMBOL_LIST>(
      key_seg.descriptor().field_count() > 0,
...
std::vector<AtomKey>
backwards_compat_get_all_symbol_list_keys(const std::shared_ptr<StreamSource>& store) {
...
BackwardsCompatCollectionType
backwards_compat_load(const std::shared_ptr<StreamSource>& store,
...
BackwardsCompatCollectionType
backwards_compat_get_symbols(const std::shared_ptr<Store>& store) {
  auto keys = backwards_compat_get_all_symbol_list_keys(store);
  return backwards_compat_load(store, keys);
}

inline StreamDescriptor
backwards_compat_symbol_stream_descriptor(const StreamId& stream_id,
...
inline StreamDescriptor
backward_compat_journal_stream_descriptor(const StreamId& action, const StreamId& id) {
  return util::variant_match(
      id,
      [&action](const NumericId&) {
        return StreamDescriptor{stream_descriptor(
            action, RowCountIndex(), {scalar_field(DataType::UINT64, "symbol")})};
      },
      [&action](const StringId&) {
        return StreamDescriptor{
            stream_descriptor(action, RowCountIndex(),
                              {scalar_field(DataType::UTF_DYNAMIC64, "symbol")})};
      });
}

folly::Future<VariantKey> backwards_compat_write(
    const std::shared_ptr<Store>& store, const BackwardsCompatCollectionType& symbols,
    const StreamId& stream_id, timestamp creation_ts, const StreamId& type_holder) {

  SegmentInMemory list_segment{
      backwards_compat_symbol_stream_descriptor(stream_id, type_holder)};
  ...
  return store->write(KeyType::SYMBOL_LIST, 0, stream_id, creation_ts, NumericIndex{0},
                      NumericIndex{0}, std::move(list_segment));
}

// Very old internal ArcticDB clients (2021) wrote non-zero version IDs in symbol list
// entries. This API supports that.
void extremely_backwards_compat_write_journal(const std::shared_ptr<Store>& store,
  ...
  util::variant_match(
      symbol, [&seg](const StringId& id) { seg.set_string(0, id); },
      [&seg](const NumericId& id) { seg.set_scalar<uint64_t>(0, id); });

  seg.end_row();
  try {
    store->write_sync(KeyType::SYMBOL_LIST, version_id, StreamId{action},
                      IndexValue{symbol}, IndexValue{symbol}, std::move(seg));
...
// Internal ArcticDB clients (2021) write symbol list entries with an obsolete schema,
// and always with version ID 0.
void backwards_compat_write_journal(const std::shared_ptr<Store>& store,
                                    const StreamId& symbol, const std::string& action) {
  extremely_backwards_compat_write_journal(store, symbol, action, 0);
}

void backwards_compat_compact(const std::shared_ptr<Store>& store,
                              std::vector<AtomKey>&& old_keys,
                              const BackwardsCompatCollectionType& symbols) {
  auto compacted_key =
      backwards_compat_write(store, symbols, StreamId{std::string{CompactionId}},
                             timestamp(12), StringId{})
          .get();
