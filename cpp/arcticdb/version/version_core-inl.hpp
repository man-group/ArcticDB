...
template <typename IndexType, typename SegmentationPolicy, typename DensityPolicy,
          typename Callable>
void merge_frames_for_keys_impl(const StreamId& target_id, const IndexType& index,
                                SegmentationPolicy segmentation_policy,
                                const std::vector<AtomKey>& index_keys,
                                const pipelines::ReadQuery& query,
                                std::shared_ptr<Store> store, Callable&& func) {
  ARCTICDB_DEBUG(log::version(), "Merging keys");
  using namespace arcticdb::pipelines;
  using KeySupplier = folly::Function<std::vector<entity::AtomKey>()>;
  using StreamReaderType =
      arcticdb::stream::StreamReader<AtomKey, KeySupplier, SegmentInMemory::Row>;

  struct StreamMergeWrapper {
    StreamMergeWrapper(KeySupplier&& key_supplier, std::shared_ptr<StreamSource> store,
                       const IndexRange& index_range, StreamId id)
        : stream_reader_(std::move(key_supplier), std::move(store),
                         storage::ReadKeyOpts{}, index_range),
          iterator_(stream_reader_.iterator_rows()), row_(iterator_.next()),
          id_(std::move(id)) {}
    ...
    SegmentInMemory::Row& row() { return row_.value(); }
  ...
  auto compare = [](const std::unique_ptr<StreamMergeWrapper>& left,
    ...
    return pipelines::index::index_value_from_row(left->row(),
                                                  IndexDescriptor::Type::TIMESTAMP, 0) >
           pipelines::index::index_value_from_row(right->row(),
                                                  IndexDescriptor::Type::TIMESTAMP, 0);
  };
  ...
  for (const auto& index_key : index_keys) {
    ...
    input_streams.emplace(std::make_unique<StreamMergeWrapper>(
        std::move(key_func), store, index_range, index_key.id()));
  }

  using AggregatorType =
      Aggregator<IndexType, DynamicSchema, SegmentationPolicy, DensityPolicy>;
  AggregatorType agg{
      DynamicSchema{index.create_stream_descriptor(target_id, {}), index},
      std::move(func), std::move(segmentation_policy)};
  do_merge<IndexType, StreamMergeWrapper, AggregatorType, decltype(input_streams)>(
      input_streams, agg, true);
}

template <typename SegmentationPolicy, typename Callable>
void merge_frames_for_keys(const StreamId& target_id, stream::Index&& index,
                           SegmentationPolicy&& segmentation_policy,
  ...
  std::visit(
      [&](auto idx, auto density) {
        merge_frames_for_keys_impl<decltype(idx), SegmentationPolicy, decltype(density),
                                   std::decay_t<decltype(func)>>(
            target_id, idx, std::move(segmentation_policy), index_keys, query, store,
            std::move(func));
      },
      index, density_policy);
}

template <typename IndexType, typename SchemaType, typename SegmentationPolicy,
          typename DensityPolicy, typename IteratorType>
void do_compact(IteratorType target_start, IteratorType target_end,
                ...
                const std::shared_ptr<Store>& store, bool convert_int_to_float,
  ...
  stream::SegmentAggregator<IndexType, SchemaType, SegmentationPolicy, DensityPolicy>
      aggregator{[&slices](pipelines::FrameSlice&& slice) {
                 ...
                 [&fut_vec, &store, &pipeline_context](SegmentInMemory&& segment) {
                   ...
                   auto local_index_end = pipelines::end_index_generator(
                       IndexType::end_value_for_segment(segment));
                 ...
                 segment_size.has_value() ? SegmentationPolicy{*segment_size}
                                          : SegmentationPolicy{}};
...
[[nodiscard]] inline ReadOptions
defragmentation_read_options_generator(const WriteOptions& options) {
