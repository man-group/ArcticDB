...
// Uniquely identifies the version to be read:
// - VersionedItem is a thin wrapper around an AtomKey representing an index or multiindex key
...
struct SetupPipelineContextTask : async::BaseTask {
    ...
    std::optional<IndexInformation> index_information_;

    SetupPipelineContextTask(
            std::shared_ptr<Store> store, VersionIdentifier version_info, std::shared_ptr<ReadQuery> read_query,
            ReadOptions read_options, std::optional<IndexInformation>&& index_information
    ) :
        store_(std::move(store)),
