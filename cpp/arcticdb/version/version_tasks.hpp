...
struct UpdateMetadataTask : async::BaseTask {
  ...
  UpdateMetadataTask(std::shared_ptr<Store> store,
      ...
      : store_(std::move(store)), update_info_(std::move(update_info)),
  ...
  AtomKey operator()() const {
    ...
    segment.mutable_index_descriptor().mutable_proto().mutable_user_meta()->CopyFrom(
        user_meta_);
    return to_atom(store_->write_sync(index_key.type(), update_info_.next_version_id_,
                                      index_key.id(), index_key.start_index(),
...
struct AsyncRestoreVersionTask : async::BaseTask {
  ...
  AsyncRestoreVersionTask(std::shared_ptr<Store> store,
                          std::shared_ptr<VersionMap> version_map, StreamId stream_id,
                          entity::AtomKey index_key, std::optional<AtomKey> maybe_prev)
      : store_(std::move(store)), version_map_(std::move(version_map)),
        stream_id_(std::move(stream_id)), index_key_(std::move(index_key)),
  ...
  folly::Future<std::pair<VersionedItem, TimeseriesDescriptor>> operator()() {
    ...
    auto [index_segment_reader, slice_and_keys] =
        index::read_index_to_vector(store_, index_key_);
    ...
    if (maybe_prev_ && maybe_prev_->version_id() == index_key_.version_id()) {
      ...
      promise.setTry(folly::Try(
          std::make_pair(VersionedItem{index_key_}, index_segment_reader.tsd())));
      return future;
    } else {
      auto tsd =
          std::make_shared<TimeseriesDescriptor>(index_segment_reader.tsd().clone());
      ...
      for (const auto& slice_and_key : *sk)
        fut_keys.emplace_back(store_->copy(slice_and_key.key().type(), stream_id_,
                                           version_id, slice_and_key.key()));
      ...
      return folly::collect(fut_keys)
          ...
          .thenValue([sk](auto keys) {
            ...
            for (std::size_t i = 0; i < res.capacity(); ++i) {
              res.emplace_back(
                  SliceAndKey{(*sk)[i].slice_, std::move(to_atom(keys[i]))});
            }
            return res;
          })
          .thenValue([store = store_, version_map = version_map_, tsd = tsd,
                      stream_id = stream_id_, version_id](auto&& new_slice_and_keys) {
            auto index = index_type_from_descriptor(tsd->as_stream_descriptor());
            return index::index_and_version(index, store, *tsd, new_slice_and_keys,
                                            stream_id, version_id);
          })
          .thenValue([store = store_, version_map = version_map_, tsd = tsd,
                      maybe_prev = maybe_prev_](auto versioned_item) {
...
struct CheckReloadTask : async::BaseTask {
  ...
  CheckReloadTask(std::shared_ptr<Store> store, std::shared_ptr<VersionMap> version_map,
                  StreamId stream_id, LoadStrategy load_strategy)
      : store_(std::move(store)), version_map_(std::move(version_map)),
        stream_id_(std::move(stream_id)), load_strategy_(load_strategy) {}
...
struct WriteVersionTask : async::BaseTask {
  ...
  WriteVersionTask(std::shared_ptr<Store> store,
                   std::shared_ptr<VersionMap> version_map, AtomKey key,
                   const std::optional<AtomKey>& previous_key)
      : store_(std::move(store)), version_map_(std::move(version_map)),
        key_(std::move(key)), previous_key_(previous_key) {}
...
struct WriteAndPrunePreviousTask : async::BaseTask {
  ...
  WriteAndPrunePreviousTask(std::shared_ptr<Store> store,
      ...
      : store_(std::move(store)), version_map_(std::move(version_map)),
        key_(std::move(key)), maybe_prev_(std::move(maybe_prev)) {}
