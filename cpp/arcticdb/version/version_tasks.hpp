...
// - VersionedItem represents an index or multiindex key
// - StreamId is used when reading incompletes is enabled, and no versions have been created yet, so we will just be
//   reading the APPEND_REF -> APPEND_DATA... linked list
...
struct SetupPipelineContextTask : async::BaseTask {
    const std::shared_ptr<Store> store_;
    VersionIdentifier version_info_;
    ...
    std::shared_ptr<PipelineContext> operator()();
};
