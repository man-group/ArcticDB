...
void declare_resample_clause(py::module& version) {
    ...
    py::class_<ResampleClause<closed_boundary>, std::shared_ptr<ResampleClause<closed_boundary>>>(
        version, class_name.c_str()
    )
        ...
        .def(
            ...
            [](ResampleClause<closed_boundary>& self,
               const std::unordered_map<std::string, std::variant<std::string, std::pair<std::string, std::string>>>
                   aggregations) { self.set_aggregations(python_util::named_aggregators_from_dict(aggregations)); }
        )
...
void register_bindings(py::module& version, py::exception<arcticdb::ArcticException>& base_exception) {
    ...
    version.def("read_dataframe_from_file", [](StreamId sid, const std::string(path), ReadQuery& read_query) {
    ...
    py::class_<pipelines::SignedRowRange, std::shared_ptr<pipelines::SignedRowRange>>(version, "SignedRowRange")
        .def(py::init([](int64_t start, int64_t end) { return SignedRowRange{start, end}; }));
    ...
    py::class_<PythonVersionStore>(version, "PythonVersionStore")
        .def(
            py::init([](const std::shared_ptr<storage::Library>& library, std::optional<std::string>) {
        ...
        .def("snapshot", &PythonVersionStore::snapshot, py::call_guard<SingleThreadMutexHolder>(), "Create a snapshot")
        .def(
            "list_snapshots",
        ...
        .def(
            ...
            py::call_guard<SingleThreadMutexHolder>(),
            "Scan the compressed sizes of all objects in the library, grouped by stream ID and KeyType. Sizes are in "
            "bytes. "
        ...
        .def(
            ...
            [&](PythonVersionStore& v, StreamId sid, const VersionQuery& version_query) {
                ...
                ReadResult res{
                    vit,
                    PythonOutputFrame{SegmentInMemory{tsd.as_stream_descriptor()}, std::make_shared<BufferHolder>()},
        ...
        .def(
            "get_store_current_timestamp_for_tests",
