    ...
    py::class_<PythonVersionStore>(version, "PythonVersionStore")
            ...
            .def(
                    ...
                    [&](PythonVersionStore& v,
                        ...
                        auto res = adapt_read_df(
                                v.read_dataframe_version(sid, version_query, read_query, read_options, handler_data),
                                handler_data.get()
                        );
                        auto t1 = std::chrono::steady_clock::now();
                        log::version().warn("read_dataframe_version C++ {}ms", std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());
                        return res;
                    },
                    py::call_guard<SingleThreadMutexHolder>(),
            ...
            .def(
                    ...
                    [&](PythonVersionStore& v,
                        ...
                        auto res = python_util::adapt_read_dfs(
                                v.batch_read(
                        ...
                        auto t1 = std::chrono::steady_clock::now();
                        log::version().warn("batch_read C++ {}ms", std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());
                        return res;
                    },
                    py::call_guard<SingleThreadMutexHolder>(),
