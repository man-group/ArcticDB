...
public:
  ...
  result_type display_timer(const name_type& name = "default") {
    ...
    if (results.count > 1) {
      auto buffer =
          fmt::format("{}:\truns: {}\ttotal time: {:.6f}\tmean: {:.6f}",
...
public:
  ...
  ScopedTimer(arcticdb::ScopedTimer& other) =
      delete; // Folly Functions are non-copyable

  ScopedTimer(arcticdb::ScopedTimer&& other)
      : name_(std::move(other.name_)), func_(std::move(other.func_)) {
    if (other.started_) {
      timer_.start_timer(name_);
      started_ = true;
  ...
  ScopedTimer&
  operator=(arcticdb::ScopedTimer& other) = delete; // Folly Functions are non-copyable
...
#define SCOPED_TIMER(name, data)                                                       \
  arcticdb::ScopedTimerTotal timer1{#name, [&data](auto totals) {                      \
                                      std::copy(std::begin(totals), std::end(totals),  \
