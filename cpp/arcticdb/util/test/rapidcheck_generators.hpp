...
inline rc::Gen<arcticdb::entity::DataType> gen_numeric_datatype() {
  return rc::gen::element<arcticdb::entity::DataType>(
      arcticdb::entity::DataType::INT8, arcticdb::entity::DataType::UINT8,
      arcticdb::entity::DataType::INT16, arcticdb::entity::DataType::UINT16,
      arcticdb::entity::DataType::INT32, arcticdb::entity::DataType::UINT32,
      arcticdb::entity::DataType::INT64, arcticdb::entity::DataType::UINT64,
      arcticdb::entity::DataType::FLOAT32, arcticdb::entity::DataType::FLOAT64,
...
namespace rc {
template <> struct Arbitrary<arcticdb::entity::TypeDescriptor> {
...
template <> struct Arbitrary<arcticdb::entity::StreamDescriptor> {
  static Gen<arcticdb::entity::StreamDescriptor> arbitrary() {
    ...
    const auto field_names = *gen::container<std::unordered_set<std::string>>(
        num_fields, gen::nonEmpty(gen::string<std::string>()));
    arcticdb::FieldCollection field_descriptors;
    for (const auto& field_name : field_names) {
      field_descriptors.add_field(
          arcticdb::entity::scalar_field(*gen_numeric_datatype(), field_name));
    }
    auto desc = stream_descriptor_from_range(arcticdb::StreamId{id},
                                             arcticdb::stream::RowCountIndex{},
                                             std::move(field_descriptors));
...
template <> struct Arbitrary<TestDataFrame> {
    ...
    return gen::build<TestDataFrame>(
        ...
        gen::set(&TestDataFrame::column_names_,
                 gen::container<std::vector<std::string>>(
                     num_columns, gen::nonEmpty(gen::string<std::string>()))));
  }
};
...
inline as::FixedSchema schema_from_test_frame(const TestDataFrame& data_frame,
                                              ac::StreamId stream_id) {
  arcticdb::FieldCollection fields;
  for (size_t i = 0; i < data_frame.num_columns_; ++i)
    fields.add_field(arcticdb::entity::scalar_field(data_frame.types_[i].data_type(),
                                                    data_frame.column_names_[i]));

  const auto index = as::TimeseriesIndex::default_index();
  return as::FixedSchema{index.create_stream_descriptor(std::move(stream_id), fields),
                         index};
}

inline ac::IndexRange test_frame_range(const TestDataFrame& data_frame) {
  return ac::IndexRange{data_frame.start_ts_,
...
write_frame_data(const TestDataFrame& data_frame, WriterType& writer) {
...
write_test_frame(ac::StreamId stream_id,
  ...
  ac::StreamWriter<ac::TimeseriesIndex, ac::FixedSchema> writer{
      std::move(schema), std::move(store), gen_id, start_end,
...
bool check_read_frame(const TestDataFrame& data_frame, ReaderType& reader,
                      std::vector<std::string>& errors) {
  bool success = true;
  auto timestamp = data_frame.start_ts_;
  auto row = 0u;

  reader.foreach_row([&row, &timestamp, &success, &data_frame,
                      &errors](auto&& row_ref) {
    timestamp += data_frame.timestamp_increments_[row];
    for (size_t col = 0; col < data_frame.num_columns_; ++col) {
      data_frame.types_[col].visit_tag([&](auto type_desc_tag) {
        ...
        arcticdb::entity::DataType stored_dt =
            row_ref.segment().column_descriptor(col + 1).type().data_type();
        if (dt != stored_dt) {
          errors.emplace_back(fmt::format("Type mismatch {} != {} at pos {}:{}", dt,
                                          stored_dt, col, row));
          success = false;
        }
        auto dimension =
            static_cast<uint64_t>(ac::TypeDescriptor(type_desc_tag).dimension());
        auto stored_dimension =
            row_ref.segment().column_descriptor(col + 1).type().dimension();
        if (dimension != static_cast<uint64_t>(stored_dimension)) {
          errors.emplace_back(fmt::format("Dimension mismatch {} != {} at pos {}:{}",
                                          dimension, stored_dimension, col, row));
          success = false;
        }
        ...
        if (value != stored_value) {
          errors.push_back(fmt::format("Value mismatch {} != {} at pos {}:{}", value,
                                       stored_value, col, row));
          success = false;
        }
      });
    }
...
inline bool check_test_frame(const TestDataFrame& data_frame,
                             ...
                             std::vector<std::string>& errors) {
  ac::StreamReader<arcticdb::entity::AtomKey,
                   folly::Function<std::vector<arcticdb::entity::AtomKey>()>,
                   arcticdb::SegmentInMemory::Row>
      stream_reader{[&]() { return std::vector<arcticdb::entity::AtomKey>{key}; },
                    std::move(store)};
...
namespace rc {
template <> struct Arbitrary<arrow::Decimal128> {
