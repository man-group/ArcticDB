...
static void BM_packed_bits_to_buffer(benchmark::State& state) {
    ...
    std::uniform_int_distribution<uint8_t> dis(std::numeric_limits<uint8_t>::min(), std::numeric_limits<uint8_t>::max());
    ...
    for (auto _ : state) {
        packed_bits_to_buffer(packed_bits, num_bits, dest_ptr);
    }
    delete[] dest_ptr;
}

BENCHMARK(BM_packed_bits_to_buffer)->Args({100'000})->Args({1'000'000})->Args({10'000'000})->Args({100'000'000})->Args({1'000'000'000});
