...
// We limit the number of concurrent threads to 8 to avoid the slab allocator poor
// performance with large amounts of threads:
...
template <typename MemoryChunk>
pointer_set<MemoryChunk> call_alloc(MemoryChunk& mc, std::size_t n,
...
template <typename MemoryChunk>
void check_sets(const pointer_set<MemoryChunk>& s1,
                const pointer_set<MemoryChunk>& s2) {
...
template <typename MemoryChunk> void run_test(MemoryChunk& mc, unsigned int K) {
  std::vector<int64_t> execution_times(num_threads);
  int64_t avg = 0;
  for (size_t k = 0; k < K; ++k) {
    std::vector<std::future<pointer_set<MemoryChunk>>> v;
    for (size_t i = 0; i < num_threads; ++i) {
      v.emplace_back(std::async(std::launch::async, call_alloc<MemoryChunk>,
                                std::ref(mc), num_blocks_per_thread,
    ...
    for (size_t i = 0; i < num_threads; ++i) {
      for (size_t j = i + 1; j < num_threads; ++j) {
        exceptions.emplace_back(std::async(std::launch::async, check_sets<MemoryChunk>,
...
TEST(SlabAlloc, MultipleThreads) {
  ...
  for (size_t k = 0; k < rounds; k++) {
    ...
    for (size_t i = 0; i < num_thds; i++) {
      res.emplace_back(std::async(std::launch::async, perform_allocations, std::ref(mc),
                                  cap / num_thds));
    }
