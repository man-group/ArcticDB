...
TEST(Cursor, Behaviour) {
  ...
  ASSERT_THROW(
      b.commit(),
      ArcticCategorizedException<ErrorCategory::INTERNAL>); // commit called twice
  ASSERT_EQ(b.size<uint8_t>(), 100);
...
template <typename BufferType> void test_cursor_backing() {
  using namespace arcticdb;
  ...
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(40 * sizeof(uint64_t), sizeof(uint64_t)),
      40);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(49 * sizeof(uint64_t), sizeof(uint64_t)),
      49);
  b.ensure_bytes(400);
  memcpy(b.cursor(), v.data() + 50, 400);
  b.commit();
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(0 * sizeof(uint64_t), sizeof(uint64_t)),
      0);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(40 * sizeof(uint64_t), sizeof(uint64_t)),
      40);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(49 * sizeof(uint64_t), sizeof(uint64_t)),
      49);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(50 * sizeof(uint64_t), sizeof(uint64_t)),
      50);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(73 * sizeof(uint64_t), sizeof(uint64_t)),
      73);
  ASSERT_EQ(
      *b.buffer().template ptr_cast<uint64_t>(99 * sizeof(uint64_t), sizeof(uint64_t)),
      99);
}
