...
struct LockData {
  ...
  LockData(size_t num_tests)
      : lock_name_("stress_test_lock"), store_(std::make_shared<InMemoryStore>()),
        vol_(0), atomic_(0), contended_(false), num_tests_(num_tests),
        timedout_(false) {}
};
...
struct PessimisticLockTask {
  ...
  PessimisticLockTask(std::shared_ptr<LockData> data,
                      std::optional<size_t> timeout_ms = std::nullopt)
      : data_(std::move(data)), timeout_ms_(timeout_ms) {}
...
struct ForceReleaseLockTask {
  ...
  folly::Future<folly::Unit> operator()() {
    ...
    // Clean up locks to avoid "mutex destroyed while active" errors on Windows debug
    // build
...
struct OptimisticForceReleaseLockTask {
  ...
  OptimisticForceReleaseLockTask(std::shared_ptr<LockData> data, size_t timeout_ms,
                                 size_t retry_ms)
      : data_(std::move(data)), timeout_ms_(timeout_ms), retry_ms_(retry_ms) {}
...
TEST(StorageLock, ForceReleaseLock) {
  ...
  // Then each thread simulates forgetting to release the lock, so that other threads
  // need to rely on the TTL expiring to be able to acquire the lock. Initially take the
  // lock, so that the first thread also has to wait for the TTL of that lock to expire.
  SKIP_MAC("StorageLock is not supported");
  using namespace arcticdb;
  ...
  // If a lock is preempted, then it will still print the warning about having
  // overridden the lock due to the TTL expiring, but will then have to retry, so there
  // may be more than the expected number of log messages. Skip on Windows as capturing
  // logs doesn't work. TODO: Configure the logger with the file output
#ifndef _WIN32
  ASSERT_TRUE(count_occurrences(stdout_str, expected) >= 4 ||
...
TEST(StorageLock, OptimisticForceReleaseLock) {
  ...
  // will then simulate forgetting to release the lock, so that other threads need to
  // rely on the TTL expiring to be able to acquire the lock. Initially take the lock
  // using the lock() method, so that the first thread also has to wait for the TTL of
  // that lock to expire.
  SKIP_MAC("StorageLock is not supported");
  using namespace arcticdb;
  ...
  for (auto i = size_t{0}; i < 4; ++i) {
    futures.emplace_back(
        exec.addFuture(OptimisticForceReleaseLockTask{lock_data, 10 * 1000, 100}));
  }
