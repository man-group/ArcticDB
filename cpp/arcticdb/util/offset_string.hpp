...
// Unset the definition of `copysign` that is defined in `Python.h` for Python < 3.8 on
// Windows. See: https://github.com/python/cpython/pull/23326
...
void remove_nones_and_nans(
    ankerl::unordered_dense::set<OffsetString::offset_t>& offsets);

template <typename LockPtrType> inline PyObject* create_py_nan(LockPtrType& lock) {
  lock->lock();
