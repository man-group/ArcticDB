...
void densify_buffer_using_bitmap(const util::BitSet& block_bitset,
  ...
  while (en < en_end) {
    ...
    ARCTICDB_TRACE(log::version(),
                   ...
                   copy_from - sparse_ptr, copy_to - dense_ptr,
...
template <typename RawType>
inline void expand_dense_buffer_using_bitmap(const util::BitMagic& bv,
                                             const uint8_t* dense_ptr,
                                             uint8_t* sparse_ptr) {
  auto en = bv.first();
  auto en_end = bv.end();
  ...
  while (en < en_end) {
    ...
    ARCTICDB_TRACE(log::version(),
                   ...
                   copy_from - dense_ptr, copy_to - sparse_ptr,
                   *(reinterpret_cast<const RawType*>(copy_from)));
  ...
  if constexpr (is_sequence_type(data_type)) {
    ...
    if (handler) {
      ...
      internal::raise<ErrorCode::E_INVALID_ARGUMENT>(
          "Default initialization for {} is not implemented.", type_descriptor);
    }
  }
}

[[nodiscard]] util::BitSet scan_object_type_to_sparse(const PyObject* const* ptr,
                                                      size_t rows_to_write);

template <typename RawType>
ChunkedBuffer scan_floating_point_to_sparse(RawType* ptr, size_t rows_to_write,
...
inline util::BitMagic deserialize_bytes_to_bitmap(const std::uint8_t*& input,
                                                  size_t bytes_in_sparse_bitmap) {
  util::BitMagic bv;
