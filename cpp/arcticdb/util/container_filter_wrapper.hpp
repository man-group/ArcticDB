 ...
 * The most efficient implementation is obviously a bit-set-based view on the original_
 * container, but that requires too much code.
 *
 * **NOT THREAD SAFE**
 */
template <typename Container> class ContainerFilterWrapper {
...
public:
  ...
  /**
   * The filter should take an item and return true to indicate if the item should be
   * removed.
   *
   * If the filter throws, the result might be corrupted.
   */
  template <typename Filter> void remove_if(Filter filter) {
    if (use_original_) {
      ...
      filtered_.erase(std::remove_if(filtered_.begin(), filtered_.end(), filter),
                      filtered_.end());
    }
  }
  ...
  void insert(const std::enable_if_t<std::is_same_v<C, std::unordered_set<value_type>>,
                                     value_type>& val) {
    if (use_original_) {
   ...
   */
  template <typename C = Container>
  void erase(const std::enable_if_t<std::is_same_v<C, std::unordered_set<value_type>>,
                                    value_type>& val) {
    if (use_original_) {
      if (original_.count(val) == 0) {
        return;
