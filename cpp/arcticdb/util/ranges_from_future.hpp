...
/**
 * Pre-C++20 emulation of ranges::views::elements, with the ability to alter the output
 * type.
 * @tparam OutType Override the output type. Must have a c'tor that can accept the
 * element's original type.
 */
template <size_t n, typename RangeOfPairs,
          typename OutType = std::remove_const_t<
              typename std::tuple_element<n, typename RangeOfPairs::value_type>::type>>
 ...
 */
template <typename Map> inline auto keys(const Map& map) {
 ...
 */
template <typename Map> inline auto values(const Map& map) {
  return copy_of_elements<1>(map);
}

/**
 * Like std::views::values(), but alters the element type. See copy_of_elements'
 * documentation.
 */
// No direct upgrade path to ranges, but trivial given how copy_of_elements is
// implemented.
