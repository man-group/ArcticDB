...
template <typename BufferType> struct CursoredBuffer {
private:
  Cursor cursor_;
  BufferType buffer_;
...
public:
  ...
  explicit CursoredBuffer(BufferType&& buffer)
      : cursor_(0), buffer_(std::move(buffer)) {}
  ...
  uint8_t* ptr() { return &buffer_[cursor_.pos()]; }

  [[nodiscard]] position_t cursor_pos() const { return cursor_.pos(); }

  template <typename T> void ensure(size_t num = 1) {
    buffer_.ensure((num * sizeof(T)) + cursor_.pos());
  }

  void ensure_bytes(size_t bytes) { buffer_.ensure(cursor_.pos() + bytes); }

  uint8_t* ensure_aligned_bytes(size_t bytes) {
    return buffer_.ensure(cursor_.pos() + bytes, true);
  }

  void commit() { cursor_.commit(buffer_.bytes()); }

  void advance(std::size_t size) { cursor_.advance(position_t(size), buffer_.bytes()); }

  template <typename T> [[nodiscard]] size_t size() const {
  ...
  [[nodiscard]] size_t bytes() const { return buffer_.bytes(); }
  ...
  void compact_blocks() {
    ...
    util::check(cursor_ == tmp.cursor_, "Cursor mismatch on compact_blocks, {} != {}",
                cursor_, tmp.cursor_);
    std::swap(tmp.buffer_, buffer_);
  }

  template <class T> T* pos_cast(size_t required_bytes) {
    return buffer_.template ptr_cast<T>(cursor_.pos(), required_bytes);
  }

  uint8_t* cursor() { return &buffer_[cursor_.pos()]; }

  template <typename T> T& typed_cursor() { return *(reinterpret_cast<T*>(cursor())); }

  template <typename T>
  const T* ptr_cast(position_t t_pos, size_t required_bytes) const {
    return reinterpret_cast<const T*>(
        buffer_.template ptr_cast<T>(t_pos * sizeof(T), required_bytes));
  }

  template <typename T> T* ptr_cast(position_t pos, size_t required_bytes) {
    return const_cast<T*>(
        const_cast<const CursoredBuffer*>(this)->ptr_cast<T>(pos, required_bytes));
  }
