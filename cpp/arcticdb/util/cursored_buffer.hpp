  ...
  public:
    ...
    CursoredBuffer(size_t size, AllocationType allocation_type) :
        ...
        buffer_(allocation_type == AllocationType::PRESIZED ? BufferType::presized(size)
                                                            : BufferType{size, allocation_type}) {}
    ...
    uint8_t* ensure_aligned_bytes(size_t bytes) { return buffer_.ensure(cursor_.pos() + bytes, true); }
    ...
    uint8_t* bytes_at(size_t bytes, size_t required) { return buffer_.bytes_at(bytes, required); }

    const uint8_t* bytes_at(size_t bytes, size_t required) const { return buffer_.bytes_at(bytes, required); }
