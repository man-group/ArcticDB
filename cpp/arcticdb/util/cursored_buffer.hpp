...
struct CursoredBuffer {
  ...
  public:
    ...
    CursoredBuffer(size_t size, AllocationType allocation_type) :
        ...
        buffer_(allocation_type == AllocationType::DYNAMIC ? BufferType{size}
                                                           : BufferType::presized(size, allocation_type)) {}
    ...
    uint8_t* bytes_at(size_t bytes, size_t required) {
        util::check(
                bytes + required <= buffer_.bytes(),
                "Bytes overflow, can't write {} bytes at position {} in buffer of size {}",
                required,
                bytes,
                buffer_.bytes()
        );
