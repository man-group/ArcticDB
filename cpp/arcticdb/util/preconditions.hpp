...
template <ErrorCode code, ErrorCategory error_category> struct Raise {
  static_assert(get_error_category(code) == error_category);

  template <typename... Args>
  [[noreturn]] void operator()(fmt::format_string<Args...> format,
                               Args&&... args) const {
    std::string msg;
    if constexpr (sizeof...(args) == 0) {
      msg = fmt::format(FMT_COMPILE("{} {}"), error_code_data<code>.name_, format);
    } else {
      std::string combo_format =
          fmt::format(FMT_COMPILE("{} {}"), error_code_data<code>.name_, format);
...
template <ErrorCode code, ErrorCategory error_category> struct Check {
...
template <ErrorCode code> constexpr auto raise = check<code>.raise;
} // namespace internal

namespace debug {
#ifdef DEBUG_BUILD
template <ErrorCode code> constexpr auto check = internal::check<code>;
#else
template <ErrorCode code, typename... Args>
inline void check(ARCTICDB_UNUSED bool cond,
...
inline void check(ARCTICDB_UNUSED bool cond, ARCTICDB_UNUSED FormatString format,
                  ARCTICDB_UNUSED Args&&... args) {}
#endif
} // namespace debug
...
constexpr auto check =
    util::detail::Check<ErrorCode::E_ASSERTION_FAILURE, ErrorCategory::INTERNAL>{};

constexpr auto check_range_impl =
    util::detail::Check<ErrorCode::E_INVALID_RANGE, ErrorCategory::INTERNAL>{};

template <typename... Args> void check_range(size_t idx, size_t size, const char* msg) {
  check_range_impl(idx < size, "{} expected  0 <= idx < size, actual idx={}, size={}",
                   msg, idx, size);
}

constexpr auto check_arg =
    util::detail::Check<ErrorCode::E_INVALID_ARGUMENT, ErrorCategory::INTERNAL>{};

// TODO Replace occurrences with specific error code
constexpr auto check_rte =
    util::detail::Check<ErrorCode::E_RUNTIME_ERROR, ErrorCategory::INTERNAL>{};
...
struct formatter<A, std::enable_if_t<std::is_invocable_v<A>, char>> {
  ...
  template <typename FormatContext> auto format(const A& a, FormatContext& ctx) const {
