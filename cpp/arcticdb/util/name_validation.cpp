...
void verify_name(const std::string& name_type_for_error, const StringId& name,
  ...
  if (name.empty()) {
    user_input::raise<ErrorCode::E_INVALID_USER_ARGUMENT>(
        "The {} cannot be an empty string.", name_type_for_error);
  }
  if (name.size() > MAX_SIZE) {
    user_input::raise<ErrorCode::E_NAME_TOO_LONG>(
        "The {} length exceeds the max supported length. {} length: {}, Max Supported "
        "Length: {}",
        name_type_for_error, name_type_for_error, name.size(), MAX_SIZE);
  }
  for (unsigned char c : name) {
    if (check_symbol_out_of_range && (c < 32 || c > 126)) {
      user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
          "The {} can contain only valid ASCII chars in the range 32-126 inclusive. "
          "{}: {} BadChar: {}",
          name_type_for_error, name_type_for_error, name, c);
    }
    if (unsupported_chars.find(c) != unsupported_chars.end()) {
      user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
          "The {} contains unsupported chars. {}: {} BadChar: {}", name_type_for_error,
          name_type_for_error, name, c);
    }
  }
  if (unsupported_prefix.has_value() && name[0] == unsupported_prefix.value()) {
    user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
        "The {} starts with an unsupported prefix. {}: {} Unsupported prefix: {} ",
        name_type_for_error, name_type_for_error, name, unsupported_prefix.value());
  }
  if (unsupported_suffix.has_value() &&
      name[name.size() - 1] == unsupported_suffix.value()) {
    user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
        "The {} ends with an unsupported suffix. {}: {} Unsupported suffix: {} ",
        name_type_for_error, name_type_for_error, name, unsupported_suffix.value());
  }
}
...
void verify_symbol_key(const StreamId& symbol_key) {
  if (ConfigsMap::instance()->get_int("VersionStore.NoStrictSymbolCheck")) {
    ARCTICDB_DEBUG(log::version(),
                   "Key with stream id {} will not be strictly checked because "
                   "VersionStore.NoStrictSymbolCheck variable is set to 1.",
                   symbol_key);
    return;
  }
  ...
  util::variant_match(
      ...
      [](const NumericId& num_symbol_key) {
        ...
        ARCTICDB_DEBUG(log::version(),
                       "Nothing to verify in stream id {} as it contains a NumericId.",
                       num_symbol_key);
        return;
      },
...
// Library names starting with "/" fail if storage is LMDB and library parts starting
// with "/" will fail in Mongo
...
void verify_library_path_part(const std::string& library_part, char delim) {
  if (library_part.empty()) {
    user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
        "Library name has an empty part. Parts are separated by delimiter: '{}'. This "
        "is currently not supported.",
        delim);
  }
  if (library_part[0] == UNSUPPORTED_LMDB_MONGO_PREFIX) {
    user_input::raise<ErrorCode::E_INVALID_CHAR_IN_NAME>(
        "Library name part starts with an invalid character. This is currently not "
        "supported. Library Name Part: '{}', Bad prefix: {}",
        library_part, UNSUPPORTED_LMDB_MONGO_PREFIX);
  }
}
...
void verify_library_path_on_write(const Store* store, const StringId& library_path) {
  ...
  user_input::check<ErrorCode::E_INVALID_CHAR_IN_NAME>(
      ...
      "The library name contains unsupported chars. Library Name: {}", library_path);
}
