...
template <typename T, std::size_t cache_line_size = 128> class SlabAllocator {
private:
  template <typename Value, typename Tag> struct tagged_value {
    // We have tag for every value to counter https://en.wikipedia.org/wiki/ABA_problem
    // of lock free implementation
    Value value;
    Tag tag;
  };
...
private:
  ...
  // Block size should be max of the value_type (used for actual storing the value) and
  // size_type (storing offset)
  using block_t =
      std::conditional_t<sizeof(value_type) < sizeof(size_type), size_type, value_type>;

  using tagged_value_t = tagged_value<size_type, size_type>;

public:
  explicit SlabAllocator(size_type capacity)
      : capacity_(capacity), main_memory_(new block_t[capacity_]),
        num_free_blocks_(capacity), next_free_offset_({0, 0}), cb_activated_(false) {
    size_type i = 0;
  ...
  pointer allocate() noexcept {
    ...
    // The LOG_SLAB_ALLOC_INTERNALS can be used to check for contention in getting the
    // next free block by many threads.
    ...
    } while (!next_free_offset_.compare_exchange_strong(curr_next_free_offset,
                                                        new_next_free_offset));
#ifdef LOG_SLAB_ALLOC_INTERNALS
    if (allocation_attempts > 10) {
      // We only print when we encounter a lot of allocation_attempts because otherwise
      // we remove the contention effect by effectively pausing every time to print.
  ...
  void deallocate(pointer p) noexcept {
    ...
    do {
      ...
      new_next_free_offset.value =
          static_cast<size_type>(reinterpret_cast<block_t*>(p) - main_memory_);
      // increase the id tag to prevent the ABA problem
      new_next_free_offset.tag = curr_next_free_offset.tag + 1;
  ...
  void manage_slab_capacity() {
    ...
    if (n / (float)capacity_ <= slab_activate_cb_cutoff) {
      ...
      if (try_changing_cb(true)) {
        ARCTICDB_TRACE(log::inmem(), "Memory reached cutoff, calling callbacks in slab "
                                     "allocator to free up memory");
        std::scoped_lock<std::mutex> lock(mutex_);
