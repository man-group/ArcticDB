...
namespace util {
/**
 * The decimal class is binary compatible with pyarrow's layout for 128 bit decimals.
 * The format keeps the number as 128 bit integer in two's complement. The scale is not
 * part of the format.
...
public:
  ...
  /**
   * Compare two decimals. Since the format does not keep decimal point position, the
