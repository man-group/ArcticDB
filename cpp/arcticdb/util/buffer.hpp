...
template <class T, bool owning> struct BaseBuffer {
...
struct Buffer : public BaseBuffer<Buffer, true> {
  ...
  static auto presized(size_t size) { return Buffer(size); };
  ...
  [[nodiscard]] Buffer clone() const {
    ...
    if (total_bytes() > 0) {
      ...
      util::check(data_ != nullptr && output.data_ != nullptr,
                  "Error in buffer allocation of size {} + {}", body_bytes_,
                  preamble_bytes_);
  ...
  [[nodiscard]] T* ptr_cast(size_t bytes_offset, size_t required_bytes) {
    ...
    if (bytes_offset + required_bytes > bytes()) {
      std::string err = fmt::format(
          "Cursor overflow in reallocating buffer ptr_cast, cannot read {} bytes from "
          "a buffer of size {} with cursor "
          "at {}, as it would require {} bytes. ",
          required_bytes, bytes(), bytes_offset, bytes_offset + required_bytes);
  ...
  inline void set_bytes(size_t bytes) {
    util::check(bytes <= available(),
                "Can't set bytes to larger than the buffer: {} > {}", bytes,
                available());
    body_bytes_ = bytes;
    check_invariants();
  }
  ...
  [[nodiscard]] BufferView view() const { return {ptr_, body_bytes_}; }

  [[nodiscard]] uint8_t& operator[](size_t bytes_offset) { return ptr_[bytes_offset]; }

  [[nodiscard]] const uint8_t& operator[](size_t bytes_offset) const {
    return ptr_[bytes_offset];
  }

  [[nodiscard]] size_t total_bytes() const { return preamble_bytes_ + body_bytes_; }

  [[nodiscard]] size_t preamble_bytes() const { return preamble_bytes_; }

  [[nodiscard]] uint8_t* preamble() { return data_; }
...
private:
  inline void resize(size_t alloc_bytes) {
    ...
    util::check(alloc_bytes >= preamble_bytes_,
                "The requested size of a resizes call is less than the preamble bytes");
    auto [mem_ptr, ts] =
        ptr_ ? Allocator::realloc(std::make_pair(data_, ts_), alloc_bytes)
             : Allocator::aligned_alloc(alloc_bytes);

    ARCTICDB_TRACE(log::codec(), "Allocating {} bytes ({} + {} bytes preamble)",
                   alloc_bytes, bytes, preamble_bytes_);
    if (mem_ptr) {
      data_ = mem_ptr;
      ...
      ARCTICDB_TRACE(log::version(),
                     ...
                     uintptr_t(this), bytes, uintptr_t(ptr_), uintptr_t(data_),
                     capacity_, body_bytes_);
    } else {
      throw std::bad_alloc();
    }
    check_invariants();
  ...
  void check_invariants() const {
    ...
    util::check(preamble_bytes_ + body_bytes_ <= capacity_,
                "total_bytes exceeds capacity {} + {} > {}", preamble_bytes_,
                body_bytes_, capacity_);
    util::check(total_bytes() == preamble_bytes_ + body_bytes_,
                "Total bytes calculation is incorrect {} != {} + {}", total_bytes(),
                preamble_bytes_, body_bytes_);
    util::check(data_ + preamble_bytes_ == ptr_,
                "Buffer pointer is in the wrong place {} + {} != {}", uintptr_t(data_),
                preamble_bytes_, uintptr_t(ptr_));
#endif
  }
...
public:
  ...
  VariantBuffer(BufferType&& buf) : buffer_(std::forward<decltype(buf)>(buf)) {}
  ...
  [[nodiscard]] VariantBuffer clone() const {
    return util::variant_match(
        ...
        [](const BufferView& bv) {
          ...
          bv.copy_to(*b);
          return VariantBuffer{std::move(b)};
        },
        [](const std::shared_ptr<Buffer>& buf) {
          return VariantBuffer{std::make_shared<Buffer>(buf->clone())};
        },
        [](const std::monostate) -> VariantBuffer {
          util::raise_rte("Uninitialized buffer");
        });
  }

  template <typename BufferType> VariantBuffer& operator=(BufferType&& buf) {
  ...
  uint8_t* data() {
    return util::variant_match(
        buffer_, [](BufferView& bv) { return bv.data(); },
        [](const std::shared_ptr<Buffer>& buf) { return buf->data(); },
        [](const std::monostate) -> uint8_t* {
          util::raise_rte("Uninitialized buffer");
        });
  }
  ...
  [[nodiscard]] BufferView view() const {
    return util::variant_match(
        ...
        [](const BufferView& b) { return b; },
        [](const std::shared_ptr<Buffer> b) { return b->view(); });
  }

  [[nodiscard]] std::size_t bytes() const {
    std::size_t s = 0;
    util::variant_match(
        buffer_, [](const std::monostate&) { /* Uninitialized buffer */ },
  ...
  void move_buffer(VariantBuffer&& that) {
    if (is_uninitialized() || that.is_uninitialized()) {
      ...
      if (is_owning_buffer()) {
        swap(*std::get<std::shared_ptr<Buffer>>(buffer_),
             *std::get<std::shared_ptr<Buffer>>(that.buffer_));
      } else {
        swap(std::get<BufferView>(buffer_), std::get<BufferView>(that.buffer_));
      }
    } else if (is_owning_buffer()) {
      log::storage().info("Copying segment");
      // data of segment being moved is not owned, moving it is dangerous, copying
      // instead
      std::get<BufferView>(that.buffer_)
          .copy_to(*std::get<std::shared_ptr<Buffer>>(buffer_));
    } else {
