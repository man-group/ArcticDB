...
struct Buffer : public BaseBuffer<Buffer, true> {
    ...
    explicit Buffer(size_t size, std::optional<size_t> preamble = std::nullopt) { reserve(size, preamble); }
    ...
    static auto presized(size_t size, entity::AllocationType) { return Buffer(size); };
    ...
    [[nodiscard]] Buffer clone() const {
        ...
        if (total_bytes() > 0) {
            ...
            util::check(
                data_ != nullptr && output.data_ != nullptr,
                "Error in buffer allocation of size {} + {}",
                body_bytes_,
                preamble_bytes_
            );
    ...
    [[nodiscard]] T* ptr_cast(size_t bytes_offset, size_t required_bytes) {
        ...
        if (bytes_offset + required_bytes > bytes()) {
            std::string err = fmt::format(
                "Cursor overflow in reallocating buffer ptr_cast, cannot read {} bytes from a buffer of size {} with "
                "cursor "
    ...
    inline void assert_size(size_t bytes) const {
        util::check(
            bytes <= body_bytes_, "Expected allocation size {} smaller than actual allocation {}", bytes, body_bytes_
        );
    }
    ...
    [[nodiscard]] const uint8_t& operator[](size_t bytes_offset) const { return ptr_[bytes_offset]; }
  ...
  private:
    inline void resize(size_t alloc_bytes) {
        ...
        util::check(
            alloc_bytes >= preamble_bytes_, "The requested size of a resizes call is less than the preamble bytes"
        );
        auto [mem_ptr, ts] =
            ptr_ ? Allocator::realloc(std::make_pair(data_, ts_), alloc_bytes) : Allocator::aligned_alloc(alloc_bytes);

        ARCTICDB_TRACE(
            log::codec(), "Allocating {} bytes ({} + {} bytes preamble)", alloc_bytes, bytes, preamble_bytes_
        );
        if (mem_ptr) {
            data_ = mem_ptr;
    ...
    void check_invariants() const {
#ifdef DEBUG_BUILD
        util::check(
            preamble_bytes_ + body_bytes_ <= capacity_,
        ...
        util::check(
            total_bytes() == preamble_bytes_ + body_bytes_,
...
class VariantBuffer {
  ...
  public:
    ...
    [[nodiscard]] bool is_uninitialized() const { return std::holds_alternative<std::monostate>(buffer_); }
    ...
    [[nodiscard]] bool is_owning_buffer() const { return std::holds_alternative<std::shared_ptr<Buffer>>(buffer_); }
