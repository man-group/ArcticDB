...
TypeHandlerRegistry::get_handler(const entity::TypeDescriptor& type_descriptor) const {
...
void TypeHandlerRegistry::register_handler(
    const entity::TypeDescriptor& type_descriptor, TypeHandler&& handler) {
  handlers_.try_emplace(type_descriptor,
                        std::make_shared<TypeHandler>(std::move(handler)));
}

size_t
TypeHandlerRegistry::Hasher::operator()(const entity::TypeDescriptor descriptor) const {
  static_assert(sizeof(descriptor) == sizeof(uint16_t),
                "Cannot compute util::TypeDescriptor's hash. The size is wrong.");
  ...
  return hasher(uint16_t(descriptor.data_type()) << 8 |
                uint16_t(descriptor.dimension()));
}
