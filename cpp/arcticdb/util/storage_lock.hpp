...
inline StreamDescriptor lock_stream_descriptor(const StreamId& stream_id) {
  return StreamDescriptor{stream_descriptor(
      stream_id, stream::RowCountIndex(), {scalar_field(DataType::UINT64, "version")})};
}
...
inline std::thread::id get_thread_id() noexcept { return std::this_thread::get_id(); }

template <class ClockType = util::SysClock> class StorageLock {
  // 1 Day
...
public:
  static void force_release_lock(const StreamId& name,
  ...
  void unlock(const std::shared_ptr<Store>& store) {
    if (auto read_ts = read_timestamp(store); !read_ts || *read_ts != ts_) {
      log::version().warn("Unexpected lock timestamp, {} != {}", read_ts ? *read_ts : 0,
                          ts_);
      mutex_.unlock();
      return;
    }
...
private:
  void do_lock(const std::shared_ptr<Store>& store,
  ...
  do_wait:
    while (ref_key_exists(store)) {
      ...
      if (timeout_ms && total_wait > *timeout_ms) {
        ...
        throw StorageLockTimeout{fmt::format(
            "Storage lock {} timeout out after {} ms. Lock held since {} (UTC)", name_,
            total_wait, date_and_time(*read_ts))};
      }
    }
    ts_ = create_ref_key(store);
    ...
    auto read_ts = read_timestamp(store);
    if (!read_ts || *read_ts != ts_) {
      log::lock().info("Lock preempted, expected timestamp {} but got {}", ts_,
                       read_ts.value_or(0));
      ts_ = 0;
      goto do_wait;
    }
  ...
  static void do_remove_ref_key(const std::shared_ptr<Store>& store,
                                const StreamId& name) {
  ...
  std::optional<timestamp> ttl_not_expired(const std::shared_ptr<Store>& store) {
    ...
    if (read_ts) {
      // check TTL
      auto ttl =
          ConfigsMap::instance()->get_int("StorageLock.TTL", DEFAULT_TTL_INTERVAL);
      if (ClockType::coarse_nanos_since_epoch() - *read_ts > ttl) {
        log::lock().warn(
            "StorageLock {} taken for more than TTL (default 1 day). Force releasing",
            name_);
...
public:
  StorageLockWrapper(const StreamId& stream_id, std::shared_ptr<Store> store)
      : store_(std::move(store)), lock_(std::make_shared<StorageLock<>>(stream_id)) {}
