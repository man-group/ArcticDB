...
/*
    ...
    child process. So special handling is required.
    The thread being forked must not have a running task. And the parent process may have another threads running task, locking the mutex. As other threads
    in the parent process won't be forked, at fork, it is safe to reset the mutex memory. The mutex cannot be simply unlocked as unlock() should only be
    called in the thread called lock() previously, which cannot be the thread being forked.
