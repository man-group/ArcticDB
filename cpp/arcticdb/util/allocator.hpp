...
public:
  ...
  static void track_realloc(AddrIdentifier old_addr, AddrIdentifier new_addr,
                            size_t size);
...
private:
  ...
  static void init_slab() {
    static const size_t page_slab_capacity = ConfigsMap::instance()->get_int(
        "Allocator.PageSlabCapacity", 1000 * 1000); // 4GB
    if (use_slab_allocator()) {
      page_size_slab_allocator_ =
          std::make_shared<SlabAllocatorType>(page_slab_capacity);
    }
  }
#endif
...
public:
  ...
  static void destroy_instance();
  static std::pair<uint8_t*, entity::timestamp>
  alloc(size_t size, bool no_realloc ARCTICDB_UNUSED = false);
  static void trim();
  static void maybe_trim();
  static std::pair<uint8_t*, entity::timestamp> aligned_alloc(size_t size,
                                                              bool no_realloc = false);
  static std::pair<uint8_t*, entity::timestamp>
  realloc(std::pair<uint8_t*, entity::timestamp> ptr, size_t size);
