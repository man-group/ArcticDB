...
constexpr static std::array<uint64_t, std::numeric_limits<uint64_t>::digits10 + 1>
    powers_of_10{1,
                 10,
                 100,
                 1000,
...
constexpr static auto check_valid_decimal =
    arcticdb::util::detail::Check<ErrorCode::E_INVALID_DECIMAL_STRING,
...
[[nodiscard]] static inline boost::multiprecision::uint128_t
to_uint128(const uint64_t most_significant, const uint64_t least_significant) {
...
// Used instead of std::strtoull to parse chuncks of strings which can contain numbers
// larger than what ull can hold. std::strtoull would try to parse the whole underlying
// string (and potentially overflow) while this stops when the string_view length is
// reached.
...
void NumberComponents::handle_exponent(std::string_view& input, int exponent_position) {
  if (exponent_position != -1) {
    ...
    exponent_ =
        std::stoi(input.data() + exponent_position + 1, &processed_digits_count);
    check_valid_decimal(exponent_position + processed_digits_count ==
                            input.length() - 1,
...
NumberComponents::SpecialSymbols
NumberComponents::scan_for_special_symbols(std::string_view input) {
  ...
  for (size_t i = 0; i < input.length(); ++i) {
    ...
    if (current_symbol == '.') {
      ...
      check_valid_decimal(is_digit(current_symbol),
...
void NumberComponents::expand_exponent(int decimal_point_position) {
  ...
  const int digits_after_decimal_point =
      is_decimal() ? size_ - decimal_point_position : 0;
  const int zeros_to_append = std::max(0, exponent_ - digits_after_decimal_point);
  check_valid_decimal(size_ + zeros_to_append <= max_digits,
                      "Cannot parse decimal from string. "
                      "Overflow. Input has more than {} significant digits.",
                      max_digits);
...
Decimal::Decimal(std::string_view number) : data_{0} {
  ...
  // Boost's uint128_t is not in two's complement, so it cannot be reinterpret_cast over
  // the data.
  // TODO: potential optimization for Clang/GCC would be to load it __uint128_t and
  // reinterpret_cast
  //  it over the array.
...
std::string Decimal::to_string(int scale) const {
  ...
  boost::multiprecision::uint128_t number = to_uint128(
      data_[MOST_SIGNIFICANT_WORD_INDEX], data_[LEAST_SIGNIFICANT_WORD_INDEX]);
