...
template <class Msg> void pack_to_any(const Msg& msg, google::protobuf::Any& any) {
...
template <class Msg> bool pb_equals(const Msg& a, const Msg& b) {
  return google::protobuf::util::MessageDifferencer::Equals(a, b);
}

template <class T> std::optional<T> as_opt(T val, const T& sentinel = T()) {
  if (val == sentinel) {
    return std::nullopt;
  }
