...
std::pair<uint8_t*, entity::timestamp> AllocatorImpl<TracingPolicy, ClockType>::realloc(
    ...
    ARCTICDB_TRACE(
        log::codec(), "Reallocating {} bytes from {} to {}", util::MemBytes{size}, uintptr_t(ptr.first), uintptr_t(ret)
    );
#endif
    auto ts = current_timestamp();
    TracingPolicy::track_realloc(
        std::make_pair(uintptr_t(ptr.first), ptr.second), std::make_pair(uintptr_t(ret), ts), size
    );
    return {ret, ts};
}
