...
bool use_slab_allocator() {
  static const bool use_it =
      ConfigsMap::instance()->get_int("Allocator.UseSlabAllocator", 1);
  return use_it;
}
...
void TracingData::track_alloc(AddrIdentifier addr_ts, size_t size) {
  ...
  ARCTICDB_TRACE(
      ...
      util::MemBytes{size}, addr_ts.first, addr_ts.second,
      util::MemBytes{impl_->total_allocs_}, impl_->total_irregular_allocs_,
      impl_->total_allocs_calls_);
}
...
void TracingData::track_free(AddrIdentifier addr_ts) {
  ...
  util::check(it != impl_->allocs_.end(), "Unrecognized address in free {}:{}",
              addr_ts.first, addr_ts.second);
  util::check(impl_->total_allocs_ >= it->second,
              "Request to free {}  from {}:{} when only {} remain", it->second,
              addr_ts.first, addr_ts.second, impl_->total_allocs_.load());
  impl_->total_allocs_ -= it->second;
  ARCTICDB_TRACE(log::codec(), "Freed {} at {}:{}, total allocation {}",
                 util::MemBytes{it->second}, addr_ts.first, addr_ts.second,
                 util::MemBytes{impl_->total_allocs_.load()});
  impl_->allocs_.erase(it);
}

void TracingData::track_realloc(AddrIdentifier old_addr, AddrIdentifier new_addr,
                                size_t size) {
  if (old_addr.first != 0)
    track_free(old_addr);
...
void InMemoryTracingPolicy::track_realloc(AddrIdentifier old_addr,
                                          AddrIdentifier new_addr, size_t size) {
...
bool InMemoryTracingPolicy::deallocated() {
  ...
  if (!all_freed) {
    log::memory().warn("Allocator has not freed all data, {} bytes counted",
                       get_data.impl_->total_allocs_);
...
namespace {
template <typename TracingPolicy, typename ClockType> auto& free_count_of() {
...
std::shared_ptr<AllocatorImpl<TracingPolicy, ClockType>>
    AllocatorImpl<TracingPolicy, ClockType>::instance_;
...
template <class TracingPolicy, class ClockType>
uint8_t* AllocatorImpl<TracingPolicy, ClockType>::internal_realloc(uint8_t* p,
                                                                   std::size_t size) {
  uint8_t* ret;
#ifdef USE_SLAB_ALLOCATOR
  std::call_once(slab_init_flag_, &init_slab);
  auto raw_pointer = reinterpret_cast<SlabAllocatorType::pointer>(p);
  if (use_slab_allocator() && page_size_slab_allocator_->is_addr_in_slab(raw_pointer)) {
    ARCTICDB_TRACE(log::codec(), "Doing slab realloc of address {} and size {}",
                   uintptr_t(p), size);
    if (size == page_size)
      return p;
    else {
    ...
    ARCTICDB_TRACE(log::codec(), "Doing normal realloc of address {} and size {}",
                   uintptr_t(p), size);
...
template <typename TracingPolicy, typename ClockType>
std::shared_ptr<AllocatorImpl<TracingPolicy, ClockType>>
AllocatorImpl<TracingPolicy, ClockType>::instance() {
  std::call_once(AllocatorImpl<TracingPolicy, ClockType>::init_flag_,
                 &AllocatorImpl<TracingPolicy, ClockType>::init);
  return instance_;
}
...
AllocatorImpl<TracingPolicy, ClockType>::alloc(size_t size,
...
void AllocatorImpl<TracingPolicy, ClockType>::maybe_trim() {
  static const uint32_t trim_count =
      ConfigsMap::instance()->get_int("Allocator.TrimCount", 250);
...
template <typename TracingPolicy, typename ClockType>
std::pair<uint8_t*, entity::timestamp>
AllocatorImpl<TracingPolicy, ClockType>::aligned_alloc(
...
std::pair<uint8_t*, entity::timestamp> AllocatorImpl<TracingPolicy, ClockType>::realloc(
  ...
  ARCTICDB_TRACE(log::codec(), "Reallocating {} bytes from {} to {}",
                 util::MemBytes{size}, uintptr_t(ptr.first), uintptr_t(ret));
#endif
  auto ts = current_timestamp();
  TracingPolicy::track_realloc(std::make_pair(uintptr_t(ptr.first), ptr.second),
                               std::make_pair(uintptr_t(ret), ts), size);
  return {ret, ts};
}

template <class TracingPolicy, class ClockType>
void AllocatorImpl<TracingPolicy, ClockType>::free(
