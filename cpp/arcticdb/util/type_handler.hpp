...
struct ITypeHandler {
  template <class Base> struct Interface : Base {
    ...
    void handle_type(const uint8_t*& source, uint8_t* dest,
      ...
      folly::poly_call<0>(*this, source, dest, encoded_field_info, mapping, dest_bytes,
                          buffers, encoding_version);
    }

    int type_size() const { return folly::poly_call<1>(*this); }
  ...
  template <class T>
  using Members =
      folly::PolyMembers<&T::handle_type, &T::type_size, &T::default_initialize>;
};

using TypeHandler = folly::Poly<ITypeHandler>;

/// Some types cannot be trivially converted from storage to Python types .This
/// singleton holds a set of type erased handlers (implementing the handle_type
/// function) which handle the parsing from storage to python.
...
public:
  ...
  std::shared_ptr<TypeHandler>
  get_handler(const entity::TypeDescriptor& type_descriptor) const;
  void register_handler(const entity::TypeDescriptor& type_descriptor,
                        TypeHandler&& handler);
...
private:
  ...
  std::unordered_map<entity::TypeDescriptor, std::shared_ptr<TypeHandler>, Hasher>
      handlers_;
};
