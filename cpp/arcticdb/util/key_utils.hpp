...
inline void delete_keys_of_type_if(const std::shared_ptr<Store>& store,
                                   Predicate&& predicate, KeyType key_type,
                                   const std::string& prefix = std::string(),
                                   bool continue_on_error = false) {
  static const size_t delete_object_limit =
      ConfigsMap::instance()->get_int("Storage.DeleteBatchSize", 1000);
  std::vector<VariantKey> keys{};
  try {
    store->iterate_type(
        key_type,
        [predicate = std::forward<Predicate>(predicate), store = store,
         &keys](VariantKey&& key) {
          if (predicate(key))
    ...
    if (continue_on_error)
      log::storage().warn("Caught exception {} trying to delete key, continuing",
                          ex.what());
    else
      throw;
  }
...
template <class Predicate>
inline void delete_keys_of_type_if_sync(const std::shared_ptr<Store>& store,
                                        Predicate&& predicate, KeyType key_type,
                                        const std::string& prefix = std::string(),
                                        bool continue_on_error = false) {
  try {
...
inline void delete_keys_of_type_for_stream(const std::shared_ptr<Store>& store,
                                           const StreamId& stream_id, KeyType key_type,
                                           bool continue_on_error = false) {
  auto prefix = std::holds_alternative<StringId>(stream_id)
                    ? std::get<StringId>(stream_id)
                    : std::string();
  auto match_stream_id = [&stream_id](const VariantKey& k) {
    return variant_key_id(k) == stream_id;
  };
  delete_keys_of_type_if(store, std::move(match_stream_id), key_type, prefix,
                         continue_on_error);
}

inline void delete_keys_of_type_for_stream_sync(const std::shared_ptr<Store>& store,
  ...
  delete_keys_of_type_if_sync(store, std::move(match_stream_id), key_type, prefix,
                              continue_on_error);
}

inline void delete_all_keys_of_type(KeyType key_type,
                                    const std::shared_ptr<Store>& store,
                                    bool continue_on_error) {
  auto match_stream_id = [](const VariantKey&) { return true; };
  delete_keys_of_type_if(store, std::move(match_stream_id), key_type, std::string{},
                         continue_on_error);
}

inline void delete_all_for_stream(const std::shared_ptr<Store>& store,
                                  const StreamId& stream_id,
                                  bool continue_on_error = false) {
...
template <typename KeyContainer, typename = std::enable_if<std::is_base_of_v<
                                     AtomKey, typename KeyContainer::value_type>>>
inline std::vector<AtomKey>
get_data_keys(const std::shared_ptr<stream::StreamSource>& store,
              const KeyContainer& keys, storage::ReadKeyOpts opts) {
  using KeySupplier = folly::Function<KeyContainer()>;
  using StreamReader =
      arcticdb::stream::StreamReader<AtomKey, KeySupplier, SegmentInMemory::Row>;
...
inline std::vector<AtomKey>
get_data_keys(const std::shared_ptr<stream::StreamSource>& store, const AtomKey& key,
              storage::ReadKeyOpts opts) {
  const std::vector<AtomKey> keys{key};
  return get_data_keys(store, keys, opts);
}

ankerl::unordered_dense::set<AtomKey>
recurse_segment(const std::shared_ptr<stream::StreamSource>& store,
                SegmentInMemory segment, const std::optional<VersionId>& version_id);

/* Given a [multi-]index key, returns a set containing the top level [multi-]index key
 * itself, and all the multi-index, index, and data keys referenced by this
 * [multi-]index key. If the version_id argument is provided, the returned set will only
 * contain keys matching that version_id. Note that this differs from
 * recurse_index_keys, which does not inclide the passed in keys in the returned set. */
inline ankerl::unordered_dense::set<AtomKey>
recurse_index_key(const std::shared_ptr<stream::StreamSource>& store,
...
/* Given a container of [multi-]index keys, returns a set containing all the
 * multi-index, index, and data keys referenced by these [multi-]index keys. Note that
 * this differs from recurse_index_key, which includes the passed in key in the returned
 * set. */
template <typename KeyContainer, typename = std::enable_if<std::is_base_of_v<
                                     AtomKey, typename KeyContainer::value_type>>>
inline ankerl::unordered_dense::set<AtomKey>
recurse_index_keys(const std::shared_ptr<stream::StreamSource>& store,
                   const KeyContainer& keys, storage::ReadKeyOpts opts) {
  ankerl::unordered_dense::set<AtomKey> res;
  ...
  for (const auto& index_key : keys) {
    if (index_key.type() == KeyType::MULTI_KEY) {
      ...
      KeySegment key_segment(store->read_sync(index_key, opts).second,
                             SymbolStructure::SAME);
...
inline AtomKey in_memory_key(KeyType key_type, const StreamId& stream_id,
...
inline void iterate_keys_of_type_if(const std::shared_ptr<Store>& store,
                                    Predicate&& predicate, KeyType key_type,
                                    const std::string& prefix, Function&& function) {
...
inline void iterate_keys_of_type_for_stream(std::shared_ptr<Store> store,
                                            KeyType key_type, const StreamId& stream_id,
                                            Function&& function) {
  auto prefix = std::holds_alternative<StringId>(stream_id)
                    ? std::get<StringId>(stream_id)
                    : std::string();
  ...
  iterate_keys_of_type_if(store, match_stream_id, key_type, prefix,
