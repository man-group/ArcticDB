...
namespace fmt {
template <> struct formatter<arcticdb::util::MemBytes> {
...
template <> struct formatter<arcticdb::util::MemorySummary> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
  ...
  auto format(arcticdb::util::MemorySummary summary, FormatContext& ctx) const {
    return fmt::format_to(
        ...
        summary.size, summary.resident, summary.max_resident, summary.shared,
        summary.text, summary.data_stack);
  }
};
...
inline MemorySummary get_memory_use_summary() {
  ...
  ARCTICDB_RUNTIME_DEBUG(log::memory(),
                         "print_total_mem_usage not implemented on Windows or Apple");
...
inline void print_total_mem_usage(const char* file ARCTICDB_UNUSED,
