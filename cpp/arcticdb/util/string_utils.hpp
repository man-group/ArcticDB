...
std::array<std::string_view, expected_size> split_to_array(std::string_view strv,
                                                           char delim) {
  ...
  while (first != strv.end() && count < expected_size) {
    ...
    if (first != second)
      output[count++] =
          strv.substr(std::distance(strv.begin(), first), std::distance(first, second));
...
inline std::vector<std::string_view> split_to_vector(std::string_view strv,
                                                     char delim) {
...
inline std::string_view strv_from_pos(const std::string& str, size_t start,
                                      size_t length) {
