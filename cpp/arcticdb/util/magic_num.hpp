...
template <char a, char b, char c, char d> struct MagicNum {
  static constexpr uint64_t Magic = a << (CHAR_BIT * 0) | b << (CHAR_BIT * 1) |
                                    c << (CHAR_BIT * 2) | d << (CHAR_BIT * 3);

  ~MagicNum() { magic_ = ~magic_; }

  void check() const {
    std::string_view expected(reinterpret_cast<const char*>(&Magic), 4);
    util::check(magic_ == Magic, "Magic number failure, expected {}({}) got {}", Magic,
                expected, magic_);
  }
...
template <char a, char b> struct SmallMagicNum {
...
template <typename MagicNumType> void check_magic_in_place(const uint8_t*& pos) {
  const auto magic_num = reinterpret_cast<const MagicNumType*>(pos);
  magic_num->check();
}

template <typename MagicNumType> void check_magic(const uint8_t*& pos) {
  check_magic_in_place<MagicNumType>(pos);
  pos += sizeof(MagicNumType);
}

template <typename MagicNumType> void write_magic(uint8_t*& pos) {
