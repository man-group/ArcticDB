...
public:
  ...
  size_t get_file_size(const std::string& file_path) {
    LARGE_INTEGER size;
    HANDLE h = CreateFile(file_path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
  ...
  void open_file(const std::string& filepath) {
    ...
    file_ = CreateFile(filepath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
                       OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  void create_file(const std::string& filepath, size_t size) {
    length_ = size;
    file_ = CreateFile(filepath.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
                       CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file_ == INVALID_HANDLE_VALUE) {
      util::raise_rte("Error opening file for writing");
    }

    map_ = CreateFileMapping(file_, nullptr, PAGE_READWRITE, 0,
...
public:
  ...
  void open_file(const std::string& filepath) {
    ...
    ARCTICDB_DEBUG(log::storage(), "Memory-mapped file at path {} with length {}",
                   filepath, length_);
    data_ =
        static_cast<uint8_t*>(mmap(nullptr, length_, PROT_READ, MAP_SHARED, fd_, 0));
    if (data_ == MAP_FAILED) {
      close(fd_);
  ...
  void create_file(const std::string& filepath, size_t size) {
    ...
    ARCTICDB_DEBUG(log::storage(), "Created memory mapped file at {} with size {}",
                   filepath, length_);
  }
