...
inline std::string format_timestamp(entity::timestamp ts) {
  ...
  auto time_point = Clock::time_point(
      std::chrono::duration_cast<std::chrono::seconds>(std::chrono::nanoseconds(ts)));
#else
  auto time_point = Clock::time_point(std::chrono::nanoseconds(ts));
#endif
  std::time_t t = Clock::to_time_t(time_point);
  auto fraction =
      time_point - std::chrono::time_point_cast<std::chrono::seconds>(time_point);
