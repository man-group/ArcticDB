...
enum class ErrorCategory : detail::BaseType {
  ...
  USER_INPUT = 7,
  COMPATIBILITY = 8,
  /// Exceptions resulting in failure to encode or decode data while reading/writing
  /// from/to disc
  CODEC = 9
  // NEW CATEGORIES MUST ALSO BE ADDED TO
  // python_module.cpp:register_error_code_ecosystem
};
...
template <ErrorCode code> inline constexpr ErrorCodeData error_code_data{};
...
constexpr ErrorCategory get_error_category(ErrorCode code) {
  return static_cast<ErrorCategory>(static_cast<detail::BaseType>(code) /
                                    detail::error_category_scale);
}
...
struct ArcticSpecificException
    : public ArcticCategorizedException<get_error_category(specific_code)> {
...
using UnexpectedLMDBErrorException =
    ArcticSpecificException<ErrorCode::E_UNEXPECTED_LMDB_ERROR>;
using UnexpectedS3ErrorException =
    ArcticSpecificException<ErrorCode::E_UNEXPECTED_S3_ERROR>;
using S3RetryableException = ArcticSpecificException<ErrorCode::E_S3_RETRYABLE>;
using UnexpectedAzureException =
    ArcticSpecificException<ErrorCode::E_UNEXPECTED_AZURE_ERROR>;
using MongoOperationNoReplyException =
    ArcticSpecificException<ErrorCode::E_MONGO_BULK_OP_NO_REPLY>;
using UnexpectedMongoException =
    ArcticSpecificException<ErrorCode::E_UNEXPECTED_MONGO_ERROR>;
...
template <ErrorCode error_code> [[noreturn]] void throw_error(const std::string& msg) {
...
throw_error<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_LMDB_MAP_FULL>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_LMDB_MAP_FULL>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_UNEXPECTED_S3_ERROR>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_UNEXPECTED_S3_ERROR>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_S3_RETRYABLE>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_S3_RETRYABLE>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_UNEXPECTED_AZURE_ERROR>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_UNEXPECTED_AZURE_ERROR>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_MONGO_BULK_OP_NO_REPLY>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_MONGO_BULK_OP_NO_REPLY>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_UNEXPECTED_MONGO_ERROR>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_UNEXPECTED_MONGO_ERROR>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_NO_SUCH_VERSION>(const std::string& msg) {
  throw ArcticSpecificException<ErrorCode::E_NO_SUCH_VERSION>(msg);
}

template <>
[[noreturn]] inline void
throw_error<ErrorCode::E_UNSORTED_DATA>(const std::string& msg) {
...
namespace fmt {
template <> struct formatter<arcticdb::ErrorCode> {
