...
public:
  explicit RegexPattern(const std::string& pattern) : text_(pattern) {
    compile_regex();
  }

  [[nodiscard]] bool valid() const { return handle_ != nullptr; }

  [[nodiscard]] HandleType handle() const { return handle_; }
...
private:
  void compile_regex() {
    handle_ =
        ::pcre_compile2(text_.data(), options_, &error_, &help_, &offset_, table_);
    util::check(handle_ != nullptr, "Error {} compiling regex {}: {}", error_, text_,
                help_);
...
public:
  ...
  explicit Regex(const RegexPattern& pattern)
      : pattern_(pattern), results_((pattern_.capturing_groups() + 1) * 3, 0) {}

  bool match(const std::string& text) {
    ResultsType res = ::pcre_exec(pattern_.handle(), extra_, text.data(),
                                  static_cast<int>(text.size()), 0, options_,
                                  &results_[0], static_cast<int>(results_.size()));
    util::check(res >= 0 || res == PCRE_ERROR_NOMATCH,
                "Invalid result in regex compile with pattern {} and text {}: {}",
                pattern_.text(), text, res);
    return res > 0;
  }
};
