...
public:
  ...
  ConfigsMap() : mutex_(std::make_unique<std::mutex>()) {}
...
#define HANDLE_TYPE(LABEL, TYPE)                                                       \
  ...
  std::optional<TYPE> get_##LABEL(const std::string& label) const {                    \
    auto it = map_of_##LABEL.find(boost::to_upper_copy<std::string>(label));           \
    return it == map_of_##LABEL.cend() ? std::nullopt                                  \
...
#undef HANDLE_TYPE
  ...
  RuntimeConfig to_proto() {
    ...
    output.mutable_string_values()->insert(std::cbegin(map_of_string),
                                           std::cend(map_of_string));
    output.mutable_double_values()->insert(std::cbegin(map_of_double),
                                           std::cend(map_of_double));
    return output;
  }
  ...
  void read_proto(const RuntimeConfig& config) {
    ...
    for (auto& val : config.string_values())
      map_of_string.try_emplace(boost::to_upper_copy<std::string>(val.first),
                                val.second);

    for (auto& val : config.double_values())
      map_of_double.try_emplace(boost::to_upper_copy<std::string>(val.first),
                                val.second);
  }
...
struct ScopedConfig {
  ...
  ScopedConfig(const std::string& name, int64_t val)
      : name_(name), original_(ConfigsMap::instance()->get_int(name)) {
