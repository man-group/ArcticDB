...
util::BitMagic truncate_sparse_map(const util::BitMagic& input_sparse_map, size_t start_row, size_t end_row) {
    ...
    if (start_row == 0) {
        // This is much faster than the implementation in the else block in the special (but not uncommon) case that we
        // want the start of the input range
        util::BitMagic output_sparse_map(input_sparse_map, 0, end_row + 1);
        output_sparse_map.resize(end_row);
