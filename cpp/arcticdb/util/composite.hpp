...
template <typename T> struct Composite {
  ...
  RangeType get_range() const { return folly::Range(values_); }
  ...
  class CompositeIterator
      : public boost::iterator_facade<Composite<ValueType>, ValueType,
  ...
  size_t level_1_size() const { return values_.size(); }
  ...
  [[nodiscard]] size_t size() const {
    return std::accumulate(
        std::begin(values_), std::end(values_), 0, [](size_t n, const ValueType& v) {
          return util::variant_match(
              v, [n](const T&) { return n + 1; },
  ...
  [[nodiscard]] bool empty() const {
    return values_.empty() ||
           std::all_of(values_.begin(), values_.end(), [](const ValueType& v) {
             return util::variant_match(
                 v, [](const T&) { return false; },
  ...
  template <typename Func> void broadcast(const Func& func) {
    for (auto& value : values_) {
      util::variant_match(
          value, [&func](T& val) { func(val); },
          [&func](std::unique_ptr<Composite<T>>& comp) { comp->broadcast(func); });
    }
  }

  template <typename Func> void broadcast(const Func& func) const {
    for (auto& value : values_) {
      util::variant_match(
          value, [&func](const T& val) { func(val); },
          [&func](const std::unique_ptr<Composite<T>>& comp) {
            comp->broadcast(func);
          });
    }
  }

  template <typename Func> auto transform(const Func& func) {
  ...
  template <typename Func> auto filter(const Func& func) {
    Composite output;
  ...
  template <typename Func, typename U> auto fold(const Func& func, U initial) {
 ...
 *         *  * *  *
 */
template <typename T> Composite<T> merge_composites(std::vector<Composite<T>>&& cmp) {
...
Composite<T> merge_composites_shallow(std::vector<Composite<T>>&& cmp) {
  ...
  for (Composite<T>& composite : composites) {
    for (size_t i = 0; i < composite.level_1_size(); i++) {
      util::variant_match(
          composite[i], [&output](T& val) { output.push_back(std::move(val)); },
...
namespace fmt {
template <typename T> struct formatter<arcticdb::Composite<T>> {
