...
void initialise_output_column(const Column& input_column, Column& output_column) {
  if (&input_column != &output_column) {
    ...
    output_column.allocate_data(output_physical_rows *
                                get_type_size(output_column.type().data_type()));
...
void initialise_output_column(const Column& left_input_column,
  ...
  if (&left_input_column != &output_column && &right_input_column != &output_column) {
    ...
    if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
      // Both dense. Could be different lengths if the data is semantically sparse, but
      // happens to be dense in the first n rows
      output_physical_rows =
          std::min(left_input_column.row_count(), right_input_column.row_count());
      output_last_row =
          std::min(left_input_column.last_row(), right_input_column.last_row());
    } else {
      // At least one sparse
      ...
      if (left_input_column.is_sparse() && right_input_column.is_sparse()) {
        ...
        // If the sparse column had more logical rows than the dense column, truncate
        // the sparse map to the length of the dense column
...
bool Column::sparse_permitted() const { return allow_sparse_; }
...
void Column::unsparsify(size_t num_rows) {
  ...
  type_.visit_tag([this, num_rows](const auto tdt) {
    ...
    util::expand_dense_buffer_using_bitmap<RawType>(sparse_map_.value(),
                                                    data_.buffer().data(), dest.data());
...
void Column::sparsify() {
  type().visit_tag([this](auto type_desc_tag) {
    ...
    if constexpr (is_floating_point_type(type_desc_tag.data_type())) {
      ...
      auto buffer =
          util::scan_floating_point_to_sparse(raw_ptr, row_count(), sparse_map());
...
void Column::set_string_array(ssize_t row_offset, size_t string_size,
                              size_t num_strings, char* input,
...
void Column::set_string_list(ssize_t row_offset, const std::vector<std::string>& input,
                             StringPool& string_pool) {
...
void Column::append(const Column& other, position_t at_row) {
  if (other.row_count() == 0)
    return;
  util::check(type() == other.type(), "Cannot append column type {} to column type {}",
              type(), other.type());
  const bool was_sparse = is_sparse();
  const bool was_empty = empty();
  util::check(last_physical_row_ + 1 == row_count(),
              "Row count calculation incorrect before dense append");
  util::check(!is_sparse() || row_count() == sparse_map_.value().count(),
              "Row count does not match bitmap count");
  ...
  ARCTICDB_DEBUG(log::version(),
                 "at_row: {}\tother.last_logical_row_: {}\tother.last_physical_row_: "
                 "{}\tother.row_count(): {}",
                 at_row, other.last_logical_row_, other.last_physical_row_,
                 other.row_count());
  ARCTICDB_DEBUG(log::version(),
                 "initial_row_count: {}\tlast_logical_row_: {}\tlast_physical_row_: "
                 "{}\trow_count: {}",
                 initial_row_count, last_logical_row_, last_physical_row_, row_count());

  util::check(last_physical_row_ + 1 == row_count(),
              "Row count calculation incorrect after dense append");

  if (at_row == initial_row_count && !other.is_sparse() && !is_sparse()) {
    util::check(last_logical_row_ == last_physical_row_,
                "Expected logical and physical rows to line up in append of non-sparse "
                "columns");
    return;
  }
  ...
  if (other.is_sparse()) {
    ...
    ARCTICDB_DEBUG(log::version(), "Other column is dense, setting range from {} to {}",
                   at_row, at_row + other.row_count());
    sparse_map().set_range(uint32_t(at_row), uint32_t(at_row + other.last_logical_row_),
                           true);
  }

  util::check(!is_sparse() || row_count() == sparse_map_.value().count(),
              "Row count incorrect exiting append", row_count(), sparse_map().count());
}
...
void Column::physical_sort_external(std::vector<uint32_t>&& sorted_pos) {
  ...
  util::check(sorted_pos.size() == physical_rows,
  ...
  type().visit_tag([&buffer, &sorted_pos, &physical_rows](auto tdt) {
    ...
    for (auto i = 0u; i < physical_rows; ++i) {
      if (i != sorted_pos[i]) {
        ...
        // Amortized O(1) complexity, because each iteration places an element where
        // it's supposed to go and once an element is in it's sorted position we never
        // move it.
...
void Column::sort_external(const JiveTable& jive_table,
  ...
  if (!is_sparse()) {
    ...
    util::check(pre_allocated_space.size() == jive_table.sorted_pos_.size(),
                "Mismatch between provided pre_allocated_space size and jive table "
                "size: {} != {}",
...
void Column::default_initialize_rows(size_t start_pos, size_t num_rows,
                                     bool ensure_alloc) {
  if (num_rows > 0) {
...
std::optional<position_t> Column::get_physical_row(position_t row) const {
  if (row > last_logical_row_) {
    if (sparse_permitted())
      ...
      util::raise_rte("Scalar index {} out of bounds in column of size {}", row,
                      row_count());
  }
...
void Column::set_allow_sparse(bool value) { allow_sparse_ = value; }
...
void Column::inflate_string_array(
  ...
  for (int i = 0; i < string_refs.size(); ++i)
    max_size = std::max(
        max_size,
        static_cast<ssize_t>(string_pool.get_const_view(string_refs.at(i)).size()));
...
void Column::inflate_string_arrays(const StringPool& string_pool) {
  util::check_arg(is_fixed_string_type(type().data_type()),
                  "Can only inflate fixed string array types");
  util::check_arg(type().dimension() == Dimension::Dim1,
                  "Fixed string inflation is for array types only");
...
void Column::change_type(DataType target_type) {
  util::check(shapes_.empty(),
              "Can't change type on multi-dimensional column with type {}", type_);
  if (type_.data_type() == target_type)
    return;

  CursoredBuffer<ChunkedBuffer> buf;
  for (const auto& block : data_.buffer().blocks()) {
    details::visit_type(type_.data_type(), [&buf, &block, type = type_,
                                            target_type](auto&& source_dtt) {
      using source_raw_type = typename std::decay_t<decltype(source_dtt)>::raw_type;
      details::visit_type(target_type, [&buf, &block, &type,
                                        target_type](auto&& target_dtt) {
...
position_t Column::row_count() const {
  ...
  if (is_sequence_type(type().data_type()) && inflated_ &&
      is_fixed_string_type(type().data_type()))
...
Column::split(const std::shared_ptr<Column>& column, size_t rows) {
  // TODO: Doesn't work the way you would expect for sparse columns - the bytes for each
  // buffer won't be uniform
  ...
  util::check(bytes % get_type_size(column->type().data_type()) == 0,
              "Bytes {} is not a multiple of type size {}", bytes, column->type());
  std::vector<std::shared_ptr<Column>> output;
  output.reserve(new_buffers.size());

  auto row = 0;
  for (auto& buffer : new_buffers) {
    output.push_back(std::make_shared<Column>(column->type(), column->allow_sparse_,
                                              std::move(buffer)));
...
/// Bytes from the underlying chunked buffer to include when truncating. Inclusive of
/// start_byte, exclusive of end_byte
[[nodiscard]] static std::pair<size_t, size_t>
column_start_end_bytes(const Column& column, size_t start_row, size_t end_row) {
  ...
  if (column.is_sparse()) {
    ...
    // Sparse columns do not include trailing 0s in the bitset, so the relevant end_row
    // is capped at the size of the biset
    end_row = std::min(end_row, static_cast<size_t>(input_sparse_map.size()));
    // count_range is inclusive at both ends
    const size_t set_bits_before_range =
        start_row > 0 ? input_sparse_map.count_range(0, start_row - 1) : 0;
    const size_t set_bits_in_range =
        input_sparse_map.count_range(start_row, end_row - 1);
    start_byte = set_bits_before_range * type_size;
    end_byte = start_byte + (set_bits_in_range * type_size);
  }
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      start_byte % type_size == 0, "start_byte {} is not a multiple of type size {}",
      start_byte, column.type());
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      end_byte % type_size == 0, "start_byte {} is not a multiple of type size {}",
      end_byte, column.type());
  return {start_byte, end_byte};
}

[[nodiscard]] static util::BitMagic
truncate_sparse_map(const util::BitMagic& input_sparse_map, size_t start_row,
                    size_t end_row) {
  ...
  if (start_row == 0) {
    // get_first can return 0 if no bits are set, but we checked earlier that
    // input_sparse_map.size() > 0, and we do not have sparse maps with no bits set
    // (except for the empty type)
...
std::shared_ptr<Column> Column::truncate(const std::shared_ptr<Column>& column,
                                         size_t start_row, size_t end_row) {
  const auto [start_byte, end_byte] =
      column_start_end_bytes(*column, start_row, end_row);
  auto buffer = ::arcticdb::truncate(column->data_.buffer(), start_byte, end_byte);
  auto res = std::make_shared<Column>(column->type(), column->allow_sparse_,
                                      std::move(buffer));
  if (column->is_sparse()) {
...
const shape_t* Column::shape_index(position_t idx) const {
  ...
  return shapes_.buffer().ptr_cast<shape_t>(
      idx * size_t(type_.dimension()) * sizeof(shape_t), sizeof(shape_t));
}

position_t Column::bytes_offset(position_t idx) const {
  regenerate_offsets();
  util::check_arg(idx < row_count(),
                  "bytes_offset index {} out of bounds in column of size {}", idx,
                  row_count());
  ...
  util::check(size_t(idx - 1) < offsets_.size(), "Offset {} out of range, only have {}",
              idx - 1, offsets_.size());
...
void Column::regenerate_offsets() const {
  ...
  position_t pos = 0;
  for (position_t i = 0, j = i + position_t(type_.dimension());
       j < position_t(num_shapes()); i = j, j += position_t(type_.dimension())) {
    auto num_elements = position_t(std::accumulate(shape_index(i), shape_index(j),
                                                   shape_t(1), std::multiplies<>()));
...
const util::BitMagic& Column::sparse_map() const {
  util::check(static_cast<bool>(sparse_map_),
              "Expected sparse map when it was not set");
  return sparse_map_.value();
}
