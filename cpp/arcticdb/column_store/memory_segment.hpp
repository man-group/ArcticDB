...
class SegmentInMemory {
  public:
    ...
    SegmentInMemory() : impl_(std::make_shared<SegmentInMemoryImpl>()) {}
    ...
    explicit SegmentInMemory(
        ...
        impl_(std::make_shared<SegmentInMemoryImpl>(
                tsd, expected_column_size, presize, allow_sparse, output_format, mode
        )) {}
    ...
    explicit SegmentInMemory(
        ...
        impl_(std::make_shared<SegmentInMemoryImpl>(
                std::move(tsd), expected_column_size, presize, allow_sparse, output_format, mode
        )) {}

    friend void swap(SegmentInMemory& left, SegmentInMemory& right) { std::swap(left.impl_, right.impl_); }
    ...
    void push_back(const Row& row) { impl_->push_back(row); }

    [[nodiscard]] auto& column_descriptor(size_t col) const { return impl_->column_descriptor(col); }

    void end_row() const { impl_->end_row(); }
    ...
    [[nodiscard]] const auto& fields() const { return impl_->fields(); }

    [[nodiscard]] const auto& field(size_t index) const { return impl_->field(index); }
    ...
    [[nodiscard]] const TimeseriesDescriptor& index_descriptor() const { return impl_->index_descriptor(); }

    TimeseriesDescriptor& mutable_index_descriptor() { return impl_->mutable_index_descriptor(); }

    [[nodiscard]] bool has_index_descriptor() const { return impl_->has_index_descriptor(); }

    void init_column_map() const { impl_->init_column_map(); }
    ...
    void set_string(position_t pos, std::string_view str) { impl_->set_string(pos, str); }
    ...
    void set_string_list(position_t idx, const std::vector<std::string>& input) { impl_->set_string_list(idx, input); }

    void set_value(position_t idx, const SegmentInMemoryImpl::Location& loc) { impl_->set_value(idx, loc); }

    // pybind11 can't resolve const and non-const version of column()
    Column& column_ref(position_t idx) { return impl_->column_ref(idx); }

    Column& column(position_t idx) { return impl_->column(idx); }

    [[nodiscard]] const Column& column(position_t idx) const { return impl_->column(idx); }

    std::vector<std::shared_ptr<Column>>& columns() { return impl_->columns(); }

    [[nodiscard]] const std::vector<std::shared_ptr<Column>>& columns() const { return impl_->columns(); }
    ...
    [[nodiscard]] size_t num_blocks() const { return impl_->num_blocks(); }

    void append(const SegmentInMemory& other) { impl_->append(*other.impl_); }
    ...
    void drop_column(std::string_view name) { impl_->drop_column(name); }
    ...
    void reset_timeseries_descriptor() { impl_->reset_timeseries_descriptor(); }

    void calculate_statistics() { impl_->calculate_statistics(); }

    [[nodiscard]] size_t num_columns() const { return impl_->num_columns(); }

    [[nodiscard]] size_t row_count() const { return impl_->row_count(); }

    void unsparsify() { impl_->unsparsify(); }

    [[nodiscard]] bool has_user_metadata() const { return impl_->has_user_metadata(); }

    [[nodiscard]] const arcticdb::proto::descriptors::UserDefinedMetadata& user_metadata() const {
        return impl_->user_metadata();
    }

    void sparsify() { impl_->sparsify(); }
    ...
    void set_offset(ssize_t offset) { impl_->set_offset(offset); }

    [[nodiscard]] ssize_t offset() const { return impl_->offset(); }

    void clear() { impl_->clear(); }

    void end_block_write(ssize_t size) { impl_->end_block_write(size); }
    ...
    [[nodiscard]] ColumnData string_pool_data() const { return impl_->string_pool_data(); }

    [[nodiscard]] ColumnData column_data(size_t col) const { return impl_->column_data(col); }

    [[nodiscard]] const StreamDescriptor& descriptor() const { return impl_->descriptor(); }

    StreamDescriptor& descriptor() { return impl_->descriptor(); }

    [[nodiscard]] const std::shared_ptr<StreamDescriptor>& descriptor_ptr() const { return impl_->descriptor_ptr(); }

    void attach_descriptor(std::shared_ptr<StreamDescriptor> desc) { impl_->attach_descriptor(std::move(desc)); }

    StringPool& string_pool() { return impl_->string_pool(); }

    [[nodiscard]] const StringPool& const_string_pool() const { return impl_->string_pool(); }

    [[nodiscard]] const std::shared_ptr<StringPool>& string_pool_ptr() const { return impl_->string_pool_ptr(); }

    void reset_metadata() { impl_->reset_metadata(); }

    void set_metadata(google::protobuf::Any&& meta) { impl_->set_metadata(std::move(meta)); }

    bool has_metadata() { return impl_->has_metadata(); }

    void set_row_id(ssize_t rid) { impl_->set_row_id(rid); }

    void set_row_data(ssize_t rid) { impl_->set_row_data(rid); }

    [[nodiscard]] const google::protobuf::Any* metadata() const { return impl_->metadata(); }

    [[nodiscard]] bool is_index_sorted() const { return impl_->is_index_sorted(); }

    void sort(const std::string& column) { impl_->sort(column); }

    void sort(const std::vector<std::string>& columns) { return impl_->sort(columns); }

    void sort(const std::vector<position_t>& columns) { return impl_->sort(columns); }

    void sort(position_t column) { impl_->sort(column); }

    [[nodiscard]] SegmentInMemory clone() const {
        return SegmentInMemory(std::make_shared<SegmentInMemoryImpl>(impl_->clone()));
    }

    void set_string_pool(const std::shared_ptr<StringPool>& string_pool) { impl_->set_string_pool(string_pool); }
    ...
    [[nodiscard]] bool empty() const { return is_null() || impl_->empty(); }

    [[nodiscard]] bool is_null() const { return !static_cast<bool>(impl_); }

    [[nodiscard]] size_t num_bytes() const { return impl_->num_bytes(); }

    [[nodiscard]] bool compacted() const { return impl_->compacted(); }

    void set_compacted(bool val) { impl_->set_compacted(val); }

    void change_schema(const StreamDescriptor& descriptor) { return impl_->change_schema(descriptor); }

    SegmentInMemoryImpl* impl() { return impl_.get(); }

    void check_magic() const { impl_->check_magic(); }

    // Not currently used but might be useful in the future
    void compact_blocks() { impl_->compact_blocks(); }

    std::shared_ptr<Column> column_ptr(position_t idx) { return impl_->column_ptr(idx); }
    ...
    [[nodiscard]] bool allow_sparse() const { return impl_->allow_sparse(); }

    [[nodiscard]] bool is_sparse() const { return impl_->is_sparse(); }
    ...
    void drop_empty_columns() { impl_->drop_empty_columns(); }
