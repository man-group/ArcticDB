...
class Column {
  public:
    ...
    Column(TypeDescriptor type) : Column(type, 0, AllocationType::DYNAMIC, Sparsity::NOT_PERMITTED) {}

    Column(TypeDescriptor type, Sparsity allow_sparse) : Column(type, 0, AllocationType::DYNAMIC, allow_sparse) {}
    ...
    void set_statistics(FieldStatsImpl stats) { stats_ = stats; }

    bool has_statistics() const { return stats_.set_; };

    FieldStatsImpl get_statistics() const { return stats_; }
    ...
    const shape_t* shape_ptr() const { return shapes_.ptr_cast<shape_t>(0, num_shapes()); }
    ...
    const uint8_t* bytes_at(size_t bytes, size_t required) const { return data_.bytes_at(bytes, required); }

    void assert_size(size_t bytes) const { data_.buffer().assert_size(bytes); }
    ...
    size_t search_sorted(
        ...
        details::visit_type(
                type().data_type(),
                [this, &res, &column_data, val, from_right, &from, &to](auto type_desc_tag) {
    ...
    requires std::is_invocable_r_v<
            void, functor, typename ColumnData::Enumeration<typename input_tdt::DataTypeTag::raw_type>>
    ...
    static void transform(
        ...
        if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
            ...
            for (auto left_it =
                         left_input_data.cbegin<left_input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
    ...
    template<
            ...
            std::relation<
                    typename left_input_tdt::DataTypeTag::raw_type, typename right_input_tdt::DataTypeTag::raw_type>
                    functor>
    static void transform(
