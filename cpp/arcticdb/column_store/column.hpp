...
// IfNeeded - Uses a non owning view when possible (e.g. numeric types). Copies when memory layout is different (e.g.
// strings)
enum class CopyMode : uint8_t { Always, IfNeeded };
...
class Column {
  public:
    ...
    void set_external_block(ssize_t row_offset, T* val, size_t size, CopyMode copy_mode = CopyMode::IfNeeded) {
        util::check_arg(
