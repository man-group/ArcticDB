...
// Compilation fails on Mac if cstdio is not included prior to folly/Function.h due to a
// missing definition of memalign in folly/Memory.h
#ifdef __APPLE__
#include <cstdio>
#endif
...
template <class T> using py_array_t = py::array_t<T>;

/// @endcond

using namespace arcticdb::entity;

// N.B. this will not catch all the things that C++ considers to be narrowing
// conversions, because it doesn't take into account integral promotion, however we
// don't care about that for the purpose for which it is used in this file.
template <typename SourceType, typename TargetType>
constexpr bool is_narrowing_conversion() {
  if (sizeof(TargetType) < sizeof(SourceType))
    return true;

  if (sizeof(SourceType) == sizeof(TargetType) && std::is_integral_v<TargetType> &&
      std::is_unsigned_v<SourceType> && std::is_signed_v<TargetType>) {
    return true;
  }
...
template <typename T> JiveTable create_jive_table(const Column& col);

void initialise_output_column(const Column& input_column, Column& output_column);

void initialise_output_column(const Column& left_input_column,
                              const Column& right_input_column, Column& output_column);

void initialise_output_bitset(const util::BitSet& input_bitset,
...
public:
  ...
  class TypedColumnIterator
      : public boost::iterator_facade<TypedColumnIterator<TDT, ValueType>, ValueType,
    ...
    typename TypedBlockData<TDT>::template TypedColumnBlockIterator<const ValueType>
        block_pos_;
    typename TypedBlockData<TDT>::template TypedColumnBlockIterator<const ValueType>
        block_end_;
  ...
  public:
    TypedColumnIterator(const Column& col, bool begin)
        : parent_(col.data()), block_(begin ? parent_.next<TDT>() : std::nullopt) {
      if (begin)
        set_block_range();
    }
    ...
    explicit TypedColumnIterator(const TypedColumnIterator<TDT, OtherValue>& other)
        : parent_(other.parent_), block_(other.block_), block_pos_(other.block_pos_),
  ...
  struct StringArrayData {
    ...
    StringArrayData(ssize_t n, ssize_t s, const char* d)
        : num_strings_(n), string_size_(s), data_(d) {}
  };
  ...
  Column(TypeDescriptor type, bool allow_sparse, ChunkedBuffer&& buffer)
      : data_(std::move(buffer)), type_(type), allow_sparse_(allow_sparse) {}

  Column(TypeDescriptor type, bool allow_sparse, ChunkedBuffer&& buffer,
         Buffer&& shapes)
      : data_(std::move(buffer)), shapes_(std::move(shapes)), type_(type),
        allow_sparse_(allow_sparse) {}

  Column(TypeDescriptor type, size_t expected_rows, bool presize, bool allow_sparse)
      : // Array types are stored on disk as flat sequences. The python layer cannot
        // work with this. We need to pass it pointers to an array type (at numpy arrays
        // at the moment). When we allocate a column for an array we need to allocate
        // space for one pointer per row. This also affects how we handle arrays to
        // python as well. Check cpp/arcticdb/column_store/column_utils.hpp::array_at
        // and cpp/arcticdb/column_store/column.hpp::Column
        data_(expected_rows * entity::sizeof_datatype(type), presize), type_(type),
        allow_sparse_(allow_sparse) {
  ...
  template <typename TagType> auto begin() const {
    using RawType = typename TagType::DataTypeTag::raw_type;
    return TypedColumnIterator<TagType, const RawType>(*this, true);
  }

  template <typename TagType> auto end() const {
    using RawType = typename TagType::DataTypeTag::raw_type;
  ...
  void set_scalar(ssize_t row_offset, T val) {
    ...
    if (row_offset != prev_logical_row + 1) {
      if (sparse_permitted()) {
        ...
        util::raise_rte("set_scalar expected row {}, actual {} ", prev_logical_row + 1,
                        row_offset);
      }
    }

    if (is_sparse()) {
      ARCTICDB_TRACE(log::version(), "setting sparse bit at position {}",
                     last_logical_row_);
      set_sparse_bit_for_row(last_logical_row_);
    }

    ARCTICDB_TRACE(log::version(), "Setting scalar {} at {} ({})", val,
    ...
    *data_.ptr_cast<T>(position_t(last_physical_row_), sizeof(T)) = val;
    ...
    util::check(last_physical_row_ + 1 == row_count(),
                "Row count calculation incorrect in set_scalar");
  }

  template <class T> void push_back(T val) { set_scalar(last_logical_row_ + 1, val); }
  ...
  inline void set_external_block(ssize_t row_offset, T* val, size_t size) {
    ...
    const_cast<ChunkedBuffer&>(data_.buffer())
        .add_external_block(reinterpret_cast<const uint8_t*>(val), bytes,
                            data_.buffer().last_offset());
  ...
  inline void set_sparse_block(ssize_t row_offset, T* ptr, size_t rows_to_write) {
    ...
    auto new_buffer =
        util::scan_floating_point_to_sparse(ptr, rows_to_write, sparse_map());
  ...
  inline void set_sparse_block(ChunkedBuffer&& buffer, Buffer&& shapes,
                               util::BitSet&& bitset) {
    data_.buffer() = std::move(buffer);
  ...
  ChunkedBuffer&& release_buffer() { return std::move(data_.buffer()); }

  Buffer&& release_shapes() { return std::move(shapes_.buffer()); }

  template <
      class T, template <class> class Tensor,
  ...
  void set_array(ssize_t row_offset, Tensor<T>& val) {
    ...
    magic_.check();
    util::check_arg(last_logical_row_ + 1 == row_offset,
                    "set_array expected row {}, actual {} ", last_logical_row_ + 1,
                    row_offset);
  ...
  void set_string_array(ssize_t row_offset, size_t string_size, size_t num_strings,
                        char* input, StringPool& string_pool);
  void set_string_list(ssize_t row_offset, const std::vector<std::string>& input,
  ...
  // pre_allocated_space can be reused across different calls to sort_external to avoid
  // unnecessary allocations. It has to be the same size as the jive_table.
  void sort_external(const JiveTable& jive_table,
                     std::vector<uint32_t>& pre_allocated_space);
  ...
  // The following two methods inflate (reduplicate) numpy string arrays that are
  // potentially multi-dimensional, i.e where the value is not a string but an array of
  // strings
  ...
  std::optional<StringArrayData> string_array_at(position_t idx,
                                                 const StringPool& string_pool) {
    util::check_arg(idx < row_count(), "String array index out of bounds in column");
    util::check_arg(type_.dimension() == Dimension::Dim1,
                    "String array should always be one dimensional");
    if (!inflated_)
  ...
  ColumnData data() const {
    return ColumnData(&data_.buffer(), &shapes_.buffer(), type_,
                      sparse_map_ ? &*sparse_map_ : nullptr);
  }

  const uint8_t* ptr() const { return data_.buffer().data(); }

  uint8_t* ptr() { return data_.buffer().data(); }

  TypeDescriptor type() const { return type_; }

  size_t num_blocks() const { return data_.buffer().num_blocks(); }

  const shape_t* shape_ptr() const {
    return shapes_.ptr_cast<shape_t>(0, num_shapes());
  }

  void set_orig_type(const TypeDescriptor& desc) { orig_type_ = desc; }

  bool has_orig_type() const { return static_cast<bool>(orig_type_); }

  const TypeDescriptor& orig_type() const { return orig_type_.value(); }

  void compact_blocks() { data_.compact_blocks(); }
  ...
  inline void advance_data(std::size_t size) { data_.advance(position_t(size)); }

  inline void advance_shapes(std::size_t size) { shapes_.advance(position_t(size)); }

  template <typename T> std::optional<T> scalar_at(position_t row) const {
  ...
  // Copies all physical scalars to a std::vector<T>. This is useful if you require many
  // random access operations and you would like to avoid the overhead of computing the
  // exact location every time.
  template <typename T> std::vector<T> clone_scalars_to_vector() const {
  ...
  // N.B. returning a value not a reference here, so it will need to be pre-checked when
  // data is sparse or it will likely 'splode.
  template <typename T> T& reference_at(position_t row) {
    util::check_arg(row < row_count(),
                    "Scalar reference index {} out of bounds in column of size {}", row,
                    row_count());
  ...
  template <typename T> std::optional<TensorType<T>> tensor_at(position_t idx) const {
    util::check_arg(idx < row_count(), "Tensor index out of bounds in column");
    util::check_arg(type_.dimension() != Dimension::Dim0,
                    "tensor_at called on scalar column");
    const shape_t* shape_ptr = shape_index(idx);
    auto ndim = ssize_t(type_.dimension());
    return TensorType<T>(shape_ptr, ndim, type().data_type(),
                         get_type_size(type().data_type()),
                         reinterpret_cast<const T*>(data_.buffer().ptr_cast<uint8_t>(
                             bytes_offset(idx), calc_elements(shape_ptr, ndim))),
                         ndim);
  }

  template <typename T> const T* ptr_cast(position_t idx, size_t required_bytes) const {
    return data_.buffer().ptr_cast<T>(bytes_offset(idx), required_bytes);
  }

  template <typename T> T* ptr_cast(position_t idx, size_t required_bytes) {
    return const_cast<T*>(
        const_cast<const Column*>(this)->ptr_cast<T>(idx, required_bytes));
  }

  [[nodiscard]] auto& buffer() { return data_.buffer(); }

  template <typename T> std::optional<position_t> search_unsorted(T val) const {
  ...
  // Returns the index such that if val were inserted before that index, the order would
  // be preserved By default returns the lowest index satisfying this property. If
  // from_right=true, returns the highest such index
  template <class T, std::enable_if_t<
                         std::is_integral_v<T> || std::is_floating_point_v<T>, int> = 0>
  size_t search_sorted(T val, bool from_right = false) const {
    ...
    details::visit_type(type().data_type(), [this, &res, &column_data, val,
                                             from_right](auto type_desc_tag) {
        ...
        while (!res.has_value()) {
          ...
          if (val == mid_value) {
            ...
            if (mid - 1 >= low && val <= accessor.at(mid + 1)) {
              ...
              // val is greater than the previous value, so we have found the right
              // interval
              res = mid;
            }
          }
  ...
  [[nodiscard]] static std::vector<std::shared_ptr<Column>>
  split(const std::shared_ptr<Column>& column, size_t num_rows);
  /// @brief Produces a new column containing only the data in range [start_row,
  /// end_row)
  /// @param[in] start_row Inclusive start of the row range
  /// @param[in] end_row Exclusive end of the row range
  [[nodiscard]] static std::shared_ptr<Column>
  truncate(const std::shared_ptr<Column>& column, size_t start_row, size_t end_row);
  ...
  static void for_each(const Column& input_column, functor&& f) {
    ...
    std::for_each(input_data.cbegin<input_tdt>(), input_data.cend<input_tdt>(),
  ...
  requires std::is_invocable_r_v<
      void, functor,
      typename ColumnData::Enumeration<typename input_tdt::DataTypeTag::raw_type>>
  static void for_each_enumerated(const Column& input_column, functor&& f) {
    auto input_data = input_column.data();
    if (input_column.is_sparse()) {
      std::for_each(
          input_data
              .cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>(),
          input_data
              .cend<input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>(),
          std::forward<functor>(f));
    } else {
      std::for_each(
          input_data
              .cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>(),
          input_data
              .cend<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>(),
          std::forward<functor>(f));
    }
  }

  template <typename input_tdt, typename output_tdt, typename functor>
  requires std::is_invocable_r_v<typename output_tdt::DataTypeTag::raw_type, functor,
                                 typename input_tdt::DataTypeTag::raw_type>
  static void transform(const Column& input_column, Column& output_column,
                        functor&& f) {
    auto input_data = input_column.data();
    initialise_output_column(input_column, output_column);
    auto output_data = output_column.data();
    std::transform(input_data.cbegin<input_tdt>(), input_data.cend<input_tdt>(),
                   output_data.begin<output_tdt>(), std::forward<functor>(f));
  }
  ...
  static void transform(const Column& left_input_column,
    ...
    if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
      // Both dense, use std::transform over the shorter column to avoid going
      // out-of-bounds
      if (left_input_column.row_count() <= right_input_column.row_count()) {
        std::transform(left_input_data.cbegin<left_input_tdt>(),
                       ...
                       right_input_data.cbegin<right_input_tdt>(), output_it,
                       std::forward<functor>(f));
      } else {
        std::transform(right_input_data.cbegin<left_input_tdt>(),
                       right_input_data.cend<left_input_tdt>(),
                       left_input_data.cbegin<right_input_tdt>(), output_it,
                       std::forward<functor>(f));
      }
    } else if (left_input_column.is_sparse() && right_input_column.is_sparse()) {
      // Both sparse, only project rows in the intersection of on-bits from both sparse
      // maps
      ...
      for (auto set_bit = output_column.sparse_map().first(); set_bit < end_bit;
           ++set_bit) {
        *output_it++ = f(left_accessor.at(*set_bit), right_accessor.at(*set_bit));
      }
    } else if (left_input_column.is_sparse() && !right_input_column.is_sparse()) {
      // One sparse, one dense. Use the enumerating forward iterator over the sparse
      // column as it is more efficient than random access
      auto right_accessor = random_accessor<right_input_tdt>(&right_input_data);
      const auto right_column_row_count = right_input_column.row_count();
      const auto left_input_data_cend =
          left_input_data.cend<left_input_tdt, IteratorType::ENUMERATED,
                               IteratorDensity::SPARSE>();
      for (auto left_it =
               left_input_data.cbegin<left_input_tdt, IteratorType::ENUMERATED,
                                      IteratorDensity::SPARSE>();
      ...
      const auto right_input_data_cend =
          right_input_data.cend<right_input_tdt, IteratorType::ENUMERATED,
                                IteratorDensity::SPARSE>();
      for (auto right_it =
               right_input_data.cbegin<right_input_tdt, IteratorType::ENUMERATED,
                                       IteratorDensity::SPARSE>();
  ...
  static void transform(const Column& input_column, util::BitSet& output_bitset,
                        bool sparse_missing_value_output, functor&& f) {
    if (input_column.is_sparse()) {
      initialise_output_bitset(input_column.sparse_map(), sparse_missing_value_output,
                               output_bitset);
    } else {
      // This allows for empty/full result optimisations, technically bitsets are always
      // dynamically sized
      output_bitset.resize(input_column.row_count());
    }
    util::BitSet::bulk_insert_iterator inserter(output_bitset);
    Column::for_each_enumerated<input_tdt>(
        input_column, [&inserter, f = std::forward<functor>(f)](auto enumerated_it) {
  ...
  template <typename left_input_tdt, typename right_input_tdt,
            std::relation<typename left_input_tdt::DataTypeTag::raw_type,
                          typename right_input_tdt::DataTypeTag::raw_type>
                functor>
  static void transform(const Column& left_input_column,
                        const Column& right_input_column, util::BitSet& output_bitset,
                        bool sparse_missing_value_output, functor&& f) {
    auto left_input_data = left_input_column.data();
    auto right_input_data = right_input_column.data();
    util::BitSet::bulk_insert_iterator inserter(output_bitset);
    ...
    if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
      // Both dense, use std::for_each over the shorter column to avoid going
      // out-of-bounds
      auto rows =
          std::max(left_input_column.last_row(), right_input_column.last_row()) + 1;
      output_bitset.resize(rows);
      if (sparse_missing_value_output &&
          left_input_column.row_count() != right_input_column.row_count()) {
        // Dense columns of different lengths, and missing values should be on in the
        // output bitset
        output_bitset.set_range(
            std::min(left_input_column.last_row(), right_input_column.last_row()) + 1,
            rows - 1);
      }
      auto pos = 0u;
      if (left_input_column.row_count() <= right_input_column.row_count()) {
        auto right_it = right_input_data.cbegin<right_input_tdt>();
        std::for_each(left_input_data.cbegin<left_input_tdt>(),
                      left_input_data.cend<left_input_tdt>(),
                      [&right_it, &inserter, &pos,
                       f = std::forward<functor>(f)](auto left_value) {
        ...
        std::for_each(right_input_data.cbegin<right_input_tdt>(),
                      right_input_data.cend<right_input_tdt>(),
                      [&left_it, &inserter, &pos,
                       f = std::forward<functor>(f)](auto right_value) {
      ...
      auto bits_to_check =
          left_input_column.sparse_map() & right_input_column.sparse_map();
      initialise_output_bitset(bits_to_check, sparse_missing_value_output,
                               output_bitset);
      auto left_accessor = random_accessor<left_input_tdt>(&left_input_data);
      auto right_accessor = random_accessor<right_input_tdt>(&right_input_data);
      // TODO: experiment with more efficient bitset traversal methods
      ...
      // One sparse, one dense. Use the enumerating forward iterator over the sparse
      // column as it is more efficient than random access
      initialise_output_bitset(left_input_column.sparse_map(),
                               sparse_missing_value_output, output_bitset);
      auto right_accessor = random_accessor<right_input_tdt>(&right_input_data);
      const auto right_column_row_count = right_input_column.row_count();
      const auto left_input_data_cend =
          left_input_data.cend<left_input_tdt, IteratorType::ENUMERATED,
      ...
      // One sparse, one dense. Use the enumerating forward iterator over the sparse
      // column as it is more efficient than random access
      initialise_output_bitset(right_input_column.sparse_map(),
                               sparse_missing_value_output, output_bitset);
      auto left_accessor = random_accessor<left_input_tdt>(&left_input_data);
      const auto left_column_row_count = left_input_column.row_count();
      const auto right_input_data_cend =
...
template <typename T> JiveTable create_jive_table(const Column& col) {
  ...
  // Calls to scalar_at are expensive, so we precompute them to speed up the sort
  // compare function.
  auto values = col.template clone_scalars_to_vector<T>();
  std::sort(
      std::begin(output.orig_pos_), std::end(output.orig_pos_),
      [&](const auto& a, const auto& b) -> bool { return values[a] < values[b]; });
