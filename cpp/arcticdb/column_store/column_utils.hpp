...
inline py::array array_at(
        const SegmentInMemory& frame, std::size_t col_pos, OutputFormat output_format, py::object& anchor
) {
    ...
    if (frame.empty()) {
        return visit_field(frame.field(col_pos), [output_format](auto tag) {
            ...
            if constexpr (is_sequence_type(data_type)) {
                ...
                dtype = "O";
                // The python representation of multidimensional columns differs from the in-memory/on-storage. In
                // memory, we hold all scalars in a contiguous buffer with the shapes buffer telling us how many
                // elements are there per array. Each element is of size sizeof(DataTypeTag::raw_type). For the python
                // representation the column is represented as an array of (numpy) arrays. Each nested arrays is
                // represented as a pointer to the (numpy) array, thus the size of the element is not the size of the
                // raw type, but the size of a pointer. This also affects how we allocate columns. Check
    ...
    return visit_field(frame.field(col_pos), [&, frame = frame, col_pos = col_pos, output_format](auto tag) {
        ...
        if constexpr (is_sequence_type(data_type)) {
            ...
            // and cpp/arcticdb/pipeline/column_mapping.hpp::datatype_size
            esize = data_type_size(TypeDescriptor{tag}, output_format, DataTypeMode::EXTERNAL);
...
inline std::shared_ptr<pipelines::FrameDataWrapper> initialize_array(
        const SegmentInMemory& frame, OutputFormat output_format, py::object& ref
) {
