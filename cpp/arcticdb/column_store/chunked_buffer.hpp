...
/*
 * ChunkedBufferImpl is an untyped buffer that is composed of blocks of data that can be
 * either regularly or irregularly sized, with optimizations for the following
 * representations:
 *
 *   - a single block
 *   - multiple blocks that are all regularly sized
 *   - multiple blocks that are regularly sized up until a point, and irregularly sized
 * after that point
 *
 * No optimization is performed when the blocks are all irregularly sized.
 *
 * This class can be wrapped in a cursor for the purposes of linear reads and writes
 * (see CursoredBuffer), and subsequently detached if required.
 */
template <size_t DefaultBlockSize> class ChunkedBufferImpl {
...
public:
  ...
  struct Iterator {
    ...
    Iterator(ChunkedBufferImpl* parent, size_t type_size)
        : parent_(parent), type_size_(type_size) {
      if (parent_->empty()) {
        end_ = true;
        return;
      }
  ...
  // If the extra space required does not fit in the current last block, and is
  // <=DefaultBlockSize, then if aligned is set to true, the current last block will be
  // padded with zeros, and a new default sized block added. This allows the buffer to
  // stay regular sized for as long as possible, which greatly improves random access
  // performance, and will also be beneficial with the slab allocator.
    ...
    if (extra_size <= free_space()) {
      ...
      if (is_regular_sized()) {
        ...
        if (extra_size <= DefaultBlockSize && (space == 0 || aligned)) {
          ...
          handle_transition_to_irregular();
          add_block(std::max(DefaultBlockSize, extra_size),
                    DefaultBlockSize * blocks().size());
        }
      } else {
  ...
  // Trim will reduce the size of the chunked buffer to the specified size by dropping
  // blocks that are wholly unneeded i.e. no allocation/memcpy is involved, only
  // deallocation Use in performance critical code where a column is being created, the
  // final size is unknown at construction time but a maximum size is known, by creating
  // a Column using a chunked buffer that is presized in blocks. This unlocks
  // ColumnDataIterator usage (more performant than repeated calls to
  // Column::push_back). Once the column is created and the number of elements known,
  // use this to drop unneeded blocks.
  void trim(size_t requested_size) {
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        requested_size <= bytes_, "Cannot trim ChunkedBuffer with {} bytes to {} bytes",
  ...
  struct BlockAndOffset {
    ...
    BlockAndOffset(MemBlock* block, size_t offset, size_t block_index)
        : block_(block), offset_(offset), block_index_(block_index) {}
  };
  ...
  BlockAndOffset block_and_offset(size_t pos_bytes) const {
    ...
    util::check(!block_offsets_.empty(),
                "Expected an irregular block-sized buffer to have offsets");
    auto block_offset = std::lower_bound(std::begin(block_offsets_),
                                         std::end(block_offsets_), pos_bytes);
  ...
  template <typename T> T& cast(size_t pos) {
  ...
  void check_bytes(size_t pos_bytes, size_t required_bytes) const {
    if (pos_bytes + required_bytes > bytes()) {
      std::string err =
          fmt::format("Cursor overflow in chunked_buffer ptr_cast, cannot read {} "
                      "bytes from a buffer of size {} with cursor "
                      "at {}, as it would require {} bytes. ",
                      required_bytes, bytes(), pos_bytes, pos_bytes + required_bytes);
      ARCTICDB_DEBUG(log::storage(), err);
      throw std::invalid_argument(err);
    }
  }

  template <typename T> T* ptr_cast(size_t pos_bytes, size_t required_bytes) {
  ...
  const T* ptr_cast(size_t pos_bytes, size_t required_bytes) const {
    return (
        const_cast<ChunkedBufferImpl*>(this)->ptr_cast<T>(pos_bytes, required_bytes));
  }
  ...
  inline void assert_size(size_t bytes) const {
    util::check(bytes <= bytes_,
                "Expected allocation size {} smaller than actual allocation {}", bytes,
                bytes_);
  }
...
std::vector<ChunkedBufferImpl<BlockSize>>
split(const ChunkedBufferImpl<BlockSize>& input, size_t nbytes);

template <size_t BlockSize>
ChunkedBufferImpl<BlockSize> truncate(const ChunkedBufferImpl<BlockSize>& input,
