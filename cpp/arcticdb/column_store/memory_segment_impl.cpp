...
SegmentInMemoryImpl::SegmentInMemoryImpl(const StreamDescriptor& desc,
                                         size_t expected_column_size, bool presize,
                                         bool allow_sparse)
    : descriptor_(std::make_shared<StreamDescriptor>(
          StreamDescriptor{desc.id(), desc.index()})),
...
// Append any columns that exist both in this segment and in the 'other' segment onto
// the end of the column in this segment. Any columns that exist in this segment but not
// in the one being appended will be default-valued (or sparse) in this segment. Any
// columns in the segment being appended not in this one will be ignored. This means
// that if you want to append a lot of segments together into a merged segment, you need
// to merge the descriptors first and create the target segment with the merged
// descriptor. Could be modified in the future to retroactively insert and
// default-initialize or sparsify the new column, but that's not required
//  at the moment.
void SegmentInMemoryImpl::append(const SegmentInMemoryImpl& other) {
  ...
  for (auto col = 0u; col < num_columns(); ++col) {
    ...
    if (other_col_index.has_value()) {
      ARCTICDB_DEBUG(log::version(), "Appending column {} at index {}", col_name,
                     *other_col_index);
      ...
      internal::check<ErrorCode::E_INVALID_ARGUMENT>(
          ...
          "Could not append type {} to type {} for column {}, this index {}, other "
          "index {}",
      ...
      if (this_type != *opt_common_type) {
        ...
        descriptor_->mutable_field(col).mutable_type().data_type_ =
            opt_common_type->data_type_;
      }
      ...
      ARCTICDB_DEBUG(log::version(), "Marking {} absent rows for column {}",
                     other.row_count(), col_name);
...
void SegmentInMemoryImpl::create_columns(size_t old_size, size_t expected_column_size,
                                         bool presize, bool allow_sparse) {
  ...
  for (size_t i = old_size; i < size_t(descriptor_->field_count()); ++i) {
    ...
    util::check(type.data_type() != DataType::UNKNOWN,
                "Can't create column in create_columns with unknown data type");
    columns_.emplace_back(std::make_shared<Column>(
        descriptor_->fields(i).type(), expected_column_size, presize, allow_sparse));
  }
  generate_column_map();
}
 ...
 * commit
 */
size_t SegmentInMemoryImpl::on_descriptor_change(const StreamDescriptor& descriptor,
                                                 size_t expected_column_size,
                                                 bool presize, bool allow_sparse) {
  ARCTICDB_TRACE(log::storage(),
                 "Entering descriptor change: descriptor is currently {}, incoming "
                 "descriptor '{}'",
  ...
  *descriptor_ = descriptor;
  ...
  ARCTICDB_TRACE(log::storage(), "Descriptor change: descriptor is now {}",
                 *descriptor_);
  return old_size;
}

std::optional<std::size_t>
SegmentInMemoryImpl::column_index(std::string_view name) const {
...
void SegmentInMemoryImpl::drop_column(std::string_view name) {
  ...
  internal::check<ErrorCode::E_INVALID_ARGUMENT>(
      opt_column_index.has_value(),
      "Cannot drop column with name '{}' as it doesn't exist", name);
...
SegmentInMemoryImpl::filter(util::BitSet&& filter_bitset, bool filter_down_stringpool,
                            bool validate) const {
  ...
  auto output_string_pool =
      filter_down_stringpool ? std::make_shared<StringPool>() : string_pool_;
  // Map from offsets in the input stringpool to offsets in the output stringpool
  // Only used if filter_down_stringpool is true
  ankerl::unordered_dense::map<entity::position_t, entity::position_t>
      input_to_output_offsets;
  // Prepopulate with None and NaN placeholder values to avoid an if statement in a
  // tight loop later
  ...
  for (const auto& column : folly::enumerate(columns())) {
    (*column)->type().visit_tag([&](auto type_desc_tag) {
      ...
      if (is_input_sparse || sparse_map) {
        ...
        output_col_idx = output->add_column(field(column.index),
                                            bitset_including_sparse.count(), true);
      ...
      // Defines the position in output sparse column where we want to write data next
      // (only used in sparse) For dense, we just do +1
        ...
        if (sparse_map) {
          while (bitset_iter != final_bitset->end()) {
            ...
            if (rank_in_filter - 1 != pos_output) {
              // setting sparse_map - marking all rows in output as NULL until this
              // point
              output_col.mark_absent_rows(rank_in_filter - pos_output - 1);
              pos_output = rank_in_filter - 1;
            }
            auto offset = sparse_map.value().rank(*bitset_iter, *sparse_idx) -
                          row_count_so_far - 1;
            ...
            if constexpr (is_sequence_type(DataTypeTag::data_type)) {
              if (filter_down_stringpool) {
                if (auto it = input_to_output_offsets.find(value);
                  ...
                  *output_ptr = it->second;
                  ...
                  auto output_string_pool_offset =
                      output_string_pool->get(str, false).offset();
              ...
              *output_ptr = value;
              ...
              internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
                  "Unexpected block in empty type column in "
                  "SegmentInMemoryImpl::filter");
            } else {
              internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
      ...
      if (sparse_map && validate)
        check_output_bitset(output_col.opt_sparse_map().value(), filter_bitset,
                            sparse_map.value());
    });
  }
  if (num_values != 0)
...
std::shared_ptr<SegmentInMemoryImpl>
SegmentInMemoryImpl::get_output_segment(size_t num_values, bool pre_allocate) const {
  ...
  if (is_sparse()) {
    ...
    if (pre_allocate)
      ...
      output = allocate_dense_segment(
          StreamDescriptor(descriptor().id(), descriptor().index(), {}), num_values);
  }
  return output;
}

std::vector<std::shared_ptr<SegmentInMemoryImpl>>
SegmentInMemoryImpl::partition(const std::vector<uint8_t>& row_to_segment,
  ...
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      ...
      row_to_segment.size(), row_count());
  std::vector<std::shared_ptr<SegmentInMemoryImpl>> output(segment_counts.size());
  if (std::all_of(segment_counts.begin(), segment_counts.end(),
                  [](const size_t& segment_count) { return segment_count == 0; })) {
    return output;
  }
      ...
      if ((*column)->is_sparse()) {
        ...
        for (const auto& segment_idx : folly::enumerate(row_to_segment)) {
          if (*segment_idx != std::numeric_limits<uint8_t>::max()) {
            auto opt_value =
                (*column)->scalar_at<typename type_info::RawType>(segment_idx.index);
        ...
        for (const auto& segment : folly::enumerate(output)) {
          if (static_cast<bool>(*segment)) {
            if (is_sparse()) {
              (*segment)->add_column(field(column.index), segment_counts[segment.index],
                                     true);
            }
            output_ptrs.at(segment.index) =
                reinterpret_cast<typename type_info::RawType*>(
                    (*segment)->column(column.index).ptr());
          }
        }
        auto row_to_segment_it = row_to_segment.cbegin();
        Column::for_each<typename type_info::TDT>(
            **column, [&row_to_segment_it, &output_ptrs](auto val) {
              if (ARCTICDB_LIKELY(*row_to_segment_it !=
...
bool operator==(const SegmentInMemoryImpl& left, const SegmentInMemoryImpl& right) {
  ...
  for (auto col = 0u; col < left.columns_.size(); ++col) {
    ...
    if (is_sequence_type(left_data_type)) {
      ...
      internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
          "Unexpected data type in SegmentInMemory equality check");
    }
  }
  return true;
}
...
std::shared_ptr<SegmentInMemoryImpl>
SegmentInMemoryImpl::truncate(size_t start_row, size_t end_row,
                              bool reconstruct_string_pool) const {
  auto num_values = end_row - start_row;
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      is_sparse() || (start_row < row_count() && end_row <= row_count()),
      "Truncate bounds start_row={} end_row={} outside valid range {}", start_row,
      end_row, row_count());
  ...
  for (const auto&& [idx, column] : folly::enumerate(columns_)) {
    ...
    std::shared_ptr<Column> truncated_column =
        Column::truncate(column, start_row, end_row);
    details::visit_type(column->type().data_type(), [&](auto col_tag) {
      using type_info = ScalarTypeInfo<decltype(col_tag)>;
            ...
            [this, &output](auto string_pool_offset) -> typename type_info::RawType {
              if (is_a_string(string_pool_offset)) {
                const std::string_view string =
                    get_string_from_pool(string_pool_offset, *string_pool_);
...
/// @param[in] unique_column_names If true, any columns from other with names matching
/// those in this are ignored
void SegmentInMemoryImpl::concatenate(SegmentInMemoryImpl&& other,
                                      bool unique_column_names) {
  internal::check<ErrorCode::E_INVALID_ARGUMENT>(
      row_count() == other.row_count(),
      "Cannot concatenate segments with differing row counts: {} {}", row_count(),
      other.row_count());
...
position_t SegmentInMemoryImpl::add_column(FieldRef field, size_t num_rows,
                                           bool presize) {
  ...
  columns_.emplace_back(
      std::make_shared<Column>(field.type(), num_rows, presize, allow_sparse_));
...
position_t SegmentInMemoryImpl::add_column(const Field& field, size_t num_rows,
                                           bool presize) {
  return add_column(FieldRef{field.type(), field.name()}, num_rows, presize);
}

position_t SegmentInMemoryImpl::add_column(FieldRef field_ref,
...
position_t SegmentInMemoryImpl::add_column(const Field& field,
                                           const std::shared_ptr<Column>& column) {
...
void SegmentInMemoryImpl::change_schema(StreamDescriptor descriptor) {
  // util::check(vector_is_unique(descriptor.fields()), "Non-unique fields in
  ...
  for (auto col = 0u; col < descriptor.field_count(); ++col) {
    ...
    if (col_index) {
      ...
      schema::check<ErrorCode::E_DESCRIPTOR_MISMATCH>(
          ...
          "Could not convert type {} to type {} for column {}, this index {}, other "
          "index {}",
          other_type, this_type, col_name, *col_index, col);
      new_columns[col] = std::move(columns_[*col_index]);
    } else {
      auto new_column =
          std::make_shared<Column>(other_type, row_count(), false, allow_sparse_);
...
std::optional<std::string_view> SegmentInMemoryImpl::string_at(position_t row,
                                                               position_t col) const {
  auto td = descriptor_->field(col).type();
  util::check(is_sequence_type(td.data_type()), "Not a sequence type");
  util::check_arg(size_t(row) < row_count(), "Segment index {} out of bounds in string",
                  row);
  ...
  if (is_fixed_string_type(td.data_type()) && col_ref.is_inflated()) {
    ...
    if (offset != std::nullopt && *offset != not_a_string() &&
        *offset != nan_placeholder())
...
std::vector<std::shared_ptr<SegmentInMemoryImpl>>
SegmentInMemoryImpl::split(size_t rows) const {
...
size_t SegmentInMemoryImpl::num_blocks() const {
  return std::accumulate(
      std::begin(columns_), std::end(columns_), 0,
      [](size_t n, const auto& col) { return n + col->num_blocks(); });
}

std::optional<Column::StringArrayData>
SegmentInMemoryImpl::string_array_at(position_t row, position_t col) {
  util::check_arg(size_t(row) < row_count(),
                  "Segment index {} out of bounds in string array", row);
  return column(col).string_array_at(row, *string_pool_);
}

size_t SegmentInMemoryImpl::num_bytes() const {
  return std::accumulate(std::begin(columns_), std::end(columns_), 0,
                         [](size_t n, const auto& col) { return n + col->bytes(); });
}
...
void SegmentInMemoryImpl::sort(position_t idx) {
  ...
  util::check(!sort_col.is_sparse(),
              "Can't sort on sparse column idx {} because it is not supported yet. The "
              "user should either fill the column data or filter the empty columns out",
              idx);
