    ...
    for (const auto& column : folly::enumerate(columns())) {
        (*column)->type().visit_tag([&](auto type_desc_tag) {
            ...
            if (is_input_sparse || sparse_map) {
                ...
                output_col_idx =
                    output->add_column(field(column.index), bitset_including_sparse.count(), AllocationType::PRESIZED);
                            ...
                            if (filter_down_stringpool) {
                                if (auto it = input_to_output_offsets.find(value);
                                    ...
                                    *output_ptr = output_string_pool_offset;
                                    input_to_output_offsets.insert(
                                        std::make_pair(entity::position_t(value), std::move(output_string_pool_offset))
                                    );
                                }
                            } else {
                                *output_ptr = value;
...
void SegmentInMemoryImpl::sort(position_t idx) {
    ...
    util::check(
        !sort_col.is_sparse(),
        "Can't sort on sparse column idx {} because it is not supported yet. The user should either fill the column "
        "data or filter the empty columns out",
        idx
    );
