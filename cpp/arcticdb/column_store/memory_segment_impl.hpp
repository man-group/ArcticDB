...
allocate_sparse_segment(const StreamId& id, const IndexDescriptorImpl& index);

inline std::shared_ptr<SegmentInMemoryImpl>
allocate_dense_segment(const StreamDescriptor& descriptor, size_t row_count);
...
inline void check_output_bitset(const arcticdb::util::BitSet& output,
  ...
  // like The set bits in filter decides the row ids in the output. The corresponding
  // values in sparse_map should match output bitset
  auto filter_iter = filter.first();
  arcticdb::util::BitSetSizeType output_pos = 0;
  while (filter_iter != filter.end()) {
    arcticdb::util::check_rte(column_bitset.test(*(filter_iter++)) ==
                                  output.test(output_pos++),
...
public:
  struct Location {
    Location(SegmentInMemoryImpl* parent, ssize_t row_id_, size_t column_id)
        : parent_(parent), row_id_(row_id_), column_id_(column_id) {}

    template <class Callable> auto visit(Callable&& c) const {
      return entity::visit_field(
          parent_->descriptor().field(column_id_),
          [that = this, c = std::forward<Callable>(c)](auto type_desc_tag) {
            using RawType =
                typename std::decay_t<decltype(type_desc_tag)>::DataTypeTag::raw_type;
            return c(
                that->parent_->scalar_at<RawType>(that->row_id_, that->column_id_));
          });
    }

    template <class Callable> auto visit_string(Callable&& c) const {
      return entity::visit_field(
          parent_->descriptor().field(column_id_),
          [that = this, c = std::forward<Callable>(c)](auto type_desc_tag) {
            using DTT = typename std::decay_t<decltype(type_desc_tag)>::DataTypeTag;
            if constexpr (is_sequence_type(DTT::data_type))
              return c(that->parent_->string_at(that->row_id_,
                                                position_t(that->column_id_)));
          });
    }

    template <class Callable> auto visit_field(Callable&& c) const {
      const auto& field = parent_->descriptor().field(column_id_);
      return entity::visit_field(
          parent_->descriptor().field(column_id_),
          [&field, that = this, c = std::forward<Callable>(c)](auto type_desc_tag) {
            using DataTypeTag =
                typename std::decay_t<decltype(type_desc_tag)>::DataTypeTag;
            using RawType = typename DataTypeTag::raw_type;
            if constexpr (is_sequence_type(DataTypeTag::data_type))
              return c(
                  that->parent_->string_at(that->row_id_, position_t(that->column_id_)),
                  std::string_view{field.name()}, field.type());
              ...
              internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
                  "visit_field does not support empty-type columns");
            else
              internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
                  "visit_field called with unexpected column type");
          });
    }

    template <class Callable> auto visit(Callable&& c) {
      return entity::visit_field(
          parent_->descriptor().field(column_id_),
          [that = this, c = std::forward<Callable>(c)](auto type_desc_tag) {
            using RawType =
                typename std::decay_t<decltype(type_desc_tag)>::DataTypeTag::raw_type;
            return c(
                that->parent_->reference_at<RawType>(that->row_id_, that->column_id_));
          });
    }
    ...
    template <typename RawType> RawType& value() {
      return parent_->reference_at<RawType>(row_id_, column_id_);
    }

    template <typename RawType> [[nodiscard]] const RawType& value() const {
      return parent_->reference_at<RawType>(row_id_, column_id_);
    }
  ...
  /*
   * Arctic is fundamentally a column oriented database. The below RowIterator however
   * iterates one row at a time, which is not an optimized operation in a columnar
   * database.
   *
   * As a result this is probably not what you want unless you *know* that it is what
   * you want.
   */
  template <class ValueType>
  class RowIterator
      : public boost::iterator_facade<RowIterator<ValueType>, ValueType,
  ...
  private:
    ...
    template <class> friend class SegmentIterator;

    template <class OtherValue> bool equal(const RowIterator<OtherValue>& other) const {
  ...
  struct Row {
    ...
    Row(SegmentInMemoryImpl* parent, ssize_t row_id_)
        : parent_(parent), row_id_(row_id_) {}

    [[nodiscard]] SegmentInMemoryImpl& segment() const { return *parent_; }
    ...
    bool operator<(const Row& other) const {
      return entity::visit_field(parent_->field(0), [this, &other](auto type_desc_tag) {
        ...
        return parent_->scalar_at<RawType>(row_id_, 0) <
    ...
    template <class IndexType> [[nodiscard]] auto index() const {
    ...
    friend void swap(Row& left, Row& right) noexcept {
      ...
      auto b = right.begin();
      for (; a != left.end(); ++a, ++b) {
        util::check(a->has_value() && b->has_value(),
                    "Can't swap sparse column values, unsparsify first?");
        a->visit([&b](auto& val) {
    ...
    iterator begin() { return {parent_, row_id_}; }

    iterator end() {
      return {parent_, row_id_, size_t(parent_->descriptor().fields().size())};
    }

    [[nodiscard]] const_iterator begin() const { return {parent_, row_id_}; }
    ...
    void assert_same_parent(const Row& other) const {
      util::check(parent_ == other.parent_,
                  "Invalid iterator comparison, different parents");
    }

    void swap_parent(const Row& other) { parent_ = other.parent_; }
        ...
        if constexpr (T::DimensionTag::value == Dimension::Dim0) {
          if constexpr (is_sequence_type(T::DataTypeTag::data_type)) {
            ...
            if constexpr (std::is_same_v<RawType, S>) {
              ...
              internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
                  "Type mismatch in scalar access");
            }
            ...
            internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
                "Unexpected data type in scalar access");
          }
        } else {
          internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
              "Scalar method called on multidimensional column");
        }
      });
      return val;
    }
  ...
  class SegmentIterator
      : public boost::iterator_facade<SegmentIterator<ValueType>, ValueType,
                                      boost::random_access_traversal_tag> {
  public:
    using value_type = ValueType;

    explicit SegmentIterator(SegmentInMemoryImpl* parent) : row_(parent, 0) {}
  ...
  explicit SegmentInMemoryImpl(const StreamDescriptor& desc,
                               size_t expected_column_size, bool presize,
                               bool allow_sparse);
  ...
  iterator end() {
    util::check(
        row_id_ != -1,
        "End iterator called with negative row id, iterator will never terminate");
  ...
  void create_columns(size_t old_size, size_t expected_column_size, bool presize,
                      bool allow_sparse);

  /**
   * @param descriptor
   * @return false is descriptor change is not compatible and should trigger a segment
   * commit
   */
  ...
  const Field& column_descriptor(size_t col) { return (*descriptor_)[col]; }

  void end_row() { row_id_++; }
  ...
  void end_block_write(ssize_t size) { row_id_ += size; }
  ...
  void set_scalar(position_t idx, T val) {
    ARCTICDB_TRACE(log::version(), "Segment setting scalar {} at row {} column {}", val,
                   row_id_ + 1, idx);
  ...
  void set_sparse_block(position_t idx, ChunkedBuffer&& buffer, Buffer&& shapes,
                        util::BitSet&& bitset) {
    column_unchecked(idx).set_sparse_block(std::move(buffer), std::move(shapes),
                                           std::move(bitset));
  }
  ...
  void set_string(position_t pos, std::string_view str) {
    ARCTICDB_TRACE(log::version(), "Segment setting string {} at row {} column {}", str,
                   row_id_ + 1, pos);
  ...
  void set_string_array(position_t idx, size_t string_size, size_t num_strings,
                        char* data) {
    check_column_index(idx);
    column_unchecked(idx).set_string_array(row_id_ + 1, string_size, num_strings, data,
                                           string_pool());
  }
  ...
  Column& column_ref(position_t idx) { return column(idx); }
  ...
  Column& column_unchecked(position_t idx) { return *columns_[idx]; }

  std::shared_ptr<Column> column_ptr(position_t idx) { return columns_[idx]; }

  const Column& column_unchecked(position_t idx) const { return *columns_[idx]; }

  std::vector<std::shared_ptr<Column>>& columns() { return columns_; }
  ...
  std::optional<T> scalar_at(position_t row, position_t col) const {
    util::check_arg(size_t(row) < row_count(),
                    "Segment index {} out of bounds in scalar", row);
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        !is_empty_type(column(col).type().data_type()),
  ...
  template <typename T> T& reference_at(position_t row, position_t col) {
    util::check_arg(size_t(row) < row_count(),
                    "Segment index {} out of bounds in scalar ref", row);
  ...
  std::optional<TensorType<T>> tensor_at(position_t row, position_t col) const {
    util::check_arg(size_t(row) < row_count(),
                    "Segment index {} out of bounds in tensor", row);
    return column(col).tensor_at<T>(row);
  }

  std::optional<std::string_view> string_at(position_t row, position_t col) const;

  std::optional<Column::StringArrayData> string_array_at(position_t row,
                                                         position_t col);
  ...
  const std::shared_ptr<StringPool>& string_pool_ptr() const { return string_pool_; }

  void check_column_index(position_t idx) const {
    util::check_arg(idx < position_t(columns_.size()), "Column index {} out of bounds",
                    idx);
  }
  ...
  ColumnData string_pool_data() const {
    return ColumnData{&string_pool_->data(), &string_pool_->shapes(),
                      string_pool_descriptor().type(), nullptr};
  }
  ...
  ColumnData column_data(size_t col) const { return columns_[col]->data(); }

  const StreamDescriptor& descriptor() const { return *descriptor_; }

  StreamDescriptor& descriptor() { return *descriptor_; }
  ...
  const Field& field(size_t index) const { return descriptor()[index]; }

  void set_row_id(ssize_t rid) { row_id_ = rid; }

  ssize_t get_row_id() { return row_id_; }
  ...
  void set_compacted(bool value) { compacted_ = value; }

  void check_magic() const { magic_.check(); }

  friend bool operator==(const SegmentInMemoryImpl& left,
                         const SegmentInMemoryImpl& right);

  bool allow_sparse() const { return allow_sparse_; }

  // TODO: Very slow, fix this by storing it in protobuf
  bool is_sparse() const {
    return std::any_of(std::begin(columns_), std::end(columns_),
                       [](const auto& c) { return c->is_sparse(); });
  }
  ...
  std::shared_ptr<SegmentInMemoryImpl>
  get_output_segment(size_t num_values, bool pre_allocate = true) const;
  ...
  /// @brief Construct a copy of the segment containing only rows in [start_row;
  /// end_row)
  /// @param start_row Start of the row range (inclusive)
  /// @param end_Row End of the row range (exclusive)
  /// @param reconstruct_string_pool When truncating some of the strings values of the
  /// original
  ///  segment might not be referenced in the resulting segment. In this case,
  ///  reconstructing the string pool will save some memory. Note that reconstructing
  ///  the string pool is an expensive operation and should be avoided if possible.
  std::shared_ptr<SegmentInMemoryImpl> truncate(size_t start_row, size_t end_row,
                                                bool reconstruct_string_pool) const;

  // Partitions the segment into n new segments. Each row in the starting segment is
  // mapped to one of the output segments by the row_to_segment vector (std::nullopt
  // means the row is not included in any output segment). segment_counts is the length
  // of the number of output segments, and should be greater than or equal to the max
  // value in row_to_segment
  std::vector<std::shared_ptr<SegmentInMemoryImpl>>
  partition(const std::vector<uint8_t>& row_to_segment,
...
private:
  ...
  mutable std::unique_ptr<std::mutex> column_map_mutex_ =
...
inline std::shared_ptr<SegmentInMemoryImpl>
allocate_sparse_segment(const StreamId& id, const IndexDescriptorImpl& index) {
  return std::make_shared<SegmentInMemoryImpl>(StreamDescriptor{id, index}, 0, false,
                                               true);
}
