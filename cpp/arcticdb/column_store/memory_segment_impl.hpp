...
inline void check_output_bitset(
    const arcticdb::util::BitSet& output,
...
class SegmentInMemoryImpl {
  public:
    struct Location {
        ...
        [[nodiscard]] bool has_value() const { return parent_->has_value_at(row_id_, position_t(column_id_)); }
    ...
    struct Row {
        ...
        Row(SegmentInMemoryImpl* parent, ssize_t row_id_) : parent_(parent), row_id_(row_id_) {}

        [[nodiscard]] SegmentInMemoryImpl& segment() const { return *parent_; }

        [[nodiscard]] const StreamDescriptor& descriptor() const { return parent_->descriptor(); }
        ...
        Location operator[](int pos) const { return Location{parent_, row_id_, size_t(pos)}; }
        ...
        iterator end() { return {parent_, row_id_, size_t(parent_->descriptor().fields().size())}; }
        ...
        bool operator==(const Row& other) const { return row_id_ == other.row_id_ && parent_ == other.parent_; }
    ...
    class SegmentIterator
      ...
      public:
        ...
        SegmentIterator(SegmentInMemoryImpl* parent, ssize_t row_id_) : row_(parent, row_id_) {}

        template<class OtherValue>
        explicit SegmentIterator(const SegmentIterator<OtherValue>& other) : row_(other.row_) {}
      ...
      private:
        ...
        std::ptrdiff_t distance_to(const SegmentIterator& other) const { return other.row_.row_id_ - row_.row_id_; }
    ...
    const_iterator begin() const { return const_iterator{const_cast<SegmentInMemoryImpl*>(this)}; }
    ...
    bool has_value_at(position_t row, position_t col) const { return column(col).has_value_at(row); }
    ...
    void attach_descriptor(std::shared_ptr<StreamDescriptor> desc) { descriptor_ = std::move(desc); }
    ...
    bool allow_sparse() const { return allow_sparse_ == Sparsity::PERMITTED; }
    ...
    void set_string_pool(std::shared_ptr<StringPool> string_pool) { string_pool_ = std::move(string_pool); }
    ...
    bool has_user_metadata() { return tsd_.has_value() && !tsd_->proto_is_null() && tsd_->proto().has_user_meta(); }

    const arcticdb::proto::descriptors::UserDefinedMetadata& user_metadata() const { return tsd_->user_metadata(); }
