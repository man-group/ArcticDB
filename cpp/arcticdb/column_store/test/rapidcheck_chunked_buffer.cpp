...
RC_GTEST_PROP(ChunkedBuffer, ReadWriteRegular,
              (const std::vector<uint8_t>& input, uint8_t chunk_size)) {
  using namespace arcticdb;
...
RC_GTEST_PROP(ChunkedBuffer, SplitBuffer,
              (const std::vector<uint8_t>& input, uint8_t chunk_size,
               uint32_t split_size)) {
  using namespace arcticdb;
  RC_PRE(input.size() > 0u);
  ...
  for (size_t i = 0; i < top; ++i) {
    ...
    if (buf_obj.cast<uint8_t>(where) != input[i])
      ARCTICDB_DEBUG(log::version(), "Mismatch at {} ({}), {} != {}", i, where, left,
                     right);
...
RC_GTEST_PROP(ChunkedBuffer, ReadWriteIrregular,
...
RC_GTEST_PROP(ChunkedBuffer, ReadWriteTransition,
              (const std::vector<std::vector<uint64_t>>& inputs,
               uint8_t regular_chunks)) {
  using namespace arcticdb;
  CursoredBuffer<ChunkedBufferImpl<64>> cb;
