...
// Tricky to construct columns using a ChunkedBuffer with the testing size of 64 bytes,
// so only use rapidcheck for single block tests, and test regualr and irregular
// accessors in test_column_data_random_accessor.cpp
RC_GTEST_PROP(ColumnDataRandomAccessor, DenseSingleBlock,
...
RC_GTEST_PROP(ColumnDataRandomAccessor, SparseSingleBlock,
              (const std::vector<int64_t>& input)) {
  using namespace arcticdb;
  RC_PRE(input.size() > 0u);
  auto n = input.size();
