...
protected:
  std::string get_name() override { return "ingestion_stress"; }
};
...
TEST(IngestionStress, ScalarInt) {
  ...
  as::FixedSchema schema{index.create_stream_descriptor(
                             NumericId{123}, fields_from_range(std::move(columns))),
                         index};
...
TEST_F(IngestionStressStore, ScalarIntAppend) {
  ...
  for (timestamp i = 0; i < timestamp(NumRows); ++i) {
    agg.start_row(timestamp{i})([&](auto& rb) {
      for (timestamp j = 1u; j <= timestamp(NumColumns); ++j)
        rb.set_scalar_by_name(columns[j - 1].name(), uint64_t(i + j),
                              columns[j - 1].type().data_type());
    });
  }
  timer.stop_timer(timer_name);
  GTEST_COUT << x << " " << timer.display_all() << std::endl;
  ...
  auto new_descriptor =
      index.create_stream_descriptor(symbol, fields_from_range(columns_second));
  for (timestamp i = 0u; i < timestamp(NumRows); ++i) {
    agg.start_row(timestamp(i + NumRows))([&](auto& rb) {
      for (uint64_t j = 1u; j <= 2; ++j)
        rb.set_scalar_by_name(columns_second[j - 1].name(), uint64_t(i + j),
                              columns_second[j - 1].type().data_type());
    });
  }
  GTEST_COUT << " 2 done";
  ...
  for (auto& seg : sink.segments_)
    arcticdb::append_incomplete_segment(test_store_->_test_get_store(), symbol,
                                        std::move(seg));
  ...
  auto read_result =
      test_store_->read_dataframe_version(symbol, VersionQuery{}, read_query, ro);
...
TEST_F(IngestionStressStore, ScalarIntDynamicSchema) {
  ...
  for (timestamp i = 0; i < timestamp(NumRows); ++i) {
    agg.start_row(timestamp{i})([&](auto& rb) {
      for (uint64_t j = 1u; j < NumColumnsFirstWrite; ++j)
        rb.set_scalar_by_name(columns_first[j - 1].name(), uint64_t(i + j),
                              columns_first[j - 1].type().data_type());
    });
  }
  timer.stop_timer(timer_name);
  GTEST_COUT << " 1 done";
  ...
  for (auto i = 0u; i < NumRows; ++i) {
    agg.start_row(timestamp{i + NumRows * 2})([&](auto& rb) {
      for (uint64_t j = 1u; j < NumColumnsFirstWrite; ++j)
        rb.set_scalar_by_name(columns_first[j].name(), uint64_t(i + j),
                              columns_first[j].type().data_type());
    });
  }
  GTEST_COUT << " 3 done";
  ...
  for (auto i = 0u; i < NumRows; ++i) {
    agg.start_row(timestamp{i + NumRows * 3})([&](auto& rb) {
      for (uint64_t j = 1u; j < NumColumnsSecondWrite; ++j)
        rb.set_scalar_by_name(columns_second[j].name(), uint64_t(i + j),
                              columns_second[j].type().data_type());
    });
  }
  ...
  read_query.row_filter = universal_range();
  auto read_result =
      test_store_->read_dataframe_internal(symbol, read_query, read_options);
}
