...
SegmentInMemory
get_shuffled_segment(const StreamId& id, size_t num_rows, size_t num_columns,
  ...
  for (auto i = 0u; i < num_columns; ++i) {
    fields.emplace_back(make_scalar_type(DataType::UINT64),
  ...
  auto segment =
      SegmentInMemory{get_test_descriptor<stream::TimeseriesIndex>(id, field_refs),
                      num_rows, false, sparsity_percentage.has_value()};

  for (auto i = 0u; i <= num_columns; ++i) {
    auto& column = segment.column(i);
    auto values = get_random_permutation(num_rows, g);
    // We ensure the column we're sorting by is NOT sparse. As of 2023/12 sorting by
    // sparse columns is not supported.
