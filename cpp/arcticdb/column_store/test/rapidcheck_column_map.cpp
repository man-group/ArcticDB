...
std::optional<size_t>
test_find_column_index_by_name(const FieldCollection& frame_fields,
                               std::string_view name) {
  auto col_in_frame_it =
      std::find_if(frame_fields.begin(), frame_fields.end(),
...
RC_GTEST_PROP(ColumnMap, FromDescriptor,
              (const arcticdb::entity::StreamDescriptor& desc)) {
