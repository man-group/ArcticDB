...
TEST(MemSegment, Empty) {
  ...
  // Even though this index is out of bounds it will not throw as there are no columns
  // to visit
  //    ASSERT_NO_THROW(s.visit(5, [] (auto&& ) { std::cout << "testing" <<
  //    std::endl;}));
}

template <typename TDT>
void test_segment_type(size_t num_values = 20, size_t num_tests = 50,
...
TEST(MemSegment, Scalar) {
  test_segment_type<
      TypeDescriptorTag<DataTypeTag<DataType::UINT8>, DimensionTag<Dimension::Dim0>>>();
  test_segment_type<TypeDescriptorTag<DataTypeTag<DataType::UINT16>,
                                      DimensionTag<Dimension::Dim0>>>();
  test_segment_type<TypeDescriptorTag<DataTypeTag<DataType::UINT32>,
                                      DimensionTag<Dimension::Dim0>>>();
  test_segment_type<TypeDescriptorTag<DataTypeTag<DataType::UINT64>,
                                      DimensionTag<Dimension::Dim0>>>();
  test_segment_type<
      TypeDescriptorTag<DataTypeTag<DataType::INT8>, DimensionTag<Dimension::Dim0>>>();
  test_segment_type<
      TypeDescriptorTag<DataTypeTag<DataType::INT16>, DimensionTag<Dimension::Dim0>>>();
  test_segment_type<
      TypeDescriptorTag<DataTypeTag<DataType::INT32>, DimensionTag<Dimension::Dim0>>>();
  test_segment_type<
      TypeDescriptorTag<DataTypeTag<DataType::INT64>, DimensionTag<Dimension::Dim0>>>();
  test_segment_type<TypeDescriptorTag<DataTypeTag<DataType::FLOAT32>,
                                      DimensionTag<Dimension::Dim0>>>();
  test_segment_type<TypeDescriptorTag<DataTypeTag<DataType::FLOAT64>,
                                      DimensionTag<Dimension::Dim0>>>();
}
...
TEST(MemSegment, IterateWithEmptyTypeColumn) {
  ...
  seg.add_column(scalar_field(int_column->type().data_type(), "int_column"),
                 int_column);

  auto empty_column = std::make_shared<Column>(generate_empty_column());
  seg.add_column(scalar_field(empty_column->type().data_type(), "empty_column"),
                 empty_column);
  seg.set_row_id(num_rows - 1);
  for (auto&& [idx, row] : folly::enumerate(seg)) {
    ASSERT_EQ(static_cast<int64_t>(idx), row.scalar_at<int64_t>(0));
    // Exception should be thrown regardless of the type requested for empty type
    // columns
    EXPECT_THROW([[maybe_unused]] auto v = row.scalar_at<int64_t>(1).has_value(),
                 InternalException);
    EXPECT_THROW([[maybe_unused]] auto v = row.scalar_at<float>(1).has_value(),
                 InternalException);
    EXPECT_THROW([[maybe_unused]] auto v = row.scalar_at<uint8_t>(1).has_value(),
                 InternalException);
  }
}
    ...
    for (auto value : folly::enumerate(*row)) {
      value->visit([&](const auto& val) {
        ...
        if (value.index == 0) {
          ...
          if constexpr (std::is_floating_point_v<ValType>) {
            ASSERT_EQ(val,
                      get_floating_point_value_for_offset<ValType>(0, row.index) + 1);
          }
        }
      });
    }
...
TEST(MemSegment, StdFindIf) {
  ...
  auto frame_wrapper = get_test_timeseries_frame("modify", num_rows, 0);
  auto& segment = frame_wrapper.segment_;
  const auto it = std::find_if(std::begin(segment), std::end(segment),
...
TEST(MemSegment, LowerBound) {
  ...
  std::copy_if(std::begin(segment), std::end(segment), std::back_inserter(odds_segment),
               [](SegmentInMemory::Row& row) {
                 return row.template index<TimeseriesIndex>() & 1;
               });
  auto lb = std::lower_bound(std::begin(odds_segment), std::end(odds_segment),
                             timestamp(50), [](SegmentInMemory::Row& row, timestamp t) {
...
TEST(MemSegment, SplitSegment) {
  ...
  for (auto i = 0u; i < 100; ++i) {
    ASSERT_EQ(split_segs[i / 10].scalar_at<int8_t>(i % 10, 1),
              segment.scalar_at<int8_t>(i, 1));
    ASSERT_EQ(split_segs[i / 10].scalar_at<uint64_t>(i % 10, 2),
              segment.scalar_at<uint64_t>(i, 2));
...
TEST(MemSegment, SplitSparseSegment) {
  ...
  using DynamicAggregator =
      Aggregator<TimeseriesIndex, DynamicSchema, stream::RowCountSegmentPolicy,
  ...
  for (auto i = 0u; i < 100; ++i) {
    ASSERT_EQ(split_segs[i / 10].scalar_at<uint32_t>(i % 10, 1),
              sparse_segment.scalar_at<uint32_t>(i, 1));
    ASSERT_EQ(split_segs[i / 10].scalar_at<uint64_t>(i % 10, 2),
              sparse_segment.scalar_at<uint64_t>(i, 2));
    if (i % 4 == 0) {
      ASSERT_EQ(split_segs[i / 10].scalar_at<uint64_t>(i % 10, 3),
                sparse_segment.scalar_at<uint64_t>(i, 3));
    } else {
      ASSERT_FALSE(
          static_cast<bool>(split_segs[i / 10].scalar_at<uint64_t>(i % 10, 3)));
    }
    if (i % 4 == 2) {
      ASSERT_EQ(split_segs[i / 10].string_at(i % 10, 4),
                sparse_segment.string_at(i, 4));
    } else {
...
TEST(MemSegment, Append) {
  StreamDescriptor descriptor{
      stream_descriptor(StreamId("test"), TimeseriesIndex::default_index(),
