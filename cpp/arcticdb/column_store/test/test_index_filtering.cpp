...
std::pair<TimeseriesDescriptor, std::vector<SliceAndKey>>
get_sample_slice_and_key(StreamId stream_id, VersionId version_id,
                         size_t col_slices = 1, size_t row_slices = 10) {
  StreamDescriptor stream_desc{
      stream_id, IndexDescriptorImpl{1, IndexDescriptorImpl::Type::TIMESTAMP}};
  ...
  for (auto col_range = 0u; col_range < col_slices; ++col_range) {
    ...
    for (auto i = 0u; i < row_slices; ++i) {
      slice_and_keys.emplace_back(SliceAndKey{
          ...
          AtomKey{stream_id, version_id, i, col_range,
                  IndexValue{NumericIndex{start_val}},
                  IndexValue{NumericIndex{end_val}}, KeyType::TABLE_DATA}});
      start_val = end_val;
      end_val += step;
    }
...
TEST(IndexFilter, Static) {
  ...
  index::IndexWriter<stream::RowCountIndex> writer(mock_store, partial_key,
                                                   std::move(tsd));
  ...
  auto pipeline_context = std::make_shared<PipelineContext>(
      StreamDescriptor{isr.tsd().as_stream_descriptor()});
  ...
  pipeline_context->slice_and_keys_ =
      filter_index(isr, combine_filter_functions(queries));
...
TEST(IndexFilter, Dynamic) {
  ...
  auto queries = get_column_bitset_and_query_functions<index::IndexSegmentReader>(
      read_query, pipeline_context, true, false);
