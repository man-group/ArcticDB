...
struct ColumnAppend : rc::state::Command<ColumnModel, arcticdb::Column> {
  ...
  void apply(ColumnModel& s0) const override { s0.data.push_back(value_); }
  ...
  void show(std::ostream& os) const override { os << "Append(" << value_ << ")"; }
};
...
struct ColumnLowerBound : rc::state::Command<ColumnModel, arcticdb::Column> {
  ...
  void run(const ColumnModel& m, arcticdb::Column& sut) const override {
    ...
    const auto model_it =
        std::lower_bound(std::begin(m.data), std::end(m.data), value_);
    const auto sut_it = std::lower_bound(sut.template begin<TagType>(),
                                         sut.template end<TagType>(), value_);
    RC_ASSERT(std::distance(std::begin(m.data), model_it) ==
              std::distance(sut.template begin<TagType>(), sut_it));
  }
...
struct ColumnUpperBound : rc::state::Command<ColumnModel, arcticdb::Column> {
  ...
  void run(const ColumnModel& m, arcticdb::Column& sut) const override {
    ...
    using TagType =
        TypeDescriptorTag<DataTypeTag<DataType::UINT64>, DimensionTag<Dimension::Dim0>>;
    const auto model_it =
        std::upper_bound(std::begin(m.data), std::end(m.data), value_);
    const auto sut_it = std::upper_bound(sut.template begin<TagType>(),
                                         sut.template end<TagType>(), value_);
    RC_ASSERT(std::distance(std::begin(m.data), model_it) ==
              std::distance(sut.template begin<TagType>(), sut_it));
  }
...
struct ColumnRead : rc::state::Command<ColumnModel, arcticdb::Column> {
  ...
  void run(const ColumnModel& s0, arcticdb::Column& sut) const override {
    RC_ASSERT(*sut.ptr_cast<uint64_t>(position_, sizeof(uint64_t)) ==
              s0.data[position_]);
  }

  void show(std::ostream& os) const override { os << "Get(" << position_ << ")"; }
};
...
RC_GTEST_PROP(Column, Rapidcheck, ()) {
  ...
  arcticdb::Column sut(TypeDescriptor(DataType::UINT64, Dimension::Dim0), 0, false,
                       false);
  rc::state::check(initial_state, sut,
                   &rc::state::gen::execOneOf<ColumnAppend, ColumnRead,
                                              ColumnLowerBound, ColumnUpperBound>);
}
...
RC_GTEST_PROP(Column, TruncateDense, (const std::vector<int64_t>& input)) {
  ...
  auto column =
      std::make_shared<Column>(static_cast<TypeDescriptor>(TDT{}), 0, false, false);
  ...
  for (auto input_idx = start_row; input_idx < end_row;
       input_idx++, truncated_column_idx++) {
...
RC_GTEST_PROP(Column, TruncateSparse, (const std::vector<int64_t>& input)) {
  ...
  using TDT =
      TypeDescriptorTag<DataTypeTag<DataType::INT64>, DimensionTag<Dimension ::Dim0>>;
  auto column =
      std::make_shared<Column>(static_cast<TypeDescriptor>(TDT{}), 0, false, true);
  for (size_t idx = 0; idx < n; ++idx) {
    if (mask[idx]) {
      column->set_scalar<int64_t>(idx, input[idx]);
    }
...
RC_GTEST_PROP(Column, SearchSorted,
              (const std::vector<int64_t>& input, int64_t value_to_find)) {
  using namespace arcticdb;
  RC_PRE(input.size() > 0u);
