...
struct ColumnRead : rc::state::Command<ColumnModel, arcticdb::Column> {
    ...
    void checkPreconditions(const ColumnModel& s0) const override { RC_PRE(position_ < s0.data.size()); }
...
RC_GTEST_PROP(Column, Rapidcheck, ()) {
    ...
    arcticdb::Column sut(
        TypeDescriptor(DataType::UINT64, Dimension::Dim0), 0, AllocationType::DYNAMIC, Sparsity::NOT_PERMITTED
    );
    rc::state::
        check(initial_state, sut, &rc::state::gen::execOneOf<ColumnAppend, ColumnRead, ColumnLowerBound, ColumnUpperBound>);
}
...
RC_GTEST_PROP(Column, TruncateDense, (const std::vector<int64_t>& input)) {
    ...
    auto column = std::make_shared<Column>(
        static_cast<TypeDescriptor>(TDT{}), 0, AllocationType::DYNAMIC, Sparsity::NOT_PERMITTED
    );
...
RC_GTEST_PROP(Column, TruncateSparse, (const std::vector<int64_t>& input)) {
    ...
    using TDT = TypeDescriptorTag<DataTypeTag<DataType::INT64>, DimensionTag<Dimension ::Dim0>>;
    auto column =
        std::make_shared<Column>(static_cast<TypeDescriptor>(TDT{}), 0, AllocationType::DYNAMIC, Sparsity::PERMITTED);
    for (size_t idx = 0; idx < n; ++idx) {
        if (mask[idx]) {
            column->set_scalar<int64_t>(idx, input[idx]);
        }
