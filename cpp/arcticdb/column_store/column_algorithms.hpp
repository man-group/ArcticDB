...
static void for_each(const Column& input_column, functor&& f) {
    auto data = ColumnData::from_column(input_column);
    std::for_each(data.cbegin<input_tdt>(), data.cend<input_tdt>(), std::forward<functor>(f));
}
...
static void for_each_enumerated(const Column& input_column, functor&& f) {
    ...
    if (input_column.is_sparse()) {
        auto begin = data.cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
        auto end = data.cend<input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
        std::for_each(begin, end, std::forward<functor>(f));
    } else {
        auto begin = data.cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>();
        auto end = data.cend<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>();
        std::for_each(begin, end, std::forward<functor>(f));
    }
}
...
static void for_each_enumerated_flattened(
    ...
    // Alternatively we could make it random access but this can be tricky because random access in a
    ...
    if (input_column.is_sparse()) {
        ...
        if (end_idx.has_value()) {
            end = data.cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
        ...
        auto end = data.cend<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>();
        if (end_idx.has_value()) {
            end = data.cbegin<input_tdt, IteratorType::ENUMERATED, IteratorDensity::DENSE>();
...
static void transform(
    ...
    if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
        ...
        if (left_input_column.row_count() <= right_input_column.row_count()) {
            std::transform(
                    left_data.cbegin<left_input_tdt>(),
                    left_data.cend<left_input_tdt>(),
                    right_data.cbegin<right_input_tdt>(),
                    output_it,
                    std::forward<functor>(f)
            );
        } else {
            std::transform(
                    right_data.cbegin<left_input_tdt>(),
                    right_data.cend<left_input_tdt>(),
                    left_data.cbegin<right_input_tdt>(),
                    output_it,
                    std::forward<functor>(f)
            );
        }
        ...
        auto left_data = ColumnData::from_column(left_input_column);
        auto right_accessor = random_accessor<right_input_tdt>(&right_input_column);
        const auto right_column_row_count = right_input_column.row_count();
        const auto left_cend = left_data.cend<left_input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
        for (auto left_it = left_data.cbegin<left_input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
             left_it != left_cend && left_it->idx() < right_column_row_count;
             ++left_it) {
        ...
        // efficient than random access
        auto right_data = ColumnData::from_column(right_input_column);
        auto left_accessor = random_accessor<left_input_tdt>(&left_input_column);
        const auto left_column_row_count = left_input_column.row_count();
        const auto right_cend = right_data.cend<right_input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
        for (auto right_it = right_data.cbegin<right_input_tdt, IteratorType::ENUMERATED, IteratorDensity::SPARSE>();
             right_it != right_cend && right_it->idx() < left_column_row_count;
             ++right_it) {
...
static void transform(
    ...
    if (!left_input_column.is_sparse() && !right_input_column.is_sparse()) {
        ...
        if (left_input_column.row_count() <= right_input_column.row_count()) {
            ...
            std::for_each(
                    right_data.cbegin<right_input_tdt>(),
                    right_data.cend<right_input_tdt>(),
