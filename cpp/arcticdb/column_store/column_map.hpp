...
public:
  ...
  void insert(std::string_view name, size_t index) {
    ...
    column_offsets_.insert(std::make_pair<std::string_view, size_t>(
        pool_.get_view(off_str.offset()), std::forward<size_t>(index)));
  ...
  std::optional<std::size_t> column_index(std::string_view name) {
    ...
    if (it != column_offsets_.end())
      ...
      ARCTICDB_TRACE(log::version(), "Column {} not found in map of size {}", name,
