...
split(const ChunkedBufferImpl<BlockSize>& input, size_t nbytes) {
  ...
  ARCTICDB_DEBUG(log::version(), "Split buffer of size {} into chunks of {}",
                 input.bytes(), nbytes);
  auto remaining_current_bytes = std::min(nbytes, input.bytes());
  auto remaining_total_bytes = input.bytes();
  ARCTICDB_DEBUG(log::version(), "Remaining total: {} Remaining current: {}",
                 remaining_total_bytes, remaining_current_bytes);
  std::optional<ChunkedBufferImpl<BlockSize>> current_buf =
      ChunkedBufferImpl<BlockSize>::presized_in_blocks(
          std::min(nbytes, remaining_current_bytes));
  ...
  for (const auto block : input.blocks()) {
    ...
    while (source_bytes != 0) {
      if (!current_buf) {
        ...
        current_buf =
            ChunkedBufferImpl<BlockSize>::presized_in_blocks(remaining_current_bytes);
        ARCTICDB_DEBUG(log::version(), "Creating new buffer with size {}",
      ...
      const auto this_write =
          std::min({remaining_current_bytes, source_bytes, remaining_block_bytes});
      ARCTICDB_DEBUG(log::version(), "Calculated this write = {} ({}, {}, {})",
                     this_write, remaining_current_bytes, source_bytes,
                     remaining_block_bytes);
      util::check(target_block != current_buf->blocks().end(),
                  "Went past end of blocks");
      ARCTICDB_DEBUG(log::version(), "Copying {} bytes from pos {} to pos {}",
                     this_write, source_pos, target_pos);
      ...
      ARCTICDB_DEBUG(log::version(),
                     "Adjusted values source_pos {} source_bytes {} target_pos {} "
                     "remaining_current {} remaining_total {}",
                     source_pos, source_bytes, target_pos, remaining_current_bytes,
                     remaining_total_bytes);

      if (static_cast<size_t>((*target_block)->bytes()) == nbytes ||
          target_pos == static_cast<size_t>((*target_block)->bytes())) {
        ARCTICDB_DEBUG(log::version(),
                       "Incrementing block as nbytes == target block bytes: {}",
                       nbytes);
        ++target_block;
        target_pos = 0;
      }
  ...
  util::check(output.size() == output_size,
              "Unexpected size in chunked buffer split {} != {}", output.size(),
              output_size);
  return output;
}

template std::vector<ChunkedBufferImpl<64>> split(const ChunkedBufferImpl<64>& input,
                                                  size_t nbytes);
template std::vector<ChunkedBufferImpl<3968>>
split(const ChunkedBufferImpl<3968>& input, size_t nbytes);
...
ChunkedBufferImpl<BlockSize> truncate(const ChunkedBufferImpl<BlockSize>& input,
  ...
  ARCTICDB_DEBUG(log::version(), "Truncating buffer of size {} between bytes {} and {}",
                 input.bytes(), start_byte, end_byte);
  const auto output_size = start_byte >= end_byte ? 0 : end_byte - start_byte;
  if (input.num_blocks() == 0 || output_size == 0)
    return {};

  // This is trivially extendable to use presized_in_blocks, but there is no use case
  // for this right now, and copy_frame_data_to_buffer expects a contiguous buffer
  ...
  for (auto idx = start_idx; idx < end_idx; idx++) {
    ...
    while (source_bytes != 0) {
      ...
      ARCTICDB_DEBUG(log::version(), "Calculated this write = {} ({}, {})", this_write,
...
template ChunkedBufferImpl<64> truncate(const ChunkedBufferImpl<64>& input,
                                        size_t start_byte, size_t end_byte);
template ChunkedBufferImpl<3968> truncate(const ChunkedBufferImpl<3968>& input,
                                          size_t start_byte, size_t end_byte);
