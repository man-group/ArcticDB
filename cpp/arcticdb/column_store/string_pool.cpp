...
ColumnData StringPool::column_data() const {
  return {&block_.buffer(), &shapes_.buffer(), string_pool_descriptor().type(),
          nullptr};
}
...
py::buffer_info StringPool::as_buffer_info() const {
  return py::buffer_info{(void*)block_.at(0).data(), 1,
                         py::format_descriptor<char>::format(),
                         ssize_t(block_.at(0).size())};
}

std::optional<position_t> StringPool::get_offset_for_column(std::string_view string,
                                                            const Column& column) {
