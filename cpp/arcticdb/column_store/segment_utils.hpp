...
namespace arcticdb {
ankerl::unordered_dense::set<entity::position_t>
unique_values_for_string_column(const Column& column) {
  ankerl::unordered_dense::set<entity::position_t> output_set;
  // Guessing that unique values is a third of the column length
  static auto map_reserve_ratio =
      ConfigsMap::instance()->get_int("UniqueColumns.AllocationRatio", 3);
  ...
  details::visit_type(column.type().data_type(), [&](auto col_desc_tag) {
    ...
    if constexpr (is_sequence_type(type_info::data_type)) {
      Column::for_each<typename type_info::TDT>(
          column, [&output_set](auto value) { output_set.insert(value); });
    } else {
