...
// Not handling regular sized until case, only skips one if statement in
// block_and_offset, cannot avoid branching completely
enum class BlockStructure { SINGLE, REGULAR, IRREGULAR };

template <typename TDT> struct IChunkedBufferRandomAccessor {
  template <class Base> struct Interface : Base {
    typename TDT::DataTypeTag::raw_type at(size_t idx) const {
      return folly::poly_call<0>(*this, idx);
    };
  };

  template <class T> using Members = folly::PolyMembers<&T::at>;
};

template <typename TDT>
using ChunkedBufferRandomAccessor = folly::Poly<IChunkedBufferRandomAccessor<TDT>>;

template <typename TDT> class ChunkedBufferSingleBlockAccessor {
  using RawType = typename TDT::DataTypeTag::raw_type;

public:
  ChunkedBufferSingleBlockAccessor(ColumnData* parent) {
    auto typed_block = parent->next<TDT>();
    // Cache the base pointer of the block, as typed_block->data() has an if statement
...
template <typename TDT> class ChunkedBufferRegularBlocksAccessor {
  using RawType = typename TDT::DataTypeTag::raw_type;

public:
  ChunkedBufferRegularBlocksAccessor(ColumnData* parent) {
    // Cache the base pointers of each block, as typed_block->data() has an if statement
    // for internal vs external
...
template <typename TDT> class ChunkedBufferIrregularBlocksAccessor {
  using RawType = typename TDT::DataTypeTag::raw_type;
...
template <typename TDT> struct IColumnDataRandomAccessor {
  template <class Base> struct Interface : Base {
    typename TDT::DataTypeTag::raw_type at(size_t idx) const {
      return folly::poly_call<0>(*this, idx);
    };
...
public:
  ColumnDataRandomAccessorSparse(ColumnData* parent) : parent_(parent) {
    ...
    if constexpr (block_structure == BlockStructure::SINGLE) {
      ...
      // Irregular
      chunked_buffer_random_accessor_ =
          ChunkedBufferIrregularBlocksAccessor<TDT>(parent);
    }
  }
  ...
  RawType at(size_t idx) const {
    ...
    // This is the same as using rank_corrected, but we always require the idx bit to be
    // true, so do the -1 ourselves for efficiency
