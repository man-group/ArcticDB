...
void swap(SegmentInMemory& left, SegmentInMemory& right) noexcept {
...
void SegmentInMemory::push_back(const Row& row) {
    impl_->push_back(row);
}

const Field& SegmentInMemory::column_descriptor(size_t col) const {
    return impl_->column_descriptor(col);
}

void SegmentInMemory::end_row() const {
    impl_->end_row();
}

bool operator==(const SegmentInMemory& left, const SegmentInMemory& right) {
    return *left.impl_ == *right.impl_;
}


const FieldCollection& SegmentInMemory::fields() const {
    return impl_->fields();
}

const Field& SegmentInMemory::field(size_t index) const {
...
const TimeseriesDescriptor& SegmentInMemory::index_descriptor() const {
    return impl_->index_descriptor();
}

TimeseriesDescriptor& SegmentInMemory::mutable_index_descriptor() {
    return impl_->mutable_index_descriptor();
}

bool SegmentInMemory::has_index_descriptor() const {
    return impl_->has_index_descriptor();
}

void SegmentInMemory::init_column_map() const  {
    impl_->init_column_map();
}

void SegmentInMemory::set_string(position_t pos, std::string_view str) {
...
Column& SegmentInMemory::column_ref(position_t idx) {
    return impl_->column_ref(idx);
}

Column& SegmentInMemory::column(position_t idx) {
    return impl_->column(idx);
}

const Column& SegmentInMemory::column(position_t idx) const {
    return impl_->column(idx);
}

std::vector<std::shared_ptr<Column>>& SegmentInMemory::columns() {
    return impl_->columns();
}

const std::vector<std::shared_ptr<Column>>& SegmentInMemory::columns() const {
    return impl_->columns();
}
...
size_t SegmentInMemory::num_blocks() const {
    return impl_->num_blocks();
}

void SegmentInMemory::append(const SegmentInMemory& other) {
...
void SegmentInMemory::drop_column(std::string_view name) {
...
void SegmentInMemory::reset_timeseries_descriptor() {
    impl_->reset_timeseries_descriptor();
}

void SegmentInMemory::calculate_statistics() {
...
void SegmentInMemory::unsparsify() {
    impl_->unsparsify();
}

bool SegmentInMemory::has_user_metadata() const {
...
void SegmentInMemory::set_offset(ssize_t offset) {
    impl_->set_offset(offset);
}

ssize_t SegmentInMemory::offset() const {
    return impl_->offset();
}

void SegmentInMemory::clear() {
    impl_->clear();
}

void SegmentInMemory::end_block_write(ssize_t size) {
...
ColumnData SegmentInMemory::string_pool_data() const {
    return impl_->string_pool_data();
}

ColumnData SegmentInMemory::column_data(size_t col) const {
    return impl_->column_data(col);
}

const StreamDescriptor& SegmentInMemory::descriptor() const {
    return impl_->descriptor();
}

StreamDescriptor& SegmentInMemory::descriptor() {
    return impl_->descriptor();
}

const std::shared_ptr<StreamDescriptor>& SegmentInMemory::descriptor_ptr() const {
...
StringPool& SegmentInMemory::string_pool() {
    return impl_->string_pool();
}

const StringPool& SegmentInMemory::const_string_pool() const {
    return impl_->string_pool();
}

const std::shared_ptr<StringPool>& SegmentInMemory::string_pool_ptr() const {
    return impl_->string_pool_ptr();
}

void SegmentInMemory::reset_metadata() {
    impl_->reset_metadata();
}

void SegmentInMemory::set_metadata(google::protobuf::Any &&meta) {
    impl_->set_metadata(std::move(meta));
}

bool SegmentInMemory::has_metadata() {
    return impl_->has_metadata();
}

void SegmentInMemory::set_row_id(ssize_t rid) {
    impl_->set_row_id(rid);
}

void SegmentInMemory::set_row_data(ssize_t rid) {
    impl_->set_row_data(rid);
}

const google::protobuf::Any* SegmentInMemory::metadata() const {
    return impl_->metadata();
}

bool SegmentInMemory::is_index_sorted() const {
    return impl_->is_index_sorted();
}

void SegmentInMemory::sort(const std::string& column) {
    impl_->sort(column);
}

void SegmentInMemory::sort(const std::vector<std::string>& columns) {
    return impl_->sort(columns);
}

void SegmentInMemory::sort(const std::vector<position_t>& columns) {
    return impl_->sort(columns);
}

void SegmentInMemory::sort(position_t column) {
    impl_->sort(column);
}
...
SegmentInMemory SegmentInMemory::filter(util::BitSet&& filter_bitset, bool filter_down_stringpool, bool validate) const{
...
std::vector<SegmentInMemory> SegmentInMemory::partition(const std::vector<uint8_t>& row_to_segment,
...
bool SegmentInMemory::empty() const {
    return is_null() || impl_->empty();
}

bool SegmentInMemory::is_null() const {
    return !static_cast<bool>(impl_);
}

size_t SegmentInMemory::num_bytes() const {
    return impl_->num_bytes();
}

bool SegmentInMemory::compacted() const  {
    return impl_->compacted();
}

void SegmentInMemory::set_compacted(bool val) {
    impl_->set_compacted(val);
}

void SegmentInMemory::change_schema(const StreamDescriptor& descriptor) {
    return impl_->change_schema(descriptor);
}

SegmentInMemoryImpl* SegmentInMemory::impl() {
    return impl_.get();
}

void SegmentInMemory::check_magic() const {
    impl_->check_magic();
}

// Not currently used but might be useful in the future
void SegmentInMemory::compact_blocks() {
    impl_->compact_blocks();
}

std::shared_ptr<Column> SegmentInMemory::column_ptr(position_t idx) const {
    return impl_->column_ptr(idx);
}

bool SegmentInMemory::allow_sparse() const {
    return impl_->allow_sparse();
}


bool SegmentInMemory::is_sparse() const {
...
void SegmentInMemory::drop_empty_columns() {
