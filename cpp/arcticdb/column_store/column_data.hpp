...
template <typename TDT> struct TypedBlockData {
  template <class ValueType>
  class TypedColumnBlockIterator
      : public boost::iterator_facade<TypedColumnBlockIterator<ValueType>, ValueType,
  ...
  TypedBlockData(const raw_type* data, const shape_t* shapes, size_t nbytes,
                 size_t row_count, const MemBlock* block)
      : data_(data), shapes_(shapes), nbytes_(nbytes), row_count_(row_count),
        block_(block) {}

  TypedBlockData(size_t nbytes, const shape_t* shapes)
      : data_(nullptr), shapes_(shapes), nbytes_(nbytes), row_count_(1u),
        block_(nullptr) {}
...
private:
  ...
  size_t row_count_;
  const MemBlock*
      block_; // pointer to the parent memblock from which this was created from.
};
...
struct ColumnData {
  /*
   * ColumnData is just a thin wrapper that helps in iteration over all the blocks in
   * the column
   */
public:
  template <typename RawType> struct Enumeration {
    ssize_t idx_{0};
  ...
  // Wrapper to keep code in iterator clean when differentiating between regular and
  // enumerating iterators
  template <typename RawType> struct PointerWrapper {
    RawType* ptr_{nullptr};
  };

  template <class T, IteratorType iterator_type>
  using IteratorValueType_t =
      typename std::conditional_t<iterator_type == IteratorType::ENUMERATED,
                                  Enumeration<T>, PointerWrapper<T>>;
  ...
  template <typename TDT, IteratorType iterator_type, IteratorDensity iterator_density,
            bool constant>
  ...
  using base_iterator_type = boost::iterator_facade<
      ...
      IteratorReferenceType_t<typename TDT::DataTypeTag::raw_type, iterator_type,
                              constant>&>;

  template <typename TDT, IteratorType iterator_type, IteratorDensity iterator_density,
            bool constant>
  class ColumnDataIterator
      : public base_iterator_type<TDT, iterator_type, iterator_density, constant> {
    using base_type =
        base_iterator_type<TDT, iterator_type, iterator_density, constant>;
  ...
  public:
    ...
    explicit ColumnDataIterator(ColumnData* parent) : parent_(parent) {
      ...
      if constexpr (iterator_type == IteratorType::ENUMERATED &&
                    iterator_density == IteratorDensity::SPARSE) {
    ...
    explicit ColumnDataIterator(
        const ColumnDataIterator<TDT, iterator_type, iterator_density, OtherConst>&
            other)
        : parent_(other.parent_), opt_block_(other.opt_block_),
  ...
  private:
    ...
    void increment_block() {
      ...
      if (ARCTICDB_LIKELY(opt_block_.has_value())) {
        ...
        data_.ptr_ =
            const_cast<typename TDT::DataTypeTag::raw_type*>(opt_block_->data());
      }
    }

    template <bool OtherConst>
    bool equal(const ColumnDataIterator<TDT, iterator_type, iterator_density,
  ...
  ColumnData(const ChunkedBuffer* data, const Buffer* shapes,
             const TypeDescriptor& type, const util::BitMagic* bit_vector)
      : data_(data), shapes_(shapes), pos_(0), shape_pos_(0), type_(type),
  ...
  ColumnDataIterator<TDT, iterator_type, iterator_density, false> end() {
    ...
    if (!data_->blocks().empty()) {
      ...
      end_ptr =
          const_cast<RawType*>(typed_block_data.data() + typed_block_data.row_count());
    }
    return ColumnDataIterator<TDT, iterator_type, iterator_density, false>(this,
                                                                           end_ptr);
  }
  ...
  template <typename TDT> std::optional<TypedBlockData<TDT>> next() {
  ...
  template <typename TDT> std::optional<TypedBlockData<TDT>> last() {
    if (data_->blocks().empty())
      return std::nullopt;
  ...
  template <typename TDT> static TypedBlockData<TDT> make_typed_block(MemBlock* block) {
    ...
    return TypedBlockData<TDT>{
        ...
        nullptr, block->bytes(),
        block->bytes() / get_type_size(TDT::DataTypeTag::data_type), block};
  }
...
private:
  template <typename TDT> TypedBlockData<TDT> next_typed_block(MemBlock* block) {
    ...
    if constexpr (dim == Dimension::Dim0) {
      ...
      if (shapes_->empty()) {
        ...
        // Blocks can contain empty tensors (empty arrays/matrices). They don't hold any
        // data, however, they have assigned shapes. In case of an empty tensor the
        // corresponding entries in the shapes array must be 0. The rationale is that
        // the shapes array describes how many elements are there in a tensor. This way
        // we can distinguish between [] (empty array) and None (no array at all). We
        // assume that no block, besides the first, can start with empty tensors. This
        // means that a block is exhausted when both are conditions are satisfied: i)
        // The processed size becomes equal to the block size ii) All zero-sized shapes
        // are parsed (i.e. the shape of the current tensor is not 0)
        while (current_tensor_is_empty() || size < block->bytes()) {
          if constexpr (dim == Dimension::Dim1) {
            ...
            util::check(row_count > 0 || (row_count == 0 && column_count == 0),
        ...
        util::check(size == block->bytes(),
                    "Element size vs block size overrun: {} > {}", size,
                    block->bytes());
      }
    }
    ...
    return TypedBlockData<TDT>{
        reinterpret_cast<const typename TDT::DataTypeTag::raw_type*>(block->data()),
        shape_ptr, block->bytes(), num_elements, block};
  }
