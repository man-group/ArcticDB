...
template <>
struct TurboPForBlockCodec<arcticdb::proto::encoding::VariantCodec::TurboPfor::FP_DELTA>
    : TurboPForBase {

  template <class T>
  inline static std::size_t encode_block(const T* in, Block& block, HashAccum& hasher,
                                         T* t_out, std::size_t out_capacity,
                                         std::ptrdiff_t& pos) {
  ...
  template <class T>
  static void decode_block(const std::uint8_t* in, std::size_t in_count, T* t_out,
...
struct ShapeEncoder
    : TurboPForBlockCodec<
          arcticdb::proto::encoding::VariantCodec::TurboPfor::FP_DELTA> {

  using Parent =
      TurboPForBlockCodec<arcticdb::proto::encoding::VariantCodec::TurboPfor::FP_DELTA>;

  template <class T>
  static std::size_t encode_block(const T* in, Block& block, HashAccum& hasher, T* out,
                                  std::size_t out_capacity, std::ptrdiff_t& pos,
                                  arcticdb::proto::encoding::VariantCodec& out_codec) {
    std::size_t compressed_size =
        Parent::encode_block(in, block, hasher, out, out_capacity, pos);

    out_codec.mutable_tp4()->set_sub_codec(
        arcticdb::proto::encoding::VariantCodec::TurboPfor::FP_DELTA);
...
using TurboPForEncoder =
    GenericBlockEncoder<F<TD>, TD, TurboPForBlockEncoder, ShapeEncoder>;
...
struct TurboPForDecoder {
  ...
  static void decode_block(const arcticdb::proto::encoding::Block& block,
    ...
    case arcticdb::proto::encoding::VariantCodec::TurboPfor::FP_DELTA:
      TurboPForBlockCodec<arcticdb::proto::encoding::VariantCodec::TurboPfor::
                              FP_DELTA>::decode_block(in, in_bytes, t_out, out_bytes);
      break;
    default:
