...
[[nodiscard]] inline size_t identifier_bytes(const StreamId& stream_id) {
  return util::variant_match(
      stream_id, [](const NumericId&) { return SegmentIdentifierSize; },
      [](const StringId& str_id) { return SegmentIdentifierSize + str_id.size(); });
}

inline void write_identifier(Buffer& buffer, std::ptrdiff_t& pos,
                             const StreamId& stream_id) {
  auto data = new (buffer.data() + pos) SegmentDescriptorImpl{};
  util::variant_match(
      stream_id,
      [data, &pos](const NumericId& num_id) {
        SegmentIdentifierHeader header{IdentifierType::NUMERIC,
                                       static_cast<uint32_t>(num_id)};
