...
struct ZstdBlockEncoder {
  ...
  static std::size_t encode_block(const Opts& opts, const T* in,
    ...
    std::size_t compressed_bytes =
        ZSTD_compress(out, out_capacity, in, block_utils.bytes_, opts.level());
...
struct ZstdDecoder {
  ...
  /// @param[in] encoder_version Used to support multiple versions but won't be used
  /// before we have them
  template <typename T>
  static void decode_block([[maybe_unused]] std::uint32_t encoder_version,
    ...
    codec::check<ErrorCode::E_DECODE_ERROR>(
        ...
        "expected out_bytes == zstd deduced bytes, actual {} != {}", out_bytes,
        decomp_size);
    std::size_t real_decomp = ZSTD_decompress(t_out, out_bytes, in, in_bytes);
    codec::check<ErrorCode::E_DECODE_ERROR>(
        real_decomp == out_bytes,
        "expected out_bytes == zstd decompressed bytes, actual {} != {}", out_bytes,
        real_decomp);
  }
};
