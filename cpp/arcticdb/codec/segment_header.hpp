...
public:
  ...
  template <HeaderFlag flag> void set_flag(bool value) {
  ...
  template <HeaderFlag flag> [[nodiscard]] bool get_flag() const {
  ...
  [[nodiscard]] size_t bytes() const {
    const auto total_bytes = sizeof(HeaderData) + header_fields_.data_bytes() +
                             header_fields_.offset_bytes() + sizeof(offset_);
    ARCTICDB_DEBUG(log::codec(),
                   "Header bytes HeaderData {} + offset {} + Header field bytes {} + "
                   "header field offset {}  = {}",
                   sizeof(HeaderData), sizeof(offset_), header_fields_.data_bytes(),
                   header_fields_.offset_bytes(), total_bytes);
    return total_bytes;
  }
  ...
  offset_name(FieldOffset field_offset) {
  ...
  [[nodiscard]] const EncodedFieldImpl& get_field() const {
    util::check(has_field(field_offset), "Field {} has not been set",
                offset_name(field_offset));
  ...
  EncodedFieldImpl& create_field(size_t num_blocks) {
    ARCTICDB_TRACE(log::codec(), "Header adding field {} with {} blocks ({} bytes)",
                   offset_names_[as_pos(field_offset)], num_blocks,
    ...
    ARCTICDB_TRACE(log::codec(), "Header size {} ({} offsets)",
                   header_fields_.data_bytes(), header_fields_.offset_bytes());
    return *new_field;
  }
  ...
  size_t serialize_to_bytes(uint8_t* dst,
                            std::optional<size_t> expected_bytes = std::nullopt) const;
...
namespace fmt {
template <> struct formatter<arcticdb::SegmentHeader> {
  ...
  auto format(const arcticdb::SegmentHeader& header, FormatContext& ctx) const {
    ...
    if (header.has_index_descriptor_field())
      fmt::format_to(ctx.out(), "{}: Index: {}\n", header.get_pos(FieldOffset::INDEX),
