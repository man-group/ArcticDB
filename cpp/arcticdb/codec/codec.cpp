...
std::optional<google::protobuf::Any> decode_metadata(
    ...
    if (hdr.has_metadata_field()) {
        ...
        data +=
            decode_ndarray(meta_type_desc, hdr.metadata_field().ndarray(), data, meta_buf, bv, hdr.encoding_version());
...
std::shared_ptr<FieldCollection> decode_index_fields(
    ...
    if (hdr.has_index_descriptor_field() && hdr.index_descriptor_field().has_ndarray()) {
        ...
        data += decode_ndarray(
            FieldCollection::type(), hdr.index_descriptor_field().ndarray(), data, *fields, bv, hdr.encoding_version()
        );
