...
SizeResult
max_compressed_size_dispatch(const SegmentInMemory& in_mem_seg,
...
Segment encode_dispatch(SegmentInMemory&& in_mem_seg,
                        const arcticdb::proto::encoding::VariantCodec& codec_opts,
                        EncodingVersion encoding_version) {
  if (encoding_version == EncodingVersion::V2) {
...
public:
  ...
  shape_t* allocate_shapes(std::size_t bytes) {
    util::check_arg(bytes == 8, "expected exactly one shape, actual {}",
                    bytes / sizeof(shape_t));
    return &shape_;
  }
...
decode_metadata(const SegmentHeader& hdr, const uint8_t*& data,
  ...
  if (hdr.has_metadata_field()) {
    ...
    ARCTICDB_DEBUG(log::codec(), "Decoding metadata at position {}: {}", data - begin,
                   dump_bytes(data, 10));
    data += decode_ndarray(meta_type_desc, hdr.metadata_field().ndarray(), data,
                           meta_buf, bv, hdr.encoding_version());
...
void decode_metadata(const SegmentHeader& hdr, const uint8_t*& data,
                     const uint8_t* begin ARCTICDB_UNUSED, SegmentInMemory& res) {
...
std::optional<google::protobuf::Any>
decode_metadata_from_segment(const Segment& segment) {
...
EncodedFieldCollection decode_encoded_fields(const SegmentHeader& hdr,
                                             const uint8_t* data,
                                             const uint8_t* begin ARCTICDB_UNUSED) {
  ARCTICDB_TRACE(log::codec(), "Decoding encoded fields");

  util::check(hdr.has_column_fields() && hdr.column_fields().has_ndarray(),
  ...
  decode_ndarray(type_desc, hdr.column_fields().ndarray(), data, encoded_column, bv,
                 hdr.encoding_version());

  ARCTICDB_TRACE(log::codec(), "Decoded encoded fields at position {}", data - begin);
  return {std::move(encoded_column.release_buffer()),
          std::move(encoded_column.release_shapes())};
}

std::shared_ptr<arcticdb::proto::descriptors::FrameMetadata>
extract_frame_metadata(SegmentInMemory& res) {
...
std::shared_ptr<FieldCollection>
decode_index_fields(const SegmentHeader& hdr, const uint8_t*& data,
  ...
  if (hdr.has_index_descriptor_field() && hdr.index_descriptor_field().has_ndarray()) {
    ...
    util::check(data != end,
                "Reached end of input block with index descriptor fields to decode");
    std::optional<util::BitMagic> bv;

    data +=
        decode_ndarray(FieldCollection::type(), hdr.index_descriptor_field().ndarray(),
                       data, *fields, bv, hdr.encoding_version());

    ARCTICDB_TRACE(log::codec(), "Decoded index descriptor to position {}",
                   data - begin);
  }
...
namespace {
inline arcticdb::proto::descriptors::TimeSeriesDescriptor
timeseries_descriptor_from_any(const google::protobuf::Any& any) {
  arcticdb::proto::descriptors::TimeSeriesDescriptor tsd;
  any.UnpackTo(&tsd);
  return tsd;
}

inline arcticdb::proto::descriptors::FrameMetadata
frame_metadata_from_any(const google::protobuf::Any& any) {
...
decode_descriptor_fields(const SegmentHeader& hdr, const uint8_t*& data,
                         const uint8_t* begin ARCTICDB_UNUSED, const uint8_t* end) {
  if (hdr.has_descriptor_field()) {
    ARCTICDB_TRACE(log::codec(), "Decoding index fields");
    util::check(data != end,
                "Reached end of input block with descriptor fields to decode");
    std::optional<util::BitMagic> bv;
    FieldCollection fields;
    data += decode_field(FieldCollection::type(), hdr.descriptor_field(), data, fields,
                         bv, hdr.encoding_version());

    ARCTICDB_TRACE(log::codec(), "Decoded descriptor fields to position {}",
                   data - begin);
...
unpack_timeseries_descriptor_from_proto(const google::protobuf::Any& any,
  ...
  auto segment_desc = std::make_shared<SegmentDescriptorImpl>(
      segment_descriptor_from_proto((tsd.stream_descriptor())));
  auto frame_desc =
      std::make_shared<FrameDescriptorImpl>(frame_descriptor_from_proto(tsd));
  const auto& desc = tsd.stream_descriptor();
  auto old_fields =
      std::make_shared<FieldCollection>(fields_from_proto(tsd.stream_descriptor()));
  StreamId stream_id =
      desc.id_case() == desc.kNumId ? StreamId(desc.num_id()) : StreamId(desc.str_id());
  return {frame_desc, segment_desc, frame_meta, old_fields, stream_id};
}

std::optional<TimeseriesDescriptor>
decode_timeseries_descriptor_v1(const SegmentHeader& hdr, const uint8_t* data,
                                const uint8_t* begin,
...
std::optional<TimeseriesDescriptor>
decode_timeseries_descriptor_v2(const SegmentHeader& hdr, const uint8_t* data,
  ...
  auto frame_meta = std::make_shared<arcticdb::proto::descriptors::FrameMetadata>(
      frame_metadata_from_any(*maybe_any));
  ...
  auto segment_desc =
      std::make_shared<SegmentDescriptorImpl>(read_segment_descriptor(data));
  auto segment_id = read_identifier(data);
  auto index_fields = decode_index_fields(hdr, data, begin, end);
  return std::make_optional<TimeseriesDescriptor>(frame_desc, segment_desc, frame_meta,
                                                  std::move(index_fields), segment_id);
}

std::optional<TimeseriesDescriptor>
decode_timeseries_descriptor(const SegmentHeader& hdr, const uint8_t* data,
                             const uint8_t* begin, const uint8_t* end,
                             const StreamDescriptor& descriptor) {
...
std::optional<TimeseriesDescriptor> decode_timeseries_descriptor_for_incompletes(
    const SegmentHeader& hdr, const StreamDescriptor& desc, const uint8_t* data,
    const uint8_t* begin, const uint8_t* end) {
  util::check(data != nullptr, "Got null data ptr from segment");
  auto encoding_version = EncodingVersion(hdr.encoding_version());
  if (encoding_version == EncodingVersion::V1) {
...
std::optional<TimeseriesDescriptor>
decode_timeseries_descriptor_for_incompletes(Segment& segment) {
  auto& hdr = segment.header();
  const uint8_t* data = segment.buffer().data();
  ...
  return decode_timeseries_descriptor_for_incompletes(hdr, segment.descriptor(), data,
                                                      begin, end);
}

std::pair<std::optional<google::protobuf::Any>, StreamDescriptor>
decode_metadata_and_descriptor_fields(Segment& segment) {
  auto& hdr = segment.header();
  const uint8_t* data = segment.buffer().data();
...
void decode_string_pool(const SegmentHeader& hdr, const uint8_t*& data,
  ...
  if (hdr.has_string_pool_field()) {
    ...
    std::optional<util::BitMagic> bv;
    data += decode_ndarray(string_pool_descriptor().type(), hdr.string_pool_field(),
                           data, res.string_pool(), bv, hdr.encoding_version());
...
void decode_v2(const Segment& segment, const SegmentHeader& hdr, SegmentInMemory& res,
  ...
  if (hdr.has_index_descriptor_field()) {
    auto index_frame_descriptor =
        std::make_shared<FrameDescriptorImpl>(read_frame_descriptor(data));
    auto frame_metadata = extract_frame_metadata(res);
    auto index_segment_descriptor =
        std::make_shared<SegmentDescriptorImpl>(read_segment_descriptor(data));
    auto index_segment_identifier = read_identifier(data);
    auto index_fields = decode_index_fields(hdr, data, begin, end);
    TimeseriesDescriptor tsd{
        std::move(index_frame_descriptor), std::move(index_segment_descriptor),
        std::move(frame_metadata), std::move(index_fields), index_segment_identifier};
  ...
  if (data != end) {
    ...
    for (std::size_t i = 0; i < fields_size; ++i) {
      ...
      log::version().debug(
          "{}", dump_bytes(begin,
                           (data - begin) +
                               encoding_sizes::field_compressed_size(*encoded_field),
                           100u));
#endif
      const auto& field_name = desc.fields(i).name();
      util::check(data != end, "Reached end of input block with {} fields to decode",
                  fields_size - i);
      if (auto col_index = res.column_index(field_name)) {
        auto& col = res.column(static_cast<position_t>(*col_index));

        data += decode_field(res.field(*col_index).type(), *encoded_field, data, col,
                             col.opt_sparse_map(), hdr.encoding_version());
        seg_row_count = std::max(seg_row_count, calculate_last_row(col));
      } else {
        data +=
            encoding_sizes::field_compressed_size(*encoded_field) + sizeof(ColumnMagic);
      }
      ++encoded_field;
...
void decode_v1(const Segment& segment, const SegmentHeader& hdr, SegmentInMemory& res,
               const StreamDescriptor& desc, bool is_decoding_incompletes) {
  ARCTICDB_SAMPLE(DecodeSegment, 0)
  const uint8_t* data = segment.buffer().data();
  if (data == nullptr) {
  ...
  decode_metadata(hdr, data, begin, res);
  if (res.has_metadata() &&
      res.metadata()->Is<arcticdb::proto::descriptors::TimeSeriesDescriptor>()) {
    ARCTICDB_DEBUG(log::version(), "Unpacking timeseries descriptor from metadata");
    auto tsd = unpack_timeseries_descriptor_from_proto(*res.metadata(), desc,
                                                       is_decoding_incompletes);
    res.set_timeseries_descriptor(tsd);
    res.reset_metadata();
  }
  ...
  if (data != end) {
    ...
    util::check(fields_size == segment.fields_size(),
                ...
                fields_size, column_fields.size());
    const auto start_row = res.row_count();
    ...
    for (std::size_t i = 0; i < fields_size; ++i) {
      ...
      util::check(data != end || is_empty_type(desc_field.type().data_type()),
                  "Reached end of input block with {} fields to decode",
                  fields_size - i);
      if (auto col_index = res.column_index(field_name)) {
        auto& col = res.column(static_cast<position_t>(*col_index));
        data += decode_field(res.field(*col_index).type(), field, data, col,
                             col.opt_sparse_map(), hdr.encoding_version());
        seg_row_count = std::max(seg_row_count, calculate_last_row(col));
        ARCTICDB_DEBUG(log::codec(), "Decoded column {} to position {}", i,
                       data - begin);
      } else {
        data += encoding_sizes::field_compressed_size(field);
        ARCTICDB_DEBUG(log::codec(), "Skipped column {}, at position {}", i,
                       data - begin);
      }
    }
    decode_string_pool(hdr, data, begin, end, res);
...
void decode_into_memory_segment(const Segment& segment, SegmentHeader& hdr,
                                SegmentInMemory& res, const StreamDescriptor& desc) {
...
/// Bitmagic achieves the theoretical best compression for booleans. Adding additional
/// encoding (lz4, zstd, etc...) will not improve anything and in fact it might worsen
/// the encoding.
[[nodiscard]] static size_t encode_bitmap(const util::BitMagic& sparse_map, Buffer& out,
                                          std::ptrdiff_t& pos) {
  ARCTICDB_DEBUG(log::version(), "Encoding sparse map of count: {}",
                 sparse_map.count());
...
void encode_sparse_map(ColumnData& column_data, EncodedFieldImpl& field, Buffer& out,
                       std::ptrdiff_t& pos) {
  if (column_data.bit_vector() != nullptr && column_data.bit_vector()->count() > 0) {
    util::check(!is_empty_type(column_data.type().data_type()),
                "Empty typed columns should not have sparse maps");
    ARCTICDB_DEBUG(log::codec(), "Sparse map count = {} pos = {}",
                   column_data.bit_vector()->count(), pos);
