...
public:
...
#endif
            debug_assert(finalin <= len + in);
            in = __decodeArray(in, finalin - in, out, thisnvalue);
            debug_assert(in > befin);
            debug_assert(in <= finalin);
            out += thisnvalue;
            totalnvalue += thisnvalue;
            debug_assert(totalnvalue <= nvalue);
        }
        debug_assert(in <= len + initin);
        debug_assert(in <= finalin);
        nvalue = totalnvalue;
        return in;
    }
    ...
    const uint32_t *__decodeArray(const uint32_t *in, const size_t len,
        ...
        debug_assert(initin + len >= in);
        debug_assert(initin + len >= endexceptpointer);
