...
struct DynamicRangeRandomAccessAdaptor {
    ColumnData column_data_;
    std::vector<T> buffer_;  // dynamically allocated, so we can resize as needed
    ...
    // Convert a count of T elements to bytes.
    [[nodiscard]] constexpr size_t t_size(size_t count) const {
        return count * sizeof(T);
    }

    // Convert bytes to a count of T elements.
    ...
    const T* at(size_t row, size_t count) {
        ...
        // If the current block has the full range already contiguously,
        // return a pointer directly into that block:
        if (block->bytes() >= block_pos + t_size(count))
            return reinterpret_cast<const T*>(block->data() + block_pos);

        // Otherwise, copy data from one or more blocks into our internal buffer.
        size_t dest_offset = 0; // destination offset in terms of T elements.
        while (required > 0) {
            // If the current block has some data left at our source position…
            if (block->bytes() > block_pos) {
                // Available bytes in the current block.
                size_t bytes_available = block->bytes() - block_pos;
                // Figure out how many T elements we can copy in this iteration.
                size_t current_elements_to_copy = std::min(required, rows(bytes_available));
                // Determine how many bytes that is.
                ...
                // No data left in the block; move on to the next block.
                ++block_num;
                util::check(block_num < column_data_.buffer().num_blocks(),
                            "Ran out of blocks in contiguous range adaptor");
                block = column_data_.buffer().blocks()[block_num];
                block_pos = 0;
            }
        }
