...
struct TypedBlockEncoderImpl {
   ...
   * @param[in] codec_opts Option used to dispatch to the appropriate encoder and
   * configure it
   * @param[in] typed_block The block to be encoded
   * @param[in, out] field description of the encoding operation
   * @param[out] out output buffer to write the encoded values to. Must be resized if
   * pos becomes > size
  ...
  static void encode(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                     const TypedBlock<TD>& typed_block, EncodedFieldType& field,
                     Buffer& out, std::ptrdiff_t& pos) {
    static_assert(encoder_version == EncodingVersion::V1,
                  "Encoding of both shapes and values at the same time is allowed only "
                  "in V1 encoding");
    visit_encoder(codec_opts, [&](auto encoder_tag) {
      decltype(encoder_tag)::Encoder::encode(get_opts(codec_opts, encoder_tag),
                                             typed_block, field, out, pos);
    });
  }

  template <class TypedBlockType, class NDArrayType>
  static void
  encode_to_values(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                   const TypedBlockType& typed_block, Buffer& out, std::ptrdiff_t& pos,
                   NDArrayType& ndarray) {
  ...
  template <typename EncodedFieldType>
  static void encode_values(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                            const TypedBlock<TD>& typed_block, EncodedFieldType& field,
                            Buffer& out, std::ptrdiff_t& pos) {
    static_assert(
    ...
    encode_to_values<TypedBlock<TD>, decltype(ndarray)>(codec_opts, typed_block, out,
                                                        pos, ndarray);
  ...
  template <typename EncodedFieldType>
  static void encode_shapes(const arcticdb::proto::encoding::VariantCodec& codec_opts,
...
private:
  ...
  using PassthroughEncoder =
      std::conditional_t<encoder_version == EncodingVersion::V1,
                         arcticdb::detail::PassthroughEncoderV1<TypedBlock, TD>,
                         arcticdb::detail::PassthroughEncoderV2<TypedBlock, TD>>;

  template <typename EncoderT> struct EncoderTag {
    using Encoder = EncoderT;
  };

  template <typename FunctorT>
  static auto visit_encoder(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                            FunctorT&& f) {
  ...
  static auto get_opts(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                       EncoderTag<Lz4Encoder>) {
    return codec_opts.lz4();
  }

  static auto get_opts(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                       EncoderTag<ZstdEncoder>) {
    return codec_opts.zstd();
  }

  static auto get_opts(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                       EncoderTag<PassthroughEncoder>) {
