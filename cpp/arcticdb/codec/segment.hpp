...
compressed(const arcticdb::proto::encoding::SegmentHeader& seg_hdr);
} // namespace segment_size
...
inline EncodingVersion
encoding_version(const storage::LibraryDescriptor::VariantStoreConfig& cfg) {
  return util::variant_match(
      ...
      [](std::monostate) { return EncodingVersion::V1; });
}
void set_body_fields(SegmentHeader& seg_hdr, const uint8_t* src);

/*
 * Segment contains compressed data as returned from storage. When reading data the next
 * step will usually be to decompress the Segment into a SegmentInMemory which allows
 * for data access and modification. At the time of writing, the primary method for
 * decompressing a Segment into a SegmentInMemory is in codec.cpp::decode (and the
 * reverse via codec.cpp:encode).
 */
class Segment {
public:
  Segment() = default;

  Segment(SegmentHeader&& header, std::shared_ptr<Buffer> buffer,
          std::shared_ptr<SegmentDescriptorImpl> data,
          std::shared_ptr<FieldCollection> fields, StreamId stream_id, size_t size)
      : header_(std::move(header)), buffer_(std::move(buffer)),
        desc_(std::move(data), std::move(fields), std::move(stream_id)), size_(size) {}

  Segment(SegmentHeader&& header, BufferView buffer,
          std::shared_ptr<SegmentDescriptorImpl> data,
          std::shared_ptr<FieldCollection> fields, StreamId stream_id, size_t size)
      : header_(std::move(header)), buffer_(buffer),
        desc_(std::move(data), std::move(fields), std::move(stream_id)), size_(size) {}

  Segment(SegmentHeader&& header, VariantBuffer&& buffer,
          std::shared_ptr<SegmentDescriptorImpl> data,
          std::shared_ptr<FieldCollection> fields, StreamId stream_id, size_t size)
      : header_(std::move(header)), buffer_(std::move(buffer)),
        desc_(std::move(data), std::move(fields), std::move(stream_id)), size_(size) {}
  ...
  void set_buffer(VariantBuffer&& buffer) { buffer_ = std::move(buffer); }

  static Segment from_bytes(const std::uint8_t* src, std::size_t readable_size,
  ...
  [[nodiscard]] BufferView buffer() const { return buffer_.view(); }
  ...
  void set_keepalive(std::any&& keepalive) { keepalive_ = std::move(keepalive); }

  [[nodiscard]] const std::any& keepalive() const { return keepalive_; }

  [[nodiscard]] const StreamDescriptor& descriptor() const { return desc_; }

  Segment clone() const {
    return Segment{header_.clone(), buffer_.clone(), desc_.clone(), size_};
  }

  static Segment initialize(SegmentHeader&& header, std::shared_ptr<Buffer>&& buffer,
                            std::shared_ptr<SegmentDescriptorImpl> data,
                            std::shared_ptr<FieldCollection> fields,
                            StreamId stream_id) {
    return {std::move(header), std::move(buffer), std::move(data), std::move(fields),
            std::move(stream_id)};
  }
...
private:
  ...
  Segment(SegmentHeader&& header, VariantBuffer&& buffer, StreamDescriptor&& desc,
      ...
      : header_(std::move(header)), buffer_(std::move(buffer)), desc_(std::move(desc)),
        size_(size) {}

  std::tuple<uint8_t*, size_t, std::unique_ptr<Buffer>>
  serialize_v1_header_to_buffer(size_t total_hdr_size);
...
namespace fmt {
template <> struct formatter<arcticdb::EncodingVersion> {
  ...
  auto format(arcticdb::EncodingVersion version, FormatContext& ctx) const {
    char c = 'U';
    switch (version) {
    case arcticdb::EncodingVersion::V1:
      c = '1';
      break;
    case arcticdb::EncodingVersion::V2:
      c = '2';
      break;
    default:
      break;
