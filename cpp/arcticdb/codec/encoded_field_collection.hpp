...
public:
  struct EncodedFieldCollectionIterator {
    ...
    [[nodiscard]] EncodedFieldImpl& current() const {
      return *reinterpret_cast<EncodedFieldImpl*>(
          buffer_->ptr_cast<uint8_t>(pos_, EncodedFieldImpl::Size));
    }
  ...
  [[nodiscard]] uint64_t get_offset(size_t pos) const {
    const auto offset =
        *offsets_.ptr_cast<uint64_t>(pos * sizeof(uint64_t), sizeof(uint64_t));
    return offset;
  }
  ...
  [[nodiscard]] EncodedFieldImpl& to_field(size_t bytes_pos) {
    return *reinterpret_cast<EncodedFieldImpl*>(
        data_.ptr_cast<uint8_t>(bytes_pos, EncodedFieldImpl::Size));
  }
  ...
  void regenerate_offsets() {
    ...
    util::check(pos == data_.bytes(), "Size mismatch in regenerate_offsets, {} != {}",
                pos, data_.bytes());
  }
  ...
  [[nodiscard]] EncodedFieldImpl* add_field(size_t num_blocks) {
    ...
    util::check(required_bytes >= EncodedFieldImpl::Size,
                "Unexpectedly small allocation size: {}", required_bytes);
    data_.ensure(offset_ + required_bytes);
    auto* field =
        new (data_.ptr_cast<uint8_t>(offset_, required_bytes)) EncodedFieldImpl;
    ARCTICDB_DEBUG(
        log::codec(),
        "Adding encoded field with {} blocks at position {}, {} bytes required",
        num_blocks, offset_, required_bytes);
    ++count_;
