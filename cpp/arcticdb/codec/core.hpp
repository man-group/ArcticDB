...
struct NdArrayBlock {
  ...
  void update_field_size(EncodedFieldType& field) const {
    ...
    field.set_items_count(existing_items_count +
                          static_cast<std::uint32_t>(item_count_));
  }

  template <typename BlockType>
  void set_block_data(BlockType* shapes_pb, BlockType* values_pb,
                      HashedValue shape_hash, std::size_t encoded_shape_bytes,
                      HashedValue values_hash, std::size_t encoded_values_bytes) {
    ARCTICDB_TRACE(log::codec(), "Setting encoded bytes: {}:{}", encoded_shape_bytes,
                   encoded_values_bytes);
  ...
  template <typename BlockType>
  void set_version(BlockType* shapes_pb, BlockType* values_pb, std::uint32_t version,
...
/**
 * Helper meant to mutualise boiler plate code that is going to exist regardless of the
 * encoding strategy
 * @tparam F field of dimension and type specified below
 * @tparam D Dimension tag class
 * @tparam DT DataType tag class
 */
template <class TD> class CodecHelper {
public:
  ...
  static NdArrayBlock nd_array_block(std::size_t row_count, const shape_t* shape) {
    ...
    return NdArrayBlock{
        row_count, {shape_count, shape_bytes}, {total_values_count, data_bytes}};
  }
};

/**
 * Turn a block encoding policy into a shape encoding one. This is essentially a
 * convenience type mapper. It is more optimal to implement a shape specific encoder to
 * avoid default option instantiation.
 * @tparam BE BlockEncoding see GenericBlockEncoder for more details
 * @tparam DefaultOpt {
 *  static void set_shape_default(Opts & opts);
 * }
 */
template <class BE, class DefaultOpt> struct ShapeEncodingFromBlock {
  ...
  static std::size_t encode_block(const T* in, BlockDataHelper& block_utils,
                                  HashAccum& hasher, T* out, std::size_t out_capacity,
                                  std::ptrdiff_t& pos, EncodedFieldType& out_codec) {
    typename BE::Opts opts;
    DefaultOpt::set_shape_defaults(opts);
    return BE::encode_block(opts, in, block_utils, hasher, out, out_capacity, pos,
                            out_codec);
  }
};

/**
 * This class is meant to simplify writing block based compression codecs by focusing on
 * the block encoding logic only. This might be suboptimal for codecs able to find
 * patterns in dim > 0 but removes a lot of boiler plate
 * @tparam F Field type (provider of actual data)
 * @tparam TD TypeDescriptorTag used to statically know the type and the dimension of
 * the inputs
 ...
 */
template <class BlockType, class TD, class EncoderType> struct GenericBlockEncoder {
  ...
  static size_t max_compressed_size(const BlockType& block) {
    if (block.nbytes() == 0) {
      ARCTICDB_TRACE(log::codec(),
                     "GenericBlockEncoder got empty block. Max compressed size is 0.");
      return 0;
    }
    ...
    if constexpr (Helper::dim == Dimension::Dim0) {
      ...
      std::size_t comp_data =
          EncoderType::max_compressed_size(helper_array_block.values_.bytes_);
      std::size_t comp_shapes =
          ShapeEncoding::max_compressed_size(helper_array_block.shapes_.bytes_);

      ARCTICDB_TRACE(log::codec(), "Array block has {} bytes ({} + {})",
                     comp_data + comp_shapes, comp_shapes, comp_data);
      return comp_data + comp_shapes;
    }
  }

  static void encode(const typename EncoderType::Opts& opts, const BlockType& block,
    ...
    if (block.nbytes() == 0) {
      ARCTICDB_TRACE(log::codec(),
                     "GenericBlockEncoder got empty block. There's nothing to encode");
      return;
    }
    ...
    if constexpr (Helper::dim == Dimension::Dim0) {
      // Only store data, no shapes since dimension is 0
      auto helper_scalar_block = Helper::scalar_block(block_row_count);
      ARCTICDB_TRACE(log::codec(), "Generic block encode writing scalar of {} elements",
                     block_row_count);

      const auto uncompressed_size = helper_scalar_block.bytes_;
      std::size_t max_compressed_size =
          EncoderType::max_compressed_size(uncompressed_size);
      helper.ensure_buffer(out, pos, max_compressed_size);

      // doing copy + hash in one pass, this might have a negative effect on perf
      // since the hashing is path dependent. This is a toy example though so not
      // critical
      ...
      const auto compressed_size = EncoderType::encode_block(
          opts, block.data(), helper_scalar_block, helper.hasher_, t_out,
          max_compressed_size, pos, *value_pb->mutable_codec());

      helper_scalar_block.set_block_data(*value_pb, helper.hasher_.digest(),
                                         compressed_size);
      ...
      const std::size_t max_compressed_shapes_size =
          ShapeEncoding::max_compressed_size(helper_array_block.shapes_.bytes_);
      const std::size_t helper_buffer_size =
          max_compressed_data_size + max_compressed_shapes_size;
      ...
      const auto shape_comp_size = ShapeEncoding::encode_block(
          block.shapes(), helper_array_block.shapes_, helper.hasher_, s_out,
          max_compressed_shapes_size, pos, *shape_pb->mutable_codec());
      ...
      const auto values_comp_size = EncoderType::encode_block(
          opts, block.data(), helper_array_block.values_, helper.hasher_, t_out,
          max_compressed_data_size, pos, *value_pb->mutable_codec());

      auto digest = helper.hasher_.digest();
      helper_array_block.update_field_size(*field_nd_array);
      helper_array_block.set_block_data(shape_pb, value_pb, shape_hash, shape_comp_size,
                                        digest, values_comp_size);
      helper_array_block.set_version(shape_pb, value_pb, EncoderType::VERSION,
...
/// Just as arcticdb::detail::struct GenericBlockEncoder this class does not care about
/// the specific content of the block and does not take advantage of specific patterns
/// of data. The main difference between this and arcticdb::detail::struct
/// GenericBlockEncoder is that the latter stores the shapes data at the beginning of
/// each block, which is suboptimal. arcticdb::detail::GenericBlockEncoder2 does not
/// care about dimensionality and does not encode the shapes of the block. For more
/// information see comment above arcticdb::ColumnEncoder2
template <class BlockType, class TD, class EncoderType> struct GenericBlockEncoderV2 {
public:
  using Helper = CodecHelper<TD>;
  using T = typename Helper::T;
  ...
  static size_t max_compressed_size(const BlockType& block) {
    ...
    if (uncompressed_size == 0) {
      ARCTICDB_TRACE(
          log::codec(),
          "GenericBlockEncoderV2 got empty block. Max compressed size is 0.");
      return 0;
    }
  ...
  static void encode(const typename EncoderType::Opts& opts, const BlockType& block,
    ...
    if (block.nbytes() == 0) {
      ARCTICDB_TRACE(
          log::codec(),
          "GenericBlockEncoderV2 got empty block. There's nothing to encode.");
      return;
    }

    Helper helper;
    helper.hasher_.reset(helper.seed);
    auto helper_scalar_block =
        BlockDataHelper{block.nbytes() / sizeof(T), block.nbytes()};
    ARCTICDB_TRACE(log::codec(), "Generic block encode writing scalar of {} elements",
                   block.row_count());
    ...
    const auto compressed_size = EncoderType::encode_block(
        opts, block.data(), helper_scalar_block, helper.hasher_, t_out,
        max_compressed_size, pos, *encoded_block->mutable_codec());
    helper_scalar_block.set_block_data(*encoded_block, helper.hasher_.digest(),
                                       compressed_size);
