...
struct CompressIdentity{
    T operator()(const T* t, size_t index, size_t ) { return t[index]; }
};

template <typename T>
struct UncompressIdentity{
    void operator()(T* t, size_t index, T value, size_t) { t[index] = value; }
};
...
TEST(BitPackFusedStress, Roundtrip) {
    using T = uint32_t;
    ...
    constexpr size_t compressed_block_size = BitPackFused<T, bit_width>::compressed_size();

    std::vector<T> packed(num_blocks * compressed_block_size, 0);
    std::vector<T> unpacked(num_blocks * BLOCK_SIZE, 0);

    auto pack_kernel = [](const T* ptr, size_t offset, size_t /*lane*/) -> T {
        return *(ptr + offset);
    };
    auto unpack_kernel = [](T* ptr, size_t offset, T, size_t /*lane*/) -> T {
        return *(ptr + offset);
    };
    ...
    for (size_t iter = 0; iter < num_iterations; iter++) {
        for (size_t b = 0; b < num_blocks; b++) {
            ...
            size_t comp_size = BitPackFused<T, bit_width>::go(in_ptr, out_ptr, pack_kernel);
            checksum += comp_size;
        }
    }
    ...
    for (size_t iter = 0; iter < num_iterations; iter++) {
        for (size_t b = 0; b < num_blocks; b++) {
            ...
            compressed += BitUnpackFused<T, bit_width>::go(in_ptr, out_ptr, unpack_kernel);
            checksum += out_ptr[0];
        }
    }
    ...
    std::cout << "Unpacking average time per block: " << unpack_avg_time << " microseconds" << std::endl;
