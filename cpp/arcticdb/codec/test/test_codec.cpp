...
TYPED_TEST(FieldEncoderTestFromColumnDim0, Passthrough) {
    ...
    column_data.reset();
    ColumnEncoder::encode(TestFixture::passthrough_encoding_options,
                          column_data,
                          field,
                          out,
                          pos,
...
TEST_F(TestMultiblockData_Dim1, EncodingVersion_2) {
    ...
    ptrdiff_t out_pos = 0;
    column_data.reset();
    ColumnEncoderV2::encode(passthrough_encoding_options, column_data, *encoded_field, out, out_pos);
