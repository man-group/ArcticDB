...
TEST(BitPack, FastlanesBinaryCompat64to11) {
    using namespace arcticdb;
    auto *base64 = new uint64_t[1]();
    *base64 = 0;
    std::vector<uint64_t> local_packed64(1024);

    std::vector<uint64_t> fl_packed64(1024);
    *(base64) = 0;
    generated::pack::fallback::scalar::pack(rand_arr_70_b11_w64_arr, const_cast<uint64_t*>(fl_packed64.data()), 11);
    bitpack<uint64_t, 11>(rand_arr_70_b11_w64_arr, local_packed64.data());
    for(auto i = 0U; i < 1024; ++i)
        ASSERT_EQ(local_packed64[i], fl_packed64[i]);
}
...
TEST(BitPack, FastlanesBinaryCompat8to3) {
    using namespace arcticdb;
    auto *base64 = new uint8_t[1]();
    *base64 = 0;
    std::vector<uint8_t> local_packed8(1024);

    std::vector<uint8_t> fl_packed8(1024);
    *(base64) = 0;
    generated::pack::fallback::scalar::pack(rand_arr_3_b3_w8_arr, const_cast<uint8_t*>(fl_packed8.data()), 3);
    bitpack<uint8_t, 3>(rand_arr_3_b3_w8_arr, local_packed8.data());
    for(auto i = 0U; i < 1024; ++i) {
        ASSERT_EQ(local_packed8[i], fl_packed8[i]);
    }
}
