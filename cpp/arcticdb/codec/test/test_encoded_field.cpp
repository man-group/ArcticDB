...
TEST(EncodedFieldTest, Roundtrip) {
  ...
  for (int i = 0; i < original_ndarray.shapes_size(); ++i) {
    ...
    ASSERT_EQ(original_shape_codec.zstd().level(),
              roundtrip_shape_codec.zstd().level());
    ASSERT_EQ(original_shape_codec.zstd().is_streaming(),
              roundtrip_shape_codec.zstd().is_streaming());
  }
  ...
  for (int i = 0; i < original_ndarray.values_size(); ++i) {
    ...
    ASSERT_EQ(original_value_codec.lz4().acceleration(),
              roundtrip_value_codec.lz4().acceleration());
  }
}