...
template <template <typename> class BlockType, class TD> struct PassthroughEncoderV1 {
  ...
  static size_t max_compressed_size(const BlockType<TD>& block) {
    ...
    if constexpr (Helper::dim == entity::Dimension::Dim0) {
      ...
      auto helper_array_block =
          Helper::nd_array_block(block.row_count(), block.shapes());
      return helper_array_block.shapes_.bytes_ + helper_array_block.values_.bytes_;
    }
  }

  template <typename EncodedFieldType>
  static void encode(const Opts&, const BlockType<TD>& block, EncodedFieldType& field,
    ...
    if constexpr (Helper::dim == entity::Dimension::Dim0) {
      ...
      scalar_block.set_block_data(*values, helper.hasher_.digest(),
                                  scalar_block.bytes_);
    } else {
      auto helper_array_block = Helper::nd_array_block(block_row_count, block.shapes());
      helper.ensure_buffer(out, pos,
      ...
      encode_block(block.shapes(), helper_array_block.shapes_, helper.hasher_, s_out,
                   pos);
      ...
      helper_array_block.set_block_data(
          shapes, values, shape_hash, helper_array_block.shapes_.bytes_,
...
private:
  template <class T>
  static void encode_block(const T* in, BlockDataHelper& block_utils, HashAccum& hasher,
                           T* out, std::ptrdiff_t& pos) {
...
/// @brief "Encoder" which stores the input in the output buffer without performing any
/// transformation
/// @note The difference between arcticdb::detail::PassthroughEncoder and
/// arcticdb::detail::PassthroughEncoder2 is that the latter does not care about the
/// shapes array.
/// @see arcticdb::ColumnEncoder2 arcticdb::detail::GenericBlockEncoder2
template <template <typename> class BlockType, class TD> struct PassthroughEncoderV2 {
  using Opts = arcticdb::proto::encoding::VariantCodec::Passthrough;
  ...
  static void encode(const Opts&, const BlockType<TD>& block, Buffer& out,
                     std::ptrdiff_t& pos, EncodedBlockType* encoded_block) {
    using namespace arcticdb::entity;
    using Helper = CodecHelper<TD>;
    using T = typename Helper::T;
    Helper helper;
...
private:
  template <class T>
  static void encode_block(const T* in, size_t in_byte_size, HashAccum& hasher, T* out,
                           std::ptrdiff_t& pos) {
