...
template <typename MagicType> void write_magic(Buffer& buffer, std::ptrdiff_t& pos) {
  new (buffer.data() + pos) MagicType{};
  pos += sizeof(MagicType);
}

void write_frame_data(Buffer& buffer, std::ptrdiff_t& pos,
...
void write_segment_descriptor(Buffer& buffer, std::ptrdiff_t& pos,
                              const SegmentDescriptorImpl& segment_desc) {
...
/// data columns for V2 encoding What differs this from the already existing
/// ColumnEncoder is that ColumnEncoder encodes the shapes of multidimensional data as
/// part of each block. ColumnEncoder2 uses a better strategy and encodes the shapes for
/// the whole column upfront (before all blocks).
/// @note Although ArcticDB did not support multidimensional user data prior to creating
/// ColumnEncoder2 some of the internal data was multidimensional and used
/// ColumnEncoder. More specifically: string pool and metadata.
/// @note This should be used for V2 encoding. V1 encoding can't use it as there is
/// already data written the other
...
public:
  static void encode(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                     ColumnData& column_data, EncodedFieldImpl& field, Buffer& out,
                     std::ptrdiff_t& pos);
...
private:
  static void encode_shapes(const ColumnData& column_data, EncodedFieldImpl& field,
                            Buffer& out, std::ptrdiff_t& pos_in_buffer);

  static void encode_blocks(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                            ColumnData& column_data, EncodedFieldImpl& field,
                            Buffer& out, std::ptrdiff_t& pos);
};

[[nodiscard]] static TypedBlockData<ShapesBlockTDT>
create_shapes_typed_block(const ColumnData& column_data) {
  static_assert(sizeof(ssize_t) == sizeof(int64_t));
  const size_t row_count = column_data.shapes()->bytes() / sizeof(shape_t);
  return {reinterpret_cast<const typename ShapesBlockTDT::DataTypeTag::raw_type*>(
              column_data.shapes()->data()),
          nullptr, column_data.shapes()->bytes(), row_count, nullptr};
}

void ColumnEncoderV2::encode(const arcticdb::proto::encoding::VariantCodec& codec_opts,
                             ColumnData& column_data, EncodedFieldImpl& field,
                             Buffer& out, std::ptrdiff_t& pos) {
...
void ColumnEncoderV2::encode_shapes(const ColumnData& column_data,
                                    EncodedFieldImpl& field, Buffer& out,
                                    std::ptrdiff_t& pos_in_buffer) {
  // There is no need to store the shapes for a column of empty type as they will be all
  // 0. The type handler will assign 0 for the shape upon reading. There is one edge
  // case - when we have None in the column, as it should not have shape at all (since
  // it's not an array). This is handled by the sparse map.
  if (column_data.type().dimension() != Dimension::Dim0 &&
      !is_empty_type(column_data.type().data_type())) {
    TypedBlockData<ShapesBlockTDT> shapes_block =
        create_shapes_typed_block(column_data);
    using ShapesEncoder =
        TypedBlockEncoderImpl<TypedBlockData, ShapesBlockTDT, EncodingVersion::V2>;
    ShapesEncoder::encode_shapes(codec::default_shapes_codec(), shapes_block, field,
                                 out, pos_in_buffer);
  }
}
...
void ColumnEncoderV2::encode_blocks(
    ...
    EncodedFieldImpl& field, Buffer& out, std::ptrdiff_t& pos) {
...
std::pair<size_t, size_t> ColumnEncoderV2::max_compressed_size(
  ...
  return column_data.type().visit_tag([&codec_opts, &column_data](auto type_desc_tag) {
    ...
    const TypedBlockData<ShapesBlockTDT> shapes_block =
        create_shapes_typed_block(column_data);
...
static void
encode_field_descriptors(const SegmentInMemory& in_mem_seg,
                         SegmentHeader& segment_header,
                         const arcticdb::proto::encoding::VariantCodec& codec_opts,
                         Buffer& out_buffer, std::ptrdiff_t& pos) {
  ...
  if (!in_mem_seg.fields().empty()) {
    ...
    auto& encoded_field =
        segment_header.mutable_descriptor_field(calc_num_blocks<EncodingPolicyV2>(col));
...
static void
encode_index_descriptors(const SegmentInMemory& in_mem_seg,
                         SegmentHeader& segment_header,
                         const arcticdb::proto::encoding::VariantCodec& codec_opts,
                         Buffer& out_buffer, std::ptrdiff_t& pos) {
  ...
  if (in_mem_seg.has_index_descriptor()) {
    ...
    ARCTICDB_TRACE(log::codec(), "Encoding index fields descriptors to position {}",
                   pos);
    auto index_field_data = tsd.fields().column_data();
    auto& index_field = segment_header.mutable_index_descriptor_field(
        calc_num_blocks<EncodingPolicyV2>(index_field_data));
...
static void
calc_encoded_blocks_size(const SegmentInMemory& in_mem_seg,
                         const arcticdb::proto::encoding::VariantCodec& codec_opts,
                         SizeResult& result) {
  ...
  result.max_compressed_bytes_ += BytesEncoder<EncodingPolicyV2>::max_compressed_size(
      codec_opts, result.encoded_blocks_bytes_);
}

static void add_stream_descriptor_data_size(SizeResult& result,
...
static void calc_stream_descriptor_fields_size(
    ...
    const arcticdb::proto::encoding::VariantCodec& codec_opts, SizeResult& result) {
  auto segment_fields = in_mem_seg.descriptor().fields().column_data();
  const auto [uncompressed, required] =
      ColumnEncoderV2::max_compressed_size(codec_opts, segment_fields);
...
max_compressed_size_v2(const SegmentInMemory& in_mem_seg,
                       const arcticdb::proto::encoding::VariantCodec& codec_opts) {
  ...
  if (in_mem_seg.row_count() > 0) {
    result.max_compressed_bytes_ +=
        sizeof(ColumnMagic) * in_mem_seg.descriptor().field_count();
...
static void
encode_encoded_fields(SegmentHeader& segment_header,
                      const arcticdb::proto::encoding::VariantCodec& codec_opts,
                      Buffer& out_buffer, std::ptrdiff_t& pos,
  ...
  Column encoded_fields_column(encoded_fields_type_desc(), false,
                               encoded_fields.release_data());
  auto data = encoded_fields_column.data();
  auto& encoded_field =
      segment_header.mutable_column_fields(calc_num_blocks<EncodingPolicyV2>(data));
...
encode_v2(SegmentInMemory&& s,
  ...
  encode_metadata<EncodingPolicyV2>(in_mem_seg, segment_header, codec_opts, *out_buffer,
                                    pos);
  write_magic<SegmentDescriptorMagic>(*out_buffer, pos);
  ...
  if (in_mem_seg.row_count() > 0) {
    ...
    for (std::size_t column_index = 0; column_index < in_mem_seg.num_columns();
      ...
      ARCTICDB_TRACE(
          log::codec(), "Beginning encoding of column {}: ({}) to position {}",
          column_index, in_mem_seg.descriptor().field(column_index).name(), pos);
      ...
      if (column_data.num_blocks() > 0) {
        ...
        ARCTICDB_TRACE(log::codec(), "Encoded column {}: ({}) to position {}",
                       column_index, in_mem_seg.descriptor().field(column_index).name(),
                       pos);
      } else {
    ...
    encode_string_pool<EncodingPolicyV2>(in_mem_seg, segment_header, codec_opts,
                                         *out_buffer, pos);
    encode_encoded_fields(segment_header, codec_opts, *out_buffer, pos,
                          std::move(encoded_fields));
  } else {
    segment_header.set_footer_offset(pos);
  }
  ...
  return Segment::initialize(std::move(segment_header), std::move(out_buffer),
                             descriptor_data, desc.fields_ptr(), desc.id());
}
