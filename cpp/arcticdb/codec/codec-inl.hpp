...
void decode_block(const BlockType& block, const std::uint8_t* input, T* output) {
  ...
  if (!block.has_codec()) {
    arcticdb::detail::PassthroughDecoder::decode_block<T>(input, size_to_decode, output,
                                                          decoded_size);
  } else {
    ...
    case arcticdb::Codec::LZ4:
      ...
      break;
    default:
      util::raise_rte("Unsupported block codec {}",
                      codec_type_to_string(block.codec().codec_type()));
    }
  }
}

template <typename FieldType, class DataSink>
inline void read_shapes(FieldType& encoded_field, DataSink& data_sink,
                        uint8_t const*& data_in, int shapes_block,
                        shape_t*& shapes_out) {
...
std::size_t
decode_ndarray(const TypeDescriptor& td, const NDArrayEncodedFieldType& field,
               const std::uint8_t* input, DataSink& data_sink,
  ...
  td.visit_tag([&](auto type_desc_tag) {
    ...
    const bool is_empty_array =
        (data_size == 0) && type_desc_tag.dimension() > Dimension::Dim0;
    // Empty array types will not contain actual data, however, its sparse map should be
    // loaded so that we can distinguish None from []
    ...
    auto data_begin = is_empty_array
                          ? nullptr
                          : static_cast<uint8_t*>(data_sink.allocate_data(data_size));
    util::check(is_empty_array || data_begin != nullptr,
                "Failed to allocate data of size {}", data_size);
    ...
    ARCTICDB_TRACE(
        log::codec(),
        "Decoding ndarray with type {}, uncompressing {} ({}) bytes in {} blocks", td,
        data_size, encoding_sizes::ndarray_field_compressed_size(field), num_blocks);
    ...
    for (auto block_num = 0; block_num < num_blocks; ++block_num) {
      if constexpr (TD::DimensionTag::value != Dimension::Dim0) {
        ...
        // In V2 encoding all shapes are put in a single block placed at the beginning
        // of the block chain.
      ...
      ARCTICDB_TRACE(log::codec(), "Decoding block {} at pos {}", block_num,
                     data_in - input);
    ...
    if (field.sparse_map_bytes()) {
      util::check(!is_empty_type(type_desc_tag.data_type()),
                  "Empty typed columns should not have sparse map");
      util::check_magic<util::BitMagicStart>(data_in);
      const auto bitmap_size =
          field.sparse_map_bytes() - util::combined_bit_magic_delimiters_size();
    ...
    util::check(data_in - input == intptr_t(read_bytes),
                "Decoding compressed size mismatch, expected decode size {} to equal "
                "total size {}",
                data_in - input, read_bytes);

    util::check(data_out - data_begin == intptr_t(data_size),
                "Decoding uncompressed size mismatch, expected position {} to be equal "
                "to data size {}",
...
std::size_t decode_field(const TypeDescriptor& td, const EncodedFieldImpl& field,
                         const std::uint8_t* input, DataSink& data_sink,
  ...
  case EncodedFieldType::NDARRAY:
    return decode_ndarray(td, field.ndarray(), input, data_sink, bv, encoding_version) +
           magic_size;
  default:
