...
public:
  DynamicFieldBuffer(const TypeDescriptor& td, const py::buffer& data,
                     const py::buffer& shapes)
      : td_(td), field_() {
    auto d_info_ = data.request();
    auto s_info_ = shapes.request();
    util::check_arg(d_info_.ndim == 1,
                    "only support dimension 1 (flattened) data. actual {}",
                    d_info_.ndim);
    util::check_arg(s_info_.ndim == 1,
                    "only support dimension 1 (flattened) shapes. actual {}",
                    s_info_.ndim);
    util::check_arg(sizeof(shape_t) == s_info_.itemsize,
                    "expected shape itemsize={:d}, actual={:d}", sizeof(shape_t),
                    s_info_.itemsize);
    ...
    std::size_t dim =
        std::max(static_cast<std::size_t>(1), static_cast<std::size_t>(td.dimension()));
    ...
    if (td.dimension() == Dimension::Dim0) {
      ...
      field_ = std::make_shared<ColumnData>(&data_.buffer(), &shapes_.buffer(), td_,
                                            nullptr);
    } else {
      data_.ensure_bytes(data_bytes);
...
struct FieldEncodingResult {
  ...
  FieldEncodingResult(std::shared_ptr<Buffer> buffer,
                      proto::encoding::EncodedField encoded_field)
      : buffer_(std::move(buffer)), encoded_field_(encoded_field) {}
...
public:
  BufferPairDataSink()
      : values_(std::make_shared<Buffer>()), shapes_(std::make_shared<Buffer>()) {}
...
struct FieldDecodingResult {
  ...
  FieldDecodingResult(std::shared_ptr<Buffer> shape_buffer,
...
void register_codec(py::module& m) {
  ...
  py::class_<FieldEncodingResult, std::shared_ptr<FieldEncodingResult>>(
      m, "FieldEncodingResult")
      .def(py::init<>())
      .def_property_readonly(
          "buffer", [](const FieldEncodingResult& self) { return self.buffer_; })
      .def_property_readonly("encoded_field", [](const FieldEncodingResult& self) {
        return python_util::pb_to_python(self.encoded_field_);
      });

  py::class_<FieldDecodingResult, std::shared_ptr<FieldDecodingResult>>(
      m, "FieldDecodingResult")
      .def(py::init<>())
      .def_property_readonly(
          "shape_buffer",
          [](const FieldDecodingResult& self) { return self.shape_buffer_; })
  ...
  py::class_<Buffer, std::shared_ptr<Buffer>>(m, "Buffer", py::buffer_protocol())
      ...
      .def_buffer([](Buffer& buffer) {
        return py::buffer_info{
            buffer.data(),    1,  py::format_descriptor<std::uint8_t>::format(), 1,
            {buffer.bytes()}, {1}};
      });
  ...
  py::class_<Segment>(m, "Segment")
      ...
      .def_property_readonly(
          "header", [](const Segment& self) { return self.header().clone(); },
          py::return_value_policy::move)
      .def_property_readonly("bytes", [](const Segment& self) {
        return py::bytes(reinterpret_cast<char*>(self.buffer().data()),
                         self.buffer().bytes());
      });
