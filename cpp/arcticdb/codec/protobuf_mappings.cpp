...
void block_from_proto(const arcticdb::proto::encoding::Block& input,
  ...
  case arcticdb::proto::encoding::VariantCodec::kPassthrough: {
    set_codec(input.codec().passthrough(),
              *output.mutable_codec()->mutable_passthrough());
    break;
  }
  default:
...
void proto_from_block(const EncodedBlock& input,
  ...
  case Codec::PASS: {
    set_passthrough(input.codec().passthrough(),
                    *output.mutable_codec()->mutable_passthrough());
    break;
  }
  default:
...
SegmentHeader deserialize_segment_header_from_proto(
  ...
  if (header.has_metadata_field())
    encoded_field_from_proto(
        ...
        output.mutable_metadata_field(num_blocks(header.metadata_field())));

  if (header.has_string_pool_field())
    encoded_field_from_proto(
        header.string_pool_field(),
        output.mutable_string_pool_field(num_blocks(header.string_pool_field())));
