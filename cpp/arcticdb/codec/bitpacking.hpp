#include <arcticdb/codec/thirdparty/fastpforlib/bitpackinghelpers.h>
...
public:
    static constexpr const uint64_t BITPACKING_ALGORITHM_GROUP_SIZE = 32;
    static constexpr const uint64_t BITPACKING_HEADER_SIZE = sizeof(uint64_t);
    ...
    inline static void PackBuffer(uint8_t* dst, T *src, uint64_t count, uint8_t width) {
        if (ASSUME_INPUT_ALIGNED) {
            for (uint64_t i = 0; i < count; i += BITPACKING_ALGORITHM_GROUP_SIZE) {
                PackGroup<T>(dst + (i * width) / 8, src + i, width);
            }
        } else {
            uint64_t misaligned_count = count % BITPACKING_ALGORITHM_GROUP_SIZE;
    ...
    template<class T>
    inline static void UnPackBuffer(uint8_t* dst, uint8_t* src, uint64_t count, uint8_t width,
    ...
    template<class T>
    inline static void PackBlock(uint8_t* dst, T *src, uint8_t width) {
        return PackGroup<T>(dst, src, width);
    }

    // Unpacks a block of BITPACKING_ALGORITHM_GROUP_SIZE values
    template<class T>
    inline static void UnPackBlock(uint8_t* dst, uint8_t* src, uint8_t width,
                                   bool skip_sign_extension = false) {
    ...
    inline static uint8_t MinimumBitWidth(T value) {
        return FindMinimumBitWidth<T, is_signed, BYTE_ALIGNED>(value, value);
    }

    // Calculates the minimum required number of bits per value that can store all values
    template<class T, bool is_signed = std::is_signed<T>::value>
    inline static uint8_t MinimumBitWidth(T *values, uint64_t count) {
    ...
    template<class T, bool is_signed = std::is_signed<T>::value>
    inline static uint8_t MinimumBitWidth(T minimum, T maximum) {
        return FindMinimumBitWidth<T, is_signed, BYTE_ALIGNED>(minimum, maximum);
    }

    inline static uint64_t GetRequiredSize(uint64_t count, uint8_t width) {
    ...
    inline static T RoundUpToAlgorithmGroupSize(T num_to_round) {
        ...
        return num_to_round + BITPACKING_ALGORITHM_GROUP_SIZE - static_cast<uint64_t>(remainder);
    }

private:
    template<class T, bool is_signed, bool round_to_next_byte = false>
    static uint8_t FindMinimumBitWidth(T *values, uint64_t count) {
    ...
    template<class T, bool is_signed, bool round_to_next_byte = false>
    static uint8_t FindMinimumBitWidth(T min_value, T max_value) {
        uint8_t bitwidth;
        T value;
    ...
    static void SignExtend(uint8_t* dst, uint8_t width) {
        T const mask = static_cast<T>(T_U(1) << (width - 1));
        for (uint64_t i = 0; i < BitpackingPrimitives::BITPACKING_ALGORITHM_GROUP_SIZE; ++i) {
            T value = Load<T>(dst + i * sizeof(T));
            value = static_cast<T>(T_U(value) & ((T_U(1) << width) - T_U(1)));
    ...
    template<class T>
    static uint8_t GetEffectiveWidth(uint8_t width) {
        uint8_t bits_of_type = sizeof(T) * 8;
    ...
    template<class T>
    static inline void PackGroup(uint8_t* dst, T *values, uint8_t width) {
        if (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value) {
            arcticdb_pforlib::fastpack(reinterpret_cast<const uint8_t *>(values),
                                         reinterpret_cast<uint8_t *>(dst),
                                         static_cast<uint32_t>(width));
        } else if (std::is_same<T, int16_t>::value || std::is_same<T, uint16_t>::value) {
            arcticdb_pforlib::fastpack(reinterpret_cast<const uint16_t *>(values),
                                         reinterpret_cast<uint16_t *>(dst),
                                         static_cast<uint32_t>(width));
        } else if (std::is_same<T, int32_t>::value || std::is_same<T, uint32_t>::value) {
            arcticdb_pforlib::fastpack(reinterpret_cast<const uint32_t *>(values),
                                         reinterpret_cast<uint32_t *>(dst),
                                         static_cast<uint32_t>(width));
        } else if (std::is_same<T, int64_t>::value || std::is_same<T, uint64_t>::value) {
            arcticdb_pforlib::fastpack(reinterpret_cast<const uint64_t *>(values),
                                           reinterpret_cast<uint32_t *>(dst),
                                           static_cast<uint32_t>(width));
        } else {
            util::raise_rte("Unsupported type for bitpacking");
        }
    }

    template<class T>
    static inline void UnPackGroup(uint8_t* dst, uint8_t* src, uint8_t width,
                                   bool skip_sign_extension = false) {
        if (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value) {
            arcticdb_pforlib::fastunpack(reinterpret_cast<const uint8_t *>(src), reinterpret_cast<uint8_t *>(dst),
                                           static_cast<uint32_t>(width));
        } else if (std::is_same<T, int16_t>::value || std::is_same<T, uint16_t>::value) {
            arcticdb_pforlib::fastunpack(reinterpret_cast<const uint16_t *>(src),
                                           reinterpret_cast<uint16_t *>(dst),
                                           static_cast<uint32_t>(width));
        } else if (std::is_same<T, int32_t>::value || std::is_same<T, uint32_t>::value) {
            arcticdb_pforlib::fastunpack(reinterpret_cast<const uint32_t *>(src),
                                           reinterpret_cast<uint32_t *>(dst),
                                           static_cast<uint32_t>(width));
        } else if (std::is_same<T, int64_t>::value || std::is_same<T, uint64_t>::value) {
            arcticdb_pforlib::fastunpack(reinterpret_cast<const uint32_t *>(src),
        ...
        if (std::is_signed<T>::value && !skip_sign_extension && width > 0 && width < sizeof(T) * 8) {
