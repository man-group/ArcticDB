...
size_t field_collection_encoded_field_bytes(const FieldCollection& fields) {
  return calc_field_bytes(
      fields.num_blocks() == 0
          ? 0
          : fields.num_blocks() +
                1); // Non-empty field collection always has shapes buffer
}

size_t SegmentHeader::serialize_to_bytes(uint8_t* dst,
                                         std::optional<size_t> expected_bytes) const {
  const auto* begin = dst;
  data_.field_buffer_.fields_bytes_ =
      static_cast<uint32_t>(header_fields_.data_bytes());
  data_.field_buffer_.offset_bytes_ =
      static_cast<uint32_t>(header_fields_.offset_bytes());
  ...
  util::check(!expected_bytes || (bytes_written == *expected_bytes),
              "Mismatch between actual and expected bytes: {} != {}", dst - begin,
              expected_bytes ? *expected_bytes : 0);
  ARCTICDB_TRACE(log::codec(), "Wrote V2 header with {} bytes ({} expected)",
                 bytes_written, expected_bytes.value_or(0));
...
size_t calc_required_header_fields_bytes(const SegmentInMemory& in_mem_seg) {
  ...
  if (in_mem_seg.has_index_descriptor()) {
    const auto index_descriptor_size =
        field_collection_encoded_field_bytes(in_mem_seg.index_descriptor().fields()) +
        sizeof(uint64_t);
  ...
  if (in_mem_seg.has_string_pool()) {
    const auto string_pool_size =
        calc_field_bytes(in_mem_seg.const_string_pool().num_blocks() + 1) +
  ...
  if (!in_mem_seg.descriptor().empty()) {
    const auto descriptor_size =
        field_collection_encoded_field_bytes(in_mem_seg.descriptor().fields()) +
        sizeof(uint64_t);
...
void SegmentHeader::deserialize_from_bytes(const uint8_t* data, bool copy_data) {
  ...
  header_fields_ =
      EncodedFieldCollection{std::move(fields_buffer), std::move(offsets_buffer)};
...
size_t SegmentHeader::required_bytes(const SegmentInMemory& in_mem_seg) {
  ...
  ARCTICDB_TRACE(log::codec(), "Overhead size {} + {} + {} = {}", FIXED_HEADER_SIZE,
                 sizeof(HeaderData), sizeof(offset_), required);
