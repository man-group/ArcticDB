...
get_segment_begin_end(const Segment& segment, const SegmentHeader& hdr);
...
struct BlockCodecImpl : public BlockCodec {
  ...
  template <class CodecType> explicit BlockCodecImpl(const CodecType& codec) {
...
struct EncodedFieldImpl : public EncodedField {
  static constexpr size_t Size = sizeof(type_) + sizeof(shapes_count_) +
                                 sizeof(values_count_) + sizeof(sparse_map_bytes_) +
                                 sizeof(items_count_) + sizeof(format_);
  ...
  struct EncodedBlockCollection {
    EncodedBlockCollection(const EncodedFieldImpl& field, bool is_shapes)
        : field_(field), is_shapes_(is_shapes) {}

    template <class ValueType>
    class EncodedBlockCollectionIterator
        : public boost::iterator_facade<EncodedBlockCollectionIterator<ValueType>,
                                        ValueType, boost::forward_traversal_tag> {
    public:
      EncodedBlockCollectionIterator(EncodedBlock* blocks, size_t increment)
          : blocks_(blocks), increment_(increment) {}

      ~EncodedBlockCollectionIterator() = default;

      EncodedBlockCollectionIterator(EncodedBlock* blocks, size_t pos, size_t increment)
          : blocks_(blocks), pos_(pos), increment_(increment) {}

      template <class OtherValue>
      explicit EncodedBlockCollectionIterator(
          const EncodedBlockCollectionIterator<OtherValue>& other)
          : blocks_(other.blocks_), pos_(other.pos_), increment_(other.increment_) {}

      EncodedBlockCollectionIterator() = default;

      EncodedBlockCollectionIterator&
      operator=(const EncodedBlockCollectionIterator& other) {
        if (&other != this) {
          pos_ = other.pos_;
      ...
      template <class OtherValue>
      [[nodiscard]] bool
      equal(const EncodedBlockCollectionIterator<OtherValue>& other) const {
    ...
    [[nodiscard]] size_t last() const {
      ...
      if (field_.is_old_style_shapes())
        ...
        return is_shapes_ ? field_.shapes_count_
                          : field_.shapes_count_ + field_.values_count_;
    }
  ...
  [[nodiscard]] const EncodedBlock& values(size_t n) const {
    util::check(n < values_count_ + shapes_count_,
                "Cannot return block {} from {} blocks ({} shapes)", n, values_count_,
                shapes_count_);
  ...
  void validate() const {
    ...
    util::check(shapes_count == static_cast<size_t>(shapes_size()),
                "Shape size mismatch: {} != {}", shapes_count, shapes_size());

    size_t values_count = 0;
    for (const auto& value : values()) {
      util::check(!value.is_shape_, "Value has is_shape set");
      util::check(value.codec().codec_type() != Codec::UNKNOWN,
                  "Unknown codec in block {}", values_count);
      ++values_count;
    }
    util::check(values_count == static_cast<size_t>(values_size()),
                "Shape size mismatch: {} != {}", values_count, values_size());
  }
  ...
  [[nodiscard]] std::string DebugString() const {
    return fmt::format("{}: {} shapes {} values", has_ndarray() ? "NDARRAY" : "DICT",
                       shapes_size(), values_size());
  }
...
static_assert(EncodedFieldImpl::Size ==
              sizeof(EncodedFieldImpl) - sizeof(EncodedBlock));
...
namespace fmt {
template <> struct formatter<arcticdb::BlockCodecImpl> {
  ...
  auto format(arcticdb::BlockCodecImpl codec, FormatContext& ctx) const {
    return format_to(ctx.out(), "{}",
                     arcticdb::codec_type_to_string(codec.codec_type()));
  }
};

template <> struct formatter<arcticdb::EncodedFieldImpl> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
  ...
  auto format(const arcticdb::EncodedFieldImpl& field, FormatContext& ctx) const {
    ...
    for (const auto& shape : field.shapes()) {
      fmt::format_to(ctx.out(), "\tCodec: {} in_bytes: {}, out_bytes {}\n",
                     ...
                     shape.in_bytes(), shape.out_bytes());
    }
    ...
    for (const auto& value : field.values()) {
      fmt::format_to(ctx.out(), "\tCodec: {} in_bytes: {}, out_bytes {}\n",
                     ...
                     value.in_bytes(), value.out_bytes());
    }
