...
struct IndexDescriptor {
  ...
  IndexDescriptor(Type type, uint32_t field_count)
      : type_(type), field_count_(field_count) {}
...
// A segment header contains a set of optional fields that describe the contents of a
// given segment
...
// Note. The structures below contain variable-length fields (represented by named
// structures) and should not be used to implement the data format directly. They are
// intended as a syntactically-correct representation of the storage format only.

// Denotes a field that may or may not be present
template <typename FieldType> struct Optional {};

// A list of field descriptors containing the name, type and dimensionality of a column
// of data
struct FieldList {};

// A list of encoded fields that describes the contents of other fields. An encoded
// field is a list of blocks with a specific set of compression types
...
// A data field described by an EncodedField, consists of a set of compressed blocks
// that may represent shapes and values, and an optional sparse bitmap
struct ColumnField {
  ColumnMagic column_magic_;
};

// A compressed block of data containing some other structure. A compressed field is
// represented by an EncodedField which contains a set of Block objects describing the
// compression type
template <typename FieldType> struct CompressedField {};

// A set of fields that are repeated, whose number corresponds to a unary field
// describing this set. For example, the number of repeated column fields should
// correspond to the number of entries in the descriptor (which describes the
// user-facing information about a column's contents, and the number of EncodedFields in
// the body fields, which describe the block structure and compression
template <typename FieldType> struct RepeatedField {};

// Binary representation of a segment header. Contains positioning information about the
// structure of the segment, and the list of fields representing the segment metadata
// fields
struct SegmentHeaderData {
  HeaderData data_;
  EncodedFieldList
      header_fields_; // Header fields containing the fields described by FieldOffsets
  std::array<uint32_t, 5> offset_ =
      {}; // Maps the entries in the FieldOffset enumeration to the header field entries
};

// The overall memory layout of an ArcticDB segment. Note that this class need not
// necessarily be used to implement a reader or writer of the memory layout - refer to
// comment above
struct MemoryLayout {
  ...
  // Optional fields present if this segment refers to a complete dataframe, i.e. if it
  // is a primary index
  ...
  EncodedFieldList
      body_fields_; // Encoded field list representing the user data fields (columns)
};
