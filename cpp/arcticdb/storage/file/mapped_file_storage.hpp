...
private:
  ...
  [[noreturn]] void do_update(Composite<KeySegmentPair>&& kvs,
                              UpdateOpts opts) override;
  ...
  bool do_supports_prefix_matching() const override { return false; };

  std::string do_key_path(const VariantKey&) const override { return {}; }

  bool do_fast_delete() override;

  [[noreturn]] void do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
...
pack_config(const std::string& path, size_t file_size, size_t items_count,
            const StreamId& id, const IndexDescriptorImpl& index_desc,
  ...
  util::variant_match(
      id, [&cfg](const StringId& str) { cfg.set_str_id(str); },
...
inline arcticdb::proto::storage::VariantStorage
pack_config(const std::string& path,
            const arcticdb::proto::encoding::VariantCodec& codec_opts) {
  arcticdb::proto::storage::VariantStorage output;
  arcticdb::proto::mapped_file_storage::Config cfg;
  cfg.set_path(path);
