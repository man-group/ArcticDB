...
void write_dataframe_to_file_internal(
    ...
    const size_t write_window = ConfigsMap::instance()->get_int(
        "VersionStore.BatchWriteWindow", static_cast<int64_t>(2 * async::TaskScheduler::instance()->io_thread_count())
    );
    auto key_seg_futs =
        folly::collect(folly::window(
                           std::move(slice_and_rowcount),
                           [frame, slicing, key = std::move(partial_key), sparsify_floats = options.sparsify_floats](
                               auto&& slice
                           ) {
...
version_store::ReadVersionOutput read_dataframe_from_file_internal(
    ...
    auto index_key = from_serialized_atom_key(
        single_file_storage->read_raw(key_data.key_offset_, key_data.key_size_), KeyType::TABLE_INDEX
    );
    ...
    auto frame_and_descriptor =
        version_store::read_frame_for_version(store, versioned_item, read_query, read_options, handler_data);
