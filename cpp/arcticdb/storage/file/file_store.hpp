...
size_t max_data_size(const std::vector<std::tuple<stream::StreamSink::PartialKey,
                                                  SegmentInMemory, FrameSlice>>& items,
                     const arcticdb::proto::encoding::VariantCodec& codec_opts,
                     EncodingVersion encoding_version) {
  auto max_file_size = 0UL;
  for (const auto& item : items) {
    const auto& [pk, seg, slice] = item;
    max_file_size += max_compressed_size_dispatch(seg, codec_opts, encoding_version)
                         .max_compressed_bytes_;
  }
...
void write_dataframe_to_file_internal(
    ...
    const std::shared_ptr<pipelines::InputTensorFrame>& frame, const std::string& path,
  ...
  const size_t write_window = ConfigsMap::instance()->get_int(
                         ...
                         [frame, slicing, key = std::move(partial_key),
                          sparsify_floats = options.sparsify_floats](auto&& slice) {
                           return async::submit_cpu_task(pipelines::WriteToSegmentTask(
                               frame, slice.first, slicing,
                               get_partial_key_gen(frame, key), slice.second,
                               frame->index, sparsify_floats));
                         },
                         write_window))
  ...
  auto config = storage::file::pack_config(
      path, data_size, segments.size(), stream_id,
      stream::get_descriptor_from_index(frame->index), encoding_version, codec_opts);

  storage::LibraryPath lib_path{std::string{"file"}, fmt::format("{}", stream_id)};
  auto library =
      create_library(lib_path, storage::OpenMode::WRITE, {std::move(config)});
  auto store = std::make_shared<async::AsyncStore<PilotedClock>>(library, codec_opts,
                                                                 encoding_version);
      ...
      folly::collect(folly::window(
          ...
          .thenValue([&frame, stream_id, store](auto&& slice_and_keys) {
            return index::write_index(
                frame, std::forward<decltype(slice_and_keys)>(slice_and_keys),
                IndexPartialKey{stream_id, VersionId{0}}, store);
          });
  ...
  single_file_store->write_raw(reinterpret_cast<const uint8_t*>(serialized_key.c_str()),
                               serialized_key.size());
...
version_store::ReadVersionOutput read_dataframe_from_file_internal(
    const StreamId& stream_id, const std::string& path, ReadQuery& read_query,
  ...
  auto library =
      create_library(lib_path, storage::OpenMode::WRITE, {std::move(config)});
  auto store = std::make_shared<async::AsyncStore<PilotedClock>>(
      library, codec::default_lz4_codec(), EncodingVersion::V1);
  ...
  auto key_data = *reinterpret_cast<KeyData*>(
      single_file_storage->read_raw(data_end, sizeof(KeyData)));

  auto index_key = from_serialized_atom_key(
      single_file_storage->read_raw(key_data.key_offset_, key_data.key_size_),
      KeyType::TABLE_INDEX);
  ...
  version_store::read_indexed_keys_to_pipeline(store, pipeline_context, versioned_item,
                                               read_query, read_options);
  ...
  auto frame = version_store::do_direct_read_or_process(store, read_query, read_options,
  ...
  FrameAndDescriptor frame_and_descriptor{
      frame,
      timeseries_descriptor_from_pipeline_context(pipeline_context, {},
