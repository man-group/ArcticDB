...
MappedFileStorage::MappedFileStorage(const LibraryPath& lib, OpenMode mode, Config conf)
    : SingleFileStorage(lib, mode), config_(std::move(conf)) {
  init();
}
...
void MappedFileStorage::do_write_raw(const uint8_t* data, size_t bytes) {
  ARCTICDB_DEBUG(log::storage(), "Writing {} bytes to mapped file storage at offset {}",
                 bytes, offset_);
...
void MappedFileStorage::init() {
  ...
  if (config_.bytes() > 0) {
    ARCTICDB_DEBUG(log::storage(), "Creating new mapped file storage at path {}",
                   config_.path());
    multi_segment_header_.initalize(StreamId{NumericId{0}}, config_.items_count());
    auto data_size =
        config_.bytes() +
        max_compressed_size_dispatch(
            multi_segment_header_.segment(), config_.codec_opts(),
    ...
    ARCTICDB_DEBUG(log::storage(), "Opening existing mapped file storage at path {}",
                   config_.path());
...
void MappedFileStorage::do_load_header(size_t header_offset, size_t header_size) {
  ...
  ARCTICDB_DEBUG(log::storage(), "Loaded mapped file header of size {}",
                 header.row_count());
...
uint64_t MappedFileStorage::get_data_offset(const Segment& seg) {
  ...
  ARCTICDB_DEBUG(log::storage(),
                 "Mapped file storage returning offset {} and adding {} bytes", offset_,
                 segment_size);
...
uint64_t MappedFileStorage::write_segment(Segment&& seg) {
  ...
  ARCTICDB_DEBUG(log::storage(),
                 "Mapped file storage wrote segment of size {} at offset {}",
                 segment.size(), offset);
  return offset;
}
...
void MappedFileStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                                storage::ReadKeyOpts) {
...
bool MappedFileStorage::do_fast_delete() {
  util::raise_rte(
      "Fast delete not implemented for file storage - just delete the file");
}

void MappedFileStorage::do_finalize(KeyData key_data) {
  multi_segment_header_.sort();
  auto header_segment = encode_dispatch(
      multi_segment_header_.detach_segment(), config_.codec_opts(),
      EncodingVersion{static_cast<uint16_t>(config_.encoding_version())});
...
uint8_t* MappedFileStorage::do_read_raw(size_t offset, size_t bytes) {
  util::check(offset + bytes <= file_.bytes(),
              "Can't read {} bytes from {} in file of size {},", bytes, offset,
              file_.bytes());
  ARCTICDB_DEBUG(log::storage(),
                 "Mapped file storage returning raw offset {} for {} bytes", offset,
                 bytes);
  return file_.data() + offset;
}

void MappedFileStorage::do_iterate_type(KeyType, const IterateTypeVisitor&,
                                        const std::string&) {
  util::raise_rte("Iterate type not implemented for file storage");
}

size_t MappedFileStorage::do_get_offset() const { return offset_; }

size_t MappedFileStorage::do_get_bytes() const { return file_.bytes(); }
} // namespace arcticdb::storage::file
