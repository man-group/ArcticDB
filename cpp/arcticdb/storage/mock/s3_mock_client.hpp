...
private:
    ...
    std::optional<Aws::S3::S3Error> check_failure(const std::string& bucket_name) const;

    std::unique_ptr<S3ClientInterface> real_client_;
    std::map<S3Key, Segment> mock_storage_;
};
