...
class ConfigCache {
public:
  ConfigCache(const EnvironmentName& environment_name,
              const std::shared_ptr<ConfigResolver>& resolver)
      : environment_name_(environment_name), descriptor_map_(),
  ...
  void add_library_config(const LibraryPath& path,
  ...
  std::shared_ptr<Storages> create_storages(const LibraryPath& path, OpenMode mode) {
    ...
    util::check(!descriptor.storage_ids_.empty(),
                "Can't configure library with no storage ids");
...
private:
  ...
  std::unordered_map<StorageName, arcticdb::proto::storage::VariantStorage>
      storage_configs_;
