...
std::optional<InMemoryConfigResolver::MemoryConfig>
InMemoryConfigResolver::get_environment(const EnvironmentName& environment_name) const {
...
InMemoryConfigResolver::get_libraries(const EnvironmentName& environment_name) const {
...
InMemoryConfigResolver::get_storages(const EnvironmentName& environment_name) const {
  auto config = get_environment(environment_name);
...
void InMemoryConfigResolver::add_library(
  ...
  config.libraries_.try_emplace(LibraryPath::from_delim_path(library_descriptor.name()),
                                library_descriptor);
}
