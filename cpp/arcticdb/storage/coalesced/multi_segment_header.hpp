...
std::pair<size_t, size_t> get_offset_and_size(size_t pos,
  ...
  ARCTICDB_DEBUG(log::storage(),
                 "At pos {}, multi segment header found offset and size {}:{}", pos,
...
public:
  ...
  void initalize(StreamId id, size_t num_rows) {
    segment_ =
        SegmentInMemory{multi_segment_descriptor(std::move(id)), num_rows, false};
  }
  ...
  void add_key_and_offset(const AtomKey& key, uint64_t offset, uint64_t size) {
    ...
    ARCTICDB_DEBUG(log::storage(),
                   "Adding key {} with offset {} and {} bytes, time_sym: {}", key,
                   offset, size, time_sym);
  ...
  get_offset_for_key(const AtomKey& key) const {
    ...
    auto start_pos =
        std::lower_bound(time_symbol_column.begin<TimeSymbolTag>(),
                         time_symbol_column.end<TimeSymbolTag>(), time_symbol.data());
    if (start_pos == time_symbol_column.end<TimeSymbolTag>()) {
      ARCTICDB_DEBUG(log::storage(), "Reached end of column looking for symbol {}",
                     key);
      return std::nullopt;
    }

    ARCTICDB_DEBUG(log::storage(), "Start pos for time symbol {} is {}",
                   time_symbol.data(), start_pos.get_offset());
    const auto& creation_ts_column =
        segment_.column(as_pos(MultiSegmentFields::creation_ts));
    ...
    auto creation_ts_pos = std::lower_bound(
        creation_it, creation_ts_column.end<CreationTsTag>(), key.creation_ts());
    if (creation_ts_pos == creation_ts_column.end<CreationTsTag>()) {
      ARCTICDB_DEBUG(log::storage(), "Reached end of column looking for timestamp {}",
                     key.creation_ts());
      return std::nullopt;
    }

    ARCTICDB_DEBUG(log::storage(), "Starting at creation timestamp {} at offset {}",
                   *creation_ts_pos, creation_ts_pos.get_offset());
    while (*creation_ts_pos == static_cast<uint64_t>(key.creation_ts())) {
      const auto creation_ts_offset = creation_ts_pos.get_offset();
      if (const auto found_key =
              get_key<MultiSegmentFields>(creation_ts_offset, segment_);
          found_key == key) {
