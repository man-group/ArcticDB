...
/*
 * Contains similar functions to stream_utils.hpp but assumes that many keys are mixed
 * in together, so we can't guarantee that either the id types or the index types will
 * be the same, so we track those with a uint8_t column.
 */
namespace arcticdb {
...
template <typename StorageType> uint64_t get_symbol_prefix(const StreamId& stream_id) {
  ...
  util::variant_match(
      ...
      [&data](const NumericId& numeric_id) {
        util::check(numeric_id < static_cast<NumericId>(NumericMask),
                    "Numeric id too large: {}", numeric_id);
        data &= NumericFlag;
        data &= numeric_id;
      });
  return data;
...
template <typename FieldType> position_t as_pos(FieldType id_type) {
...
StreamId get_id(position_t pos, const SegmentInMemory& segment) {
  auto id_type =
      IdType(segment.scalar_at<uint8_t>(pos, as_pos(FieldType::id_type)).value());
  const auto id = segment.scalar_at<uint64_t>(pos, as_pos(FieldType::stream_id));
  switch (id_type) {
  case IdType::String:
    return StreamId{
        std::string{segment.const_string_pool().get_const_view(id.value())}};
  case IdType::Numeric:
...
IndexValue get_index(position_t pos, FieldType field, const SegmentInMemory& segment) {
  auto index_type = VariantType(
      segment.scalar_at<uint8_t>(pos, as_pos(FieldType::index_type)).value());
  const auto index = segment.scalar_at<uint64_t>(pos, as_pos(field));
  switch (index_type) {
  case VariantType::STRING_TYPE:
    return StreamId{
        std::string{segment.const_string_pool().get_const_view(index.value())}};
  case VariantType::NUMERIC_TYPE:
    return StreamId{static_cast<NumericId>(index.value())};
  default:
    util::raise_rte("Unknown index type {} in multi_segment",
...
entity::AtomKey get_key(position_t pos, const SegmentInMemory& segment) {
  ...
  const auto key_type_num =
      segment.scalar_at<int32_t>(pos, as_pos(FieldType::key_type)).value();
  const auto key_type = entity::key_type_from_int(key_type_num);
  const auto version_id =
      segment.scalar_at<uint64_t>(pos, as_pos(FieldType::version_id)).value();
  const auto content_hash =
      segment.scalar_at<uint64_t>(pos, as_pos(FieldType::content_hash)).value();
  const auto creation_ts =
      segment.scalar_at<uint64_t>(pos, as_pos(FieldType::creation_ts)).value();
...
void set_key(const AtomKey& key, SegmentInMemory& segment) {
  ...
  segment.set_scalar<int32_t>(as_pos(FieldType::key_type),
                              static_cast<int32_t>(key.type()));
}
