...
template <class It> using Range = folly::Range<It>;
...
inline void raise_s3_exception(const Aws::S3::S3Error& err,
  ...
  auto error_message_suffix = fmt::format(
      "S3Error#{} {}: {} for object '{}'", int(err.GetErrorType()),
      err.GetExceptionName().c_str(), err.GetMessage().c_str(), object_name);

  // s3_client.HeadObject returns RESOURCE_NOT_FOUND if a key is not found.
  if (type == Aws::S3::S3Errors::NO_SUCH_KEY ||
      type == Aws::S3::S3Errors::RESOURCE_NOT_FOUND) {
    throw KeyNotFoundException(
        fmt::format("Key Not Found Error: {}", error_message_suffix));
  }

  if (type == Aws::S3::S3Errors::ACCESS_DENIED ||
      type == Aws::S3::S3Errors::INVALID_ACCESS_KEY_ID ||
      type == Aws::S3::S3Errors::SIGNATURE_DOES_NOT_MATCH) {
    raise<ErrorCode::E_PERMISSION>(
        fmt::format("Permission error: {}", error_message_suffix));
  }

  if (err.ShouldRetry()) {
    raise<ErrorCode::E_S3_RETRYABLE>(
        fmt::format("Retry-able error: {}", error_message_suffix));
  }

  // We create a more detailed error explanation in case of NETWORK_CONNECTION errors to
  // remedy #880.
  if (type == Aws::S3::S3Errors::NETWORK_CONNECTION) {
    error_message = fmt::format(
        ...
        "Having more than one open Arctic instance is not advised, you should reuse "
        "them. "
        "If you absolutely need many open Arctic instances, consider increasing "
        "`ulimit -n`.",
...
void do_write_impl(Composite<KeySegmentPair>&& kvs, const std::string& root_folder,
                   const std::string& bucket_name, S3ClientWrapper& s3_client,
                   KeyBucketizer&& bucketizer) {
  ARCTICDB_SAMPLE(S3StorageWrite, 0)
  auto fmt_db = [](auto&& kv) { return kv.key_type(); };

  (fg::from(kvs.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&s3_client, &bucket_name, &root_folder,
                 b = std::move(bucketizer)](auto&& group) {
        ...
        for (auto& kv : group.values()) {
          ...
          auto put_object_result =
              s3_client.put_object(s3_object_name, std::move(seg), bucket_name);

          if (!put_object_result.is_success()) {
            auto& error = put_object_result.get_error();
            // No DuplicateKeyException is thrown because S3 overwrites the given key if
            // it already exists.
...
template <class KeyBucketizer>
void do_update_impl(Composite<KeySegmentPair>&& kvs, const std::string& root_folder,
                    const std::string& bucket_name, S3ClientWrapper& s3_client,
                    KeyBucketizer&& bucketizer) {
  // s3 updates the key if it already exists. We skip the check for key not found to
  // save a round-trip.
  do_write_impl(std::move(kvs), root_folder, bucket_name, s3_client,
                std::move(bucketizer));
}

template <class KeyBucketizer>
void do_read_impl(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                  const std::string& root_folder, const std::string& bucket_name,
                  const S3ClientWrapper& s3_client, KeyBucketizer&& bucketizer,
                  ReadKeyOpts opts) {
  ...
  (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&s3_client, &bucket_name, &root_folder, b = std::move(bucketizer),
                 &visitor, &keys_not_found, opts = opts](auto&& group) {
        for (auto& k : group.values()) {
          ...
          if (get_object_result.is_success()) {
            ...
            log::storage().log(opts.dont_warn_about_missing_key ? spdlog::level::debug
                                                                : spdlog::level::warn,
                               "Failed to find segment for key '{}' {}: {}",
                               variant_key_view(k), error.GetExceptionName().c_str(),
...
struct FailedDelete {
  ...
  FailedDelete(VariantKey&& failed_key, std::string&& error_message)
      : failed_key(failed_key), error_message(error_message) {}
};

template <class KeyBucketizer>
void do_remove_impl(Composite<VariantKey>&& ks, const std::string& root_folder,
                    const std::string& bucket_name, S3ClientWrapper& s3_client,
                    KeyBucketizer&& bucketizer) {
  ...
  static const size_t delete_object_limit =
      std::min(DELETE_OBJECTS_LIMIT,
               static_cast<size_t>(
                   ConfigsMap::instance()->get_int("S3Storage.DeleteBatchSize", 1000)));

  (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&s3_client, &root_folder, &bucket_name, &to_delete,
                 b = std::move(bucketizer), &failed_deletes](auto&& group) {
        auto key_type_dir = key_type_folder(root_folder, group.key());
        for (auto k : folly::enumerate(group.values())) {
          ...
          if (to_delete.size() == delete_object_limit || k.index + 1 == group.size()) {
            ...
            if (delete_object_result.is_success()) {
              ...
              std::string failed_objects =
                  fmt::format("{}", fmt::join(to_delete, ", "));
  ...
  if (!failed_deletes.empty()) {
    ...
    for (auto i = 0u; i < failed_deletes.size(); ++i) {
      ...
      failed_deletes_message << fmt::format("'{}' failed with '{}'",
                                            to_serialized_key(failed.failed_key),
...
inline auto default_prefix_handler() {
  return [](const std::string& prefix, const std::string& key_type_dir,
            const KeyDescriptor& key_descriptor, KeyType) {
    return !prefix.empty()
               ? fmt::format("{}/{}*{}", key_type_dir, key_descriptor, prefix)
               : key_type_dir;
  };
}

template <class Visitor, class KeyBucketizer, class PrefixHandler>
void do_iterate_type_impl(KeyType key_type, Visitor&& visitor,
  ...
  const auto path_to_key_size =
      key_type_dir.size() + 1 + bucketizer.bucketize_length(key_type);
  // if prefix is empty, add / to avoid matching both log and logc when key_type_dir is
  // {root_folder}/log
  if (prefix.empty()) {
    key_type_dir += "/";
  }

  // Generally we get the key descriptor from the AtomKey, but in the case of iterating
  // version journals where we want to have a narrower prefix, we can use the info that
  // it's a version journal and derive the Descriptor.
  // TODO: Set the IndexDescriptorImpl correctly
  KeyDescriptor key_descriptor(prefix,
                               is_ref_key_class(key_type)
                                   ? IndexDescriptorImpl::Type::UNKNOWN
  ...
  do {
    auto list_objects_result =
        s3_client.list_objects(key_prefix, bucket_name, continuation_token);
    ...
    if (list_objects_result.is_success()) {
      ...
      for (auto& s3_object_name : output.s3_object_names) {
        ...
        ARCTICDB_TRACE(log::version(), "Got object_list: {}, key: {}", s3_object_name,
                       key);
        auto k = variant_key_from_bytes(reinterpret_cast<uint8_t*>(key.data()),
                                        key.size(), key_type);
      ...
      log::storage().warn("Failed to iterate key type with key '{}' {}: {}", key_type,
                          error.GetExceptionName().c_str(), error.GetMessage().c_str());
      // We don't raise on expected errors like NoSuchKey because we want to return an
...
template <class KeyBucketizer>
bool do_key_exists_impl(const VariantKey& key, const std::string& root_folder,
                        const std::string& bucket_name,
                        const S3ClientWrapper& s3_client, KeyBucketizer&& b) {
  ...
  if (!head_object_result.is_success()) {
    ...
    ARCTICDB_DEBUG(log::storage(), "Head object returned false for key {} {} {}:{}",
                   variant_key_view(key), int(error.GetErrorType()),
                   error.GetExceptionName().c_str(), error.GetMessage().c_str());
  }
