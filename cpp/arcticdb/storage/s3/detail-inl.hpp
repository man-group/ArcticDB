...
inline void raise_s3_exception(const Aws::S3::S3Error& err, const std::string& object_name) {
    ...
    if (err.ShouldRetry()) {
        raise<ErrorCode::E_S3_RETRYABLE>(fmt::format("Retry-able error: {}", error_message_suffix));
    }
...
void do_read_impl(
    ...
    (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
        .foreach (
            [&s3_client, &bucket_name, &root_folder, b = std::move(bucketizer), &visitor, &keys_not_found, opts = opts](
                auto&& group
            ) {
...
void do_remove_impl(
    ...
    static const size_t delete_object_limit = std::min(
        DELETE_OBJECTS_LIMIT, static_cast<size_t>(ConfigsMap::instance()->get_int("S3Storage.DeleteBatchSize", 1000))
    );

    (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
        .foreach ([&s3_client, &root_folder, &bucket_name, &to_delete, b = std::move(bucketizer), &failed_deletes](
                      auto&& group
                  ) {
            auto key_type_dir = key_type_folder(root_folder, group.key());
...
bool do_iterate_type_impl(
    ...
    ARCTICDB_RUNTIME_DEBUG(
        log::storage(), "Searching for objects in bucket {} with prefix {}", bucket_name, key_prefix
    );
