...
RealS3Client::head_object(const std::string& s3_object_name,
                          const std::string& bucket_name) const {

  ARCTICDB_RUNTIME_DEBUG(log::storage(), "Looking for head of object {}",
                         s3_object_name);
...
protected:
  std::streamsize xsputn(const char_type* s, std::streamsize n) override {
    ARCTICDB_TRACE(log::version(), "xsputn {} pos at {}, {} bytes",
                   uintptr_t(buffer_.get()), pos_, n);
    if (buffer_->bytes() < pos_ + n) {
      ARCTICDB_TRACE(log::version(), "{} Calling ensure for {}",
                     uintptr_t(buffer_.get()), (pos_ + n) * 2);
    ...
    memcpy(target, s, n);
    pos_ += n;
    ARCTICDB_TRACE(log::version(), "{} pos is now {}, returning {}",
                   uintptr_t(buffer_.get()), pos_, n);
    return n;
  }
...
S3Result<Segment> RealS3Client::get_object(const std::string& s3_object_name,
                                           const std::string& bucket_name) const {
...
S3Result<std::monostate> RealS3Client::put_object(const std::string& s3_object_name,
                                                  Segment&& segment,
  ...
  ARCTICDB_RUNTIME_DEBUG(log::storage(), "Wrote key '{}', with {} bytes of data",
                         s3_object_name, segment.size());
  return {std::monostate()};
}

S3Result<DeleteOutput>
RealS3Client::delete_objects(const std::vector<std::string>& s3_object_names,
                             const std::string& bucket_name) {
  ...
  for (auto& s3_object_name : s3_object_names) {
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "Removing s3 object with key {}",
                           s3_object_name);
    del_objects.AddObjects(
        Aws::S3::Model::ObjectIdentifier().WithKey(s3_object_name.c_str()));
  }
  ...
  // AN-256: Per AWS S3 documentation, deleting non-exist objects is not an error, so
  // not handling RemoveOpts.ignores_missing_key_
...
RealS3Client::list_objects(const std::string& name_prefix,
                           const std::string& bucket_name,
