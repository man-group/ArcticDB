...
std::string S3Storage::get_key_path(const VariantKey& key) const {
  ...
  // FUTURE: if we can reuse `detail::do_*` below doing polymorphism instead of
  // templating, then this method is useful to most of them
}

void S3Storage::do_write(Composite<KeySegmentPair>&& kvs) {
  detail::do_write_impl(std::move(kvs), root_folder_, bucket_name_, *s3_client_,
                        FlatBucketizer{});
}

void S3Storage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts) {
  detail::do_update_impl(std::move(kvs), root_folder_, bucket_name_, *s3_client_,
                         FlatBucketizer{});
}

void S3Storage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                        ReadKeyOpts opts) {
  detail::do_read_impl(std::move(ks), visitor, root_folder_, bucket_name_, *s3_client_,
                       FlatBucketizer{}, opts);
}

void S3Storage::do_remove(Composite<VariantKey>&& ks, RemoveOpts) {
  detail::do_remove_impl(std::move(ks), root_folder_, bucket_name_, *s3_client_,
                         FlatBucketizer{});
}

void S3Storage::do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
  ...
  detail::do_iterate_type_impl(key_type, visitor, root_folder_, bucket_name_,
                               *s3_client_, FlatBucketizer{}, std::move(prefix_handler),
                               prefix);
}
...
S3Storage::S3Storage(const LibraryPath& library_path, OpenMode mode, const Config& conf)
    : Storage(library_path, mode), s3_api_(S3ApiInstance::instance()),
  ...
  if (conf.use_mock_storage_for_testing()) {
    ...
    s3_client_ = std::make_unique<RealS3Client>(
        get_s3_config(conf), Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never,
        conf.use_virtual_addressing());
  } else {
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "Using provided auth credentials");
    s3_client_ = std::make_unique<RealS3Client>(
        creds, get_s3_config(conf),
  ...
  if (!conf.prefix().empty()) {
    ...
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "S3 prefix not found, will use {}",
                           root_folder_);
  }
