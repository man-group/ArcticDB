...
KeySegmentPair S3Storage::do_read(VariantKey&& variant_key, ReadKeyOpts opts) {
    ...
    return detail::do_read_impl(
            std::move(variant_key), root_folder_, bucket_name_, client(), FlatBucketizer{}, std::move(identity), opts
    );
}

folly::Future<folly::Unit> S3Storage::do_async_read(
        entity::VariantKey&& variant_key, const ReadVisitor& visitor, ReadKeyOpts opts
) {
    auto identity = [](auto&& k) { return k; };
    return detail::do_async_read_impl(
                   std::move(variant_key),
                   root_folder_,
                   bucket_name_,
                   client(),
            ...
            .thenValue([&visitor](auto&& key_seg) {
...
bool S3Storage::do_iterate_type_until_match(
    ...
    return detail::do_iterate_type_impl(
            key_type, visitor, root_folder_, bucket_name_, client(), FlatBucketizer{}, std::move(prefix_handler), prefix
    );
}
...
void S3Storage::create_s3_client(const S3Settings& conf, const Aws::Auth::AWSCredentials& creds) {
    if (conf.use_mock_storage_for_testing()) {
        ...
        auto sts_client_factory = [conf, this](const Aws::Auth::AWSCredentials& creds) { // Get default allocation tag
            ...
            sts_client_ = std::make_unique<Aws::STS::STSClient>(
                    creds, Aws::MakeShared<Aws::STS::Endpoint::STSEndpointProvider>(allocation_tag), sts_config
            );
        ...
        auto cred_provider = Aws::MakeShared<Aws::Auth::STSProfileCredentialsProvider>(
                ...
                conf.aws_profile(),
                std::chrono::minutes(
                        static_cast<size_t>(ConfigsMap::instance()->get_int("S3Storage.STSTokenExpiryMin", 60))
                ),
                sts_client_factory
        );
...
GCPXMLStorage::GCPXMLStorage(
        const arcticdb::storage::LibraryPath& lib, arcticdb::storage::OpenMode mode,
