...
std::string MockS3Client::get_failure_trigger(const std::string& s3_object_name,
                                              StorageOperation operation_to_fail,
                                              Aws::S3::S3Errors error_to_fail_with,
                                              bool retryable) {
  return fmt::format("{}#Failure_{}_{}_{}", s3_object_name,
                     operation_to_string(operation_to_fail),
                     static_cast<int>(error_to_fail_with), static_cast<int>(retryable));
}

std::optional<Aws::S3::S3Error> has_failure_trigger(const std::string& s3_object_name,
  ...
  try {
    ...
    auto failure_code_string =
        s3_object_name.substr(start, s3_object_name.find_last_of('_') - start);
    auto failure_code = Aws::S3::S3Errors(std::stoi(failure_code_string));
    bool retryable =
        std::stoi(s3_object_name.substr(s3_object_name.find_last_of('_') + 1));
    return Aws::S3::S3Error(Aws::Client::AWSError<Aws::S3::S3Errors>(
        failure_code, "Simulated error",
...
const auto not_found_error = Aws::S3::S3Error(Aws::Client::AWSError<Aws::S3::S3Errors>(
    Aws::S3::S3Errors::RESOURCE_NOT_FOUND, false));

S3Result<std::monostate>
MockS3Client::head_object(const std::string& s3_object_name,
...
S3Result<Segment> MockS3Client::get_object(const std::string& s3_object_name,
                                           const std::string& bucket_name) const {
...
S3Result<std::monostate> MockS3Client::put_object(const std::string& s3_object_name,
                                                  Segment&& segment,
...
S3Result<DeleteOutput>
MockS3Client::delete_objects(const std::vector<std::string>& s3_object_names,
                             const std::string& bucket_name) {
  ...
  for (auto& s3_object_name : s3_object_names) {
    auto maybe_error =
        has_failure_trigger(s3_object_name, StorageOperation::DELETE_LOCAL);
    if (maybe_error.has_value()) {
...
MockS3Client::list_objects(const std::string& name_prefix,
                           const std::string& bucket_name,
  ...
  for (auto& key : s3_contents) {
    if (key.first.bucket_name == bucket_name &&
        key.first.s3_object_name.rfind(name_prefix, 0) == 0) {
