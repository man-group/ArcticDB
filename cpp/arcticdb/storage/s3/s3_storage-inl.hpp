...
void do_write_impl(
    ...
    (fg::from(kvs.as_range()) | fg::move | fg::groupBy(fmt_db)).foreach(
        ...
        for (auto& kv : group.values()) {
            ...
            auto s3_object_name = object_store_utils::object_path(b.bucketize(key_type_folder, k), k);
            auto& seg = kv.segment();
...
auto get_object(
    ...
    auto s3_object_name = object_store_utils::object_path(b.bucketize(key_type_folder, key), key);
...
inline std::string S3Storage::get_key_path(const VariantKey& key) const {
    auto b = object_store_utils::FlatBucketizer{};
    auto key_type_folder = object_store_utils::key_type_folder(root_folder_, variant_key_type(key));
    return object_store_utils::object_path(b.bucketize(key_type_folder, key), key);
...
inline void S3Storage::do_write(Composite<KeySegmentPair>&& kvs) {
    detail::do_write_impl(std::move(kvs), root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{});
}

inline void S3Storage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts) {
    detail::do_update_impl(std::move(kvs), root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{});
}

template<class Visitor>
void S3Storage::do_read(Composite<VariantKey>&& ks, Visitor&& visitor, ReadKeyOpts opts) {
    detail::do_read_impl(std::move(ks), std::move(visitor), root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{}, opts);
}

inline void S3Storage::do_remove(Composite<VariantKey>&& ks, RemoveOpts) {
    detail::do_remove_impl(std::move(ks), root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{});
}
...
void S3Storage::do_iterate_type(KeyType key_type, Visitor&& visitor, const std::string& prefix) {
    ...
    detail::do_iterate_type_impl(key_type, std::move(visitor), root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{}, std::move(prefix_handler), prefix);
}

inline bool S3Storage::do_key_exists(const VariantKey& key) {
    return detail::do_key_exists_impl(key, root_folder_, bucket_name_, s3_client_, object_store_utils::FlatBucketizer{});
}
