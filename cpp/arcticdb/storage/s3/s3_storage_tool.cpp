...
void S3StorageTool::iterate_bucket(Visitor&& visitor, const std::string& prefix) {
  ...
  do {
    ...
    if (list_objects_outcome.IsSuccess()) {
      ...
      log::storage().error("Failed to iterate bucket '{}' {}:{}", bucket_name_,
...
void S3StorageTool::set_object(const std::string& key, const std::string& data) {
  ...
  if (!put_object_outcome.IsSuccess()) {
    ...
    util::raise_rte("Failed to write s3 with key '{}' {}: {}", key,
                    error.GetExceptionName().c_str(), error.GetMessage().c_str());
  }
  ARCTICDB_DEBUG(log::storage(), "Wrote key {} with {} bytes of data", key,
                 data.size());
}
...
std::string S3StorageTool::get_object(const std::string& key) {
  ...
  if (get_object_outcome.IsSuccess()) {
    ...
    log::storage().warn("Failed to find data for key '{}' {}: {}", key,
                        error.GetExceptionName().c_str(), error.GetMessage().c_str());
    return std::string();
  }
}

void S3StorageTool::delete_object(const std::string& key) {
  ...
  if (delete_object_outcome.IsSuccess())
    ...
    log::storage().warn("Failed to delete segment with key '{}': {}", key,
                        error.GetExceptionName().c_str(), error.GetMessage().c_str());
  }
}

std::vector<std::string> S3StorageTool::list_bucket(const std::string& prefix) {
  ...
  do {
    ...
    if (list_objects_outcome.IsSuccess()) {
      ...
      log::storage().error("Failed to iterate bucket to get sizes'{}' {}:{}",
                           bucket_name_, error.GetExceptionName().c_str(),
                           error.GetMessage().c_str());
      return {0, 0};
    }
  } while (more);
  ...
  iterate_bucket(
      [&](const std::string& key) {
        ...
        if (delete_object_outcome.IsSuccess())
          ...
          const auto& error = delete_object_outcome.GetError();
          log::storage().warn("Failed to delete object with key '{}' {}:{}", key,
                              error.GetExceptionName().c_str(),
                              error.GetMessage().c_str());
        }
      },
...
S3StorageTool::S3StorageTool(const Config& conf)
    : s3_api_(S3ApiInstance::instance()),
      s3_client_(get_aws_credentials(conf), get_s3_config(conf),
