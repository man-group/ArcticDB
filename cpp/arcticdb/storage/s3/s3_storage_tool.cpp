...
void S3StorageTool::set_object(const std::string& key, const std::string& data) {
    ...
    if (!put_object_outcome.IsSuccess()) {
        ...
        util::raise_rte(
            "Failed to write s3 with key '{}' {}: {}", key, error.GetExceptionName().c_str(), error.GetMessage().c_str()
        );
    }
...
std::string S3StorageTool::get_object(const std::string& key) {
    ...
    if (get_object_outcome.IsSuccess()) {
        ...
        log::storage().warn(
            "Failed to find data for key '{}' {}: {}", key, error.GetExceptionName().c_str(), error.GetMessage().c_str()
        );
        return std::string();
    }
}
