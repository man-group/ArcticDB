...
bool is_running_inside_aws_fast(){
    ...
    if (has_connection_to_ec2_imds()){
        ARCTICDB_RUNTIME_DEBUG(log::storage(),
                               "Fast check determined we're running inside AWS because we managed to connect to the instance metadata service.");
        return true;
    }else{
        ARCTICDB_RUNTIME_DEBUG(log::storage(),
                               "Fast check determined we're NOT running inside AWS because we didn't find aws env vars and couldn't connect to instance metadata service");
        return false;
    }
}
}
