 ...
 */
 ...
 S3ApiInstance::S3ApiInstance(Aws::Utils::Logging::LogLevel log_level) : log_level_(log_level), options_() {
     ...
     // Set AWS_EC2_METADATA_DISABLED BEFORE InitAPI if we're not in AWS to skip 200ms IMDS timeout.
     // Use standard getenv (not AWS SDK functions) since SDK isn't initialized yet.
     auto is_running_inside_aws = is_running_inside_aws_fast();
     if (!is_running_inside_aws) {
         ARCTICDB_RUNTIME_DEBUG(log::storage(), "Does not appear to be using AWS. Will set AWS_EC2_METADATA_DISABLED before InitAPI");
         #ifdef WIN32
