...
S3ApiInstance::S3ApiInstance(Aws::Utils::Logging::LogLevel log_level)
    : log_level_(log_level), options_() {
  // Use correct URI encoding rather than legacy compat one in AWS SDK. PURE S3 needs
  // this to handle symbol names that have special characters (eg ':').
  options_.httpOptions.compliantRfc3986Encoding = true;

  if (log_level_ > Aws::Utils::Logging::LogLevel::Off) {
    Aws::Utils::Logging::InitializeAWSLogging(
        Aws::MakeShared<Aws::Utils::Logging::DefaultLogSystem>("v", log_level,
                                                               "aws_sdk_"));
  }
...
S3ApiInstance::~S3ApiInstance() {
  ...
  // Aws::ShutdownAPI(options_); This causes a crash on shutdown in
  // Aws::CleanupMonitoring
}

void S3ApiInstance::init() {
  auto log_level = ConfigsMap::instance()->get_int("AWS.LogLevel", 0);
  S3ApiInstance::instance_ =
      std::make_shared<S3ApiInstance>(Aws::Utils::Logging::LogLevel(log_level));
}
