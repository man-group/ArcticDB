...
std::string remove_suffix_char(const std::string& str) { return str.substr(0, str.size() - 1); }
...
bool NfsBackedStorage::do_iterate_type_until_match(
    ...
    auto prefix_handler = [](const std::string& prefix,
        ...
        if (!prefix.empty()) {
            uint32_t id =
                get_id_bucket(encode_item<StreamId, StringId, NumericId>(StringId{prefix}, is_ref_key_class(key_type)));
    ...
    return s3::detail::do_iterate_type_impl(
        key_type, func, root_folder_, bucket_name_, *s3_client_, NfsBucketizer{}, prefix_handler, prefix
    );
}
