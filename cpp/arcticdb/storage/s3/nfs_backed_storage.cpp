...
MixedType decode_item(const MixedType& input, bool remove_suffix) {
  return util::variant_match(
      ...
      [remove_suffix](const StringType& str) {
        return MixedType{
            util::safe_decode(remove_suffix ? remove_suffix_char(str) : str)};
      },
      [](const NumericType& id) { return MixedType{id}; });
}
...
VariantKey encode_object_id(const VariantKey& key) {
  return util::variant_match(
      ...
      [](const AtomKey& k) {
        ...
        auto start_index =
            encode_item<IndexValue, StringIndex, NumericIndex>(k.start_index(), false);
        auto end_index =
            encode_item<IndexValue, StringIndex, NumericIndex>(k.end_index(), false);
...
uint32_t id_to_number(const StreamId& stream_id) {
  return util::variant_match(
      stream_id, [](const NumericId& num_id) { return static_cast<uint32_t>(num_id); },
...
std::string get_root_folder(const std::string& root_folder, const VariantKey& vk) {
  return util::variant_match(
      vk, [&root_folder](const auto& k) { return get_root_folder(root_folder, k); });
}

std::string NfsBucketizer::bucketize(const std::string& root_folder,
                                     const VariantKey& vk) {
...
VariantKey unencode_object_id(const VariantKey& key) {
  ...
  return util::variant_match(
      ...
      [](const AtomKey& k) {
        ...
        auto start_index =
            decode_item<IndexValue, StringIndex, NumericIndex>(k.start_index(), false);
        auto end_index =
            decode_item<IndexValue, StringIndex, NumericIndex>(k.end_index(), false);
        return VariantKey{atom_key_builder()
                              .version_id(k.version_id())
                              .start_index(start_index)
                              .end_index(end_index)
...
std::string NfsBackedStorage::name() const {
  return fmt::format("nfs_backed_storage-{}/{}/{}", region_, bucket_name_,
                     root_folder_);
}

void NfsBackedStorage::do_write(Composite<KeySegmentPair>&& kvs) {
  auto enc = kvs.transform([](auto&& key_seg) {
    return KeySegmentPair{encode_object_id(key_seg.variant_key()),
                          key_seg.segment_ptr()};
  });
  s3::detail::do_write_impl(std::move(enc), root_folder_, bucket_name_, *s3_client_,
                            NfsBucketizer{});
}
...
void NfsBackedStorage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts) {
  ...
  s3::detail::do_update_impl(std::move(enc), root_folder_, bucket_name_, *s3_client_,
                             NfsBucketizer{});
}

void NfsBackedStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                               ReadKeyOpts opts) {
  ...
  s3::detail::do_read_impl(std::move(enc), func, root_folder_, bucket_name_,
                           *s3_client_, NfsBucketizer{}, opts);
}

void NfsBackedStorage::do_remove(Composite<VariantKey>&& ks, RemoveOpts) {
  auto enc = ks.transform([](auto&& key) { return encode_object_id(key); });
  s3::detail::do_remove_impl(std::move(enc), root_folder_, bucket_name_, *s3_client_,
                             NfsBucketizer{});
}

void NfsBackedStorage::do_iterate_type(KeyType key_type,
  ...
  auto prefix_handler = [](const std::string& prefix, const std::string& key_type_dir,
                           const KeyDescriptor&, KeyType key_type) {
    std::string new_prefix;
    if (!prefix.empty()) {
      uint32_t id = get_id_bucket(encode_item<StreamId, StringId, NumericId>(
          StringId{prefix}, is_ref_key_class(key_type)));
      new_prefix = fmt::format("{:03}", id);
    }

    return !prefix.empty() ? fmt::format("{}/{}", key_type_dir, new_prefix)
                           : key_type_dir;
  };

  s3::detail::do_iterate_type_impl(key_type, std::move(func), root_folder_,
                                   bucket_name_, *s3_client_, NfsBucketizer{},
                                   prefix_handler, prefix);
}

bool NfsBackedStorage::do_key_exists(const VariantKey& key) {
  auto encoded_key = encode_object_id(key);
  return s3::detail::do_key_exists_impl(encoded_key, root_folder_, bucket_name_,
                                        *s3_client_, NfsBucketizer{});
}

NfsBackedStorage::NfsBackedStorage(const LibraryPath& library_path, OpenMode mode,
                                   const Config& conf)
    : Storage(library_path, mode),
      root_folder_(object_store_utils::get_root_folder(library_path)),
      bucket_name_(conf.bucket_name()), region_(conf.region()) {
  ...
  if (conf.use_mock_storage_for_testing()) {
    ...
    s3_client_ = std::make_unique<s3::RealS3Client>(
        s3::get_aws_credentials(conf), s3::get_s3_config(conf),
  ...
  // When linking against libraries built with pre-GCC5 compilers, the num_put facet is
  // not initalized on the classic locale Rather than change the locale globally, which
  // might cause unexpected behaviour in legacy code, just add the required facet here
