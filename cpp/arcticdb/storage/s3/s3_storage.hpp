...
private:
  ...
  void do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
               ReadKeyOpts opts) final;

  void do_remove(Composite<VariantKey>&& ks, RemoveOpts opts) final;

  void do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
  ...
  std::string do_key_path(const VariantKey& key) const final {
...
inline arcticdb::proto::storage::VariantStorage
pack_config(const std::string& bucket_name) {
...
inline arcticdb::proto::storage::VariantStorage
pack_config(const std::string& bucket_name, const std::string& credential_name,
            const std::string& credential_key, const std::string& endpoint) {
  arcticdb::proto::storage::VariantStorage output;
  arcticdb::proto::s3_storage::Config cfg;
  cfg.set_bucket_name(bucket_name);
...
inline std::optional<Aws::Client::ClientConfiguration>
parse_proxy_env_var(Aws::Http::Scheme endpoint_scheme, const char* opt_env_var) {
  ...
  if (port_start_idx == std::string::npos) {
    ...
    try {
      port =
          std::stoul(std::string(env_var.substr(port_start_idx + 1, env_var.length())));
    } catch (const std::logic_error& e) {
      log::storage().warn("Failed to parse port in '{}': {}", env_var, e.what());
      return std::nullopt;
    }
    if (port > std::numeric_limits<uint16_t>::max()) {
      log::storage().warn("Failed to parse '{}': port {} > {}", env_var, port,
...
inline std::optional<Aws::Utils::Array<Aws::String>>
parse_no_proxy_env_var(const char* opt_env_var) {
  if (opt_env_var == nullptr) {
    return std::nullopt;
  }
...
/* Use the environment variables http_proxy, https_proxy, no_proxy, and upper-case
 * versions thereof, to configure the proxy. Lower-case environment variables take
 * precedence over upper-case in keeping with curl etc:
 * https://about.gitlab.com/blog/2021/01/27/we-need-to-talk-no-proxy/
 * Protocol, username, password, and port are all optional, and will default to http,
 * empty, empty and 80/443 (for http and https) respectively. Maximal and minimal
 * example valid environment variables: http://username:password@my-proxy.com:8080
 * my-proxy.com => http://my-proxy.com:80 (no authentication)
 * no_proxy and it's uppercase equivalent should be a comma-separated list of hosts not
 * to apply other proxy settings to e.g. no_proxy="host-1.com,host-2.com"
 * */
inline Aws::Client::ClientConfiguration
get_proxy_config(Aws::Http::Scheme endpoint_scheme) {
  // The ordering in the vectors matter, lowercase should be checked in preference of
  // upper case.
  const std::unordered_map<Aws::Http::Scheme, std::vector<std::string>>
      scheme_env_var_names{{Aws::Http::Scheme::HTTP, {"http_proxy", "HTTP_PROXY"}},
  ...
  if (client_configuration.has_value()) {
    for (const auto& env_var_name : {"no_proxy", "NO_PROXY"}) {
      ...
      if (non_proxy_hosts) {
        ...
        log::storage().info("S3 proxy exclusion list set from env var '{}'",
                            opt_env_var);
        break;
      }
    }
...
template <typename ConfigType> auto get_s3_config(const ConfigType& conf) {
  auto endpoint_scheme =
      conf.https() ? Aws::Http::Scheme::HTTPS : Aws::Http::Scheme::HTTP;
  Aws::Client::ClientConfiguration client_configuration =
      get_proxy_config(endpoint_scheme);
  client_configuration.scheme = endpoint_scheme;
  ARCTICDB_RUNTIME_DEBUG(log::storage(), "Endpoint Scheme: {}",
  ...
  client_configuration.requestTimeoutMs =
...
Aws::Auth::AWSCredentials get_aws_credentials(const ConfigType& conf) {
  return Aws::Auth::AWSCredentials(conf.credential_name().c_str(),
                                   conf.credential_key().c_str());
}
