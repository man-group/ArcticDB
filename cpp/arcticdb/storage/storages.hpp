...
/*
 * The Storages class abstracts over multiple physical stores and controls how ArcticDB
 * read and write to multiple ones.
 *
 * Possible future use-case for Storages:
 *  - Disaster recovery for Storages: if the first storage fails we can fall back to the
 * second one, etc.
 *  - Tiered storage: recent data goes to a fast, comparatively expensive storage and
 * then is gradually moved into a slower, cheaper one.
 */
class Storages {
public:
  ...
  auto read(Composite<VariantKey>&& ks, const ReadVisitor& visitor, ReadKeyOpts opts,
            bool primary_only = true) {
    ARCTICDB_RUNTIME_SAMPLE(StoragesRead, 0)
    if (primary_only)
      return primary().read(std::move(ks), visitor, opts);

    if (auto rg = ks.as_range();
        !std::all_of(std::begin(rg), std::end(rg), [](const auto& vk) {
    ...
    for (const auto& storage : storages_) {
      try {
        ...
        ARCTICDB_DEBUG(log::version(),
                       "Keys not found in storage, continuing to next storage");
        ks = std::move(ex.keys());
      }
    }
  ...
  /** Calls Storage::do_key_path on the primary storage. Remember to check the open
   * mode. */
...
inline std::shared_ptr<Storages>
create_storages(const LibraryPath& library_path, OpenMode mode,
