...
class Storages {
  public:
    ...
    void write_if_none(KeySegmentPair& kv) { primary().write_if_none(kv); }
    ...
    [[nodiscard]] bool supports_atomic_writes() { return primary().supports_atomic_writes(); }

    [[nodiscard]] bool supports_object_size_calculation() {
        return std::all_of(storages_.begin(), storages_.end(), [](const auto& storage) {
            return storage->supports_object_size_calculation();
        });
    }
    ...
    void read_sync(
            const VariantKey& variant_key, const ReadVisitor& visitor, ReadKeyOpts opts, bool primary_only = true
    ) {
    ...
    static folly::Future<folly::Unit> async_read(
            Storage& storage, VariantKey&& variant_key, const ReadVisitor& visitor, ReadKeyOpts opts
    ) {
    ...
    folly::Future<folly::Unit> read(
            VariantKey&& variant_key, const ReadVisitor& visitor, ReadKeyOpts opts, bool primary_only = true
    ) {
        ARCTICDB_RUNTIME_SAMPLE(StoragesRead, 0)
        if (primary_only || variant_key_type(variant_key) != KeyType::TABLE_DATA)
    ...
    void iterate_type(
            KeyType key_type, const IterateTypeVisitor& visitor, const std::string& prefix = std::string{},
    ...
    void visit_object_sizes(
            KeyType key_type, const std::string& prefix, const ObjectSizesVisitor& visitor, bool primary_only = true
    ) {
        if (primary_only) {
    ...
    [[nodiscard]] std::string key_path(const VariantKey& key) const { return primary().key_path(key); }

    void remove(VariantKey&& variant_key, storage::RemoveOpts opts) { primary().remove(std::move(variant_key), opts); }
