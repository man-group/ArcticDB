...
void MemoryStorage::do_write(KeySegmentPair &&key_seg) {
    ...
    util::variant_match(key_seg.variant_key(),
        [&](const RefKey &key) {
...
void MemoryStorage::do_remove(VariantKey&& variant_key, RemoveOpts opts) {
    ...
    if (it != key_vec.end()) {
        ARCTICDB_DEBUG(log::storage(), "Removed key {}: {}", variant_key_type(variant_key), variant_key_view(variant_key));
        key_vec.erase(it);
...
bool MemoryStorage::do_iterate_type_until_match(KeyType key_type,
