...
public:
  Library(LibraryPath path, std::shared_ptr<Storages>&& storages,
      ...
      : library_path_(std::move(path)), storages_(std::move(storages)),
        config_(std::move(cfg)) {
    ARCTICDB_DEBUG(log::storage(), fmt::format("Opened library {}", library_path()));
    util::variant_match(
        config_,
        [that =
             this](const arcticdb::proto::storage::VersionStoreConfig& version_config) {
  ...
  /**
   * Tries to get every key of the given type (and prefix if not empty). Please assume
   * this can skip keys sometimes and code defensively.
  ...
  std::string key_path(const VariantKey& key) const { return storages_->key_path(key); }
...
inline std::shared_ptr<Library> create_library(
  ...
  return std::make_shared<Library>(
      library_path, create_storages(library_path, mode, storage_configs));
}
