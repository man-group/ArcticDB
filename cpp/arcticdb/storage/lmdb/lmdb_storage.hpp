...
private:
  ...
  void do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
               storage::ReadKeyOpts opts) final;
  ...
  std::vector<VariantKey> do_remove_internal(Composite<VariantKey>&& ks,
                                             ::lmdb::txn& txn, RemoveOpts opts);
  ...
  // Opening an LMDB env over the same path twice in the same process is unsafe, so we
