...
void raise_if_has_failure_trigger(const LmdbKey& key, StorageOperation operation) {
    ...
    try {
        ...
        auto error_message = fmt::format(
            "Simulated Error, message: operation {}, error code {}", operation_to_string(operation), error_code
        );
...
void raise_key_exists_error(std::string_view lmdb_op) { ::lmdb::error::raise(lmdb_op.data(), MDB_KEYEXIST); }

bool MockLmdbClient::has_key(const LmdbKey& key) const { return lmdb_contents_.find(key) != lmdb_contents_.end(); }
...
std::vector<VariantKey> MockLmdbClient::list(
    ...
    for (const auto& [key, segment] : lmdb_contents_) {
        if (key.db_name_ == db_name && util::string_starts_with(prefix, key.path_)) {
            ...
            auto k =
                variant_key_from_bytes(reinterpret_cast<const uint8_t*>(key.path_.data()), key.path_.size(), key_type);
