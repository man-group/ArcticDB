...
::lmdb::env& LmdbStorage::env() {
    storage::check<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(
        lmdb_instance_,
        "Unexpected LMDB Error: Invalid operation: LMDB environment has been removed. Possibly because the library has "
        "been deleted"
    );
    ...
    for (const auto& file : files) {
        ...
        try {
            ...
            raise<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(fmt::format(
                "Unexpected LMDB Error: Failed to remove LMDB file at path: {} error: {}", file_path.string(), e.what()
            ));
        }
    }
...
LmdbStorage::LmdbStorage(const LibraryPath& library_path, OpenMode mode, const Config& conf)
    ...
    if (!fs::exists(lib_dir_)) {
        util::check_arg(
            mode > OpenMode::READ, "Missing dir {} for lib={}. mode={}", lib_dir_.generic_string(), lib_path_str, mode
        );
