...
void raise_lmdb_exception(const ::lmdb::error& e, const std::string& object_name) {
  ...
  auto error_message_suffix =
      fmt::format("LMDBError#{}: {} for object {}", error_code, e.what(), object_name);
  ...
  if (error_code == MDB_KEYEXIST) {
    throw DuplicateKeyException(
        fmt::format("Duplicate Key Error: {}", error_message_suffix));
  }
  ...
  raise<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(
      fmt::format("Unexpected LMDB Error: {}", error_message_suffix));
}

::lmdb::env& LmdbStorage::env() {
  if (!env_) {
    raise<ErrorCode::E_UNEXPECTED_LMDB_ERROR>(
        "Unexpected LMDB Error: Invalid operation: LMDB environment has been removed. "
  ...
  (fg::from(kvs.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&](auto&& group) {
        ...
        for (auto& kv : group.values()) {
          ARCTICDB_DEBUG(log::storage(), "Lmdb storage writing segment with key {}",
                         kv.key_view());
          auto k = to_serialized_key(kv.variant_key());
          auto& seg = kv.segment();
          int64_t overwrite_flag =
              std::holds_alternative<RefKey>(kv.variant_key()) ? 0 : MDB_NOOVERWRITE;
          try {
...
void LmdbStorage::do_write(Composite<KeySegmentPair>&& kvs) {
  ...
  auto txn =
      ::lmdb::txn::begin(env()); // scoped abort on exception, so no partial writes
...
void LmdbStorage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts opts) {
  ...
  if (!failed_deletes.empty()) {
    ...
    std::string err_message =
        fmt::format("do_update called with upsert=false on non-existent key(s): {}",
                    failed_deletes);
...
void LmdbStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                          storage::ReadKeyOpts) {
  ARCTICDB_SAMPLE(LmdbStorageRead, 0)
  auto txn =
      std::make_shared<::lmdb::txn>(::lmdb::txn::begin(env(), nullptr, MDB_RDONLY));
        ...
        for (auto& k : group.values()) {
          ...
          try {
            ...
            if (segment.has_value()) {
              ...
              ARCTICDB_DEBUG(log::storage(), "Read key {}: {}, with {} bytes of data",
                             variant_key_type(k), variant_key_view(k),
                             segment.value().size());
              visitor(k, std::move(segment.value()));
            } else {
              ARCTICDB_DEBUG(log::storage(), "Failed to find segment for key {}",
                             variant_key_view(k));
...
std::vector<VariantKey> LmdbStorage::do_remove_internal(Composite<VariantKey>&& ks,
                                                        ::lmdb::txn& txn,
                                                        RemoveOpts opts) {
  auto fmt_db = [](auto&& k) { return variant_key_type(k); };
          ...
          for (auto& k : group.values()) {
            ...
            try {
              if (lmdb_client_->remove(db_name, stored_key, txn, dbi)) {
                ARCTICDB_DEBUG(log::storage(), "Deleted segment for key {}",
                               variant_key_view(k));
              } else {
                if (!opts.ignores_missing_key_) {
                  log::storage().warn("Failed to delete segment for key {}",
                                      variant_key_view(k));
...
bool LmdbStorage::do_fast_delete() {
  std::lock_guard<std::mutex> lock{*write_mutex_};
  // bool is probably not the best return type here but it does help prevent the insane
  // boilerplate for an additional function that checks whether this is supported (like
  // the prefix matching)
...
void LmdbStorage::do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
...
bool LmdbStorage::do_is_path_valid(
    const std::string_view pathString ARCTICDB_UNUSED) const {
#ifdef _WIN32
  // Note that \ and / are valid characters as they will create subdirectories which are
  // expected to work. The filenames such as COM1, LPT1, AUX, CON etc. are reserved but
  // not strictly disallowed by Windows as directory names. Therefore, paths with these
  // names are allowed.
  ...
  if (!pathString.empty() &&
      (pathString.back() == '.' || std::isspace(pathString.back()))) {
    return false;
  }
#endif
  return true;
...
void remove_db_files(const fs::path& lib_path) {
  ...
  if (fs::exists(lib_path)) {
    if (!fs::is_empty(lib_path)) {
      log::storage().warn(
          fmt::format("Skipping deletion of directory holding LMDB library during "
...
namespace {
template <class T> T or_else(T val, T or_else_val, T def = T()) {
  return val == def ? or_else_val : val;
}
} // namespace

LmdbStorage::LmdbStorage(const LibraryPath& library_path, OpenMode mode,
                         const Config& conf)
  ...
  util::check_arg(is_read_only || mode != OpenMode::READ,
                  "Flags {} and operating mode {} are conflicting", conf.flags(), mode);
  // Windows needs a sensible size as it allocates disk for the whole file even before
  // any writes. Linux just gets an arbitrarily large size that it probably won't ever
  // reach.
#ifdef _WIN32
  ...
  try {
    ...
    arcticdb::entity::foreach_key_type([&txn, this](KeyType&& key_type) {
      ...
      dbi_by_key_type_.insert(std::make_pair(
          std::move(db_name), std::make_unique<::lmdb::dbi>(std::move(dbi))));
    });
  } catch (const ::lmdb::error& ex) {
  ...
  ARCTICDB_DEBUG(log::storage(), "Opened lmdb storage at {} with map size {}",
                 lib_dir_.string(), format_bytes(mapsize));
}

void LmdbStorage::warn_if_lmdb_already_open() {
  uint64_t& count_for_pid = ++times_path_opened[lib_dir_.string()];
  // Only warn for the "base" config library to avoid spamming users with more warnings
  // if they decide to ignore it and continue
  if (count_for_pid != 1 &&
      lib_dir_.string().find(CONFIG_LIBRARY_NAME) != std::string::npos) {
    ...
    log::storage().warn(fmt::format(
        "LMDB path at {} has already been opened in this process which is not "
        "supported by LMDB. "
        "You should only open a single Arctic instance over a given LMDB path. "
        "To continue safely, you should delete this Arctic instance and any others "
        "over the LMDB path in this "
...
LmdbStorage::LmdbStorage(LmdbStorage&& other) noexcept
    : Storage(std::move(static_cast<Storage&>(other))),
      write_mutex_(std::move(other.write_mutex_)), env_(std::move(other.env_)),
