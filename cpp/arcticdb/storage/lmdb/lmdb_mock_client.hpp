...
struct LmdbKeyHash {
  std::size_t operator()(const LmdbKey& k) const {
    return std::hash<std::pair<std::string, std::string>>{}(
        std::pair(k.db_name_, k.path_));
  }
};

class MockLmdbClient : public LmdbClientWrapper {
public:
  static std::string get_failure_trigger(const std::string& path,
                                         StorageOperation operation_to_fail,
                                         int error_code);

  bool exists(const std::string& db_name, std::string& path, ::lmdb::txn& txn,
              ::lmdb::dbi& dbi) const override;

  std::optional<Segment> read(const std::string& db_name, std::string& path,
                              ::lmdb::txn& txn, ::lmdb::dbi& dbi) const override;

  void write(const std::string& db_name, std::string& path, Segment&& segment,
             ::lmdb::txn& txn, ::lmdb::dbi& dbi, int64_t overwrite_flag) override;

  bool remove(const std::string& db_name, std::string& path, ::lmdb::txn& txn,
              ::lmdb::dbi& dbi) override;

  std::vector<VariantKey> list(const std::string& db_name, const std::string& prefix,
