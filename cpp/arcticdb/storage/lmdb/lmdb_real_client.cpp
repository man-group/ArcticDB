...
bool RealLmdbClient::exists(const std::string&, std::string& path, ::lmdb::txn& txn,
                            ::lmdb::dbi& dbi) const {
...
std::optional<Segment> RealLmdbClient::read(const std::string&, std::string& path,
                                            ::lmdb::txn& txn, ::lmdb::dbi& dbi) const {
  MDB_val mdb_key{path.size(), path.data()};
  MDB_val mdb_val;
...
void RealLmdbClient::write(const std::string&, std::string& path,
                           arcticdb::Segment&& seg, ::lmdb::txn& txn, ::lmdb::dbi& dbi,
  ...
  int rc = ::mdb_put(txn.handle(), dbi.handle(), &mdb_key, &mdb_val,
                     MDB_RESERVE | overwrite_flag);
  if (rc != MDB_SUCCESS) {
...
bool RealLmdbClient::remove(const std::string&, std::string& path, ::lmdb::txn& txn,
                            ::lmdb::dbi& dbi) {
  MDB_val mdb_key{path.size(), path.data()};
...
std::vector<VariantKey> RealLmdbClient::list(const std::string&,
  ...
  do {
    auto k = variant_key_from_bytes(static_cast<uint8_t*>(mdb_db_key.mv_data),
                                    mdb_db_key.mv_size, key_type);
