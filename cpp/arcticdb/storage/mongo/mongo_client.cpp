...
void add_common_key_values(bsoncxx::builder::basic::document& basic_builder,
                           const KeyType& key) {
  ...
  basic_builder.append(
      kvp("key_type", types::b_int32{static_cast<int32_t>(key.type())}));
  ...
  if (std::holds_alternative<std::string>(key.id()))
    ...
    basic_builder.append(
        kvp("stream_id", types::b_int64{int64_t(std::get<NumericId>(key.id()))}));
}

void add_atom_key_values(bsoncxx::builder::basic::document& basic_builder,
                         const AtomKey& key) {
  using namespace bsoncxx::builder::basic;
  using namespace mongocxx;
  using namespace bsoncxx;
  ...
  basic_builder.append(
      kvp("content_hash", types::b_int64{int64_t(key.content_hash())}));

  auto index_type = arcticdb::stream::get_index_value_type(key);
  basic_builder.append(
      kvp("index_type", types::b_int32{static_cast<int32_t>(index_type)}));
  if (index_type == IndexDescriptorImpl::Type::TIMESTAMP) {
    basic_builder.append(kvp(
        "start_time", types::b_int64{int64_t(std::get<NumericId>(key.start_index()))}));
    basic_builder.append(
        kvp("end_time", types::b_int64{int64_t(std::get<NumericId>(key.end_index()))}));
  } else {
...
class MongoClientImpl {
  ...
  std::string get_connection_string(std::string uri, uint64_t min_pool_size,
...
public:
  explicit MongoClientImpl(const Config& config, uint64_t min_pool_size,
                           uint64_t max_pool_size, uint64_t selection_timeout_ms)
      : connection_string_(get_connection_string(config.uri(), min_pool_size,
                                                 max_pool_size, selection_timeout_ms)),
        pool_(mongocxx::uri(connection_string_)) {}

  bool write_segment(const std::string& database_name,
                     const std::string& collection_name, storage::KeySegmentPair&& kv);

  UpdateResult update_segment(const std::string& database_name,
                              const std::string& collection_name,
                              storage::KeySegmentPair&& kv, bool upsert);
...
private:
  auto get_client() {
    ...
    auto client =
        ExponentialBackoff<std::runtime_error>(100, 2000).go(std::move(try_get));
...
bool MongoClientImpl::write_segment(const std::string& database_name,
  ...
  ARCTICDB_DEBUG(log::storage(), "Mongo client writing data with key {}",
                 variant_key_view(kv.variant_key()));
  if (std::holds_alternative<RefKey>(kv.variant_key())) {
    mongocxx::model::replace_one replace{
        document{} << "key" << fmt::format("{}", kv.ref_key()) << finalize, doc.view()};
    replace.upsert(true);
...
UpdateResult MongoClientImpl::update_segment(const std::string& database_name,
  ...
  mongocxx::model::replace_one replace{
      document{} << "key" << fmt::format("{}", kv.variant_key()) << finalize,
      doc.view()};
...
std::optional<KeySegmentPair>
MongoClientImpl::read_segment(const std::string& database_name,
                              const std::string& collection_name,
                              const entity::VariantKey& key) {
  using namespace bsoncxx::builder::stream;
  using bsoncxx::builder::stream::document;
  ...
  auto result =
      collection.find_one(document{} << "key" << fmt::format("{}", key) << "stream_id"
  ...
  if (result) {
    ...
    return storage::KeySegmentPair(
        std::move(stored_key),
        Segment::from_bytes(
            const_cast<uint8_t*>(result->view()["data"].get_binary().bytes),
            std::size_t(size), true));
  } else {
    // find_one returned nothing, returns null_opt which would be handled by the caller
    // to throw a KeyNotFoundException
    return std::nullopt;
  }
}
...
bool MongoClientImpl::key_exists(const std::string& database_name,
  ...
  auto result =
      collection.find_one(document{} << "key" << fmt::format("{}", key) << finalize);
  return static_cast<bool>(result);
}

DeleteResult MongoClientImpl::remove_keyvalue(const std::string& database_name,
                                              const std::string& collection_name,
                                              const entity::VariantKey& key) {
  using namespace bsoncxx::builder::stream;
  using bsoncxx::builder::stream::document;
...
MongoClientImpl::list_keys(const std::string& database_name,
                           const std::string& collection_name, KeyType key_type,
                           const std::optional<std::string>& prefix) {
  using namespace bsoncxx::builder::stream;
  using bsoncxx::builder::stream::document;
...
void MongoClientImpl::ensure_collection(std::string_view database_name,
                                        std::string_view collection_name) {
  using bsoncxx::builder::stream::document;
...
void MongoClientImpl::drop_collection(std::string database_name,
...
/*
 * Pimpl idiom hides mongo headers from python code, and avoids problems with Mongo
 * Cxx's rather promiscuous namespace usage.
 */
MongoClient::MongoClient(const Config& config, uint64_t min_pool_size,
                         uint64_t max_pool_size, uint64_t selection_timeout_ms)
    : client_(new MongoClientImpl(config, min_pool_size, max_pool_size,
...
std::optional<KeySegmentPair>
MongoClient::read_segment(const std::string& database_name,
                          const std::string& collection_name,
                          const entity::VariantKey& key) {
  return client_->read_segment(database_name, collection_name, key);
}

DeleteResult MongoClient::remove_keyvalue(const std::string& database_name,
                                          const std::string& collection_name,
                                          const entity::VariantKey& key) {
...
std::vector<VariantKey>
MongoClient::list_keys(const std::string& database_name,
                       const std::string& collection_name, KeyType key_type,
                       const std::optional<std::string>& prefix) {
  return client_->list_keys(database_name, collection_name, key_type, prefix);
}

void MongoClient::ensure_collection(std::string_view database_name,
                                    std::string_view collection_name) {
  client_->ensure_collection(database_name, collection_name);
}

void MongoClient::drop_collection(std::string database_name,
                                  std::string collection_name) {
