 ...
 * All the exceptions thrown by mongocxx are derived from mongocxx::exception.
 * https://mongocxx.org/api/mongocxx-3.5.1/classmongocxx_1_1exception.html
 * - The exceptions that triggered by read, write, delete operations are derived from
 * mongocxx::operation_exception.
 * - mongocxx::operation_exception has an error_code which is returned by the server as
 * documented here: https://www.mongodb.com/docs/manual/reference/error-codes/
...
void raise_mongo_exception(const mongocxx::operation_exception& e,
  ...
  auto mongo_error_suffix =
      fmt::format("MongoError#{}: {} for object {}", error_code, e.what(), object_name);

  if (error_code == static_cast<int>(MongoError::NoSuchKey) ||
      error_code == static_cast<int>(MongoError::KeyNotFound)) {
    throw KeyNotFoundException(
        fmt::format("Key Not Found Error: {}", mongo_error_suffix));
  }
  ...
  if (error_code == static_cast<int>(MongoError::UnAuthorized) ||
    ...
    raise<ErrorCode::E_PERMISSION>(
        fmt::format("Permission error: {}", mongo_error_suffix));
  }

  raise<ErrorCode::E_UNEXPECTED_MONGO_ERROR>(
      fmt::format("Unexpected Mongo Error: {} {} {}", mongo_error_suffix,
                  e.code().category().name(), e.code().message()));
}
...
std::string MongoStorage::name() const { return fmt::format("mongo_storage-{}", db_); }
        ...
        for (auto& kv : group.values()) {
          ...
          try {
            ...
            storage::check<ErrorCode::E_MONGO_BULK_OP_NO_REPLY>(
                success, "Mongo did not acknowledge write for key {}", key_view);
      ...
      .foreach ([&](auto&& group) {
        for (auto& kv : group.values()) {
          ...
          try {
            ...
            storage::check<ErrorCode::E_MONGO_BULK_OP_NO_REPLY>(
                result.modified_count.has_value(),
                "Mongo did not acknowledge write for key {}", key_view);
            if (!opts.upsert_ && result.modified_count.value() == 0) {
              throw storage::KeyNotFoundException(fmt::format(
...
void MongoStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                           ReadKeyOpts opts) {
  namespace fg = folly::gen;
        ...
        for (auto& k : group.values()) {
          ...
          try {
            ...
            log::storage().log(opts.dont_warn_about_missing_key ? spdlog::level::debug
                               ...
                               variant_key_view(k), ex.code().value(), ex.what());
            keys_not_found.push_back(k);
          }
        }
      });
      ...
      .foreach ([&](auto&& group) {
        for (auto& k : group.values()) {
          ...
          try {
            ...
            // mongo delete does not throw exception if key not found, it returns 0 as
            // delete count
            std::string object_name = std::string(variant_key_view(k));
            raise_mongo_exception(ex, object_name);
          }
...
void MongoStorage::do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
  ...
  try {
    ...
    // We don't raise when key is not found because we want to return an empty list
    // instead of raising.
...
MongoStorage::MongoStorage(const LibraryPath& lib, OpenMode mode, const Config& config)
    : Storage(lib, mode), instance_(MongoInstance::instance()) {
  if (config.use_mock_storage_for_testing()) {
    ...
    client_ = std::make_unique<MongoClient>(
        config, ConfigsMap::instance()->get_int("MongoClient.MinPoolSize", 100),
