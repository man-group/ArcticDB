...
std::string MockMongoClient::get_failure_trigger(const std::string& key,
                                                 StorageOperation operation_to_fail,
                                                 MongoError error_code) {
  return fmt::format("{}#Failure_{}_{}", key, operation_to_string(operation_to_fail),
                     static_cast<int>(error_code));
}

template <typename exception_type>
MongoFailure create_failure(const std::string& message, MongoError error_code) {
  static_assert(std::is_base_of<mongocxx::operation_exception, exception_type>::value,
                "exception_type must be a subclass of mongocxx::operation_exception");
  if (error_code == MongoError::NoAcknowledge) {
    return {no_ack_failure()};
  }
  bsoncxx::document::value empty_doc_value =
...
MongoFailure get_failure(const std::string& message, StorageOperation operation,
                         MongoError error_code) {
  switch (operation) {
...
std::optional<MongoFailure> has_failure_trigger(const MongoKey& key,
...
bool matches_prefix(const MongoKey& key, const std::string& database_name,
                    const std::string& collection_name, const std::string& prefix) {

  return key.database_name_ == database_name &&
         key.collection_name_ == collection_name &&
...
bool MockMongoClient::write_segment(const std::string& database_name,
...
UpdateResult MockMongoClient::update_segment(const std::string& database_name,
                                             const std::string& collection_name,
                                             storage::KeySegmentPair&& kv,
                                             bool upsert) {
  auto key = MongoKey(database_name, collection_name, kv.variant_key());
...
MockMongoClient::read_segment(const std::string& database_name,
                              const std::string& collection_name,
...
DeleteResult MockMongoClient::remove_keyvalue(const std::string& database_name,
                                              const std::string& collection_name,
                                              const entity::VariantKey& key) {
  auto mongo_key = MongoKey(database_name, collection_name, key);
...
bool MockMongoClient::key_exists(const std::string& database_name,
                                 const std::string& collection_name,
                                 const entity::VariantKey& key) {
  auto mongo_key = MongoKey(database_name, collection_name, key);
...
MockMongoClient::list_keys(const std::string& database_name,
                           const std::string& collection_name, KeyType,
...
void MockMongoClient::drop_collection(std::string database_name,
                                      std::string collection_name) {
  for (auto it = mongo_contents.begin(); it != mongo_contents.end();) {
    if (it->first.database_name_ == database_name &&
        it->first.collection_name_ == collection_name) {
