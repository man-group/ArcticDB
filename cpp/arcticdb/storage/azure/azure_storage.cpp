...
// TODO: fix this temporary workaround to read error code. azure-sdk-cpp client
// sometimes doesn't properly set the error code.
//  This issue has been raised on the sdk repo
//  https://github.com/Azure/azure-sdk-for-cpp/issues/5369. Once fixed, we should no
//  longer need the following function and would just read e.ErrorCode.
...
void raise_azure_exception(const Azure::Core::RequestFailedException& e,
  ...
  auto error_message_suffix = fmt::format("AzureError#{} {}: {} {} for object {}",
                                          static_cast<int>(status_code), error_code,
                                          e.ReasonPhrase, e.what(), object_name);
...
bool is_expected_error_type(const std::string& error_code,
...
void do_write_impl(
    Composite<KeySegmentPair>&& kvs, const std::string& root_folder,
    AzureClientWrapper& azure_client, KeyBucketizer&& bucketizer,
  ...
  (fg::from(kvs.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&azure_client, &root_folder, b = std::move(bucketizer), &upload_option,
                 &request_timeout](auto&& group) {
        ...
        for (auto& kv : group.values()) {
          ...
          try {
            azure_client.write_blob(blob_name, std::move(seg), upload_option,
                                    request_timeout);
...
void do_update_impl(
  ...
  do_write_impl(std::move(kvs), root_folder, azure_client, std::move(bucketizer),
                upload_option, request_timeout);
}
...
void do_read_impl(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                  const std::string& root_folder, AzureClientWrapper& azure_client,
  ...
  (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&azure_client, &root_folder, b = std::move(bucketizer), &visitor,
                 &failed_reads, opts = opts, &download_option,
                 &request_timeout](auto&& group) {
        for (auto& k : group.values()) {
          ...
          try {
            visitor(
                k, azure_client.read_blob(blob_name, download_option, request_timeout));
            ...
            if (!opts.dont_warn_about_missing_key) {
              log::storage().warn(
                  "Failed to read azure segment with key '{}' {} {}: {}", k, blob_name,
                  static_cast<int>(e.StatusCode), e.ReasonPhrase);
            }
...
void do_remove_impl(Composite<VariantKey>&& ks, const std::string& root_folder,
  ...
  static const size_t delete_object_limit =
      std::min(BATCH_SUBREQUEST_LIMIT,
               static_cast<size_t>(ConfigsMap::instance()->get_int(
                   "AzureStorage.DeleteBatchSize", BATCH_SUBREQUEST_LIMIT)));
  ...
  (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
      .foreach ([&root_folder, b = std::move(bucketizer),
                 delete_object_limit = delete_object_limit, &to_delete,
                 &submit_batch](auto&& group) { // bypass incorrect 'set but no used"
                                                // error for delete_object_limit
        auto key_type_dir = key_type_folder(root_folder, group.key());
...
std::string prefix_handler(const std::string& prefix, const std::string& key_type_dir,
                           const KeyDescriptor& key_descriptor, KeyType) {
  return !prefix.empty() ? fmt::format("{}/{}*{}", key_type_dir, key_descriptor, prefix)
                         : key_type_dir;
}

template <class KeyBucketizer>
void do_iterate_type_impl(KeyType key_type, const IterateTypeVisitor& visitor,
  ...
  try {
    for (auto page = azure_client.list_blobs(key_prefix); page.HasPage();
         page.MoveToNextPage()) {
    ...
    log::storage().warn("Failed to iterate azure blobs '{}' {}: {}", key_type,
                        static_cast<int>(e.StatusCode), e.ReasonPhrase);
  }
}
...
bool do_key_exists_impl(const VariantKey& key, const std::string& root_folder,
  ...
  try {
    ...
    raise_if_unexpected_error(e, blob_name);
    log::storage().debug("Failed to check azure key '{}' {} {}: {}", key, blob_name,
                         static_cast<int>(e.StatusCode), e.ReasonPhrase);
  }
  return false;
}
...
void AzureStorage::do_write(Composite<KeySegmentPair>&& kvs) {
  detail::do_write_impl(std::move(kvs), root_folder_, *azure_client_, FlatBucketizer{},
                        upload_option_, request_timeout_);
}

void AzureStorage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts) {
  detail::do_update_impl(std::move(kvs), root_folder_, *azure_client_, FlatBucketizer{},
                         upload_option_, request_timeout_);
}

void AzureStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                           ReadKeyOpts opts) {
  detail::do_read_impl(std::move(ks), visitor, root_folder_, *azure_client_,
                       FlatBucketizer{}, opts, download_option_, request_timeout_);
}

void AzureStorage::do_remove(Composite<VariantKey>&& ks, RemoveOpts) {
  detail::do_remove_impl(std::move(ks), root_folder_, *azure_client_, FlatBucketizer{},
                         request_timeout_);
}

void AzureStorage::do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
                                   const std::string& prefix) {
  detail::do_iterate_type_impl(key_type, visitor, root_folder_, *azure_client_,
                               FlatBucketizer{}, prefix);
}
...
AzureStorage::AzureStorage(const LibraryPath& library_path, OpenMode mode,
                           const Config& conf)
  ...
  ARCTICDB_RUNTIME_DEBUG(log::storage(),
                         ...
                         conf.endpoint(), conf.container_name());

  if (!conf.prefix().empty()) {
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "Azure prefix found, using: {}",
                           conf.prefix());
    auto prefix_path = LibraryPath::from_delim_path(conf.prefix(), '.');
    root_folder_ = object_store_utils::get_root_folder(prefix_path);
  } else
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "Azure prefix not found, will use {}",
                           root_folder_);
