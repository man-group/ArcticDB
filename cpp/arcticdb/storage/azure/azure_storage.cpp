...
void do_remove_impl(
    ...
    (fg::from(ks.as_range()) | fg::move | fg::groupBy(fmt_db))
        .foreach ([&root_folder,
                   ...
                   &to_delete,
                   &submit_batch](auto&& group) { // bypass incorrect 'set but no used" error for delete_object_limit
            auto key_type_dir = key_type_folder(root_folder, group.key());
...
bool do_key_exists_impl(const VariantKey& key, const std::string& root_folder, AzureClientWrapper& azure_client) {
    ...
    try {
        ...
        raise_if_unexpected_error(e, blob_name);
        log::storage().debug(
            "Failed to check azure key '{}' {} {}: {}", key, blob_name, static_cast<int>(e.StatusCode), e.ReasonPhrase
        );
    }
    return false;
}
...
void AzureStorage::do_write(Composite<KeySegmentPair>&& kvs) {
    detail::do_write_impl(
        std::move(kvs), root_folder_, *azure_client_, FlatBucketizer{}, upload_option_, request_timeout_
    );
}

void AzureStorage::do_update(Composite<KeySegmentPair>&& kvs, UpdateOpts) {
    detail::do_update_impl(
        std::move(kvs), root_folder_, *azure_client_, FlatBucketizer{}, upload_option_, request_timeout_
    );
}
...
void AzureStorage::do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor, ReadKeyOpts opts) {
    detail::do_read_impl(
        std::move(ks), visitor, root_folder_, *azure_client_, FlatBucketizer{}, opts, download_option_, request_timeout_
    );
}
