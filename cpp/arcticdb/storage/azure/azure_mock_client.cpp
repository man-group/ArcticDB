...
std::string MockAzureClient::get_failure_trigger(
    const std::string& blob_name, StorageOperation operation_to_fail,
    const std::string& error_code,
    Azure::Core::Http::HttpStatusCode error_to_fail_with) {
  return fmt::format("{}#Failure_{}_{}_{}", blob_name,
                     operation_to_string(operation_to_fail), error_code,
                     static_cast<int>(error_to_fail_with));
}

Azure::Core::RequestFailedException
get_exception(const std::string& message, const std::string& error_code,
              Azure::Core::Http::HttpStatusCode status_code) {
  auto rawResponse =
      std::make_unique<Azure::Core::Http::RawResponse>(0, 0, status_code, message);
...
std::optional<Azure::Core::RequestFailedException>
has_failure_trigger(const std::string& blob_name, StorageOperation operation) {
  ...
  try {
    ...
    auto error_message = fmt::format("Simulated Error, message: operation {}, blob "
                                     "name {} error code {} statuscode {}",
                                     operation_to_string(operation), blob_name,
                                     error_code, static_cast<int>(status_code));
...
void MockAzureClient::write_blob(
    const std::string& blob_name, arcticdb::Segment&& segment,
    const Azure::Storage::Blobs::UploadBlockBlobFromOptions&, unsigned int) {
...
Segment MockAzureClient::read_blob(const std::string& blob_name,
  ...
  if (pos == azure_contents.end()) {
    ...
    std::string message =
        fmt::format("Simulated Error, message: Read failed {} {}", error_code,
                    static_cast<int>(Azure::Core::Http::HttpStatusCode::NotFound));
    throw get_exception(message, error_code,
...
void MockAzureClient::delete_blobs(const std::vector<std::string>& blob_names,
                                   unsigned int) {
...
MockAzureClient::list_blobs(const std::string& prefix) {
