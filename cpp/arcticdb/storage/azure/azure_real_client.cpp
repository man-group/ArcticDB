...
Azure::Core::Context get_context(unsigned int request_timeout) {
  Azure::Core::Context
      requestContext; // TODO: Maybe can be static but need to be careful with its
                      // shared_ptr and ContextSharedState
  return requestContext.WithDeadline(std::chrono::system_clock::now() +
...
RealAzureClient::get_client_options(const Config& conf) {
  BlobClientOptions client_options;
  if (!conf.ca_cert_path().empty() ||
      !conf.ca_cert_dir()
           .empty()) { // WARNING: Setting ca_cert_path or ca_cert_dir will force Azure
                       // sdk uses libcurl as backend support, instead of winhttp
    ...
    client_options.Transport.Transport =
        std::make_shared<Azure::Core::Http::CurlTransport>(curl_transport_options);
  }
  ...
  auto properties =
      blob_client
          .GetProperties(Azure::Storage::Blobs::GetBlobPropertiesOptions{},
                         get_context(request_timeout))
          .Value;
  std::shared_ptr<Buffer> buffer = std::make_shared<Buffer>(properties.BlobSize);
  blob_client.DownloadTo(buffer->preamble(), buffer->available(), download_option,
                         get_context(request_timeout));
...
void RealAzureClient::delete_blobs(const std::vector<std::string>& blob_names,
                                   unsigned int request_timeout) {
  ...
  for (auto& blob_name : blob_names) {
    ARCTICDB_RUNTIME_DEBUG(log::storage(), "Removing azure blob with key {}",
                           blob_names);
  ...
  container_client.SubmitBatch(
      batch, Azure::Storage::Blobs::SubmitBlobBatchOptions(),
      get_context(
          request_timeout)); // To align with s3 behaviour, deleting non-exist objects
                             // is not an error, so not handling response
...
ListBlobsPagedResponse RealAzureClient::list_blobs(const std::string& prefix) {
  ARCTICDB_RUNTIME_DEBUG(log::storage(), "Searching for objects with prefix {}",
                         prefix);
