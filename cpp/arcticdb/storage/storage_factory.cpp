...
create_storage(const LibraryPath& library_path, OpenMode mode,
  ...
  if (type_name == s3::S3Storage::Config::descriptor()->full_name()) {
    ...
    storage =
        std::make_shared<memory::MemoryStorage>(library_path, mode, memory_config);
  } else if (type_name ==
             nfs_backed::NfsBackedStorage::Config::descriptor()->full_name()) {
    nfs_backed::NfsBackedStorage::Config nfs_backed_config;
    storage_descriptor.config().UnpackTo(&nfs_backed_config);
    storage = std::make_shared<nfs_backed::NfsBackedStorage>(library_path, mode,
                                                             nfs_backed_config);
    ...
    storage =
        std::make_shared<file::MappedFileStorage>(library_path, mode, mapped_config);
  } else
