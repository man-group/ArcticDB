...
std::unique_ptr<Storage> create_storage(
    ...
    OpenMode mode,
    const StorageConfig &storage_conf) {

    std::unique_ptr<Storage> storage;
    auto type_name = util::get_arcticdb_pb_type_name(storage_conf.pb_config.config());

    if (type_name == s3::S3Storage::Config::descriptor()->full_name()) {
        s3::S3Storage::Config s3_config;
        storage_conf.pb_config.config().UnpackTo(&s3_config);
        ...
        storage_conf.pb_config.config().UnpackTo(&lmbd_config);
        ...
        storage_conf.pb_config.config().UnpackTo(&mongo_config);
        ...
        storage_conf.pb_config.config().UnpackTo(&memory_config);
        ...
        storage_conf.pb_config.config().UnpackTo(&nfs_backed_config);
        ...
        storage_conf.pb_config.config().UnpackTo(&azure_config);
        storage = std::make_unique<azure::AzureStorage>(
            azure::AzureStorage(library_path, mode, azure_config, storage_conf.credential.get_azure_credential())
        );
    } else
