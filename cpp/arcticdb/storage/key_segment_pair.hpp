...
/*
 * KeySegmentPair contains compressed data as returned from storage. Unlike FrameSlice,
 * this does not contain any positioning information for the contained data.
 */
...
public:
  ...
  template <typename T> void set_key(T&& key) {
    key_ = std::make_shared<VariantKey>(std::forward<T>(key));
  }

  [[nodiscard]] const AtomKey& atom_key() const {
    util::check(std::holds_alternative<AtomKey>(variant_key()),
                "Expected atom key access");
    return std::get<AtomKey>(variant_key());
  }

  [[nodiscard]] const RefKey& ref_key() const {
    util::check(std::holds_alternative<RefKey>(variant_key()),
                "Expected ref key access");
  ...
  [[nodiscard]] const Segment& segment() const {
    util::check(segment_,
                "Attempting to access segment_ (const) but it has not been set");
    return *segment_;
  }
