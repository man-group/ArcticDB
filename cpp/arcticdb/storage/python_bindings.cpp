...
void register_bindings(py::module& storage, py::exception<arcticdb::ArcticException>& base_exception) {
    ...
    py::class_<s3::S3Settings>(storage, "S3Settings")
        ...
        .def(py::pickle(
            ...
            [](py::tuple t) {
                if (t.size() != 2) {
                    util::raise_rte("Invalid S3Settings pickle objects");
                }
                s3::S3Settings settings;
                settings.set_aws_auth(t[static_cast<uint32_t>(S3SettingsPickleOrder::AWS_AUTH)].cast<s3::AWSAuthMethod>());
                settings.set_aws_profile(t[static_cast<uint32_t>(S3SettingsPickleOrder::AWS_PROFILE)].cast<std::string>());
                return settings;
            }
        ));

    
    py::bind_map<NativeVariantStorageMap>(storage, "NativeVariantStorageMap");
    ...
    py::class_<LibraryManager, std::shared_ptr<LibraryManager>>(storage, "LibraryManager")
        ...
        .def("get_library", [](
                ...
                const NativeVariantStorageMap& native_storage_map) {
            return library_manager.get_library(LibraryPath{library_path, '.'}, storage_override, ignore_cache, native_storage_map);
        },
             py::arg("library_path"),
             py::arg("storage_override") = StorageOverride{},
             py::arg("ignore_cache") = false,
             py::arg("native_storage_map") = NativeVariantStorageMap()
        )
