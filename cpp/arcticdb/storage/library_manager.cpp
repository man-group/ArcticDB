...
const std::string BAD_CONFIG_IN_ATTEMPTED_WRITE =
    "Attempting to write forbidden storage config. This indicates a "
    "bug in ArcticDB.";

template <typename T> struct StorageVisitor {
...
void apply_storage_override(const StorageOverride& storage_override,
                            arcticdb::proto::storage::LibraryConfig& lib_cfg_proto) {
  util::variant_match(
      storage_override.variant(), StorageVisitor<S3Override>{lib_cfg_proto},
      StorageVisitor<AzureOverride>{lib_cfg_proto},
      StorageVisitor<LmdbOverride>{lib_cfg_proto}, [](const std::monostate&) {});
}
...
bool is_storage_config_ok(const arcticdb::proto::storage::VariantStorage& storage,
                          const std::string& error_message, bool throw_on_failure) {
  bool is_ok{true};
  if (storage.config().Is<arcticdb::proto::s3_storage::Config>()) {
    ...
    is_ok = is_s3_credential_ok(s3_storage.credential_key()) &&
            is_s3_credential_ok(s3_storage.credential_name());
  }
...
void LibraryManager::write_library_config(const py::object& lib_cfg,
  ...
  store_->write_sync(entity::KeyType::LIBRARY_CONFIG, StreamId(library_name),
                     std::move(segment));
}

py::object
LibraryManager::get_library_config(const LibraryPath& path,
                                   const StorageOverride& storage_override) const {
  arcticdb::proto::storage::LibraryConfig config =
      get_config_internal(path, storage_override);

  return arcticdb::python_util::pb_to_python(config);
}

bool LibraryManager::is_library_config_ok(const LibraryPath& path,
                                          bool throw_on_failure) const {
  arcticdb::proto::storage::LibraryConfig config =
      get_config_internal(path, StorageOverride{});
  return std::all_of(config.storage_by_id().begin(), config.storage_by_id().end(),
                     [&throw_on_failure](const auto& storage) {
                       return is_storage_config_ok(storage.second,
                                                   BAD_CONFIG_IN_STORAGE_ERROR,
                                                   throw_on_failure);
                     });
}
...
LibraryManager::get_library(const LibraryPath& path,
                            const StorageOverride& storage_override,
  ...
  auto lib =
      std::make_shared<Library>(path, std::move(storages), config.lib_desc().version());
...
bool LibraryManager::has_library(const LibraryPath& path) const {
  return store_->key_exists_sync(
      RefKey{StreamId(path.to_delim_path()), entity::KeyType::LIBRARY_CONFIG});
}

arcticdb::proto::storage::LibraryConfig
LibraryManager::get_config_internal(const LibraryPath& path,
                                    const StorageOverride& storage_override) const {
