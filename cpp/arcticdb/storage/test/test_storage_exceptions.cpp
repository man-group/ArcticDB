...
public:
  explicit LMDBStorageFactory(uint64_t map_size, bool use_mock = false)
      : map_size(map_size), use_mock(use_mock),
        db_path(TEST_DATABASES_PATH / "test_lmdb"), lib_name("test_lib") {}
...
class GenericStorageTest
    : public ::testing::TestWithParam<std::shared_ptr<StorageFactory>> {
protected:
...
TEST_P(GenericStorageTest, WriteDuplicateKeyException) {
  ...
  ASSERT_THROW(
      { write_in_store(*storage, "sym"); }, arcticdb::storage::DuplicateKeyException);
}

TEST_P(GenericStorageTest, ReadKeyNotFoundException) {
  ASSERT_FALSE(exists_in_store(*storage, "sym"));
  ASSERT_THROW(
      { read_in_store(*storage, "sym"); }, arcticdb::storage::KeyNotFoundException);
}

TEST_P(GenericStorageTest, UpdateKeyNotFoundException) {
  ASSERT_FALSE(exists_in_store(*storage, "sym"));
  ASSERT_THROW(
      { update_in_store(*storage, "sym"); }, arcticdb::storage::KeyNotFoundException);
}

TEST_P(GenericStorageTest, RemoveKeyNotFoundException) {
  ASSERT_FALSE(exists_in_store(*storage, "sym"));
  ASSERT_THROW(
      { remove_in_store(*storage, {"sym"}); }, arcticdb::storage::KeyNotFoundException);
}

INSTANTIATE_TEST_SUITE_P(AllStoragesCommonTests, GenericStorageTest,
                         ::testing::Values(std::make_shared<LMDBStorageFactory>(),
                                           std::make_shared<LMDBStorageFactory>(true),
...
TEST_F(LMDBStorageTestBase, WriteMapFullError) {
  ...
  arcticdb::entity::AtomKey k = arcticdb::entity::atom_key_builder()
                                    .gen_id(0)
                                    .build<arcticdb::entity::KeyType::VERSION>("sym");

  auto segment_in_memory =
      get_test_frame<arcticdb::stream::TimeseriesIndex>("symbol", {}, 40000, 0)
          .segment_;
  auto codec_opts = proto::encoding::VariantCodec();
  auto segment = encode_dispatch(std::move(segment_in_memory), codec_opts,
                                 arcticdb::EncodingVersion::V2);
...
TEST_F(LMDBStorageTestBase, MockMapFullError) {
  ...
  std::string failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "sym", StorageOperation::WRITE, MDB_MAP_FULL);
...
TEST_F(LMDBStorageTestBase, MockUnexpectedLMDBErrorException) {
  ...
  std::string failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "sym3", StorageOperation::WRITE, MDB_INVALID);
  ASSERT_THROW(
      { write_in_store(*storage, failureSymbol); }, UnexpectedLMDBErrorException);

  failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "symx", StorageOperation::READ, MDB_CORRUPTED);
  ASSERT_THROW(
      { read_in_store(*storage, failureSymbol); }, UnexpectedLMDBErrorException);

  failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "sym1", StorageOperation::EXISTS, MDB_PAGE_NOTFOUND);
  ASSERT_THROW(
      { exists_in_store(*storage, failureSymbol); }, UnexpectedLMDBErrorException);

  failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "sym1", StorageOperation::DELETE, MDB_PANIC);
  ASSERT_THROW(
      { remove_in_store(*storage, {failureSymbol}); }, UnexpectedLMDBErrorException);

  ASSERT_EQ(list_in_store(*storage), symbols);

  failureSymbol = storage::lmdb::MockLmdbClient::get_failure_trigger(
      "sym3", StorageOperation::LIST, MDB_CURSOR_FULL);
...
TEST_F(LMDBStorageTestBase, RemoveLibPath) {
  ...
  // Once we call close, any other operations should throw UnexpectedLMDBErrorException
  // as lmdb env is closed
...
// Check that Permission exception is thrown when Access denied or invalid access key
// error occurs on various calls
...
TEST(MongoMockStorageTest, test_remove) {
  ...
  ASSERT_THROW(remove_in_store(*store, {"symbol_2", "symbol_3",
                                        mongo::MockMongoClient::get_failure_trigger(
                                            "symbol_4", StorageOperation::DELETE,
                                            mongo::MongoError::HostUnreachable)}),
               UnexpectedMongoException);
  remaining = std::set<std::string>{"symbol_4"};
  ASSERT_EQ(list_in_store(*store), remaining);

  ASSERT_THROW(remove_in_store(*store, {"symbol_non_existent"}),
               KeyNotFoundException); // removing non-existent keys should throw
                                      // KeyNotFoundException in Mongo storage
  ASSERT_EQ(list_in_store(*store), remaining);
}
...
TEST(MongoMockStorageTest, test_list) {
  ...
  write_in_store(*store, mongo::MockMongoClient::get_failure_trigger(
                             "symbol_{99}", StorageOperation::LIST,
                             mongo::MongoError::HostNotFound));
