...
inline Segment get_test_segment() {
  ...
  return encode_dispatch(std::move(segment_in_memory), codec_opts,
...
inline void update_in_store(Storage& store, std::string symbol,
                            entity::KeyType key_type = entity::KeyType::TABLE_DATA) {
  auto variant_key = get_test_key(symbol, key_type);
  store.update(KeySegmentPair(std::move(variant_key), get_test_segment()),
               arcticdb::storage::UpdateOpts{});
}
...
inline std::string
read_in_store(Storage& store, std::string symbol,
              entity::KeyType key_type = entity::KeyType::TABLE_DATA) {
  auto variant_key = get_test_key(symbol, key_type);
...
list_in_store(Storage& store, entity::KeyType key_type = entity::KeyType::TABLE_DATA) {
...
inline std::set<std::string>
populate_store(Storage& store, std::string_view symbol_prefix, int start, int end,
               entity::KeyType key_type = entity::KeyType::TABLE_DATA) {
