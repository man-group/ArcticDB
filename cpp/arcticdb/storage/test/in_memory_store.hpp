...
public:
  ...
  batch_write_compressed(std::vector<storage::KeySegmentPair>) override {
  ...
  batch_read_uncompressed(std::vector<pipelines::RangesAndKey>&&,
  ...
  AtomKey get_key(stream::KeyType key_type, VersionId gen_id, const StreamId& stream_id,
  ...
  folly::Future<VariantKey> write(KeyType key_type, VersionId gen_id,
  ...
  folly::Future<entity::VariantKey> write(stream::KeyType key_type, VersionId gen_id,
                                          const StreamId& stream_id,
                                          timestamp creation_ts, IndexValue start_index,
                                          IndexValue end_index,
                                          SegmentInMemory&& segment) override {
    auto key =
        get_key(key_type, gen_id, stream_id, start_index, end_index, creation_ts);
    add_segment(key, std::move(segment));
    ARCTICDB_DEBUG(log::storage(), "Mock store adding atom key {}", key);
    return folly::makeFuture(key);
  }

  folly::Future<VariantKey> update(const VariantKey& key, SegmentInMemory&& segment,
                                   storage::UpdateOpts opts) override {
    if (!opts.upsert_) {
      util::check_rte(key_exists(key).get(),
  ...
  entity::VariantKey write_sync(stream::KeyType key_type, VersionId version_id,
                                ...
                                IndexValue end_index,
                                SegmentInMemory&& segment) override {
    return write(key_type, version_id, stream_id, start_index, end_index,
                 std::move(segment))
        .get();
  }
  ...
  folly::Future<entity::VariantKey> write(stream::KeyType key_type,
                                          const StreamId& stream_id,
                                          SegmentInMemory&& segment) override {
    util::check(is_ref_key_class(key_type),
                "Cannot write ref key with atom key type {}", key_type);
  ...
  folly::Future<VariantKey> copy(arcticdb::entity::KeyType, const StreamId&,
                                 arcticdb::entity::VersionId,
                                 const VariantKey&) override {
    util::raise_rte("Not implemented");
  }

  VariantKey copy_sync(arcticdb::entity::KeyType, const StreamId&,
                       arcticdb::entity::VersionId, const VariantKey&) override {
    util::raise_rte("Not implemented");
  }
  ...
  std::pair<VariantKey, SegmentInMemory> read_sync(const VariantKey& key,
    ...
    return util::variant_match(
        ...
        [&](const RefKey& ref_key) {
          ...
          std::pair<VariantKey, arcticdb::SegmentInMemory> res = {it->first,
                                                                  it->second->clone()};
          return res;
        },
  ...
  read_compressed(const entity::VariantKey&, storage::ReadKeyOpts) override {
    throw std::runtime_error("Not implemented");
  }

  storage::KeySegmentPair read_compressed_sync(const entity::VariantKey&,
                                               storage::ReadKeyOpts) override {
    throw std::runtime_error("Not implemented");
  }

  folly::Future<std::pair<VariantKey, SegmentInMemory>>
  read(const VariantKey& key, storage::ReadKeyOpts opts) override {
  ...
  RemoveKeyResultType remove_key_sync(const entity::VariantKey& key,
    ...
    size_t removed = util::variant_match(
        key, [&](const AtomKey& ak) { return seg_by_atom_key_.erase(ak); },
  ...
  folly::Future<RemoveKeyResultType> remove_key(const VariantKey& key,
                                                storage::RemoveOpts opts) override {
  ...
  void iterate_type(KeyType kt, const entity::IterateTypeVisitor& func,
                    const std::string& prefix = "") override {
    auto prefix_matcher = stream_id_prefix_matcher(prefix);
    auto failure_sim = StorageFailureSimulator::instance();

    std::lock_guard lock{mutex_};
    for (auto it = seg_by_atom_key_.cbegin(), next_it = it;
         it != seg_by_atom_key_.cend(); it = next_it) {
      ++next_it;
    ...
    for (auto it = seg_by_ref_key_.cbegin(), next_it = it; it != seg_by_ref_key_.cend();
         it = next_it) {
      ++next_it;
      const auto& key = it->first;
      if (key.type() == kt && prefix_matcher(key.id())) {
    ...
    for (const auto& key : keys) {
      util::variant_match(
          ...
          [&output, &atoms = seg_by_atom_key_](const AtomKey& atom) {
            output.emplace_back(
                folly::makeFuture<bool>(atoms.find(atom) != atoms.end()));
          });
    }
    return output;
  }
  ...
  size_t num_atom_keys_of_type(KeyType key_type) const {
    util::check(!is_ref_key_class(key_type),
                "Num atom keys of type for ref key doesn't make sense");
  ...
  size_t num_ref_keys_of_type(KeyType key_type) const {
    util::check(is_ref_key_class(key_type),
                "Num ref keys of type for non-ref key doesn't make sense");
  ...
  read_metadata(const entity::VariantKey& key, storage::ReadKeyOpts) override {
    return util::variant_match(
        ...
        [&](const AtomKey& ak) {
          ...
          // util::check_rte(it != seg_by_atom_key_.end(), "atom key {} not found in
          // remove", ak);
          if (it == seg_by_atom_key_.end())
            throw storage::KeyNotFoundException(Composite<VariantKey>(ak));
          ARCTICDB_DEBUG(log::storage(), "Mock store removing data for atom key {}",
                         ak);
        ...
        [&](const RefKey& rk) {
          ...
          // util::check_rte(it != seg_by_ref_key_.end(), "ref key {} not found in
          // remove", rk);
          if (it == seg_by_ref_key_.end())
    ...
    auto components = util::variant_match(
        ...
        [&](const AtomKey& ak) {
          ...
          return std::make_tuple(
              key, std::make_optional<google::protobuf::Any>(*it->second->metadata()),
  ...
  folly::Future<
      std::pair<std::variant<arcticdb::entity::AtomKeyImpl, arcticdb::entity::RefKey>,
                arcticdb::TimeseriesDescriptor>>
  ...
  void set_failure_sim(
      const arcticdb::proto::storage::VersionStoreConfig::StorageFailureSimulator&)
      override {}
