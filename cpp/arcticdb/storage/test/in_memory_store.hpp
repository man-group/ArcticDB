...
class InMemoryStore : public Store {
  ...
  public:
    ...
    storage::KeySegmentPair read_compressed_sync(const entity::VariantKey&, storage::ReadKeyOpts) override {
        throw std::runtime_error("Not implemented");
    }
    ...
    void write_compressed_sync(storage::KeySegmentPair) override { util::raise_rte("Not implemented"); }
    ...
    bool scan_for_matching_key(KeyType, const IterateTypePredicate&) override {
        ...
        auto components = util::variant_match(
            ...
            [&](const AtomKey& ak) {
                ...
                return std::make_tuple(
                    key, std::make_optional<google::protobuf::Any>(*it->second->metadata()), it->second->descriptor()
                );
            },
            [&](const RefKey& rk) {
                auto it = seg_by_ref_key_.find(rk);
    ...
    folly::Future<std::pair<
        std::variant<arcticdb::entity::AtomKeyImpl, arcticdb::entity::RefKey>,
    ...
    folly::Future<std::pair<
        std::variant<arcticdb::entity::AtomKeyImpl, arcticdb::entity::RefKey>,
        arcticdb::TimeseriesDescriptor>>
    read_timeseries_descriptor_for_incompletes(const entity::VariantKey& key, storage::ReadKeyOpts /*opts*/) override {
        return util::variant_match(
            key,
            [&](const AtomKey& ak) {
                auto it = seg_by_atom_key_.find(ak);
