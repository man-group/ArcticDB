...
public:
    ...
    AtomKey get_key(
            ...
            std::optional<timestamp> creation_ts = std::nullopt) const {
        return atom_key_builder().gen_id(gen_id).content_hash(content_hash_).creation_ts(creation_ts.value_or(
    ...
    folly::Future<VariantKey>update(const VariantKey& key,
    ...
    std::pair<VariantKey, SegmentInMemory> read_sync(
        ...
        return util::variant_match(key,
           [&](const AtomKey& atom_key) {
               ...
               ARCTICDB_DEBUG(log::storage(),
                              "Mock store removing data for atom key {}",
                              atom_key);
           ...
           [&](const RefKey& ref_key) {
               ...
               if (it == seg_by_ref_key_.end())
                   throw storage::KeyNotFoundException(ref_key);
               ARCTICDB_DEBUG(log::storage(),
                              "Mock store removing data for ref key {}",
                              ref_key);
               return std::make_pair(std::make_optional<VariantKey>(key),
                                     std::make_optional<google::protobuf::Any>(*it->second->metadata()));
           });
