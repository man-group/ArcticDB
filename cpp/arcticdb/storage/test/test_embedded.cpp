...
public:
  ...
  std::unique_ptr<as::Storage> new_backend() const {
    ...
    if (backend_ == "lmdb") {
      ...
      return std::make_unique<as::lmdb::LmdbStorage>(library_path, as::OpenMode::WRITE,
                                                     cfg);
    } else if (backend_ == "mem") {
      arcticdb::proto::memory_storage::Config cfg;

      as::LibraryPath library_path{"a", "b"};
      return std::make_unique<as::memory::MemoryStorage>(library_path,
                                                         as::OpenMode::WRITE, cfg);
    } else {
...
TEST_P(SimpleTestSuite, Example) {
  ...
  segment = encode_dispatch(std::move(segment_in_memory), codec_opts,
                            arcticdb::EncodingVersion::V2);
...
std::vector<BackendGenerator> get_backend_generators() { return {"lmdb"s, "mem"s}; }
auto backend_generators = get_backend_generators();

INSTANTIATE_TEST_SUITE_P(
    TestEmbedded, SimpleTestSuite, testing::ValuesIn(backend_generators),
    [](const testing::TestParamInfo<SimpleTestSuite::ParamType>& info) {
      return info.param.get_name();
    });
