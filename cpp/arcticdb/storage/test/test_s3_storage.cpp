...
TEST(TestS3Storage, proxy_env_var_parsing) {
  ...
  struct ProxyConfig {
    ...
    bool operator==(const Aws::Client::ClientConfiguration& client_config) const {
      return proxy_scheme_ == client_config.proxyScheme &&
             host_ == client_config.proxyHost && port_ == client_config.proxyPort &&
  ...
  std::unordered_map<std::string, ProxyConfig> passing_test_cases{
      {"http-proxy.com:2222",
       {Scheme::HTTP, Scheme::HTTP, "http-proxy.com", 2222, "", ""}},
      {"https://https-proxy.com",
       {Scheme::HTTPS, Scheme::HTTPS, "https-proxy.com", 443, "", ""}},
      // Test setting http proxy for https endpoint
      {"http://http-proxy.com",
       {Scheme::HTTPS, Scheme::HTTP, "http-proxy.com", 443, "", ""}},
      {"http://username@proxy.com",
       {Scheme::HTTP, Scheme::HTTP, "proxy.com", 80, "username", ""}},
      {"http://username:pass@proxy.com:2222",
       {Scheme::HTTP, Scheme::HTTP, "proxy.com", 2222, "username", "pass"}},
      {"http://username:p@ss@proxy.com:2222",
       {Scheme::HTTP, Scheme::HTTP, "proxy.com", 2222, "username", "p@ss"}}};
  for (const auto& [env_var, expected_proxy_config] : passing_test_cases) {
    auto client_config =
        parse_proxy_env_var(expected_proxy_config.endpoint_scheme_, env_var.c_str());
  ...
  std::unordered_map<std::string, ProxyConfig> failing_test_cases{
      ...
      {"https://username:pass@proxy.com:99999",
       {Scheme::HTTPS, Scheme::HTTP, "", 0, "", ""}}};
...
TEST_F(S3StorageFixture, test_key_exists) {
  ...
  ASSERT_THROW(
      exists_in_store(store, MockS3Client::get_failure_trigger(
                                 "symbol", StorageOperation::EXISTS,
...
TEST_F(S3StorageFixture, test_read) {
  ...
  ASSERT_THROW(read_in_store(store, MockS3Client::get_failure_trigger(
                                        "symbol", StorageOperation::READ,
                                        Aws::S3::S3Errors::THROTTLING, false)),
               UnexpectedS3ErrorException);
}

TEST_F(S3StorageFixture, test_write) {
  write_in_store(store, "symbol");
  ASSERT_THROW(write_in_store(store, MockS3Client::get_failure_trigger(
                                         "symbol", StorageOperation::WRITE,
                                         Aws::S3::S3Errors::NETWORK_CONNECTION, false)),
               UnexpectedS3ErrorException);
}
...
TEST_F(S3StorageFixture, test_remove) {
  ...
  ASSERT_THROW(remove_in_store(
                   store, {"symbol_2", MockS3Client::get_failure_trigger(
                                           "symbol_3", StorageOperation::DELETE_LOCAL,
  ...
  ASSERT_THROW(
      remove_in_store(store,
                      {"symbol_3", MockS3Client::get_failure_trigger(
...
TEST_F(S3StorageFixture, test_list) {
  ...
  write_in_store(store, MockS3Client::get_failure_trigger(
                            "symbol_99", StorageOperation::LIST,
                            Aws::S3::S3Errors::NETWORK_CONNECTION, false));
...
TEST_F(S3StorageFixture, test_matching_key_type_prefix_list) {
  ...
  auto log_compacted_symbols =
      populate_store(store, "symbol_logc", 0, 5, entity::KeyType::LOG_COMPACTED);
  ASSERT_EQ(list_in_store(store, entity::KeyType::LOG_COMPACTED),
            log_compacted_symbols);
