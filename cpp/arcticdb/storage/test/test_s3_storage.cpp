...
class S3StorageFixture : public testing::Test {
  protected:
    S3StorageFixture() : store(LibraryPath("lib", '.'), OpenMode::DELETE, get_test_s3_config()) {}
...
TEST_F(S3StorageFixture, test_read) {
    ...
    ASSERT_THROW(
        read_in_store(
            store,
            MockS3Client::get_failure_trigger("symbol", StorageOperation::READ, Aws::S3::S3Errors::THROTTLING, false)
        ),
        UnexpectedS3ErrorException
    );
}
