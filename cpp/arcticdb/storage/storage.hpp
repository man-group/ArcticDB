...
class KeyNotFoundException : public ArcticSpecificException<ErrorCode::E_KEY_NOT_FOUND> {
  public:
    explicit KeyNotFoundException(std::string message) : ArcticSpecificException<ErrorCode::E_KEY_NOT_FOUND>(message) {}
...
class Storage {
  public:
    ...
    void write(KeySegmentPair&& kv) { return write(Composite<KeySegmentPair>{std::move(kv)}); }
    ...
    void update(KeySegmentPair&& kv, UpdateOpts opts) { return update(Composite<KeySegmentPair>{std::move(kv)}, opts); }
    ...
    void remove(Composite<VariantKey>&& ks, RemoveOpts opts) { do_remove(std::move(ks), opts); }

    void remove(VariantKey&& key, RemoveOpts opts) { return remove(Composite<VariantKey>{std::move(key)}, opts); }
    ...
    bool is_path_valid(const std::string_view path) const { return do_is_path_valid(path); }
