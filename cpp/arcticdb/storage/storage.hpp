...
class DuplicateKeyException
    : public ArcticSpecificException<ErrorCode::E_DUPLICATE_KEY> {
public:
  ...
  explicit DuplicateKeyException(VariantKey key)
      : ArcticSpecificException<ErrorCode::E_DUPLICATE_KEY>(
            std::string(variant_key_view(key))),
        key_(std::move(key)) {}
...
class NoDataFoundException
    : public ArcticCategorizedException<ErrorCategory::MISSING_DATA> {
public:
...
class KeyNotFoundException
    : public ArcticSpecificException<ErrorCode::E_KEY_NOT_FOUND> {
public:
  ...
  explicit KeyNotFoundException(Composite<VariantKey>&& keys)
      : ArcticSpecificException<ErrorCode::E_KEY_NOT_FOUND>(
            fmt::format("Not found: {}", keys)),
  ...
  Composite<VariantKey>& keys() { return *keys_; }
...
public:
  Storage(LibraryPath library_path, OpenMode mode)
      : lib_path_(std::move(library_path)), mode_(mode) {}
  ...
  template <class KeyType> KeySegmentPair read(KeyType&& key, ReadKeyOpts opts) {
  ...
  bool supports_prefix_matching() const { return do_supports_prefix_matching(); }
  ...
  void iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
                    const std::string& prefix = std::string()) {
    do_iterate_type(key_type, visitor, prefix);
  }

  std::string key_path(const VariantKey& key) const { return do_key_path(key); }
...
private:
  ...
  virtual void do_read(Composite<VariantKey>&& ks, const ReadVisitor& visitor,
                       ReadKeyOpts opts) = 0;
  ...
  virtual void do_iterate_type(KeyType key_type, const IterateTypeVisitor& visitor,
