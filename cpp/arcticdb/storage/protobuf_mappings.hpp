...
inline arcticdb::proto::storage::LibraryPath
encode_library_path(const storage::LibraryPath& library_path) {
...
inline arcticdb::storage::LibraryPath
decode_library_path(const arcticdb::proto::storage::LibraryPath& protobuf_path) {
...
inline arcticdb::proto::storage::LibraryDescriptor
encode_library_descriptor(storage::LibraryDescriptor library_descriptor) {
...
inline storage::LibraryDescriptor decode_library_descriptor(
  ...
  case arcticdb::proto::storage::LibraryDescriptor::StoreTypeCase::STORE_TYPE_NOT_SET:
    ...
    break;
  default:
    util::raise_rte("Unsupported store config type {}",
                    int(protobuf_descriptor.store_type_case()));
  }
...
inline std::vector<std::pair<std::string, MemConfig>>
convert_environment_config(arcticdb::proto::storage::EnvironmentConfigsMap envs) {
  ...
  for (auto& [env_key, env_config] : envs.env_by_id()) {
    ...
    for (auto& [library_key, library_value] : env_config.lib_by_path())
      current.libraries_.try_emplace(LibraryPath::from_delim_path(library_key),
                                     library_value);
...
inline proto::mongo_storage::Config create_mongo_config(InstanceUri uri,
                                                        uint32_t flags = 0) {
