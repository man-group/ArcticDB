...
// Using StringViewable to make it easily pluggable with a custom internalized string
// class
...
inline bool operator==(const DefaultStringViewable& l, const DefaultStringViewable& r) {
  return static_cast<std::shared_ptr<std::string>>(l) ==
             static_cast<std::shared_ptr<std::string>>(r) ||
...
template <> struct formatter<arcticdb::storage::DefaultStringViewable> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const arcticdb::storage::DefaultStringViewable& dsv,
...
template <> struct hash<arcticdb::storage::DefaultStringViewable> {

  inline arcticdb::HashedValue
  operator()(const arcticdb::storage::DefaultStringViewable& v) const noexcept {
    return v.hash();
  }
};
...
public:
  ...
  static LibraryPathImpl<StringViewable> from_delim_path(std::string_view delim_path,
                                                         char delim = '.') {
  ...
  std::string to_delim_path(char delim = '.') const {
    ...
    return std::accumulate(std::next(rg.begin()), rg.end(), fmt::format("{}", rg[0]),
                           delim_fold);
  }

  auto as_range() const { return folly::range(parts_.cbegin(), parts_.cend()); }
...
inline bool operator==(const LibraryPathImpl<StringViewable>& l,
                       const LibraryPathImpl<StringViewable>& r) {
...
template <> struct formatter<arcticdb::storage::LibraryPath> {
  template <typename ParseContext> constexpr auto parse(ParseContext& ctx) {
    return ctx.begin();
  }
...
struct hash<arcticdb::storage::LibraryPathImpl<StringViewable>> {
  inline arcticdb::HashedValue operator()(
      const arcticdb::storage::LibraryPathImpl<StringViewable>& v) const noexcept {
    return v.hash();
  }
};
