...
class LibraryPathImpl {
  ...
  public:
    template<class S>
    LibraryPathImpl(std::initializer_list<S> values) : parts_(values.begin(), values.end()),
                                                       hash_(compute_hash()) {}

    template<class StringViewableRange>
    LibraryPathImpl(const StringViewableRange& parts) : parts_(parts.begin(), parts.end()),
                                                        hash_(compute_hash()) {}
