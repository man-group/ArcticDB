...
SegmentInMemory
merge_column_stats_segments(const std::vector<SegmentInMemory>& segments) {
  SegmentInMemory merged;
  merged.init_column_map();
  merged.descriptor().set_index(
      IndexDescriptorImpl{0, IndexDescriptor::Type::ROWCOUNT});
  ...
  for (auto& segment : segments) {
    for (const auto& field : segment.descriptor().fields()) {
      ...
      if (auto it = field_name_to_index.find(std::string{field.name()});
          it != field_name_to_index.end()) {
        auto& merged_type =
            type_descriptors.at(field_name_to_index.at(std::string{field.name()}));
        auto opt_common_type = has_valid_common_type(merged_type, new_type);
        internal::check<ErrorCode::E_ASSERTION_FAILURE>(
            opt_common_type.has_value(), "No valid common type between {} and {} in {}",
  ...
  for (const auto& type_descriptor : folly::enumerate(type_descriptors)) {
    merged.add_column(FieldRef{*type_descriptor, field_names.at(type_descriptor.index)},
                      0, false);
  }
  for (auto& segment : segments) {
...
std::string type_to_operator_string(ColumnStatTypeInternal type) {
  struct Tag {};
  using TypeToOperatorStringMap =
      semi::static_map<ColumnStatTypeInternal, std::string, Tag>;
...
std::string
to_segment_column_name_v1(const std::string& column,
  ...
  return fmt::format("v1.{}_{}({})", minor_version.value_or(latest_minor_version),
                     type_to_operator_string(column_stat_type), column);
}
...
std::pair<std::string, ColumnStatType>
from_segment_column_name_v1(std::string_view pattern) {
  const semi::map<std::string, ColumnStatType> name_to_type_map;
  const ankerl::unordered_dense::map<std::string, ColumnStatTypeInternal>
      operator_string_to_type{{"MIN", ColumnStatTypeInternal::MIN},
  ...
  struct Tag {};
  using InternalToExternalColumnStatType =
      semi::static_map<ColumnStatTypeInternal, ColumnStatType, Tag>;
  InternalToExternalColumnStatType::get(ColumnStatTypeInternal::MIN) =
      ColumnStatType::MINMAX;
  InternalToExternalColumnStatType::get(ColumnStatTypeInternal::MAX) =
      ColumnStatType::MINMAX;
  return std::make_pair(std::string(pattern.substr(1, pattern.size() - 2)),
                        InternalToExternalColumnStatType::get(*type));
}
...
std::string type_to_name(ColumnStatType type) {
  ...
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(TypeToNameMap::contains(type),
                                                  "Unknown column stat type requested");
...
std::optional<ColumnStatType> name_to_type(const std::string& name) {
  ...
  return name_to_type_map.contains(name)
             ? std::make_optional<ColumnStatType>(name_to_type_map.get(name))
             : std::nullopt;
}
    ...
    if (!column_stat_names.empty()) {
      ...
      for (const auto& column_stat_name : column_stat_names) {
        ...
        user_input::check<ErrorCode::E_INVALID_USER_ARGUMENT>(
            opt_index_type.has_value(), "Unknown column stat type provided: {}",
            column_stat_name);
...
ColumnStats::ColumnStats(const FieldCollection& column_stats_fields) {
  for (const auto& field : column_stats_fields) {
    if (field.name() != start_index_column_name &&
        field.name() != end_index_column_name) {
...
void ColumnStats::drop(const ColumnStats& to_drop, bool warn_if_missing) {
  for (const auto& [column, column_stat_types] : to_drop.column_stats_) {
    if (auto it = column_stats_.find(column); it == column_stats_.end()) {
      if (warn_if_missing) {
        log::version().warn("Requested column stats drop but column '{}' does not have "
                            "any column stats",
                            column);
      }
    } else {
      for (const auto& column_stat_type : column_stat_types) {
        if (it->second.erase(column_stat_type) == 0 && warn_if_missing) {
          log::version().warn("Requested column stats drop but column '{}' does not "
                              "have the specified column stat '{}'",
                              column, type_to_name(column_stat_type));
        }
      }
    }
  }
...
ankerl::unordered_dense::set<std::string> ColumnStats::segment_column_names() const {
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      version_.has_value(),
      "Cannot construct column stat column names without specified versions");
  struct Tag {};
  ...
  ExternalToInternalColumnStatType::get(ColumnStatType::MINMAX) =
      std::unordered_set<ColumnStatTypeInternal>{ColumnStatTypeInternal::MIN,
                                                 ColumnStatTypeInternal::MAX};
  ankerl::unordered_dense::set<std::string> res;
  for (const auto& [column, column_stat_types] : column_stats_) {
    for (const auto& column_stat_type : column_stat_types) {
      for (const auto& column_stat_type_internal :
           ExternalToInternalColumnStatType::get(column_stat_type)) {
        res.emplace(
            to_segment_column_name(column, column_stat_type_internal, version_));
      }
    }
  }
  return res;
...
std::optional<Clause> ColumnStats::clause() const {
  ...
  auto index_generation_aggregators =
      std::make_shared<std::vector<ColumnStatsAggregator>>();
  for (const auto& [column, column_stat_types] : column_stats_) {
  ...
  return ColumnStatsGenerationClause(std::move(input_columns),
                                     index_generation_aggregators);
}
...
void ColumnStats::parse_version(std::string_view version_string) {
  ...
  auto result = std::from_chars(candidate.data(), candidate.data() + candidate.size(),
                                major_version);
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
  ...
  result = std::from_chars(candidate.data(), candidate.data() + candidate.size(),
                           minor_version);
  internal::check<ErrorCode::E_ASSERTION_FAILURE>(
      result.ec != std::errc::invalid_argument,
      "Expected positive integer in version string, but got: {}", candidate);
...
std::pair<std::string, ColumnStatType>
ColumnStats::from_segment_column_name(std::string_view segment_column_name) {
  ...
  default:
    internal::raise<ErrorCode::E_ASSERTION_FAILURE>(
        "Unsupported major version {} when parsing column stats column name",
        version_->first);
  }
}
