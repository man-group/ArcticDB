...
std::string to_segment_column_name_v1(
    ...
    return fmt::format(
        "v1.{}_{}({})", minor_version.value_or(latest_minor_version), type_to_operator_string(column_stat_type), column
    );
}
...
void ColumnStats::parse_version(std::string_view version_string) {
    ...
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(
        result.ec != std::errc::invalid_argument, "Expected positive integer in version string, but got: {}", candidate
    );
