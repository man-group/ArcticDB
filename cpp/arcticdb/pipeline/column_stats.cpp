...
void ColumnStats::map_stats_to_column_name(
        const std::string& column_name, const std::unordered_set<std::string>& stats_names
) {
    for (const auto& stat_name : stats_names) {
        auto opt_stat = stat_name_to_stat(stat_name);
        input_column_name_to_stats_[column_name].insert(*opt_stat);
    }
}

ColumnStats::ColumnStats(const std::unordered_map<std::string, std::unordered_set<std::string>>& column_stats) {
    for (const auto& [column_name, stats_names] : column_stats) {
        if (!stats_names.empty()) {
            map_stats_to_column_name(column_name, stats_names);
        }
    }
}
...
void map_offset_to_input_column_and_stats_(
        const std::vector<utils::OffsetAndMangledName>& offsets_and_mangled_names,
        const std::map<std::string, std::set<ColumnStatType>>& column_name_to_stats,
        std::unordered_map<size_t, NameAndStats>& offset_to_input_column_and_stats
) {
    for (const auto& offset_and_mangled_name : offsets_and_mangled_names) {
        const auto& offset = offset_and_mangled_name.offset;
        const auto& mangled_name = offset_and_mangled_name.mangled_name;
        const auto& unmangled_name = offset_and_mangled_name.unmangled_name;

        offset_to_input_column_and_stats.emplace(
                offset, NameAndStats{mangled_name, column_name_to_stats.at(unmangled_name)}
        );
    }
}

void ColumnStats::calculate_offsets(const TimeseriesDescriptor& tsd, utils::MissingColumnsBehavior missing_columns) {
    util::check(!offset_to_input_column_and_stats_calculated_, "Should not calculate offsets twice");

    auto unmangled_names = get_unmangled_column_names(input_column_name_to_stats_);
    auto offsets_and_mangled_names = utils::find_offset_and_mangled_name(
            std::move(unmangled_names), tsd.as_stream_descriptor(), missing_columns
    );

    util::check(
            missing_columns == utils::MissingColumnsBehavior::IGNORE_MISSING ||
                    offsets_and_mangled_names.size() == input_column_name_to_stats_.size(),
            "Expect calculated offsets_and_mangled_names to match column_stats_"
    );

    map_offset_to_input_column_and_stats_(
            offsets_and_mangled_names, input_column_name_to_stats_, offset_to_input_column_and_stats_
    );
    offset_to_input_column_and_stats_calculated_ = true;
}
...
bool ColumnStats::empty() const {
    return input_column_name_to_stats_.empty() && offset_to_input_column_and_stats_.empty();
}

bool ColumnStats::operator==(const ColumnStats& right) const {
    if (offset_to_input_column_and_stats_calculated_) {
        return offset_to_input_column_and_stats_ == right.offset_to_input_column_and_stats_;
    } else {
        return input_column_name_to_stats_ == right.input_column_name_to_stats_;
    }
}
