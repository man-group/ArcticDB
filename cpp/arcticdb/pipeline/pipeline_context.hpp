...
/*
 * A PipelineContext instance persists throughout the lifetime of an operation. It is
 * instantiated high up in the call stack and is then passed through the call stack
 * allowing for convenient access to in-scope data.
 */
struct PipelineContext : public std::enable_shared_from_this<PipelineContext> {

  template <class ValueType>
  class PipelineContextIterator
      : public boost::iterator_facade<PipelineContextIterator<ValueType>, ValueType,
  ...
  // StreamDescriptor that can mutated for modifying operations. At the end of the
  // pipeline, the modified descriptor can be written out to storage, otherwise this'll
  // match what was read from storage.
  std::optional<StreamDescriptor> desc_;
  // If user requests strings to be returned in a different format (e.g. fixed rather
  // than dynamic) than they were written in, desc_ will be modified such that the
  // return matches what's requested, and this'll be set to the original value. It's
  // only set in this edge case.
  ...
  // Used to keep track of the total number of rows when compacting incomplete segments
  // and in sort merge
  ...
  /// Columns the user selected explicitly via the columns read option. These are the
  /// columns we must return as a result of a read operation,
  std::optional<util::BitSet> selected_columns_;
  /// All columns that must be read. This is a superset of
  /// PipelineContext::selected_columns_ and is used in cases where
  /// PipelineContext::selected_columns_ depend on other columns, e.g. when projecting a
  /// column with the QueryBuilder.
  ...
  size_t last_row() const {
    if (slice_and_keys_.empty())
      ...
      if (bucketize_dynamic_) {
        size_t max_row = 0;
        std::for_each(slice_and_keys_.begin(), slice_and_keys_.end(),
                      [&max_row](const auto& sk) {
  ...
  size_t first_row() const {
    return slice_and_keys_.empty() ? 0
                                   : slice_and_keys_.begin()->slice_.row_range.first;
  }

  size_t calc_rows() const { return last_row() - first_row(); }

  const StreamDescriptor& descriptor() const {
    util::check(static_cast<bool>(desc_),
                "Stream descriptor not found in pipeline context");
    return *desc_;
  }
  ...
  IndexRange index_range() const {
    ...
    return IndexRange{slice_and_keys_.begin()->key().start_index(),
                      slice_and_keys_.rbegin()->key().end_index()};
  }
  ...
  bool is_in_filter_columns_set(std::string_view name) {
    return !filter_columns_set_ ||
           filter_columns_set_->find(name) != filter_columns_set_->end();
  }
  ...
  void ensure_vectors() {
    util::check(slice_and_keys_.size() == fetch_index_.size(),
                "Size mismatch in pipeline context index vector");
  ...
  bool is_pickled() const {
    ...
    return norm_meta_->input_type_case() ==
           arcticdb::proto::descriptors::NormalizationMetadata::InputTypeCase::
               kMsgPackFrame;
  }
