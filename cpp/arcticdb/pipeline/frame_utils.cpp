...
TimeseriesDescriptor make_timeseries_descriptor(
    // TODO: It would be more explicit to use uint64_t instead of size_t. Not doing now
    // as it involves a lot of type changes and needs to be done carefully.
  ...
  return TimeseriesDescriptor{std::move(frame_desc), std::move(segment_desc),
                              std::move(proto), desc.fields_ptr(), desc.id()};
}
...
TimeseriesDescriptor timeseries_descriptor_from_pipeline_context(
  ...
  return make_timeseries_descriptor(
      pipeline_context->total_rows_, pipeline_context->descriptor(),
      std::move(*pipeline_context->norm_meta_),
      pipeline_context->user_meta_
          ? std::make_optional<arcticdb::proto::descriptors::UserDefinedMetadata>(
                std::move(*pipeline_context->user_meta_))
          : std::nullopt,
      std::move(prev_key), std::nullopt, bucketize_dynamic);
}
...
index_descriptor_from_frame(const std::shared_ptr<pipelines::InputTensorFrame>& frame,
  ...
  return make_timeseries_descriptor(frame->num_rows + existing_rows, frame->desc,
                                    std::move(frame->norm_meta),
                                    std::move(frame->user_meta), std::move(prev_key),
                                    std::nullopt, frame->bucketize_dynamic);
}
...
void adjust_slice_rowcounts(
  ...
  pipeline_context->total_rows_ =
      adjust_slice_rowcounts(pipeline_context->slice_and_keys_);
}
...
offset_and_row_count(const std::shared_ptr<pipelines::PipelineContext>& context) {
  ...
  for (auto s = 0u; s < context->slice_and_keys_.size(); ++s) {
    if (context->fetch_index_[s]) {
      ...
      ARCTICDB_DEBUG(log::version(), "Adding {} rows",
  ...
  std::size_t offset =
      row_count ? context->slice_and_keys_[0].slice_.row_range.first : 0ULL;
