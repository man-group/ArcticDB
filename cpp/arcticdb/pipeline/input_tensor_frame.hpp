...
concept ValidIndex =
    is_any_of<std::remove_cvref_t<std::remove_pointer_t<std::decay_t<IndexT>>>,
              stream::TimeseriesIndex, stream::RowCountIndex, stream::TableIndex,
              stream::EmptyIndex>;

struct InputTensorFrame {
  InputTensorFrame() : index(stream::empty_index()) {}
  ...
  void set_offset(ssize_t off) const { offset = off; }
    ...
    if (num_rows == 0) {
      ...
      visit_field(desc.field(0), [&](auto&& tag) {
        ...
        if constexpr (std::is_integral_v<RawType> ||
                      std::is_floating_point_v<RawType>) {
          ...
          auto end_t =
              tensor.ptr_cast<RawType>(static_cast<size_t>(tensor.shape(0) - 1));
