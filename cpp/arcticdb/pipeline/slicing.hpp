...
class FixedSlicer {
public:
  explicit FixedSlicer(std::size_t col_per_slice = 127,
                       std::size_t row_per_slice = 100'000)
...
template <typename T>
inline auto end_index_generator(
    T end_index) { // works for both rawtype and rawtype encapsulated in variant
  if constexpr (std::is_same_v<T, stream::IndexValue>) {
    std::visit(
        [](auto& index) {
          if constexpr (std::is_same_v<std::remove_reference_t<decltype(index)>,
                                       entity::NumericIndex>) {
            index += timestamp(1);
          }
        },
...
inline auto get_partial_key_gen(std::shared_ptr<InputTensorFrame> frame,
                                IndexPartialKey key) {
  ...
  return [frame = std::move(frame), key = std::move(key)](const FrameSlice& s) {
    if (frame->has_index()) {
      util::check(static_cast<bool>(frame->index_tensor),
                  "Got null index tensor in get_partial_key_gen");
      ...
      return PartialKey{KeyType::TABLE_DATA, key.version_id, key.id, start,
                        end_index_generator(end)};
    } else {
      return PartialKey{
