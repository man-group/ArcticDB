...
inline size_t get_first_string_size(const pipelines::PipelineContextRow& context_row,
                                    ChunkedBuffer& src,
  ...
  util::check(context_row.has_string_pool(), "String pool not found for context row {}",
              context_row.index());
  return get_first_string_size(num_rows, src, offset, context_row.string_pool());
}

inline size_t get_max_string_size(const pipelines::PipelineContextRow& context_row,
  ...
  for (auto row = 0u; row < num_rows; ++row) {
    ...
    max_length = std::max(
        max_length, get_string_from_pool(offset_val, context_row.string_pool()).size());
  }
  return max_length;
}
...
TimeseriesDescriptor make_timeseries_descriptor(
    ...
    std::optional<AtomKey>&& prev_key, std::optional<AtomKey>&& next_key,
...
index_descriptor_from_frame(const std::shared_ptr<pipelines::InputTensorFrame>& frame,
                            size_t existing_rows,
                            std::optional<entity::AtomKey>&& prev_key = {});

template <typename RawType>
RawType* flatten_tensor(std::optional<ChunkedBuffer>& flattened_buffer,
                        size_t rows_to_write, const NativeTensor& tensor,
...
std::optional<convert::StringEncodingError>
aggregator_set_data(const TypeDescriptor& type_desc, const entity::NativeTensor& tensor,
                    Aggregator& agg, size_t col, size_t rows_to_write, size_t row,
  ...
  return type_desc.visit_tag([&](auto tag) {
    ...
    util::check(type_desc.data_type() == tensor.data_type(),
                "Type desc {} != {} tensor type", type_desc.data_type(),
                tensor.data_type());
    util::check(type_desc.data_type() == dt, "Type desc {} != {} static type",
                type_desc.data_type(), dt);
    ...
    if constexpr (is_sequence_type(dt)) {
      ...
      if (is_fixed_string_type(dt)) {
        ...
        ptr_data += row;
        if (!c_style)
          ptr_data = flatten_tensor<PyObject*>(flattened_buffer, rows_to_write, tensor,
                                               slice_num, regular_slice_size);
        ...
        // If such a string is encountered in a column, then the GIL will be held until
        // that whole column has been processed, on the assumption that if a column has
        // one such string it will probably have many.
      ...
      if (sparsify_floats) {
        if constexpr (is_floating_point_type(dt)) {
          ...
          util::raise_rte(
              "sparse currently supported for floating point columns only.");
        }
      } else {
        if (c_style) {
          ...
          ARCTICDB_DEBUG(
              log::version(),
              "Data contains non-contiguous columns, writing will be inefficient, "
              "consider coercing to c_style ndarray (shape={}, data_size={})",
              tensor.strides(0), sizeof(RawType));
      ...
      util::BitSet values_bitset =
          util::scan_object_type_to_sparse(ptr_data, rows_to_write);
      util::check(!values_bitset.empty(),
                  "Empty bit set means empty colum and should be processed by the "
                  "empty column code path.");
      if constexpr (is_empty_type(dt)) {
        // If we have a column of type {EMPTYVAL, Dim1} and all values of the bitset are
        // set to 1 this means that we have a column full of empty arrays. In this case
        // there is no need to proceed further and store anything on disc. Empty arrays
        // can be reconstructed given the type descriptor. However, if there is at least
        // one "missing" value this means that we're mixing empty arrays and None
        // values. In that case we need to save the bitset so that we can distinguish
        // empty array from None during the read.
        if (values_bitset.size() == values_bitset.count()) {
          Column arr_col{TypeDescriptor{DataType::EMPTYVAL, Dimension::Dim2}, true};
          agg.set_sparse_block(col, arr_col.release_buffer(), arr_col.release_shapes(),
                               std::move(values_bitset));
          return std::optional<convert::StringEncodingError>();
        }
      }

      ssize_t last_logical_row{0};
      const auto column_type_descriptor =
          TypeDescriptor{tensor.data_type(), Dimension::Dim2};
      ...
      for (auto en = values_bitset.first(); en < values_bitset.end(); ++en) {
        ...
        const auto row_type_descriptor =
            TypeDescriptor{row_tensor.data_type(), Dimension::Dim1};
        const std::optional<TypeDescriptor>& common_type =
            has_valid_common_type(row_type_descriptor, secondary_type);
        ...
        // TODO: If the input array contains unexpected elements such as None, NaN,
        // string the type
        //  descriptor will have data_type == BYTES_DYNAMIC64. TypeDescriptor::visit_tag
        //  does not have a case for it and it will throw exception which is not
        //  meaningful. Adding BYTES_DYNAMIC64 in TypeDescriptor::visit_tag leads to a
        //  bunch of compilation errors spread all over the code.
        normalization::check<ErrorCode::E_UNIMPLEMENTED_COLUMN_SECONDARY_TYPE>(
            is_numeric_type(row_type_descriptor.data_type()) ||
                is_empty_type(row_type_descriptor.data_type()),
            "Numpy array type {} is not implemented. Only dense int and float arrays "
            "are supported.",
            datatype_to_str(row_type_descriptor.data_type()));
        row_type_descriptor.visit_tag(
            [&arr_col, &row_tensor, &last_logical_row](auto tdt) {
              ...
              if constexpr (is_empty_type(ArrayDataTypeTag::data_type)) {
                ...
                normalization::raise<ErrorCode::E_UNIMPLEMENTED_COLUMN_SECONDARY_TYPE>(
                    "Numpy array type is not implemented. Only dense int and float "
                    "arrays are supported.");
              }
            });
        last_logical_row++;
      }
      arr_col.set_type(TypeDescriptor{secondary_type.data_type(),
                                      column_type_descriptor.dimension()});
      agg.set_sparse_block(col, arr_col.release_buffer(), arr_col.release_shapes(),
                           std::move(values_bitset));
...
offset_and_row_count(const std::shared_ptr<pipelines::PipelineContext>& context);

bool index_is_not_timeseries_or_is_sorted_ascending(
