...
void PipelineContext::set_selected_columns(
...
bool PipelineContext::only_index_columns_selected() const {
  return overall_column_bitset_ &&
         ((overall_column_bitset_->count() == 1 && (*overall_column_bitset_)[0]) ||
...
bool PipelineContextRow::fetch_index() { return parent_->fetch_index_[index_]; }
...
const StreamDescriptor& PipelineContextRow::descriptor() const {
  util::check(index_ < parent_->segment_descriptors_.size(),
              "Descriptor out of bounds for index {}", index_);
  util::check(static_cast<bool>(parent_->segment_descriptors_[index_]),
              "Null descriptor at index {}", index_);
