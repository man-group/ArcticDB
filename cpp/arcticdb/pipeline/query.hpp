...
/*
 * A structure which is used to store the potentially negative values for indices of a
 * row range
 */
...
struct ReadQuery {
  ...
  /*
   * This is used to set the row filter to a row range not to perform a query of the
   * index key to get the total number of rows in the index, preventing the cost of
   * performing an extra request.
   */
  void calculate_row_filter(int64_t total_rows) {
    if (row_range.has_value()) {
      ...
      size_t end = row_range->end_ >= 0 ? std::min(row_range->end_, total_rows)
                                        : std::max(total_rows + row_range->end_,
                                                   static_cast<int64_t>(0));
...
using VersionQueryType =
    std::variant<std::monostate, // Represents "latest"
...
struct VersionQuery {
  ...
  void set_timestamp(timestamp ts) { content_ = TimestampVersionQuery{ts}; }
...
using FilterQuery = folly::Function<std::unique_ptr<util::BitSet>(
    const ContainerType&, std::unique_ptr<util::BitSet>&&)>;

template <typename ContainerType>
using CombinedQuery =
    folly::Function<std::unique_ptr<util::BitSet>(const ContainerType&)>;

inline FilterQuery<index::IndexSegmentReader>
create_static_col_filter(std::shared_ptr<PipelineContext> pipeline_context) {
  return [pipeline = std::move(pipeline_context)](
             const index::IndexSegmentReader& isr,
             std::unique_ptr<util::BitSet>&& input) mutable {
    auto res =
        std::make_unique<util::BitSet>(static_cast<util::BitSetSizeType>(isr.size()));
    auto start_col =
        isr.column(index::Fields::start_col).begin<stream::SliceTypeDescriptorTag>();
    auto end_col =
        isr.column(index::Fields::end_col).begin<stream::SliceTypeDescriptorTag>();
    ...
    if (input) {
      ...
      while (en < en_end) {
        ...
        (*res)[*en] =
            selected_only_index ||
            pipeline->overall_column_bitset_->any_range(*start_col, *end_col - 1);
        ++en;
      }
...
inline FilterQuery<index::IndexSegmentReader>
create_dynamic_col_filter(std::shared_ptr<PipelineContext> pipeline_context) {
  return [pipeline = std::move(pipeline_context)](
             const index::IndexSegmentReader& isr,
             std::unique_ptr<util::BitSet>&& input) mutable {
    auto res = std::make_unique<util::BitSet>(
        static_cast<util::BitSetSizeType>(pipeline->overall_column_bitset_->size()));
    util::check(isr.bucketize_dynamic(),
                "Expected column group in index segment reader dynamic column filter");
    auto hash_bucket =
        isr.column(index::Fields::hash_bucket).begin<stream::SliceTypeDescriptorTag>();
    auto num_buckets =
        isr.column(index::Fields::num_buckets).begin<stream::SliceTypeDescriptorTag>();
    ...
    while (col < col_end) {
      ...
      // A FrameSlice stores (hash_bucket, total_buckets) at the time of writing that
      // slice so a column will exist inside a slice iff col_hash % total_buckets ==
      // hash_bucket
    ...
    ARCTICDB_DEBUG(log::version(), "Dynamic column filter has {} bits set",
                   res->count());
    return res;
  };
}
...
inline FilterQuery<ContainerType> create_row_filter(RowRange&& range) {
  return [rg = std::move(range)](const ContainerType& container,
                                 std::unique_ptr<util::BitSet>&& input) mutable {
    auto res = std::make_unique<util::BitSet>(
        static_cast<util::BitSetSizeType>(container.size()));
    for (std::size_t r = 0, end = container.size(); r < end; ++r) {
      bool included =
          start_row(container, r) < rg.second && end_row(container, r) > rg.first;
      ARCTICDB_DEBUG(log::version(), "Row {} is {} range {}", r,
                     included ? "inside" : "outside", rg);
      (*res)[r] = included;
    }
...
template <typename ContainerType>
inline FilterQuery<ContainerType>
create_index_filter(const IndexRange& range, bool dynamic_schema, bool column_groups) {
  static_assert(std::is_same_v<ContainerType, index::IndexSegmentReader>);
  return [rg = range, dynamic_schema,
          column_groups](const ContainerType& container,
                         std::unique_ptr<util::BitSet>&& input) mutable {
    auto maybe_index_type =
        container.seg().template scalar_at<uint8_t>(0u, int(index::Fields::index_type));
    ...
    default:
      util::raise_rte("Unknown index type {} in create_index_filter",
                      uint32_t(index_type));
    }
  };
}
...
template <typename ContainerType>
inline void
build_row_read_query_filters(const FilterRange& range, bool dynamic_schema,
                             bool column_groups,
                             std::vector<FilterQuery<ContainerType>>& queries) {
  util::variant_match(
      range,
      [&](const RowRange& row_range) {
        queries.emplace_back(create_row_filter<ContainerType>(
            RowRange{row_range.first, row_range.second}));
      },
      [&](const IndexRange& index_range) {
        if (index_range.specified_) {
          queries.emplace_back(create_index_filter<ContainerType>(
              index_range, dynamic_schema, column_groups));
        }
      },
      [](const auto&) {});
}
...
template <typename ContainerType>
inline void
build_col_read_query_filters(std::shared_ptr<PipelineContext> pipeline_context,
                             bool dynamic_schema, bool column_groups,
                             std::vector<FilterQuery<ContainerType>>& queries) {
  if (pipeline_context->only_index_columns_selected() &&
      pipeline_context->overall_column_bitset_->count() > 0) {
    auto query = [pipeline = std::move(pipeline_context)](
                     const index::IndexSegmentReader& isr,
                     std::unique_ptr<util::BitSet>&&) mutable {
      auto res =
          std::make_unique<util::BitSet>(static_cast<util::BitSetSizeType>(isr.size()));
      auto start_row =
          isr.column(index::Fields::start_row).begin<stream::SliceTypeDescriptorTag>();
      auto start_row_end =
          isr.column(index::Fields::start_row).end<stream::SliceTypeDescriptorTag>();
      size_t index_segment_row = 0;
      ankerl::unordered_dense::set<decltype(start_row)::value_type>
          requested_start_rows;
    ...
    util::check(!dynamic_schema || column_groups,
                "Did not expect a column bitset with dynamic schema");
...
template <typename ContainerType>
inline std::vector<FilterQuery<ContainerType>>
build_read_query_filters(const std::shared_ptr<PipelineContext>& pipeline_context,
                         const FilterRange& range, bool dynamic_schema,
                         bool column_groups) {
  using namespace arcticdb::pipelines;
  std::vector<FilterQuery<ContainerType>> queries;

  build_row_read_query_filters(range, dynamic_schema, column_groups, queries);
  build_col_read_query_filters(pipeline_context, dynamic_schema, column_groups,
                               queries);
...
template <typename ContainerType>
inline std::vector<FilterQuery<ContainerType>>
build_update_query_filters(const FilterRange& range, const stream::Index& index,
                           const IndexRange& index_range, bool dynamic_schema,
                           bool column_groups) {
  // If a range was supplied, construct a query based on the type of the supplied range,
  // otherwise create a query based on the index type of the incoming update frame. All
  // three types must match, i.e. the index type of the frame to be appended to, the
  // type of the frame being appended, and the specified range, if supplied.
  std::vector<FilterQuery<ContainerType>> queries;
  util::variant_match(
      range,
      [&](const RowRange& row_range) {
        util::check(std::holds_alternative<stream::RowCountIndex>(index),
                    "Cannot partition by row count when a timeseries-indexed frame was "
                    "supplied");
        queries.emplace_back(create_row_filter<ContainerType>(
            RowRange{row_range.first, row_range.second}));
      },
        ...
        util::variant_match(
            ...
            [&](const stream::RowCountIndex&) {
              ...
              queries.emplace_back(
                  create_row_filter<ContainerType>(std::move(row_range)));
            },
            [&](const auto&) {});
      });
...
inline FilterRange get_query_index_range(const stream::Index& index,
                                         const IndexRange& index_range) {
  if (std::holds_alternative<stream::TimeseriesIndex>(index))
    return index_range;
  else
    return RowRange{std::get<NumericIndex>(index_range.start_),
                    std::get<NumericIndex>(index_range.end_)};
}

inline std::vector<SliceAndKey> strictly_before(const FilterRange& range,
                                                const std::vector<SliceAndKey>& input) {
...
inline std::vector<SliceAndKey> strictly_after(const FilterRange& range,
                                               const std::vector<SliceAndKey>& input) {
  std::vector<SliceAndKey> output;
  util::variant_match(
      range,
...
template <> struct formatter<VersionQuery> {
  ...
  auto format(const VersionQuery& q, FormatContext& ctx) const {
    return arcticdb::util::variant_match(
        ...
        [&ctx](const SnapshotVersionQuery& s) {
          return fmt::format_to(ctx.out(), "snapshot '{}'", s.name_);
        },
        [&ctx](const TimestampVersionQuery& t) {
          return fmt::format_to(ctx.out(), "{}", t.timestamp_);
        },
