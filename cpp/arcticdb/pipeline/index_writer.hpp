...
template <ValidIndex Index> class IndexWriter {
  ...
  using SliceAggregator = stream::Aggregator<AggregatorIndexType, stream::FixedSchema,
...
public:
  ...
  IndexWriter(std::shared_ptr<stream::StreamSink> sink, IndexPartialKey partial_key,
              const TimeseriesDescriptor& tsd,
              const std::optional<KeyType>& key_type = std::nullopt)
      : bucketize_columns_(tsd.column_groups()), partial_key_(std::move(partial_key)),
        agg_(
            Desc::schema(partial_key_.id, bucketize_columns_),
            [&](auto&& segment) { on_segment(std::forward<SegmentInMemory>(segment)); },
            stream::NeverSegmentPolicy{}),
        sink_(std::move(sink)),
        key_being_committed_(folly::Future<AtomKey>::makeEmpty()), key_type_(key_type) {
  ...
  void add_unchecked(const arcticdb::entity::AtomKey& key, const FrameSlice& slice) {
    auto add_to_row ARCTICDB_UNUSED = [&](auto& rb) {
      ...
      rb.set_scalar(int(Fields::index_type),
                    static_cast<uint8_t>(stream::get_index_value_type(key)));

      std::visit([&rb](auto&& val) { rb.set_scalar(int(Fields::stream_id), val); },
                 key.id());

      // note that we don't se the start index since its presence is index type specific
      std::visit([&rb](auto&& val) { rb.set_scalar(int(Fields::end_index), val); },
                 key.end_index());
      ...
      if (bucketize_columns_) {
        util::check(static_cast<bool>(slice.hash_bucket()) &&
                        static_cast<bool>(slice.num_buckets()),
    ...
    agg_.start_row()([&](auto& rb) {
      std::visit([&rb](auto&& val) { rb.set_scalar(int(Fields::start_index), val); },
                 key.start_index());
      add_to_row(rb);
    });
  }
  ...
  void add(const arcticdb::entity::AtomKey& key, const FrameSlice& slice) {
    // ensure sorted by col group then row group, this is normally the case but in
    // append scenario, one will need to ensure that this holds, otherwise the
    // assumptions in the read pipeline will be broken.
...
private:
  ...
  void on_segment(SegmentInMemory&& s) {
    ...
    key_being_committed_ =
        sink_
            ->write(key_type, partial_key_.version_id, partial_key_.id,
                    segment_start(seg), segment_end(seg), std::move(seg))
