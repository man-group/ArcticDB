...
public:
    ...
    IndexWriter(std::shared_ptr<stream::StreamSink> sink, IndexPartialKey partial_key, TimeseriesDescriptor &&tsd, const std::optional<KeyType>& key_type = std::nullopt) :
            ...
            key_type_(key_type) {
        agg_.segment().set_timeseries_descriptor(std::move(tsd));
    }
