...
ValueSet::ValueSet(py::array value_list) {
  ...
  util::variant_match(numeric_base_set_, [&](const auto& numeric_base_set) {
...
ValueSet::get_fixed_width_string_set(size_t width) {
  ...
  if (it != typed_set_fixed_width_strings_.end()) {
    ...
    typed_set_fixed_width_strings_.insert(
        std::make_pair(width, std::move(fixed_width_string_set)));
