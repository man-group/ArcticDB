...
struct Value {
  ...
  template <typename T> T get() const { return *reinterpret_cast<const T*>(&data_); }

  template <typename T> void set(T t) { *reinterpret_cast<T*>(data_) = t; }
  ...
  template <typename RawType> std::string to_string() const {
...
inline Value convert_to_utf32(const Value& other) {
  Value output;
  util::check(other.has_sequence_type(),
              "Can't convert value of non-sequence type to utf32");
  output.assign_utf32_string(*other.str_data(), other.len());
  output.data_type_ = DataType::UTF_FIXED64;
  return output;
}

template <typename T> Value construct_value(T val) {
  util::raise_rte("Unrecognized value {}", val);
  return Value{};
}

#define VALUE_CONSTRUCT(__T__, __DT__)                                                 \
  template <> inline Value construct_value(__T__ val) {                                \
    Value value;                                                                       \
...
inline Value construct_string_value_from_char(const char* c, size_t len,
                                              DataType data_type) {
  Value value;
...
inline std::optional<std::string> ascii_to_padded_utf32(std::string_view str,
                                                        size_t width) {
