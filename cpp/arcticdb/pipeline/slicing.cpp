...
get_index_and_field_count(const arcticdb::pipelines::InputTensorFrame& frame) {
  return {frame.desc.index().field_count(), frame.desc.fields().size()};
}

SlicingPolicy get_slicing_policy(const WriteOptions& options,
  ...
  if (frame.bucketize_dynamic) {
    ...
    const auto num_buckets = std::min(
        static_cast<size_t>(std::ceil(double(col_count) / options.column_group_size)),
...
void add_index_fields(const arcticdb::pipelines::InputTensorFrame& frame,
                      FieldCollection& current_fields) {
...
get_first_and_last_row(const arcticdb::pipelines::InputTensorFrame& frame) {
  return {frame.offset, frame.num_rows + frame.offset};
}

std::vector<FrameSlice>
FixedSlicer::operator()(const arcticdb::pipelines::InputTensorFrame& frame) const {
  ...
  slices.reserve(field_count / col_per_slice_ +
                 std::size_t((field_count % col_per_slice_) == 0ULL));

  const auto [first_row, last_row] = get_first_and_last_row(frame);

  // order of the frame slices is used in the mark_index_slices impl. If slices are not
  // grouped and ordered the same way, one will need to modify the mark_index_slices
  // method to use two passes instead of one
  auto col = index_count;
  do {
    auto tensor_next = tensor_pos;
    auto fields_next = fields_pos;
    auto distance =
        std::min(size_t(std::distance(tensor_pos, std::end(frame.field_tensors))),
                 col_per_slice_);
    ...
    for (std::size_t r = first_row, end = last_row; r < end; r += row_per_slice_) {
      ...
      slices.push_back(
          FrameSlice(desc, ColRange{col, col + distance}, RowRange{r, r + rdist}));
    }
...
std::vector<FrameSlice>
HashedSlicer::operator()(const arcticdb::pipelines::InputTensorFrame& frame) const {
  ...
  std::sort(std::begin(indices), std::end(indices),
            [&buckets, index_count = index_count](size_t left, size_t right) {
  ...
  do {
    ...
    const auto end_pos =
        std::find_if(start_pos, std::cend(indices),
    ...
    auto desc = std::make_shared<StreamDescriptor>(frame.desc.id(), frame.desc.index(),
                                                   std::move(current_fields));

    for (std::size_t r = first_row, end = last_row; r < end; r += row_per_slice_) {
      auto rdist = std::min(last_row - r, row_per_slice_);
      slices.emplace_back(FrameSlice(
          desc, ColRange{col, col + distance}, RowRange{r, r + rdist}, current_bucket,
          num_buckets_, std::vector<size_t>(start_pos, end_pos)));
    }
