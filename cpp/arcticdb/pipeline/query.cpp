...
std::unique_ptr<util::BitSet> build_bitset_for_index(
    ...
    if (dynamic_schema && !column_groups) {
        ...
        while (!range_intersects<RawType>(range_start, range_end, *end_pos, *end_range_last) &&
...
template std::unique_ptr<util::BitSet> build_bitset_for_index<
    TestContainer,
    TimeseriesIndex>(const TestContainer&, IndexRange, bool, bool, std::unique_ptr<util::BitSet>&&);
} // namespace arcticdb::pipelines
