...
#include <arcticdb/pipeline/segment_filter_analysis.hpp>
...
std::optional<SimplePredicate> try_extract_predicate_from_node(
...
void extract_predicates_recursive(
        const VariantNode& node_ref, const ExpressionContext& context, std::vector<SimplePredicate>& predicates
) {
