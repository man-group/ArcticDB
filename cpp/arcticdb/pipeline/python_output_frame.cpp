...
PythonOutputFrame::PythonOutputFrame(const SegmentInMemory& frame,
                                     std::shared_ptr<BufferHolder> buffers)
    : module_data_(ModuleData::instance()), frame_(frame),
  ...
  for (std::size_t c = frame.descriptor().index().field_count();
       c < static_cast<size_t>(frame.fields().size()); ++c) {
      ...
      column_data.type().visit_tag([&](auto type_desc_tag) {
        ...
        if constexpr (is_pyobject_type(TypeDescriptor(type_desc_tag))) {
          if constexpr (is_numpy_array(td)) {
            ...
            while (!it.finished()) {
              util::check(reinterpret_cast<PyObject*>(it.value()) != nullptr,
                          "Can't delete null item");
            ...
            while (auto block = column_data.next<TDT>()) {
              for (auto item : *block) {
                util::check(reinterpret_cast<PyObject*>(item) != nullptr,
                            "Can't delete null item");
