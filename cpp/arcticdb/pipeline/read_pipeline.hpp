...
inline std::optional<util::BitSet> requested_column_bitset_including_index(const StreamDescriptor& desc, const std::vector<std::string>& columns) {
    ...
    if (!columns.empty()) {
        auto bitset = build_column_bitset(desc, add_index_column(columns, desc));
        bitset.set_bit(0);  //TODO handle multiple index columns?
    }
    return std::nullopt;
}

inline std::optional<util::BitSet> clause_column_bitset(const StreamDescriptor& desc,
...
inline std::optional<util::BitSet> overall_column_bitset(const StreamDescriptor& desc,
                                                         const std::vector<std::shared_ptr<Clause>>& clauses,
