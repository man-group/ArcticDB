...
std::optional<CombinedQuery<ContainerType>>
combine_filter_functions(std::vector<FilterQuery<ContainerType>>& filters) {
  if (filters.empty())
    return std::nullopt;
...
template <class C> void foreach_active_bit(const util::BitSet& bs, C&& visitor) {
...
filter_index(const ContainerType& container,
  ...
  if (container.size() > 0) {
    if (query) {
      ...
      ARCTICDB_DEBUG(log::version(), "Row bitset has {} bits set of {}",
                     row_bitset->count(), row_bitset->size());
      output.reserve(row_bitset->count());
      foreach_active_bit(*row_bitset,
                         [&](auto r) { output.emplace_back(get_row(container, r)); });
    } else {
  ...
  ARCTICDB_DEBUG(log::version(), "Returning filtered output with {} data keys",
                 output.size());
  return output;
}

inline util::BitSet
build_column_bitset(const StreamDescriptor& desc,
  ...
  for (std::size_t c = 0; c < static_cast<std::size_t>(desc.fields().size()); ++c) {
    ...
    col_bitset[c] =
        c < desc.index().field_count() || columns.find(f.name()) != columns.end();
  }
...
inline util::BitSet build_column_bitset(const StreamDescriptor& desc,
                                        const std::vector<std::string>& columns) {
  folly::F14FastSet<std::string_view> col_set{columns.begin(), columns.end()};
  return build_column_bitset(desc, col_set);
}

inline auto add_index_column(const std::vector<std::string>& columns,
...
inline bool contains_index_column(const std::vector<std::string>& columns,
                                  const StreamDescriptor& desc) {
...
inline std::optional<util::BitSet>
clause_column_bitset(const StreamDescriptor& desc,
...
// Returns std::nullopt if all columns are required, which is the case if
// requested_columns is std::nullopt Otherwise augment the requested_columns bitset with
// columns that are required by any of the clauses
inline std::optional<util::BitSet>
overall_column_bitset(const StreamDescriptor& desc,
                      const std::vector<std::shared_ptr<Clause>>& clauses,
                      const std::optional<util::BitSet>& requested_columns) {
  // std::all_of returns true if the range is empty
  auto clauses_can_combine_with_column_selection = std::all_of(
      clauses.begin(), clauses.end(), [](const std::shared_ptr<Clause>& clause) {
  ...
  if (clauses_can_combine_with_column_selection) {
    if (requested_columns.has_value()) {
      ...
      return clause_bitset.has_value() ? *requested_columns | *clause_bitset
                                       : requested_columns;
    } else {
      return std::nullopt;
    }
  } else {
    // clauses_can_combine_with_column_selection is false implies
    // requested_columns.has_value() is false by the previous check
...
inline void generate_filtered_field_descriptors(
  ...
  if (columns.has_value()) {
    std::unordered_set<std::string_view> column_set{std::begin(*columns),
                                                    std::end(*columns)};
...
inline std::vector<FilterQuery<ContainerType>> get_column_bitset_and_query_functions(
  ...
  if (!dynamic_schema || column_groups) {
    ...
    pipeline_context->overall_column_bitset_ =
        overall_column_bitset(pipeline_context->descriptor(), query.clauses_,
                              pipeline_context->selected_columns_);
  }
  return build_read_query_filters<ContainerType>(pipeline_context, query.row_filter,
                                                 dynamic_schema, column_groups);
}
