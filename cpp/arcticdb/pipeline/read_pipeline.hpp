...
inline SliceAndKey get_row(const index::IndexSegmentReader& isr, size_t row) { return isr.row(row); }
...
inline std::vector<SliceAndKey> filter_index(
    ...
    if (!index_segment_reader.empty()) {
        if (query) {
            ...
            foreach_active_bit(*row_bitset, [&](auto r) { output.emplace_back(get_row(index_segment_reader, r)); });
        } else {
...
inline std::vector<FilterQuery<ContainerType>> get_column_bitset_and_query_functions(
        const ReadQuery& query, const std::shared_ptr<PipelineContext>& pipeline_context, bool dynamic_schema,
        bool column_groups
) {
    ...
    if (!dynamic_schema || column_groups) {
        ...
        pipeline_context->overall_column_bitset_ = overall_column_bitset(
                pipeline_context->descriptor(), query.clauses_, pipeline_context->selected_columns_
        );
    }
