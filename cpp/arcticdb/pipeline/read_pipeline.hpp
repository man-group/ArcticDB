...
std::vector<SliceAndKey> filter_index(
    ...
    if (container.size() > 0) {
        if (query) {
            ...
            foreach_active_bit(*row_bitset, [&](auto r) { output.emplace_back(get_row(container, r)); });
        } else {
...
inline std::optional<util::BitSet> overall_column_bitset(
    ...
    auto clauses_can_combine_with_column_selection =
        std::all_of(clauses.begin(), clauses.end(), [](const std::shared_ptr<Clause>& clause) {
