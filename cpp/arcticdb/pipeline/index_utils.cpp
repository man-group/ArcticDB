...
write_index(const TimeseriesDescriptor& metadata, std::vector<SliceAndKey>&& sk,
...
write_index(const std::shared_ptr<InputTensorFrame>& frame,
  ...
  return write_index(index, timeseries_desc, std::move(slice_and_keys), partial_key,
                     sink);
}
...
write_index(const std::shared_ptr<InputTensorFrame>& frame,
  ...
  return std::move(keys_fut).thenValue(
      [frame = frame, &partial_key, &sink](auto&& slice_and_keys_vals) mutable {
...
get_merged_tsd(size_t row_count, bool dynamic_schema,
  ...
  if (existing_tsd.total_rows() == 0) {
    ...
    for (size_t i = 0; i < new_fields.size(); ++i) {
      ...
      if (is_empty_type(result_type.data_type()) &&
          !is_empty_type(new_type.data_type())) {
  ...
  merged_descriptor.set_sorted(
      deduce_sorted(existing_descriptor.sorted(), new_frame->desc.sorted()));
  return make_timeseries_descriptor(row_count, std::move(merged_descriptor),
                                    std::move(new_frame->norm_meta),
                                    std::move(new_frame->user_meta), std::nullopt,
                                    std::nullopt, new_frame->bucketize_dynamic);
}
