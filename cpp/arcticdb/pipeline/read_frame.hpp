...
std::optional<util::BitSet>
check_and_mark_slices(const KeySliceContainer& slice_and_keys, bool dynamic_schema,
                      bool return_bitset, std::optional<size_t> incompletes_after,
                      bool has_column_groups) {
  ARCTICDB_SAMPLE_DEFAULT(MarkIndexSlices)
  std::optional<util::BitSet> output =
      return_bitset ? std::make_optional<util::BitSet>(0u) : std::nullopt;
  ...
  for (auto [opt_seg, slice, key] : slice_and_keys) {
    ...
    if (return_bitset) {
      ...
      output.value()[output->size()] =
          (dynamic_schema && !has_column_groups) || is_first ||
  ...
  if (!row_ranges.empty()) {
    ...
    for (; pos != row_ranges.end(); ++pos) {
      sorting::check<ErrorCode::E_UNSORTED_DATA>(
          ...
          "Non-contiguous rows, range search on unsorted data? {} {}", current, *pos);
      current = *pos;
    }
  }
...
StreamDescriptor
get_filtered_descriptor(const StreamDescriptor& desc,
