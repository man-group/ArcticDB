        ...
        details::visit_type(left_type, [&]<typename LeftTypeTag>(LeftTypeTag) {
            if constexpr (!(is_empty_type(LeftTypeTag::data_type) || is_bool_object_type(LeftTypeTag::data_type))) {
                ...
                details::visit_type(right_type, [&]<typename RightTypeTag>(RightTypeTag) {
                    if constexpr (!(is_empty_type(RightTypeTag::data_type) ||
                                    is_bool_object_type(RightTypeTag::data_type))) {
                        const std::pair right_raw_values =
                                generate_pair_of_different_values_for_data_type<RightTypeTag>();
                        ASSERT_FALSE(
                                Value(left_raw_values.first, LeftTypeTag::data_type) ==
                                Value(right_raw_values.first, RightTypeTag::data_type)
                        );
                        ASSERT_FALSE(
                                Value(left_raw_values.first, LeftTypeTag::data_type) ==
                                Value(right_raw_values.second, RightTypeTag::data_type)
                        );
                        ASSERT_FALSE(
                                Value(left_raw_values.second, LeftTypeTag::data_type) ==
                                Value(right_raw_values.second, RightTypeTag::data_type)
                        );
                        ASSERT_FALSE(
                                Value(left_raw_values.second, LeftTypeTag::data_type) ==
...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::Combine(testing::ValuesIn(data_types()), testing::ValuesIn(data_types()))
);
