...
template <class TDT> struct TestProjection {
  ...
  TestProjection(std::string field_name, ProjectionFunction&& func)
      : field_name_(std::move(field_name)), projection_func_(std::move(func)) {}
...
template <class TDT> struct TestAggregation {
  using RawType = typename TDT::DataTypeTag::raw_type;
  using AggregationFunction =
      folly::Function<std::optional<RawType>(const SegmentInMemoryImpl::Location&)>;
  std::string field_name_;
  AggregationFunction aggregation_func_;

  TestAggregation(std::string field_name, AggregationFunction&& func)
      : field_name_(std::move(field_name)), aggregation_func_(std::move(func)) {}
  ...
  SegmentInMemory operator()(SegmentInMemory input) {
    ...
    if (std::find_if(desc.fields().begin(), desc.fields().end(),
                     [&agg_field](const auto& field) { return agg_field == field; }) ==
        desc.fields().end())
...
TEST(Pipeline, Basic) {
  ...
  TestProjection<TypeDescriptor> double_it{
      "doubled", [](const SegmentInMemory::Row& row) {
        return row[0].visit([](auto val) { return static_cast<int32_t>(val * 2); });
      }};
