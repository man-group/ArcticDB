...
SegmentInMemory allocate_chunked_frame(
        const std::shared_ptr<PipelineContext>& context, OutputFormat output_format, AllocationType allocation_type
) {
...
ColumnTruncation get_truncate_range_from_index(
        const Column& column, const IndexValueType& start, const IndexValueType& end,
        std::optional<int64_t> start_offset = std::nullopt, std::optional<int64_t> end_offset = std::nullopt
) {
...
std::pair<std::optional<int64_t>, std::optional<int64_t>> get_truncate_range_from_rows(
        const RowRange& row_range, size_t start_offset, size_t end_offset
) {
    std::optional<int64_t> truncate_start;
    std::optional<int64_t> truncate_end;
...
ColumnTruncation get_truncate_range(
    ...
    if (read_options.output_format() == OutputFormat::ARROW) {
        util::variant_match(
                ...
                [&truncate_rows, &frame, &context, &index_field, index_field_offset, encoding_version](
                    ...
                    if (contains(slice_time_range, time_range.first) || contains(slice_time_range, time_range.second)) {
                        if (context.fetch_index()) {
                            ...
                            truncate_rows = get_truncate_range_from_index(
                                    ...
                                    time_range.first,
                                    time_range.second,
                                    current_row_range.first,
                                    current_row_range.second
                            );
                        } else {
                            const auto& frame_index_desc = frame.descriptor().fields(0UL);
                            Column sink{
                                    frame_index_desc.type(),
                                    encoding_sizes::field_uncompressed_size(index_field),
                                    AllocationType::PRESIZED,
                                    Sparsity::PERMITTED
                            };
                            std::optional<util::BitMagic> bv;
                            (void)decode_field(
                                    frame_index_desc.type(), index_field, index_field_offset, sink, bv, encoding_version
                            );
...
void decode_into_frame_static(
        SegmentInMemory& frame, PipelineContextRow& context, const Segment& seg, const DecodePathData& shared_data,
        std::any& handler_data, const ReadQuery& read_query, const ReadOptions& read_options
) {
        ...
        if (dynamic_schema && column_data == slice_map_->columns_.end()) {
            ...
            if (dynamic_schema) {
                NullValueReducer null_reducer{
                        column, context_, frame_, shared_data_, handler_data_, read_options_.output_format()
                };
                     ...
                     read_options](auto&& ks) mutable {
                        ...
                        if (dynamic_schema) {
                            decode_into_frame_dynamic(
                                    frame, row, key_seg.segment(), shared_data, handler_data, read_query, read_options
                            );
                        } else {
