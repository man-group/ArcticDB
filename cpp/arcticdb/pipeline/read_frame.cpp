...
{
    context->fetch_index_ = check_and_mark_slices(context->slice_and_keys_,
        dynamic_schema,
        true,
    ...
    const std::shared_ptr<std::vector<FieldDescriptor::Proto>>& filter_columns)
{
    ...
    return util::variant_match(index, [&desc, &filter_columns](const auto& idx) {
        if (filter_columns) {
            ...
            return StreamDescriptor{index_descriptor(desc.id(), idx, desc.fields())};
        }
    });
}
...
{
    return get_filtered_descriptor(context->descriptor(), context->filter_columns_);
}
...
SegmentInMemory allocate_frame(const std::shared_ptr<PipelineContext>& context)
{
    ...
    if (context->orig_desc_) {
        for (const auto& field : context->orig_desc_.value().fields()) {
            ...
            if (data_type_from_proto(field.type_desc()) != column.type().data_type())
                column.set_orig_type(type_desc_from_proto(field.type_desc()));
        }
    }
...
const uint8_t* skip_metadata_field(const arcticdb::proto::encoding::SegmentHeader& hdr, const uint8_t* data)
{
    if (hdr.has_metadata_field()) {
        // Skip it since it doesn't contain any useful info
        auto metadata_size = encoding_size::compressed_size(hdr.metadata_field().ndarray());
...
void decode_index_field(SegmentInMemory& frame,
    ...
    PipelineContextRow& context)
{
    if (get_index_field_count(frame)) {
        ...
        if (!context.fetch_index()) {
            ...
            data += encoding_size::compressed_size(field.ndarray());
        } else {
            auto& buffer = frame.column(0).data().buffer(); // TODO assert size
            auto& frame_field_descriptor = frame.field(0);  //TODO better method
            auto sz = sizeof_datatype(type_desc_from_proto(frame_field_descriptor.type_desc()));
            ...
            const auto fields_match =
                diff.Compare(frame_field_descriptor.type_desc(), context.descriptor().fields(0).type_desc());
            util::check(fields_match,
                "Cannot coerce index type from {} to {}",
                context.descriptor().fields(0).type_desc(),
                frame_field_descriptor.type_desc());

            std::optional<util::BitMagic> bv;
            data += decode(type_desc_from_proto(frame_field_descriptor.type_desc()), field, data, sink, bv);
...
void decode_or_expand(const uint8_t*& data,
    uint8_t* dest,
    ...
    std::shared_ptr<BufferHolder> buffers)
{
    if (auto handler = TypeHandlerRegistry::instance()->get_handler(type_descriptor.data_type()); handler) {
        handler->handle_type(data, dest, encoded_field_info, type_descriptor, dest_bytes, buffers);
    } else {
        std::optional<util::BitMagic> bv;
        if (encoded_field_info.has_ndarray() && encoded_field_info.ndarray().sparse_map_bytes() > 0) {
            ...
            const auto bytes = encoding_size::data_uncompressed_size(ndarray);
            ...
            const auto& ndarray = encoded_field_info.ndarray();
            if (const auto bytes = encoding_size::data_uncompressed_size(ndarray); bytes < dest_bytes) {
            ...
            data += decode(type_descriptor, encoded_field_info, data, sink, bv);
        }
    }
}

size_t get_field_range_compressed_size(size_t start_idx,
    size_t num_fields,
    const arcticdb::proto::encoding::SegmentHeader& hdr)
{
    auto start_rg = hdr.fields().begin();
    std::advance(start_rg, start_idx);
    auto end_rg = start_rg;
    std::advance(end_rg, num_fields);
    auto total = std::accumulate(start_rg, end_rg, 0u, [](size_t sz, const auto& fld) {
        return sz + encoding_size::compressed_size(fld.ndarray());
    });
    ARCTICDB_DEBUG(log::version(), "Fields {} to {} contain {} bytes", start_idx, start_idx + num_fields, total);
    return total;
}

void advance_skipped_cols(const uint8_t*& data,
    ...
    const arcticdb::proto::encoding::SegmentHeader& hdr)
{
    ...
    if (skipped_cols) {
        const auto bytes_to_skip =
            get_field_range_compressed_size((next_col - first_col_offset) + index_fieldcount, skipped_cols, hdr);
        data += bytes_to_skip;
    }
}
    ...
    const std::shared_ptr<BufferHolder>& buffers)
{
    ...
    if (data != end) {
        decode_index_field(frame, hdr, data, begin, end, context);
        ...
        while (it.has_next()) {
            ...
            util::check(it.source_field_pos() < size_t(hdr.fields_size()),
                "Field index out of range: {} !< {}",
                it.source_field_pos(),
                hdr.fields_size());
            auto& encoded_field_info = hdr.fields(static_cast<int>(it.source_field_pos()));
            ...
            util::check(trivially_compatible_types(m.source_type_desc_, m.dest_type_desc_),
    ...
    const std::shared_ptr<BufferHolder>& buffers)
{
    ...
    if (data != end) {
        ...
        for (auto field_col = index_fieldcount; field_col < field_count; ++field_col) {
            auto& encoded_field_info = hdr.fields(static_cast<int>(field_col));
            auto field_name = context.descriptor().fields(field_col).name();
            // TODO: In case of sparse bitmap we might need to skip more bytes in segment?

            auto frame_loc_opt = frame.column_index(field_name);
            if (!frame_loc_opt) {
                // Column is not selected in the output frame.
                data += encoding_size::compressed_size(encoded_field_info.ndarray());
                continue;
            }

            auto dst_col = frame_loc_opt.value();
            auto& buffer = frame.column(static_cast<position_t>(dst_col)).data().buffer();
            if (ColumnMapping m{frame, dst_col, field_col, context};
                !trivially_compatible_types(m.source_type_desc_, m.dest_type_desc_))
            {
                util::check(static_cast<bool>(has_valid_type_promotion(m.source_type_desc_, m.dest_type_desc_)),
                    "Can't promote type {} to type {} in field {}",
                    m.source_type_desc_,
                    m.dest_type_desc_,
                    m.frame_field_descriptor_.name());

                m.dest_type_desc_.visit_tag([&buffer, &m, &data, &encoded_field_info, &buffers](auto dest_desc_tag) {
                    using DestinationType = typename decltype(dest_desc_tag)::DataTypeTag::raw_type;
                    m.source_type_desc_.visit_tag(
                        [&buffer, &m, &data, &encoded_field_info, &buffers](auto src_desc_tag) {
...
public:
    NullValueReducer(Column& column, std::shared_ptr<PipelineContext>& context, SegmentInMemory frame)
        : column_(column),
          context_(context),
...
size_t get_max_string_size_in_column(ChunkedBuffer& src_buffer,
    ...
    const FieldDescriptor::Proto& frame_field,
    ...
    bool check_all)
{
    ...
    util::check(column_data != slice_map.columns_.end(),
        "Data for column {} was not generated in map",
        frame_field.name());
...
protected:
    ...
    std::shared_ptr<PipelineContext> context_;
    SegmentInMemory frame_;
    const FieldDescriptor::Proto frame_field_;
    size_t row_;
...
public:
    EmptyDynamicStringReducer(Column& column,
        SegmentInMemory frame,
        const FieldDescriptor::Proto& frame_field,
        size_t alloc_width,
...
public:
    FixedStringReducer(Column& column,
        std::shared_ptr<PipelineContext>& context,
        SegmentInMemory frame,
        const FieldDescriptor::Proto& frame_field,
        size_t alloc_width)
    ...
    void reduce(PipelineContextRow& context_row, size_t) override
    {
        ...
        for (; row_ < end; ++row_) {
            ...
            util::variant_match(
                ...
                [&](StringPool::offset_t) { memset(dst_, 0, column_width_); });
            dst_ += column_width_;
        }
    }
};
...
public:
    UnicodeConvertingStringReducer(Column& column,
        ...
        const FieldDescriptor::Proto& frame_field,
        size_t alloc_width)
        : StringReducer(column,
              std::move(context),
              std::move(frame),
              std::move(frame_field),
...
class DynamicStringReducer : public StringReducer {
    ...
    struct UnicodeFromUnicodeCreator {
        static PyObject* create(std::string_view sv, bool)
        {
            const auto actual_length =
                std::min(sv.size() / UNICODE_WIDTH, wcslen(reinterpret_cast<const wchar_t*>(sv.data())));
    ...
    void assign_strings_shared(size_t end,
    ...
    template<typename StringCreator, typename LockPolicy>
    void assign_strings_local(size_t end,
        const StringPool::offset_t* ptr_src,
        bool has_type_conversion,
        const StringPool& string_pool)
    {
    ...
    inline void process_string_views(bool has_type_conversion,
        bool is_utf,
        size_t end,
        const StringPool::offset_t* ptr_src,
        const StringPool& string_pool)
    {
        ...
        case PyStringConstructor::Unicode_FromUnicode:
            if (unique_string_map_)
                assign_strings_shared<UnicodeFromUnicodeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            else
                assign_strings_local<UnicodeFromUnicodeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            break;
        case PyStringConstructor::Unicode_FromStringAndSize:
            if (unique_string_map_)
                assign_strings_shared<UnicodeFromStringAndSizeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            else
                assign_strings_local<UnicodeFromStringAndSizeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            break;
        case PyStringConstructor::Bytes_FromStringAndSize:
            if (unique_string_map_)
                assign_strings_shared<BytesFromStringAndSizeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            else
                assign_strings_local<BytesFromStringAndSizeCreator, LockPolicy>(end,
                    ptr_src,
                    has_type_conversion,
                    string_pool);
            break;
...
public:
    DynamicStringReducer(Column& column,
        std::shared_ptr<PipelineContext>& context,
        SegmentInMemory frame,
        FieldDescriptor::Proto frame_field,
    ...
    void reduce(PipelineContextRow& context_row, size_t column_index) override
    {
        ...
        google::protobuf::util::MessageDifferencer diff;
        auto has_type_conversion = !diff.Compare(frame_field_.type_desc(), segment_field.type_desc());
        util::check(!has_type_conversion ||
                        trivially_compatible_types(frame_field_.type_desc(), segment_field.type_desc()),
            "Cannot convert from type {} to {} in frame field",
            frame_field_.type_desc(),
            segment_field.type_desc());

        auto is_utf = is_utf_type(slice_value_type(data_type_from_proto(frame_field_.type_desc())));
        size_t end = context_row.slice_and_key().slice_.row_range.second - frame_.offset();
...
std::unique_ptr<StringReducer> get_string_reducer(Column& column,
    std::shared_ptr<PipelineContext>& context,
    SegmentInMemory frame,
    const FieldDescriptor::Proto& frame_field,
    const FrameSliceMap& slice_map,
    ...
    bool do_lock)
{
    const auto& field_type = data_type_from_proto(frame_field.type_desc());
    ...
    if (is_fixed_string_type(field_type)) {
        if (was_coerced_from_dynamic_to_fixed(field_type, column)) {
            ...
            string_reducer =
                std::make_unique<UnicodeConvertingStringReducer>(column, context, frame, frame_field, alloc_width);
        } else {
            const auto alloc_width =
        ...
        string_reducer = std::make_unique<DynamicStringReducer>(column,
            context,
            frame,
            frame_field,
            unique_string_map,
...
struct ReduceColumnTask : async::BaseTask {
    ...
    ReduceColumnTask(const SegmentInMemory& frame,
        size_t c,
    ...
    folly::Unit operator()()
    {
        ...
        if (dynamic_schema_ && column_data == slice_map_->columns_.end()) {
            ...
            if (is_sequence_type(field_type)) {
                auto string_reducer = get_string_reducer(column,
                    context_,
                    frame_,
                    frame_field,
                    *slice_map_,
    ...
    const ReadOptions& read_options)
{
    ...
    auto spinlock = std::make_shared<LockType>();
    std::shared_ptr<UniqueStringMapType> unique_string_map;
    std::shared_ptr<PyObject> py_nan;
    ...
    if (parallel_strings) {
        ...
        const auto batch_size = ConfigsMap::instance()->get_int("StringAllocation.BatchSize", 50);
        for (size_t c = 0; c < static_cast<size_t>(frame.descriptor().fields().size()); ++c) {
            jobs.emplace_back(async::submit_cpu_task(ReduceColumnTask(frame,
                c,
                slice_map,
                context,
                unique_string_map,
