...
void mark_index_slices(const std::shared_ptr<PipelineContext>& context, bool dynamic_schema, bool column_groups) {
    context->fetch_index_ =
        check_and_mark_slices(
            context->slice_and_keys_, dynamic_schema, true, context->incompletes_after_, column_groups
        )
            .value();
}
...
SegmentInMemory allocate_frame(const std::shared_ptr<PipelineContext>& context) {
    ...
    SegmentInMemory output{
        get_filtered_descriptor(context),
        row_count,
...
void decode_or_expand(
    ...
    if (auto handler = get_type_handler(m.source_type_desc_, m.dest_type_desc_); handler) {
        handler->handle_type(
            data, dest_buffer, encoded_field_info, m, shared_data, handler_data, encoding_version, string_pool
        );
    } else {
...
size_t get_field_range_compressed_size(
    ...
    for (auto i = start_idx; i < start_idx + num_fields; ++i) {
        ...
        ARCTICDB_DEBUG(
            log::version(), "Adding {}", encoding_sizes::ndarray_field_compressed_size(field.ndarray()) + magic_num_size
        );
...
void decode_into_frame_static(
    ...
    if (data != end || !fields.empty()) {
        ...
        while (it.has_next()) {
            ...
            decode_or_expand(
                data, buffer, encoded_field, shared_data, handler_data, encoding_version, m, context.string_pool_ptr()
            );
                ...
                m.dest_type_desc_.visit_tag([&buffer, &m, shared_data](auto dest_desc_tag) {
                    ...
                    m.source_type_desc_.visit_tag([&buffer, &m](auto src_desc_tag) {
                        ...
                        if constexpr (std::is_arithmetic_v<SourceType> && std::is_arithmetic_v<DestinationType>) {
                            // If the source and destination types are different, then sizeof(destination type) >=
                            // sizeof(source type) We have decoded the column of source type directly onto the output
                            // buffer above We therefore need to iterate backwards through the source values, static
                            // casting them to the destination type to avoid overriding values we haven't cast yet.
                            const auto src_ptr_offset =
                                data_type_size(m.source_type_desc_, DataTypeMode::INTERNAL) * (m.num_rows_ - 1);
                            const auto dest_ptr_offset =
                                data_type_size(m.dest_type_desc_, DataTypeMode::INTERNAL) * (m.num_rows_ - 1);
                            auto src_ptr =
                                reinterpret_cast<SourceType*>(buffer.data() + m.offset_bytes_ + src_ptr_offset);
                            auto dest_ptr =
                                reinterpret_cast<DestinationType*>(buffer.data() + m.offset_bytes_ + dest_ptr_offset);
    ...
    void reduce(PipelineContextRow& context_row) {
        ...
        if (current_pos != pos_) {
            ...
            if (const std::shared_ptr<TypeHandler>& handler =
                    arcticdb::TypeHandlerRegistry::instance()->get_handler(column_.type());
                handler) {
...
struct ReduceColumnTask : async::BaseTask {
    ...
    folly::Unit operator()() {
        ...
        if (dynamic_schema_ && column_data == slice_map_->columns_.end()) {
            if (const std::shared_ptr<TypeHandler>& handler =
                    arcticdb::TypeHandlerRegistry::instance()->get_handler(column.type());
                handler) {
                handler->default_initialize(
        ...
        } else if (!dynamic_schema_ && column_data == slice_map_->columns_.end() &&
...
void reduce_and_fix_columns(
    ...
    folly::collect(folly::window(
                       frame.descriptor().fields().size(),
                       [&](size_t field) {
                           return async::submit_cpu_task(ReduceColumnTask(
                               frame, field, slice_map, context, shared_data, handler_data, dynamic_schema
                           ));
                       },
                       batch_size
                   ))
    ...
    context->ensure_vectors();
    {
        ...
        for (auto& row : *context) {
            keys_and_continuations.emplace_back(
                ...
                [row = row, frame = frame, dynamic_schema = dynamic_schema, shared_data, &handler_data](auto&& ks
                ) mutable {
