...
/*
   _  _  _
  |i||_||_|    We mark the first slice of each row of the grid as requiring a
  fetch_index |i||_||_|    This helps us calculating the row_count during allocation.
  Note that all slices |i||_||_|    contain the relevant index, but we only want to
  count it once per grid row.

 */
void mark_index_slices(const std::shared_ptr<PipelineContext>& context,
                       bool dynamic_schema, bool column_groups) {
  context->fetch_index_ =
      check_and_mark_slices(context->slice_and_keys_, dynamic_schema, true,
                            context->incompletes_after_, column_groups)
          .value();
}

StreamDescriptor
get_filtered_descriptor(StreamDescriptor&& descriptor,
  ...
  return util::variant_match(index, [&desc, &filter_columns](const auto& idx) {
    const std::shared_ptr<FieldCollection>& fields =
        filter_columns ? filter_columns : desc.fields_ptr();
    return StreamDescriptor{index_descriptor_from_range(desc.id(), idx, *fields)};
  });
}

StreamDescriptor
get_filtered_descriptor(const std::shared_ptr<PipelineContext>& context) {
  return get_filtered_descriptor(context->descriptor().clone(),
                                 context->filter_columns_);
}
...
SegmentInMemory allocate_frame(const std::shared_ptr<PipelineContext>& context) {
  ...
  ARCTICDB_DEBUG(log::version(), "Allocated frame with offset {} and row count {}",
                 offset, row_count);
...
const uint8_t* skip_heading_fields(const SegmentHeader& hdr, const uint8_t*& data) {
  ...
  if (hdr.has_metadata_field()) {
    auto metadata_size =
        encoding_sizes::ndarray_field_compressed_size(hdr.metadata_field().ndarray());
  ...
  if (hdr.has_descriptor_field()) {
    auto descriptor_field_size =
        encoding_sizes::ndarray_field_compressed_size(hdr.descriptor_field().ndarray());
    ARCTICDB_DEBUG(log::version(), "Skipping {} bytes of descriptor",
                   descriptor_field_size);
  ...
  if (hdr.has_index_descriptor_field()) {
    auto index_fields_size = encoding_sizes::ndarray_field_compressed_size(
        hdr.index_descriptor_field().ndarray());
    ARCTICDB_DEBUG(log::version(), "Skipping {} bytes of index descriptor",
                   index_fields_size);
...
void decode_string_pool(const SegmentHeader& hdr, const uint8_t*& data,
                        const uint8_t* begin ARCTICDB_UNUSED, const uint8_t* end,
  ...
  if (hdr.has_string_pool_field()) {
    ...
    util::check(data != end,
                "Reached end of input block with string pool fields to decode");
    context.allocate_string_pool();
    std::optional<util::BitMagic> bv;

    // Note that this will decode the entire string pool into a ChunkedBuffer with
    // exactly 1 chunk
    if (EncodingVersion(hdr.encoding_version()) == EncodingVersion::V2)
      util::check_magic<StringPoolMagic>(data);

    util::check(hdr.string_pool_field().has_ndarray(),
                "Expected string pool field to be ndarray");
    data += decode_ndarray(string_pool_descriptor().type(),
                           hdr.string_pool_field().ndarray(), data,
                           context.string_pool(), bv, hdr.encoding_version());

    ARCTICDB_TRACE(log::codec(), "Decoded string pool to position {}", data - begin);
  }
}

void decode_index_field(SegmentInMemory& frame, const EncodedFieldImpl& field,
                        const uint8_t*& data, const uint8_t* begin ARCTICDB_UNUSED,
                        const uint8_t* end ARCTICDB_UNUSED, PipelineContextRow& context,
  ...
  if (get_index_field_count(frame)) {
    if (!context.fetch_index()) {
      ...
      ARCTICDB_DEBUG(log::storage(),
                     "Creating index slice with total size {} ({} - {})", tot_size, sz,
                     slice_and_key.slice_.row_range.diff());

      const auto fields_match =
          frame_field_descriptor.type() == context.descriptor().fields(0).type();
      ...
      ARCTICDB_DEBUG(log::codec(), "Decoded index column {} to position {}", 0,
                     data - begin);
    }
  }
}

void decode_or_expand(const uint8_t*& data, uint8_t* dest,
                      const EncodedFieldImpl& encoded_field_info, size_t dest_bytes,
                      std::shared_ptr<BufferHolder> buffers,
                      EncodingVersion encoding_version, const ColumnMapping& m) {
  if (auto handler = TypeHandlerRegistry::instance()->get_handler(m.source_type_desc_);
      handler) {
    handler->handle_type(data, dest, encoded_field_info, m, dest_bytes,
                         std::move(buffers), encoding_version);
  } else {
    std::optional<util::BitMagic> bv;
    if (encoded_field_info.has_ndarray() &&
        encoded_field_info.ndarray().sparse_map_bytes() > 0) {
      ...
      data += decode_field(m.source_type_desc_, encoded_field_info, data, sparse_sink,
                           bv, encoding_version);
      m.source_type_desc_.visit_tag([dest, dest_bytes, &bv, &sparse](const auto tdt) {
        ...
        util::expand_dense_buffer_using_bitmap<RawType>(bv.value(), sparse.data(),
                                                        dest);
      });
    } else {
      SliceDataSink sink(dest, dest_bytes);
      const auto& ndarray = encoded_field_info.ndarray();
      if (const auto bytes = encoding_sizes::data_uncompressed_size(ndarray);
          bytes < dest_bytes) {
      ...
      data += decode_field(m.source_type_desc_, encoded_field_info, data, sink, bv,
                           encoding_version);
    }
  }
}
...
size_t get_field_range_compressed_size(size_t start_idx, size_t num_fields,
  ...
  const size_t magic_num_size =
      EncodingVersion(hdr.encoding_version()) == EncodingVersion::V2
          ? sizeof(ColumnMagic)
          : 0u;
  ARCTICDB_DEBUG(log::version(), "Skipping between {} and {}", start_idx,
                 start_idx + num_fields);
  for (auto i = start_idx; i < start_idx + num_fields; ++i) {
    const auto& field = fields.at(i);
    ARCTICDB_DEBUG(log::version(), "Adding {}",
                   encoding_sizes::ndarray_field_compressed_size(field.ndarray()) +
                       magic_num_size);
    total +=
        encoding_sizes::ndarray_field_compressed_size(field.ndarray()) + magic_num_size;
  }
  ARCTICDB_DEBUG(log::version(), "Fields {} to {} contain {} bytes", start_idx,
                 start_idx + num_fields, total);
  return total;
}

void advance_field_size(const EncodedFieldImpl& field, const uint8_t*& data,
...
void advance_skipped_cols(const uint8_t*& data, ssize_t prev_col_offset,
                          size_t source_col, size_t first_col_offset,
                          size_t index_fieldcount, const EncodedFieldCollection& fields,
                          const SegmentHeader& hdr) {
  ...
  if (skipped_cols) {
    const auto bytes_to_skip = get_field_range_compressed_size(
        (next_col - first_col_offset) + index_fieldcount, skipped_cols, hdr, fields);
    data += bytes_to_skip;
  }
}
...
void decode_into_frame_static(SegmentInMemory& frame, PipelineContextRow& context,
                              Segment&& s,
  ...
  // data == end in case we have empty data types (e.g. {EMPTYVAL, Dim0}, {EMPTYVAL,
  // Dim1}) for which we store nothing in storage as they can be reconstructed in the
  // type handler on the read path.
  if (data != end || !fields.empty()) {
    ...
    while (it.has_next()) {
      advance_skipped_cols(data, static_cast<ssize_t>(it.prev_col_offset()),
                           it.source_col(), it.first_slice_col_offset(),
                           index_fieldcount, fields, hdr);
      if (has_magic_nums)
      ...
      util::check(it.source_field_pos() < size_t(fields.size()),
                  "Field index out of range: {} !< {}", it.source_field_pos(),
                  fields.size());
      ...
      const bool types_trivially_compatible =
          trivially_compatible_types(m.source_type_desc_, m.dest_type_desc_);
      const bool any_type_is_empty = is_empty_type(m.source_type_desc_.data_type()) ||
                                     is_empty_type(m.dest_type_desc_.data_type());
      util::check(types_trivially_compatible || any_type_is_empty,
                  "Column type conversion from {} to {} not implemented in column "
                  "{}:{} -> {}:{}",
                  m.source_type_desc_, m.dest_type_desc_, it.source_col(), field_name,
                  it.dest_col(), m.frame_field_descriptor_.name());
      util::check(data != end || remaining_fields_empty(it, context),
                  "Reached end of input block with {} fields to decode",
                  it.remaining_fields());
      decode_or_expand(data, buffer.data() + m.offset_bytes_, encoded_field,
                       m.dest_bytes_, buffers, encoding_version, m);
      ARCTICDB_TRACE(log::codec(), "Decoded column {} to position {}", field_name,
                     data - begin);
      ...
      if (it.at_end_of_selected()) {
        advance_skipped_cols(data, static_cast<ssize_t>(it.prev_col_offset()),
                             it.last_slice_col_offset(), it.first_slice_col_offset(),
                             it.index_fieldcount(), fields, hdr);
        break;
      } else {
        if (has_magic_nums)
          util::check_magic_in_place<ColumnMagic>(data);
...
void decode_into_frame_dynamic(SegmentInMemory& frame, PipelineContextRow& context,
                               Segment&& s,
                               const std::shared_ptr<BufferHolder>& buffers) {
  ARCTICDB_SAMPLE_DEFAULT(DecodeIntoFrame)
  auto seg = std::move(s);
  ...
  if (!hdr.body_fields().empty()) {
    ...
    auto field_count =
        context.slice_and_key().slice_.col_range.diff() + index_fieldcount;
    for (auto field_col = index_fieldcount; field_col < field_count; ++field_col) {
      ...
      util::check(static_cast<bool>(
                      has_valid_type_promotion(m.source_type_desc_, m.dest_type_desc_)),
                  "Can't promote type {} to type {} in field {}", m.source_type_desc_,
                  m.dest_type_desc_, m.frame_field_descriptor_.name());
      ARCTICDB_TRACE(log::storage(),
                     "Creating data slice at {} with total size {} ({} rows)",
                     m.offset_bytes_, m.dest_bytes_,
                     context.slice_and_key().slice_.row_range.diff());
      const bool source_is_empty = is_empty_type(m.source_type_desc_.data_type());
      util::check(data != end || source_is_empty,
                  "Reached end of input block with {} fields to decode",
      ...
      if (!trivially_compatible_types(m.source_type_desc_, m.dest_type_desc_) &&
        ...
        m.dest_type_desc_.visit_tag([&buffer, &m, buffers](auto dest_desc_tag) {
          using DestinationType =
              typename decltype(dest_desc_tag)::DataTypeTag::raw_type;
          m.source_type_desc_.visit_tag([&buffer, &m](auto src_desc_tag) {
            ...
            if constexpr (std::is_arithmetic_v<SourceType> &&
              ...
              // If the source and destination types are different, then
              // sizeof(destination type) >= sizeof(source type) We have decoded the
              // column of source type directly onto the output buffer above We
              // therefore need to iterate backwards through the source values, static
              // casting them to the destination type to avoid overriding values we
              // haven't casted yet.
              const auto src_ptr_offset =
                  sizeof_datatype(m.source_type_desc_) * (m.num_rows_ - 1);
              const auto dest_ptr_offset =
                  sizeof_datatype(m.dest_type_desc_) * (m.num_rows_ - 1);
              auto src_ptr = reinterpret_cast<SourceType*>(
                  buffer.data() + m.offset_bytes_ + src_ptr_offset);
              auto dest_ptr = reinterpret_cast<DestinationType*>(
                  buffer.data() + m.offset_bytes_ + dest_ptr_offset);
              for (auto i = 0u; i < m.num_rows_; ++i) {
                *dest_ptr-- = static_cast<DestinationType>(*src_ptr--);
              }
            } else {
              util::raise_rte("Can't promote type {} to type {} in field {}",
                              m.source_type_desc_, m.dest_type_desc_,
                              m.frame_field_descriptor_.name());
            }
          });
        });
      }
      ARCTICDB_TRACE(log::codec(), "Decoded column {} to position {}",
                     frame.field(dst_col).name(), data - begin);
    }
...
/*
 * For message data written with append_incomplete we might have a column missing in a
 * given slice, this code block takes a column and the final allocated buffer in the
 * frame, and zeroes out the memory area corresponding to the appropriate slice if the
 * field is missing in that slice.
 */
...
public:
  NullValueReducer(Column& column, std::shared_ptr<PipelineContext>& context,
                   SegmentInMemory frame)
      : column_(column), context_(context), frame_(std::move(frame)),
        pos_(frame_.offset()) {}
...
size_t get_max_string_size_in_column(ChunkedBuffer& src_buffer,
                                     std::shared_ptr<PipelineContext>& context,
                                     SegmentInMemory& frame, const Field& frame_field,
                                     const FrameSliceMap& slice_map, bool check_all) {
  const auto column_data = slice_map.columns_.find(frame_field.name());
  util::check(column_data != slice_map.columns_.end(),
              "Data for column {} was not generated in map", frame_field.name());
...
public:
  StringReducer(Column& column, std::shared_ptr<PipelineContext> context,
                SegmentInMemory frame, const Field& frame_field, size_t alloc_width)
      : column_(column), context_(std::move(context)), frame_(std::move(frame)),
        frame_field_(frame_field), row_(0), src_buffer_(column.data().buffer()),
...
public:
  EmptyDynamicStringReducer(Column& column, SegmentInMemory frame, const Field&,
                            size_t alloc_width, std::shared_ptr<LockType> spinlock)
      : column_(column), frame_(std::move(frame)), row_(0),
        src_buffer_(column.data().buffer()), column_width_(alloc_width),
        dest_buffer_(ChunkedBuffer::presized(frame_.row_count() * column_width_)),
        dst_(dest_buffer_.data()), ptr_dest_(reinterpret_cast<PyObject**>(dst_)),
...
public:
  FixedStringReducer(Column& column, std::shared_ptr<PipelineContext>& context,
                     SegmentInMemory frame, const Field& frame_field,
                     size_t alloc_width)
  ...
  void reduce(PipelineContextRow& context_row, size_t) override {
    ...
    for (; row_ < end; ++row_) {
      ...
      util::variant_match(
          val, [&](std::string_view sv) { std::memcpy(dst_, sv.data(), sv.size()); },
...
public:
  UnicodeConvertingStringReducer(Column& column,
                                 std::shared_ptr<PipelineContext> context,
                                 SegmentInMemory frame, const Field& frame_field,
                                 size_t alloc_width)
      : StringReducer(column, std::move(context), std::move(frame), frame_field,
                      alloc_width * UNICODE_WIDTH),
        conv_("UTF32", "UTF8"), buf_(new uint8_t[column_width_ + UNICODE_PREFIX]) {}
...
inline PyStringConstructor get_string_constructor(bool has_type_conversion,
                                                  bool is_utf) {
  if (is_utf) {
...
class DynamicStringReducer : public StringReducer {
  ...
  struct UnicodeFromUnicodeCreator {
    static PyObject* create(std::string_view sv, bool) {
      ...
      const auto actual_length = std::min(
          sv.size() / UNICODE_WIDTH, wcslen(reinterpret_cast<const wchar_t*>(buffer)));
      return PyUnicode_FromKindAndData(PyUnicode_4BYTE_KIND,
                                       reinterpret_cast<const UnicodeType*>(sv.data()),
                                       actual_length);
    }
  };
  ...
  struct BytesFromStringAndSizeCreator {
    static PyObject* create(std::string_view sv, bool has_type_conversion) {
      const auto actual_length =
          has_type_conversion ? std::min(sv.size(), strlen(sv.data())) : sv.size();
      return PYBIND11_BYTES_FROM_STRING_AND_SIZE(sv.data(), actual_length);
    }
  };

  template <typename StringCreator, typename LockPolicy>
  void assign_strings_shared(size_t end, const entity::position_t* ptr_src,
                             bool has_type_conversion, const StringPool& string_pool) {
  ...
  template <typename StringCreator, typename LockPolicy>
  void assign_strings_local(size_t end, const entity::position_t* ptr_src,
                            bool has_type_conversion, const StringPool& string_pool) {
    LockPolicy::lock(*lock_);
    auto none = std::make_unique<py::none>(py::none{});
    LockPolicy::unlock(*lock_);
    size_t none_count = 0u;
    ankerl::unordered_dense::map<entity::position_t,
                                 std::pair<PyObject*, std::unique_ptr<std::mutex>>>
        local_map;
        ...
        *ptr_dest_ = py_nan_.get();
        ...
        if (it != local_map.end()) {
          ...
          *ptr_dest_ = StringCreator::create(sv, has_type_conversion);
          ...
          local_map.insert(std::make_pair(
              std::move(offset),
              std::make_pair(std::move(dest), std::make_unique<std::mutex>())));
        }
      }
    }
  ...
  inline void process_string_views(bool has_type_conversion, bool is_utf, size_t end,
    ...
    case PyStringConstructor::Unicode_FromUnicode:
      if (unique_string_map_)
        assign_strings_shared<UnicodeFromUnicodeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      else
        assign_strings_local<UnicodeFromUnicodeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      break;
    case PyStringConstructor::Unicode_FromStringAndSize:
      if (unique_string_map_)
        assign_strings_shared<UnicodeFromStringAndSizeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      else
        assign_strings_local<UnicodeFromStringAndSizeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      break;
    case PyStringConstructor::Bytes_FromStringAndSize:
      if (unique_string_map_)
        assign_strings_shared<BytesFromStringAndSizeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      else
        assign_strings_local<BytesFromStringAndSizeCreator, LockPolicy>(
            end, ptr_src, has_type_conversion, string_pool);
      break;
    }
  }
...
public:
  DynamicStringReducer(Column& column, std::shared_ptr<PipelineContext>& context,
                       SegmentInMemory frame, const Field& frame_field,
                       std::shared_ptr<UniqueStringMapType> unique_string_map,
                       std::shared_ptr<PyObject> py_nan, std::shared_ptr<LockType> lock,
                       bool do_lock)
      : StringReducer(column, context, std::move(frame), frame_field,
                      sizeof(entity::position_t)),
        ptr_dest_(reinterpret_cast<PyObject**>(dst_)),
        unique_string_map_(std::move(unique_string_map)), py_nan_(py_nan),
        lock_(std::move(lock)), do_lock_(do_lock) {
    if (!py_nan_)
      py_nan_ = std::shared_ptr<PyObject>(create_py_nan(lock_),
                                          [lock = lock_](PyObject* py_obj) {
                                            lock->lock();
                                            Py_DECREF(py_obj);
                                            lock->unlock();
                                          });
  ...
  struct LockActive {
    static void lock(LockType& lock) { lock.lock(); }

    static void unlock(LockType& lock) { lock.unlock(); }
  };
  ...
  void reduce(PipelineContextRow& context_row, size_t column_index) override {
    ...
    const bool trivially_compatible =
        trivially_compatible_types(frame_field_.type(), segment_field.type());
    // In case the segment type is EMPTYVAL the empty type handler should have ran and
    // set all missing values to not_a_string()
    util::check(trivially_compatible || is_empty_type(segment_field.type().data_type()),
                "String types are not trivially compatible. Cannot convert from type "
                "{} to {} in frame field.",
                frame_field_.type(), segment_field.type());
    ...
    auto ptr_src = get_offset_ptr_at(row_, src_buffer_);
    if (do_lock_)
      process_string_views<LockActive>(is_type_different, is_utf, end, ptr_src,
                                       context_row.string_pool());
    else
      process_string_views<LockDisabled>(is_type_different, is_utf, end, ptr_src,
                                         context_row.string_pool());
  }
...
get_string_reducer(Column& column, std::shared_ptr<PipelineContext>& context,
                   SegmentInMemory frame, const Field& frame_field,
                   const FrameSliceMap& slice_map,
                   std::shared_ptr<UniqueStringMapType>& unique_string_map,
                   std::shared_ptr<PyObject> py_nan,
                   std::shared_ptr<LockType>& spinlock, bool do_lock) {
  ...
  if (is_fixed_string_type(field_type)) {
    if (was_coerced_from_dynamic_to_fixed(field_type, column)) {
      const auto alloc_width = get_max_string_size_in_column(
          column.data().buffer(), context, frame, frame_field, slice_map, true);
      string_reducer = std::make_unique<UnicodeConvertingStringReducer>(
          column, context, frame, frame_field, alloc_width);
    } else {
      const auto alloc_width = get_max_string_size_in_column(
          column.data().buffer(), context, frame, frame_field, slice_map, false);
      string_reducer = std::make_unique<FixedStringReducer>(column, context, frame,
                                                            frame_field, alloc_width);
    }
  } else {
    string_reducer = std::make_unique<DynamicStringReducer>(
        column, context, frame, frame_field, unique_string_map, py_nan, spinlock,
        do_lock);
  }
...
struct ReduceColumnTask : async::BaseTask {
  ...
  ReduceColumnTask(const SegmentInMemory& frame, size_t c,
                   std::shared_ptr<FrameSliceMap> slice_map,
                   std::shared_ptr<PipelineContext>& context,
                   std::shared_ptr<UniqueStringMapType> unique_string_map,
                   std::shared_ptr<PyObject> py_nan, std::shared_ptr<LockType> lock,
                   bool dynamic_schema, bool do_lock)
      : frame_(frame), column_index_(c), slice_map_(std::move(slice_map)),
        context_(context), unique_string_map_(std::move(unique_string_map)),
        py_nan_(py_nan), lock_(std::move(lock)), dynamic_schema_(dynamic_schema),
        do_lock_(do_lock) {}
  ...
  folly::Unit operator()() {
    ...
    if (dynamic_schema_ && column_data == slice_map_->columns_.end()) {
      ...
      if (dynamic_type) {
        EmptyDynamicStringReducer reducer(column, frame_, frame_field,
                                          sizeof(entity::position_t), lock_);
      ...
      if (is_sequence_type(field_type)) {
        auto string_reducer =
            get_string_reducer(column, context_, frame_, frame_field, *slice_map_,
                               unique_string_map_, py_nan_, lock_, do_lock_);
        for (const auto& row : column_data->second) {
          PipelineContextRow context_row{context_, row.second.context_index_};
...
void reduce_and_fix_columns(std::shared_ptr<PipelineContext>& context,
                            SegmentInMemory& frame, const ReadOptions& read_options) {
  ...
  if (opt_false(read_options.optimise_string_memory_)) {
    ...
    py_nan = std::shared_ptr<PyObject>(create_py_nan(spinlock),
                                       [lock = spinlock](PyObject* py_obj) {
                                         lock->lock();
                                         Py_DECREF(py_obj);
                                         lock->unlock();
                                       });
  ...
  if (parallel_strings) {
    ...
    static const auto batch_size =
        ConfigsMap::instance()->get_int("StringAllocation.BatchSize", 50);
    for (size_t c = 0; c < static_cast<size_t>(frame.descriptor().fields().size());
         ++c) {
      jobs.emplace_back(async::submit_cpu_task(
          ReduceColumnTask(frame, c, slice_map, context, unique_string_map, py_nan,
                           spinlock, dynamic_schema, true)));
...
fetch_data(const SegmentInMemory& frame,
           const std::shared_ptr<PipelineContext>& context,
           const std::shared_ptr<stream::StreamSource>& ssource, bool dynamic_schema,
           std::shared_ptr<BufferHolder> buffers) {
  ...
  context->ensure_vectors();
  {
    ...
    for (auto& row : *context) {
      keys_and_continuations.emplace_back(
          ...
          [row = row, frame = frame, dynamic_schema = dynamic_schema,
           buffers](auto&& ks) mutable {
            auto key_seg = std::forward<storage::KeySegmentPair>(ks);
            if (dynamic_schema)
              decode_into_frame_dynamic(frame, row, std::move(key_seg.segment()),
                                        buffers);
            else
  ...
  return ssource->batch_read_compressed(std::move(keys_and_continuations),
                                        BatchReadArgs{});
}
