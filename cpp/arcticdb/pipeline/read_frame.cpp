...
void decode_into_frame_dynamic(
    ...
    if (!hdr.body_fields().empty()) {
        // TODO: What about row range truncation if no fields in segment. Add test.
        auto string_pool_data = skip_to_string_pool(hdr, data);
        decode_string_pool(hdr, string_pool_data, begin, end, context);
...
class NullValueReducer {
    ...
    // Both positions are with respect to the global frame
  ...
  public:
    NullValueReducer(
        ...
        default_value_(default_value) {
            if (column_.block_offsets().empty()) {
                // It is possible after truncation to have a column with no blocks. In this case we want applying the NullValueReducer to be a no-op
                first_pos_ = frame_offset_;
                last_pos_ = frame_offset_;
            } else {
                // When the frame is truncated, we need to look at the block offsets to determine the actual first and last positions
