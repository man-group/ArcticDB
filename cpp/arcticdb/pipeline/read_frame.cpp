...
SegmentInMemory allocate_chunked_frame(
    ...
    for (auto i = 0u; i < desc.fields().size(); ++i) {
        const auto& field = desc.fields()[i];
        auto column = std::make_shared<Column>(
                field.type(), 0, AllocationType::DETACHABLE, Sparsity::NOT_PERMITTED, extra_bytes_per_column[i]
        );
        ...
        output.add_column(field.ref(), column);
    }
