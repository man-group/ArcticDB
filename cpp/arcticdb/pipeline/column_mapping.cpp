...
namespace arcticdb {
ColumnMapping::ColumnMapping(SegmentInMemory& frame, size_t dst_col, size_t field_col,
    ...
    : source_type_desc_(context.descriptor().fields(field_col).type()),
      ...
      offset_bytes_(dest_size_ * first_row_), dest_bytes_(dest_size_ * num_rows_) {}
...
StaticColumnMappingIterator::StaticColumnMappingIterator(
  ...
  if (bit_set_) {
    ...
    if ((*bit_set_)[bv_size(first_slice_col_offset_)]) {
      ...
      // We have to do this extra check in bitmagic, get_next returns 0 in case no next
      // present
  ...
  dst_col_ =
      bit_set_ ? bit_set_->count_range(0, bv_size(source_col_)) - 1 : source_col_;
