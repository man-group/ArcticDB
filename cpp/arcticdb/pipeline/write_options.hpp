...
struct WriteOptions {
  static WriteOptions
  from_proto(const arcticdb::proto::storage::VersionStoreConfig::WriteOptions& opt) {
    WriteOptions def;
    return {opt.dynamic_schema() && !opt.bucketize_dynamic()
                ? std::numeric_limits<size_t>::max()
                : (opt.column_group_size() > 0 ? size_t(opt.column_group_size())
                                               : def.column_group_size),
            opt.segment_row_size() > 0 ? size_t(opt.segment_row_size())
                                       : def.segment_row_size,
            ...
            opt.max_num_buckets() > 0 ? size_t(opt.max_num_buckets())
                                      : def.max_num_buckets};
  }
