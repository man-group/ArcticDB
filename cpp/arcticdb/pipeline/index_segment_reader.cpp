...
IndexSegmentReader get_index_reader(const AtomKey& prev_index,
...
IndexRange get_index_segment_range(const AtomKey& prev_index,
                                   const std::shared_ptr<Store>& store) {
  auto isr = get_index_reader(prev_index, store);
  return IndexRange{isr.begin()->key().start_index(), isr.last()->key().end_index()};
}

bool IndexSegmentReader::bucketize_dynamic() const { return tsd().column_groups(); }
...
size_t IndexSegmentReader::size() const { return seg_.row_count(); }
...
bool IndexSegmentReader::is_pickled() const {
  return tsd().proto().normalization().input_type_case() ==
...
void check_column_and_date_range_filterable(
  ...
  util::check(!index_segment_reader.is_pickled() ||
                  (!read_query.columns.has_value() &&
                   std::holds_alternative<std::monostate>(read_query.row_filter)),
              "The data for this symbol is pickled and does not support column stats, "
              "date_range, row_range, or column queries");
  ...
  sorting::check<ErrorCode::E_UNSORTED_DATA>(
      index_segment_reader.sorted() == SortedValue::UNKNOWN ||
          ...
          !std::holds_alternative<IndexRange>(read_query.row_filter),
      "When filtering data using date_range, the symbol must be sorted in ascending "
      "order. ArcticDB believes it is not sorted in ascending order and cannot "
      "therefore filter the data using date_range.");
}
