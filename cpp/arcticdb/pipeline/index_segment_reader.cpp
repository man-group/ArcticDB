...
const Column& IndexSegmentReader::column(Fields field) const { return seg_.column(position_t(field)); }
...
SliceAndKey IndexSegmentReader::row(std::size_t r) const {
    ...
    ColRange col_rg{
            column(index::Fields::start_col).scalar_at<std::size_t>(i).value(),
            column(index::Fields::end_col).scalar_at<std::size_t>(i).value()
    };
    RowRange row_rg{
            column(index::Fields::start_row).scalar_at<std::size_t>(i).value(),
...
IndexSegmentIterator IndexSegmentReader::begin() const { return IndexSegmentIterator(this); }

IndexSegmentIterator IndexSegmentReader::end() const { return {this, static_cast<int64_t>(size())}; }

IndexSegmentIterator IndexSegmentReader::last() const { return {this, static_cast<int64_t>(size() - 1)}; }
