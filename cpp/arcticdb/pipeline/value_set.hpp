...
public:
  ...
  template <typename T> std::shared_ptr<std::unordered_set<T>> get_set() {
    util::raise_rte("ValueSet::get_set called with unexpected template type");
  }

  std::shared_ptr<std::unordered_set<std::string>>
  get_fixed_width_string_set(size_t width);
...
private:
  ...
  template <typename T> class typed_set {
  public:
    ARCTICDB_VISIBILITY_HIDDEN std::shared_ptr<std::unordered_set<T>>
    create(py::array value_list) {
      std::call_once(flag_, [&] { set_ = create_internal(value_list); });
      return set_;
    }

    std::shared_ptr<std::unordered_set<T>>
    transform(const NumericSetType& numeric_base_set) {
  ...
  private:
    ...
    std::shared_ptr<std::unordered_set<T>>
    transform_internal(const NumericSetType& numeric_base_set) {
  ...
  std::unordered_map<size_t, std::shared_ptr<std::unordered_set<std::string>>>
      typed_set_fixed_width_strings_;
...
template <>
inline std::shared_ptr<std::unordered_set<std::string>>
ValueSet::get_set<std::string>() {
