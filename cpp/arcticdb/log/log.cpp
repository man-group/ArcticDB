...
struct Loggers::Impl {
  ...
  std::unique_ptr<spdlog::logger> unconfigured_ = std::make_unique<spdlog::logger>(
      "arcticdb", std::make_shared<spdlog::sinks::stderr_sink_mt>());
...
constexpr auto get_default_log_level() { return spdlog::level::info; }

spdlog::logger& storage() { return Loggers::instance().storage(); }

spdlog::logger& inmem() { return Loggers::instance().inmem(); }

spdlog::logger& codec() { return Loggers::instance().codec(); }

spdlog::logger& root() { return Loggers::instance().root(); }

spdlog::logger& memory() { return Loggers::instance().memory(); }

spdlog::logger& version() { return Loggers::instance().version(); }

spdlog::logger& timings() { return Loggers::instance().timings(); }

spdlog::logger& lock() { return Loggers::instance().lock(); }

spdlog::logger& schedule() { return Loggers::instance().schedule(); }

spdlog::logger& message() { return Loggers::instance().message(); }
...
Loggers::Loggers() : impl_(std::make_unique<Impl>()) {
...
spdlog::logger& Loggers::storage() { return impl_->logger_ref(impl_->storage_); }

spdlog::logger& Loggers::inmem() { return impl_->logger_ref(impl_->inmem_); }

spdlog::logger& Loggers::codec() { return impl_->logger_ref(impl_->codec_); }

spdlog::logger& Loggers::version() { return impl_->logger_ref(impl_->version_); }

spdlog::logger& Loggers::memory() { return impl_->logger_ref(impl_->memory_); }

spdlog::logger& Loggers::timings() { return impl_->logger_ref(impl_->timings_); }

spdlog::logger& Loggers::lock() { return impl_->logger_ref(impl_->lock_); }

spdlog::logger& Loggers::schedule() { return impl_->logger_ref(impl_->schedule_); }

spdlog::logger& Loggers::message() { return impl_->logger_ref(impl_->message_); }

spdlog::logger& Loggers::symbol() { return impl_->logger_ref(impl_->symbol_); }
...
void Loggers::init() { loggers_instance_ = std::make_shared<Loggers>(); }
...
bool Loggers::configure(const arcticdb::proto::logger::LoggersConfig& conf,
                        bool force) {
  ...
  for (auto&& [sink_id, sink_conf] : conf.sink_by_id()) {
      ...
      if (sink_conf.console().has_color()) {
        if (sink_conf.console().std_err()) {
          impl_->sink_by_id_.try_emplace(
              sink_id, std::make_shared<spdlog::sinks::stderr_color_sink_mt>());
        } else {
          impl_->sink_by_id_.try_emplace(
              sink_id, std::make_shared<spdlog::sinks::stdout_color_sink_mt>());
        }
      } else {
        if (sink_conf.console().std_err()) {
          impl_->sink_by_id_.try_emplace(
              sink_id, std::make_shared<spdlog::sinks::stderr_sink_mt>());
        } else {
          impl_->sink_by_id_.try_emplace(
              sink_id, std::make_shared<spdlog::sinks::stdout_sink_mt>());
        }
      }
      break;
    case SinkConf::kFile:
      impl_->sink_by_id_.try_emplace(
          sink_id, std::make_shared<spdlog::sinks::basic_file_sink_mt>(make_parent_dir(
                       sink_conf.file().path(), "./arcticdb.basic.log")));
      break;
    case SinkConf::kRotFile:
      impl_->sink_by_id_.try_emplace(
          ...
          std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
              ...
              util::as_opt(sink_conf.rot_file().max_size_bytes())
                  .value_or(64ULL * (1ULL << 20)),
    ...
    case SinkConf::kDailyFile:
      ...
      break;
    default:
      util::raise_rte("Unsupported sink_conf {}", sink_conf.DebugString());
    }
  }

  // Now associate loggers with sinks
  auto check_and_configure = [&](const std::string& name, const std::string& fallback,
                                 auto& logger) {
    ...
    if (auto it = logger_by_id.find(name); it != logger_by_id.end()) {
      ...
      if (fallback.empty())
        throw std::invalid_argument(
            fmt::format("missing conf for logger {} without fallback", name));
  ...
  if (auto flush_sec = util::as_opt(conf.flush_interval_seconds()).value_or(1);
      flush_sec != 0) {
...
void Loggers::Impl::configure_logger(const arcticdb::proto::logger::LoggerConfig& conf,
                                     const std::string& name,
                                     std::unique_ptr<spdlog::logger>& logger) {
  ...
  for (const auto& sink_id : conf.sink_ids()) {
    if (auto it = sink_by_id_.find(sink_id); it != sink_by_id_.end()) {
      ...
      throw std::invalid_argument(
          fmt::format("invalid sink_id {} for logger {}", sink_id, name));
    }
  }
  auto fq_name = fmt::format("arcticdb.{}", name);
  if (thread_pool_) {
    // async logger
    logger = std::make_unique<spdlog::async_logger>(
        fq_name, sink_ptrs.begin(), sink_ptrs.end(), thread_pool_,
        spdlog::async_overflow_policy::block);
  } else {
    logger =
        std::make_unique<spdlog::logger>(fq_name, sink_ptrs.begin(), sink_ptrs.end());
  }
