...
static void BM_arrow_string_handler(benchmark::State& state) {
    ...
    auto [dest_type_desc, extra_bytes_per_block] =
            handler.output_type_and_extra_bytes({DataType::UTF_DYNAMIC32, Dimension::Dim0}, "col", read_options);
    ...
    for (auto _ : state) {
        ...
        // TODO: Move the visit_scalar to the allocate_and_fill method.
        details::visit_scalar(dest_column.type(), [&](auto&& impl) {
            using TagType = std::decay_t<decltype(impl)>;
            using RawType = typename TagType::DataTypeTag::raw_type;
            allocate_and_fill_chunked_column<RawType>(dest_column, num_rows, num_rows);
        });
        state.ResumeTiming();
