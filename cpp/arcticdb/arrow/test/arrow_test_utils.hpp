...
sparrow::array create_array(const std::vector<T>& data) {
    if constexpr (std::is_same_v<T, bool>) {
        ...
        auto strings_buffer_size = std::accumulate(data.cbegin(), data.cend(), size_t(0), [](const size_t& accum, const std::string& str) {
        ...
        return sparrow::array{sparrow::string_array{std::move(sparrow_strings_buffer), std::move(sparrow_offsets_buffer)}};
    }
}
