    ...
    for (auto field : fields) {
        ...
        segment.add_column(
                ...
                std::make_shared<Column>(
                        new_field.type(), 0, AllocationType::DETACHABLE, Sparsity::NOT_PERMITTED, extra_bytes_per_block
                )
        );
    }
...
TEST(ArrowRead, ConvertSegmentMultipleStringColumns) {
    ...
    for (auto i = 0u; i < num_chunks; ++i) {
        ...
        EXPECT_EQ(columns[3].data_type(), sparrow::data_type::INT64); // The offset values are INT64s
        assert_arrow_string_array_as_expected(
