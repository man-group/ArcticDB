...
DataType arcticdb_type_from_arrow_type(sparrow::data_type arrow_type) {
    ...
    case sparrow::data_type::TIMESTAMP_SECONDS:
        return DataType::SECONDS_UTC64;
    case sparrow::data_type::TIMESTAMP_MILLISECONDS:
        return DataType::MILLISECONDS_UTC64;
    case sparrow::data_type::TIMESTAMP_MICROSECONDS:
        return DataType::MICROSECONDS_UTC64;
...
std::pair<SegmentInMemory, std::optional<size_t>> arrow_data_to_segment(
    ...
    for (; record_batch != record_batches.cend(); ++record_batch) {
        for (size_t idx = 0; idx < record_batch->nb_columns(); ++idx) {
            ...
            if (is_bool_type(data_types.at(idx))) {
                ...
                // multiple record batches. This approach is much simpler, and BM_packed_bits_to_buffer show a rate of
                // 10 billion rows per second handled in serial. If this proves to be a bottleneck in some situations,
    ...
    if (index_column_position.has_value()) {
        ...
        seg.add_column(
                scalar_field(data_types.at(idx), column_names[idx]),
                std::make_shared<Column>(
                        make_scalar_type(data_types.at(idx)),
                        Sparsity::NOT_PERMITTED,
                        std::move(chunked_buffers.at(idx))
                )
        );
    }
